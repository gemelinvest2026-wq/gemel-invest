...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  // Example: const GOFI_URL = '';


  // =========================
  // TalkMe Bridge (postMessage)
  // Allows the embedded TalkMe iframe to request customers/proposal data from GEMEL INVEST
  // =========================
  (function setupTalkMeBridge(){
    try{
      const TALKME_ORIGINS = [
      ...
      window.addEventListener('message', (event) => {
        ...
        const originOk = !event.origin || TALKME_ORIGINS.includes(event.origin);
        if(!originOk) return;
        ...
        if(data.type === 'TALKME_REQUEST_CUSTOMERS'){
          ...
          reply({ type:'TALKME_CUSTOMERS', customers, ts: Date.now() });
          return;
        }
        ...
        if(data.type === 'TALKME_REQUEST_PROPOSAL'){
          ...
          let prop = proposals.filter(p => String(p.customerId||'') === cid).slice(-1)[0] || null;

          if(!prop){
            const c = (Array.isArray(state.customers) ? state.customers : []).find(x => String(x.id||'') === cid);
            const tz = c ? String(c.tz||'') : '';
            if(tz){
              prop = proposals.filter(p => String(p.customerTz||p.tz||'') === tz).slice(-1)[0] || null;
            }
          }

          reply({ type:'TALKME_PROPOSAL', customerId: cid, proposal: prop, ts: Date.now() });
          return;
        }
      });
    }catch(_){}
