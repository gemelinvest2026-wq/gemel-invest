...
txt.textContent = "";
  ...
  const UsersUI = {
    ...
    async _saveFromModal(){
      ...
      this.render();
      setTimeout(() => {
        if(this._newPolicyAnimateId === p.id) this._newPolicyAnimateId = null;
      }, 520);
    },
...
// open health covers drawer (Health only)
    ...
    renderStep5(){
      ...
      const renderPolicyCard = (p, showCoupleBadge=false) => {
        ...
        const enterCls = (this._newPolicyAnimateId && this._newPolicyAnimateId === p.id) ? " is-entering" : "";
        return `<div class="lcPolCard${enterCls}" data-pol="${p.id}">
