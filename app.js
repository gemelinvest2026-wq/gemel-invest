...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  function updateSafeSystemTopIcon_(){
    const el = document.getElementById('safeSystemTopIcon');
    if(!el) return;
    const isOpen = !!(wizardOverlay && !wizardOverlay.classList.contains('hidden') && wizardOverlay.getAttribute('aria-hidden') !== 'true');
