...
(() => {
  ...
  const BUILD = "20260324-customer-edit-modal-1";
...
txt.textContent = "";
  ...
  const CustomerEditUI = {
    ...
    async save(){
      ...
      if(!result?.ok) console.warn("CUSTOMER_EDIT_SAVE_LOCAL_ONLY", rec.id);
    }
  };
...
// ---------- Step 6 ----------
    ...
    renderOperationalReport(payloadOverride){
      ...
      const getPolicyPledgeSummarySafe = (policy) => {
        ...
        if(!isLienType) return { active:false, status:'—', inline:'—', rows:[], html:'' };
        if(!policy?.pledge) return { active:false, status:'ללא שיעבוד', inline:'ללא שיעבוד', rows:[], html:'' };
        ...
        const inlineParts = rows.filter(([label]) => label !== 'סוג שיעבוד').map(([label, value]) => `${label}: ${value}`);
        const html = rows.length
          ? `<div class="lcReportPledgeBox"><div class="lcReportPledgeBox__title">שיעבוד פוליסה</div><div class="lcReportPledgeBox__grid">${rows.map(([label, value]) => `<div class="lcReportPledgeField"><span>${escapeHtml(label)}</span><strong>${escapeHtml(value)}</strong></div>`).join('')}</div></div>`
          : `<div class="lcReportPledgeBox"><div class="lcReportPledgeBox__title">שיעבוד פוליסה</div><div class="lcReportPledgeBox__empty">סומן שיעבוד פעיל אך לא הוזנו פרטים נוספים.</div></div>`;
        return {
          active:true,
          status:'יש שיעבוד',
          inline:inlineParts.length ? `יש שיעבוד · ${inlineParts.join(' · ')}` : 'יש שיעבוד',
          rows,
          html
        };
      };
      ...
      const newRows = newPolicies.map(p => {
        ...
        return [
          ...
          this.renderReportValue(pledge.inline)
        ];
      });
      ...
      const coverCards = newPolicies.map((p, idx) => {
        ...
        const policyMeta = [
          ...
          ['תחילת ביטוח', this.renderReportValue(p?.startDate)],
          ['סכום / פיצוי', this.renderReportValue(coverageValue)],
          ['שיעבוד', this.renderReportValue(pledge.status)]
        ];
        return `<article class="lcReportCoverCard">
          <div class="lcReportCoverCard__top">
            <div>
              ...
              <div class="lcReportCoverCard__sub">פוליסה ${idx + 1} · ${this.renderReportValue(getPolicyInsuredLabelSafe(p))}</div>
            </div>
            <span class="lcReportChip">${escapeHtml(pledge.status)}</span>
          </div>
          <div class="lcReportMetaGrid">${policyMeta.map(([label, value]) => `<div class="lcReportMetaCell"><span>${escapeHtml(label)}</span><strong>${value}</strong></div>`).join('')}</div>
      ...
      return `<div class="lcReportDoc lcReportDoc--clean">
        <section class="lcReportShowcase">
          <div class="lcReportShowcase__top">
            <div>
              ...
              <h2 class="lcReportShowcase__title">דוח תפעולי מסודר להמשך טיפול</h2>
              <div class="lcReportShowcase__sub">ריכוז נקי של כל נתוני ההקמה: פרטי לקוח, מבוטחים, פוליסות קיימות, פוליסות חדשות, שיעבודים, תשלום והצהרת בריאות.</div>
            </div>
        ...
        <section class="lcReportSection">
          ...
          <div class="lcReportSection__sub">כולל פרמיה לפני/אחרי הנחה, תחילת ביטוח, כיסוי / סכום ושיקוף שיעבוד כאשר קיים.</div>
          ${this.renderTable(['מבוטח','חברה','סוג ביטוח','פרמיה חודשית','דירוג הנחה','פרמיה אחרי הנחה','תאריך תחילה','כיסוי / סכום','שיעבוד'], newRows)}
        </section>

        <section class="lcReportSection">
          <div class="lcReportSection__title">פירוט פוליסות חדשות וכיסויים</div>
          <div class="lcReportSection__sub">כרטיס מסודר לכל פוליסה חדשה, כולל כיסויים ופרטי שיעבוד מלאים במידת הצורך.</div>
