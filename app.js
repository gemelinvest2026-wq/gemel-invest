  ...
  const Storage = {
    ...
    async sendAdminContact(payload){
      ...
      try {
        ...
        }, 12000);        const json = await res.json();
  ...
  const Auth = {
    ...
    async _submit(){
      try {
        ...
        // do not block login forever on slow / broken Sheets fetch
        try {
          await Promise.race([
            App._bootPromise,
        ...
        this._setError("אירעה שגיאה בכניסה למערכת. בדוק שם משתמש וסיסמה ונסה שוב.");
      }
    }
  };
...
const App = {
  ...
  function safeModuleInit(name, fn){
    try { fn(); }
    catch(err){ console.error("INIT_ERROR", name, err); }
  }

  // ---------- Start ----------
  safeModuleInit("UI", () => UI.init());
  safeModuleInit("Auth", () => Auth.init());
  safeModuleInit("ForgotPasswordUI", () => ForgotPasswordUI.init());
  safeModuleInit("CustomersUI", () => CustomersUI.init());
  safeModuleInit("ArchiveCustomerUI", () => ArchiveCustomerUI.init());
  safeModuleInit("Wizard", () => Wizard.init());
  App._bootPromise = App.boot().catch(err => {
