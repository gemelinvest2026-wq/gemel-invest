...
(() => {
  ...
  const BUILD = "20260327-phoenix-original-pdf-v2";
...
// ---------- Step 6 ----------
    ...
    paintPhoenixTemplatePage(ctx, pageIndex, pageWidth, pageHeight, fillCtx){
      ...
      if(pageIndex === 0){
        const c = [21.6, 90.0, 157.5, 225.5, 293.0, 360.9, 428.9, 553.1];
        drawCell(25, 66, 62, 20, fillCtx.proposalDateText, { fontSize:8.5, align:'center' });
        drawCell(116, 66, 62, 20, fillCtx.firstPolicyStartDateText, { fontSize:8.5, align:'center' });
        drawCell(c[6], 92, c[7]-c[6], 16, fillCtx.agentName, { fontSize:8.3 });
        drawCell(c[5], 92, c[6]-c[5], 16, fillCtx.phoenixAgentNumber, { fontSize:8.3, align:'center' });
        drawCell(c[2], 92, c[3]-c[2], 16, fillCtx.companyName, { fontSize:7.6, align:'center' });
        drawCell(c[5], 132, c[6]-c[5], 10, fillCtx.insureds?.[0]?.firstName || '', { fontSize:7.6, align:'center' });
        drawCell(c[4], 132, c[5]-c[4], 10, fillCtx.insureds?.[0]?.lastName || '', { fontSize:7.6, align:'center' });
        drawCell(c[3], 132, c[4]-c[3], 10, fillCtx.primaryId, { fontSize:7.2, align:'center' });
        drawCell(c[0], 132, c[1]-c[0], 10, fillCtx.primaryBirthDate, { fontSize:7.2, align:'center' });
        const primaryGender = fillCtx.primaryGender;
        if(primaryGender === 'male') this.drawPhoenixX(ctx, 178, 136, 3.5);
        if(primaryGender === 'female') this.drawPhoenixX(ctx, 163, 136, 3.5);
        ...
        const rowMap = {
          lastName:[145.4, 162.9],
          firstName:[162.9, 179.6],
          idNumber:[179.6, 193.1],
          birthDate:[216.5, 234.5],
          clinic:[234.5, 251.6],
          shaban:[251.6, 268.7],
          occupation:[268.7, 285.3],
          hobbies:[285.3, 302.4],
          phone:[336.6, 353.7],
          email:[353.7, 387.5],
          street:[404.6, 421.7],
        ...
        (fillCtx.insureds || []).forEach((ins, idx) => {
          ...
          if(!bounds) return;
          const x = bounds[0] + 2;
          const w = (bounds[1] - bounds[0]) - 4;
          Object.entries(rowMap).forEach(([key, pair]) => drawCell(x, pair[0] + 1, w, (pair[1] - pair[0]) - 2, ins[key] || '', { fontSize:7.1, align:'center', minFontSize:5.8 }));
          if(ins.gender === 'male') this.drawPhoenixX(ctx, x + (w * 0.65), 202, 3.2);
          if(ins.gender === 'female') this.drawPhoenixX(ctx, x + (w * 0.35), 202, 3.2);
          drawCell(x, 198, w, 13, ins.genderText || '', { fontSize:6.4, align:'center' });
          drawCell(x, 210.5, w, 13, ins.maritalStatus || '', { fontSize:6.4, align:'center' });
          const shabanText = safeTrim(ins.shaban).toLowerCase();
          if(shabanText.includes('כן')) this.drawPhoenixX(ctx, x + (w * 0.68), 260.2, 3.2);
          if(shabanText.includes('לא')) this.drawPhoenixX(ctx, x + (w * 0.32), 260.2, 3.2);
          drawCell(x, 319.5, w, 17, ins.delivery === 'email' ? 'e-mail' : (ins.delivery === 'mail' ? 'דואר ישראל' : ''), { fontSize:6.4, align:'center' });
        });
        return true;
      }
      if(pageIndex === 8){
