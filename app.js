...
(() => {
  ...
  const BUILD = "20260331-customer-summary-actions-v19";
...
init(){
      ...
      // report + finish flow
      this.els.customerSummary = $("#lcCustomerSummary");
      this.els.customerSummaryBody = $("#lcCustomerSummaryBody");
      this.els.customerSummaryClose = $("#lcCustomerSummaryClose");
      this.els.customerSummaryDownload = $("#lcCustomerSummaryDownload");
      this.els.customerSummaryActions = $("#lcCustomerSummaryActions");
      this.els.customerSummaryActionsClose = $("#lcCustomerSummaryActionsClose");
      this.els.customerSummaryActionPreview = $("#lcCustomerSummaryActionPreview");
      this.els.customerSummaryActionEmail = $("#lcCustomerSummaryActionEmail");
      this.els.customerSummaryActionWhatsapp = $("#lcCustomerSummaryActionWhatsapp");
      ...
      this.els.btnPreviewCustomerSummary = $("#lcPreviewCustomerSummary");
      this.els.btnDownloadCustomerSummary = $("#lcDownloadCustomerSummary");
      ...
      on(this.els.customerSummaryClose, "click", () => this.closeCustomerSummary());
      on(this.els.customerSummaryDownload, "click", () => this.exportCustomerSummaryPdf(null, this.els.customerSummaryDownload));
      on(this.els.customerSummaryActionsClose, "click", () => this.closeCustomerSummaryActions());
      on(this.els.customerSummaryActionPreview, "click", () => {
        this.closeCustomerSummaryActions();
        this.openCustomerSummary();
      });
      on(this.els.customerSummaryActionEmail, "click", () => this.sendCustomerSummaryByEmail());
      on(this.els.customerSummaryActionWhatsapp, "click", () => this.sendCustomerSummaryByWhatsapp());
      ...
      on(this.els.customerSummary, "click", (e) => {
        const t = e.target;
        if(t && t.getAttribute && t.getAttribute("data-close") === "1") this.closeCustomerSummary();
      });
      on(this.els.customerSummaryActions, "click", (e) => {
        const t = e.target;
        if(t && t.getAttribute && t.getAttribute("data-close") === "1") this.closeCustomerSummaryActions();
      });
      ...
      on(this.els.btnPreviewCustomerSummary, "click", () => this.openCustomerSummaryActions());
      on(this.els.btnDownloadCustomerSummary, "click", () => this.exportCustomerSummaryPdf(null, this.els.btnDownloadCustomerSummary));
...
// ---------- Step 6 ----------
    ...
    getCustomerSummaryPdfFileName(payload){
      const primary = payload?.primary || {};
      const name = safeTrim(((primary.firstName || '') + ' ' + (primary.lastName || '')).trim()) || 'לקוח';
      const stamp = new Date().toISOString().slice(0,19).replace(/[T:]/g,'-');
      return `GEMEL_INVEST_סיכום_התאמה_והמלצות_${name.replace(/[\\/:*?"<>|]/g,'_')}_${stamp}.pdf`;
    },

    getCustomerSummaryExistingPolicies(payload){
      const insureds = Array.isArray(payload?.insureds) ? payload.insureds : [];
      return insureds.flatMap((ins, index) => {
        const insData = ins?.data && typeof ins.data === 'object' ? ins.data : {};
        const insLabel = safeTrim(ins?.label) || safeTrim(insData?.fullName) || `מבוטח ${index + 1}`;
        const policies = Array.isArray(insData?.existingPolicies) ? insData.existingPolicies : [];
        return policies.map((policy) => ({
          insuredLabel: insLabel,
          company: safeTrim(policy?.company || policy?.carrier || '—'),
          type: safeTrim(policy?.type || policy?.product || policy?.policyType || '—'),
          premium: safeTrim(policy?.premium || policy?.monthlyPremium || policy?.premiumAmount || ''),
          policyNumber: safeTrim(policy?.policyNumber || policy?.number || ''),
          status: safeTrim(policy?.status || 'ללא סטטוס'),
          cancelItems: safeTrim(policy?.cancelItems || policy?.partialCancelText || policy?.cancelDetails || ''),
          details: safeTrim(policy?.details || policy?.coverage || policy?.period || ''),
          raw: policy || {}
        }));
      });
    },

    getCustomerSummaryBuckets(payload){
      const existingPolicies = this.getCustomerSummaryExistingPolicies(payload);
      const keep = [];
      const cancel = [];
      const partial = [];
      const appoint = [];
      existingPolicies.forEach((policy) => {
        const status = safeTrim(policy?.status);
        const item = {
          title: `${policy.type || 'מוצר'} · ${policy.company || 'חברה'}`,
          insuredLabel: policy.insuredLabel,
          premium: policy.premium,
          details: policy.cancelItems || policy.details || status || ''
        };
        if(status.includes('ביטול חלקי')) partial.push(item);
        else if(status.includes('ביטול מלא') || status.includes('ביטול מוצר קיים')) cancel.push(item);
        else if(status.includes('מינוי סוכן')) appoint.push(item);
        else keep.push(item);
      });
      const newPolicies = (Array.isArray(payload?.newPolicies) ? payload.newPolicies : []).map((policy) => {
        const type = safeTrim(policy?.type || policy?.product || 'פוליסה חדשה');
        const company = safeTrim(policy?.company || policy?.carrier || 'חברה');
        const premiumAfter = safeTrim(policy?.premiumAfterDiscount || policy?.premium || policy?.monthlyPremium || '');
        const premiumBefore = safeTrim(policy?.premiumBeforeDiscount || policy?.basePremium || '');
        const coverItems = (typeof this.getPolicyCoverItems === 'function' ? this.getPolicyCoverItems(policy || {}) : (Array.isArray(policy?.healthCovers) ? policy.healthCovers : [])).map((item) => safeTrim(item)).filter(Boolean);
        const coverSummary = coverItems.slice(0,4).join(' • ');
        const insuredLabel = (() => {
          if(policy?.insuredMode === 'couple') return 'מבוטח ראשי + בן/בת זוג';
          const match = (Array.isArray(payload?.insureds) ? payload.insureds : []).find((ins) => safeTrim(ins?.id) === safeTrim(policy?.insuredId));
          return safeTrim(match?.label) || 'מבוטח';
        })();
        return {
          title: `${type} · ${company}`,
          insuredLabel,
          premiumAfter,
          premiumBefore,
          details: safeTrim(policy?.coverage || policy?.sumInsured || policy?.compensation || coverSummary || '')
        };
      });
      return { existingPolicies, keep, cancel, partial, appoint, newPolicies };
    },

    getCustomerSummaryNextSteps(payload){
      const primary = payload?.primary || {};
      ...
      const companies = Object.keys(companyAgentNumbers).filter(Boolean);
      const steps = [
        'לאחר אישור ההצעה נתחיל בהפקת הפוליסות החדשות בהתאם למה שסוכם איתך.',
        'עם סיום ההפקה יישלחו אליך מסמכי הפוליסה ופרטי הכיסוי המעודכנים.',
        'במידת הצורך ניצור איתך קשר להשלמת מסמכים או חתימות נוספות.'
      ];
      if(companies.length){
        steps.unshift(`מספרי הסוכן מול החברות ${companies.join(' / ')} הוזנו במערכת ומוכנים להמשך טיפול.`);
      }
      if(safeTrim(primary?.paymentMethod || primary?.payerType || '')){
        steps.push(`אמצעי התשלום שנבחר: ${safeTrim(primary?.paymentMethod || primary?.payerType)}.`);
      }
      return steps;
    },

    buildCustomerSummaryPdfMarkup(payloadOverride, options = {}){
      ...
      const phone = safeTrim(primary?.phone || '');
      const email = safeTrim(primary?.email || '');
      ...
      const buckets = this.getCustomerSummaryBuckets(payload);
      const nextSteps = this.getCustomerSummaryNextSteps(payload);
      const money = (value) => {
        const formatted = typeof this.formatMoney === 'function' ? this.formatMoney(value) : safeTrim(value);
        return safeTrim(formatted || '');
      };
      const statusTone = (status) => {
        const val = safeTrim(status);
        if(val.includes('ללא שינוי')) return 'success';
        if(val.includes('ביטול')) return 'danger';
        if(val.includes('מינוי סוכן') || val.includes('חלקי')) return 'warn';
        return 'neutral';
      };
      const renderBucket = (title, items, tone, emptyText) => `
        <section class="lcClientPdfSection lcClientPdfSection--bucket">
          <div class="lcClientPdfSection__head">
            <div>
              <div class="lcClientPdfSection__eyebrow">התאמת צרכים</div>
              <div class="lcClientPdfSection__title">${escapeHtml(title)}</div>
            </div>
            <span class="lcClientPdfCount lcClientPdfCount--${escapeHtml(tone)}">${items.length}</span>
          </div>
          ${items.length ? `<div class="lcClientPdfBucketList">${items.map((item) => `<article class="lcClientPdfBucketItem lcClientPdfBucketItem--${escapeHtml(tone)}"><div class="lcClientPdfBucketItem__title">${escapeHtml(item.title || 'פריט')}</div><div class="lcClientPdfBucketItem__meta">${escapeHtml(item.insuredLabel || 'מבוטח')} ${item.premium ? `· ${escapeHtml(money(item.premium))}` : ''}</div>${item.details ? `<div class="lcClientPdfBucketItem__text">${escapeHtml(item.details)}</div>` : ''}</article>`).join('')}</div>` : `<div class="lcClientPdfEmpty">${escapeHtml(emptyText)}</div>`}
        </section>`;
      const existingRows = buckets.existingPolicies.map((policy) => {
        const premium = money(policy.premium);
        return `<tr>
          <td>${escapeHtml(policy.insuredLabel || '—')}</td>
          <td>${escapeHtml(policy.company || '—')}</td>
          <td>${escapeHtml(policy.type || '—')}</td>
          <td>${policy.policyNumber ? escapeHtml(policy.policyNumber) : '—'}</td>
          <td>${premium ? escapeHtml(premium) : '—'}</td>
          <td>${this.renderReportStatusBadge(policy.status || '—', statusTone(policy.status))}</td>
        </tr>`;
      }).join('');
      const newPolicyCards = buckets.newPolicies.map((policy) => `
        <article class="lcClientPdfPolicyCard">
          <div class="lcClientPdfPolicyCard__top">
            <div>
              <div class="lcClientPdfPolicyCard__title">${escapeHtml(policy.title || 'פוליסה חדשה')}</div>
              <div class="lcClientPdfPolicyCard__meta">${escapeHtml(policy.insuredLabel || 'מבוטח')}</div>
            </div>
            <div class="lcClientPdfPolicyCard__price">${escapeHtml(money(policy.premiumAfter) || '—')}</div>
          </div>
          ${policy.premiumBefore ? `<div class="lcClientPdfPolicyCard__before">לפני הנחה: ${escapeHtml(money(policy.premiumBefore) || policy.premiumBefore)}</div>` : ''}
          ${policy.details ? `<div class="lcClientPdfPolicyCard__details">${escapeHtml(policy.details)}</div>` : ''}
        </article>`).join('');
      const style = `
        <style>
          .lcClientPdfRoot{font-family:system-ui,-apple-system,"Segoe UI",Arial,"Noto Sans Hebrew",sans-serif;color:#0f172a;background:${previewMode ? '#eef1f4' : '#ffffff'};padding:${previewMode ? '0' : '0'};direction:rtl}
          .lcClientPdfDoc{max-width:980px;margin:0 auto;display:grid;gap:18px}
          .lcClientPdfHero,.lcClientPdfSection{background:#fff;border:1px solid rgba(15,23,42,.08);border-radius:24px;box-shadow:0 18px 38px rgba(15,23,42,.06)}
          .lcClientPdfHero{padding:26px 28px;background:linear-gradient(135deg,rgba(231,240,255,.95),rgba(255,255,255,.98) 48%,rgba(251,247,239,.98));overflow:hidden;position:relative}
          .lcClientPdfHero::after{content:"";position:absolute;inset-inline-start:-120px;top:-120px;width:240px;height:240px;border-radius:50%;background:radial-gradient(circle,rgba(58,108,255,.14),transparent 68%)}
          .lcClientPdfHero__top{display:flex;justify-content:space-between;gap:18px;align-items:flex-start;position:relative;z-index:1}
          .lcClientPdfHero__brand{width:170px;max-width:38vw;height:auto;filter:drop-shadow(0 14px 24px rgba(11,31,74,.14))}
          .lcClientPdfHero__eyebrow{font-size:12px;font-weight:900;letter-spacing:1.4px;color:#2b4b8a}
          .lcClientPdfHero__title{margin:8px 0 0;font-size:34px;line-height:1.08;font-weight:950;color:#10234f}
          .lcClientPdfHero__sub{margin-top:12px;max-width:720px;color:#475467;line-height:1.75;font-size:14px}
          .lcClientPdfHero__grid{margin-top:20px;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;position:relative;z-index:1}
          .lcClientPdfMeta{padding:14px 16px;border-radius:18px;background:rgba(255,255,255,.76);border:1px solid rgba(43,75,138,.12);min-height:88px}
          .lcClientPdfMeta__label{font-size:11px;font-weight:900;color:#667085;letter-spacing:.4px}
          .lcClientPdfMeta__value{margin-top:6px;font-size:15px;font-weight:900;color:#0f172a;line-height:1.45;overflow-wrap:anywhere}
          .lcClientPdfSection{padding:22px}
          .lcClientPdfSection__head{display:flex;align-items:flex-start;justify-content:space-between;gap:14px;margin-bottom:14px}
          .lcClientPdfSection__eyebrow{font-size:11px;font-weight:900;color:#667085;letter-spacing:.6px}
          .lcClientPdfSection__title{font-size:24px;font-weight:950;color:#111827}
          .lcClientPdfSection__sub{margin-top:5px;font-size:13px;color:#667085;line-height:1.6}
          .lcClientPdfTableWrap{border:1px solid rgba(15,23,42,.08);border-radius:18px;overflow:hidden;background:#fff}
          .lcClientPdfTable{width:100%;border-collapse:collapse}
          .lcClientPdfTable th,.lcClientPdfTable td{padding:12px 14px;border-bottom:1px solid rgba(15,23,42,.08);text-align:right;font-size:13px;vertical-align:top}
          .lcClientPdfTable th{background:#f8fafc;color:#475467;font-size:12px;font-weight:900}
          .lcClientPdfTable tr:last-child td{border-bottom:0}
          .lcClientPdfBuckets{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}
          .lcClientPdfSection--bucket{padding:18px}
          .lcClientPdfCount{display:inline-flex;align-items:center;justify-content:center;min-width:40px;height:40px;padding:0 12px;border-radius:999px;font-size:14px;font-weight:950}
          .lcClientPdfCount--success{background:rgba(34,197,94,.12);color:#166534}
          .lcClientPdfCount--danger{background:rgba(239,68,68,.12);color:#991b1b}
          .lcClientPdfCount--warn{background:rgba(245,158,11,.14);color:#9a6700}
          .lcClientPdfBucketList{display:grid;gap:10px}
          .lcClientPdfBucketItem{padding:14px 15px;border-radius:18px;border:1px solid rgba(15,23,42,.08);background:#fff}
          .lcClientPdfBucketItem--success{background:rgba(240,253,244,.82);border-color:rgba(34,197,94,.16)}
          .lcClientPdfBucketItem--danger{background:rgba(254,242,242,.84);border-color:rgba(239,68,68,.16)}
          .lcClientPdfBucketItem--warn{background:rgba(255,251,235,.9);border-color:rgba(245,158,11,.18)}
          .lcClientPdfBucketItem__title{font-size:14px;font-weight:950;color:#111827}
          .lcClientPdfBucketItem__meta{margin-top:4px;font-size:12px;color:#475467}
          .lcClientPdfBucketItem__text{margin-top:8px;font-size:13px;color:#1f2937;line-height:1.65}
          .lcClientPdfPolicies{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}
          .lcClientPdfPolicyCard{padding:18px;border-radius:20px;border:1px solid rgba(43,75,138,.12);background:linear-gradient(180deg,rgba(255,255,255,.96),rgba(243,247,255,.98));box-shadow:0 14px 28px rgba(43,75,138,.06)}
          .lcClientPdfPolicyCard__top{display:flex;justify-content:space-between;gap:14px;align-items:flex-start}
          .lcClientPdfPolicyCard__title{font-size:18px;font-weight:950;color:#10234f}
          .lcClientPdfPolicyCard__meta{margin-top:6px;font-size:12px;color:#667085}
          .lcClientPdfPolicyCard__price{font-size:20px;font-weight:950;color:#0b1f4a;white-space:nowrap}
          .lcClientPdfPolicyCard__before{margin-top:10px;font-size:12px;color:#667085}
          .lcClientPdfPolicyCard__details{margin-top:12px;font-size:13px;line-height:1.7;color:#1f2937}
          .lcClientPdfSteps{display:grid;gap:12px}
          .lcClientPdfStep{display:grid;grid-template-columns:42px 1fr;gap:12px;align-items:start;padding:14px 16px;border-radius:18px;background:#fff;border:1px solid rgba(15,23,42,.08)}
          .lcClientPdfStep__num{width:42px;height:42px;border-radius:14px;display:grid;place-items:center;font-weight:950;color:#0b1f4a;background:linear-gradient(180deg,rgba(217,234,255,.9),rgba(255,255,255,.98));border:1px solid rgba(43,75,138,.14)}
          .lcClientPdfStep__text{font-size:14px;line-height:1.75;color:#1f2937}
          .lcClientPdfFooter{padding:18px 22px;border-radius:22px;background:linear-gradient(135deg,rgba(11,31,74,.96),rgba(43,75,138,.96));color:#fff;display:flex;justify-content:space-between;gap:16px;align-items:center}
          .lcClientPdfFooter__title{font-size:20px;font-weight:950}
          .lcClientPdfFooter__sub{margin-top:6px;font-size:13px;line-height:1.7;color:rgba(255,255,255,.84)}
          .lcClientPdfFooter__badge{padding:10px 14px;border-radius:999px;background:rgba(255,255,255,.12);border:1px solid rgba(255,255,255,.16);font-size:12px;font-weight:900;white-space:nowrap}
          .lcClientPdfEmpty{padding:16px;border-radius:16px;background:#f8fafc;color:#667085;font-size:13px;line-height:1.7}
          @media (max-width: 900px){.lcClientPdfHero__grid,.lcClientPdfBuckets,.lcClientPdfPolicies{grid-template-columns:1fr}.lcClientPdfHero__top,.lcClientPdfFooter{flex-direction:column;align-items:flex-start}.lcClientPdfHero__brand{width:150px}.lcClientPdfHero__title{font-size:28px}}
        </style>`;
      return {
        payload,
        html: `${style}<div class="lcClientPdfRoot"><div class="lcClientPdfDoc">
          <section class="lcClientPdfHero">
            <div class="lcClientPdfHero__top">
              <div>
                <div class="lcClientPdfHero__eyebrow">GEMEL INVEST · מסמך סיכום ללקוח</div>
                <h1 class="lcClientPdfHero__title">סיכום התאמה והמלצות ביטוח</h1>
                <div class="lcClientPdfHero__sub">ריכזנו עבורך את תמונת המצב הביטוחית הקיימת, את ההמלצות שסוכמו איתך, ואת השלבים הבאים להמשך הטיפול — בצורה פשוטה, ברורה ונגישה.</div>
              </div>
              <img src="${escapeHtml(logoSrc)}" alt="GEMEL INVEST" class="lcClientPdfHero__brand">
            </div>
            <div class="lcClientPdfHero__grid">
              <div class="lcClientPdfMeta"><div class="lcClientPdfMeta__label">שם הלקוח</div><div class="lcClientPdfMeta__value">${escapeHtml(customerName)}</div></div>
              <div class="lcClientPdfMeta"><div class="lcClientPdfMeta__label">תאריך הפקה</div><div class="lcClientPdfMeta__value">${escapeHtml(exportedAt)}</div></div>
              <div class="lcClientPdfMeta"><div class="lcClientPdfMeta__label">סוכן מטפל</div><div class="lcClientPdfMeta__value">${escapeHtml(agentName)}</div></div>
              <div class="lcClientPdfMeta"><div class="lcClientPdfMeta__label">פרטי קשר</div><div class="lcClientPdfMeta__value">${escapeHtml([phone, email, city].filter(Boolean).join(' · ') || 'יעודכן מול הסוכן')}</div></div>
            </div>
          </section>

          <section class="lcClientPdfSection">
            <div class="lcClientPdfSection__head">
              <div>
                <div class="lcClientPdfSection__eyebrow">תמונת מצב</div>
                <div class="lcClientPdfSection__title">הביטוחים הקיימים שלך כיום</div>
                <div class="lcClientPdfSection__sub">להלן הפוליסות שזוהו בתהליך ההקמה והסטטוס שסוכם עבור כל אחת מהן.</div>
              </div>
            </div>
            ${existingRows ? `<div class="lcClientPdfTableWrap"><table class="lcClientPdfTable"><thead><tr><th>מבוטח</th><th>חברה</th><th>סוג מוצר</th><th>מספר פוליסה</th><th>פרמיה</th><th>סטטוס</th></tr></thead><tbody>${existingRows}</tbody></table></div>` : `<div class="lcClientPdfEmpty">לא נמצאו פוליסות קיימות בתיק שנבנה עבור הלקוח.</div>`}
          </section>

          <section class="lcClientPdfSection">
            <div class="lcClientPdfSection__head">
              <div>
                <div class="lcClientPdfSection__eyebrow">המלצה מסכמת</div>
                <div class="lcClientPdfSection__title">התאמת צרכים</div>
                <div class="lcClientPdfSection__sub">חלוקה ברורה בין מה שנשאר כפי שהוא, מה שמתבטל או מתעדכן, ומה מתווסף כתוצאה מההמלצה החדשה.</div>
              </div>
            </div>
            <div class="lcClientPdfBuckets">
              ${renderBucket('נשאר ללא שינוי', buckets.keep, 'success', 'לא סומנו מוצרים שנשארים ללא שינוי.')}
              ${renderBucket('ביטול מוצר קיים', buckets.cancel, 'danger', 'לא סומנו מוצרים לביטול מלא.')}
              ${renderBucket('ביטול חלקי / שינוי במוצר קיים', buckets.partial, 'warn', 'לא סומנו מוצרים לביטול חלקי או שינוי.')}
              ${renderBucket('מינוי סוכן / המשך ליווי', buckets.appoint, 'warn', 'לא סומנו מוצרים למינוי סוכן או המשך טיפול.')}
            </div>
          </section>
          ...
          <section class="lcClientPdfSection">
            <div class="lcClientPdfSection__head">
              <div>
                <div class="lcClientPdfSection__eyebrow">רכישה חדשה</div>
                <div class="lcClientPdfSection__title">הפוליסות החדשות שאנו מתכננים עבורך</div>
                <div class="lcClientPdfSection__sub">אלו המוצרים החדשים שהוזנו במערכת עבורך, כולל עיקרי הכיסוי והפרמיה שסוכמה.</div>
              </div>
            </div>
            ${newPolicyCards ? `<div class="lcClientPdfPolicies">${newPolicyCards}</div>` : `<div class="lcClientPdfEmpty">לא הוזנו פוליסות חדשות במסמך זה.</div>`}
          </section>

          <section class="lcClientPdfSection">
            <div class="lcClientPdfSection__head">
              <div>
                <div class="lcClientPdfSection__eyebrow">מכאן והלאה</div>
                <div class="lcClientPdfSection__title">מה הולך לקרות עכשיו</div>
                <div class="lcClientPdfSection__sub">אלו השלבים הבאים בתהליך, כדי שתדע בדיוק מה צפוי בהמשך.</div>
              </div>
            </div>
            <div class="lcClientPdfSteps">${nextSteps.map((step, index) => `<div class="lcClientPdfStep"><div class="lcClientPdfStep__num">${index + 1}</div><div class="lcClientPdfStep__text">${escapeHtml(step)}</div></div>`).join('')}</div>
          </section>

          <section class="lcClientPdfFooter">
            <div>
              <div class="lcClientPdfFooter__title">תודה שבחרת GEMEL INVEST</div>
              <div class="lcClientPdfFooter__sub">המסמך הופק עבורך מתוך מערכת הסוכנות ומשקף את הנתונים וההמלצות שסוכמו עד למועד ההפקה.</div>
            </div>
            <div class="lcClientPdfFooter__badge">לשאלות והבהרות ניתן לפנות לסוכן המטפל</div>
          </section>
        </div></div>`
      };
    },

    renderCustomerSummary(payloadOverride){
      try{
        const pack = this.buildCustomerSummaryPdfMarkup(payloadOverride, { forPreview:true });
        return pack?.html || '<div class="lcClientPdfRoot"><div class="lcClientPdfDoc"><div class="lcClientPdfEmpty">לא ניתן להציג את סיכום הלקוח כרגע.</div></div></div>';
      }catch(err){
        console.error('renderCustomerSummary failed', err);
        return `<div class="lcClientPdfRoot"><div class="lcClientPdfDoc"><div class="lcClientPdfEmpty">אירעה שגיאה בעת בניית סיכום הלקוח: ${escapeHtml(err?.message || 'שגיאה לא ידועה')}</div></div></div>`;
      }
    },

    getCustomerSummaryContactInfo(payloadOverride){
      const payload = this.normalizeOperationalReportPayload(payloadOverride || this.getOperationalPayload());
      const primary = payload?.primary || {};
      const customerName = safeTrim(((primary.firstName || '') + ' ' + (primary.lastName || '')).trim()) || 'לקוח';
      const phone = safeTrim(primary?.phone || payload?.phone || '');
      const email = safeTrim(primary?.email || payload?.email || '');
      return { payload, primary, customerName, phone, email };
    },

    getCustomerSummaryShareText(payloadOverride){
      const info = this.getCustomerSummaryContactInfo(payloadOverride);
      const agentName = safeTrim(Auth?.current?.name) || safeTrim(info.payload?.agentName) || 'הסוכן המטפל';
      return `שלום ${info.customerName}, מצורף סיכום התאמה והמלצות ביטוח שהוכן עבורך על ידי ${agentName} ממערכת GEMEL INVEST. ניתן לעיין בסיכום ולפנות אלינו לכל שאלה.`;
    },

    openCustomerSummaryActions(){
      if(!this.els.customerSummaryActions) return;
      this.els.customerSummaryActions.classList.add('is-open');
      this.els.customerSummaryActions.setAttribute('aria-hidden','false');
    },

    closeCustomerSummaryActions(){
      if(!this.els.customerSummaryActions) return;
      this.els.customerSummaryActions.classList.remove('is-open');
      this.els.customerSummaryActions.setAttribute('aria-hidden','true');
    },

    sendCustomerSummaryByEmail(payloadOverride){
      const info = this.getCustomerSummaryContactInfo(payloadOverride);
      const body = this.getCustomerSummaryShareText(payloadOverride) + `

שים לב: מתוך התצוגה במערכת ניתן גם להוריד PDF מעוצב של הסיכום.`;
      const subject = `סיכום התאמה והמלצות ביטוח - ${info.customerName}`;
      const recipient = info.email || '';
      this.closeCustomerSummaryActions();
      window.location.href = `mailto:${encodeURIComponent(recipient)}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
    },

    sendCustomerSummaryByWhatsapp(payloadOverride){
      const info = this.getCustomerSummaryContactInfo(payloadOverride);
      let phone = safeTrim(info.phone).replace(/[^0-9]/g, '');
      if(phone.startsWith('0')) phone = '972' + phone.slice(1);
      const text = this.getCustomerSummaryShareText(payloadOverride) + `

אם תרצה, נשלח לך גם את המסמך כ-PDF.`;
      const url = phone
    ...
    openCustomerSummary(payloadOverride){
      if(!this.els.customerSummary || !this.els.customerSummaryBody) return;
      this.els.customerSummaryBody.innerHTML = this.renderCustomerSummary(payloadOverride);
      this.els.customerSummary.classList.add('is-open');
      this.els.customerSummary.setAttribute('aria-hidden','false');
    },

    closeCustomerSummary(){
      if(!this.els.customerSummary) return;
      this.els.customerSummary.classList.remove('is-open');
      this.els.customerSummary.setAttribute('aria-hidden','true');
    },

    async exportCustomerSummaryPdf(payloadOverride, sourceBtn){
      ...
      const triggerBtn = sourceBtn || this.els.customerSummaryDownload || this.els.btnDownloadCustomerSummary || null;
      ...
      try{
        ...
        const payload = this.normalizeOperationalReportPayload(payloadOverride || this.getOperationalPayload());
        const filename = this.getCustomerSummaryPdfFileName(payload);
        const pack = this.buildCustomerSummaryPdfMarkup(payload, { forPreview:false });
        if(!pack?.html) throw new Error('Customer summary PDF markup was not created');
        ...
        exportFrame.style.minWidth = '794px';
        exportFrame.style.maxWidth = '794px';
        ...
        await this.waitForImages(exportFrame);
        const root = exportFrame.querySelector('.lcClientPdfRoot');
        if(!root) throw new Error('Customer summary PDF root was not found');
        ...
        console.error('exportCustomerSummaryPdf failed', err);
        window.alert('אירעה שגיאה בעת יצירת קובץ הסיכום ללקוח. נסה שוב.');
      }finally{
        if(host && host.parentNode) host.parentNode.removeChild(host);
        if(triggerBtn){
          triggerBtn.disabled = false;
          triggerBtn.textContent = originalText || 'הורד קובץ ללקוח';
        }
      }
    },
