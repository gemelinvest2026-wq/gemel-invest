...
(() => {
  ...
  const BUILD = "20260327-phoenix-original-pdf-v3-page1-calibrated";
...
init(){
      ...
      this.els.btnDownloadPhoenixForm = $("#lcDownloadPhoenixForm");
      ...
      on(this.els.btnDownloadPhoenixForm, "click", () => this.exportPhoenixProposalPdf(null, this.els.btnDownloadPhoenixForm));
...
// ---------- Step 6 ----------
        ...
        <section class="lcOpSummary">
          <div class="lcOpHero">
            ...
            <div class="lcOpHero__actions">
              ...
              <button type="button" class="btn btn--brandGhost" data-op-download-phoenix="1">הורד טופס הצעה - הפניקס</button>
            </div>
    ...
    bindOperationalSummaryInputs(){
      ...
      $$("[data-op-download-phoenix]", this.els.body).forEach(btn => {
        on(btn, "click", () => this.exportPhoenixProposalPdf(null, btn));
      });
    },
    ...
    getPhoenixEligiblePolicies(payloadOverride){
      const payload = this.normalizeOperationalReportPayload(payloadOverride || this.getOperationalPayload());
      const allowedTypes = new Set(["בריאות","מחלות קשות","סרטן"]);
      const policies = (Array.isArray(payload?.newPolicies) ? payload.newPolicies : []).filter((policy) => {
        const company = safeTrim(policy?.company);
        const type = safeTrim(policy?.type || policy?.product);
        return company === "הפניקס" && allowedTypes.has(type);
      });
      return { payload, policies };
    },

    getPhoenixProposalPdfFileName(payloadOverride){
      const { payload } = this.getPhoenixEligiblePolicies(payloadOverride);
      const primary = payload?.primary || {};
      const name = safeTrim(((primary.firstName || '') + ' ' + (primary.lastName || '')).trim()) || 'לקוח';
      const stamp = new Date().toISOString().slice(0,19).replace(/[T:]/g,'-');
      return `Phoenix_טופס_הצעה_${name.replace(/[\\/:*?"<>|]/g,'_')}_${stamp}.pdf`;
    },

    buildPhoenixProposalMarkup(payloadOverride){
      const { payload, policies } = this.getPhoenixEligiblePolicies(payloadOverride);
      if(!policies.length){
        throw new Error('לא נמצאו פוליסות חדשות של הפניקס מסוג בריאות / מחלות קשות / סרטן.');
      }
      ...
      const exportedAt = exportDate.toLocaleDateString('he-IL', { day:'2-digit', month:'2-digit', year:'numeric' }) + ' · ' + exportDate.toLocaleTimeString('he-IL', { hour:'2-digit', minute:'2-digit' });
      const customerName = safeTrim(((primary.firstName || '') + ' ' + (primary.lastName || '')).trim()) || 'לקוח ללא שם';
      const currentAgentName = safeTrim(Auth?.current?.name) || safeTrim(payload?.agentName) || 'נציג מטפל';
      const companyAgentNumbers = (payload?.companyAgentNumbers && typeof payload.companyAgentNumbers === 'object') ? payload.companyAgentNumbers : {};
      const phoenixAgentNumber = safeTrim(companyAgentNumbers['הפניקס']);
      const esc = escapeHtml;
      const roleMap = { primary:'מבוטח ראשי', spouse:'מבוטח משני', adultChild:'בגיר', child:'קטין' };
      const fullNameOf = (data={}) => safeTrim(((data.firstName || '') + ' ' + (data.lastName || '')).trim());
      const insuredRole = (ins, index) => safeTrim(roleMap[safeTrim(ins?.type)]) || (index === 0 ? 'מבוטח ראשי' : `מבוטח ${index + 1}`);
      const insuredAddress = (data={}) => [
        safeTrim(data.city),
        [safeTrim(data.street), safeTrim(data.houseNo || data.houseNumber), safeTrim(data.apartmentNo || data.apartment)].filter(Boolean).join(' ')
      ].filter(Boolean).join(' - ');
      ...
      const getPolicyInsuredText = (policy={}) => {
        if(policy?.insuredMode === 'couple'){
          const primaryLabel = safeTrim(insureds?.[0]?.label) || 'מבוטח ראשי';
          const spouseLabel = safeTrim((insureds || []).find(x => x?.type === 'spouse')?.label);
          return spouseLabel ? `${primaryLabel} + ${spouseLabel}` : primaryLabel;
        }
        const ins = (insureds || []).find(x => safeTrim(x?.id) === safeTrim(policy?.insuredId));
        return safeTrim(ins?.label) || safeTrim(insureds?.[0]?.label) || 'מבוטח';
      };
      ...
      const summaryRows = [
        ['חברה', 'הפניקס'],
        ['לקוח ראשי', customerName],
        ['ת״ז ראשית', safeTrim(primary.idNumber)],
        ['סוכן מטפל', currentAgentName],
        ['מספר סוכן הפניקס', phoenixAgentNumber],
        ['תאריך הפקה', exportedAt]
      ].filter(([, value]) => safeTrim(value));

      const productRows = policies.map((policy) => {
        const covers = Array.isArray(policy?.healthCovers) ? policy.healthCovers.map((x) => safeTrim(x)).filter(Boolean) : [];
        return [
          safeTrim(policy?.type || policy?.product) || 'מוצר',
          getPolicyInsuredText(policy),
          safeTrim(policy?.startDate) || '—',
          safeTrim(policy?.premiumMonthly) || '—',
          safeTrim(this.getPolicyDiscountDisplayText(policy)) || '—',
          safeTrim(this.formatMoneyValue(this.getPolicyPremiumAfterDiscount(policy))) || '—',
          covers.length ? esc(covers.join(', ')) : '—'
        ];
      });

      const insuredCards = insureds.map((ins, index) => {
        const d = ins?.data || {};
        const infoRows = [
          ['שם מלא', fullNameOf(d)],
          ['סוג מבוטח', insuredRole(ins, index)],
          ['ת״ז', safeTrim(d.idNumber)],
          ['תאריך לידה', safeTrim(d.birthDate)],
          ['מגדר', safeTrim(d.gender)],
          ['מצב משפחתי', safeTrim(d.maritalStatus)],
          ['טלפון', safeTrim(d.phone)],
          ['אימייל', safeTrim(d.email)],
          ['כתובת', insuredAddress(d)],
          ['קופת חולים', safeTrim(d.clinic || d.hmo || d.kupatHolim)],
          ['שב״ן', safeTrim(d.shaban || d.shabanLevel)],
          ['עיסוק', safeTrim(d.occupation)]
        ].filter((item) => safeTrim(item[1]));
        return `
          <section class="giPhoenixInsuredCard">
            <div class="giPhoenixInsuredCard__head">
              <div class="giPhoenixInsuredCard__title">${esc(insuredRole(ins, index))}</div>
              <div class="giPhoenixInsuredCard__name">${esc(fullNameOf(d) || safeTrim(ins?.label) || 'מבוטח')}</div>
            </div>
            <div class="giPhoenixInfoGrid">${infoRows.map(([label, value]) => `<div class="giPhoenixInfoRow"><span>${esc(label)}</span><strong>${esc(value)}</strong></div>`).join('')}</div>
          </section>`;
      }).join('');
      ...
      questionList.forEach((item) => {
        ...
        insureds.forEach((ins, index) => {
          const insId = safeTrim(ins?.id) || safeTrim(ins?.data?.id) || `payload_ins_${index}`;
          const answer = responses?.[q.key]?.[insId] || {};
          const answerRaw = safeTrim(answer?.answer);
          if(!answerRaw) return;
          const answerLabel = answerRaw === 'yes' ? 'כן' : answerRaw === 'no' ? 'לא' : answerRaw;
          const detailText = answerRaw === 'yes'
            ? (safeTrim(this.summarizeHealthFields(answer?.fields || {})) || 'סומן כן ללא פירוט נוסף')
            : 'נענה לא';
          const insuredName = fullNameOf(ins?.data || {}) || safeTrim(ins?.label) || insuredRole(ins, index);
          healthEntries.push({ insuredName, question: safeTrim(q.text || q.key || 'שאלה רפואית'), answerLabel, detailText });
          if(safeTrim(q.key).toLowerCase().includes('smoking')){
            smokingRows.push({ insuredName, answerLabel, detailText });
          }
        });
      });
      const healthChunks = [];
      for(let i = 0; i < healthEntries.length; i += 8){
        healthChunks.push(healthEntries.slice(i, i + 8));
      }
      if(!healthChunks.length) healthChunks.push([]);

      const renderSimpleRows = (rows, emptyText='—') => {
        if(!rows.length) return `<div class="giPhoenixEmpty">${esc(emptyText)}</div>`;
        return `<div class="giPhoenixSummaryGrid">${rows.map(([label, value]) => `<div class="giPhoenixSummaryItem"><span>${esc(label)}</span><strong>${esc(value)}</strong></div>`).join('')}</div>`;
      };

      const renderProductsTable = () => {
        if(!productRows.length) return `<div class="giPhoenixEmpty">לא נמצאו מוצרי הפניקס בטופס זה.</div>`;
        return `
          <table class="giPhoenixTable">
            <thead>
              <tr>
                <th>מוצר</th>
                <th>מבוטח</th>
            ...
            <tbody>
              ${productRows.map((cols) => `<tr>${cols.map((col) => `<td>${col}</td>`).join('')}</tr>`).join('')}
            </tbody>
          </table>`;
      };

      const renderSmokingTable = () => {
        if(!smokingRows.length) return `<div class="giPhoenixEmpty">לא זוהו תשובות עישון במידע שנשמר.</div>`;
        return `
          <table class="giPhoenixTable giPhoenixTable--compact">
            <thead><tr><th>מבוטח</th><th>תשובה</th><th>פירוט</th></tr></thead>
            <tbody>
              ${smokingRows.map((row) => `<tr><td>${esc(row.insuredName)}</td><td>${esc(row.answerLabel)}</td><td>${esc(row.detailText)}</td></tr>`).join('')}
            </tbody>
          </table>`;
      };

      const renderHealthChunk = (entries) => {
        if(!entries.length) return `<div class="giPhoenixEmpty">טרם הוזנו תשובות הצהרת בריאות.</div>`;
        return entries.map((entry) => `
          <article class="giPhoenixHealthItem">
            <div class="giPhoenixHealthItem__top">
              <strong>${esc(entry.insuredName)}</strong>
              <span class="giPhoenixAnswerTag giPhoenixAnswerTag--${entry.answerLabel === 'כן' ? 'yes' : 'no'}">${esc(entry.answerLabel)}</span>
            </div>
            <div class="giPhoenixHealthItem__question">${esc(entry.question)}</div>
            <div class="giPhoenixHealthItem__detail">${esc(entry.detailText)}</div>
          </article>`).join('');
      };

      const style = `<style>
        .giPhoenixRoot{font-family:Arial,"Noto Sans Hebrew","Segoe UI",sans-serif;color:#11213f;background:#fff;direction:rtl}
        .giPhoenixPage{width:720px;min-height:1046px;margin:0 auto 8px;background:#fff;padding:22px 24px 24px;box-sizing:border-box;page-break-inside:avoid;break-inside:avoid}
        .giPhoenixPage + .giPhoenixPage{page-break-before:always}
        .giPhoenixHeader{display:flex;align-items:flex-start;justify-content:space-between;gap:20px;margin-bottom:18px;padding-bottom:14px;border-bottom:2px solid #dbe7ff}
        .giPhoenixHeader__title{margin:0;font-size:28px;font-weight:900;color:#16346f}
        .giPhoenixHeader__sub{margin-top:6px;font-size:13px;color:#4e648d;line-height:1.5}
        .giPhoenixHeader__meta{display:flex;flex-wrap:wrap;gap:8px 12px;margin-top:10px;font-size:12px;color:#4d5e82}
        .giPhoenixHeader__logo{width:110px;height:auto;object-fit:contain}
        .giPhoenixHero{padding:18px 18px 16px;border-radius:22px;background:linear-gradient(135deg,#edf4ff,#f9fbff);border:1px solid #d9e7ff;box-shadow:0 12px 28px rgba(17,33,63,.06);margin-bottom:18px}
        .giPhoenixHero__eyebrow{font-size:12px;font-weight:800;color:#3562b8;letter-spacing:.3px}
        .giPhoenixHero__title{margin:8px 0 0;font-size:24px;font-weight:900;color:#0f2247}
        .giPhoenixHero__text{margin-top:8px;font-size:13px;line-height:1.7;color:#40557c}
        .giPhoenixSection{margin-top:18px}
        .giPhoenixSection__title{font-size:17px;font-weight:900;color:#173b78;margin-bottom:8px}
        .giPhoenixSection__sub{font-size:12px;color:#60739a;margin-bottom:10px}
        .giPhoenixSummaryGrid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}
        .giPhoenixSummaryItem,.giPhoenixInfoRow{display:flex;justify-content:space-between;gap:12px;padding:10px 12px;border-radius:14px;background:#fff;border:1px solid #e0e9fa;align-items:center}
        .giPhoenixSummaryItem span,.giPhoenixInfoRow span{font-size:12px;color:#5c6f93;font-weight:700}
        .giPhoenixSummaryItem strong,.giPhoenixInfoRow strong{font-size:13px;color:#132a58;text-align:left}
        .giPhoenixTable{width:100%;border-collapse:collapse;border-spacing:0;overflow:hidden;border-radius:16px;border:1px solid #dbe6f7;background:#fff}
        .giPhoenixTable th,.giPhoenixTable td{padding:11px 10px;border-bottom:1px solid #e8eef9;font-size:12px;vertical-align:top}
        .giPhoenixTable th{background:#f2f6ff;color:#274a88;font-weight:900}
        .giPhoenixTable tbody tr:last-child td{border-bottom:0}
        .giPhoenixTable--compact th,.giPhoenixTable--compact td{font-size:13px}
        .giPhoenixInsuredCard{margin-top:12px;border:1px solid #dde7fa;border-radius:18px;padding:14px;background:linear-gradient(180deg,#fff,#fbfcff)}
        .giPhoenixInsuredCard__head{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:10px}
        .giPhoenixInsuredCard__title{font-size:12px;font-weight:900;color:#4569a7;background:#edf4ff;border-radius:999px;padding:6px 10px}
        .giPhoenixInsuredCard__name{font-size:18px;font-weight:900;color:#112a58}
        .giPhoenixInfoGrid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}
        .giPhoenixHealthItem{padding:12px 14px;border-radius:16px;border:1px solid #dfe8fb;background:#fff;margin-bottom:10px}
        .giPhoenixHealthItem__top{display:flex;justify-content:space-between;gap:10px;align-items:center;margin-bottom:8px}
        .giPhoenixHealthItem__question{font-size:13px;font-weight:800;color:#193567;line-height:1.6}
        .giPhoenixHealthItem__detail{margin-top:6px;font-size:12px;color:#566b91;line-height:1.6}
        .giPhoenixAnswerTag{display:inline-flex;align-items:center;justify-content:center;border-radius:999px;padding:4px 10px;font-size:12px;font-weight:900}
        .giPhoenixAnswerTag--yes{background:#ffe8ea;color:#9c2430}
        .giPhoenixAnswerTag--no{background:#e8f8ef;color:#1f7a46}
        .giPhoenixFooter{margin-top:18px;padding-top:12px;border-top:1px solid #dfe7f6;display:flex;justify-content:space-between;gap:10px;font-size:11px;color:#6a7c9d}
        .giPhoenixEmpty{padding:18px;border:1px dashed #cedaf2;border-radius:16px;background:#f9fbff;font-size:13px;color:#61749a}
        .giPhoenixSignatureGrid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px;margin-top:18px}
        .giPhoenixSignature{padding:14px 12px;border:1px dashed #b9cbed;border-radius:16px;min-height:84px;background:#fff}
        .giPhoenixSignature__label{font-size:12px;font-weight:900;color:#2c4f8f;margin-bottom:34px}
        .giPhoenixSignature__line{height:1px;background:#9db3dd}
      </style>`;

      const pages = [];
      pages.push(`
        <div class="giPhoenixPage">
          <header class="giPhoenixHeader">
            <div>
              <h1 class="giPhoenixHeader__title">טופס הצעה לביטוח - הפניקס</h1>
              <div class="giPhoenixHeader__sub">גרסה פנימית ראשונה למילוי אוטומטי מתוך מערכת GEMEL INVEST עבור מוצרי בריאות, מחלות קשות וסרטן.</div>
              <div class="giPhoenixHeader__meta">
                <div>לקוח: ${esc(customerName)}</div>
                <div>סוכן: ${esc(currentAgentName)}</div>
                ${phoenixAgentNumber ? `<div>מספר סוכן הפניקס: ${esc(phoenixAgentNumber)}</div>` : ''}
                <div>${esc(exportedAt)}</div>
              </div>
            </div>
            <img class="giPhoenixHeader__logo" src="${esc(logoSrc)}" alt="GEMEL INVEST" />
          </header>
          <section class="giPhoenixHero">
            <div class="giPhoenixHero__eyebrow">Phoenix Proposal Engine</div>
            <div class="giPhoenixHero__title">מוכן לעבודה מול טופס ההצעה</div>
            <div class="giPhoenixHero__text">המסמך מרכז את כל הנתונים שכבר הוזנו במערכת ומסדר אותם במבנה ברור לקראת מילוי סופי של הטופס המקורי. בגרסה זו המערכת מייצרת טופס פנימי מסודר להורדה ולא Overlay על גבי ה-PDF המקורי.</div>
          </section>
          <section class="giPhoenixSection">
            <div class="giPhoenixSection__title">פרטי הצעה</div>
            <div class="giPhoenixSection__sub">סיכום בסיסי של הלקוח, החברה והמשתמש שהפיק את המסמך.</div>
            ${renderSimpleRows(summaryRows, 'לא נמצאו נתוני הצעה')}
          </section>
          <section class="giPhoenixSection">
            <div class="giPhoenixSection__title">מוצרי הפניקס שנבחרו בהצעה</div>
            <div class="giPhoenixSection__sub">רשימת המוצרים החדשים של הפניקס מתוך שלב ההקמה.</div>
            ${renderProductsTable()}
          </section>
          <div class="giPhoenixFooter"><div>GEMEL INVEST</div><div>עמוד 1</div></div>
        </div>`);
      ...
      pages.push(`
        <div class="giPhoenixPage">
          <header class="giPhoenixHeader">
            <div>
              <h1 class="giPhoenixHeader__title">פרטי מבוטחים</h1>
              <div class="giPhoenixHeader__sub">הנתונים האישיים שנאספו בשלבי ההקמה ונדרשים לצורך מילוי טופס ההצעה.</div>
            </div>
            <img class="giPhoenixHeader__logo" src="${esc(logoSrc)}" alt="GEMEL INVEST" />
          </header>
          <section class="giPhoenixSection">
            <div class="giPhoenixSection__title">כל המבוטחים בהצעה</div>
            <div class="giPhoenixSection__sub">מבוטח ראשי, מבוטחים משניים, בגירים וקטינים - לפי מה שהוזן במערכת.</div>
            ${insuredCards || `<div class="giPhoenixEmpty">לא נמצאו פרטי מבוטחים.</div>`}
          </section>
          <section class="giPhoenixSection">
            <div class="giPhoenixSection__title">שאלת עישון</div>
            <div class="giPhoenixSection__sub">ריכוז תשובות עישון שנשמרו בהצהרת הבריאות.</div>
            ${renderSmokingTable()}
          </section>
          <div class="giPhoenixFooter"><div>GEMEL INVEST</div><div>עמוד 2</div></div>
        </div>`);
      ...
      healthChunks.forEach((chunk, index) => {
        pages.push(`
          <div class="giPhoenixPage">
            <header class="giPhoenixHeader">
              <div>
                <h1 class="giPhoenixHeader__title">הצהרת בריאות - המשך</h1>
                <div class="giPhoenixHeader__sub">תשובות שנשמרו עבור המבוטחים במערכת, כולל סימון כן/לא ופירוט כאשר נדרש.</div>
              </div>
              <img class="giPhoenixHeader__logo" src="${esc(logoSrc)}" alt="GEMEL INVEST" />
            </header>
            <section class="giPhoenixSection">
              <div class="giPhoenixSection__title">תשובות רפואיות</div>
              <div class="giPhoenixSection__sub">עמוד ${index + 3} במסמך.</div>
              ${renderHealthChunk(chunk)}
            </section>
            <section class="giPhoenixSection">
              <div class="giPhoenixSection__title">אישור וחתימות</div>
              <div class="giPhoenixSection__sub">האזור הזה נשאר פנוי כדי לאפשר מעבר סופי וחתימת לקוח לאחר השלמת הטופס המקורי.</div>
              <div class="giPhoenixSignatureGrid">
                <div class="giPhoenixSignature"><div class="giPhoenixSignature__label">חתימת מועמד ראשי</div><div class="giPhoenixSignature__line"></div></div>
                <div class="giPhoenixSignature"><div class="giPhoenixSignature__label">חתימת סוכן / בודק</div><div class="giPhoenixSignature__line"></div></div>
              </div>
            </section>
            <div class="giPhoenixFooter"><div>GEMEL INVEST</div><div>עמוד ${index + 3}</div></div>
          </div>`);
      });

      return { payload, html: `${style}<div class="giPhoenixRoot">${pages.join('')}</div>` };
    },
    ...
    async ensurePdfLibLoaded(){
      ...
      await new Promise((resolve, reject) => {
        ...
        if(existing){
          ...
          existing.addEventListener('error', () => reject(new Error('PDFLIB_LOAD_FAILED')), { once:true });
          return;
        }
        ...
        script.onerror = () => reject(new Error('PDFLIB_LOAD_FAILED'));
        document.head.appendChild(script);
      });
      if(!window.PDFLib?.PDFDocument) throw new Error('PDFLIB_NOT_AVAILABLE');
      return window.PDFLib;
    },

    async fetchPhoenixTemplateBytes(){
      const attempts = [
        './GetPDF.pdf?v=' + encodeURIComponent(BUILD || Date.now()),
        './GetPDF.pdf',
        'GetPDF.pdf'
      ];
      let lastErr = null;
      ...
      throw lastErr || new Error('PHOENIX_TEMPLATE_FETCH_FAILED');
    },

    buildPhoenixPdfFillContext(payloadOverride){
      const { payload, policies } = this.getPhoenixEligiblePolicies(payloadOverride);
      const insureds = Array.isArray(payload?.insureds) ? payload.insureds : [];
      ...
      const companyAgentNumbers = (payload?.companyAgentNumbers && typeof payload.companyAgentNumbers === 'object') ? payload.companyAgentNumbers : {};
      const phoenixAgentNumber = safeTrim(companyAgentNumbers['הפניקס']);
      const roleMap = { primary:'מועמד ראשון', spouse:'מועמד שני', adultChild:'מועמד נוסף', child:'מועמד נוסף' };
      const fullNameOf = (data={}) => safeTrim(((data.firstName || '') + ' ' + (data.lastName || '')).trim());
      const primaryName = fullNameOf(primary) || 'לקוח';
      ...
      const splitName = (data={}) => ({ first:safeTrim(data.firstName), last:safeTrim(data.lastName) });
      const firstPolicyStart = safeTrim((policies || []).map((p) => safeTrim(p?.startDate)).find(Boolean));
      const deliveryText = (data={}) => safeTrim(data.deliveryMethod || data.documentsDelivery || data.deliveryPreference || (safeTrim(data.email) ? 'email' : 'דואר ישראל'));
      const normalizeDelivery = (value='') => {
        const v = safeTrim(value).toLowerCase();
        if(v.includes('mail') || v.includes('מייל') || v.includes('email') || v.includes('e-mail')) return 'email';
        if(v.includes('דואר')) return 'mail';
        return v === 'email' ? 'email' : (v === 'mail' ? 'mail' : '');
      };
      const normalizeGender = (value='') => {
        const v = safeTrim(value).toLowerCase();
        if(v.includes('ז')) return 'male';
        if(v.includes('נ') || v.includes('female')) return 'female';
        if(v === 'male') return 'male';
        if(v === 'female') return 'female';
        return '';
      };
      const normalizedInsureds = insureds.slice(0,6).map((ins, index) => {
        const data = ins?.data || {};
        const nameParts = splitName(data);
        return {
          role: safeTrim(roleMap[safeTrim(ins?.type)]) || (index === 0 ? 'מועמד ראשון' : `מועמד ${index + 1}`),
          ...
          genderText: safeTrim(data.gender),
          maritalStatus: safeTrim(data.maritalStatus),
          clinic: safeTrim(data.clinic || data.hmo || data.kupatHolim),
          shaban: safeTrim(data.shaban || data.shabanLevel),
          occupation: safeTrim(data.occupation),
          hobbies: safeTrim(data.hobbies || data.dangerousHobbies),
          phone: safeTrim(data.phone),
          email: safeTrim(data.email),
          delivery: normalizeDelivery(deliveryText(data)),
          street: safeTrim(data.street),
          houseNo: safeTrim(data.houseNo || data.houseNumber),
          city: safeTrim(data.city),
          zip: safeTrim(data.zip || data.postalCode)
        };
      });
      const existingPhoenixPolicies = ((primary?.existingPolicies || []).filter((p) => safeTrim(p?.company).includes('פניקס') || safeTrim(p?.company).toLowerCase().includes('phoenix')));
      const healthDeclaration = primary?.healthDeclaration && typeof primary.healthDeclaration === 'object' ? primary.healthDeclaration : {};
      const responses = healthDeclaration?.responses && typeof healthDeclaration.responses === 'object' ? healthDeclaration.responses : {};
      const questionList = this.getHealthQuestionListFromPayload(payload) || [];
      const positiveHealthRows = [];
      questionList.forEach((item, qIndex) => {
        const q = item?.question || {};
        insureds.forEach((ins, insIndex) => {
          const data = ins?.data || {};
          const insId = safeTrim(ins?.id) || safeTrim(data?.id) || `payload_ins_${insIndex}`;
          const answer = responses?.[q.key]?.[insId] || {};
          const answerRaw = safeTrim(answer?.answer).toLowerCase();
          if(answerRaw !== 'yes' && answerRaw !== 'כן') return;
          positiveHealthRows.push({
            insuredName: fullNameOf(data) || safeTrim(ins?.label) || `מבוטח ${insIndex + 1}`,
            questionNo: String(qIndex + 1),
            diagnosis: safeTrim(q.text || q.key || 'שאלה רפואית'),
            period: safeTrim(this.summarizeHealthFields(answer?.fields || {})) || 'סומן כן',
            treatment: safeTrim(answer?.details || answer?.notes || '')
          });
        });
      });
      const underwritingRows = policies.slice(0, 12).map((policy) => ({
        plan: safeTrim(policy?.type || policy?.product) || 'מוצר',
        amount: safeTrim(policy?.coverageAmount || policy?.sumInsured || policy?.insuranceAmount || policy?.sum) || safeTrim(this.formatMoneyValue(policy?.coverageAmount || policy?.sumInsured || policy?.insuranceAmount || policy?.sum)),
        duration: safeTrim(this.getPolicyDiscountDisplayText(policy)) || safeTrim(policy?.discountYearsText) || 'לפי תנאי הפוליסה',
        code: safeTrim(policy?.planCode || policy?.code || policy?.productCode)
      }));
      return {
        payload,
        policies,
        ...
        primaryName,
        primaryId: safeTrim(primary.idNumber),
        primaryBirthDate: formatDate(primary.birthDate),
        primaryGender: normalizeGender(primary.gender),
        primaryGenderText: safeTrim(primary.gender),
        agentName: currentAgentName,
        phoenixAgentNumber,
        proposalDateText: proposalDate.toLocaleDateString('he-IL', { day:'2-digit', month:'2-digit', year:'numeric' }),
        firstPolicyStartDateText: formatDate(firstPolicyStart),
        positiveHealthRows: positiveHealthRows.slice(0, 8),
        underwritingRows,
        existingPhoenixPolicies,
        customerCity: safeTrim(primary.city),
        customerStreet: safeTrim(primary.street),
        customerHouseNo: safeTrim(primary.houseNo || primary.houseNumber),
        customerEmail: safeTrim(primary.email),
        customerPhone: safeTrim(primary.phone),
        companyName: 'GEMEL INVEST',
        onlyChildrenPolicy: insureds.length > 0 && insureds.every((ins) => ['child','קטין'].includes(safeTrim(ins?.type).toLowerCase()))
      };
    },

    createPhoenixOverlayCanvas(pageWidth, pageHeight, painter){
      const scale = 2;
    ...
    drawPhoenixTextBox(ctx, text, x, y, width, height, options = {}){
      const value = safeTrim(text);
      if(!value) return;
      const paddingX = Number(options.paddingX ?? 2);
      const paddingY = Number(options.paddingY ?? 2);
      const align = options.align || 'right';
      const color = options.color || '#163f82';
      const fontFamily = options.fontFamily || 'Arial, "Noto Sans Hebrew", sans-serif';
      let fontSize = Number(options.fontSize || 9);
      const minFontSize = Number(options.minFontSize || 6.5);
      const lineHeightFactor = Number(options.lineHeight || 1.18);
      const availableWidth = Math.max(8, width - (paddingX * 2));
      const availableHeight = Math.max(8, height - (paddingY * 2));
      const toLines = (size) => {
        ctx.font = `${options.fontWeight || 700} ${size}px ${fontFamily}`;
      ...
      let lines = toLines(fontSize);
      while(fontSize > minFontSize && (lines.length * fontSize * lineHeightFactor) > availableHeight){
        fontSize -= 0.4;
        lines = toLines(fontSize);
      }
      ctx.save();
      ctx.fillStyle = color;
      ctx.font = `${options.fontWeight || 700} ${fontSize}px ${fontFamily}`;
      ctx.textAlign = align;
      const totalHeight = lines.length * fontSize * lineHeightFactor;
      let lineY = y + (height / 2) - (totalHeight / 2) + (fontSize * lineHeightFactor / 2);
      const drawX = align === 'center' ? x + (width / 2) : (align === 'left' ? x + paddingX : x + width - paddingX);
      lines.forEach((line) => {
        ctx.fillText(line, drawX, lineY, availableWidth);
        lineY += fontSize * lineHeightFactor;
      });
      ctx.restore();
    },

    drawPhoenixX(ctx, x, y, size = 6, color = '#163f82'){
      ctx.save();
    ...
    drawPhoenixChoiceX(ctx, centerX, centerY, size = 3.2, color = '#163f82'){
      this.drawPhoenixX(ctx, Number(centerX || 0), Number(centerY || 0), Number(size || 3.2), color);
    },

    paintPhoenixTemplatePage(ctx, pageIndex, pageWidth, pageHeight, fillCtx){
      const pt = (v) => Number(v || 0);
      const drawCell = (x, y, w, h, value, opts = {}) => this.drawPhoenixTextBox(ctx, value, pt(x), pt(y), pt(w), pt(h), opts);
      if(pageIndex === 0){
        const agentCols = {
          name:[429.0,553.1],
          number:[361.0,429.0],
          supervisor:[293.0,361.0],
          manager:[225.5,293.0],
          team:[157.5,225.5],
          proposal:[21.6,157.5]
        };
        const ownerCols = {
          firstName:[361.0,429.0],
          lastName:[293.0,361.0],
          idNumber:[225.5,293.0],
          birthDate:[21.6,157.5]
        };
        ...
        const rowMap = {
          lastName:[145.6,162.8],
          firstName:[162.8,179.8],
          idNumber:[179.8,196.8],
          birthDate:[214.6,231.8],
          clinic:[231.8,249.0],
          occupation:[266.3,283.4],
          hobbies:[283.4,300.5],
          phone:[317.6,334.9],
          email:[334.9,388.8],
          street:[404.8,422.0],
        ...
        const onlyChildrenPolicy = !!fillCtx.onlyChildrenPolicy;
        const markMarital = (x, w, value) => {
          const v = safeTrim(value).toLowerCase();
          if(!v) return;
          const map = {
            'רווק':'single', 'single':'single', 'ר':'single',
            'נשוי':'married', 'married':'married', 'נ':'married',
            'גרוש':'divorced', 'divorced':'divorced', 'ג':'divorced',
            'אלמן':'widowed', 'widowed':'widowed', 'א':'widowed'
          };
          const key = map[v] || map[v.replace(/\s+/g,'')];
          if(!key) return;
          const y = 209.5;
          const points = {
            single: x + (w * 0.82),
          ...
          this.drawPhoenixChoiceX(ctx, points[key], y, 2.6);
        };
        const markDelivery = (x, w, delivery) => {
          const yEmail = 370.6;
          const yMail = 383.8;
          if(delivery === 'email') this.drawPhoenixChoiceX(ctx, x + (w * 0.84), yEmail, 2.6);
          if(delivery === 'mail') this.drawPhoenixChoiceX(ctx, x + (w * 0.84), yMail, 2.6);
        };

        drawCell(25, 66, 62, 20, fillCtx.proposalDateText, { fontSize:8.2, align:'center', fontWeight:800 });
        drawCell(116, 66, 62, 20, fillCtx.firstPolicyStartDateText, { fontSize:8.2, align:'center', fontWeight:800 });
        drawCell(agentCols.name[0] + 3, 100, (agentCols.name[1]-agentCols.name[0]) - 6, 16, fillCtx.agentName, { fontSize:7.1, align:'right', minFontSize:5.8, fontWeight:800 });
        drawCell(agentCols.number[0] + 2, 100, (agentCols.number[1]-agentCols.number[0]) - 4, 16, fillCtx.phoenixAgentNumber, { fontSize:7.0, align:'center', minFontSize:5.8, fontWeight:800 });
        drawCell(agentCols.manager[0] + 2, 100, (agentCols.manager[1]-agentCols.manager[0]) - 4, 16, fillCtx.companyName, { fontSize:6.2, align:'center', minFontSize:5.4, fontWeight:800 });

        if(onlyChildrenPolicy){
          drawCell(ownerCols.firstName[0] + 2, 143.2, (ownerCols.firstName[1]-ownerCols.firstName[0]) - 4, 12, fillCtx.insureds?.[0]?.firstName || '', { fontSize:7.0, align:'center', minFontSize:5.8, fontWeight:800 });
          drawCell(ownerCols.lastName[0] + 2, 143.2, (ownerCols.lastName[1]-ownerCols.lastName[0]) - 4, 12, fillCtx.insureds?.[0]?.lastName || '', { fontSize:7.0, align:'center', minFontSize:5.8, fontWeight:800 });
          drawCell(ownerCols.idNumber[0] + 2, 143.2, (ownerCols.idNumber[1]-ownerCols.idNumber[0]) - 4, 12, fillCtx.primaryId, { fontSize:6.8, align:'center', minFontSize:5.8, fontWeight:800 });
          drawCell(ownerCols.birthDate[0] + 2, 143.2, (ownerCols.birthDate[1]-ownerCols.birthDate[0]) - 4, 12, fillCtx.primaryBirthDate, { fontSize:6.8, align:'center', minFontSize:5.8, fontWeight:800 });
          if(fillCtx.primaryGender === 'male') this.drawPhoenixChoiceX(ctx, 178, 135.7, 2.8);
          if(fillCtx.primaryGender === 'female') this.drawPhoenixChoiceX(ctx, 163, 135.7, 2.8);
        }

        (fillCtx.insureds || []).forEach((ins, idx) => {
          const bounds = candidateBounds[idx];
          if(!bounds) return;
          const x = bounds[0] + 3;
          const w = (bounds[1] - bounds[0]) - 6;
          Object.entries(rowMap).forEach(([key, pair]) => {
            const align = (key === 'email' || key === 'street' || key === 'city' || key === 'occupation' || key === 'hobbies') ? 'center' : 'center';
            const fontSize = key === 'email' ? 6.0 : (key === 'occupation' || key === 'hobbies' ? 6.2 : 6.8);
            const minFontSize = key === 'email' ? 4.9 : 5.4;
            drawCell(x, pair[0] + 0.6, w, (pair[1] - pair[0]) - 1.2, ins[key] || '', { fontSize, align, minFontSize, fontWeight:800, paddingX:1.6, lineHeight:1.06 });
          });
          if(ins.gender === 'male') this.drawPhoenixChoiceX(ctx, x + (w * 0.66), 196.4, 2.6);
          if(ins.gender === 'female') this.drawPhoenixChoiceX(ctx, x + (w * 0.37), 196.4, 2.6);
          markMarital(x, w, ins.maritalStatus);
          const shabanText = safeTrim(ins.shaban).toLowerCase();
          if(shabanText.includes('כן')) this.drawPhoenixChoiceX(ctx, x + (w * 0.66), 248.3, 2.6);
          if(shabanText.includes('לא')) this.drawPhoenixChoiceX(ctx, x + (w * 0.37), 248.3, 2.6);
          markDelivery(x, w, ins.delivery);
        });
        return true;
      }
      if(pageIndex === 8){
        const cols = { treatment:[21.6,128.7], period:[128.7,277.3], diagnosis:[277.3,438.0], qno:[438.0,502.0], name:[502.0,553.6] };
        const rowTops = [78.0, 92.2, 106.5, 120.7, 134.9, 149.2, 163.4, 177.6];
        (fillCtx.positiveHealthRows || []).slice(0, rowTops.length).forEach((row, idx) => {
          const y = rowTops[idx];
          drawCell(cols.name[0], y, cols.name[1]-cols.name[0], 14.2, row.insuredName, { fontSize:7.3, align:'center' });
          drawCell(cols.qno[0], y, cols.qno[1]-cols.qno[0], 14.2, row.questionNo, { fontSize:7.2, align:'center' });
          drawCell(cols.diagnosis[0], y, cols.diagnosis[1]-cols.diagnosis[0], 14.2, row.diagnosis, { fontSize:6.3, align:'right', minFontSize:5.4 });
          drawCell(cols.period[0], y, cols.period[1]-cols.period[0], 14.2, row.period, { fontSize:6.1, align:'right', minFontSize:5.2 });
          drawCell(cols.treatment[0], y, cols.treatment[1]-cols.treatment[0], 14.2, row.treatment || 'לפי פירוט בהצהרה', { fontSize:6.0, align:'right', minFontSize:5.0 });
        });
        drawCell(184, 247, 66, 20, fillCtx.insureds?.[0]?.lastName || '', { fontSize:8.2, align:'center' });
        drawCell(250, 247, 86, 20, fillCtx.insureds?.[0]?.firstName || '', { fontSize:8.2, align:'center' });
        drawCell(336, 247, 103, 20, fillCtx.primaryId, { fontSize:8.0, align:'center' });
        drawCell(477, 247, 53, 20, fillCtx.proposalDateText, { fontSize:8.0, align:'center' });
        if(fillCtx.insureds?.[1]){
          drawCell(184, 269, 66, 20, fillCtx.insureds[1].lastName || '', { fontSize:8.0, align:'center' });
          drawCell(250, 269, 86, 20, fillCtx.insureds[1].firstName || '', { fontSize:8.0, align:'center' });
          drawCell(336, 269, 103, 20, fillCtx.insureds[1].idNumber || '', { fontSize:8.0, align:'center' });
          drawCell(477, 269, 53, 20, fillCtx.proposalDateText, { fontSize:8.0, align:'center' });
        }
        drawCell(184, 291, 66, 20, fillCtx.insureds?.[2]?.lastName || '', { fontSize:8.0, align:'center' });
        drawCell(250, 291, 86, 20, fillCtx.insureds?.[2]?.firstName || '', { fontSize:8.0, align:'center' });
        drawCell(336, 291, 103, 20, fillCtx.insureds?.[2]?.idNumber || '', { fontSize:8.0, align:'center' });
        drawCell(477, 291, 53, 20, fillCtx.proposalDateText, { fontSize:8.0, align:'center' });
        drawCell(22, 785, 115, 18, fillCtx.proposalDateText, { fontSize:8.0, align:'center' });
        return true;
      }
      if(pageIndex === 9){
        drawCell(457, 102, 82, 16, fillCtx.primaryId, { fontSize:8.6, align:'center' });
        drawCell(328, 102, 120, 16, fillCtx.primaryName, { fontSize:8.3, align:'center' });
        drawCell(447, 187, 90, 18, fillCtx.proposalDateText, { fontSize:8.4, align:'center' });
        drawCell(22, 444, 99, 17, fillCtx.underwritingRows?.[0]?.code || '', { fontSize:7.3, align:'center' });
        const rowTops = [253, 270, 287, 304, 321, 338, 355, 372, 389, 406, 423, 440];
        (fillCtx.underwritingRows || []).slice(0, rowTops.length).forEach((row, idx) => {
          const y = rowTops[idx];
          drawCell(402, y, 151, 17, row.plan, { fontSize:7.2, align:'center' });
          drawCell(311, y, 91, 17, row.amount, { fontSize:7.0, align:'center' });
          drawCell(122, y, 189, 17, row.duration, { fontSize:6.6, align:'center', minFontSize:5.6 });
          drawCell(22, y, 100, 17, row.code, { fontSize:7.0, align:'center' });
        });
        return true;
      }
      return false;
    ...
    async exportPhoenixOriginalProposalPdf(payloadOverride, sourceBtn){
      const triggerBtn = sourceBtn || this.els.btnDownloadPhoenixForm || null;
      const originalText = triggerBtn ? triggerBtn.textContent : '';
      if(triggerBtn){
        triggerBtn.disabled = true;
        triggerBtn.textContent = 'ממלא טופס מקורי...';
      }
      try{
        const PDFLib = await this.ensurePdfLibLoaded();
        const bytes = await this.fetchPhoenixTemplateBytes();
        const pdfDoc = await PDFLib.PDFDocument.load(bytes, { ignoreEncryption:true });
        const pages = pdfDoc.getPages();
        const fillCtx = this.buildPhoenixPdfFillContext(payloadOverride);
        let anyOverlay = false;
        for(let i = 0; i < pages.length; i += 1){
          const page = pages[i];
          const { width, height } = page.getSize();
          const canvas = this.createPhoenixOverlayCanvas(width, height, (ctx) => {
            this.paintPhoenixTemplatePage(ctx, i, width, height, fillCtx);
          });
          const pngBytes = await new Promise((resolve) => canvas.toBlob(async (blob) => resolve(await blob.arrayBuffer()), 'image/png'));
          const pngImage = await pdfDoc.embedPng(pngBytes);
          page.drawImage(pngImage, { x:0, y:0, width, height });
          anyOverlay = true;
        }
        if(!anyOverlay) throw new Error('PHOENIX_OVERLAY_NOT_CREATED');
        const out = await pdfDoc.save();
        const blob = new Blob([out], { type:'application/pdf' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = this.getPhoenixProposalPdfFileName(payloadOverride);
        document.body.appendChild(a);
        a.click();
        a.remove();
        ...
        if(triggerBtn){
          triggerBtn.disabled = false;
          triggerBtn.textContent = originalText || 'הורד טופס הצעה - הפניקס';
        }
      }
    },

    async exportPhoenixProposalPdfPageByPage(payloadOverride, sourceBtn){
      const JsPdfCtor = window?.jspdf?.jsPDF || window?.jsPDF || null;
      const html2canvasFn = typeof window?.html2canvas === 'function' ? window.html2canvas : null;
      ...
      if(triggerBtn){
        triggerBtn.disabled = true;
        triggerBtn.textContent = 'מייצא טופס...';
      }

      let host = null;
      try{
        await new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve)));
        const filename = this.getPhoenixProposalPdfFileName(payloadOverride);
        const pack = this.buildPhoenixProposalMarkup(payloadOverride);
        if(!pack?.html) throw new Error('Phoenix proposal markup was not created');
        ...
        const root = exportFrame.querySelector('.giPhoenixRoot');
        if(!root) throw new Error('Phoenix proposal root was not found');
        root.style.width = A4_WIDTH_PX + 'px';
        root.style.minWidth = A4_WIDTH_PX + 'px';
        root.style.maxWidth = A4_WIDTH_PX + 'px';
        root.style.margin = '0 auto';
        ...
        root.querySelectorAll('.giPhoenixPage').forEach((page) => {
          page.style.width = PDF_CONTENT_WIDTH_PX + 'px';
          page.style.minWidth = PDF_CONTENT_WIDTH_PX + 'px';
          page.style.maxWidth = PDF_CONTENT_WIDTH_PX + 'px';
          page.style.minHeight = PDF_CONTENT_MIN_HEIGHT_PX + 'px';
          page.style.margin = '0 auto 8px';
          page.style.padding = '18px 20px 18px';
          page.style.boxShadow = 'none';
          page.style.borderRadius = '0';
        });
        ...
        const pages = Array.from(root.querySelectorAll('.giPhoenixPage'));
        if(!pages.length) throw new Error('No Phoenix PDF pages found');
        ...
        console.warn('exportPhoenixProposalPdfPageByPage failed', err);
        return false;
      }finally{
        if(host && host.parentNode) host.parentNode.removeChild(host);
        if(triggerBtn){
    ...
    async downloadPhoenixProposalHtmlFallback(payloadOverride, sourceBtn, originalError){
      try{
        const baseName = this.getPhoenixProposalPdfFileName(payloadOverride).replace(/\.pdf$/i, '');
        const pack = this.buildPhoenixProposalMarkup(payloadOverride);
        if(!pack?.html) throw new Error('Phoenix fallback markup was not created');
        const docHtml = `<!DOCTYPE html><html dir="rtl" lang="he"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(baseName)}</title></head><body style="margin:0;background:#ffffff;">${pack.html}</body></html>`;
        const blob = new Blob([docHtml], { type: 'text/html;charset=utf-8' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        ...
        a.remove();
        window.setTimeout(() => URL.revokeObjectURL(url), 2000);
        window.alert('יצירת ה-PDF של הפניקס נכשלה, אבל הורד עבורך מסמך HTML מסודר שאפשר לפתוח ולהדפיס ל-PDF.');
        return true;
      }catch(fallbackErr){
        console.error('downloadPhoenixProposalHtmlFallback failed', originalError, fallbackErr);
        return false;
      }
    },

    async exportPhoenixProposalPdf(payloadOverride, sourceBtn){
      const { policies } = this.getPhoenixEligiblePolicies(payloadOverride);
      if(!policies.length){
        window.alert('כדי להוריד טופס הצעה של הפניקס צריך שתהיה לפחות פוליסה חדשה אחת של הפניקס מסוג בריאות / מחלות קשות / סרטן.');
        return;
      }
      try{
        const usedOriginalTemplate = await this.exportPhoenixOriginalProposalPdf(payloadOverride, sourceBtn);
        if(usedOriginalTemplate) return;
      }catch(err){
        console.warn('exportPhoenixOriginalProposalPdf failed, falling back to internal engine', err);
      }
      const usedPageEngine = await this.exportPhoenixProposalPdfPageByPage(payloadOverride, sourceBtn);
      if(usedPageEngine) return;
      try{
        const fallbackUsed = await this.downloadPhoenixProposalHtmlFallback(payloadOverride, sourceBtn, null);
        if(!fallbackUsed) window.alert('אירעה שגיאה בעת יצירת טופס ההצעה של הפניקס.');
      }catch(err){
        console.error('exportPhoenixProposalPdf failed', err);
        window.alert('אירעה שגיאה בעת יצירת טופס ההצעה של הפניקס.');
      }
    },
