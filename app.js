...
ההכנסה מעבודה שאתה צפוי להרוויח תעמוד על סך של ______ ₪ לחודש. ידוע לך כי בקרות מקרה הביטוח יוגבלו תגמולי הפיצוי החודשי להם תהיה זכאי לגובה שלא יעלה על 75% מהכנסתך מעבודה בפועל ב־12 החודשים שקדמו למועד קרות מקרה הביטוח, או בתקופה ממועד תחילת הביטוח ועד מועד קרות מקרה הביטוח, אם חלפו פחות מ־12 חודשים.`
  ...
  const MirrorsUI = {
    ...
    init(){
      ...
      on(window, "resize", () => this.fitActiveStage());
      on(this.els.scriptCard, "click", (ev) => {
        ...
        if(yesBtn){
          ...
          const rec = this.current();
          if(rec){
            this.setFocusStep(rec, 3);
            this.render();
            this.notifyStepSaved('אישור הלקוח נשמר ונפתח שלב אימות הנתונים.');
          }
          return;
        }
        if(noBtn){
    ...
    fitActiveStage(){
      const flow = this.els.flow;
      if(!flow || flow.style.display === 'none') return;
      const active = flow.querySelector('.is-wizard-active');
      if(!active) return;
      active.style.transform = '';
      const flowRect = flow.getBoundingClientRect();
      const activeRect = active.getBoundingClientRect();
      const availableHeight = Math.max(320, flowRect.height - 4);
      const availableWidth = Math.max(320, flowRect.width - 4);
      const neededHeight = Math.max(active.scrollHeight, activeRect.height);
      const neededWidth = Math.max(active.scrollWidth, activeRect.width);
      const scaleH = availableHeight / Math.max(1, neededHeight);
      const scaleW = availableWidth / Math.max(1, neededWidth);
      const scale = Math.min(1, scaleH, scaleW);
      active.style.transform = scale < 0.999 ? `scale(${Math.max(0.76, scale).toFixed(3)})` : '';
    },

    goToNextStep(rec){
      if(!rec) return;
      this.setFocusStep(rec, this.getFocusStep(rec) + 1);
      this.render();
      this.fitActiveStage();
    },

    notifyStepSaved(message){
      if(typeof Toast !== 'undefined' && Toast?.success){
        Toast.success(message || 'השלב נשמר בהצלחה');
      }
    },
    ...
    render(){
      ...
      requestAnimationFrame(() => this.fitActiveStage());
    },
    ...
    async saveVerification(options = {}){
      ...
      if(result?.ok && !options.silent){
        this.notifyStepSaved(options.openReflection ? 'שלב האימות נשמר ונפתח השלב הבא.' : 'שלב אימות הנתונים נשמר בהצלחה.');
      }
      return !!result?.ok;
    },
    ...
    async saveCancellationChoice(){
      ...
      State.data.meta.updatedAt = nowISO();
      rec.updatedAt = State.data.meta.updatedAt;
      const result = await App.persist('שלב ביטול בחברה נגדית נשמר');
      if(result?.ok){
        this.goToNextStep(rec);
        this.notifyStepSaved('שלב ביטול בחברה נגדית נשמר ונפתח השלב הבא.');
      }
    },
    ...
    async saveDisclosure(){
      ...
      State.data.meta.updatedAt = nowISO();
      rec.updatedAt = State.data.meta.updatedAt;
      const result = await App.persist('שלב גילוי נאות נשמר');
      if(result?.ok){
        this.goToNextStep(rec);
        this.notifyStepSaved('שלב גילוי נאות נשמר ונפתח השלב הבא.');
      }
    },
    ...
    async saveHealthDeclaration(){
      ...
      State.data.meta.updatedAt = nowISO();
      rec.updatedAt = State.data.meta.updatedAt;
      const result = await App.persist('שלב הצהרת בריאות נשמר');
      if(result?.ok){
        this.goToNextStep(rec);
        this.notifyStepSaved('שלב הצהרת בריאות נשמר ונפתח השלב הבא.');
      }
    },
    ...
    async savePaymentDetails(){
      ...
      State.data.meta.updatedAt = nowISO();
      rec.updatedAt = State.data.meta.updatedAt;
      const result = await App.persist('שלב אמצעי תשלום נשמר');
      if(result?.ok){
        this.goToNextStep(rec);
        this.notifyStepSaved('שלב אמצעי תשלום נשמר ונפתח השלב הבא.');
      }
    },
    ...
    async saveIssuanceStep(){
      ...
      State.data.meta.updatedAt = nowISO();
      rec.updatedAt = State.data.meta.updatedAt;
      const result = await App.persist('שלב כניסה לתוקף נשמר');
      if(result?.ok){
        this.render();
        this.fitActiveStage();
        this.notifyStepSaved('שלב כניסה לתוקף נשמר בהצלחה.');
      }
    },
