...
(() => {
  ...
  const BUILD = "20260324-operational-report-policies-redesign-1";
...
// ---------- Step 6 ----------
    ...
    renderOperationalReport(payloadOverride){
      ...
      const getPolicyInsuredLabelSafe = (policy) => {
        try{
          if(typeof this.getNewPolicyInsuredLabel === 'function'){
            return this.getNewPolicyInsuredLabel(payload, policy, payload.insureds || []);
          }
        }catch(_e){}
      ...
      const getPolicyCoverageDisplaySafe = (policy) => {
        try{
          if(typeof this.getPolicyCoverageDisplayValue === 'function'){
            return this.getPolicyCoverageDisplayValue(policy || {});
          }
        }catch(_e){}
        const type = safeTrim(policy?.type || policy?.product || '');
      ...
      const getPolicyCoverItemsSafe = (policy) => {
        try{
          if(typeof this.getPolicyCoverItems === 'function'){
      ...
      const getPledgeDetailsSafe = (policy) => {
        if(!policy || !policy.pledge) return null;
        const src = policy?.pledgeBank && typeof policy.pledgeBank === 'object' ? policy.pledgeBank : policy;
        const details = [
          ['בנק', safeTrim(src.bankName)],
          ['מספר בנק', safeTrim(src.bankNo)],
          ['סניף', safeTrim(src.branch)],
          ['סכום לשיעבוד', safeTrim(src.amount)],
          ['שנים', safeTrim(src.years)],
          ['כתובת נכס', safeTrim(src.address)]
        ].filter(([, value]) => value);
        return details.length ? details : [['שיעבוד', 'קיים']];
      };
      const createdAtText = (() => {
      ...
      const newRows = newPolicies.map(p => {
        const insuredLabel = getPolicyInsuredLabelSafe(p);
        const coverageValue = getPolicyCoverageDisplaySafe(p || {});
        const pledgeShort = getPledgeDetailsSafe(p) ? 'יש' : 'אין';
        return [
          ...
          this.renderReportValue(pledgeShort)
        ];
      });

      const newPolicyCards = newPolicies.map((p, idx) => {
        const coverItems = getPolicyCoverItemsSafe(p || {});
        const coverageValue = getPolicyCoverageDisplaySafe(p || {});
        const pledgeDetails = getPledgeDetailsSafe(p || {});
        const summaryPairs = [
          ['מבוטח', getPolicyInsuredLabelSafe(p)],
          ['חברה', safeTrim(p?.company)],
          ['מוצר', safeTrim(p?.type)],
          ['פרמיה חודשית', safeTrim(p?.premiumMonthly)],
          ['אחרי הנחה', this.formatMoneyValue(this.getPolicyPremiumAfterDiscount(p || {}))],
          ['תאריך תחילה', safeTrim(p?.startDate)],
          ['כיסוי / סכום', coverageValue]
        ].filter(([, value]) => safeTrim(value));
        const coverMarkup = coverItems.length
          ? `<div class="lcReportLineList">${coverItems.map(item => `<div class="lcReportLineList__item">${escapeHtml(item)}</div>`).join('')}</div>`
          : `<div class="lcReportMutedBox">${escapeHtml(coverageValue || 'לא הוגדרו כיסויים')}</div>`;
        const pledgeMarkup = pledgeDetails
          ? `<div class="lcReportMiniGrid">${pledgeDetails.map(([label, value]) => `<div><span>${escapeHtml(label)}</span><strong>${this.renderReportValue(value)}</strong></div>`).join('')}</div>`
          : `<div class="lcReportMutedBox">ללא שיעבוד</div>`;
        return `<article class="lcReportPolicyBlock">
          <div class="lcReportPolicyBlock__head">
            <div>
              <div class="lcReportPolicyBlock__eyebrow">פוליסה חדשה ${idx + 1}</div>
              <div class="lcReportPolicyBlock__title">${escapeHtml(safeTrim(p?.company) || 'חברה')} · ${escapeHtml(safeTrim(p?.type) || 'פוליסה')}</div>
              <div class="lcReportPolicyBlock__sub">${this.renderReportValue(getPolicyInsuredLabelSafe(p))}</div>
            </div>
            <span class="lcReportChip ${pledgeDetails ? '' : 'lcReportChip--soft'}">${escapeHtml(pledgeDetails ? 'עם שיעבוד' : 'ללא שיעבוד')}</span>
          </div>
          <div class="lcReportInfoRows">${summaryPairs.map(([label, value]) => `<div class="lcReportInfoRow"><span>${escapeHtml(label)}</span><strong>${this.renderReportValue(value)}</strong></div>`).join('')}</div>
          <div class="lcReportPolicyBlock__split">
            <section class="lcReportPolicyPane">
              <div class="lcReportPolicyPane__title">כיסויים / הרחבות</div>
              ${coverMarkup}
            </section>
            <section class="lcReportPolicyPane">
              <div class="lcReportPolicyPane__title">שיעבוד</div>
              ${pledgeMarkup}
            </section>
          </div>
        </article>`;
      ...
      return `<div class="lcReportDoc lcReportDoc--clean">
        <section class="lcReportShowcase lcReportShowcase--clean">
          <div class="lcReportShowcase__top">
            <div>
              <div class="lcReportShowcase__eyebrow">דוח פנימי למחלקת תפעול</div>
              <h2 class="lcReportShowcase__title">דוח תפעולי מסודר לתיק הלקוח</h2>
              <div class="lcReportShowcase__sub">מבנה נקי לקריאה מהירה, כולל פירוט מלא של פוליסות חדשות, כיסויים ושיעבודים.</div>
            </div>
            <div class="lcReportChipRow">
              <span class="lcReportChip lcReportChip--soft">${escapeHtml(safeTrim(Auth?.current?.name) || 'נציג')}</span>
              <span class="lcReportChip lcReportChip--soft">${escapeHtml(createdAtText)}</span>
            </div>
          </div>
          <div class="lcReportHeroGrid">
            <article class="lcReportPrimeCard lcReportPrimeCard--clean">
              <div class="lcReportPrimeCard__head">
                ...
                <span class="lcReportChip lcReportChip--soft">לקוח פעיל</span>
              </div>
            ...
            <article class="lcReportPrimeCard lcReportPrimeCard--clean lcReportPrimeCard--stats">
              <div class="lcReportPrimeCard__head">
                <div>
                  <div class="lcReportPrimeCard__title">מדדי תפעול מהירים</div>
                  <div class="lcReportPrimeCard__sub">תמונת מצב לפני מעבר לטיפול</div>
                </div>
              </div>
        ...
        <section class="lcReportSection lcReportSection--newPolicies">
          <div class="lcReportSection__title">פוליסות חדשות שנמכרו</div>
          <div class="lcReportSection__sub">טבלת סיכום קצרה ולאחריה פירוט מלא ומסודר לכל פוליסה חדשה.</div>
          ${this.renderTable(['מבוטח','חברה','סוג ביטוח','פרמיה חודשית','אחרי הנחה','תאריך תחילה','כיסוי / סכום','שיעבוד'], newRows)}
          <div class="lcReportPolicyStack">${newPolicyCards.length ? newPolicyCards.join('') : '<div class="muted">לא נוספו פוליסות חדשות.</div>'}</div>
        </section>
    ...
    buildOperationalPdfMarkup(payloadOverride){
      ...
      return {
        ...
        html: `<div class="lcPdfSheet">
          <div class="lcPdfHeader lcPdfHeader--clean">
            <div class="lcPdfHeader__brand">
              ...
              <div>
                ...
                <div class="lcPdfHeader__sub">מסמך מסודר ונקי למחלקת תפעול, כולל פוליסות חדשות, כיסויים ושיעבודים.</div>
              </div>
            </div>
