...
(() => {
  ...
  const BUILD = "20260330-har-vehicle-import-fix-v11";
...
// open health covers drawer (Health only)
    ...
    parseElementaryVehicleHarWorkbook(buffer, ins){
      ...
      const normalizeHeaderCell = (value) => safeTrim(value)
        .replace(/[\n\r\t]+/g, ' ')
        .replace(/\s+/g, ' ')
        .replace(/[״"']/g, '')
        .trim();
      const getHeaderIndex = (header, options=[]) => {
        const normalizedHeader = header.map(normalizeHeaderCell);
        for(const option of options){
          const needle = normalizeHeaderCell(option);
          const exact = normalizedHeader.indexOf(needle);
          if(exact !== -1) return exact;
          const contains = normalizedHeader.findIndex((cell) => cell.includes(needle) || needle.includes(cell));
          if(contains !== -1) return contains;
        }
        return -1;
      };
      const normalizeSection = (value) => safeTrim(value)
        .replace(/[\n\r\t]+/g, ' ')
        .replace(/\s+/g, ' ')
        .replace(/[–—−]/g, '-')
        .trim();
      const isGeneralSection = (value) => {
        const normalized = normalizeSection(value);
        return /תחום\s*-\s*כללי/.test(normalized) || normalized === 'כללי';
      };
      const hasVehicleKeywords = (value) => /רכב|מקיף|חובה|צד\s*ג|צד ג׳|צד ג'|גרירה|שמשות|אופנוע|קטנוע|דו\s*גלגלי/.test(value);
      const hasHomeKeywords = (value) => /דירה|מבנה|תכולה|משכנתא/.test(value);

      const candidateSheets = Array.isArray(wb.SheetNames) ? wb.SheetNames : [];
      let selectedRows = null;
      let selectedHeaderIdx = -1;
      ...
      candidateSheets.forEach((sheetName) => {
        ...
        const headerIdx = rows.findIndex((row) => {
          ...
          return normalized.includes('תעודת זהות') && normalized.includes('מספר פוליסה');
        });
      ...
      if(!selectedRows || selectedHeaderIdx === -1) throw new Error('מבנה קובץ הר הביטוח לא זוהה.');

      const header = selectedRows[selectedHeaderIdx].map((x) => safeTrim(x));
      const idx = {
        idNumber: getHeaderIndex(header, ['תעודת זהות']),
        main: getHeaderIndex(header, ['ענף ראשי']),
        sub: getHeaderIndex(header, ['ענף (משני)', 'ענף משני']),
        productType: getHeaderIndex(header, ['סוג מוצר', 'מוצר']),
        company: getHeaderIndex(header, ['חברה']),
        period: getHeaderIndex(header, ['תקופת ביטוח']),
        extra: getHeaderIndex(header, ['פרטים נוספים']),
        premium: getHeaderIndex(header, ['פרמיה בש"ח', 'פרמיה בשח', 'פרמיה']),
        premiumType: getHeaderIndex(header, ['סוג פרמיה']),
        policyNumber: getHeaderIndex(header, ['מספר פוליסה'])
      };
      let currentSection = '';
      const extracted = [];
      selectedRows.slice(selectedHeaderIdx + 1).forEach((row) => {
        if(!Array.isArray(row)) return;
        const mainCell = safeTrim(row[idx.main]);
        if(normalizeSection(mainCell).startsWith('תחום -')){
        ...
        const content = `${mainCell} ${sub} ${productType} ${extra}`.replace(/\s+/g, ' ').trim();
        const policyNumber = safeTrim(row[idx.policyNumber]);
        if(!policyNumber) return;

        const inGeneralSection = isGeneralSection(currentSection);
        const isVehicle = hasVehicleKeywords(content);
        const isHome = hasHomeKeywords(content);
        if(isHome) return;
        if(!(inGeneralSection && isVehicle) && !isVehicle) return;
        ...
        extracted.push({
          ...
          sourceSection: safeTrim(currentSection)
        });
      });
      return { policies: extracted, fileName:'', sheetName:selectedSheetName };
    },
    ...
    async handleElementaryVehicleHarFile(ins, file){
      ...
      try{
        ...
        if(!policies.length){
          this.setElementaryHarImportState(ins, { status:'empty', count:0, fileName:file.name || '', message:'לא נמצאו פוליסות רכב בקובץ שהועלה. המערכת בדקה תחום כללי וגם שורות רכב עם ניסוחים חלופיים.' });
          this.render();
          return;
        }
        const approved = await showHarBituachImportModal({
          ...
          note: 'המערכת מייבאת רק רשומות רכב ומדלגת על דירה/מבנה/תכולה, גם אם ניסוח השורות בקובץ מעט שונה.',
          confirmText: 'אישור',
          cancelText: 'ביטול'
        });
        if(!approved){
        ...
        await showHarBituachProcessingModal({
          ...
          subtitle:'מאתר רשומות רכב גם כשמבנה השורות בקובץ מעט שונה',
        ...
        this.setElementaryHarImportState(ins, { status:'done', count:policies.length, fileName:file.name || '', importedAt:nowISO(), message:`יובאו ${policies.length} פוליסות רכב. רשומות דירה/מבנה/תכולה לא נמשכו למסך זה.` });
        this.render();
      }catch(err){
