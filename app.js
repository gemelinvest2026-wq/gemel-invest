...
(() => {
  ...
  function renderCompanyCell(name, opt={}){
    const src = companyLogoSrc(name);
    const txt = escapeHtml(String(name||''));
    const showName = (opt.showName !== false);
    if(!src) return txt;
    if(showName){
      return `<span class="coCell"><img class="coLogo" src="${escapeAttr(src)}" alt="${escapeAttr(String(name||''))}"/><span class="coName">${txt}</span></span>`;
    }
    return `<span class="coCell"><img class="coLogo" src="${escapeAttr(src)}" alt="${escapeAttr(String(name||''))}"/></span>`;
  }

  function companyLogoPickerField(label, hiddenId, companies, groupName){
    const gid = escapeAttr(groupName || (hiddenId + '_pick'));
    const hid = escapeAttr(hiddenId);
    const items = (companies||[]).map((name, i) => {
      const id = `${hiddenId}_co_${i}`;
      const src = companyLogoSrc(name);
      const alt = escapeAttr(String(name||''));
      const checked = i===0 ? 'checked' : '';
      return `
        <div class="coPickItem">
          <input class="coPickRadio" type="radio" id="${escapeAttr(id)}" name="${gid}" value="${escapeAttr(String(name||''))}" data-co-pick="${hid}" ${checked}>
          <label class="coPickBtn" for="${escapeAttr(id)}" title="${alt}">
            ${src ? `<img class="coLogo coPickLogo" src="${escapeAttr(src)}" alt="${alt}">` : `<span class="coPickFallback">${escapeHtml(String(name||''))}</span>`}
          </label>
        </div>
      `;
    }).join('');
    ...
    return `
      <div class="field">
        <div class="label">${escapeHtml(label||'חברה')}</div>
        <input type="hidden" id="${hid}" value="">
        <div class="coPicker" role="radiogroup" aria-label="${escapeAttr(label||'חברה')}">
          ${items}
        </div>
      </div>
    `;
  ...
  function wireCompanyLogoPicker(hiddenId){
    const hid = String(hiddenId||'');
    const hidden = document.getElementById(hid);
    if(!hidden) return;

    const radios = $$(`input[type="radio"][data-co-pick="${CSS.escape(hid)}"]`);
    ...
    radios.forEach(r => r.addEventListener('change', () => { sync(); markDraft(); }));
    // initial
    sync();
  }
...
// Keep wizard title in sync
  ...
  function renderOldPoliciesStep(){
    ...
    wizardMain.innerHTML = `
      ...
      <div class="grid2">
        ${companyLogoPickerField('חברה', 'old_company', ['הראל','כלל','מנורה','הפניקס','מגדל','הכשרה'], 'old_company_pick')}
          ...
          <tbody>
            ${rows.length ? rows.map(({x,idx}) => {
              ...
              if(dec==='לביטול') dec='cancel_full';
                            return `
              <tr>
                <td>${renderCompanyCell(x.company||'',{showName:false})}</td>
...
function renderNewPoliciesStep(){
    ...
    wizardMain.innerHTML = `
      ...
      <div class="grid3">
        ${companyLogoPickerField('חברה', 'new_company', ['הראל','כלל','מנורה','הפניקס','מגדל','הכשרה'], 'new_company_pick')}
