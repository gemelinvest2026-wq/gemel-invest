...
(() => {
  ...
  const Storage = {
    ...
    // extra safety: try to recover from any older/alternate backup keys (prevents "נשאר רק מנהל מערכת")
    loadBackupAny(){
      const direct = this.loadBackup();
      ...
      try {
        ...
        for(let i=0;i<localStorage.length;i++){
          ...
          if(!k) continue;
          if(k === LS_BACKUP_KEY) continue;
          if(/^GEMEL_STATE_BACKUP/i.test(k)) keys.push(k);
        }
        for(const k of keys){
          try{
            ...
            if(st && Array.isArray(st.agents) && st.agents.length) return st;
          }catch(_e){}
        }
      } catch(_) {}
      return null;
  ...
  const Auth = {
    ...
    async _submit(){
      ...
      if (masterOk || (adminAuth.active !== false && username === safeTrim(adminAuth.username) && pin === safeTrim(adminAuth.pin))) {
        ...
        UI.renderAuthPill();
        UI.goView("settings");
        // refresh state from Sheets after login (prevents missing users if boot happened before login)
        setTimeout(() => { try { App.syncNow(); } catch(_){} }, 200);
        return;
      }
...
init(){
    ...
    renderBody(){
      ...
      else if(this.step === 6) body = this.renderStep6();
    ...
    bindHealthDecl(){
      const q = this.hdQuestions[Math.max(0, Math.min(this.hdQuestions.length-1, this.hdIndex || 0))];
      if(!q) return;

      // per insured yes/no
      $$('[data-hd][data-ins]', this.els.body).forEach(btn => {
        on(btn, 'click', () => {
          const insId = btn.getAttribute('data-ins');
          const mode = btn.getAttribute('data-hd');
          const ins = this.insureds.find(x => x.id === insId);
          if(!ins) return;
          ins.data = ins.data || {};
          ins.data.healthDecl = ins.data.healthDecl || { answers: {} };
          ins.data.healthDecl.answers = ins.data.healthDecl.answers || {};
          ins.data.healthDecl.answers[q.id] = ins.data.healthDecl.answers[q.id] || { v:null, details:'' };
          const a = ins.data.healthDecl.answers[q.id];
          a.v = (mode === 'yes');
          if(a.v === false) a.details = '';
          this.setHint('');
          this.render();
        });
      });
      ...
      // bulk yes/no
      $$('[data-hd-all]', this.els.body).forEach(btn => {
        on(btn, 'click', () => {
          const mode = btn.getAttribute('data-hd-all');
          const v = (mode === 'yes');
          this.insureds.forEach(ins => {
            ...
            ins.data.healthDecl.answers = ins.data.healthDecl.answers || {};
            ins.data.healthDecl.answers[q.id] = ins.data.healthDecl.answers[q.id] || { v:null, details:'' };
            ins.data.healthDecl.answers[q.id].v = v;
            if(!v) ins.data.healthDecl.answers[q.id].details = '';
          });
          this.setHint('');
          this.render();
        });
      ...
      // details typing
      $$('[data-hd-details]', this.els.body).forEach(input => {
        on(input, 'input', () => {
          const insId = input.getAttribute('data-hd-details');
          const ins = this.insureds.find(x => x.id === insId);
          if(!ins) return;
          ins.data.healthDecl = ins.data.healthDecl || { answers: {} };
          const a = ins.data.healthDecl.answers?.[q.id];
          if(!a) return;
          a.details = safeTrim(input.value);
        });
        on(input, 'change', () => {
          const insId = input.getAttribute('data-hd-details');
          const ins = this.insureds.find(x => x.id === insId);
      ...
      // question nav inside step
      $$('[data-hd-nav]', this.els.body).forEach(btn => {
        on(btn, 'click', () => {
          const dir = btn.getAttribute('data-hd-nav');
          if(dir === 'prev') this.hdIndex = Math.max(0, (this.hdIndex||0) - 1);
          if(dir === 'next') this.hdIndex = Math.min(this.hdQuestions.length-1, (this.hdIndex||0) + 1);
          this.render();
        });
      });
    },
...
// ---------- Step 6 (Health Declaration) ----------
    renderStep6(){
      const q = this.hdQuestions[Math.max(0, Math.min(this.hdQuestions.length-1, this.hdIndex || 0))];
      const qPos = (this.hdIndex || 0) + 1;
      const total = this.hdQuestions.length;

      // ensure per-insured structure
      this.insureds.forEach(ins => {
        ins.data = ins.data || {};
        ins.data.healthDecl = ins.data.healthDecl || { answers: {} };
        ins.data.healthDecl.answers = ins.data.healthDecl.answers || {};
        if (!ins.data.healthDecl.answers[q.id]) ins.data.healthDecl.answers[q.id] = { v: null, details: "" };
      });

      const allNo = () => {
        this.insureds.forEach(ins => {
          ins.data.healthDecl.answers[q.id].v = false;
          ins.data.healthDecl.answers[q.id].details = "";
        });
        this.render();
      };
      const allYes = () => {
        this.insureds.forEach(ins => {
          ins.data.healthDecl.answers[q.id].v = true;
        });
        this.render();
      };

      const rows = this.insureds.map(ins => {
        const a = ins.data.healthDecl.answers[q.id] || { v:null, details:"" };
        const yesCls = a.v === true ? "is-active" : "";
        const noCls  = a.v === false ? "is-active" : "";
        const needDetails = (a.v === true);
        const details = needDetails ? `
          <div class="field" style="margin-top:8px">
            <label class="label">פירוט חובה</label>
            <input class="input" type="text" data-hd-details="${ins.id}" value="${escapeHtml(a.details||"")}" placeholder="אבחנה/שנה/טיפול/מצב נוכחי…" />
            <div class="help">אם סומן כן — חובה למלא פירוט קצר.</div>
          </div>` : "";

        return `
          <div class="lcGlassCard" style="padding:12px; margin:10px 0">
            <div class="row row--between" style="gap:10px; align-items:center">
              <div>
                <div style="font-weight:800">${escapeHtml(ins.label)}</div>
                <div class="muted small">${escapeHtml((safeTrim(ins.data.firstName)+" "+safeTrim(ins.data.lastName)).trim() || "")}</div>
              </div>
              <div class="row" style="gap:8px">
                <button type="button" class="btn btn--ghost ${noCls}" data-hd="no" data-ins="${ins.id}">לא</button>
                <button type="button" class="btn ${yesCls}" data-hd="yes" data-ins="${ins.id}">כן</button>
              </div>
            </div>
            ${details}
          </div>
      ...
      const innerPrevDisabled = (this.hdIndex || 0) <= 0;
      const innerNextDisabled = (this.hdIndex || 0) >= (total - 1);
        ...
        <div class="lcWSection">
          <div class="row row--between" style="gap:10px">
            <div>
              ...
              <div class="muted small">שאלה ${qPos} מתוך ${total} • מצב A (שאלה אחת + סימון למבוטחים)</div>
            </div>
            <div class="row" style="gap:8px">
              <button type="button" class="btn btn--ghost" data-hd-all="no">לא לכולם</button>
              <button type="button" class="btn btn--ghost" data-hd-all="yes">כן לכולם</button>
            </div>
          </div>

          <div class="divider"></div>

          <div class="lcGlassCard" style="padding:14px">
            <div style="font-weight:900; font-size:16px">${escapeHtml(q.title)}</div>
            <div class="muted" style="margin-top:6px; line-height:1.55">${escapeHtml(q.text)}</div>
          </div>
          ...
          <div class="row row--between" style="gap:10px; margin-top:12px">
            <button type="button" class="btn btn--ghost" data-hd-nav="prev" ${innerPrevDisabled?"disabled":""}>הקודם</button>
            <button type="button" class="btn" data-hd-nav="next" ${innerNextDisabled?"disabled":""}>השאלה הבאה</button>
          </div>

          <div class="help" style="margin-top:10px">כדי להמשיך לסיכום (שלב 7) חובה לענות על כל השאלות ולמלא פירוט בכל "כן".</div>
        </div>
      `;
    },
    ...
    validateStep(stepId){
      ...
      if(stepId === 6){
        const missing = [];
        const totalQ = this.hdQuestions.length;
        for(const ins of this.insureds){
          const hd = ins.data?.healthDecl;
          const ans = hd?.answers || {};
          let ok = true;
          for(const q of this.hdQuestions){
            const a = ans[q.id];
            if(!a || (a.v !== true && a.v !== false)) { ok = false; break; }
            if(a.v === true && !safeTrim(a.details)) { ok = false; break; }
          }
          if(!ok) missing.push(ins.label);
        }
        if(!missing.length) return { ok:true };
        return { ok:false, msg: `חסרים תשובות/פירוט בהצהרת בריאות עבור: ${missing.join(", ")}. חובה להשלים ${totalQ} שאלות.` };
      }
    ...
    isStepCompleteForInsured(stepId, ins){
      ...
      if(stepId === 6){
        const hd = d.healthDecl;
        const ans = hd?.answers || {};
        for(const q of this.hdQuestions){
          const a = ans[q.id];
          if(!a || (a.v !== true && a.v !== false)) return false;
          if(a.v === true && !safeTrim(a.details)) return false;
        }
        return true;
      }
...
const App = {
    ...
    async boot(){
      ...
      if (r.ok) {
        ...
        const backup = Storage.loadBackupAny();
        if (backup) {
          State.data = backup;
        } else {
