...
// ===== Phoenix original PDF engine (Step 8 override) =====
Object.assign(Wizard, {
  phoenixForms: {
    health: { key:'health', path:'assets/forms/phoenix/phoenix-health-critical-illness.pdf', title:'הפניקס · בריאות / מחלות קשות', description:'הטופס המקורי של הפניקס לביטוח בריאות. אם קיימת בעסקה פוליסת בריאות — זהו טופס האם שממנו יתר ההצעות יורשות נתונים.', questionnairePath:'assets/forms/phoenix/phoenix-questionnaires.pdf' },
    ...
    mortgage_short: { key:'mortgage_short', path:'assets/forms/phoenix/phoenix-mortgage-up-to-2m.pdf', title:'הפניקס · משכנתא עד 2 מיליון', description:'הטופס המקורי של הפניקס למשכנתא עד 2 מיליון. הקובץ נפתח גדול וברור, אך משום שאין בו שדות PDF מובנים לא ניתן לבצע עליו עריכת PDF מלאה בלי מיפוי ידני.', questionnairePath:'assets/forms/phoenix/phoenix-questionnaires.pdf' },
    mortgage_long: { key:'mortgage_long', path:'assets/forms/phoenix/phoenix-mortgage-over-2m.pdf', title:'הפניקס · משכנתא מעל 2 מיליון', description:'הטופס המקורי של הפניקס למשכנתא מעל 2 מיליון. הקובץ נפתח גדול וברור, אך משום שאין בו שדות PDF מובנים לא ניתן לבצע עליו עריכת PDF מלאה בלי מיפוי ידני.', questionnairePath:'assets/forms/phoenix/phoenix-questionnaires.pdf' }
  },
  ...
  getPhoenixHealthState(){
    ...
    if(!st.confirmed) st.confirmed = false;
    if(!st.lastGeneratedAt) st.lastGeneratedAt = '';
    if(!st.prefillSnapshot) st.prefillSnapshot = {};
    if(!st.pdfOpened) st.pdfOpened = false;
    return st;
  },
  ...
  getPrimaryInsuredAge(){
    return Number(this.calcAge(this.insureds?.[0]?.data?.birthDate) || 0) || 0;
  },
  ...
  choosePhoenixPrimaryForm(){
    ...
    const risk = policies.find(p => ['ריסק','ריסק משכנתא'].includes(safeTrim(p.type)));
    if(risk){
      const amount = this.parseMoneyNumber(risk.sumInsured);
      const age = this.getPrimaryInsuredAge();
      const isMortgage = safeTrim(risk.type) === 'ריסק משכנתא';
      if(isMortgage){
    ...
    const ci = policies.find(p => ['מחלות קשות','סרטן'].includes(safeTrim(p.type)));
  ...
  buildPhoenixPrefillSnapshot(formDef){
    const primary = this.insureds?.[0]?.data || {};
    const spouseIns = this.insureds?.find(x => x.type === 'spouse');
    ...
    const address = [safeTrim(primary.street), safeTrim(primary.houseNumber)].filter(Boolean).join(' ');
    return {
      generatedAt: nowISO(),
      formKey: formDef?.key || '',
      fullName, spouseFullName,
      firstName: safeTrim(primary.firstName), lastName: safeTrim(primary.lastName), idNumber: safeTrim(primary.idNumber),
      spouseFirstName: safeTrim(spouse.firstName), spouseLastName: safeTrim(spouse.lastName), spouseId: safeTrim(spouse.idNumber),
      birthDate: safeTrim(primary.birthDate), spouseBirthDate: safeTrim(spouse.birthDate),
      age: this.calcAge(primary.birthDate), spouseAge: this.calcAge(spouse.birthDate),
      gender: safeTrim(primary.gender), phone: safeTrim(primary.phone), email: safeTrim(primary.email), city: safeTrim(primary.city),
      address, zip: safeTrim(primary.zip), clinic: safeTrim(primary.clinic), shaban: safeTrim(primary.shaban), occupation: safeTrim(primary.occupation),
      heightCm: safeTrim(primary.heightCm), weightKg: safeTrim(primary.weightKg), bmi: safeTrim(primary.bmi),
      smoker: /כן|מעשן/i.test(String(primary.isSmoking || primary.smoking || primary.smoker || '')),
      smokingAmount: safeTrim(primary.smokingAmount || primary.cigarettesPerDay || ''), stopSmokingYear: safeTrim(primary.stopSmokingYear || ''),
      spouseHeightCm: safeTrim(spouse.heightCm), spouseWeightKg: safeTrim(spouse.weightKg),
      spouseSmoker: /כן|מעשן/i.test(String(spouse.isSmoking || spouse.smoking || spouse.smoker || '')),
  ...
  resolvePhoenixSourcePath(sourcePath){
    ...
    const uniq = [];
    variants.forEach(v => { if(v && !uniq.includes(v)) uniq.push(v); });
    return uniq[0] || clean;
  },

  async createFilledPhoenixPdf(formDef, questionnaireMode=false){
    if(!formDef) return { url:'', usedOriginal:true };
    const sourcePath = questionnaireMode ? formDef.questionnairePath : formDef.path;
    const st = this.getPhoenixHealthState();
    st.prefillSnapshot = this.buildPhoenixPrefillSnapshot(formDef);

    // טעינה יציבה: פותחים תמיד את ה-PDF המקורי מהתיקייה במקום להיתקע על fetch/PDFLib.
    // כך לפחות המסמך יעלה בוודאות בדפדפן אם הנתיב קיים בריפו.
    const resolvedSourcePath = this.resolvePhoenixSourcePath(sourcePath);
    return { url: resolvedSourcePath, usedOriginal:true, fetchFailed:false };
  },
  ...
  async openPhoenixPdf(questionnaireMode=false){
    ...
    try{
      ...
      this.setHint(questionnaireMode ? 'שאלוני ההמשך של הפניקס נטענו.' : 'הטופס המקורי של הפניקס נטען בהצלחה.');
      this.render();
    }catch(err){
      console.error('PHOENIX_PDF_OPEN_ERROR', err);
      this.setHint('טעינת טופס הפניקס נכשלה. בדוק שהקבצים נמצאים ב-assets/forms/phoenix ושהמערכת לא חוסמת גישה לקובצי PDF.');
    }
  },
...
// Phoenix form autosave
document.addEventListener('input',function(e){
  const f=e.target;
  if(!f.closest('.phoenix-form')) return;
  const data=JSON.parse(localStorage.getItem('phoenixFormData')||"{}");
  data[f.name||f.id||Math.random()] = f.value;
  localStorage.setItem('phoenixFormData',JSON.stringify(data));
});

function loadPhoenixFormState(){
  const data=JSON.parse(localStorage.getItem('phoenixFormData')||"{}");
  document.querySelectorAll('.phoenix-form input,.phoenix-form textarea').forEach(el=>{
     const k=el.name||el.id;
     if(k && data[k]!=null) el.value=data[k];
  });
}


// Auto load phoenix form when step 8 opens
function autoLoadPhoenixForm(){
   const btn=document.querySelector('[data-load-phoenix]');
   if(btn) btn.click();
}
document.addEventListener('DOMContentLoaded',()=>{
 setTimeout(autoLoadPhoenixForm,500);
});
