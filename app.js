  ...
  const MirrorsUI = {
    ...
    init(){
      ...
      on(this.els.healthCard, "click", (ev) => {
        const rec = this.current();
        if(!rec) return;
        const ansBtn = ev.target?.closest?.('[data-mirror-health-answer]');
        if(ansBtn){
          const qKey = safeTrim(ansBtn.getAttribute('data-mirror-health-question'));
          const insId = safeTrim(ansBtn.getAttribute('data-mirror-health-insured'));
          const answer = safeTrim(ansBtn.getAttribute('data-mirror-health-answer'));
          if(!qKey || !insId || !answer) return;
          if(answer === 'no') this.setMirrorHealthResponse(rec, qKey, insId, { answer:'no', fields:{}, saved:false });
          else {
            const prev = this.getMirrorHealthResponse(rec, qKey, insId);
            this.setMirrorHealthResponse(rec, qKey, insId, { answer:'yes', saved:false, fields: prev.fields || {} });
          }
          rec.updatedAt = nowISO();
          this.render();
          return;
        ...
        if(saveBtn){
          const qKey = safeTrim(saveBtn.getAttribute('data-mirror-health-question'));
          const insId = safeTrim(saveBtn.getAttribute('data-mirror-health-insured'));
          if(qKey && insId) this.saveMirrorHealthQuestion(qKey, insId);
          return;
        }
        const editBtn = ev.target?.closest?.('[data-mirror-health-edit]');
        if(editBtn){
          const qKey = safeTrim(editBtn.getAttribute('data-mirror-health-question'));
          const insId = safeTrim(editBtn.getAttribute('data-mirror-health-insured'));
          if(qKey && insId){
            this.setMirrorHealthResponse(rec, qKey, insId, { answer:'yes', saved:false });
            rec.updatedAt = nowISO();
            this.render();
          }
          return;
        }
        const clearBtn = ev.target?.closest?.('[data-mirror-health-clear]');
        if(clearBtn){
          const qKey = safeTrim(clearBtn.getAttribute('data-mirror-health-question'));
          const insId = safeTrim(clearBtn.getAttribute('data-mirror-health-insured'));
          if(qKey && insId){
            this.setMirrorHealthResponse(rec, qKey, insId, { answer:'', fields:{}, saved:false });
            rec.updatedAt = nowISO();
            this.render();
          }
          return;
        }
        const finalSaveBtn = ev.target?.closest?.('[data-mirror-health-final-save]');
        if(finalSaveBtn) this.saveMirrorHealth();
      });
      on(this.els.healthCard, "input", (ev) => {
        const rec = this.current();
        if(!rec) return;
        const field = ev.target?.closest?.('[data-mirror-health-field]');
        if(!field) return;
        const qKey = safeTrim(field.getAttribute('data-mirror-health-question'));
        const insId = safeTrim(field.getAttribute('data-mirror-health-insured'));
        const fieldKey = safeTrim(field.getAttribute('data-mirror-health-field'));
        if(!qKey || !insId || !fieldKey) return;
        const prev = this.getMirrorHealthResponse(rec, qKey, insId);
        const fields = { ...(prev.fields || {}) };
        fields[fieldKey] = safeTrim(field.value);
        this.setMirrorHealthResponse(rec, qKey, insId, { answer:'yes', saved:false, fields });
        rec.updatedAt = nowISO();
      });
    ...
    getMirrorHealthInsureds(rec){
      return this.getInsureds(rec).map((ins, index) => ({
        id: safeTrim(ins?.id) || `mirror_ins_${index}`,
        label: this.getInsuredLabel(ins, index),
        type: safeTrim(ins?.type) || (index === 0 ? 'primary' : 'other'),
        data: ins?.data || {}
      }));
    },

    getMirrorHealthStore(rec){
      const insureds = this.getInsureds(rec);
      ...
      if(!store.ui) store.ui = { currentIndex: 0, summary: false };
      if(!store.responses) store.responses = {};
      return store;
    },

    buildMirrorHealthContext(rec){
      const ctx = {
        newPolicies: JSON.parse(JSON.stringify(this.getNewPolicies(rec) || [])),
        insureds: this.getMirrorHealthInsureds(rec),
        parseMoneyNumber: (...args) => Wizard.parseMoneyNumber.apply(Wizard, args),
        getPolicyLabel: (...args) => Wizard.getPolicyLabel.apply(Wizard, args),
        buildPhoenixQuestionnaireCatalog: (...args) => Wizard.buildPhoenixQuestionnaireCatalog.apply(Wizard, args),
        buildPhoenixFollowupFields: (...args) => Wizard.buildPhoenixFollowupFields.apply(Wizard, args),
        getPhoenixFollowupSchemas: (...args) => Wizard.getPhoenixFollowupSchemas.apply(Wizard, args),
        calcAge: (...args) => Wizard.calcAge.apply(Wizard, args),
      };
      ctx.getHealthCompanies = () => Wizard.getHealthCompanies.apply(ctx);
      ctx.getPhoenixHealthSchema = () => Wizard.getPhoenixHealthSchema.apply(ctx);
      ctx.getHealthSchema = () => Wizard.getHealthSchema.apply(ctx);
      ctx.getHealthQuestionsFiltered = () => Wizard.getHealthQuestionsFiltered.apply(ctx);
      ctx.getHealthQuestionList = () => Wizard.getHealthQuestionList.apply(ctx);
      return ctx;
    },

    getMirrorHealthQuestionList(rec){
      return this.buildMirrorHealthContext(rec).getHealthQuestionList();
    },

    getMirrorHealthResponse(rec, qKey, insId){
      const store = this.getMirrorHealthStore(rec);
      const qBlock = store.responses[qKey] || {};
      const out = qBlock[insId] || { answer:'', fields:{}, saved:false };
      if(!out.fields) out.fields = {};
      return out;
    ...
    setMirrorHealthResponse(rec, qKey, insId, patch){
      const store = this.getMirrorHealthStore(rec);
      store.responses[qKey] = store.responses[qKey] || {};
      const prev = this.getMirrorHealthResponse(rec, qKey, insId);
      store.responses[qKey][insId] = {
    ...
    summarizeMirrorHealthFields(fields){
      const vals = Object.values(fields || {}).map(v => safeTrim(v)).filter(Boolean);
      if(!vals.length) return 'נשמר';
      return vals.slice(0,2).join(' • ');
    },

    validateMirrorHealthDetail(question, rec, insId){
      const r = this.getMirrorHealthResponse(rec, question.key, insId);
      if(r.answer !== 'yes') return true;
      const required = (question.fields || []).filter(f => f.type !== 'section');
      if(!required.length) return true;
      return required.every(f => safeTrim(r.fields?.[f.key]));
    ...
    renderMirrorHealthField(rec, question, insId, field){
      if(field.type === 'section') return `<div class="mirrorsHealthSectionTitle">${escapeHtml(field.label)}</div>`;
      const r = this.getMirrorHealthResponse(rec, question.key, insId);
      const val = safeTrim(r.fields?.[field.key] || '');
      if(field.type === 'textarea'){
        return `<label class="mirrorsHealthField mirrorsHealthField--full"><span>${escapeHtml(field.label)}</span><textarea class="textarea mirrorsHealthTextarea" rows="3" data-mirror-health-question="${escapeHtml(question.key)}" data-mirror-health-insured="${escapeHtml(insId)}" data-mirror-health-field="${escapeHtml(field.key)}">${escapeHtml(val)}</textarea></label>`;
      }
      return `<label class="mirrorsHealthField"><span>${escapeHtml(field.label)}</span><input class="input" type="text" value="${escapeHtml(val)}" data-mirror-health-question="${escapeHtml(question.key)}" data-mirror-health-insured="${escapeHtml(insId)}" data-mirror-health-field="${escapeHtml(field.key)}" /></label>`;
    },

    getMirrorHealthBlockingIssue(rec){
      const list = this.getMirrorHealthQuestionList(rec);
      const insureds = this.getMirrorHealthInsureds(rec);
      if(!list.length) return { ok:false, msg:'לא נמצאה הצהרת בריאות שמורה בטופס המקורי של הלקוח.' };
      for(const item of list){
        for(const ins of insureds){
          const r = this.getMirrorHealthResponse(rec, item.question.key, ins.id);
          if(r.answer !== 'yes' && r.answer !== 'no') return { ok:false, msg:`חסרה תשובה בהצהרת הבריאות עבור ${ins.label}` };
          if(r.answer === 'yes'){
            if(!this.validateMirrorHealthDetail(item.question, rec, ins.id)) return { ok:false, msg:`יש להשלים את כל שדות התת־שאלון עבור ${ins.label}` };
    ...
    async saveMirrorHealthQuestion(qKey, insId){
      const rec = this.current();
      if(!rec) return;
      const item = this.getMirrorHealthQuestionList(rec).find(x => x.question.key === qKey);
      if(!item) return;
      if(!this.validateMirrorHealthDetail(item.question, rec, insId)){
        alert('יש למלא את כל שדות התת־שאלון לפני שמירה.');
        return;
      }
      this.setMirrorHealthResponse(rec, qKey, insId, { answer:'yes', saved:true });
      rec.updatedAt = nowISO();
      this.render();
    },
    ...
    renderHealth(rec){
      ...
      const insureds = this.getMirrorHealthInsureds(rec);
      const store = this.getMirrorHealthStore(rec);
      const hasSavedDeclaration = Object.keys(store.responses || {}).length > 0;
      const cards = list.length ? list.map((item, idx) => {
        const q = item.question || {};
        const rows = insureds.map(ins => {
          const r = this.getMirrorHealthResponse(rec, q.key, ins.id);
          const yes = r.answer === 'yes';
          const no = r.answer === 'no';
          const valid = this.validateMirrorHealthDetail(q, rec, ins.id);
          const savedBox = yes && r.saved ? `<div class="mirrorsHealthSaved"><span class="mirrorsHealthSaved__ok">✓ נשמר עבור ${escapeHtml(ins.label)}</span><span class="mirrorsHealthSaved__meta">${escapeHtml(this.summarizeMirrorHealthFields(r.fields || {}))}</span><div class="mirrorsHealthSaved__actions"><button class="btn" data-mirror-health-edit type="button" data-mirror-health-question="${escapeHtml(q.key)}" data-mirror-health-insured="${escapeHtml(ins.id)}">ערוך</button><button class="btn btn--danger" data-mirror-health-clear type="button" data-mirror-health-question="${escapeHtml(q.key)}" data-mirror-health-insured="${escapeHtml(ins.id)}">נקה</button></div></div>` : '';
          const editor = yes && !r.saved ? `<div class="mirrorsHealthDetail"><div class="mirrorsHealthDetail__title">פירוט עבור: ${escapeHtml(ins.label)}</div><div class="mirrorsHealthFields">${(q.fields || []).map(field => this.renderMirrorHealthField(rec, q, ins.id, field)).join('')}</div><div class="mirrorsHealthDetail__foot"><button class="btn btn--primary" data-mirror-health-save type="button" data-mirror-health-question="${escapeHtml(q.key)}" data-mirror-health-insured="${escapeHtml(ins.id)}">שמור</button>${!valid ? `<span class="mirrorsHealthInlineWarn">יש למלא את כל שדות התת־שאלון לפני שמירה</span>` : ''}</div></div>` : '';
          return `<div class="mirrorsHealthMatrixRow ${yes ? 'is-yes' : no ? 'is-no' : ''}"><div class="mirrorsHealthMatrixRow__who">${escapeHtml(ins.label)}</div><div class="mirrorsHealthAnswerBtns"><button class="mirrorsMiniChoice${yes ? ' is-selected' : ''}" type="button" data-mirror-health-answer="yes" data-mirror-health-question="${escapeHtml(q.key)}" data-mirror-health-insured="${escapeHtml(ins.id)}">כן</button><button class="mirrorsMiniChoice${no ? ' is-selected' : ''}" type="button" data-mirror-health-answer="no" data-mirror-health-question="${escapeHtml(q.key)}" data-mirror-health-insured="${escapeHtml(ins.id)}">לא</button></div><div class="mirrorsHealthMatrixRow__content">${savedBox}${editor}</div></div>`;
        }).join('');
        return `<article class="mirrorsHealthQuestionCard" style="animation-delay:${idx * 40}ms"><div class="mirrorsHealthQuestionCard__head"><div><div class="mirrorsHealthQuestionCard__eyebrow">${escapeHtml(item.catTitle || 'הצהרת בריאות')}</div><div class="mirrorsHealthQuestionCard__title">${escapeHtml(q.text || 'שאלה')}</div></div><span class="mirrorsSummaryBadge">שאלה ${idx+1} / ${list.length}</span></div><div class="mirrorsHealthQuestionCard__body">${rows}</div></article>`;
      }).join('') : `<div class="mirrorsReflectNote">לא נמצאה הצהרת בריאות תואמת בפוליסות החדשות של הלקוח.</div>`;
      const savedNote = safeTrim(store.savedAt) ? `<div class="mirrorsDisclosureSaved">השלב נשמר בתאריך ${escapeHtml(this.formatDate(store.savedAt))}${safeTrim(store.savedBy) ? ` · על ידי ${escapeHtml(store.savedBy)}` : ''}</div>` : '';
      const hint = hasSavedDeclaration ? 'המערכת שלפה את הצהרת הבריאות והשאלונים מהטופס המקורי. ניתן לערוך, לעדכן ולשמור מחדש מתוך מסך השיקוף.' : 'לא נמצאו תשובות שמורות בהצהרת הבריאות המקורית. ניתן למלא ולעדכן כאן מתוך שלב השיקוף.';
      this.els.healthCard.innerHTML = `<div class="mirrorsCard__head"><div><div class="mirrorsCard__title">הצהרת בריאות</div><div class="mirrorsCard__hint">${escapeHtml(hint)}</div></div><span class="mirrorsSummaryBadge">שלב 7 · הצהרה ושאלונים</span></div><div class="mirrorsDisclosureIntro">ענה או עדכן כל שאלה בדיוק לפי הטופס המקורי של החברה. תשובה "כן" מחייבת פירוט ושמירה של התת־שאלון.</div><div class="mirrorsHealthList">${cards}</div>${savedNote}<div class="mirrorsReflectActions"><button class="btn btn--primary" data-mirror-health-final-save type="button">שמור שלב הצהרת בריאות</button></div>`;
    },

    async saveMirrorHealth(){
      const rec = this.current();
      if(!rec) return;
      const issue = this.getMirrorHealthBlockingIssue(rec);
      if(!issue.ok){
        alert(issue.msg);
        return;
      }
