  ...
  const CustomersUI = {
    ...
    render(){
      ...
      UI.els.customersTbody.querySelectorAll("[data-open-customer]").forEach(btn => {
        on(btn, "click", () => this.openById(btn.getAttribute("data-open-customer")));
      });
    ...
    openById(id){
      ...
      // open first so a render issue inside the premium cards will not block the modal itself
      ...
      if(this.els.meta){
        this.els.meta.innerHTML = [
          ...
          rec.email ? `אימייל: <strong>${escapeHtml(rec.email)}</strong>` : ""
        ].filter(Boolean).map(x => `<span>${x}</span>`).join("");
      }
      if(this.els.dash){
        this.els.dash.innerHTML = "";
        this.els.dash.style.display = "none";
      }
      if(this.els.body){
        try{
          const insuredCards = (payload.insureds || []).map(ins => {
            const d = ins.data || {};
            return `<div class="lcCfMiniCard">
              <div class="lcCfMiniCard__title">${escapeHtml(ins.label || ins.type || "מבוטח")}</div>
              <div class="lcCfInfoGrid">
                ${this.infoLine("שם מלא", ((d.firstName || "") + " " + (d.lastName || "")).trim())}
                ${this.infoLine("ת״ז", d.idNumber)}
                ${this.infoLine("תאריך לידה", d.birthDate)}
                ${this.infoLine("טלפון", d.phone, true)}
                ${this.infoLine("קופת חולים", d.clinic)}
                ${this.infoLine("שב״ן", d.shaban)}
                ${this.infoLine("עיסוק", d.occupation)}
              </div>
            </div>`;
          }).join("");

          const existingCards = this.renderExistingPoliciesCards(payload.insureds || []);
          const newCards = this.renderNewPoliciesCards(payload.newPolicies || [], payload.insureds || []);

          this.els.body.innerHTML = `
            <div class="lcCustomerFile">
              <section class="lcCfSection">
                <div class="lcCfSection__head">
                  <div class="lcCfSection__title">פרטי לקוח ראשיים</div>
                  <div class="lcCfSection__sub">כרטיס 360 בסיסי שנשמר אוטומטית מתוך תהליך ההקמה</div>
                </div>
                <div class="lcCfInfoGrid lcCfInfoGrid--primary">
                  ${this.infoLine("שם מלא", rec.fullName)}
                  ${this.infoLine("ת״ז", rec.idNumber)}
                  ${this.infoLine("טלפון", rec.phone, true)}
                  ${this.infoLine("אימייל", rec.email, true)}
                  ${this.infoLine("עיר", rec.city)}
                  ${this.infoLine("סטטוס", rec.status)}
                  ${this.infoLine("נציג מטפל", rec.agentName)}
                  ${this.infoLine("עודכן", this.formatDate(rec.updatedAt || rec.createdAt))}
                </div>
              </section>

              <section class="lcCfSection">
                <div class="lcCfSection__head">
                  <div class="lcCfSection__title">מבוטחים בתיק</div>
                  <div class="lcCfSection__sub">כל המבוטחים שנבנו בוויזארד</div>
                </div>
                <div class="lcCfMiniCards">${insuredCards || `<div class="muted">אין מבוטחים להצגה.</div>`}</div>
              </section>

              <section class="lcCfSection lcCfSection--policies">
                <div class="lcCfSection__head">
                  <div class="lcCfSection__title">פוליסות קיימות</div>
                  <div class="lcCfSection__sub">תצוגה מלאה ומסודרת של כל הנתונים שנשמרו בשלב פוליסות קיימות</div>
                </div>
                ${existingCards}
              </section>
              ...
              <section class="lcCfSection lcCfSection--policies">
                <div class="lcCfSection__head">
                  <div class="lcCfSection__title">פוליסות חדשות</div>
                  <div class="lcCfSection__sub">הפוליסות החדשות שהוקמו בתהליך עם כל הפרטים שנשמרו</div>
                </div>
                ${newCards}
              </section>
              ...
              <section class="lcCfSection">
                <div class="lcCfSection__head">
                  <div class="lcCfSection__title">תשלום ומשלם</div>
                  <div class="lcCfSection__sub">פרטי משלם שנשמרו בשלב התשלום</div>
                </div>
                <div class="lcCfInfoGrid">
                  ${this.infoLine("סוג משלם", primary.payerChoice === "external" ? "משלם חיצוני" : "מבוטח קיים")}
                  ${this.infoLine("אמצעי תשלום", primary.paymentMethod === "ho" ? "הוראת קבע" : "כרטיס אשראי")}
                  ${this.infoLine("שם מחזיק", primary.cc?.holderName || primary.externalPayer?.firstName || "")}
                  ${this.infoLine("תז מחזיק", primary.cc?.holderId || primary.externalPayer?.idNumber || "")}
                  ${this.infoLine("טלפון משלם", primary.externalPayer?.phone || primary.phone || "", true)}
                </div>
              </section>
            </div>
          `;
        }catch(err){
          console.error("CUSTOMER_FILE_RENDER_ERROR", err);
          this.els.body.innerHTML = `
            <div class="lcCustomerFile">
              <section class="lcCfSection">
                <div class="lcCfSection__head">
                  <div class="lcCfSection__title">תיק לקוח</div>
                  <div class="lcCfSection__sub">המסך נפתח, אך הייתה תקלה ברינדור אחד הבלוקים החדשים.</div>
                </div>
                <div class="lcCfInfoGrid lcCfInfoGrid--primary">
                  ${this.infoLine("שם מלא", rec.fullName)}
                  ${this.infoLine("ת״ז", rec.idNumber)}
                ...
                <div class="help" style="padding:12px 0 0">זוהתה תקלה בתצוגת כרטיסי הפוליסות החדשים. פתח F12 לפרטים טכניים.</div>
              </section>
            </div>
          `;
        }
    ...
    infoLine(label, value, ltr=false){
      const val = safeTrim(value);
      return `<div class="lcCfInfo"><div class="lcCfInfo__label">${escapeHtml(label)}</div><div class="lcCfInfo__value"${ltr ? ' dir="ltr"' : ""}>${escapeHtml(val || "—")}</div></div>`;
    },

    formatPolicyInsuredLabel(policy, insureds=[]){
      if(!policy) return "—";
      ...
      const ins = (insureds || []).find(x => String(x.id) === String(policy.insuredId));
      return safeTrim(ins?.label || ins?.type) || "מבוטח";
    },

    renderPolicyPills(items){
      const valid = (items || []).filter(item => safeTrim(item?.value));
      if(!valid.length) return `<div class="muted">אין נתונים להצגה.</div>`;
      return `<div class="lcPolicyPills">${valid.map(item => `
        <div class="lcPolicyPill${item.ltr ? ' is-ltr' : ''}">
          <span class="lcPolicyPill__label">${escapeHtml(item.label || "")}</span>
          <span class="lcPolicyPill__value"${item.ltr ? ' dir="ltr"' : ''}>${escapeHtml(item.value || "")}</span>
        </div>`).join("")}</div>`;
    },

    getCompanyAccentColor(company){
      const map = {
        "הראל": "#003A8F",
        "הפניקס": "#F36F21",
        "כלל": "#0072CE",
      ...
      return map[safeTrim(company)] || "#5d84d6";
    },

    renderPolicyCardHeader(kindLabel, company, type, badge){
      const logoSrc = this.getCompanyLogoSrc(company);
      const accent = this.getCompanyAccentColor(company);
      const logo = logoSrc ? `<div class="lcPolicyGlassCard__logoWrap"><img class="lcPolicyGlassCard__logo" src="${escapeHtml(logoSrc)}" alt="${escapeHtml(company || "")}" /></div>` : `<div class="lcPolicyGlassCard__logoWrap lcPolicyGlassCard__logoWrap--fallback">${escapeHtml((company || "?").slice(0,1))}</div>`;
      return `
        <div class="lcPolicyGlassCard__accent" style="--policyAccent:${escapeHtml(accent)}"></div>
        <div class="lcPolicyGlassCard__top">
          <div class="lcPolicyGlassCard__brandRow">
            ${logo}
            <div>
              <div class="lcPolicyGlassCard__eyebrow">${escapeHtml(kindLabel || "פוליסה")}</div>
              <div class="lcPolicyGlassCard__title">${escapeHtml(company || "ללא חברה")} · ${escapeHtml(type || "ללא סוג")}</div>
            </div>
          </div>
          <div class="lcPolicyGlassCard__badge">${escapeHtml(badge || "—")}</div>
        </div>`;
    },

    renderExistingPoliciesCards(insureds=[]){
      const cards = [];
      (insureds || []).forEach(ins => {
        const holderLabel = safeTrim(ins?.label || ins?.type) || "מבוטח";
        const policies = ins?.data?.existingPolicies || [];
        policies.forEach(pol => {
          const isRisk = (pol.type === "ריסק" || pol.type === "ריסק משכנתא");
          const isCI = (pol.type === "מחלות קשות" || pol.type === "סרטן");
          const pledgeParts = [];
          if(pol.hasPledge) pledgeParts.push("קיים שיעבוד");
          if(safeTrim(pol.pledgeBankName)) pledgeParts.push(`בנק: ${safeTrim(pol.pledgeBankName)}`);
          cards.push(`
            <article class="lcPolicyGlassCard lcPolicyGlassCard--existing">
              ${this.renderPolicyCardHeader("פוליסה קיימת", pol.company, pol.type, holderLabel)}
              ${this.renderPolicyPills([
                { label:"מבוטח", value: holderLabel },
                { label:"חברה", value: pol.company || "—" },
                { label:"סוג ביטוח", value: pol.type || "—" },
                { label:"מספר פוליסה", value: pol.policyNumber || "—", ltr:true },
                { label:"פרמיה חודשית", value: pol.monthlyPremium || "—", ltr:true },
                { label: isRisk ? "סכום ביטוח" : isCI ? "סכום פיצוי" : "כיסויים", value: isRisk ? (pol.sumInsured || "—") : isCI ? (pol.compensation || "—") : ((pol.covers || []).join(", ") || "—") },
                { label:"שיעבוד", value: pledgeParts.join(" · ") || (pol.hasPledge ? "כן" : "לא") }
              ])}
            </article>`);
        });
      });
      return cards.length ? `<div class="lcPolicyGlassGrid">${cards.join("")}</div>` : `<div class="muted">אין פוליסות קיימות להצגה.</div>`;
    },

    renderNewPoliciesCards(list=[], insureds=[]){
      const cards = (list || []).map(pol => {
        const insuredLabel = this.formatPolicyInsuredLabel(pol, insureds);
        const isRisk = (pol.type === "ריסק" || pol.type === "ריסק משכנתא");
        const isCI = (pol.type === "מחלות קשות" || pol.type === "סרטן");
        const pledgeBank = pol.pledgeBank || {};
        const pledgeParts = [];
        if(pol.pledge) pledgeParts.push("כן");
        if(safeTrim(pledgeBank.bankName)) pledgeParts.push(`בנק: ${safeTrim(pledgeBank.bankName)}`);
        if(safeTrim(pledgeBank.branch)) pledgeParts.push(`סניף: ${safeTrim(pledgeBank.branch)}`);
        if(safeTrim(pledgeBank.amount)) pledgeParts.push(`סכום: ${safeTrim(pledgeBank.amount)}`);
        if(safeTrim(pledgeBank.years)) pledgeParts.push(`שנים: ${safeTrim(pledgeBank.years)}`);
        return `
          <article class="lcPolicyGlassCard lcPolicyGlassCard--new">
            ${this.renderPolicyCardHeader("פוליסה חדשה", pol.company, pol.type, insuredLabel)}
            ${this.renderPolicyPills([
              { label:"מבוטח", value: insuredLabel },
              { label:"חברה", value: pol.company || "—" },
              { label:"סוג ביטוח", value: pol.type || "—" },
              { label:"פרמיה חודשית", value: pol.premiumMonthly || "—", ltr:true },
              { label:"תאריך תחילת ביטוח", value: pol.startDate || "—", ltr:true },
              { label: isRisk ? "סכום ביטוח" : isCI ? "סכום פיצוי" : "פרטי מוצר", value: isRisk ? (pol.sumInsured || "—") : isCI ? (pol.compensation || "—") : (pol.type || "—") },
              { label:"שיעבוד", value: pledgeParts.join(" · ") || (pol.pledge ? "כן" : "לא") },
              { label:"פרטי בנק לשיעבוד", value: [pledgeBank.bankNo, pledgeBank.address].filter(v => safeTrim(v)).join(" · ") || "—" }
            ])}
          </article>`;
      }).join("");
      return cards ? `<div class="lcPolicyGlassGrid">${cards}</div>` : `<div class="muted">עדיין לא נוספו פוליסות חדשות.</div>`;
    },

    renderFileTable(headers, bodyRowsHtml){
      if(!bodyRowsHtml) return `<div class="muted">אין נתונים להצגה.</div>`;
      return `<div class="lcCfTableWrap"><table class="table lcCfTable"><thead><tr>${headers.map(h => `<th>${escapeHtml(h)}</th>`).join("")}</tr></thead><tbody>${bodyRowsHtml}</tbody></table></div>`;
    },
...
init(){
      ...
      on(this.els.btnOpenCustomerFile, "click", () => {
        ...
        this.close();
        UI.goView("customers");
        if(customerId) setTimeout(() => CustomersUI.openById(customerId), 80);
      });
...
const App = {
    ...
    async syncNow(){
      ...
      if (r.ok) {
        ...
        if (Auth.current) CustomersUI.render();
      } else {
