...
(() => {
  ...
  const BUILD = "20260406-customer-save-verify-securecore-v47";
    ...
    async saveCompletedCustomer(){
      ...
      if(!persistRes?.ok){
        ...
        pushCustomerCreationAuditEntry({
          ...
          status: 'danger',
          message: safeTrim(persistRes?.error) || 'שמירת הלקוח ל-Supabase נכשלה.',
          createdAt
        });
        throw new Error(safeTrim(persistRes?.error) || 'SAVE_CUSTOMER_FAILED');
      }
