...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  // Mail UX enhancements (pulse / active state / sound / sent toast)
  let _mailIsOpen = false;
  let _mailLastUnread = null; // unknown until first badge update
  let _mailPulseTimer = null;

  const MAIL_SOUND_ENABLED_KEY = 'gemel_mail_sound_enabled_v1';
  let _mailSoundEnabled = true;
  let _mailUserGestured = false;
  loadMailSoundPref_();
...
// Track first user interaction so audio is allowed in most browsers
document.addEventListener('pointerdown', () => { _mailUserGestured = true; }, { once:true, capture:true });

function loadMailSoundPref_(){
  try{
    const v = localStorage.getItem(MAIL_SOUND_ENABLED_KEY);
    if(v === '0') _mailSoundEnabled = false;
    if(v === '1') _mailSoundEnabled = true;
  }catch(_){}
}
function setMailSoundEnabled_(on){
  _mailSoundEnabled = !!on;
  try{ localStorage.setItem(MAIL_SOUND_ENABLED_KEY, on ? '1' : '0'); }catch(_){}
}

function playTone_(freq=523.25, durMs=120, gain=0.06){
  try{
    if(!_mailSoundEnabled) return;
    if(!_mailUserGestured) return;
    ...
    o.start(now);
    o.stop(now + (durMs/1000) + 0.03);

    o.onended = () => { try{ ctx.close(); }catch(_){ } };
  }catch(_){}
}

function playMailSound_(kind){
  // kind: 'new' | 'sent'
  if(kind === 'sent'){
    playTone_(659.25, 110, 0.05);
    setTimeout(() => playTone_(783.99, 90, 0.045), 80);
    return;
  }
  playTone_(523.25, 120, 0.055);
  setTimeout(() => playTone_(659.25, 110, 0.045), 90);
}

function setMailBtnOpenState_(open){
  _mailIsOpen = !!open;
  if(!mailBtn) return;
  if(open) mailBtn.classList.add('isOpen');
  else mailBtn.classList.remove('isOpen');
}

function triggerMailPulse_(){
  if(!mailBtn) return;
  mailBtn.classList.remove('pulse');
  void mailBtn.offsetWidth;
  mailBtn.classList.add('pulse');
  if(_mailPulseTimer) clearTimeout(_mailPulseTimer);
  _mailPulseTimer = setTimeout(() => mailBtn.classList.remove('pulse'), 1300);
}

function showMailNewToast_(fromName, msgId){
  const existing = document.getElementById('mailNewToast');
  if(existing) existing.remove();

  const wrap = document.createElement('div');
  wrap.id = 'mailNewToast';
  wrap.className = 'mailToast';
  wrap.innerHTML = `
    <div class="mailToastCard" role="status" aria-live="polite">
      <div class="mailToastTop">
        <div class="mailToastIcon" aria-hidden="true">✉️</div>
        <div class="mailToastTxt">
          <div class="mailToastTitle">הודעה חדשה</div>
          <div class="mailToastSub">מאת: <b>${escapeHtml(fromName||'')}</b></div>
        </div>
      </div>
      <div class="mailToastActions">
        <button type="button" class="btn btnTiny" data-open>פתח</button>
        <button type="button" class="btn btnSoft btnTiny" data-dismiss>דחה</button>
      </div>
    </div>
  `;
  document.body.appendChild(wrap);

  const kill = () => { try{ wrap.remove(); }catch(_){ } };
  const t = setTimeout(kill, 4500);

  wrap.querySelector('[data-dismiss]')?.addEventListener('click', () => { clearTimeout(t); kill(); });
  wrap.querySelector('[data-open]')?.addEventListener('click', () => {
    ...
    if(msgId){
      ...
      // mark as read immediately when user opens from toast
      markRead_(msgId);
    }
    renderMailOverlay_();
  });
...
function updateMailBadge_(){
  ...
  // Pulse + sound only when unread increases
  if(_mailLastUnread === null){
    _mailLastUnread = n;
    return;
  }
  if(n > _mailLastUnread){
    if(!_mailIsOpen) triggerMailPulse_();
    playMailSound_('new');
    // Toast for incoming message (only when inbox is closed)
    if(!_mailIsOpen){
      try{
        const me = myUserKey_();
        const newestUnread = inboxMessages_().find(m => m && !isReadForMe_(m, me));
        if(newestUnread){
          showMailNewToast_(newestUnread.fromName || newestUnread.fromUser || '', newestUnread.id);
        }
      }catch(_){}
    }
  ...
  function openMail_(){
    ...
    setMailBtnOpenState_(true);
  }

  function closeMail_(){
    if(!mailOverlay) return;
    mailOverlay.classList.add('hidden');
    if(mailBtn) mailBtn.classList.remove('pulse');
    mailOverlay.setAttribute('aria-hidden','true');
    document.body.style.overflow = '';
    setMailBtnOpenState_(false);
  }
  ...
  // Bind mail events
// Keep badge in sync across tabs/windows (pulse on new mail)
window.addEventListener('storage', (e) => {
  try{
    if(e && e.key === MESSAGES_LS_KEY){
      updateMailBadge_();
      if(_mailIsOpen) renderMailOverlay_();
    }
  }catch(_){}
});

// Light polling (helps later when moving to server mode)
setInterval(() => {
  try{
    if(state.currentUser) updateMailBadge_();
  }catch(_){}
}, 4500);
  ...
  if(mailOverlay){
    ...
    document.addEventListener('keydown', (e) => {
      if(!_mailIsOpen) return;
      if(e.key === 'Escape') closeMail_();
    });
