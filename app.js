...
(() => {
  ...
  function normalizeState(s){
    ...
    out.activityLog = (out.activityLog || []).map((item, idx) => normalizeActivityRecord(item, idx)).filter(Boolean).sort((a,b) => new Date(b.createdAtISO || 0) - new Date(a.createdAtISO || 0));
  ...
  function formatActivityDate(dateObj){
    try { return dateObj.toLocaleDateString('he-IL', { day:'2-digit', month:'2-digit', year:'numeric' }); } catch(_e) { return ''; }
  }

  function formatActivityTime(dateObj){
    try { return dateObj.toLocaleTimeString('he-IL', { hour:'2-digit', minute:'2-digit', second:'2-digit' }); } catch(_e) { return ''; }
  }

  function normalizeActivityRecord(item, idx=0){
    const iso = safeTrim(item?.createdAtISO) || nowISO();
    const d = new Date(iso);
    const valid = Number.isNaN(+d) ? new Date() : d;
    return {
      id: safeTrim(item?.id) || ("act_" + idx + "_" + Math.random().toString(16).slice(2)),
      type: safeTrim(item?.type) || 'general',
      action: safeTrim(item?.action) || 'activity',
      label: safeTrim(item?.label) || 'פעילות מערכת',
      status: safeTrim(item?.status) || 'info',
      userName: safeTrim(item?.userName) || '',
      userRole: safeTrim(item?.userRole) || '',
      entityId: safeTrim(item?.entityId) || '',
      entityType: safeTrim(item?.entityType) || '',
      details: item?.details && typeof item.details === 'object' ? item.details : {},
      createdAtISO: valid.toISOString(),
      dateFull: safeTrim(item?.dateFull) || formatActivityDate(valid),
      timeFull: safeTrim(item?.timeFull) || formatActivityTime(valid)
    };
  }
  ...
  const Activity = {
    ...
    roleLabel(role){
      ...
      return r === 'admin' ? 'מנהל מערכת' : r === 'manager' ? 'מנהל' : r === 'ops' ? 'תפעול' : 'נציג';
    },

    actionLabel(action){
      const map = {
        login_success: 'התחברות למערכת',
        login_failed: 'ניסיון התחברות כושל',
        logout: 'התנתקות מהמערכת',
        proposal_opened: 'נפתחה הצעה חדשה',
        proposal_saved: 'נשמרה הצעה',
        proposal_deleted: 'נמחקה הצעה',
        proposal_edit_opened: 'נפתחה הצעה להמשך עריכה',
        customer_saved: 'לקוח נשמר במערכת',
        submitted_to_ops: 'הוגשה הצעה למחלקת תפעול',
        mirror_started: 'התחלת שיקוף',
        mirror_finished: 'סיום שיקוף'
      };
      return map[action] || 'פעילות מערכת';
    },
    ...
    push(action, extras){
      ...
      const iso = nowISO();
      const d = new Date(iso);
      const rec = normalizeActivityRecord({
        id: 'act_' + Date.now().toString(16) + '_' + Math.random().toString(16).slice(2,8),
        type: extras?.type || 'general',
        action,
        label: extras?.label || this.actionLabel(action),
        status: extras?.status || 'info',
        userName: safeTrim(extras?.userName) || safeTrim(Auth?.current?.name) || safeTrim(extras?.username) || '',
        userRole: safeTrim(extras?.userRole) || safeTrim(Auth?.current?.role) || '',
        entityId: safeTrim(extras?.entityId) || '',
        entityType: safeTrim(extras?.entityType) || '',
        details: extras?.details || {},
        createdAtISO: iso,
        dateFull: formatActivityDate(d),
        timeFull: formatActivityTime(d)
      });
      State.data.activityLog.unshift(rec);
      if(State.data.activityLog.length > this.maxItems){
        State.data.activityLog = State.data.activityLog.slice(0, this.maxItems);
      }
      State.data.meta = State.data.meta || {};
      State.data.meta.updatedAt = iso;
      return rec;
    },
    ...
    stats(list){
      ...
      return {
        total: items.length,
        logins: items.filter(x => x.action === 'login_success').length,
        logouts: items.filter(x => x.action === 'logout').length,
        proposalsOpened: items.filter(x => x.action === 'proposal_opened').length,
        proposalsSaved: items.filter(x => x.action === 'proposal_saved').length,
        proposalsToOps: items.filter(x => x.action === 'submitted_to_ops').length,
        failedLogins: items.filter(x => x.action === 'login_failed').length
      };
    }
  };
  ...
  const Auth = {
    ...
    async logout(){
      ...
      if(prev){
        Activity.push("logout", { type:"auth", status:"success", userName: prev.name, userRole: prev.role });
        await App.persist("התנתקות נשמרה", { skipActivity:true });
      }
      this.current = null;
      try { localStorage.removeItem(LS_SESSION_KEY); } catch(_) {}
      this.lock();
    ...
    async _submit(){
      ...
      if (adminAuth.active !== false && username === safeTrim(adminAuth.username) && pin === safeTrim(adminAuth.pin)) {
        ...
        Activity.push("login_success", { type:"auth", status:"success", userName: this.current.name, userRole: this.current.role });
        try { localStorage.removeItem(LS_SESSION_KEY); } catch(_) {}
        await App.persist("התחברות נשמרה", { skipActivity:true });
      ...
      if(!matched){
        Activity.push("login_failed", { type:"auth", status:"error", username, details:{ username, reason:"user_not_found" } });
        await App.persist("ניסיון התחברות כושל", { skipActivity:true });
        return this._setError("שם משתמש לא נמצא");
      }
      if(matched.active === false){
        Activity.push("login_failed", { type:"auth", status:"error", username, details:{ username, reason:"user_disabled" } });
        await App.persist("ניסיון התחברות כושל", { skipActivity:true });
        return this._setError("המשתמש מושבת");
      }
      const expected = safeTrim(matched.pin) || "0000";
      if(pin !== expected){
        Activity.push("login_failed", { type:"auth", status:"error", username, details:{ username, reason:"wrong_pin" } });
        await App.persist("ניסיון התחברות כושל", { skipActivity:true });
...
this.els.syncDot = $("#syncDot");
      ...
      this.els.navActivity = $("#navActivity");
      this.els.activityTbody = $("#activityTbody");
      this.els.activitySearch = $("#activitySearch");
      this.els.activityFilter = $("#activityFilter");
      this.els.activityCountBadge = $("#activityCountBadge");
      this.els.activityKpiLogins = $("#activityKpiLogins");
      this.els.activityKpiOpened = $("#activityKpiOpened");
      this.els.activityKpiOps = $("#activityKpiOps");
      this.els.activityKpiFailed = $("#activityKpiFailed");
      this.els.btnActivityRefresh = $("#btnActivityRefresh");
...
// nav
      ...
      on(this.els.activitySearch, "input", () => ActivityUI.render());
      on(this.els.activityFilter, "change", () => ActivityUI.render());
      on(this.els.btnActivityRefresh, "click", () => ActivityUI.render());
...
this.applyRoleUI();
    ...
    applyRoleUI(){
      ...
      if (this.els.navActivity) this.els.navActivity.style.display = isAdmin ? "" : "none";
    ...
    goView(view){
      ...
      if(safe === "activity" && !Auth.isAdmin()) safe = "dashboard";
      ...
      document.body.classList.remove("view-users-active","view-dashboard-active","view-settings-active","view-discountSpec-active","view-customers-active","view-proposals-active","view-mirrors-active","view-activity-active");
      ...
      if (safe === "activity") ActivityUI.render();
...
txt.textContent = "";
  ...
  const ProposalsUI = {
    ...
    openById(id){
      ...
      if(!rec) return;
      Activity.push("proposal_edit_opened", { type:"proposal", entityId: rec.id, entityType:"proposal", details:{ proposalName: rec.fullName, step: rec.currentStep } });
      App.persist("פתיחת הצעה לעריכה", { skipActivity:true });
    ...
    async deleteById(id){
      ...
      Activity.push("proposal_deleted", { type:"proposal", entityId: rec.id, entityType:"proposal", details:{ proposalName: rec.fullName } });
      State.data.meta.updatedAt = nowISO();
      await App.persist("ההצעה נמחקה", { skipActivity:true });
      this.render();
    }
  };
  ...
  const ActivityUI = {
    getRows(){
      const all = Array.isArray(State.data?.activityLog) ? State.data.activityLog.slice() : [];
      const q = safeTrim(UI.els.activitySearch?.value).toLowerCase();
      const filter = safeTrim(UI.els.activityFilter?.value);
      return all.filter(item => {
        if(filter && filter !== 'all' && item.action !== filter) return false;
        if(!q) return true;
        return [item.label, item.userName, item.userRole, item.dateFull, item.timeFull, item.action, JSON.stringify(item.details || {})].join(' ').toLowerCase().includes(q);
      });
    },
    ...
    detailText(item){
      ...
      const parts = [];
      if(d.username) parts.push('שם משתמש: ' + d.username);
      if(d.reason) parts.push('סיבה: ' + d.reason);
      if(d.proposalName) parts.push('הצעה: ' + d.proposalName);
      if(d.customerName) parts.push('לקוח: ' + d.customerName);
      if(d.step) parts.push('שלב: ' + d.step);
      if(d.target) parts.push('יעד: ' + d.target);
      return parts.join(' · ') || '—';
    },

    render(){
      if(!UI.els.activityTbody) return;
      const rows = this.getRows();
      const stats = Activity.stats(rows);
      if(UI.els.activityCountBadge) UI.els.activityCountBadge.textContent = rows.length + ' רשומות';
      if(UI.els.activityKpiLogins) UI.els.activityKpiLogins.textContent = String(stats.logins);
      if(UI.els.activityKpiOpened) UI.els.activityKpiOpened.textContent = String(stats.proposalsOpened);
      if(UI.els.activityKpiOps) UI.els.activityKpiOps.textContent = String(stats.proposalsToOps);
      if(UI.els.activityKpiFailed) UI.els.activityKpiFailed.textContent = String(stats.failedLogins);
      UI.els.activityTbody.innerHTML = rows.length ? rows.map(item => `
        <tr>
          <td><strong>${escapeHtml(item.label || '—')}</strong></td>
          <td>${escapeHtml(item.userName || '—')}</td>
          <td>${escapeHtml(Activity.roleLabel(item.userRole || ''))}</td>
          <td>${escapeHtml(item.dateFull || '—')}</td>
          <td>${escapeHtml(item.timeFull || '—')}</td>
          <td>${escapeHtml(this.detailText(item))}</td>
        </tr>
      `).join('') : `<tr><td colspan="6"><div class="emptyState"><div class="emptyState__title">אין פעילות להצגה</div><div class="emptyState__text">ברגע שמשתמשים יתחברו, ישמרו הצעות או יגישו לתפעול — הלוג יופיע כאן.</div></div></td></tr>`;
    }
  };
...
init(){
      ...
      on(this.els.btnOpen, "click", () => {
        ...
        this.reset();
        Activity.push("proposal_opened", { type:"proposal", status:"success", details:{ target:"new_customer_wizard" } });
        App.persist("נפתחה הצעה חדשה", { skipActivity:true });
        this.open();
      });
      ...
      on(this.els.btnSendToOps, "click", async () => {
        ...
        Activity.push("submitted_to_ops", { type:"proposal", entityType:"customer", entityId: this.lastSavedCustomerId || "", details:{ customerName: safeTrim(((primary.firstName || "") + " " + (primary.lastName || "")).trim()) } });
        await App.persist("הוגש לתפעול", { skipActivity:true });
    ...
    async startCall(){
      ...
      Activity.push("mirror_started", { type:"mirror", entityId: rec.id, entityType:"customer", details:{ customerName: rec.fullName } });
      this.render();
      this.startTimerLoop();
      await App.persist('שיחת שיקוף התחילה', { skipActivity:true });
    },
    ...
    async finishCall(){
      ...
      this.stopTimerLoop();
      Activity.push("mirror_finished", { type:"mirror", entityId: rec.id, entityType:"customer", details:{ customerName: rec.fullName, duration: store.durationText } });
      this.render();
      await App.persist('שיחת שיקוף הסתיימה', { skipActivity:true });
...
// ---------- Step 6 ----------
    ...
    async saveDraft(){
      ...
      Activity.push("proposal_saved", { type:"proposal", entityId: record.id, entityType:"proposal", details:{ proposalName: record.fullName, step: this.step || 1 } });
      State.data.meta.updatedAt = nowISO();
      const persistRes = await App.persist("ההצעה נשמרה", { skipActivity:true });
      ProposalsUI.render();
      if(persistRes?.ok){
    ...
    async saveCompletedCustomer(){
      ...
      Activity.push("customer_saved", { type:"customer", entityId: record.id, entityType:"customer", details:{ customerName: record.fullName } });
      State.data.meta.updatedAt = nowISO();
      await App.persist("הלקוח נשמר", { skipActivity:true });
      return record;
    },
