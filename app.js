...
// open health covers drawer (Health only)
    ...
    parseHarBituachWorkbook(buffer, ins){
      ...
      const matchHeader = (value, patterns=[]) => patterns.some((pattern) => pattern.test(normalizeHeader(value)));

      const isLikelyDataRow = (row = {}) => {
        const source = [safeTrim(row.main), safeTrim(row.sub), safeTrim(row.productType), safeTrim(row.company), safeTrim(row.extra)].join(" | ");
      ...
      const isHealthLifeRow = (row = {}) => {
        const source = [safeTrim(row.main), safeTrim(row.sub), safeTrim(row.productType), safeTrim(row.section), safeTrim(row.extra)].join(" | ");
        if(!source) return false;
      ...
      sheetNames.forEach((sheetName) => {
        ...
        if(!sheet) return;
        const rows = window.XLSX.utils.sheet_to_json(sheet, { header: 1, defval: "" });
        const headerIdx = rows.findIndex((row) => Array.isArray(row)
          && row.some((cell) => matchHeader(cell, headerMatchers.idNumber))
          && row.some((cell) => matchHeader(cell, headerMatchers.main))
          && row.some((cell) => matchHeader(cell, headerMatchers.company))
        );
