...
(() => {
  ...
  const BUILD = "20260406-customer-save-hotfix-v48";
  ...
  const CUSTOMER_AUDIT_LIMIT = 120;
  ...
  function isCustomerOwnedByCurrentUser(rec){
    if(Auth.canViewAllCustomers()) return true;
    const currentName = safeTrim(Auth?.current?.name);
    if(!currentName || !rec || typeof rec !== 'object') return false;
    ...
    const candidateKeys = [
      ...
      safeTrim(rec?.payload?.systemAudit?.customerCreation?.createdByKey)
    ].filter(Boolean).map((v) => String(v).toLowerCase());
    return !!(currentIdentity?.key && candidateKeys.includes(String(currentIdentity.key).toLowerCase()));
  }

  function normalizeCustomerCreationAuditEntry(raw, idx=0){
    const entry = raw && typeof raw === 'object' ? raw : {};
    return {
      id: safeTrim(entry.id) || ('cust_audit_' + idx + '_' + Math.random().toString(16).slice(2,8)),
      customerId: safeTrim(entry.customerId),
      customerName: safeTrim(entry.customerName) || 'לקוח',
      idNumber: safeTrim(entry.idNumber),
      phone: safeTrim(entry.phone),
      agentName: safeTrim(entry.agentName || entry.createdBy || entry.ownerName),
      agentKey: safeTrim(entry.agentKey || entry.createdByKey),
      stage: safeTrim(entry.stage) || 'pending',
      status: safeTrim(entry.status) || 'info',
      message: safeTrim(entry.message),
      build: safeTrim(entry.build) || BUILD,
      createdAt: safeTrim(entry.createdAt) || nowISO(),
      updatedAt: safeTrim(entry.updatedAt) || safeTrim(entry.createdAt) || nowISO(),
      serverFound: entry.serverFound === true,
      visibleToCreator: entry.visibleToCreator === true,
      serverUpdatedAt: safeTrim(entry.serverUpdatedAt),
      source: safeTrim(entry.source) || 'customer_creation'
    };
  }

  function getCustomerCreationAuditStore(){
    State.data.meta = State.data.meta && typeof State.data.meta === 'object' ? State.data.meta : {};
    const rows = Array.isArray(State.data.meta.customerCreationAudit) ? State.data.meta.customerCreationAudit : [];
    State.data.meta.customerCreationAudit = rows.map((item, idx) => normalizeCustomerCreationAuditEntry(item, idx)).filter(Boolean).slice(-CUSTOMER_AUDIT_LIMIT);
    return State.data.meta.customerCreationAudit;
  }

  function pushCustomerCreationAuditEntry(entry = {}){
    const store = getCustomerCreationAuditStore();
    const normalized = normalizeCustomerCreationAuditEntry({ ...entry, updatedAt: nowISO() }, store.length);
    store.push(normalized);
    State.data.meta.customerCreationAudit = store.slice(-CUSTOMER_AUDIT_LIMIT);
    State.data.meta.updatedAt = nowISO();
    return normalized;
  }
  ...
  function pickNewestCustomerRecord(localRec, remoteRec, fallbackIdx = 0){
    const localNorm = localRec ? normalizeCustomerRecord(localRec, fallbackIdx) : null;
    const remoteNorm = remoteRec ? normalizeCustomerRecord(remoteRec, fallbackIdx) : null;
    if(localNorm && !remoteNorm) return localNorm;
    if(remoteNorm && !localNorm) return remoteNorm;
    if(!localNorm && !remoteNorm) return null;
    const localStamp = Math.max(parseComparableTimestamp(localNorm?.updatedAt), parseComparableTimestamp(localNorm?.createdAt));
    const remoteStamp = Math.max(parseComparableTimestamp(remoteNorm?.updatedAt), parseComparableTimestamp(remoteNorm?.createdAt));
    return localStamp >= remoteStamp ? localNorm : remoteNorm;
  }
  ...
  const Storage = {
    ...
    buildMetaRow(state){
      return {
        ...
        payload: {
          ...
          customerCreationAudit: Array.isArray(state?.meta?.customerCreationAudit) ? state.meta.customerCreationAudit.map((item, idx) => normalizeCustomerCreationAuditEntry(item, idx)).filter(Boolean).slice(-CUSTOMER_AUDIT_LIMIT) : [],
          updatedAt: nowISO()
        },
        updated_at: nowISO()
      };
    ...
    async syncTable(tableName, rows, options = {}){
      ...
      const allowDeleteMissing = options?.deleteMissing !== false;
      let existing = [];
      let canDelete = false;
      ...
      if(allowDeleteMissing && canDelete){
    ...
    mapMeta(metaRow){
      ...
      return {
        ...
        customerCreationAudit: Array.isArray(payload?.customerCreationAudit) ? payload.customerCreationAudit.map((item, idx) => normalizeCustomerCreationAuditEntry(item, idx)).filter(Boolean).slice(-CUSTOMER_AUDIT_LIMIT) : []
      };
    },
    ...
    async fetchCustomerRowById(customerId){
      const id = safeTrim(customerId);
      if(!id) return { ok:false, error:'MISSING_CUSTOMER_ID' };
      try {
        const client = this.getClient();
        const { data, error } = await client.from(SUPABASE_TABLES.customers).select('*').eq('id', id).maybeSingle();
        if(error) throw error;
        return { ok:true, found: !!data, row: data || null };
      } catch(primaryErr) {
        try {
          const data = await this.restRequest(SUPABASE_TABLES.customers + '?id=eq.' + encodeURIComponent(id) + '&select=*', { method:'GET' });
          const row = Array.isArray(data) ? (data[0] || null) : (data || null);
          return { ok:true, found: !!row, row };
        } catch(restErr) {
          return { ok:false, error: String(restErr?.message || primaryErr?.message || restErr || primaryErr) };
        }
      }
    ...
    async upsertSingleCustomerRow(row){
      if(!row || typeof row !== 'object') return { ok:false, error:'MISSING_CUSTOMER_ROW' };
      const cleanRow = { ...row };
      try {
        const client = this.getClient();
        const { data, error } = await client.from(SUPABASE_TABLES.customers).upsert([cleanRow], { onConflict:'id' }).select('*').maybeSingle();
        if(error) throw error;
        return { ok:true, row: data || cleanRow };
      } catch(primaryErr) {
        try {
          const updated = await this.restRequest(SUPABASE_TABLES.customers + '?id=eq.' + encodeURIComponent(safeTrim(cleanRow.id)) + '&select=*', {
            method:'PATCH',
            body: cleanRow
          });
          const updatedRow = Array.isArray(updated) ? (updated[0] || null) : (updated || null);
          if(updatedRow) return { ok:true, row: updatedRow };
          const created = await this.restRequest(SUPABASE_TABLES.customers, {
            method:'POST',
            body: cleanRow
          });
          const createdRow = Array.isArray(created) ? (created[0] || cleanRow) : (created || cleanRow);
    ...
    async buildMergedCustomerRows(state){
      const localCustomers = Array.isArray(state?.customers) ? state.customers.map((item, idx) => normalizeCustomerRecord(item, idx)).filter(Boolean) : [];
      const remoteRes = await this.loadTableRows(SUPABASE_TABLES.customers);
      if(!remoteRes?.ok){
        return { ok:false, error: safeTrim(remoteRes?.error) || 'CUSTOMERS_REMOTE_LOAD_FAILED' };
      }
      const merged = new Map();
      (remoteRes.data || []).forEach((row, idx) => {
        const mapped = this.mapCustomerRow(row, idx);
        if(mapped?.id) merged.set(String(mapped.id), mapped);
      });
      localCustomers.forEach((localRec, idx) => {
        const key = safeTrim(localRec?.id) || ('customer_' + idx);
        const remoteRec = merged.get(key) || null;
        const chosen = pickNewestCustomerRecord(localRec, remoteRec, idx);
        if(chosen?.id) merged.set(String(chosen.id), chosen);
      });
      const rows = this.buildCustomerRows({ customers: Array.from(merged.values()) });
      return { ok:true, rows };
    },
    ...
    async saveSheets(state){
      try {
        ...
        const mergedCustomersRes = await this.buildMergedCustomerRows(state);
        if(!mergedCustomersRes?.ok) throw new Error(safeTrim(mergedCustomersRes?.error) || 'CUSTOMERS_MERGE_FAILED');
        await this.syncTable(SUPABASE_TABLES.customers, mergedCustomersRes.rows, { deleteMissing:false });
...
this.applyRoleUI();
  ...
  const CustomersUI = {
    ...
    list(){
      ...
      const visible = all.filter((rec) => isCustomerOwnedByCurrentUser(rec));
...
// ---------- Step 6 ----------
    ...
    async saveCompletedCustomer(){
      ...
      const customerId = "cust_" + Date.now().toString(16) + "_" + Math.random().toString(16).slice(2,8);
      const payloadWithAudit = {
        ...
        agentId: creator.key,
        systemAudit: {
          ...((payload?.systemAudit && typeof payload.systemAudit === 'object') ? payload.systemAudit : {}),
          customerCreation: {
            status: 'prepared',
            build: BUILD,
            createdAt,
      ...
      pushCustomerCreationAuditEntry({
        ...
        stage: 'prepared',
        status: 'info',
        message: 'הלקוח הוכן לשמירה מתוך אשף הקמת הלקוח.',
        createdAt,
        source: 'wizard_finish'
      });
      ...
      State.data.meta.updatedAt = nowISO();
      record.payload.systemAudit.customerCreation.status = 'persisting';
      record.payload.systemAudit.customerCreation.updatedAt = nowISO();
      pushCustomerCreationAuditEntry({
        customerId: record.id,
        customerName: record.fullName,
        idNumber: record.idNumber,
        ...
        stage: 'persisting',
        status: 'info',
        message: 'המערכת שומרת את הלקוח ל-Supabase.',
        createdAt
      });

      const directCustomerSaveRes = await Storage.upsertSingleCustomerRow(Storage.buildCustomerRows({ customers:[record] })[0]);
      if(!directCustomerSaveRes?.ok){
        record.payload.systemAudit.customerCreation.status = 'persist_failed';
        record.payload.systemAudit.customerCreation.updatedAt = nowISO();
        pushCustomerCreationAuditEntry({
          customerId: record.id,
          customerName: record.fullName,
          ...
          status: 'danger',
          message: safeTrim(directCustomerSaveRes?.error) || 'שמירת הלקוח הישירה ל-Supabase נכשלה.',
          createdAt
        });
        throw new Error(safeTrim(directCustomerSaveRes?.error) || 'SAVE_CUSTOMER_DIRECT_FAILED');
      }

      const persistRes = await App.persist("הלקוח נשמר");
      if(!persistRes?.ok){
        record.payload.systemAudit.customerCreation.status = 'persist_warning';
        record.payload.systemAudit.customerCreation.updatedAt = nowISO();
        pushCustomerCreationAuditEntry({
          customerId: record.id,
          customerName: record.fullName,
          ...
          status: 'warn',
          message: safeTrim(persistRes?.error) || 'הלקוח נשמר ישירות ל-Supabase אך שמירת יתר מצב המערכת החזירה אזהרה.',
          createdAt
        });
      }
      ...
      pushCustomerCreationAuditEntry({
        ...
        stage: 'persisted',
        status: 'ok',
        message: 'הלקוח נשמר בהצלחה ב-Supabase. מתחיל אימות חוזר.',
        createdAt
      });

      const verifyRes = await Storage.fetchCustomerRowById(record.id);
      if(!verifyRes?.ok || !verifyRes?.found || !verifyRes?.row){
        record.payload.systemAudit.customerCreation.status = 'verify_failed';
        record.payload.systemAudit.customerCreation.updatedAt = nowISO();
        pushCustomerCreationAuditEntry({
          customerId: record.id,
          customerName: record.fullName,
          ...
          stage: 'verify_failed',
          status: 'danger',
          message: safeTrim(verifyRes?.error) || 'הלקוח לא חזר מאימות ישיר מול טבלת customers ב-Supabase.',
          createdAt
        });
        try { await App.persist('שגיאת אימות לקוח חדש'); } catch(_e) {}
        throw new Error(safeTrim(verifyRes?.error) || 'CUSTOMER_VERIFY_NOT_FOUND');
      }

      const refreshed = await Storage.loadSheets();
      if(refreshed?.ok && refreshed?.payload){
        State.data = refreshed.payload;
        try { Storage.saveBackup(State.data); } catch(_e) {}
      }
      let refreshedRecord = (Array.isArray(State.data?.customers) ? State.data.customers : []).find((item) => String(item.id) === String(record.id)) || null;
      if(!refreshedRecord){
        refreshedRecord = Storage.mapCustomerRow(verifyRes.row, 0);
        State.data.customers = Array.isArray(State.data?.customers) ? State.data.customers : [];
        State.data.customers.unshift(refreshedRecord);
      }
      const visibleToCreator = isCustomerOwnedByCurrentUser(refreshedRecord);
      refreshedRecord.payload = refreshedRecord.payload && typeof refreshedRecord.payload === 'object' ? refreshedRecord.payload : {};
      refreshedRecord.payload.systemAudit = refreshedRecord.payload.systemAudit && typeof refreshedRecord.payload.systemAudit === 'object' ? refreshedRecord.payload.systemAudit : {};
      refreshedRecord.payload.systemAudit.customerCreation = {
        ...(refreshedRecord.payload.systemAudit.customerCreation || {}),
        status: visibleToCreator ? 'verified' : 'verified_visibility_warning',
        build: BUILD,
        createdAt,
        updatedAt: nowISO(),
        createdBy: creator.name,
        createdByKey: creator.key,
        serverFound: true,
        visibleToCreator: !!visibleToCreator,
        serverUpdatedAt: safeTrim(verifyRes?.row?.updated_at || verifyRes?.row?.created_at)
      };
      pushCustomerCreationAuditEntry({
        customerId: refreshedRecord.id,
        customerName: refreshedRecord.fullName,
        idNumber: refreshedRecord.idNumber,
        phone: refreshedRecord.phone,
        agentName: creator.name,
        agentKey: creator.key,
        stage: visibleToCreator ? 'verified' : 'verified_visibility_warning',
        status: visibleToCreator ? 'ok' : 'warn',
        message: visibleToCreator ? 'הלקוח אומת בהצלחה מול Supabase וגם זמין לרשימת הלקוחות של הנציג.' : 'הלקוח נשמר ואומת ב-Supabase אך אינו גלוי כרגע לרשימת הלקוחות של היוצר.',
        serverFound: true,
        visibleToCreator: !!visibleToCreator,
        serverUpdatedAt: safeTrim(verifyRes?.row?.updated_at || verifyRes?.row?.created_at),
        createdAt
      ...
      State.data.meta.updatedAt = nowISO();
      await App.persist(visibleToCreator ? 'אומת לקוח חדש' : 'אומת לקוח חדש עם אזהרת תצוגה');
...
ההכנסה מעבודה שאתה צפוי להרוויח תעמוד על סך של ______ ₪ לחודש. ידוע לך כי בקרות מקרה הביטוח יוגבלו תגמולי הפיצוי החודשי להם תהיה זכאי לגובה שלא יעלה על 75% מהכנסתך מעבודה בפועל ב־12 החודשים שקדמו למועד קרות מקרה הביטוח, או בתקופה ממועד תחילת הביטוח ועד מועד קרות מקרה הביטוח, אם חלפו פחות מ־12 חודשים.`
  ...
  const SecureCoreUI = {
    ...
    busy: false,
    lastScan: null,
    checksOrder: ["session", "permissions", "server", "ui", "state", "customers", "activity"],
    ...
    resetUi(){
      this.setSummary('המערכת מוכנה להריץ בדיקה חכמה.', 'הבדיקות כוללות session, הרשאות, חיבור שרת, יציבות מסך, state, לקוחות חסרים וחריגות פעילות.', 'idle');
    ...
    async runSingleCheck(key){
      ...
      const titleMap = { session:'Session ואימות', permissions:'הרשאות ותפריטים', server:'חיבור שרת', ui:'יציבות מסך', state:'מצב נתונים', customers:'לקוחות חסרים', activity:'חריגות פעילות' };
      let severity = 'ok';
      ...
      try {
        ...
        if(key === 'customers'){
          const serverCustomers = await Storage.loadTableRows(SUPABASE_TABLES.customers, 'id,full_name,id_number,phone,agent_name,created_at,updated_at,payload');
          if(!serverCustomers?.ok){
            severity = 'danger';
            message = 'לא הצלחתי למשוך את טבלת הלקוחות מהשרת לצורך השוואה.';
          } else {
            const localRows = Array.isArray(State?.data?.customers) ? State.data.customers : [];
            const serverRows = (serverCustomers.data || []).map((row, idx) => Storage.mapCustomerRow(row, idx));
            const localIds = new Set(localRows.map((row) => safeTrim(row?.id)).filter(Boolean));
            const missingLocal = serverRows.filter((row) => !localIds.has(safeTrim(row?.id)));
            const recentAudit = getCustomerCreationAuditStore().filter((item) => {
              const age = Date.now() - Date.parse(item.updatedAt || item.createdAt || '');
              return Number.isFinite(age) && age <= (14 * 24 * 60 * 60 * 1000);
            });
            const suspiciousAudit = recentAudit.filter((item) => ['persist_failed','verify_failed','verified_visibility_warning'].includes(safeTrim(item.stage)) || ['danger','warn'].includes(safeTrim(item.status)));
            const visibleServer = serverRows.filter((row) => isCustomerOwnedByCurrentUser(row));
            const visibleLocal = localRows.filter((row) => isCustomerOwnedByCurrentUser(row));
            const visibleLocalIds = new Set(visibleLocal.map((row) => safeTrim(row?.id)).filter(Boolean));
            const hiddenForCurrent = visibleServer.filter((row) => !visibleLocalIds.has(safeTrim(row?.id)));
            if(missingLocal.length){
              severity = 'danger';
              const sample = missingLocal[0];
              message = `זוהו ${missingLocal.length} לקוחות שקיימים ב-Supabase אך לא נטענו ל-State המקומי. לדוגמה: ${safeTrim(sample?.fullName) || 'לקוח'}${safeTrim(sample?.idNumber) ? ' · ת"ז ' + safeTrim(sample.idNumber) : ''}.`;
            } else if(hiddenForCurrent.length){
              severity = 'warn';
              const sample = hiddenForCurrent[0];
              message = `נמצא פער תצוגה: ${hiddenForCurrent.length} לקוחות שאמורים להיות גלויים למשתמש הנוכחי בשרת, אך אינם מופיעים כרגע ברשימה המקומית. לדוגמה: ${safeTrim(sample?.fullName) || 'לקוח'}.`;
            } else if(suspiciousAudit.length){
              severity = 'warn';
              const sample = suspiciousAudit[suspiciousAudit.length - 1];
              message = `לוג יצירת הלקוחות מצא ${suspiciousAudit.length} אירועים חריגים לאחרונה. האחרון: ${safeTrim(sample?.customerName) || 'לקוח'} · ${safeTrim(sample?.message) || safeTrim(sample?.stage)}.`;
            } else {
              message = `טבלת הלקוחות תקינה: ${serverRows.length} לקוחות בשרת ו-${localRows.length} ב-State המקומי. לא זוהו לקוחות חסרים.`;
            }
          }
        }
        if(key === 'activity'){
