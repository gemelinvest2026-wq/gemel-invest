...
ההכנסה מעבודה שאתה צפוי להרוויח תעמוד על סך של ______ ₪ לחודש. ידוע לך כי בקרות מקרה הביטוח יוגבלו תגמולי הפיצוי החודשי להם תהיה זכאי לגובה שלא יעלה על 75% מהכנסתך מעבודה בפועל ב־12 החודשים שקדמו למועד קרות מקרה הביטוח, או בתקופה ממועד תחילת הביטוח ועד מועד קרות מקרה הביטוח, אם חלפו פחות מ־12 חודשים.`
  ...
  const MirrorsUI = {
    ...
    async openReflection(){
      const rec = this.current();
      if(!rec) return;
      this.collectVerificationInputsFromDom(rec);
      ...
      try {
        ...
        console.error('MIRRORS_OPEN_REFLECTION_RENDER_FAILED:', err);
        try {
          this.renderVerification(rec);
          this.renderReflection(rec);
          this.renderWizardNav(rec);
          ...
          console.error('MIRRORS_OPEN_REFLECTION_FALLBACK_FAILED:', innerErr);
        }
      }
    },
    ...
    collectVerificationInputsFromDom(rec){
      if(!rec || !this.els.verifyCard) return;
      const store = this.getVerifyState(rec);
      const selectedSmoking = this.els.verifyCard.querySelector('.mirrorsMiniChoice.is-selected[data-mirror-smoking]');
      const selectedDelivery = this.els.verifyCard.querySelector('.mirrorsMiniChoice.is-selected[data-mirror-delivery]');
      if(selectedSmoking) store.smokingAnswer = safeTrim(selectedSmoking.getAttribute('data-mirror-smoking'));
      if(selectedDelivery) store.deliveryMethod = safeTrim(selectedDelivery.getAttribute('data-mirror-delivery'));
      const smokingProducts = Array.from(this.els.verifyCard.querySelectorAll('[data-mirror-smoke-product]:checked')).map(el => safeTrim(el.value)).filter(Boolean);
      if(smokingProducts.length || safeTrim(store.smokingAnswer) === 'yes') store.smokingProducts = smokingProducts;
      const smokeQtyInput = this.els.verifyCard.querySelector('[data-mirror-smoke-qty]');
      if(smokeQtyInput) store.smokingQuantity = safeTrim(smokeQtyInput.value);
      const emailInput = this.els.verifyCard.querySelector('[data-mirror-email]');
      if(emailInput) store.deliveryEmail = safeTrim(emailInput.value);
    },
    ...
    async saveVerification(options = {}){
      ...
      const persistPromise = (async () => {
        try {
          const result = await Promise.race([
            App.persist('שיקוף נשמר'),
            new Promise(resolve => setTimeout(() => resolve({ ok:false, error:'VERIFY_PERSIST_TIMEOUT' }), 4500))
          ]);
          return !!(result?.ok);
        } catch(err) {
          console.error('MIRRORS_VERIFY_PERSIST_FAILED:', err);
          return false;
        }
      })();
      ...
      if(options.openReflection){
        this.render();
        persistPromise.then((persistOk) => {
          if(!persistOk) console.warn('MIRRORS_VERIFY_BACKGROUND_PERSIST_NOT_COMPLETED');
        }).catch(err => console.error('MIRRORS_VERIFY_BACKGROUND_PERSIST_FAILED:', err));
        if(!options.silent){
          alert('שלב האימות הושלם ונפתח מסך שיקוף הר הביטוח.');
        }
        return true;
      }
      ...
      if(!options.silent){
        alert(
          persistOk
            ? 'שלב אימות הנתונים נשמר בהצלחה.'
            : 'שלב אימות הנתונים נשמר מקומית, אך השמירה לשרת לא הושלמה כרגע.'
        );
      }
      return true;
    },
