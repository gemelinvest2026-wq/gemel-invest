...
(() => {
  ...
  function normalizeState(s){
    ...
    out.meta.activityLog = Array.isArray(out.meta.activityLog) ? out.meta.activityLog.map(item => ({
      at: safeTrim(item?.at) || nowISO(),
      actorName: safeTrim(item?.actorName) || "מערכת",
      actorRole: safeTrim(item?.actorRole) || "system",
      action: safeTrim(item?.action) || "",
  ...
  const Auth = {
    ...
    init(){
      this.els = {
        ...
        err: $("#lcLoginError"),
        emergencyReset: $("#btnEmergencyAdminReset"),
      };
      ...
      on(this.els.emergencyReset, "click", async () => {
        await this.emergencyAdminReset();
      });
    },
    ...
    async emergencyAdminReset(){
      try { await App._bootPromise; } catch(_) {}
      const entered = window.prompt("להזנת איפוס חירום, הקלד את קוד החירום:");
      if(entered === null) return;
      if(safeTrim(entered) !== EMERGENCY_ADMIN_RESET_CODE){
        return this._setError("קוד חירום שגוי");
      }
      const ok = window.confirm("לאפס עכשיו את מנהל המערכת ל-"מנהל מערכת" עם קוד 1234?");
      if(!ok) return;

      State.data.meta = State.data.meta || {};
      State.data.meta.adminAuth = { username: "מנהל מערכת", pin: "1234", active: true };
      State.data.meta.updatedAt = nowISO();
      Audit.add("איפוס חירום למנהל מערכת", "שוחזרו פרטי ברירת מחדל: מנהל מערכת / 1234");
      const r = await App.persist("בוצע איפוס חירום");
      if(!r.ok){
        return this._setError(r.error || "איפוס חירום נכשל");
      }
      if(this.els.user) this.els.user.value = "מנהל מערכת";
      if(this.els.pin) this.els.pin.value = "1234";
      if(this.els.remember) this.els.remember.checked = false;
      this._setError("איפוס חירום בוצע. אפשר להתחבר עכשיו עם: מנהל מערכת / 1234");
      this.lock();
    },
    ...
    async _submit(){
      ...
      const adminUsername = safeTrim(adminAuth.username) || defAdmin.username;
      const adminPin = safeTrim(adminAuth.pin) || defAdmin.pin;

      if (adminAuth.active !== false && username === adminUsername && pin === adminPin) {
        this.current = { name: adminUsername, role:"admin" };
...
this.els.syncDot = $("#syncDot");
      ...
      this.els.adminOwnUsername = $("#adminOwnUsername");
      this.els.adminOwnPin = $("#adminOwnPin");
      this.els.btnSaveAdminAuth = $("#btnSaveAdminAuth");
      this.els.adminOwnAuthError = $("#adminOwnAuthError");
      this.els.auditLogTbody = $("#auditLogTbody");
...
// nav
      ...
      on(this.els.btnSaveAdminAuth, "click", async () => {
        await SettingsUI.saveOwnAdminAuth();
      });
...
this.applyRoleUI();
    ...
    goView(view){
      ...
      if (safe === "settings") SettingsUI.render();
    },
...
txt.textContent = "";
  ...
  const UsersUI = {
    ...
    async _saveFromModal(){
      ...
      if(isEdit){
        ...
        const prevUsername = a.username;
        const prevRole = a.role;
        const prevActive = a.active !== false;
        a.name = name;
        a.username = username;
        a.pin = pin;
        ...
        a.active = active;
        State.data.meta.updatedAt = nowISO();
        Audit.add("עריכת משתמש", `${prevName} / ${prevUsername} → ${name} / ${username}${prevRole !== a.role ? ` · תפקיד: ${prevRole}→${a.role}` : ""}${prevActive !== active ? ` · סטטוס: ${prevActive ? "פעיל" : "מושבת"}→${active ? "פעיל" : "מושבת"}` : ""}`);
        ...
        State.data.meta.updatedAt = nowISO();
        Audit.add("יצירת משתמש", `${name} / ${username} · תפקיד: ${role === "manager" ? "מנהל" : role === "ops" ? "תפעול" : "נציג"}`);
    ...
    async toggleUser(id){
      ...
      State.data.meta.updatedAt = nowISO();
      Audit.add(a.active ? "הפעלת משתמש" : "השבתת משתמש", `${a.name} / ${a.username}`);
  ...
  const Audit = {
    add(action, detail=""){
      State.data.meta = State.data.meta || {};
      const list = Array.isArray(State.data.meta.activityLog) ? State.data.meta.activityLog : [];
      const actorName = safeTrim(Auth?.current?.name) || "מערכת";
      const actorRole = safeTrim(Auth?.current?.role) || "system";
      list.unshift({
        at: nowISO(),
        actorName,
        actorRole,
      ...
      State.data.meta.activityLog = list.slice(0, MAX_ACTIVITY_LOG);
    }
  };

  const SettingsUI = {
    _showOwnAuthError(msg=""){
      const el = UI.els.adminOwnAuthError;
      if(!el) return;
      el.textContent = String(msg || "");
      el.style.display = msg ? "block" : "none";
    },
    ...
    render(){
      ...
      if(UI.els.adminOwnUsername) UI.els.adminOwnUsername.value = safeTrim(adminAuth.username);
      if(UI.els.adminOwnPin) UI.els.adminOwnPin.value = safeTrim(adminAuth.pin);
      this._showOwnAuthError("");
      this.renderAuditLog();
    },

    renderAuditLog(){
      const tbody = UI.els.auditLogTbody;
      if(!tbody) return;
      if(!Auth.isAdmin()){
        tbody.innerHTML = "";
        return;
      }
      const rows = Array.isArray(State.data?.meta?.activityLog) ? State.data.meta.activityLog : [];
      if(!rows.length){
        tbody.innerHTML = '<tr><td colspan="5" class="lcAuditEmpty">אין פעילות הרשאות עדיין</td></tr>';
        return;
      }
      tbody.innerHTML = rows.map(item => {
        const roleMap = item.actorRole === "admin" ? "מנהל מערכת" : item.actorRole === "manager" ? "מנהל" : item.actorRole === "ops" ? "תפעול" : item.actorRole || "-";
        return `<tr>
          <td>${escapeHtml(item.at || "")}</td>
          <td>${escapeHtml(item.actorName || "-")}</td>
          <td>${escapeHtml(roleMap)}</td>
    ...
    async saveOwnAdminAuth(){
      if(!Auth.isAdmin()) return;
      const username = safeTrim(UI.els.adminOwnUsername?.value);
      const pin = safeTrim(UI.els.adminOwnPin?.value);
      if(!username) return this._showOwnAuthError("נא להזין שם משתמש למנהל המערכת");
      if(!pin) return this._showOwnAuthError("נא להזין סיסמה / PIN למנהל המערכת");

      const prev = State.data?.meta?.adminAuth || {};
      ...
      State.data.meta.updatedAt = nowISO();
      Auth.current = { name: username, role: "admin" };
      Audit.add("שינוי פרטי מנהל מערכת", `שם משתמש: ${safeTrim(prev.username) || "-"} ← ${username}`);
      const r = await App.persist("פרטי מנהל מערכת נשמרו");
      if(r.ok){
        Auth._saveSession(Auth.current);
        UI.renderAuthPill();
        this._showOwnAuthError("");
        this.render();
      } else {
        this._showOwnAuthError(r.error || "אירעה שגיאה בשמירה");
      }
    }
  };
...
const App = {
    ...
    async boot(){
      ...
      if (Auth.current) {
        ...
        if (Auth.isAdmin()) SettingsUI.render();
      } else {
        UI.goView("dashboard");
      }
    },
