...
(() => {
  ...
  const BUILD = "20260328-clal-risk-original-pdf-assets-v5";
...
// open health covers drawer (Health only)
    ...
    parseHarBituachWorkbook(buffer, ins){
      ...
      rows.slice(headerIdx + 1).forEach((row) => {
        ...
        // בהר הביטוח אנו מציגים רק פרמיה חודשית.
        // אם הקובץ מכיל ערך שנתי — נמיר אותו לחודשי, אך לא נציג/נשמור פרמיה שנתית נפרדת.
          ...
          <tr class="lcPolMetaRow">
            <td colspan="7">
              <div class="lcPolMetaLine">
                ...
                ${annualPremiumTxt && !p.importedFromHarBituach ? `<span class="lcPolMetaBadge"><b>פרמיה שנתית:</b> ${escapeHtml(annualPremiumTxt)} ₪</span>` : ""}
...
const SystemRepairUI = {
  ...
  const NewCustomerEntryUI = {
    ...
    handleType(type){
      if(type === "health"){
        this.showStatus("פותח בחירת חברה ומוצר…", "ready");
        window.setTimeout(() => {
          this.close();
          try{
            HealthRoutePickerUI.open();
          }catch(err){
            console.error("NEW_CUSTOMER_HEALTH_ROUTE_OPEN_FAILED:", err);
            this.showStatus("אירעה תקלה בפתיחת בחירת המסלול", "dev");
          }
        }, 140);
        return;
      }
  ...
  const HealthRoutePickerUI = {
    els: {},
    init(){
      this.els.modal = document.getElementById("lcHealthRouteModal");
      this.els.btnClose = document.getElementById("lcHealthRouteModalClose");
      this.els.status = document.getElementById("lcHealthRouteStatus");
      if(!this.els.modal) return;
      on(this.els.btnClose, "click", () => this.close());
      on(this.els.modal, "click", (ev) => {
        const t = ev.target;
        if(t && t.getAttribute && t.getAttribute("data-close") === "1") return this.close();
        const card = t && t.closest ? t.closest("[data-route-company][data-route-product]") : null;
        if(!card) return;
        const company = safeTrim(card.getAttribute("data-route-company"));
        const product = safeTrim(card.getAttribute("data-route-product"));
        this.choose(company, product);
      });
    },
    isOpen(){ return !!this.els.modal && this.els.modal.classList.contains("is-open"); },
    setStatus(msg){ if(this.els.status) this.els.status.textContent = msg || ""; },
    open(){
      try{ Wizard?.close?.(); }catch(_e){}
      try{ ClalRiskWizard?.close?.(true); }catch(_e){}
      this.setStatus("");
      this.els.modal?.classList.add("is-open");
      this.els.modal?.setAttribute("aria-hidden", "false");
      document.body.classList.add("modal-open");
    ...
    choose(company, product){
      const isClalRisk = company === "כלל" && product === "ריסק";
      this.setStatus(isClalRisk ? "פותח מסלול ייעודי לכלל ריסק…" : `פותח את הוויזארד הרגיל עבור ${company} · ${product}`);
      window.setTimeout(() => {
        this.close();
        try{
          if(isClalRisk){
            ClalRiskWizard.reset({ company, product });
            ClalRiskWizard.open();
          } else {
            Wizard.entryContext = { company, product, routeMode: "regular" };
            prepareInteractiveWizardOpen();
            Wizard.reset();
            Wizard.open();
          }
        }catch(err){
          console.error("HEALTH_ROUTE_CHOOSE_FAILED:", err);
          this.setStatus("אירעה תקלה בפתיחת המסלול");
          this.open();
        }
      }, 120);
    }
...
const ClalRiskWizard = {
    els: {},
    isOpen: false,
    step: 1,
    context: { company: "כלל", product: "ריסק" },
    steps: [
      { id:1, title:"פרטי מועמד" },
      { id:2, title:"שאלות סיכון" },
      { id:3, title:"מוטבים" },
      { id:4, title:"תאריך ופרטי בסיס" },
      { id:5, title:"כיסויי כלל ריסק" }
    ],
    state: {},
    init(){
      this.els.modal = document.getElementById("lcClalRiskWizard");
      this.els.close = document.getElementById("lcClalRiskWizardClose");
      this.els.body = document.getElementById("lcClalRiskWizardBody");
      this.els.steps = document.getElementById("lcClalRiskWizardSteps");
      this.els.meta = document.getElementById("lcClalRiskWizardMeta");
      this.els.btnPrev = document.getElementById("lcClalRiskPrev");
      this.els.btnNext = document.getElementById("lcClalRiskNext");
      this.els.btnSave = document.getElementById("lcClalRiskSave");
      this.els.btnExport = document.getElementById("lcClalRiskExport");
      if(!this.els.modal) return;
      on(this.els.close, "click", () => this.close());
      on(this.els.modal, "click", (ev) => {
        const t = ev.target;
      ...
      on(this.els.btnSave, "click", () => this.saveDraft());
      on(this.els.btnExport, "click", () => this.exportPdf());
      on(this.els.body, "input", (ev) => this.handleField(ev));
      on(this.els.body, "change", (ev) => this.handleField(ev));
    },
    defaultState(){
      return {
        insured: {
          firstName:"", lastName:"", idNumber:"", birthDate:"", gender:"",
          maritalStatus:"", childrenCount:"", mobile:"", phone:"",
          email:"", occupation:"", street:"", houseNumber:"", city:"", zipCode:""
        },
        risk: {
          isSmoker:"", workHazards:"", workHazardsDetails:"",
          hobbyHazards:"", hobbyHazardsDetails:"",
          abroad180:"", abroad180Details:"",
          israeliCitizen:""
        },
        beneficiaries: [{ fullName:"", idNumber:"", relationship:"", sharePercent:"100" }],
        basics: { startDate:"", docsDelivery:"דיגיטלי", initiatedMarketing:"לא" },
        covers: {
          sapphire: { enabled:false, amount:"", termAge:"80" },
          sapphire10: { enabled:false, amount:"", termAge:"80" },
          sapphire15: { enabled:false, amount:"", termAge:"80" },
          accidentalDeath: { enabled:false, amount:"", termAge:"75" },
          accidentalDisability: { enabled:false, amount:"", termAge:"65" },
          waiverAndDisability: {
            enabled:false, monthlyBenefit:"", termAge:"67",
            specificOccupation:false, franchise:false,
            noBituachLeumiOffset:false, shortWaiting:false
          }
        }
      };
    },
    reset(ctx = {}){
      this.context = { company: ctx.company || "כלל", product: ctx.product || "ריסק" };
      this.step = 1;
      this.state = this.defaultState();
      try{
        const raw = localStorage.getItem("GI_CLAL_RISK_DRAFT_V1");
        if(raw){
          const draft = JSON.parse(raw);
          if(draft && draft.state) this.state = this.mergeDeep(this.defaultState(), draft.state);
        }
      }catch(_e){}
    },
    mergeDeep(base, patch){
      if(Array.isArray(base)) return Array.isArray(patch) ? patch : base;
      if(!base || typeof base !== 'object') return (patch == null ? base : patch);
      const out = { ...base };
      if(patch && typeof patch === 'object'){
        Object.keys(patch).forEach((key) => {
          const nextBase = out[key];
          const nextPatch = patch[key];
          if(Array.isArray(nextBase)) out[key] = Array.isArray(nextPatch) ? nextPatch : nextBase;
          else if(nextBase && typeof nextBase === 'object') out[key] = this.mergeDeep(nextBase, nextPatch);
          else out[key] = nextPatch;
        });
      }
      return out;
    },
    ...
    close(skipBodyUnlock = false){
      this.isOpen = false;
      this.els.modal?.classList.remove("is-open");
      this.els.modal?.setAttribute("aria-hidden", "true");
      if(skipBodyUnlock) return;
      const hasOtherOpenModal = !!document.querySelector('.modal.is-open, .drawer.is-open, .lcWizard.is-open');
      if(!hasOtherOpenModal) document.body.classList.remove("modal-open");
    },
    prev(){ if(this.step > 1){ this.step -= 1; this.render(); } },
    next(){ if(this.step < this.steps.length){ this.step += 1; this.render(); } else { this.saveDraft(true); } },
    saveDraft(showDone = false){
      const payload = {
        context: this.context,
        state: this.state,
        clalRiskFormData: this.getFormData(),
        updatedAt: nowISO()
      };
      try{ localStorage.setItem("GI_CLAL_RISK_DRAFT_V1", JSON.stringify(payload)); }catch(_e){}
      if(showDone) this.els.meta.textContent = "טיוטת כלל ריסק נשמרה. חיבור ה־PDF הראשוני מוכן להורדה.";
      else this.els.meta.textContent = "טיוטת כלל ריסק נשמרה מקומית בדפדפן.";
    },
    handleField(ev){
      const el = ev.target;
      if(!el || !el.name) return;
      const val = el.type === 'checkbox' ? !!el.checked : safeTrim(el.value);
      const path = el.name.split('.');
      let ref = this.state;
      while(path.length > 1){
        const key = path.shift();
        if(!(key in ref) || typeof ref[key] !== 'object' || ref[key] === null) ref[key] = {};
        ref = ref[key];
      }
      ref[path[0]] = val;
      if(this.step === 5) this.render();
    },
    render(){
      if(!this.els.modal) return;
      this.els.steps.innerHTML = this.steps.map(step => `<div class="lcClalRiskWizard__step ${this.step===step.id?'is-active':''}"><span>${step.id}</span><span>${escapeHtml(step.title)}</span></div>`).join('');
      this.els.meta.textContent = `פאזה 2 · ${this.context.company} ${this.context.product} · שלב ${this.step} מתוך ${this.steps.length}`;
      this.els.btnPrev.disabled = this.step === 1;
      this.els.btnNext.textContent = this.step === this.steps.length ? 'סיום' : 'הבא';
      if(this.els.btnExport) this.els.btnExport.disabled = this.step !== this.steps.length;
      this.els.body.innerHTML = this.renderCurrentStep();
    },
    input(label, name, value='', type='text', attrs=''){
      return `<div class="lcClalRiskField"><label>${escapeHtml(label)}</label><input type="${type}" name="${escapeHtml(name)}" value="${escapeHtml(value||'')}" ${attrs}></div>`;
    },
    select(label, name, value, options){
      return `<div class="lcClalRiskField"><label>${escapeHtml(label)}</label><select name="${escapeHtml(name)}">${options.map(o=>`<option value="${escapeHtml(o)}"${String(o)===String(value)?' selected':''}>${escapeHtml(o)}</option>`).join('')}</select></div>`;
    },
    renderRiskDetailField(label, toggleValue, name, value, placeholder){
      if(toggleValue !== 'כן') return '';
      return this.input(label, name, value, 'text', `placeholder="${escapeHtml(placeholder || 'פרט כאן')}"`);
    },
    renderCoverToggle(path, label, checked){
      return `<label class="lcClalRiskChip"><input type="checkbox" name="${path}" ${checked?'checked':''}> <span>${escapeHtml(label)}</span></label>`;
    },
    renderCoverConfig(title, coverKey, config){
      if(!config?.enabled) return '';
      if(coverKey === 'waiverAndDisability'){
        return `<div class="lcClalRiskCard"><div class="lcClalRiskCard__title">${escapeHtml(title)}</div>
          <div class="lcClalRiskGrid lcClalRiskGrid--compact">
            ${this.input('פיצוי חודשי מבוקש',`covers.${coverKey}.monthlyBenefit`,config.monthlyBenefit,'text','inputmode="numeric"')}
            ${this.select('גיל תום',`covers.${coverKey}.termAge`,config.termAge,['60','62','64','65','67'])}
          </div>
          <div class="lcClalRiskInline">
            ${this.renderCoverToggle(`covers.${coverKey}.specificOccupation`,'עיסוק ספציפי',config.specificOccupation)}
            ${this.renderCoverToggle(`covers.${coverKey}.franchise`,'פרנצ׳יזה',config.franchise)}
            ${this.renderCoverToggle(`covers.${coverKey}.noBituachLeumiOffset`,'ביטול קיזוז ביטוח לאומי',config.noBituachLeumiOffset)}
            ${this.renderCoverToggle(`covers.${coverKey}.shortWaiting`,'קיצור המתנה ל-1 חודש',config.shortWaiting)}
          </div>
        </div>`;
      }
      return `<div class="lcClalRiskCard"><div class="lcClalRiskCard__title">${escapeHtml(title)}</div>
        <div class="lcClalRiskGrid lcClalRiskGrid--compact">
          ${this.input('סכום ביטוח',`covers.${coverKey}.amount`,config.amount,'text','inputmode="numeric"')}
          ${this.select('גיל תום',`covers.${coverKey}.termAge`,config.termAge,['65','67','75','80'])}
        </div>
      </div>`;
    },
    renderCurrentStep(){
      const s = this.state;
      if(this.step === 1){
        return `<div class="lcClalRiskGrid">
          <div class="lcClalRiskCard"><div class="lcClalRiskCard__title">פרטי מועמד לביטוח</div>
            ${this.input('שם פרטי','insured.firstName',s.insured.firstName)}
            ${this.input('שם משפחה','insured.lastName',s.insured.lastName)}
            ${this.input('תעודת זהות','insured.idNumber',s.insured.idNumber,'text','inputmode="numeric"')}
            ${this.input('תאריך לידה','insured.birthDate',s.insured.birthDate,'text','placeholder="DD/MM/YYYY" inputmode="numeric"')}
            ${this.select('מין','insured.gender',s.insured.gender,['','זכר','נקבה'])}
            ${this.select('מצב משפחתי','insured.maritalStatus',s.insured.maritalStatus,['','רווק','נשוי','גרוש','אלמן'])}
            ${this.input('מספר ילדים','insured.childrenCount',s.insured.childrenCount,'text','inputmode="numeric"')}
          </div>
          <div class="lcClalRiskCard"><div class="lcClalRiskCard__title">פרטי קשר וכתובת</div>
            ${this.input('נייד','insured.mobile',s.insured.mobile,'text','inputmode="numeric"')}
            ${this.input('טלפון נוסף','insured.phone',s.insured.phone,'text','inputmode="numeric"')}
            ${this.input('דוא״ל','insured.email',s.insured.email,'email')}
            ${this.input('עיסוק','insured.occupation',s.insured.occupation)}
            ${this.input('רחוב','insured.street',s.insured.street)}
            ${this.input('מספר בית','insured.houseNumber',s.insured.houseNumber)}
            ${this.input('יישוב','insured.city',s.insured.city)}
            ${this.input('מיקוד','insured.zipCode',s.insured.zipCode,'text','inputmode="numeric"')}
          </div>
        </div>`;
      }
      if(this.step === 2){
        return `<div class="lcClalRiskGrid">
          <div class="lcClalRiskCard"><div class="lcClalRiskCard__title">שאלות סיכון ראשוניות</div>
            ${this.select('האם מעשן/ת בשנתיים האחרונות?','risk.isSmoker',s.risk.isSmoker,['','כן','לא'])}
            ${this.select('חשיפה לסיכונים בעבודה','risk.workHazards',s.risk.workHazards,['','כן','לא'])}
            ${this.renderRiskDetailField('פירוט סיכוני עבודה','כן','risk.workHazardsDetails',s.risk.workHazardsDetails,'טיסה, כימיקלים, קרינה וכו׳')}
            ${this.select('חשיפה לספורט/תחביב מסוכן','risk.hobbyHazards',s.risk.hobbyHazards,['','כן','לא'])}
            ${this.renderRiskDetailField('פירוט תחביב/ספורט','כן','risk.hobbyHazardsDetails',s.risk.hobbyHazardsDetails,'צלילה, אומנויות לחימה וכו׳')}
          </div>
          <div class="lcClalRiskCard"><div class="lcClalRiskCard__title">שהייה ואזרחות</div>
            ${this.select('שהייה בחו״ל 180+ יום','risk.abroad180',s.risk.abroad180,['','כן','לא'])}
            ${this.renderRiskDetailField('פירוט שהייה בחו״ל','כן','risk.abroad180Details',s.risk.abroad180Details,'מדינה / תקופה')}
            ${this.select('אזרחות ישראלית','risk.israeliCitizen',s.risk.israeliCitizen,['','כן','לא'])}
            <div class="muted small">שלב זה מחובר למיפוי הראשוני של עמוד 1 בטופס כלל ריסק.</div>
          </div>
        </div>`;
      }
      if(this.step === 3){
        const ben = (s.beneficiaries||[])[0] || {};
        return `<div class="lcClalRiskGrid">
          <div class="lcClalRiskCard"><div class="lcClalRiskCard__title">מוטב ראשי</div>
            ${this.input('שם מלא','beneficiaries.0.fullName',ben.fullName)}
            ${this.input('תעודת זהות','beneficiaries.0.idNumber',ben.idNumber,'text','inputmode="numeric"')}
            ${this.input('קרבה','beneficiaries.0.relationship',ben.relationship)}
            ${this.input('חלק באחוזים','beneficiaries.0.sharePercent',ben.sharePercent,'text','inputmode="numeric"')}
          </div>
          <div class="lcClalRiskCard"><div class="lcClalRiskCard__title">מיפוי מוטבים</div><div class="muted">בשלב הזה החיבור ממלא את שורת המוטב הראשית על גבי הטופס המקורי של כלל. בהמשך נוסיף גם מספר שורות מוטבים נוספות.</div></div>
        </div>`;
      }
      if(this.step === 4){
        return `<div class="lcClalRiskGrid">
          <div class="lcClalRiskCard"><div class="lcClalRiskCard__title">פרטי בסיס להצעה</div>
            ${this.input('תאריך תחילת ביטוח','basics.startDate',s.basics.startDate,'text','placeholder="DD/MM/YYYY" inputmode="numeric"')}
            ${this.select('אופן קבלת מסמכים','basics.docsDelivery',s.basics.docsDelivery,['דיגיטלי','דואר רגיל'])}
            ${this.select('שיווק יזום','basics.initiatedMarketing',s.basics.initiatedMarketing,['לא','כן'])}
          </div>
          <div class="lcClalRiskCard"><div class="lcClalRiskCard__title">חיבור PDF</div><div class="muted">היצוא החדש עובד על גבי הטופס המקורי של כלל (כל 8 העמודים). כרגע ממופים עמודי הפרטים והכיסויים, ושאר העמודים מצורפים מתוך הטופס המקורי כדי שהקובץ שירד יהיה תואם למסמך החברה.</div></div>
        </div>`;
      }
      const checks = [
        ['covers.sapphire.enabled','ספיר'],
        ['covers.sapphire10.enabled','ספיר 10'],
        ['covers.sapphire15.enabled','ספיר 15'],
        ['covers.accidentalDeath.enabled','מוות מתאונה'],
        ['covers.accidentalDisability.enabled','נכות מתאונה'],
        ['covers.waiverAndDisability.enabled','אכ״ע - פיצוי ושחרור']
      ];
      return `<div class="lcClalRiskSummary">
        <div class="lcClalRiskCard">
          <div class="lcClalRiskCard__title">בחירת כיסויי כלל ריסק</div>
          <div class="lcClalRiskInline">${checks.map(([name,label]) => `<label class="lcClalRiskChip"><input type="checkbox" name="${name}" ${name.split('.').reduce((o,k)=>o&&o[k], s)?'checked':''}> <span>${escapeHtml(label)}</span></label>`).join('')}</div>
          <div class="muted small" style="margin-top:12px;">הפעימה הנוכחית מחברת את בחירת הכיסויים ואת נתוני הסכומים/גיל תום לייצוא PDF ראשוני.</div>
        </div>
        ${this.renderCoverConfig('כיסוי ספיר', 'sapphire', s.covers.sapphire)}
        ${this.renderCoverConfig('כיסוי ספיר 10', 'sapphire10', s.covers.sapphire10)}
        ${this.renderCoverConfig('כיסוי ספיר 15', 'sapphire15', s.covers.sapphire15)}
        ${this.renderCoverConfig('כיסוי מוות מתאונה', 'accidentalDeath', s.covers.accidentalDeath)}
        ${this.renderCoverConfig('כיסוי נכות מתאונה', 'accidentalDisability', s.covers.accidentalDisability)}
        ${this.renderCoverConfig('אכ״ע - פיצוי ושחרור', 'waiverAndDisability', s.covers.waiverAndDisability)}
        <div class="lcClalRiskCard"><div class="lcClalRiskCard__title">סיכום פאזה 2</div><div class="lcClalRiskSummary">
          <div class="lcClalRiskSummaryRow"><b>מבוטח</b><span>${escapeHtml([s.insured.firstName,s.insured.lastName].filter(Boolean).join(' ') || '—')}</span></div>
          <div class="lcClalRiskSummaryRow"><b>מוצר</b><span>${escapeHtml(this.context.company)} · ${escapeHtml(this.context.product)}</span></div>
          <div class="lcClalRiskSummaryRow"><b>כיסויים שנבחרו</b><span>${escapeHtml(this.getSelectedCoverLabels().join(' • ') || 'לא נבחרו')}</span></div>
          <div class="lcClalRiskSummaryRow"><b>יצוא</b><span>לחץ על “הורד PDF” כדי להפיק את טופס כלל ריסק על גבי המסמך המקורי של החברה.</span></div>
        </div></div>
      </div>`;
    },
    getSelectedCoverLabels(){
      const c = this.state.covers || {};
      const labels = [];
      if(c.sapphire?.enabled) labels.push('ספיר');
      if(c.sapphire10?.enabled) labels.push('ספיר 10');
      if(c.sapphire15?.enabled) labels.push('ספיר 15');
      if(c.accidentalDeath?.enabled) labels.push('מוות מתאונה');
      if(c.accidentalDisability?.enabled) labels.push('נכות מתאונה');
      if(c.waiverAndDisability?.enabled) labels.push('אכ״ע - פיצוי ושחרור');
      return labels;
    },
    getFormData(){
      const s = this.state;
      const beneficiary = (s.beneficiaries || [])[0] || {};
      return {
        ...
        covers: JSON.parse(JSON.stringify(s.covers || {})),
        generatedAt: nowISO(),
        selectedCoverLabels: this.getSelectedCoverLabels()
      };
    },
    pdfText(textValue, left, top, cls=''){
      if(!safeTrim(textValue)) return '';
      return `<div class="lcClalRiskPdfText ${cls}" style="left:${left}%; top:${top}%;">${escapeHtml(textValue)}</div>`;
    },
    pdfCheck(checked, left, top){
      if(!checked) return '';
      return `<div class="lcClalRiskPdfCheck" style="left:${left}%; top:${top}%;">✓</div>`;
    },
    buildPdfMarkup(data){
      return `<div class="lcClalRiskPdfRoot" dir="rtl">
        ${this.buildPdfPage1(data)}
        ${this.buildPdfPage2(data)}
        ${this.buildPdfPage3(data)}
      </div>`;
    },
    buildPdfPage1(data){
      const d = data;
      const genderMale = d.insured.gender === 'זכר';
      const genderFemale = d.insured.gender === 'נקבה';
      const marital = safeTrim(d.insured.maritalStatus);
      const risk = d.risk || {};
      const ben = d.beneficiary || {};
      return `<div class="lcClalRiskPdfPage">
        <img class="lcClalRiskPdfBg" src="./clal-risk-page-1.png" alt="כלל ריסק עמוד 1">
        ${this.pdfText(d.insured.lastName, 64.4, 30.6)}
        ${this.pdfText(d.insured.firstName, 52.3, 30.6)}
        ${this.pdfText(d.insured.idNumber, 39.1, 30.6,'lcClalRiskPdfText--ltr')}
        ${this.pdfText(d.insured.birthDate, 27.5, 30.6,'lcClalRiskPdfText--ltr')}
        ${this.pdfCheck(genderMale, 16.9, 30.6)}
        ${this.pdfCheck(genderFemale, 12.0, 30.6)}
        ${this.pdfCheck(marital === 'רווק', 89.6, 34.7)}
        ${this.pdfCheck(marital === 'נשוי', 85.4, 34.7)}
        ${this.pdfCheck(marital === 'גרוש', 81.1, 34.7)}
        ${this.pdfCheck(marital === 'אלמן', 76.8, 34.7)}
        ${this.pdfText(d.insured.childrenCount, 60.2, 35.0,'lcClalRiskPdfText--ltr')}
        ${this.pdfText(d.insured.mobile, 46.9, 35.0,'lcClalRiskPdfText--ltr')}
        ${this.pdfText(d.insured.phone, 33.3, 35.0,'lcClalRiskPdfText--ltr')}
        ${this.pdfText(d.insured.email, 18.0, 35.0,'lcClalRiskPdfText--small lcClalRiskPdfText--ltr')}
        ${this.pdfText(d.insured.street, 63.2, 39.7)}
        ${this.pdfText(d.insured.houseNumber, 47.7, 39.7,'lcClalRiskPdfText--ltr')}
        ${this.pdfText(d.insured.city, 37.7, 39.7)}
        ${this.pdfText(d.insured.zipCode, 29.0, 39.7,'lcClalRiskPdfText--ltr')}
        ${this.pdfText(d.insured.occupation, 16.2, 39.7)}
        ${this.pdfCheck(d.basics.docsDelivery === 'דיגיטלי', 39.2, 46.0)}
        ${this.pdfCheck(d.basics.docsDelivery === 'דואר רגיל', 29.4, 46.0)}
        ${this.pdfCheck(d.basics.initiatedMarketing === 'כן', 10.6, 17.4)}
        ${this.pdfCheck(risk.abroad180 === 'כן', 36.7, 66.0)}
        ${this.pdfCheck(risk.abroad180 === 'לא', 42.8, 66.0)}
        ${this.pdfText(risk.abroad180Details, 18.6, 65.8,'lcClalRiskPdfText--tiny')}
        ${this.pdfCheck(risk.israeliCitizen === 'כן', 36.7, 70.1)}
        ${this.pdfCheck(risk.israeliCitizen === 'לא', 42.8, 70.1)}
        ${this.pdfCheck(risk.isSmoker === 'כן', 36.7, 74.2)}
        ${this.pdfCheck(risk.isSmoker === 'לא', 42.8, 74.2)}
        ${this.pdfText('מעשן/ת בשנתיים האחרונות', 15.9, 74.3,'lcClalRiskPdfText--tiny')}
        ${this.pdfCheck(risk.workHazards === 'כן', 36.7, 78.2)}
        ${this.pdfCheck(risk.workHazards === 'לא', 42.8, 78.2)}
        ${this.pdfText(risk.workHazardsDetails, 15.5, 78.2,'lcClalRiskPdfText--tiny')}
        ${this.pdfCheck(risk.hobbyHazards === 'כן', 36.7, 82.2)}
        ${this.pdfCheck(risk.hobbyHazards === 'לא', 42.8, 82.2)}
        ${this.pdfText(risk.hobbyHazardsDetails, 15.5, 82.2,'lcClalRiskPdfText--tiny')}
        ${this.pdfText(ben.fullName, 63.6, 92.0)}
        ${this.pdfText(ben.idNumber, 48.5, 92.0,'lcClalRiskPdfText--ltr')}
        ${this.pdfText(ben.relationship, 29.2, 92.0)}
        ${this.pdfText(ben.sharePercent, 10.4, 92.0,'lcClalRiskPdfText--ltr')}
        ${this.pdfText(d.basics.startDate, 63.0, 97.4,'lcClalRiskPdfText--ltr')}
      </div>`;
    },
    buildPdfPage2(data){
      const c = data.covers || {};
      return `<div class="lcClalRiskPdfPage">
        <img class="lcClalRiskPdfBg" src="./clal-risk-page-2.png" alt="כלל ריסק עמוד 2">
        ${this.pdfCheck(c.sapphire?.enabled, 91.6, 22.5)}
        ${this.pdfText(c.sapphire?.amount, 55.7, 22.9,'lcClalRiskPdfText--ltr')}
        ${this.pdfCheck(c.sapphire10?.enabled, 91.6, 28.1)}
        ${this.pdfText(c.sapphire10?.amount, 55.7, 28.4,'lcClalRiskPdfText--ltr')}
        ${this.pdfCheck(c.sapphire15?.enabled, 91.6, 33.7)}
        ${this.pdfText(c.sapphire15?.amount, 55.7, 34.1,'lcClalRiskPdfText--ltr')}
        ${this.pdfCheck(c.accidentalDeath?.enabled, 91.6, 39.4)}
        ${this.pdfText(c.accidentalDeath?.amount, 55.7, 39.8,'lcClalRiskPdfText--ltr')}
        ${this.pdfCheck(c.accidentalDisability?.enabled, 91.6, 45.1)}
        ${this.pdfText(c.accidentalDisability?.amount, 55.7, 45.4,'lcClalRiskPdfText--ltr')}
        ${this.pdfCheck(c.waiverAndDisability?.enabled, 91.6, 51.1)}
        ${this.pdfText(c.waiverAndDisability?.monthlyBenefit, 55.7, 51.4,'lcClalRiskPdfText--ltr')}
        ${this.pdfCheck(c.waiverAndDisability?.termAge === '60', 61.5, 53.0)}
        ${this.pdfCheck(c.waiverAndDisability?.termAge === '62', 61.5, 56.9)}
        ${this.pdfCheck(c.waiverAndDisability?.termAge === '64', 61.5, 60.8)}
        ${this.pdfCheck(c.waiverAndDisability?.termAge === '65', 61.5, 64.8)}
        ${this.pdfCheck(c.waiverAndDisability?.termAge === '67', 61.5, 68.7)}
        ${this.pdfCheck(c.waiverAndDisability?.specificOccupation, 91.6, 58.8)}
        ${this.pdfCheck(c.waiverAndDisability?.franchise, 91.6, 62.5)}
        ${this.pdfCheck(c.waiverAndDisability?.noBituachLeumiOffset, 91.6, 66.0)}
        ${this.pdfCheck(c.waiverAndDisability?.shortWaiting, 91.6, 69.8)}
      </div>`;
    },
    buildPdfPage3(data){
      return `<div class="lcClalRiskPdfPage">
        <img class="lcClalRiskPdfBg" src="./clal-risk-page-3.png" alt="כלל ריסק עמוד 3">
      </div>`;
    },
...
makeTextImage(textValue, opts = {}){
  const value = safeTrim(textValue);
  if(!value) return null;
  const width = Math.max(20, Math.round(Number(opts.width) || 120));
  const height = Math.max(18, Math.round(Number(opts.height) || 24));
  const fontSize = Math.max(8, Math.round(Number(opts.fontSize) || 12));
  const color = safeTrim(opts.color) || '#17386d';
  const align = safeTrim(opts.align) || 'right';
  const rtl = opts.rtl !== false;
  const bold = opts.bold !== false;
  const canvas = document.createElement('canvas');
  const ratio = window.devicePixelRatio && window.devicePixelRatio > 1 ? 2 : 1;
  ...
  ctx.font = `${bold ? '700' : '400'} ${fontSize}px system-ui, -apple-system, "Segoe UI", Arial, "Noto Sans Hebrew", sans-serif`;
  ctx.textAlign = align;
...
makeCheckImage(mark = '✓', opts = {}){
  const size = Math.max(12, Math.round(Number(opts.size) || 16));
  const canvas = document.createElement('canvas');
  const ratio = window.devicePixelRatio && window.devicePixelRatio > 1 ? 2 : 1;
  ...
  ctx.fillStyle = safeTrim(opts.color) || '#17386d';
  ctx.textBaseline = 'middle';
  ctx.textAlign = 'center';
  ctx.font = `900 ${Math.max(12, size - 1)}px system-ui, -apple-system, "Segoe UI", Arial, sans-serif`;
  ctx.fillText(mark, size / 2, size / 2 + 0.5);
  return { dataUrl: canvas.toDataURL('image/png'), width: size, height: size };
},
async buildPdfAssets(data){
  const d = data;
  const genderMale = d.insured.gender === 'זכר';
  const genderFemale = d.insured.gender === 'נקבה';
  const marital = safeTrim(d.insured.maritalStatus);
  ...
  const page1 = [
    { type:'text', value:d.insured.lastName, x:64.4, y:30.6, w:10.8, h:2.2, fs:11, align:'right', rtl:true },
    { type:'text', value:d.insured.firstName, x:52.3, y:30.6, w:10.8, h:2.2, fs:11, align:'right', rtl:true },
    { type:'text', value:d.insured.idNumber, x:39.1, y:30.6, w:10.2, h:2.2, fs:11, align:'left', rtl:false },
    { type:'text', value:d.insured.birthDate, x:27.5, y:30.6, w:9.0, h:2.2, fs:11, align:'left', rtl:false },
    { type:'check', checked:genderMale, x:16.9, y:30.6, size:1.35 },
    { type:'check', checked:genderFemale, x:12.0, y:30.6, size:1.35 },

    { type:'check', checked:marital === 'רווק', x:89.6, y:34.7, size:1.25 },
    { type:'check', checked:marital === 'נשוי', x:85.4, y:34.7, size:1.25 },
    { type:'check', checked:marital === 'גרוש', x:81.1, y:34.7, size:1.25 },
    { type:'check', checked:marital === 'אלמן', x:76.8, y:34.7, size:1.25 },

    { type:'text', value:d.insured.childrenCount, x:60.2, y:35.0, w:8.0, h:2.0, fs:10, align:'left', rtl:false },
    { type:'text', value:d.insured.mobile, x:46.9, y:35.0, w:12.0, h:2.0, fs:10, align:'left', rtl:false },
    { type:'text', value:d.insured.phone, x:33.3, y:35.0, w:11.5, h:2.0, fs:10, align:'left', rtl:false },
    { type:'text', value:d.insured.email, x:18.0, y:35.0, w:14.5, h:2.0, fs:8.7, align:'left', rtl:false },

    { type:'text', value:d.insured.street, x:63.2, y:39.7, w:13.5, h:2.0, fs:10, align:'right', rtl:true },
    { type:'text', value:d.insured.houseNumber, x:47.7, y:39.7, w:7.0, h:2.0, fs:10, align:'left', rtl:false },
    { type:'text', value:d.insured.city, x:37.7, y:39.7, w:9.0, h:2.0, fs:10, align:'right', rtl:true },
    { type:'text', value:d.insured.zipCode, x:29.0, y:39.7, w:7.5, h:2.0, fs:10, align:'left', rtl:false },
    { type:'text', value:d.insured.occupation, x:16.2, y:39.7, w:11.2, h:2.0, fs:9.5, align:'right', rtl:true },

    { type:'check', checked:d.basics.docsDelivery === 'דיגיטלי', x:39.2, y:46.0, size:1.2 },
    { type:'check', checked:d.basics.docsDelivery === 'דואר רגיל', x:29.4, y:46.0, size:1.2 },
    { type:'check', checked:d.basics.initiatedMarketing === 'כן', x:10.6, y:17.4, size:1.2 },

    { type:'check', checked:risk.abroad180 === 'כן', x:36.7, y:66.0, size:1.2 },
    { type:'check', checked:risk.abroad180 === 'לא', x:42.8, y:66.0, size:1.2 },
    { type:'text', value:risk.abroad180Details, x:18.6, y:65.8, w:16.0, h:2.0, fs:8.4, align:'right', rtl:true },

    { type:'check', checked:risk.israeliCitizen === 'כן', x:36.7, y:70.1, size:1.2 },
    { type:'check', checked:risk.israeliCitizen === 'לא', x:42.8, y:70.1, size:1.2 },
    { type:'check', checked:risk.isSmoker === 'כן', x:36.7, y:74.2, size:1.2 },
    { type:'check', checked:risk.isSmoker === 'לא', x:42.8, y:74.2, size:1.2 },
    { type:'text', value:'מעשן/ת בשנתיים האחרונות', x:15.9, y:74.3, w:16.0, h:2.0, fs:8.4, align:'right', rtl:true },

    { type:'check', checked:risk.workHazards === 'כן', x:36.7, y:78.2, size:1.2 },
    { type:'check', checked:risk.workHazards === 'לא', x:42.8, y:78.2, size:1.2 },
    { type:'text', value:risk.workHazardsDetails, x:15.5, y:78.2, w:16.0, h:2.0, fs:8.4, align:'right', rtl:true },

    { type:'check', checked:risk.hobbyHazards === 'כן', x:36.7, y:82.2, size:1.2 },
    { type:'check', checked:risk.hobbyHazards === 'לא', x:42.8, y:82.2, size:1.2 },
    { type:'text', value:risk.hobbyHazardsDetails, x:15.5, y:82.2, w:16.0, h:2.0, fs:8.4, align:'right', rtl:true },

    { type:'text', value:ben.fullName, x:63.6, y:92.0, w:18.0, h:2.0, fs:10, align:'right', rtl:true },
    { type:'text', value:ben.idNumber, x:48.5, y:92.0, w:11.0, h:2.0, fs:10, align:'left', rtl:false },
    { type:'text', value:ben.relationship, x:29.2, y:92.0, w:15.0, h:2.0, fs:10, align:'right', rtl:true },
    { type:'text', value:ben.sharePercent, x:10.4, y:92.0, w:8.0, h:2.0, fs:10, align:'left', rtl:false },
    { type:'text', value:d.basics.startDate, x:63.0, y:97.4, w:13.0, h:2.0, fs:10, align:'left', rtl:false }
  ];

  const page2 = [
    { type:'check', checked:c.sapphire?.enabled, x:91.6, y:22.5, size:1.2 },
    { type:'text', value:c.sapphire?.amount, x:55.7, y:22.9, w:10.0, h:2.0, fs:11, align:'left', rtl:false },
    { type:'check', checked:c.sapphire10?.enabled, x:91.6, y:28.1, size:1.2 },
    { type:'text', value:c.sapphire10?.amount, x:55.7, y:28.4, w:10.0, h:2.0, fs:11, align:'left', rtl:false },
    { type:'check', checked:c.sapphire15?.enabled, x:91.6, y:33.7, size:1.2 },
    { type:'text', value:c.sapphire15?.amount, x:55.7, y:34.1, w:10.0, h:2.0, fs:11, align:'left', rtl:false },
    { type:'check', checked:c.accidentalDeath?.enabled, x:91.6, y:39.4, size:1.2 },
    { type:'text', value:c.accidentalDeath?.amount, x:55.7, y:39.8, w:10.0, h:2.0, fs:11, align:'left', rtl:false },
    { type:'check', checked:c.accidentalDisability?.enabled, x:91.6, y:45.1, size:1.2 },
    { type:'text', value:c.accidentalDisability?.amount, x:55.7, y:45.4, w:10.0, h:2.0, fs:11, align:'left', rtl:false },
    { type:'check', checked:c.waiverAndDisability?.enabled, x:91.6, y:51.1, size:1.2 },
    { type:'text', value:c.waiverAndDisability?.monthlyBenefit, x:55.7, y:51.4, w:10.0, h:2.0, fs:11, align:'left', rtl:false },
    { type:'check', checked:c.waiverAndDisability?.termAge === '60', x:61.5, y:53.0, size:1.15 },
    { type:'check', checked:c.waiverAndDisability?.termAge === '62', x:61.5, y:56.9, size:1.15 },
    { type:'check', checked:c.waiverAndDisability?.termAge === '64', x:61.5, y:60.8, size:1.15 },
    { type:'check', checked:c.waiverAndDisability?.termAge === '65', x:61.5, y:64.8, size:1.15 },
    { type:'check', checked:c.waiverAndDisability?.termAge === '67', x:61.5, y:68.7, size:1.15 },
    { type:'check', checked:c.waiverAndDisability?.specificOccupation, x:91.6, y:58.8, size:1.2 },
    { type:'check', checked:c.waiverAndDisability?.franchise, x:91.6, y:62.5, size:1.2 },
    { type:'check', checked:c.waiverAndDisability?.noBituachLeumiOffset, x:91.6, y:66.0, size:1.2 },
    { type:'check', checked:c.waiverAndDisability?.shortWaiting, x:91.6, y:69.8, size:1.2 }
  ];

  const pages = [
    { background:'./clal-risk-page-1.png', fields: page1 },
    { background:'./clal-risk-page-2.png', fields: page2 },
    { background:'./clal-risk-page-3.png', fields: [] },
    { background:'./clal-risk-page-4.png', fields: [] },
    { background:'./clal-risk-page-5.png', fields: [] },
    { background:'./clal-risk-page-6.png', fields: [] },
    { background:'./clal-risk-page-7.png', fields: [] },
    { background:'./clal-risk-page-8.png', fields: [] }
  ];

  const renderedPages = [];
  for(const page of pages){
    const bg = await this.loadImageAsDataUrl(page.background);
    renderedPages.push({ ...page, backgroundData: bg });
  }
  return renderedPages;
},
drawPdfField(pdf, field, dims){
  if(!field) return;
  const { originX, originY, width, height } = dims;
  if(field.type === 'check'){
    if(!field.checked) return;
    const sizePt = width * ((Number(field.size) || 1.2) / 100);
    const img = this.makeCheckImage('✓', { size: Math.max(12, Math.round(sizePt)) });
    const x = originX + (width * (Number(field.x) / 100)) - (sizePt / 2);
    const y = originY + (height * (Number(field.y) / 100)) - (sizePt / 2);
    pdf.addImage(img.dataUrl, 'PNG', x, y, sizePt, sizePt, undefined, 'FAST');
    return;
  }
  const value = safeTrim(field.value);
  if(!value) return;
  const boxWidth = width * ((Number(field.w) || 10) / 100);
  const boxHeight = Math.max(12, height * ((Number(field.h) || 2.0) / 100));
  const fontSizePt = Math.max(8, Number(field.fs) || 10);
  const img = this.makeTextImage(value, {
    width: Math.max(24, Math.round(boxWidth * 1.5)),
    height: Math.max(16, Math.round(boxHeight * 1.8)),
    fontSize: Math.max(8, Math.round(fontSizePt * 1.45)),
    align: field.align || 'right',
    rtl: field.rtl !== false,
    bold: true
  });
  const x = originX + (width * (Number(field.x) / 100));
  const y = originY + (height * (Number(field.y) / 100)) - (boxHeight / 2);
  pdf.addImage(img.dataUrl, 'PNG', x, y, boxWidth, boxHeight, undefined, 'FAST');
},
  ...
  async exportPdf(){
    try{
      ...
      if(!safeTrim(data.insured.firstName) || !safeTrim(data.insured.lastName)){
        window.alert('לפני הורדת ה-PDF צריך למלא לפחות שם פרטי ושם משפחה.');
        return;
      }
      const triggerBtn = this.els.btnExport || this.els.btnSave || null;
      const originalText = triggerBtn ? triggerBtn.textContent : '';
      if(triggerBtn){
        triggerBtn.disabled = true;
        triggerBtn.textContent = 'מייצא PDF...';
      ...
      try{
        const renderedPages = await this.buildPdfAssets(data);
        if(!renderedPages.length) throw new Error('No Clal Risk PDF pages found');
        ...
        for(let i = 0; i < renderedPages.length; i += 1){
          ...
          if(i > 0) pdf.addPage();
          pdf.addImage(page.backgroundData.dataUrl, 'PNG', marginX, marginY, usableWidth, usableHeight, undefined, 'FAST');
          const dims = { originX: marginX, originY: marginY, width: usableWidth, height: usableHeight };
          (page.fields || []).forEach((field) => this.drawPdfField(pdf, field, dims));
        }

        const insuredName = [data.insured.firstName, data.insured.lastName].filter(Boolean).join(' ').trim() || 'מבוטח';
        const fileName = `clal-risk-${insuredName.replace(/\s+/g,'-')}.pdf`;
        pdf.save(fileName);
        this.els.meta.textContent = 'PDF כלל ריסק נוצר על גבי הטופס המקורי של כלל (8 עמודים).';
        this.saveDraft(false);
      } finally {
        if(triggerBtn){
          triggerBtn.disabled = this.step !== this.steps.length;
          triggerBtn.textContent = originalText || 'הורד PDF';
        }
      }
    }catch(err){
      console.error('CLAL_RISK_PDF_EXPORT_FAILED:', err);
      window.alert('לא הצלחתי להפיק את ה-PDF של כלל ריסק. ודא שקבצי הטופס המקורי (clal-risk-page-1 עד clal-risk-page-8) נמצאים לצד קבצי המערכת ונסה שוב.');
    }
  }
};
