...
(() => {
  ...
  function normalizeCustomerRecord(c, idx=0){
    ...
    const agentName = safeTrim(c?.agentName) || safeTrim(c?.agent) || safeTrim(c?.createdBy) || "";
    ...
    return {
      ...
      city,
      agentName,
      agent: safeTrim(c?.agent) || agentName,
      createdBy: safeTrim(c?.createdBy) || agentName,
      agentRole: safeTrim(c?.agentRole) || safeTrim(c?.role) || "",
      createdAt,
      updatedAt,
      insuredCount,
  ...
  function normalizeProposalRecord(p, idx=0){
    ...
    const agentName = safeTrim(p?.agentName) || safeTrim(p?.agent) || safeTrim(p?.createdBy) || "";
    ...
    return {
      ...
      city,
      agentName,
      agent: safeTrim(p?.agent) || agentName,
      createdBy: safeTrim(p?.createdBy) || agentName,
      agentRole: safeTrim(p?.agentRole) || safeTrim(p?.role) || "",
      createdAt,
      updatedAt,
      currentStep,
      insuredCount,
  ...
  const Storage = {
    ...
    async saveSheets(state){
      ...
      const body = JSON.stringify({ payload: state, user: (this.session().name || ""), role: (this.session().role || "") });
      const tryRequest = async (contentType) => {
        ...
        let json = null;
        try { json = raw ? JSON.parse(raw) : null; } catch(_e) {}
        if(!res.ok) return { ok:false, error:(json?.error || raw || ("HTTP " + res.status)) };
        if(json && json.ok === true) return { ok:true, at: json.at || nowISO() };
        return { ok:false, error: json?.error || raw || "put failed" };
      };
      try {
        let out = await tryRequest("text/plain;charset=utf-8");
        if(!out.ok) out = await tryRequest("application/json;charset=utf-8");
        return out;
      } catch(e) {
        return { ok:false, error: String(e?.message || e) };
      }
...
txt.textContent = "";
      ...
      UI.els.customersTbody.innerHTML = rows.length ? rows.map(rec => {
        ...
        return `<tr>
          ...
          <td><div class="lcCustomers__rowActions">
            ...
            ${Auth.isAdmin() ? `<button class="btn btn--danger lcCustomers__archiveBtn" data-archive-customer="${escapeHtml(rec.id)}" type="button">מחק לקוח</button>` : ""}
          </div></td>
        </tr>`;
  ...
  const ArchiveCustomerUI = {
    ...
    open(id){
      if(!Auth.isAdmin()){
        window.alert("רק מנהל מערכת יכול למחוק לקוח");
        return;
      }
    ...
    async confirm(){
      ...
      const customerIdNumber = safeTrim(rec.idNumber);
      const customerPhone = safeTrim(rec.phone);
      const next = prevCustomers.filter(x => String(x.id) !== String(id));
      State.data.customers = next;
      State.data.proposals = prevProposals.filter(x => {
        const sameId = customerIdNumber && safeTrim(x?.idNumber) === customerIdNumber;
        const samePhone = customerPhone && safeTrim(x?.phone) === customerPhone;
        return !(sameId || samePhone);
      });
      State.data.meta = State.data.meta || {};
      State.data.meta.updatedAt = nowISO();

      const r = await App.persist("הלקוח נמחק לחלוטין");
      if(!r?.ok){
        State.data.customers = prevCustomers;
        State.data.proposals = prevProposals;
        State.data.meta.updatedAt = nowISO();
        this.showError("מחיקת הלקוח מ-Google Sheets נכשלה. שום דבר לא נמחק. בדוק חיבור ונסה שוב.");
        CustomersUI.render();
        ProposalsUI.render();
        return;
      }
...
// ---------- Step 6 ----------
    ...
    getPhoenixHealthSchema(){
      ...
      (this.newPolicies || []).forEach(policy => {
        if(safeTrim(policy?.company) !== 'הפניקס') return;
        const type = safeTrim(policy?.type);
        let schemaKey = '';
        if(type === 'בריאות') schemaKey = 'full_health';
        else if(type === 'מחלות קשות' || type === 'סרטן') schemaKey = 'critical_illness';
    ...
    async saveDraft(){
      ...
      const record = normalizeProposalRecord({
        ...
        agent: safeTrim(Auth?.current?.name),
      ...
      if(persistRes?.ok){
        await App.syncNow();
        this.setHint("ההצעה נשמרה ב-Google Sheets ותופיע במסך הצעות להמשך עריכה");
      }else{
        this.setHint("שמירת ההצעה ל-Google Sheets נכשלה. בדוק חיבור והרשאות ונסה שוב.");
      }
    },
    ...
    async saveCompletedCustomer(){
      ...
      if(!persistRes?.ok){
        throw new Error(persistRes?.error || "CUSTOMER_SAVE_FAILED");
      }
      await App.syncNow();
      return record;
    },
