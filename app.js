...
txt.textContent = "";
  ...
  const UsersUI = {
    ...
    async _saveFromModal(){
      ...
      this.closeModal();
      this.render();
      this.showPolicyToast(wasEditing ? "השינויים בפוליסה נשמרו" : "הפוליסה נוספה בהצלחה");
    },
...
init(){
    ...
    showPolicyToast(msg){
      try {
        if(!msg) return;
        let el = document.getElementById("lcPolicyToast");
        if(!el){
          el = document.createElement("div");
          el.id = "lcPolicyToast";
          el.className = "lcPolicyToast";
        ...
        clearTimeout(this._policyToastTimer);
        this._policyToastTimer = setTimeout(() => {
          try { el.classList.remove("is-visible"); } catch(_e){}
        }, 2200);
      } catch(_e) {}
    },
...
// open health covers drawer (Health only)
    ...
    addDraftPolicy(){
      this.ensurePolicyDraft();
      const d = this.policyDraft;
      const wasEditing = !!this.editingPolicyId;
    ...
    validateStep5(){
      ...
      const selectedCompanySrc = d.company ? this.getCompanyLogoSrc(d.company) : "";
      const selectedCompanyLogo = d.company
        ? (selectedCompanySrc
            ? `<img class="lcPolBuilderCompanyMark__logo" src="${escapeHtml(selectedCompanySrc)}" alt="${escapeHtml(d.company)}" />`
            : `<span class="lcPolBuilderCompanyMark__fallback">${escapeHtml((d.company || "").slice(0,2) || "•")}</span>`)
        : `<span class="lcPolBuilderCompanyMark__fallback">•</span>`;
    ...
    renderStep5(){
      ...
      const companyCards = this.companies.map(c => {
        ...
        return `<button type="button" class="${cls}" data-co="${escapeHtml(c)}" aria-pressed="${selected ? "true" : "false"}"><span class="lcCoCheck" aria-hidden="true">✓</span>${logo}<div class="lcCoName">${escapeHtml(c)}</div></button>`;
      }).join("");
      ...
      const form = `<div class="lcWSection lcPolBuilderSection">
        ...
        <div class="lcPolForm lcPolForm--premium">
          <div class="lcPolBuilderCard lcPolBuilderCard--assign">
          ...
          <div class="lcPolStageCard lcPolStageCard--companies">
            <div class="lcPolStageCard__head">
              <div>
                <div class="lcPolStageCard__title">בחר חברת ביטוח</div>
                <div class="lcPolStageCard__sub">לחץ על לוגו החברה כדי להמשיך להזנת פרטי הפוליסה.</div>
              </div>
            </div>
          ...
          <div class="lcPolStageCard lcPolStageCard--details ${d.company ? "is-ready" : "is-idle"}">
            <div class="lcPolStageCard__head lcPolStageCard__head--details">
              <div class="lcPolBuilderCompanyMark" aria-hidden="true">${selectedCompanyLogo}</div>
              <div>
                <div class="lcPolStageCard__title">${this.editingPolicyId ? "עריכת פרטי הפוליסה" : "פרטי הפוליסה"}${d.company ? ` – ${escapeHtml(d.company)}` : ""}</div>
                <div class="lcPolStageCard__sub">${d.company ? "המשך למילוי פרטי המוצר, תחילת הביטוח והפרמיה." : "בחר חברה למעלה כדי לפתוח את אזור פרטי הפוליסה."}</div>
              </div>
            </div>
            ...
            <div class="lcPolGrid lcPolGrid--top lcPolGrid--mainRow">
              ...
              <div class="lcField lcPolField lcPolField--date">
                ...
                <div class="lcPolDateWrap lcPolControlShell">
                  <input class="lcInput lcPolDateInput" type="date" data-pdraft="startDate" value="${escapeHtml(d.startDate || "")}" ${!d.company?"disabled":""} />
                </div>
              </div>
              ...
              <div class="lcField lcPolField lcPolField--premiumMain">
                ...
                <div class="lcPolMoneyWrap lcPolControlShell">
                  ...
                  <input class="lcInput lcPolMoneyWrap__input" type="text" inputmode="numeric" data-pdraft="premiumMonthly" value="${escapeHtml(d.premiumMonthly || "")}" placeholder="לדוגמה: 250" ${!d.company?"disabled":""} />
                </div>
              </div>
            </div>
            ...
            <div class="lcPolBuilderActions">
              ${this.editingPolicyId ? `<button type="button" class="lcBtn" data-cancel-editpol="1">ביטול עריכה</button>` : ``}<button type="button" class="lcBtn lcBtn--primary lcBtn--polAdd" data-addpol="1">${this.editingPolicyId ? "שמור שינויים" : "הוסף פוליסה"}</button>
            </div>
          </div>
        </div>
      </div>`;
