...
(() => {
  ...
  const USER_THEME_STORAGE_KEY = "GI_USER_THEME_V1";
  ...
  const Auth = {
    ...
    logout(reason = "manual"){
      ...
      UI.applyRoleUI();
      UserSettingsUI.applyTheme("default");
      UI.goView("dashboard");
    },
...
this.els.syncDot = $("#syncDot");
      ...
      this.els.btnUserSettingsOpen = $("#btnUserSettingsOpen");
      this.els.userSettingsRoot = $("#userSettingsRoot");
      ...
      on(this.els.btnUserSettingsOpen, "click", () => { if(Auth.current && !Auth.isAdmin()) this.goView("userSettings"); });
// nav
...
this.applyRoleUI();
    ...
    applyRoleUI(){
      ...
      const userSettingsBtn = this.els.btnUserSettingsOpen;
      const newCustomerBtn = document.getElementById("btnNewCustomerWizard");
      if (settingsBtn) settingsBtn.style.display = isAdmin ? "" : "none";
      if (userSettingsBtn) userSettingsBtn.style.display = (Auth.current && !isAdmin) ? "" : "none";
    ...
    goView(view){
      ...
      if(safe === "userSettings" && (!Auth.current || Auth.isAdmin())) safe = "dashboard";
      ...
      document.body.classList.remove("view-users-active","view-dashboard-active","view-settings-active","view-userSettings-active","view-discountSpec-active","view-customers-active","view-proposals-active","view-myProcesses-active","view-mirrors-active");
      document.body.classList.add("view-" + safe + "-active");

      // render view data
      if (safe === "users") UsersUI.render();
      if (safe === "userSettings") UserSettingsUI.render();
...
ההכנסה מעבודה שאתה צפוי להרוויח תעמוד על סך של ______ ₪ לחודש. ידוע לך כי בקרות מקרה הביטוח יוגבלו תגמולי הפיצוי החודשי להם תהיה זכאי לגובה שלא יעלה על 75% מהכנסתך מעבודה בפועל ב־12 החודשים שקדמו למועד קרות מקרה הביטוח, או בתקופה ממועד תחילת הביטוח ועד מועד קרות מקרה הביטוח, אם חלפו פחות מ־12 חודשים.`
  ...
  const App = {
    ...
    async reloadSessionState(){
      ...
      if (r.ok) {
        ...
        if (Auth.current) {
          UI.renderAuthPill();
          UserSettingsUI.applyCurrentTheme();
          CustomersUI.render();
          ProposalsUI.render();
  ...
  const normalizeUserProfileEntry = (raw) => {
    const input = raw && typeof raw === 'object' ? raw : {};
    const theme = safeTrim(input.theme).toLowerCase();
    return {
      email: safeTrim(input.email || input.mail),
      avatar: safeTrim(input.avatar || input.profileImage || input.profile_image || input.image),
      theme: ['dark-blue','dark','default'].includes(theme) ? theme : 'default',
      lastLoginAt: safeTrim(input.lastLoginAt || input.last_login_at),
      updatedAt: safeTrim(input.updatedAt || input.updated_at) || nowISO()
    };
  };
  const normalizeUserProfileMap = (raw) => {
    const input = raw && typeof raw === 'object' ? raw : {};
    const out = {};
    Object.entries(input).forEach(([key, value]) => { if(key) out[String(key)] = normalizeUserProfileEntry(value); });
    return out;
  };
  const getUserProfileStore = () => {
    State.data.meta = State.data.meta && typeof State.data.meta === 'object' ? State.data.meta : {};
    State.data.meta.userProfiles = normalizeUserProfileMap(State.data.meta.userProfiles);
    return State.data.meta.userProfiles;
  };
  const getUserProfileKey = (agent) => safeTrim(agent?.id) || safeTrim(agent?.username) || safeTrim(agent?.name);
  const getUserProfile = (agent) => {
    const key = getUserProfileKey(agent);
    if(!key) return normalizeUserProfileEntry({});
    return normalizeUserProfileEntry(getUserProfileStore()[key] || {});
  };
  const setUserProfile = (agent, patch = {}) => {
    const key = getUserProfileKey(agent);
    if(!key) return normalizeUserProfileEntry({});
    const store = getUserProfileStore();
    const merged = normalizeUserProfileEntry({ ...(store[key] || {}), ...(patch || {}), updatedAt: nowISO() });
    store[key] = merged;
    State.data.meta.updatedAt = nowISO();
    return merged;
  };

  const UserSettingsUI = {
    els: {},
    activeTheme: 'default',
    init(){},
    getCurrentAgent(){
    ...
    getRoleLabel(role){
      const key = safeTrim(role || Auth.current?.role || 'agent');
    ...
    getInitials(name){
      ...
      return (parts.map((part) => part.charAt(0)).join('') || 'GI').slice(0,2).toUpperCase();
    },
    getCurrentProfile(){
      ...
      return {
        agent,
        email: safeTrim(agent?.email) || safeTrim(profile.email),
        avatar: safeTrim(profile.avatar),
        theme: safeTrim(profile.theme) || 'default',
        lastLoginAt: safeTrim(profile.lastLoginAt)
      };
    },
    ...
    formatRelativeTime(value){
      const ms = Date.parse(value || '');
      if(!Number.isFinite(ms)) return 'טרם תועדה כניסה';
      const diff = Date.now() - ms;
      const minute = 60 * 1000;
      const hour = 60 * minute;
      const day = 24 * hour;
      if(diff < minute) return 'כרגע';
      if(diff < hour) return `לפני ${Math.max(1, Math.floor(diff / minute))} דקות`;
      if(diff < day) return `לפני ${Math.max(1, Math.floor(diff / hour))} שעות`;
      return `לפני ${Math.max(1, Math.floor(diff / day))} ימים`;
    },
    setMessage(el, msg, tone='ok'){
      if(!el) return;
      el.textContent = msg ? String(msg) : '';
      el.dataset.tone = tone === 'err' ? 'err' : 'ok';
    },
    compressAvatarFile(file){
      return new Promise((resolve, reject) => {
        const reader = new FileReader();
    ...
    renderAvatarMarkup(agent, profile){
      const avatar = safeTrim(profile?.avatar);
      return avatar ? `<img alt="${escapeHtml(agent?.name || 'משתמש')}" src="${avatar}" />` : `<span>${escapeHtml(this.getInitials(agent?.name || 'GI'))}</span>`;
    },
    render(){
      const root = UI.els?.userSettingsRoot;
      if(!root) return;
      const { agent, email, avatar, theme, lastLoginAt } = this.getCurrentProfile();
      if(!agent){
        root.innerHTML = `<div class="card"><div class="emptyState"><div class="emptyState__icon">⚙️</div><div class="emptyState__title">הגדרות המשתמש אינן זמינות כרגע</div><div class="emptyState__text">התחבר כמשתמש פעיל כדי לפתוח את הגדרות המשתמש שלך.</div></div></div>`;
        return;
      }
      const security = typeof getAgentSecurity === 'function' ? getAgentSecurity(agent.id) : null;
      const mfaActive = !!(security && (security.mfaEnabled || security.mfaRequired || security.factorId));
      root.innerHTML = `
        <div class="userSettingsLayout">
          <div class="card userSettingsHeroCard">
            <div class="userSettingsHero">
              <div class="userSettingsHero__avatarWrap">
                <div class="userSettingsHero__avatar" id="userSettingsAvatarPreview">${this.renderAvatarMarkup(agent, { avatar })}</div>
                <input id="userSettingsAvatarInput" type="file" accept="image/*" hidden />
                <button class="btn userSettingsHero__uploadBtn" id="userSettingsAvatarBtn" type="button">העלה תמונת פרופיל</button>
              </div>
              <div>
                <div class="userSettingsHero__eyebrow">הגדרות משתמש</div>
                <div class="userSettingsHero__name">${escapeHtml(agent.name || 'משתמש')}</div>
                <div class="userSettingsHero__meta">${escapeHtml(this.getRoleLabel(agent.role))} · ${escapeHtml(safeTrim(email) || 'ללא אימייל')}</div>
                <div class="userSettingsHero__hint">כאן ניתן לעדכן את הפרופיל האישי שלך, לשנות קוד כניסה ולעבור בין מצבי התצוגה של המערכת.</div>
              </div>
            </div>
          </div>

          <div class="userSettingsGrid">
            <div class="card userSettingsCard">
              <div class="card__head"><div class="card__title">פרטי משתמש</div><div class="card__hint">המידע האישי שלך במערכת</div></div>
              <div class="formGrid userSettingsFormGrid">
                <label class="field"><span class="label">שם מלא</span><input class="input" id="userSettingsName" type="text" value="${escapeHtml(agent.name || '')}" /></label>
                <label class="field"><span class="label">אימייל</span><input class="input" id="userSettingsEmail" type="email" dir="ltr" value="${escapeHtml(email || '')}" /></label>
                <label class="field"><span class="label">תאריך לידה</span><input class="input" id="userSettingsBirthDate" type="text" placeholder="dd/mm/yyyy" value="${escapeHtml(agent.birthDate || '')}" /></label>
                <label class="field"><span class="label">תפקיד</span><input class="input" type="text" value="${escapeHtml(this.getRoleLabel(agent.role))}" readonly /></label>
              </div>
              <div class="userSettingsActions">
                <div class="help" id="userSettingsProfileMsg"></div>
                <button class="btn btn--primary" id="userSettingsSaveProfile" type="button">שמור פרטי משתמש</button>
              </div>
            </div>

            <div class="card userSettingsCard">
              <div class="card__head"><div class="card__title">אבטחה אישית</div><div class="card__hint">שינוי קוד כניסה ותצוגת 2FA</div></div>
              <div class="formGrid userSettingsFormGrid">
                <label class="field"><span class="label">קוד כניסה חדש</span><input class="input" id="userSettingsPin" type="password" inputmode="numeric" maxlength="8" placeholder="הזן קוד חדש" /></label>
                <label class="field"><span class="label">אימות קוד כניסה</span><input class="input" id="userSettingsPinConfirm" type="password" inputmode="numeric" maxlength="8" placeholder="הזן שוב" /></label>
              </div>
              <div class="userSettingsActions">
                <div class="help" id="userSettingsPinMsg"></div>
                <button class="btn btn--primary" id="userSettingsSavePin" type="button">עדכן קוד כניסה</button>
              </div>
              <div class="divider"></div>
              <div class="userSettingsInfoList">
                <div class="userSettingsInfoRow"><span>אימות דו שלבי</span><strong>${mfaActive ? 'פעיל 🔒' : 'מנוהל על ידי מנהל 🔒'}</strong></div>
                <div class="userSettingsInfoRow"><span>גישה להגדרות MFA</span><strong>לקריאה בלבד</strong></div>
              </div>
            </div>

            <div class="card userSettingsCard">
              <div class="card__head"><div class="card__title">מצב תצוגה</div><div class="card__hint">התאם את מראה המערכת שלך</div></div>
              <div class="userThemePicker" id="userThemePicker">
                <button class="userThemeChip${theme === 'default' ? ' is-active' : ''}" data-theme-choice="default" type="button"><span class="userThemeChip__dot userThemeChip__dot--default"></span><span>רגיל</span></button>
                <button class="userThemeChip${theme === 'dark-blue' ? ' is-active' : ''}" data-theme-choice="dark-blue" type="button"><span class="userThemeChip__dot userThemeChip__dot--darkBlue"></span><span>שחור כחול זכוכית</span></button>
                <button class="userThemeChip${theme === 'dark' ? ' is-active' : ''}" data-theme-choice="dark" type="button"><span class="userThemeChip__dot userThemeChip__dot--dark"></span><span>שחור זכוכית</span></button>
              </div>
              <div class="userSettingsActions">
                <div class="help" id="userSettingsThemeMsg">העיצוב נשמר לך אוטומטית.</div>
                <div class="userSettingsLockText">אפשר לעבור בין המצבים בלי לרענן את המערכת.</div>
              </div>
            </div>

            <div class="card userSettingsCard">
              <div class="card__head"><div class="card__title">פעילות אחרונה</div><div class="card__hint">מידע אישי מהסשן שלך</div></div>
              <div class="userSettingsInfoList">
                <div class="userSettingsInfoRow"><span>כניסה אחרונה</span><strong>${escapeHtml(this.formatRelativeTime(lastLoginAt))}</strong></div>
                <div class="userSettingsInfoRow"><span>תאריך מלא</span><strong>${escapeHtml(this.formatDateTime(lastLoginAt))}</strong></div>
                <div class="userSettingsInfoRow"><span>שם משתמש להתחברות</span><strong>${escapeHtml(agent.username || '—')}</strong></div>
              </div>
              <div class="divider"></div>
              <div class="userSettingsActions">
                <div class="userSettingsLockText">התצוגה הזאת אישית למשתמש המחובר בלבד.</div>
                <button class="btn btn--danger" id="userSettingsLogout" type="button">התנתק מהמערכת</button>
              </div>
            </div>
          </div>
        </div>`;
    ...
    bind(){
      const root = UI.els?.userSettingsRoot;
      if(!root) return;
      on($('#userSettingsSaveProfile', root), 'click', () => this.saveProfile());
      on($('#userSettingsSavePin', root), 'click', () => this.savePin());
      on($('#userSettingsLogout', root), 'click', () => Auth.logout());
      on($('#userSettingsAvatarBtn', root), 'click', () => $('#userSettingsAvatarInput', root)?.click?.());
      on($('#userSettingsAvatarInput', root), 'change', (ev) => this.handleAvatarChange(ev));
      $$('#userThemePicker [data-theme-choice]', root).forEach((btn) => on(btn, 'click', () => this.saveTheme(btn.getAttribute('data-theme-choice'))));
    },
    async saveProfile(){
      const { agent } = this.getCurrentProfile();
      if(!agent) return;
      const msgEl = $('#userSettingsProfileMsg', UI.els.userSettingsRoot);
      const nextName = safeTrim($('#userSettingsName', UI.els.userSettingsRoot)?.value);
      const nextEmail = safeTrim($('#userSettingsEmail', UI.els.userSettingsRoot)?.value);
      const nextBirthDate = safeTrim($('#userSettingsBirthDate', UI.els.userSettingsRoot)?.value);
      if(!nextName) return this.setMessage(msgEl, 'יש למלא שם מלא.', 'err');
      if(!nextBirthDate) return this.setMessage(msgEl, 'יש למלא תאריך לידה.', 'err');
      agent.name = nextName;
      agent.birthDate = nextBirthDate;
      agent.email = nextEmail;
      setUserProfile(agent, { email: nextEmail });
      if(Auth.current) Auth.current.name = nextName;
      this.setMessage(msgEl, 'שומר פרטי משתמש…', 'ok');
      const res = await App.persist('פרטי המשתמש נשמרו');
      if(!res?.ok) return this.setMessage(msgEl, 'השמירה נכשלה. נסה שוב.', 'err');
      UI.renderAuthPill();
      this.render();
      this.setMessage($('#userSettingsProfileMsg', UI.els.userSettingsRoot), 'פרטי המשתמש נשמרו בהצלחה.', 'ok');
    },
    async savePin(){
      const { agent } = this.getCurrentProfile();
      if(!agent) return;
      const msgEl = $('#userSettingsPinMsg', UI.els.userSettingsRoot);
      const pin = safeTrim($('#userSettingsPin', UI.els.userSettingsRoot)?.value);
      const confirm = safeTrim($('#userSettingsPinConfirm', UI.els.userSettingsRoot)?.value);
      if(!pin || pin.length < 4) return this.setMessage(msgEl, 'יש להזין קוד כניסה של לפחות 4 ספרות/תווים.', 'err');
      if(pin !== confirm) return this.setMessage(msgEl, 'אימות קוד הכניסה אינו תואם.', 'err');
      agent.pin = pin;
      this.setMessage(msgEl, 'שומר קוד כניסה…', 'ok');
      const res = await App.persist('קוד הכניסה עודכן');
      if(!res?.ok) return this.setMessage(msgEl, 'עדכון קוד הכניסה נכשל.', 'err');
      const pinEl = $('#userSettingsPin', UI.els.userSettingsRoot); if(pinEl) pinEl.value = '';
      const pinConfirmEl = $('#userSettingsPinConfirm', UI.els.userSettingsRoot); if(pinConfirmEl) pinConfirmEl.value = '';
      this.setMessage(msgEl, 'קוד הכניסה עודכן בהצלחה.', 'ok');
    },
    async handleAvatarChange(ev){
      const input = ev?.target;
      const file = input?.files?.[0];
      const msgEl = $('#userSettingsProfileMsg', UI.els.userSettingsRoot);
      if(!file) return;
      if(!/^image\//i.test(file.type || '')) return this.setMessage(msgEl, 'יש לבחור קובץ תמונה בלבד.', 'err');
      try {
        this.setMessage(msgEl, 'מעבד תמונת פרופיל…', 'ok');
        const dataUrl = await this.compressAvatarFile(file);
        const { agent } = this.getCurrentProfile();
        setUserProfile(agent, { avatar: dataUrl });
        const res = await App.persist('תמונת הפרופיל נשמרה');
        if(!res?.ok) throw new Error(res?.error || 'SAVE_AVATAR_FAILED');
        this.decorateUserPill();
        this.render();
        this.setMessage($('#userSettingsProfileMsg', UI.els.userSettingsRoot), 'תמונת הפרופיל נשמרה בהצלחה.', 'ok');
      } catch(err){
        console.error('USER_AVATAR_SAVE_FAILED', err);
        this.setMessage(msgEl, 'לא הצלחתי לשמור את התמונה. נסה קובץ אחר.', 'err');
      } finally {
    ...
    async saveTheme(theme){
      const { agent } = this.getCurrentProfile();
      if(!agent) return;
      const nextTheme = ['dark-blue','dark','default'].includes(safeTrim(theme)) ? safeTrim(theme) : 'default';
      this.applyTheme(nextTheme);
      setUserProfile(agent, { theme: nextTheme });
      const res = await App.persist('ערכת הנושא נשמרה');
      const msgEl = $('#userSettingsThemeMsg', UI.els.userSettingsRoot);
      if(!res?.ok) return this.setMessage(msgEl, 'שמירת מצב התצוגה נכשלה.', 'err');
      this.render();
      this.setMessage($('#userSettingsThemeMsg', UI.els.userSettingsRoot), 'מצב התצוגה נשמר בהצלחה.', 'ok');
    },
    applyTheme(theme='default'){
      const clean = ['dark-blue','dark','default'].includes(safeTrim(theme)) ? safeTrim(theme) : 'default';
      this.activeTheme = clean;
      document.body.classList.remove('theme-dark-blue','theme-dark');
      if(clean === 'dark-blue') document.body.classList.add('theme-dark-blue');
      if(clean === 'dark') document.body.classList.add('theme-dark');
    },
    applyCurrentTheme(){
      const { theme } = this.getCurrentProfile();
      this.applyTheme(theme || 'default');
    },
    async trackLogin(){
      const { agent } = this.getCurrentProfile();
      if(!agent) return;
      setUserProfile(agent, { lastLoginAt: nowISO(), theme: getUserProfile(agent).theme || 'default', email: safeTrim(agent?.email) || getUserProfile(agent).email });
      this.applyCurrentTheme();
      this.decorateUserPill();
      await App.persist('עודכן לוגין משתמש');
    },
    decorateUserPill(){
      const pill = document.getElementById('lcUserPill');
      if(!pill || !Auth.current) return;
      this.clearUserPillAvatar();
      const { agent, avatar } = this.getCurrentProfile();
      if(!avatar) return;
      const avatarEl = document.createElement('span');
      avatarEl.className = 'lcUserPill__avatar';
      avatarEl.setAttribute('aria-hidden', 'true');
      avatarEl.innerHTML = `<img alt="" src="${avatar}" />`;
      const anchor = document.getElementById('lcUserPillText') || pill.firstChild;
      pill.insertBefore(avatarEl, anchor);
    },
    clearUserPillAvatar(){
      document.querySelectorAll('#lcUserPill .lcUserPill__avatar').forEach((el) => el.remove());
    }
  };
  ...
  Storage.buildMetaRow = function(state){
    ...
    row.payload.userProfiles = normalizeUserProfileMap(state?.meta?.userProfiles);
    return row;
  };
  ...
  Storage.mapMeta = function(metaRow){
    ...
    out.userProfiles = normalizeUserProfileMap(metaRow?.payload?.userProfiles || out.userProfiles);
    return out;
  };
  ...
  const completeAgentLogin = async (matched) => {
    ...
    BirthdaysUI.maybeCelebrateLogin();
    try { await UserSettingsUI.trackLogin(); } catch(_e) { UserSettingsUI.applyCurrentTheme(); }
    UI.goView('dashboard');
    try { ChatUI.onLogin(); } catch(_e) {}
    try { SupportNoticeUI.showAfterLogin(); } catch(_e) {}
  };
  ...
  Auth._submit = async function(){
    ...
    if (adminAuth.active !== false && username === safeTrim(adminAuth.username) && pin === safeTrim(adminAuth.pin)) {
      this.current = { name: safeTrim(adminAuth.username) || defAdmin.username, role:'admin' };
      await App.reloadSessionState(); this.unlock(); InactivityGuard.start(); UI.applyRoleUI(); UI.renderAuthPill(); await WelcomeLoader.play(this.current.name, 1200); UserSettingsUI.applyTheme('default', null); UI.goView('settings'); return;
    }
    const agents = Array.isArray(State.data?.agents) ? State.data.agents : [];
    const matched = agents.find(a => safeTrim(a?.username) === username) || agents.find(a => safeTrim(a?.name) === username);
    if(!matched) return this._setError('שם משתמש לא נמצא');
