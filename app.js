...
(() => {
  ...
  function openHarBituachProcessingOverlay(options = {}){
    const title = safeTrim(options.title) || 'מוריד נתוני הר הביטוח למערכת';
    const subtitle = safeTrim(options.subtitle) || 'מכין את הקובץ לקריאה';
    const existing = document.getElementById('lcHarProcessingModal');
    if(existing) existing.remove();
    const root = document.createElement('div');
    root.id = 'lcHarProcessingModal';
    root.className = 'lcHarProcessingModal is-open is-loading';
    root.setAttribute('dir', 'rtl');
    root.innerHTML = `
    ...
    window.requestAnimationFrame(() => root.classList.add('is-open'));
    const setStage = (mode, nextTitle, nextSubtitle) => {
      root.classList.remove('is-loading', 'is-success', 'is-error');
      if(mode) root.classList.add(mode);
      if(titleEl && nextTitle != null) titleEl.textContent = String(nextTitle);
      if(subtitleEl && nextSubtitle != null) subtitleEl.textContent = String(nextSubtitle);
      if(loaderEl) loaderEl.style.display = mode === 'is-loading' ? '' : 'none';
    };
    return {
      element: root,
      setLoading(nextTitle, nextSubtitle){ setStage('is-loading', nextTitle, nextSubtitle); },
      setSuccess(nextTitle, nextSubtitle){ setStage('is-success', nextTitle, nextSubtitle); },
      setError(nextTitle, nextSubtitle){ setStage('is-error', nextTitle, nextSubtitle); },
      async close(delayMs = 220){
        await new Promise((resolve) => window.setTimeout(resolve, Math.max(0, Number(delayMs) || 0)));
        root.classList.remove('is-open', 'is-loading', 'is-success', 'is-error');
        root.classList.add('is-closing');
  ...
  async function ensureXlsxReady(){
    ...
    throw new Error('ספריית הקריאה לקובצי Excel לא נטענה במחשב הזה. נסה רענון קשיח או בדוק חסימת רשת/אבטחה.');
  }

  function normalizeHarHeaderCell(value){
  ...
  function findHarHeaderMeta(rows){
    if(!Array.isArray(rows)) return null;
    for(let i = 0; i < rows.length; i++){
      const row = Array.isArray(rows[i]) ? rows[i].map((cell) => normalizeHarHeaderCell(cell)) : [];
      if(!row.length) continue;
      const hasId = row.some((cell) => /תעודת זהות/.test(cell));
      const hasPolicy = row.some((cell) => /מספר פוליסה/.test(cell));
      if(hasId && hasPolicy){
        return { index:i, header:row };
      }
    }
    return null;
  }

  function resolveHarColumnIndex(header, aliases){
    const list = Array.isArray(header) ? header : [];
    for(const alias of aliases){
      const normalized = normalizeHarHeaderCell(alias);
      const idx = list.findIndex((cell) => cell === normalized || cell.includes(normalized));
      if(idx !== -1) return idx;
    }
    return -1;
  }
...
// open health covers drawer (Health only)
    ...
    getElementaryVehicleWorkbookCandidates(wb){
      const candidates = [];
      ...
      names.forEach((sheetName) => {
        ...
        const meta = findHarHeaderMeta(rows);
        if(!meta) return;
        candidates.push({ sheetName, rows, headerIdx:meta.index, header:meta.header });
      });
      return candidates;
    },

    buildHarColumnIndex(header){
      return {
        idNumber: resolveHarColumnIndex(header, ['תעודת זהות']),
        main: resolveHarColumnIndex(header, ['ענף ראשי']),
        sub: resolveHarColumnIndex(header, ['ענף משני', 'ענף (משני)']),
        productType: resolveHarColumnIndex(header, ['סוג מוצר']),
        company: resolveHarColumnIndex(header, ['חברה']),
        period: resolveHarColumnIndex(header, ['תקופת ביטוח']),
        extra: resolveHarColumnIndex(header, ['פרטים נוספים']),
        premium: resolveHarColumnIndex(header, ['פרמיה בשח', 'פרמיה בש"ח', 'פרמיה']),
        premiumType: resolveHarColumnIndex(header, ['סוג פרמיה']),
        policyNumber: resolveHarColumnIndex(header, ['מספר פוליסה'])
      };
    },

    isElementaryVehicleHarRow(ctx = {}){
      const currentSection = safeTrim(ctx.currentSection);
      const mainCell = safeTrim(ctx.mainCell);
      const sub = safeTrim(ctx.sub);
      const productType = safeTrim(ctx.productType);
      const extra = safeTrim(ctx.extra);
      const text = [currentSection, mainCell, sub, productType, extra].filter(Boolean).join(' | ');
      const isHome = /דירה|מבנה|תכולה|עסק|משכנתא/.test(text);
      const hasVehicleWord = /רכב|מקיף|צד ג|חובה|דו גלגלי|משאית|אופנוע|קטנוע/.test(text);
      if(isHome) return false;
      if(/תחום\s*-\s*כללי/.test(currentSection) && hasVehicleWord) return true;
      if(/תחום\s*-\s*רכב/.test(currentSection)) return true;
      return hasVehicleWord;
    },

    parseElementaryVehicleHarWorkbook(buffer, ins){
      if(!window.XLSX) throw new Error('ספריית הקריאה לקובצי Excel לא נטענה.');
      const wb = window.XLSX.read(buffer, { type:'array' });
      const candidates = this.getElementaryVehicleWorkbookCandidates(wb);
      if(!candidates.length) throw new Error('מבנה קובץ הר הביטוח לא זוהה.');
      const extracted = [];
      candidates.forEach((candidate) => {
        const idx = this.buildHarColumnIndex(candidate.header);
        let currentSection = '';
        candidate.rows.slice(candidate.headerIdx + 1).forEach((row) => {
          ...
          if(!this.isElementaryVehicleHarRow({ currentSection, mainCell, sub, productType, extra })) return;
          const policyNumber = safeTrim(row[idx.policyNumber]);
          if(!policyNumber) return;
          const company = this.normalizeHarCompanyName(row[idx.company]) || safeTrim(row[idx.company]);
      ...
      extracted.forEach((item) => {
        const key = [safeTrim(item.policyNumber), safeTrim(item.company), safeTrim(item.productType), safeTrim(item.sub)].join('|');
        if(seen.has(key)) return;
        seen.add(key);
        deduped.push(item);
      });
      return { policies: deduped, fileName:'' };
    },
    ...
    async handleElementaryVehicleHarFile(ins, file){
      if(!file || !ins) return;
      const overlay = openHarBituachProcessingOverlay({
        title:'טוען את קובץ רכב מהר הביטוח',
        subtitle:'מכין את סביבת הקריאה במחשב הזה…'
      });
      try{
        this.setElementaryHarImportState(ins, { status:'loading', fileName:file.name || '', message:'טוען את קובץ הר הביטוח ומכין את סביבת הקריאה…' });
        this.render();
        await ensureXlsxReady();
        overlay.setLoading('טוען את קובץ רכב מהר הביטוח', 'קורא את הקובץ בזיכרון בלבד…');
        const buffer = await file.arrayBuffer();
        overlay.setLoading('מנתח את קובץ רכב מהר הביטוח', 'מאתר לשוניות, כותרות ופוליסות רכב רלוונטיות…');
        ...
        if(!policies.length){
          overlay.setError('לא נמצאו פוליסות רכב', 'הקובץ נקרא בהצלחה, אבל לא זוהו שורות רכב לייבוא.');
          this.setElementaryHarImportState(ins, { status:'empty', count:0, fileName:file.name || '', message:'הקובץ נקרא, אך לא נמצאו בו פוליסות רכב מזוהות לייבוא.' });
          this.render();
          await overlay.close(900);
          return;
        }
        overlay.setSuccess('זוהו פוליסות רכב בקובץ', `נמצאו ${policies.length} רשומות רכב. ממתין לאישור הייבוא…`);
        await overlay.close(520);
        const approved = await showHarBituachImportModal({
          ...
          note: 'המערכת מייבאת רק רשומות רכב ומסננת דירה/מבנה/תכולה. הבדיקה כעת סורקת את כל לשוניות הקובץ ולא רק את הראשונה.',
          confirmText: 'אישור',
          cancelText: 'ביטול'
        });
        if(!approved){
        ...
        const finalOverlay = openHarBituachProcessingOverlay({
          title:'מוריד נתוני רכב מהר הביטוח',
          subtitle:'מוסיף את פוליסות הרכב למסך…'
        });
        this.setElementaryHarImportState(ins, { status:'loading', count:policies.length, fileName:file.name || '', message:'מוסיף את פוליסות הרכב למסך…' });
        this.render();
        await new Promise((resolve) => window.setTimeout(resolve, 420));
        ins.data.elementaryVehiclePolicies = policies;
        finalOverlay.setSuccess('פוליסות הרכב נטענו בהצלחה', `נמצאו ${policies.length} רשומות רכב להצגה`);
        this.setElementaryHarImportState(ins, { status:'done', count:policies.length, fileName:file.name || '', importedAt:nowISO(), message:`יובאו ${policies.length} פוליסות רכב. המנוע סרק את כל לשוניות הקובץ והפעיל hardening למחשב העבודה.` });
        this.render();
        await finalOverlay.close(920);
      }catch(err){
        overlay.setError('ייבוא הר הביטוח נכשל', safeTrim(err?.message || err) || 'שגיאה לא ידועה');
    ...
    async handleHarBituachFile(ins, file){
      ...
      try {
        this.setHarImportState(ins, {
          status: "loading",
          fileName: file.name || "",
          message: "טוען את קובץ הר הביטוח ומכין את סביבת הקריאה…"
        });
        this.render();
        await ensureXlsxReady();
        overlay.setLoading('טוען את קובץ הר הביטוח', 'קורא את הקובץ בזיכרון בלבד…');
        const buffer = await file.arrayBuffer();
        overlay.setLoading('מנתח את קובץ הר הביטוח', 'מאתר לשוניות, כותרות ופוליסות רלוונטיות…');
        const parsed = this.parseHarBituachWorkbook(buffer, ins);
        const policies = Array.isArray(parsed?.policies) ? parsed.policies : [];
        if(!policies.length){
          overlay.setError('לא נמצאו פוליסות מתאימות', 'הקובץ נקרא בהצלחה, אך לא זוהו פוליסות מתוך תחומי בריאות / חיים.');
          this.setHarImportState(ins, {
            status: "empty",
            count: 0,
            fileName: file.name || "",
            message: "הקובץ נקרא, אך לא נמצאו פוליסות מתאימות לייבוא מתוך תחומי בריאות / חיים."
          });
          this.render();
          await overlay.close(900);
          return;
        ...
        overlay.setSuccess('זוהו פוליסות רלוונטיות', `נמצאו ${policies.length} פוליסות. ממתין לאישור הייבוא…`);
        await overlay.close(520);
        const approved = await showHarBituachImportModal({
          policies,
          title: `זוהו ${policies.length} פוליסות רלוונטיות לייבוא`,
          note: 'המערכת תוריד את הנתונים, תבצע עיבוד קצר ואז תציג את הפוליסות בשורות קריאה בלבד. כעת הסריקה עוברת על כל לשוניות הקובץ.',
          confirmText: 'אישור',
          cancelText: 'ביטול'
        });
        if(!approved){
        ...
        const finalOverlay = openHarBituachProcessingOverlay({
          ...
          subtitle:'מוסיף את הפוליסות למסך…'
        });
        this.setHarImportState(ins, {
          status: "loading",
          count: policies.length,
          fileName: file.name || "",
          message: "מוסיף את הפוליסות למסך…"
        });
        this.render();
        await new Promise((resolve) => window.setTimeout(resolve, 420));
        policies.forEach((policy) => this.mergeImportedExistingPolicy(ins, policy, file.name || ""));
        finalOverlay.setSuccess('בוצע בהצלחה', `המערכת הכינה ${policies.length} פוליסות להצגה ברשימה`);
        this.setHarImportState(ins, {
          status: "done",
          count: policies.length,
          fileName: file.name || "",
