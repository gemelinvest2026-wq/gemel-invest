...
(() => {
  ...
  const BUILD = "20260328-existing-policies-row-ui-v1";
...
// open health covers drawer (Health only)
    ...
    renderStep3(ins){
      ...
      const rows = (d.existingPolicies || []).map((p) => {
        ...
        const logo = logoSrc
          ? `<img class="lcPolRow__logoImg" src="${escapeHtml(logoSrc)}" alt="${escapeHtml(p.company||"")}" />`
          : `<div class="lcPolRow__logoFallback">${escapeHtml((safeTrim(p.company) || "?").slice(0,1))}</div>`;
        ...
        const includedSummary = includedList.length ? includedList.join(" • ") : "—";
        ...
        const monthlyPremium = safeTrim(p.monthlyPremium);
        const importedBadge = p.importedFromHarBituach ? `<span class="lcPolBadge lcPolBadge--imported">יובא מהר הביטוח</span>` : "";
        const openClass = p._expanded ? " is-open" : "";
        const pledgeSummary = isRisk ? (p.hasPledge ? `יש שיעבוד${safeTrim(p.pledgeBankName) ? ` · ${escapeHtml(p.pledgeBankName)}` : ""}` : "ללא שיעבוד") : "לא רלוונטי";

        const inlineDetail = includedList.length
          ? `<div class="lcPolInlineSummary" title="${escapeHtml(includedSummary)}"><b>מה נכלל:</b> ${escapeHtml(includedSummary)}</div>`
          : (periodText ? `<div class="lcPolInlineSummary"><b>תקופת ביטוח:</b> ${escapeHtml(periodText)}</div>` : `<div class="lcPolInlineSummary lcPolInlineSummary--muted">אין פירוט נוסף כרגע</div>`);

        const detailFieldHtml = includedList.length
          ? `<div class="lcPolFieldValue">${escapeHtml(includedSummary)}</div>`
          : (isHealth ? `
              <button class="btn lcSmallBtn lcCoversBtn" data-open-covers="${escapeHtml(p.id)}" type="button">${escapeHtml(coversLabel)}</button>
            ` : isRisk ? `<input class="input" data-bind="existingPolicies.${p.id}.sumInsured" value="${escapeHtml(p.sumInsured||"")}" placeholder="סכום ביטוח" />` : isCI ? `<input class="input" data-bind="existingPolicies.${p.id}.compensation" value="${escapeHtml(p.compensation||"")}" placeholder="סכום פיצוי" />` : `<div class="lcPolFieldValue lcPolFieldValue--muted">—</div>`);

        return `
          <div class="lcPolRow${openClass}" data-policy-row="${escapeHtml(p.id)}">
            <div class="lcPolRow__main">
              <div class="lcPolRow__companyCell">
                <div class="lcPolRow__logoWrap">${logo}</div>
                <select class="input" data-bind="existingPolicies.${p.id}.company" aria-label="חברת ביטוח">
                  <option value="">בחר…</option>${compOpts}
                </select>
              </div>

              <div class="lcPolRow__cell">
                <div class="lcPolRow__label">סוג</div>
              ...
              <div class="lcPolRow__cell">
                <div class="lcPolRow__label">מספר</div>
                <input class="input" data-bind="existingPolicies.${p.id}.policyNumber" value="${escapeHtml(p.policyNumber||"")}" placeholder="מספר פוליסה" />
              </div>

              <div class="lcPolRow__cell lcPolRow__cell--premium">
                <div class="lcPolRow__label">פרמיה חודשית</div>
                <div class="moneyField" title="פרמיה חודשית">
                  <input class="input moneyField__input" data-money="ils" data-bind="existingPolicies.${p.id}.monthlyPremium" value="${escapeHtml(monthlyPremium)}" placeholder="0" inputmode="decimal" />
                  <span class="moneyField__sym">₪</span>
                </div>
              </div>

              <div class="lcPolRow__actions">
                <button class="btn lcSmallBtn" data-policy-toggle="${escapeHtml(p.id)}" type="button" aria-expanded="${p._expanded ? "true" : "false"}">פירוט</button>
                <button class="btn lcSmallBtn" data-del-exist="${escapeHtml(p.id)}" type="button">הסר</button>
              </div>
            </div>

            <div class="lcPolRow__inlineMeta">
              ${importedBadge}
              ${insuredName ? `<span class="lcPolBadge"><b>מבוטח:</b> ${escapeHtml(insuredName)}</span>` : ""}
              ${insuredIdNumber ? `<span class="lcPolBadge"><b>ת"ז:</b> ${escapeHtml(insuredIdNumber)}</span>` : ""}
              ${annualPremiumTxt ? `<span class="lcPolBadge"><b>פרמיה שנתית:</b> ${escapeHtml(annualPremiumTxt)} ₪</span>` : ""}
              ${periodText ? `<span class="lcPolBadge"><b>תקופת ביטוח:</b> ${escapeHtml(periodText)}</span>` : ""}
            </div>

            <div class="lcPolRow__summaryBar">
              ${inlineDetail}
            </div>

            <div class="lcPolRow__details">
              <div class="lcPolDetailGrid">
                <div class="lcPolDetailCard">
                  <div class="lcPolDetailCard__title">מה נכלל / סכום / פיצוי</div>
                  ${detailFieldHtml}
                </div>

                <div class="lcPolDetailCard">
                  <div class="lcPolDetailCard__title">שיעבוד</div>
                  ${isRisk ? `
                    <label class="row" style="gap:8px">
                      <input type="checkbox" data-bind="existingPolicies.${p.id}.hasPledge" ${p.hasPledge ? "checked":""} />
                      <span class="small">יש שיעבוד</span>
                    </label>
                    <div class="lcPolFieldValue lcPolFieldValue--subtle">${pledgeSummary}</div>
                    ${p.hasPledge ? `
                      <select class="input" style="margin-top:8px" data-bind="existingPolicies.${p.id}.pledgeBankName">
                      ...
                      ${p.bankAgency ? `
                        <select class="input" style="margin-top:8px" data-bind="existingPolicies.${p.id}.bankAgencyName">
                          <option value="">בחר סוכנות…</option>
                          ${agencyOpts}
                        </select>
                      `:""}
                    `:""}
                  ` : `<div class="lcPolFieldValue lcPolFieldValue--muted">לא רלוונטי למוצר זה</div>`}
                </div>
              </div>
            </div>
          </div>
      ...
      return `
        <div class="lcWSection">
          ...
          <div class="lcPolRowsWrap">
            <div class="lcPolRowsHead">
              <div>חברה</div>
              <div>סוג</div>
              <div>מספר</div>
              <div>פרמיה חודשית</div>
              <div>פעולות</div>
            </div>
            <div class="lcPolRowsList">
              ${rows || `<div class="lcPolEmpty">אין פוליסות עדיין</div>`}
            </div>
          </div>
        </div>
      `;
    ...
    bindInputs(ins){
      ...
      $$("[data-del-exist]", this.els.body).forEach(btn => on(btn, "click", (ev) => {
        ev.stopPropagation();
        this.delExistingPolicy(ins, btn.getAttribute("data-del-exist"));
      }));

      $$("[data-policy-toggle]", this.els.body).forEach(btn => {
        on(btn, "click", (ev) => {
          ev.preventDefault();
          ev.stopPropagation();
          const pid = btn.getAttribute("data-policy-toggle");
          const row = (ins.data.existingPolicies || []).find(x => x && x.id === pid);
          if(!row) return;
