...
(() => {
  ...
  // ---------- Policy discount specs (reference site) ----------
...
function openUserModal_(opt){
  ...
  function renderDiscountSpec(){
    const url = (typeof POLICY_SPEC_URL === 'string' && POLICY_SPEC_URL.trim()) ? POLICY_SPEC_URL.trim() : '';
    pageTitleEl.textContent = 'מפרט הנחות';
    crumbEl.textContent = 'Reference';
    if(tabsEl){ tabsEl.innerHTML = ''; tabsEl.style.display = 'none'; }

    if(!url){
      viewEl.innerHTML = `<div class="panel">
        <div class="h2">חסר קישור למפרט הנחות</div>
        <div class="muted">אנא עדכן את POLICY_SPEC_URL בקובץ app.js</div>
      </div>`;
      return;
    }
    ...
    viewEl.innerHTML = `
      <div class="specPage">
        <div class="specActions">
          <button class="btn btnSoft" type="button" id="specBackBtn">חזרה</button>
          <div class="specActionsRight">
            <button class="btn btnSoft" type="button" id="specNewWinBtn">פתח בחלון חדש</button>
          </div>
        </div>

        <div class="specFrameWrap">
          <iframe id="specFrame" title="מפרט הנחות" loading="lazy" referrerpolicy="no-referrer" allow="clipboard-read; clipboard-write" src="${escapeHtml(url)}"></iframe>
        </div>

        <div class="muted" style="font-size:12px">
          אם הדף לא נטען בתוך המערכת – לחץ על "פתח בחלון חדש".
        </div>
      </div>
    `;

    const backBtn = document.getElementById('specBackBtn');
    const newWinBtn = document.getElementById('specNewWinBtn');

    if(backBtn) backBtn.addEventListener('click', () => setRoute('proposals'));
    if(newWinBtn) newWinBtn.addEventListener('click', () => window.open(url, '_blank', 'noopener,noreferrer'));
  }
