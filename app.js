...
(() => {
  ...
  const BUILD = "20260401-user-settings-v22";
...
ההכנסה מעבודה שאתה צפוי להרוויח תעמוד על סך של ______ ₪ לחודש. ידוע לך כי בקרות מקרה הביטוח יוגבלו תגמולי הפיצוי החודשי להם תהיה זכאי לגובה שלא יעלה על 75% מהכנסתך מעבודה בפועל ב־12 החודשים שקדמו למועד קרות מקרה הביטוח, או בתקופה ממועד תחילת הביטוח ועד מועד קרות מקרה הביטוח, אם חלפו פחות מ־12 חודשים.`
  ...
  const normalizeUserPreferenceEntry = (raw) => {
    const input = raw && typeof raw === 'object' ? raw : {};
    const theme = safeTrim(input.theme || input.appearance || input.mode);
    return {
      theme: ['classic','darkBlue','darkGlass'].includes(theme) ? theme : 'classic',
      updatedAt: safeTrim(input.updatedAt || input.updated_at) || nowISO()
    };
  };
  const normalizeUserPreferenceMap = (raw) => {
    const input = raw && typeof raw === 'object' ? raw : {};
    const out = {};
    Object.entries(input).forEach(([key, value]) => { if(key) out[String(key)] = normalizeUserPreferenceEntry(value); });
    return out;
  };
  const getUserPreferenceStore = () => {
    State.data.meta = State.data.meta && typeof State.data.meta === 'object' ? State.data.meta : {};
    State.data.meta.userPreferences = normalizeUserPreferenceMap(State.data.meta.userPreferences);
    return State.data.meta.userPreferences;
  };
  const getUserPreference = (agentId) => {
    const key = safeTrim(agentId);
    if(!key) return normalizeUserPreferenceEntry({});
    return normalizeUserPreferenceEntry(getUserPreferenceStore()[key] || {});
  };
  const setUserPreference = (agentId, patch = {}) => {
    const key = safeTrim(agentId);
    if(!key) return normalizeUserPreferenceEntry({});
    const store = getUserPreferenceStore();
    const merged = normalizeUserPreferenceEntry({ ...(store[key] || {}), ...(patch || {}), updatedAt: nowISO() });
    store[key] = merged;
    State.data.meta.updatedAt = nowISO();
    return merged;
  };

  const _origBuildMetaRow2 = Storage.buildMetaRow.bind(Storage);
  Storage.buildMetaRow = function(state){
    const row = _origBuildMetaRow2(state);
    row.payload = row.payload && typeof row.payload === 'object' ? row.payload : {};
    row.payload.userPreferences = normalizeUserPreferenceMap(state?.meta?.userPreferences);
    return row;
  };
  const _origMapMeta2 = Storage.mapMeta.bind(Storage);
  Storage.mapMeta = function(metaRow){
    const out = _origMapMeta2(metaRow);
    out.userPreferences = normalizeUserPreferenceMap(metaRow?.payload?.userPreferences || out.userPreferences);
    return out;
  };
  ...
  const UserSettingsUI = {
    ...
    init(){
      this.els = {
        nav: $('#btnUserSettingsNav'),
        name: $('#userSettingsName'),
        role: $('#userSettingsRole'),
        email: $('#userSettingsEmail'),
        avatarPreview: $('#userSettingsAvatarPreview'),
        avatarFile: $('#userSettingsAvatarFile'),
        avatarHint: $('#userSettingsAvatarHint'),
        saveAvatar: $('#userSettingsSaveAvatar'),
        removeAvatar: $('#userSettingsRemoveAvatar'),
        themePicker: $('#userThemePicker'),
        saveTheme: $('#userSettingsSaveTheme'),
        currentPassword: $('#userSettingsCurrentPassword'),
        newPassword: $('#userSettingsNewPassword'),
        confirmPassword: $('#userSettingsConfirmPassword'),
        changePassword: $('#userSettingsChangePassword'),
        passwordMsg: $('#userSettingsPasswordMsg')
      };
      on(this.els.nav, 'click', () => { if(Auth.current) UI.goView('userSettings'); });
      on(this.els.avatarFile, 'change', (ev) => this.handleAvatarFile(ev));
      on(this.els.saveAvatar, 'click', () => this.saveAvatar());
      on(this.els.removeAvatar, 'click', () => this.removeAvatar());
      on(this.els.themePicker, 'click', (ev) => {
        const btn = ev.target?.closest?.('[data-theme]');
        if(!btn) return;
        this.selectedTheme = safeTrim(btn.getAttribute('data-theme')) || 'classic';
        this.renderThemeButtons();
        this.applyTheme(this.selectedTheme);
      });
      on(this.els.saveTheme, 'click', () => this.saveTheme());
      on(this.els.changePassword, 'click', () => this.changePassword());
    },
    getCurrentAgent(){ return getCurrentAgentRecord(); },
    getRoleLabel(raw){ const role = safeTrim(raw); return role === 'manager' ? 'מנהל' : role === 'ops' ? 'נציג תפעול' : role === 'admin' ? 'מנהל מערכת' : 'נציג'; },
    avatarForAgent(agent){ return getChatAvatarEntry(agent?.id)?.image || ''; },
    renderAvatar(image, fallbackName=''){
      if(!this.els?.avatarPreview) return;
      if(safeTrim(image)){
        this.els.avatarPreview.innerHTML = `<img src="${escapeHtml(image)}" alt="תמונת פרופיל" />`;
      } else {
        const initials = safeTrim(fallbackName).split(/\s+/).slice(0,2).map((part) => part.charAt(0)).join('').slice(0,2) || '--';
        this.els.avatarPreview.textContent = initials;
      }
    },
    render(){
      const agent = this.getCurrentAgent();
      if(!agent || !this.els) return;
      const security = getAgentSecurity(agent.id);
      const pref = getUserPreference(agent.id);
      this.selectedTheme = pref.theme || 'classic';
      this.draftAvatar = this.avatarForAgent(agent);
      if(this.els.name) this.els.name.textContent = safeTrim(agent.name) || '—';
      if(this.els.role) this.els.role.textContent = this.getRoleLabel(agent.role || Auth.current?.role);
      if(this.els.email) this.els.email.textContent = safeTrim(security.authEmail) || 'לא הוגדר מייל';
      if(this.els.avatarHint) this.els.avatarHint.textContent = this.draftAvatar ? 'התמונה תוצג במערכת ובצ׳אט בין הנציגים.' : 'מומלץ להעלות תמונה מרובעת ונקייה.';
      this.renderAvatar(this.draftAvatar, agent.name || Auth.current?.name || '');
      this.renderThemeButtons();
      this.clearPasswordFields();
    },
    renderThemeButtons(){
      $$('[data-theme]', this.els?.themePicker || document).forEach((btn) => btn.classList.toggle('is-active', safeTrim(btn.getAttribute('data-theme')) === this.selectedTheme));
    },
    applyTheme(theme){
      const safeTheme = ['classic','darkBlue','darkGlass'].includes(theme) ? theme : 'classic';
      document.body.classList.remove('theme-darkBlue','theme-darkGlass');
      if(safeTheme !== 'classic') document.body.classList.add('theme-' + safeTheme);
    },
    async handleAvatarFile(ev){
      const file = ev?.target?.files?.[0];
      if(!file) return;
      if(!/^image\//i.test(file.type || '')){
        if(this.els.avatarHint) this.els.avatarHint.textContent = 'אפשר להעלות רק קובץ תמונה.';
        return;
      }
      const reader = new FileReader();
      reader.onload = () => {
        this.draftAvatar = safeTrim(reader.result);
        const agent = this.getCurrentAgent();
        this.renderAvatar(this.draftAvatar, agent?.name || '');
        if(this.els.avatarHint) this.els.avatarHint.textContent = 'התמונה נטענה. לחץ על "שמור תמונה" כדי לעדכן במערכת.';
      };
      reader.readAsDataURL(file);
    },
    async saveAvatar(){
      const agent = this.getCurrentAgent();
      if(!agent) return;
      setChatAvatarEntry(agent.id, this.draftAvatar || '');
      await App.persist(this.draftAvatar ? 'תמונת הפרופיל נשמרה' : 'תמונת הפרופיל הוסרה');
      if(this.els.avatarHint) this.els.avatarHint.textContent = this.draftAvatar ? 'תמונת הפרופיל נשמרה בהצלחה.' : 'תמונת הפרופיל הוסרה.';
      try { ChatUI.renderHeader?.(); ChatUI.renderUsers?.(); } catch(_e) {}
      this.render();
    },
    async removeAvatar(){
      this.draftAvatar = '';
      const agent = this.getCurrentAgent();
      this.renderAvatar('', agent?.name || '');
      await this.saveAvatar();
    },
    async saveTheme(){
      const agent = this.getCurrentAgent();
      if(!agent) return;
      setUserPreference(agent.id, { theme:this.selectedTheme });
      this.applyTheme(this.selectedTheme);
      await App.persist('הגדרות התצוגה נשמרו');
      if(this.els.avatarHint) this.els.avatarHint.textContent = 'מצב התצוגה נשמר בהצלחה.';
    },
    clearPasswordFields(){
      if(this.els.currentPassword) this.els.currentPassword.value = '';
      if(this.els.newPassword) this.els.newPassword.value = '';
      if(this.els.confirmPassword) this.els.confirmPassword.value = '';
      if(this.els.passwordMsg) this.els.passwordMsg.textContent = '';
    },
    setPasswordMsg(msg){ if(this.els.passwordMsg) this.els.passwordMsg.textContent = msg ? String(msg) : ''; },
    async changePassword(){
      const agent = this.getCurrentAgent();
      if(!agent) return;
      const currentPin = safeTrim(this.els.currentPassword?.value);
      const newPin = safeTrim(this.els.newPassword?.value);
      const confirmPin = safeTrim(this.els.confirmPassword?.value);
      this.setPasswordMsg('');
      if(!currentPin) return this.setPasswordMsg('נא להזין את הסיסמה הנוכחית.');
      if(currentPin !== safeTrim(agent.pin)) return this.setPasswordMsg('הסיסמה הנוכחית אינה תואמת למשתמש המחובר.');
      if(!newPin || newPin.length < 4) return this.setPasswordMsg('הסיסמה החדשה חייבת להכיל לפחות 4 תווים.');
      if(newPin !== confirmPin) return this.setPasswordMsg('אימות הסיסמה החדשה לא תואם.');
      if(newPin === currentPin) return this.setPasswordMsg('בחר סיסמה חדשה ששונה מהקיימת.');
      const security = getAgentSecurity(agent.id);
      if(safeTrim(security.authEmail)){
        try {
          const client = Storage.getClient();
          const { error } = await client.auth.updateUser({ password:newPin });
          if(error) throw error;
        } catch(err) {
          return this.setPasswordMsg('לא הצלחתי לעדכן את סיסמת ה-Auth של המשתמש.');
        }
      }
      agent.pin = newPin;
      agent.updated_at = nowISO();
      State.data.meta.updatedAt = nowISO();
      await App.persist('סיסמת המשתמש עודכנה');
      this.clearPasswordFields();
      this.setPasswordMsg('הסיסמה עודכנה בהצלחה. מהכניסה הבאה יש להשתמש בסיסמה החדשה.');
    }
  };

  const _uiInit2 = UI.init.bind(UI);
  UI.init = function(){
    _uiInit2();
    this.els.navUserSettings = $('#btnUserSettingsNav');
    if(this.els.navUserSettings) on(this.els.navUserSettings, 'click', () => { if(Auth.current) this.goView('userSettings'); });
  };
  const _uiApplyRoleUI2 = UI.applyRoleUI.bind(UI);
  UI.applyRoleUI = function(){
    _uiApplyRoleUI2();
    if(this.els?.navUserSettings) this.els.navUserSettings.style.display = Auth.current && !Auth.isAdmin() ? '' : 'none';
  };
  const _uiGoView2 = UI.goView.bind(UI);
  UI.goView = function(view){
    let safe = String(view || 'dashboard');
    if(safe === 'userSettings' && (!Auth.current || Auth.isAdmin())) safe = 'dashboard';
    _uiGoView2(safe);
    if(this.els?.navUserSettings) this.els.navUserSettings.classList.toggle('is-active', safe === 'userSettings');
    document.body.classList.remove('view-userSettings-active');
    if(safe === 'userSettings'){
      document.body.classList.add('view-userSettings-active');
      UserSettingsUI.render();
    }
    const map = { userSettings:'הגדרות משתמש' };
    if(this.els?.pageTitle && map[safe]) this.els.pageTitle.textContent = map[safe];
  };

  const _renderAuthPill2 = UI.renderAuthPill.bind(UI);
  UI.renderAuthPill = function(){
    _renderAuthPill2();
    const agent = getCurrentAgentRecord();
    const pref = getUserPreference(agent?.id);
    UserSettingsUI.applyTheme(pref.theme || 'classic');
  };

  const _appBoot2 = App.boot.bind(App);
