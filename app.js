...
// nav
      ...
      on(this.els.customersTbody, "click", (ev) => {
        const openBtn = ev.target?.closest?.("[data-open-customer]");
        if(openBtn){
          ev.preventDefault();
          const id = openBtn.getAttribute("data-open-customer");
          if(id) CustomersUI.openByIdWithLoader(id);
          return;
        }
        const archiveBtn = ev.target?.closest?.("[data-archive-customer]");
        if(archiveBtn){
          ev.preventDefault();
          const id = archiveBtn.getAttribute("data-archive-customer");
          if(id) ArchiveCustomerUI.open(id);
        }
      });
      ...
      (payload.insureds || []).forEach((ins, idx) => {
        ...
        (ins?.data?.existingPolicies || []).forEach((p, pIdx) => {
          ...
          policies.push({
            ...
            discountYears: safeTrim(p?.discountYears),
            premiumAfterDiscount: this.formatMoneyValue(this.getPolicyPremiumAfterDiscount(p)),
...
init(){
    ...
    handleStepEntry(fromStep, toStep){
      if(Number(toStep) !== 7 || Number(fromStep) === 7) return;
    ...
    renderBody(){
      ...
      else if(this.step === 7) this.bindHealthInputs();
      else if(this.step === 8) this.bindSummaryActions();
    },
...
// ---------- Step 6 ----------
    ...
    renderStep7(){
      ...
      const renderExistingSummaryTable = (list=[], ins=null) => {
        ...
        const rows = list.map(p => {
          ...
          const action = this.getCancellationSummary(ins, p);
          return `<tr>
            ...
            <td>${pledgeTxt}</td>
            <td><span class="lcSummaryActionBadge ${escapeHtml(action.cls)}">${escapeHtml(action.text)}</span></td>
            <td>${escapeHtml(action.reason || '—')}</td>
          </tr>`;
        }).join("");
        return `<div class="lcPolTableWrap" style="margin-top:10px">
          <table class="lcPolTable">
            <thead><tr><th>חברה</th><th>סוג</th><th>מספר</th><th>סכום/פיצוי</th><th>שיעבוד</th><th>תוצאה</th><th>סיבת פעולה</th></tr></thead>
            <tbody>${rows}</tbody>
          </table>
        </div>`;
      };
      ...
      const existingBlocks = this.insureds.map(ins => {
        ...
        return `<div class="lcWSection lcSummarySection">
          ...
          ${renderExistingSummaryTable(list, ins)}
        </div>`;
      }).join("");
      ...
      const healthBlock = this.renderSummaryHealthBlock();

      return `
        <div class="lcWSection lcSummaryHero lcSummaryHero--premium">
          <div class="lcSummaryHero__top">
            <div>
              <div class="lcSummaryHero__kicker">בדיקה אחרונה לפני יצירת תיק</div>
              <div class="lcWTitle lcSummaryHero__title">סיכום הקמה</div>
              <div class="lcSummaryHero__sub">המערכת מציגה כאן את כל מה שיקרה עם הפוליסות הקיימות, את הפוליסות החדשות שנבנו, ואת סטטוס הצהרת הבריאות לפני יצירת התיק.</div>
            </div>
            <div class="lcSummaryHero__seal">✓</div>
          </div>
    ...
    getCancellationSummary(ins, policy){
      const c = ins?.data?.cancellations?.[policy?.id] || {};
      const status = safeTrim(c.status || '');
      const map = {
        full: { text:'ביטול מלא', cls:'is-danger' },
        partial_health: { text:'ביטול חלקי', cls:'is-warn' },
        nochange_client: { text:'ללא שינוי – לבקשת הלקוח', cls:'is-muted' },
        agent_appoint: { text:'מינוי סוכן', cls:'is-info' },
        nochange_collective: { text:'ללא שינוי – קולקטיב', cls:'is-muted' }
      };
      const picked = map[status] || { text:'טרם נבחרה פעולה', cls:'is-pending' };
      return {
        text: picked.text,
        cls: picked.cls,
        reason: safeTrim(c.reason || '')
      };
    },

    getHealthFindingsForInsured(ins){
      const findings = [];
      this.getHealthQuestionList().forEach(item => {
        const r = this.getHealthResponse(item.question.key, ins.id);
        if(r.answer === 'yes' && r.saved){
          findings.push({
            question: item.question,
            summary: this.summarizeHealthFields(r.fields || {}),
            companies: item.question?.companies || []
          });
        }
      });
      return findings;
    ...
    renderSummaryHealthBlock(){
      const list = this.getHealthQuestionList();
      if(!list.length){
        return `<div class="lcWSection lcSummarySection"><div class="lcWTitle">הצהרת בריאות</div><div class="muted small">לא נדרשה הצהרת בריאות בתהליך הזה כי לא נבחרה פוליסה רלוונטית בשלב 5.</div></div>`;
      }
      const cards = this.insureds.map(ins => {
        const findings = this.getHealthFindingsForInsured(ins);
        const status = findings.length ? 'יש ממצאים בהצהרה זו' : 'הצהרת בריאות הושלמה';
        const badgeCls = findings.length ? 'is-warn' : 'is-ok';
        return `<div class="lcSummaryHealthCard">
          <div class="lcSummaryHealthCard__head">
            <div>
              <div class="lcSummaryHealthCard__name">${escapeHtml(ins.label)}</div>
              <div class="lcSummaryHealthCard__status ${badgeCls}">${escapeHtml(status)}</div>
            </div>
            <div class="lcSummaryHealthCard__icon">🩺</div>
          </div>
          <div class="lcSummaryHealthCard__body">
            <div class="lcSummaryHealthCard__line">${findings.length ? `נמצאו ${findings.length} ממצאים שסומנו כן ונשמרו.` : 'כל השאלון הושלם ואין ממצאים רפואיים פתוחים.'}</div>
            ${findings.length ? `<button type="button" class="btn btn--primary lcSummaryHealthCard__btn" data-health-findings="${escapeHtml(ins.id)}">הצג ממצאים</button>` : ''}
          </div>
        </div>`;
      }).join('');
      return `<div class="lcWSection lcSummarySection lcSummarySection--health">
        <div class="lcWTitle">הצהרת בריאות</div>
        <div class="muted small">במסך הסיכום מוצג סטטוס חכם בלבד. בלחיצה על “הצג ממצאים” יופיעו רק השאלות שסומנו כן.</div>
        <div class="lcSummaryHealthGrid">${cards}</div>
      </div>`;
    },

    bindSummaryActions(){
      this.els.body.querySelectorAll('[data-health-findings]').forEach(btn => {
        on(btn, 'click', () => {
          const insId = btn.getAttribute('data-health-findings');
          const ins = this.insureds.find(x => String(x.id) === String(insId));
          if(ins) this.openHealthFindingsModal(ins);
        });
      });
    },

    openHealthFindingsModal(ins){
      let modal = document.getElementById('lcHealthFindingsModal');
      if(!modal){
        modal = document.createElement('div');
        modal.id = 'lcHealthFindingsModal';
        modal.className = 'modal lcHealthFindingsModal';
        modal.setAttribute('aria-hidden', 'true');
        modal.innerHTML = `<div class="modal__backdrop" data-close="1"></div>
          <div class="modal__panel lcHealthFindingsModal__panel" role="dialog" aria-modal="true" aria-label="ממצאי הצהרת בריאות">
            <div class="modal__head">
              <div>
                ...
                <div class="modal__title" id="lcHealthFindingsTitle">ממצאי הצהרת בריאות</div>
              </div>
              <button class="iconBtn" type="button" id="lcHealthFindingsClose">✕</button>
            </div>
            <div class="modal__body">
              <div class="lcHealthFindingsModal__body" id="lcHealthFindingsBody"></div>
            </div>
            <div class="modal__foot">
              <button class="btn" type="button" id="lcHealthFindingsCloseBottom">סגור</button>
            </div>
          </div>`;
        document.body.appendChild(modal);
        const close = () => this.closeHealthFindingsModal();
        on(modal.querySelector('[data-close="1"]'), 'click', close);
        on(modal.querySelector('#lcHealthFindingsClose'), 'click', close);
        on(modal.querySelector('#lcHealthFindingsCloseBottom'), 'click', close);
      }
      const title = modal.querySelector('#lcHealthFindingsTitle');
      const body = modal.querySelector('#lcHealthFindingsBody');
      const findings = this.getHealthFindingsForInsured(ins);
      if(title) title.textContent = `ממצאי הצהרת בריאות · ${ins.label}`;
      if(body){
        body.innerHTML = findings.length ? findings.map((item, idx) => `<div class="lcHealthFindingsItem">
          <div class="lcHealthFindingsItem__top">
            <span class="lcHealthFindingsItem__index">${idx + 1}</span>
            <div>
              <div class="lcHealthFindingsItem__question">${escapeHtml(item.question.text)}</div>
              <div class="lcHealthFindingsItem__summary">${escapeHtml(item.summary || 'סומן כן ונשמר')}</div>
            </div>
          </div>
        </div>`).join('') : `<div class="muted">לא נמצאו ממצאים שמורים עבור מבוטח זה.</div>`;
      }
    ...
    closeHealthFindingsModal(){
      const modal = document.getElementById('lcHealthFindingsModal');
      if(!modal) return;
