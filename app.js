...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  // Policy Spec (module)
  const policySpecOverlay = $('#policySpecOverlay');
  const policySpecCloseBtn = $('#policySpecCloseBtn');
  const policySpecFrame = $('#policySpecFrame');
  const policySpecFallback = $('#policySpecFallback');
  ...
  // Set your Policy Spec module URL (GitHub Pages) here when ready
  // Example: const POLICY_SPEC_URL = 'https://YOURNAME.github.io/policy-spec/';
  const POLICY_SPEC_URL = '';
...
function openUserModal_(opt){
  ...
  if(policySpecCloseBtn){
    policySpecCloseBtn.addEventListener('click', closePolicySpec_);
  }
  if(policySpecOverlay){
    policySpecOverlay.addEventListener('click', (e) => { if(e.target === policySpecOverlay) closePolicySpec_(); });
  }
...
// Listen to events from the GOFI iframe (optional)
  ...
  // ---------- Policy Spec (Insurance Companies module via iframe) ----------
  function getPolicySpecOrigin_(){
    try{ return POLICY_SPEC_URL ? new URL(POLICY_SPEC_URL).origin : ''; }catch(_){ return ''; }
  }

  let __policySpecPrevBodyOverflow = null;
  function ensurePolicySpecFullscreen_(){
    try{
      if(policySpecOverlay){
        policySpecOverlay.style.position = 'fixed';
        policySpecOverlay.style.inset = '0';
        policySpecOverlay.style.width = '100vw';
        policySpecOverlay.style.height = '100vh';
        policySpecOverlay.style.maxWidth = '100vw';
        policySpecOverlay.style.maxHeight = '100vh';
        policySpecOverlay.style.zIndex = '999999';
        policySpecOverlay.style.display = 'flex';
        policySpecOverlay.style.flexDirection = 'column';
        policySpecOverlay.style.alignItems = 'stretch';
        policySpecOverlay.style.justifyContent = 'stretch';
        policySpecOverlay.style.padding = '0';
        if(!policySpecOverlay.style.background || policySpecOverlay.style.background === 'transparent'){
          policySpecOverlay.style.background = 'rgba(0,0,0,0.25)';
        }
        const inner = policySpecOverlay.firstElementChild;
        if(inner){
          inner.style.width = '100%';
          inner.style.height = '100%';
          inner.style.maxHeight = '100vh';
      ...
      if(policySpecFrame){
        const p = policySpecFrame.parentElement;
        if(p){
          p.style.display = 'flex';
          p.style.flexDirection = 'column';
          p.style.flex = '1 1 auto';
        ...
        policySpecFrame.style.flex = '1 1 auto';
        policySpecFrame.style.width = '100%';
        policySpecFrame.style.height = '100%';
        policySpecFrame.style.minHeight = '0';
        policySpecFrame.style.maxHeight = '100vh';
        policySpecFrame.style.border = '0';
        policySpecFrame.style.borderRadius = '0';
        policySpecFrame.style.display = 'block';
      }

      if(__policySpecPrevBodyOverflow === null){
        __policySpecPrevBodyOverflow = document.body.style.overflow || '';
      }
      document.body.style.overflow = 'hidden';
    }catch(_){ }
  }

  function restorePolicySpecFullscreen_(){
    try{
      if(__policySpecPrevBodyOverflow !== null){
        document.body.style.overflow = __policySpecPrevBodyOverflow;
        __policySpecPrevBodyOverflow = null;
      }else{
        document.body.style.overflow = '';
      }
    }catch(_){ }
  ...
  function buildDiscountsPayload_(){
    try{
      const proposal = wiz ? wiz.proposal : null;
      const arr = proposal && Array.isArray(proposal.newPolicies) ? proposal.newPolicies : [];
      const discounts = arr.map((x, idx) => ({
        idx,
        insured: labelInsured(x.insuredId||'main'),
        company: x.company || '',
        product: x.product || '',
        premium: Number(x.premium||0) || 0,
        tier: x.discount && x.discount.tier ? String(x.discount.tier) : '',
        types: (x.discount && Array.isArray(x.discount.types)) ? x.discount.types : [],
        note: x.discount && x.discount.note ? String(x.discount.note) : ''
      })).filter(x => (x.tier && x.tier.trim() && x.tier.trim()!=='0%') || (x.types && x.types.length) || (x.note && x.note.trim()));

      return {
        type: 'POLICY_SPEC_DATA',
        ts: Date.now(),
        agentId: getAgentId_(),
        customerName: proposal ? (proposal.customerName||'') : '',
        proposalId: proposal ? (proposal.id||'') : '',
        discounts
      };
    }catch(e){
      return { type:'POLICY_SPEC_DATA', ts: Date.now(), discounts: [] };
    }
  }

  function openPolicySpec_(){
    if(!policySpecOverlay) return;
    ensurePolicySpecFullscreen_();

    policySpecOverlay.classList.remove('hidden');
    policySpecOverlay.setAttribute('aria-hidden', 'false');

    const hasUrl = !!String(POLICY_SPEC_URL||'').trim();
    if(policySpecFallback){
      policySpecFallback.classList.toggle('hidden', hasUrl);
    }
    if(policySpecFrame){
      policySpecFrame.classList.toggle('hidden', !hasUrl);
      if(hasUrl){
        policySpecFrame.src = POLICY_SPEC_URL;
        policySpecFrame.onload = () => {
          try{
            const origin = getPolicySpecOrigin_();
            const payload = buildDiscountsPayload_();
            if(origin && policySpecFrame.contentWindow){
              policySpecFrame.contentWindow.postMessage(payload, origin);
            }
          }catch(_){ }
        };
      }else{
        policySpecFrame.removeAttribute('src');
      }
    }
  }

  function closePolicySpec_(){
    if(!policySpecOverlay) return;
    policySpecOverlay.classList.add('hidden');
    policySpecOverlay.setAttribute('aria-hidden', 'true');
    restorePolicySpecFullscreen_();
    if(policySpecFrame){
      try{ policySpecFrame.removeAttribute('src'); }catch(_){ }
    }
  }
  ...
  window.addEventListener('message', (event) => {
    ...
    if(!origin || event.origin !== origin) return;
    const data = event.data || {};
    if(data.type === 'POLICY_SPEC_CLOSE'){ closePolicySpec_(); }
    if(data.type === 'POLICY_SPEC_REQUEST_REFRESH'){
      try{
        const payload = buildDiscountsPayload_();
        if(policySpecFrame && policySpecFrame.contentWindow){
          policySpecFrame.contentWindow.postMessage(payload, origin);
        }
      }catch(_){ }
    }
...
function openPolicyDiscountsModal(policy, onSave){
  ...
  // Open external Policy Spec module (iframe)
  const specBtn = overlay.querySelector('[data-spec]');
  if(specBtn){
    specBtn.addEventListener('click', () => {
      try{ openPolicySpec_(); }catch(_){ }
    });
  }
