...
(() => {
  ...
  const defaultState = () => ({
    meta: {
      updatedAt: null,
      adminAuth: { username: "מנהל מערכת", pin: "1234", active: true, changePin: "2468" },
      securityLog: []
    },
    agents: [
  ...
  function normalizeState(s){
    ...
    out.meta.adminAuth = {
      ...
      changePin: safeTrim(rawAdmin.changePin) || safeTrim(defAdmin.changePin) || "2468"
    };

    out.meta.securityLog = Array.isArray(out.meta.securityLog) ? out.meta.securityLog.map((item, idx) => ({
      id: safeTrim(item?.id) || ("log_" + idx),
      at: safeTrim(item?.at) || nowISO(),
      actorName: safeTrim(item?.actorName) || "מערכת",
      actorRole: safeTrim(item?.actorRole) || "system",
      action: safeTrim(item?.action) || "עדכון",
      targetId: safeTrim(item?.targetId),
      targetName: safeTrim(item?.targetName) || "—",
      details: safeTrim(item?.details)
    })) : [];
...
this.els.syncDot = $("#syncDot");
      ...
      this.els.usersLogTbody = $("#usersLogTbody");
...
txt.textContent = "";
  ...
  const UsersUI = {
    ...
    _ensureModal(){
      ...
      this._modalEls = {
        ...
        pinErr: $("#lcUserPinErr"),
        verifyWrap: $("#lcAdminVerifyWrap"),
        verifyPin: $("#lcAdminVerifyPin"),
        verifyErr: $("#lcAdminVerifyErr"),
        changePinWrap: $("#lcAdminChangePinWrap"),
        changePin: $("#lcAdminChangePin"),
        modalNote: $("#lcUserModalNote"),
      };
    ...
    openModal(mode, user){
      ...
      hide(E.err); hide(E.nameErr); hide(E.userErr); hide(E.pinErr); hide(E.verifyErr);

      const isAdminSelf = !!(user && String(user.id) === "admin" && Auth.isAdmin());

      if(E.title) E.title.textContent = isAdminSelf ? "עריכת מנהל מערכת" : (this._modalMode === "edit") ? "עריכת משתמש" : "הוסף נציג/סוכן";
      ...
      if(E.role) {
        E.role.value = user ? (user.role || "agent") : "agent";
        E.role.disabled = isAdminSelf;
      }
      if(E.active) {
        E.active.checked = user ? (user.active !== false) : true;
        E.active.disabled = isAdminSelf;
      }
      if(E.verifyWrap) E.verifyWrap.style.display = isAdminSelf ? "" : "none";
      if(E.changePinWrap) E.changePinWrap.style.display = isAdminSelf ? "" : "none";
      if(E.verifyPin) E.verifyPin.value = "";
      if(E.changePin) E.changePin.value = isAdminSelf ? safeTrim(State.data?.meta?.adminAuth?.changePin || "") : "";
      if(E.modalNote) {
        E.modalNote.style.display = isAdminSelf ? "block" : "none";
        E.modalNote.textContent = isAdminSelf ? "שינוי מנהל מערכת דורש PIN אבטחה נוסף. רק אתה יכול לבצע את הפעולה הזאת." : "";
      }
      ...
      setTimeout(() => (isAdminSelf ? E.verifyPin : E.name)?.focus?.(), 50);
    },
    ...
    async _saveFromModal(){
      ...
      const isAdminSelf = (id === "admin");
      if(isEdit){
        if(isAdminSelf){
          if(!Auth.isAdmin()){
            this._showErr(E.err, "אין הרשאה לערוך את מנהל המערכת");
            pushSecurityLog("ניסיון חסום לעריכת מנהל מערכת", { id:"admin", name: State.data?.meta?.adminAuth?.username || "מנהל מערכת" }, "הפעולה נחסמה לפי הרשאות");
            await App.persist("נחסם ניסיון עריכה");
            return;
          }
          const verifyPin = safeTrim(E.verifyPin?.value);
          const expectedVerify = safeTrim(State.data?.meta?.adminAuth?.changePin || "2468");
          const newChangePin = safeTrim(E.changePin?.value);
          if(!verifyPin){
            this._showErr(E.verifyErr, "נא להזין PIN אבטחה נוסף");
            return;
          }
          if(verifyPin !== expectedVerify){
            this._showErr(E.verifyErr, "PIN האבטחה הנוסף שגוי");
            pushSecurityLog("ניסיון חסום לעריכת מנהל מערכת", { id:"admin", name: State.data?.meta?.adminAuth?.username || "מנהל מערכת" }, "PIN אבטחה נוסף שגוי");
            await App.persist("נחסם ניסיון עריכה");
            return;
          }
          if(!newChangePin){
            this._showErr(E.verifyErr, "נא להזין PIN אבטחה נוסף חדש/קיים");
            return;
          }
          State.data.meta.adminAuth = {
            ...
            pin,
            active: true,
            changePin: newChangePin
          };
          State.data.meta.updatedAt = nowISO();
          pushSecurityLog("עודכן מנהל מערכת", { id:"admin", name: username }, "עודכנו שם משתמש / PIN / PIN אבטחה נוסף");
          await App.persist("עודכן מנהל מערכת");
          if(Auth.current){
            Auth.current.name = username;
            UI.renderAuthPill();
          }
        } else {
          ...
          a.active = active;
          State.data.meta.updatedAt = nowISO();
          pushSecurityLog("עודכן משתמש", { id: a.id, name: a.name }, "עודכנו פרטי משתמש והרשאות");
        ...
        State.data.meta.updatedAt = nowISO();
        pushSecurityLog("נוצר משתמש חדש", { id: newId, name }, "נוצר משתמש חדש במערכת");
    ...
    _filtered(){
      ...
      if (Auth.isAdmin()) {
        ...
        arr.unshift({
          id: "admin",
          name: safeTrim(adminAuth.username) || "מנהל מערכת",
          username: safeTrim(adminAuth.username) || "מנהל מערכת",
          pin: safeTrim(adminAuth.pin),
          role: "admin",
    ...
    render(){
      ...
      UI.els.usersTbody.innerHTML = rows.map(a => {
        ...
        const role = formatRoleLabel(a.role);
        const canToggle = a.id !== "admin";
        return `
          <tr>
            ...
            <td>
              <div class="lcUsers__rowActions">
                ...
                ${canToggle ? `<button class="btn btn--danger" data-act="toggle" data-id="${escapeHtml(a.id)}">${a.active===false ? "הפעל" : "השבת"}</button>` : '<span class="badge">מוגן</span>'}
              </div>
            </td>
          </tr>`;
      }).join("");
      ...
      if(UI.els.usersLogTbody){
        const logs = Array.isArray(State.data?.meta?.securityLog) ? State.data.meta.securityLog.slice(0, 12) : [];
        UI.els.usersLogTbody.innerHTML = logs.length ? logs.map(item => `
          <tr>
            <td>${escapeHtml(item.at || "—")}</td>
            <td>${escapeHtml(item.actorName || "—")}</td>
            <td>${escapeHtml(formatRoleLabel(item.actorRole))}</td>
            <td>${escapeHtml(item.action || "—")}</td>
            <td>${escapeHtml(item.targetName || "—")}</td>
            <td>${escapeHtml(item.details || "—")}</td>
          </tr>`).join("") : `<tr><td colspan="6" class="muted">עדיין אין לוגים</td></tr>`;
      }
    },
    ...
    async toggleUser(id){
      if(String(id) === "admin"){
        pushSecurityLog("ניסיון חסום להשבתת מנהל מערכת", { id:"admin", name: State.data?.meta?.adminAuth?.username || "מנהל מערכת" }, "החשבון הראשי מוגן מהשבתה");
        await App.persist("נחסם ניסיון השבתה");
        this.render();
        return;
      }
      const a = (State.data.agents || []).find(x => String(x.id) === String(id));
      ...
      State.data.meta.updatedAt = nowISO();
      pushSecurityLog(a.active ? "הופעל משתמש" : "הושבת משתמש", { id: a.id, name: a.name }, a.active ? "החשבון הופעל מחדש" : "החשבון הושבת");
  ...
  function currentActor(){
    return {
      ...
      actorRole: safeTrim(Auth?.current?.role) || "system"
    };
  }

  function pushSecurityLog(action, target, details=""){
    try {
      if(!State.data.meta) State.data.meta = {};
      if(!Array.isArray(State.data.meta.securityLog)) State.data.meta.securityLog = [];
      const actor = currentActor();
      State.data.meta.securityLog.unshift({
        id: "log_" + Date.now().toString(16) + "_" + Math.random().toString(16).slice(2,8),
        at: nowISO(),
        actorName: actor.actorName,
        actorRole: actor.actorRole,
        action: safeTrim(action) || "עדכון",
        targetId: safeTrim(target?.id),
        targetName: safeTrim(target?.name) || "—",
        details: safeTrim(details)
      });
      State.data.meta.securityLog = State.data.meta.securityLog.slice(0, 200);
    } catch(_e) {}
  }
  ...
  function formatRoleLabel(role){
    ...
    if(r === "agent") return "נציג";
    if(r === "system") return "מערכת";
    return r || "—";
  }
