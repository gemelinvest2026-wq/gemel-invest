...
// ---------- Step 6 ----------
    ...
    renderOperationalReport(){
      ...
      const mirrorSummary = payload?.mirrorFlow?.summary || {};
      const mirrorRows = Array.isArray(mirrorSummary.insureds) ? mirrorSummary.insureds.map(item => [
        this.renderReportValue(item.label || this.getMirrorRoleLabel(item.type)),
        this.renderReportValue(item.fullName),
        this.renderReportValue(item.idNumber),
        this.renderReportValue(item.phone),
        this.renderReportValue(item.email || item.city || item.clinic || '—')
      ]) : [];
      const mirrorSection = mirrorSummary.savedAt ? `<section class="lcReportSection"><div class="lcReportSection__title">סיכום שיחת שיקוף</div><div class="lcReportSection__sub">הנתונים בסעיף זה נשמרו מתוך מסך השיקופים לאחר אימות טלפוני מול הלקוח.</div><div class="lcReportMeta"><div class="lcReportMetaItem"><b>נשמר בתאריך</b><span>${this.renderReportValue(this.formatDate(mirrorSummary.savedAt))}</span></div><div class="lcReportMetaItem"><b>נשמר על ידי</b><span>${this.renderReportValue(mirrorSummary.savedBy)}</span></div><div class="lcReportMetaItem"><b>דיוור</b><span>${this.renderReportValue(mirrorSummary.deliveryMethod === 'email' ? 'מייל' : mirrorSummary.deliveryMethod === 'home' ? 'בית' : '')}</span></div><div class="lcReportMetaItem"><b>עישון</b><span>${this.renderReportValue(mirrorSummary.smokingAnswer === 'yes' ? 'כן' : mirrorSummary.smokingAnswer === 'no' ? 'לא' : '')}</span></div></div>${mirrorRows.length ? this.renderTable(['סוג מבוטח','שם מלא','תעודת זהות','טלפון','פרט מאומת'], mirrorRows) : ''}</section>` : '';
  ...
  const MirrorsUI = {
    ...
    init(){
      ...
      on(this.els.verifyCard, "change", (ev) => {
        ...
        if(target?.matches?.('[data-mirror-ins-field]')){
          const insuredId = safeTrim(target.getAttribute('data-mirror-ins-id'));
          const field = safeTrim(target.getAttribute('data-mirror-ins-field'));
          this.updateVerifyInsuredField(rec, insuredId, field, target.value);
        }
      });
    ...
    updateSteps(rec){
      const activeStep = !rec ? 1 : (this.consent === 'yes' ? 3 : 2);
    ...
    getVerifyState(rec){
      ...
      if(!store.insuredEdits || typeof store.insuredEdits !== 'object') store.insuredEdits = {};
      if(!safeTrim(store.deliveryEmail)) store.deliveryEmail = safeTrim(rec.email) || safeTrim(this.getPrimary(rec)?.email);
      return store;
    },

    getMirrorRoleLabel(type){
      if(type === 'primary') return 'מבוטח ראשי';
      if(type === 'spouse') return 'מבוטח משני';
      if(type === 'adult') return 'בגיר';
      if(type === 'child') return 'קטין';
      return 'מבוטח';
    },

    getMirrorInsuredFieldDefs(type){
      const base = [
        ['firstName','שם פרטי','text'],
        ['lastName','שם משפחה','text'],
        ['idNumber','תעודת זהות','text'],
        ['birthDate','תאריך לידה','date'],
        ['phone','טלפון','text']
      ];
      if(type !== 'child'){
        base.push(['email','אימייל','email']);
        base.push(['city','עיר','text']);
        base.push(['street','רחוב','text']);
        base.push(['houseNumber','מספר בית','text']);
        base.push(['clinic','קופת חולים','text']);
        base.push(['shaban','שב״ן','text']);
        base.push(['occupation','עיסוק','text']);
      }
      return base;
    },

    getPoliciesForInsured(rec, insured){
      return this.getNewPolicies(rec).filter(policy => {
        if(safeTrim(policy?.insuredMode) === 'couple'){
          return insured.type === 'primary' || insured.type === 'spouse';
        }
        return String(policy?.insuredId || '') === String(insured.id || '');
      });
    },

    getMirrorInsuredBlocks(rec){
      const store = this.getVerifyState(rec);
      return this.getInsureds(rec).map((ins, idx) => {
        ins.data = ins.data || {};
        const insuredId = safeTrim(ins.id) || ('ins_' + idx);
        if(!store.insuredEdits[insuredId] || typeof store.insuredEdits[insuredId] !== 'object'){
          store.insuredEdits[insuredId] = {};
        }
        const edits = store.insuredEdits[insuredId];
        const merged = { ...(ins.data || {}), ...edits };
        const fullName = `${safeTrim(merged.firstName)} ${safeTrim(merged.lastName)}`.trim() || ins.label || this.getMirrorRoleLabel(ins.type);
        return {
          insuredId,
          idx,
          type: safeTrim(ins.type) || (idx === 0 ? 'primary' : ''),
          label: safeTrim(ins.label) || this.getMirrorRoleLabel(ins.type),
          raw: ins,
          data: merged,
          fullName,
          policies: this.getPoliciesForInsured(rec, ins)
        };
      });
    },

    updateVerifyInsuredField(rec, insuredId, field, value){
      const store = this.getVerifyState(rec);
      if(!store.insuredEdits[insuredId] || typeof store.insuredEdits[insuredId] !== 'object'){
        store.insuredEdits[insuredId] = {};
      }
      store.insuredEdits[insuredId][field] = safeTrim(value);
    },

    syncMirrorEditsToRecord(rec){
      const store = this.getVerifyState(rec);
      ...
      rec.payload.insureds = insureds;
      if(!rec.payload.operational || typeof rec.payload.operational !== 'object') rec.payload.operational = {};
      rec.payload.operational.insureds = JSON.parse(JSON.stringify(insureds));
      const primaryIns = insureds[0]?.data || {};
      rec.payload.primary = { ...(rec.payload.primary || {}), ...primaryIns };
      rec.payload.operational.primary = { ...(rec.payload.operational.primary || {}), ...primaryIns };
      rec.fullName = `${safeTrim(primaryIns.firstName)} ${safeTrim(primaryIns.lastName)}`.trim() || rec.fullName;
      rec.idNumber = safeTrim(primaryIns.idNumber) || rec.idNumber;
      rec.phone = safeTrim(primaryIns.phone) || rec.phone;
      rec.email = safeTrim(primaryIns.email) || rec.email;
      rec.city = safeTrim(primaryIns.city) || rec.city;
      if(safeTrim(store.deliveryEmail)) {
        rec.email = safeTrim(store.deliveryEmail);
        rec.payload.primary.email = safeTrim(store.deliveryEmail);
        rec.payload.operational.primary.email = safeTrim(store.deliveryEmail);
        if(insureds[0]?.data) insureds[0].data.email = safeTrim(store.deliveryEmail);
        rec.payload.operational.insureds = JSON.parse(JSON.stringify(insureds));
      }
    },
    ...
    renderScript(rec){
      ...
      const familyBlocks = this.getMirrorInsuredBlocks(rec);
      const familyText = familyBlocks.length ? familyBlocks.map(block => {
        const role = this.getMirrorRoleLabel(block.type);
        const policyText = block.policies.length
          ? block.policies.map(p => `${safeTrim(p.company)}${safeTrim(p.type) ? ' · ' + safeTrim(p.type) : ''}`).filter(Boolean).join(', ')
          : 'ללא פוליסות חדשות שנשמרו';
        return `<div class="mirrorsFamilyScriptCard">
          <div class="mirrorsFamilyScriptCard__head">
            <span class="mirrorsFamilyScriptCard__role">${escapeHtml(role)}</span>
            <strong>${escapeHtml(block.fullName || role)}</strong>
          </div>
          <div class="mirrorsFamilyScriptCard__meta">ת״ז ${escapeHtml(block.data.idNumber || '—')} · טלפון ${escapeHtml(block.data.phone || '—')}</div>
          <div class="mirrorsFamilyScriptCard__policies">${escapeHtml(policyText)}</div>
        </div>`;
      }).join('') : '';
      ...
      this.els.scriptCard.innerHTML = `<div class="mirrorsCard__head">
          <div>
            ...
            <div class="mirrorsCard__hint">השיקוף נבנה כעת לכלל המבוטחים בתיק. עבור כל אדם מוצגים הנתונים והפוליסות שלו בלבד.</div>
          </div>
        ...
        <div class="mirrorsScriptBody">
          ...
          <p>בשיחה זו נעבור יחד על פרטי העסקה והמבוטחים שבתיק, כדי לוודא שכל הנתונים מדויקים לפני המשך טיפול והפקה.</p>
          <p>חשוב לי להדגיש בפניך שזוהי שיחת מכירה מוקלטת. האם אתה מאשר להמשיך בשיחה?</p>
        </div>
        ${familyText ? `<div class="mirrorsFamilyScript"><div class="mirrorsFamilyScript__title">משתתפי העסקה בשיקוף</div><div class="mirrorsFamilyScript__grid">${familyText}</div></div>` : ''}
    ...
    renderVerification(rec){
      ...
      const familyBlocks = this.getMirrorInsuredBlocks(rec);
      const infoCards = [
        ['מספר מבוטחים בתיק', String(familyBlocks.length || 1)],
        ['חברות בתיק', this.getCompanyNames(rec).join(', ') || '—'],
        ['נציג מטפל', rec.agentName || '—'],
        ['כתובת למשלוח הפוליסה', addressText || '—']
      ];
      const familyHtml = familyBlocks.map(block => {
        const role = this.getMirrorRoleLabel(block.type);
        const policySummary = block.policies.length
          ? block.policies.map(p => `${safeTrim(p.company)}${safeTrim(p.type) ? ' · ' + safeTrim(p.type) : ''}`).filter(Boolean).join(' | ')
          : 'אין פוליסות חדשות מקושרות למבוטח זה';
        const fieldsHtml = this.getMirrorInsuredFieldDefs(block.type).map(([fieldKey, fieldLabel, inputType]) => {
          const value = safeTrim(block.data?.[fieldKey]);
          return `<label class="mirrorsEditableField">
            <span>${escapeHtml(fieldLabel)}</span>
            <input class="input" data-mirror-ins-id="${escapeHtml(block.insuredId)}" data-mirror-ins-field="${escapeHtml(fieldKey)}" type="${escapeHtml(inputType)}" value="${escapeHtml(value)}" />
          </label>`;
        }).join('');
        return `<section class="mirrorsInsuredBlock">
          <div class="mirrorsInsuredBlock__head">
            <div>
              <div class="mirrorsInsuredBlock__role">${escapeHtml(role)}</div>
              <div class="mirrorsInsuredBlock__name">${escapeHtml(block.fullName || role)}</div>
            </div>
            <div class="mirrorsInsuredBlock__badge">${escapeHtml(String(block.policies.length))} פוליסות</div>
          </div>
          <div class="mirrorsInsuredBlock__policies">${escapeHtml(policySummary)}</div>
          <div class="mirrorsEditableGrid">${fieldsHtml}</div>
        </section>`;
      }).join('');
      this.els.verifyCard.innerHTML = `<div class="mirrorsCard__head">
          <div>
            <div class="mirrorsCard__title">אימות נתוני לקוח</div>
            <div class="mirrorsCard__hint">אם הלקוח ענה אחרת ממה שרשום, עדכן כאן במקום. השמירה תעדכן סופית את נתוני התיק ואת הדוח התפעולי.</div>
          </div>
          <span class="mirrorsVerifyBadge">שלב המשך שיחה</span>
        </div>

        <div class="mirrorsPromptBar">ברשותך אשאל אותך מספר שאלות ואוודא את הנתונים של כל המבוטחים בתיק:</div>
        ...
        <div class="mirrorsFamilyVerify">
          <div class="mirrorsFamilyVerify__title">אימות מבוטחים בתיק</div>
          <div class="mirrorsFamilyVerify__sub">הנתונים למטה ניתנים לעריכה בזמן אמת לפי מה שהלקוח מוסר בשיחת השיקוף.</div>
          <div class="mirrorsInsuredBlocks">${familyHtml}</div>
        </div>

        <div class="mirrorsFormBlock">
          <div class="mirrorsFormBlock__title">שאלת עישון</div>
          <div class="mirrorsFormBlock__hint">האם המבוטח הראשי מעשן או עישן בשנתיים האחרונות? סיגריות, טבק, אלקטרוניות, נרגילה, קנאביס או מוצרי טבק אחרים.</div>
    ...
    async saveVerification(){
      ...
      this.syncMirrorEditsToRecord(rec);
      store.savedAt = nowISO();
      store.savedBy = safeTrim(Auth?.current?.name);
      if(!rec.payload.mirrorFlow.summary || typeof rec.payload.mirrorFlow.summary !== 'object') rec.payload.mirrorFlow.summary = {};
      rec.payload.mirrorFlow.summary = {
        savedAt: store.savedAt,
        savedBy: store.savedBy,
        deliveryMethod,
        deliveryEmail: safeTrim(store.deliveryEmail),
        smokingAnswer,
        smokingProducts: Array.isArray(store.smokingProducts) ? store.smokingProducts.slice() : [],
        smokingQuantity: safeTrim(store.smokingQuantity),
        insureds: this.getMirrorInsuredBlocks(rec).map(block => ({
          type: block.type,
          label: block.label,
          fullName: block.fullName,
          idNumber: safeTrim(block.data.idNumber),
          phone: safeTrim(block.data.phone),
          email: safeTrim(block.data.email),
          city: safeTrim(block.data.city),
          clinic: safeTrim(block.data.clinic),
          shaban: safeTrim(block.data.shaban),
          occupation: safeTrim(block.data.occupation)
        }))
      };
      State.data.meta.updatedAt = nowISO();
      ...
      this.render();
      alert('שלב אימות הנתונים נשמר בהצלחה ועודכן בדוח התפעולי.');
    },
