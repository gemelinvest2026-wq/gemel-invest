...
txt.textContent = "";
  ...
  const CustomersUI = {
    ...
    render(){
      ...
      UI.els.customersTbody.innerHTML = rows.length ? rows.map(rec => {
        ...
        return `<tr>
          ...
          <td><div class="lcCustomers__rowActions">
            ...
            <button class="btn" data-open-ops="${escapeHtml(rec.id)}" type="button">דוח תפעולי</button>
            <button class="btn btn--danger lcCustomers__archiveBtn" data-archive-customer="${escapeHtml(rec.id)}" type="button">גניזת לקוח</button>
          </div></td>
        </tr>`;
      ...
      UI.els.customersTbody.querySelectorAll("[data-open-ops]").forEach(btn => {
        on(btn, "click", () => this.openOpsById(btn.getAttribute("data-open-ops")));
      });
    ...
    openById(id){
      ...
      if(this.els.dash){
        this.els.dash.innerHTML = `
          <div class="dashCard"><div class="dashCard__k">מספר מבוטחים</div><div class="dashCard__v">${rec.insuredCount || 0}</div><div class="dashCard__sub">כולל ראשי / בני זוג / ילדים</div></div>
          <div class="dashCard"><div class="dashCard__k">פוליסות קיימות</div><div class="dashCard__v">${rec.existingPoliciesCount || 0}</div><div class="dashCard__sub">משתקף מהוויזארד</div></div>
          <div class="dashCard"><div class="dashCard__k">פוליסות חדשות</div><div class="dashCard__v">${rec.newPoliciesCount || 0}</div><div class="dashCard__sub">הצעות חדשות שנשמרו</div></div>
        `;
      }
      if(this.els.body){
        ...
        (payload.insureds || []).forEach(ins => {
          (ins.data?.existingPolicies || []).forEach(pol => {
            existingRows.push(`<tr>
              <td>${escapeHtml(ins.label || "")}</td>
              <td>${escapeHtml(pol.company || "—")}</td>
              <td>${escapeHtml(pol.type || "—")}</td>
              <td>${escapeHtml(pol.policyNumber || "—")}</td>
              <td>${escapeHtml(pol.monthlyPremium || "—")}</td>
            </tr>`);
          });
        });

        const newRows = (payload.newPolicies || []).map(pol => `<tr>
          <td>${escapeHtml(pol.company || "—")}</td>
          <td>${escapeHtml(pol.type || "—")}</td>
          <td>${escapeHtml(pol.premiumBefore || "—")}</td>
          <td>${escapeHtml(pol.discountPct || "—")}</td>
          <td>${escapeHtml(pol.startDate || "—")}</td>
        </tr>`).join("");
          ...
          <div class="lcCustomerFile">
            ...
            <section class="lcCfSection">
              <div class="lcCfSection__head">
                ...
                <div class="lcCfSection__sub">נתונים שנאספו מהלקוח</div>
              </div>
              ${this.renderFileTable(["מבוטח","חברה","סוג ביטוח","מספר פוליסה","פרמיה חודשית"], existingRows.join(""))}
            </section>

            <section class="lcCfSection">
              <div class="lcCfSection__head">
                <div class="lcCfSection__title">פוליסות חדשות</div>
                <div class="lcCfSection__sub">הפוליסות החדשות שהוקמו בתהליך</div>
              </div>
              ${this.renderFileTable(["חברה","סוג ביטוח","פרמיה לפני הנחה","% הנחה","תאריך תחילה"], newRows)}
            </section>
  ...
  const ArchiveCustomerUI = {
    ...
    getAdminPin(){
      const fallback = "1234";
      const pin = safeTrim(State.data?.meta?.adminAuth?.pin);
      return pin || fallback;
    },
    ...
    async confirm(){
      ...
      const next = (Array.isArray(State.data?.customers) ? State.data.customers : []).filter(x => String(x.id) !== String(id));
      ...
      if(!r?.ok){
        this.showError("הלקוח הוסר מקומית, אבל השמירה ל-Google Sheets נכשלה. בדוק חיבור ונסה שוב סנכרון.");
        CustomersUI.render();
        return;
      }
