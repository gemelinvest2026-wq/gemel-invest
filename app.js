...
(() => {
  ...
  const BUILD = "20260330-har-force-clean-v15";

  (function forceCleanCachesOnce(){
    try {
      const url = new URL(window.location.href);
      const already = url.searchParams.get("forceClean") === "1";
      if(already) return;

      const finish = () => {
        try {
          url.searchParams.set("forceClean", "1");
      ...
      const cachePromise = (window.caches && typeof window.caches.keys === "function")
        ? window.caches.keys().then((keys) => Promise.all(keys.map((key) => window.caches.delete(key)))).catch(() => null)
        : Promise.resolve(null);

      const swPromise = (navigator.serviceWorker && typeof navigator.serviceWorker.getRegistrations === "function")
        ? navigator.serviceWorker.getRegistrations().then((regs) => Promise.all((regs || []).map((reg) => reg.unregister()))).catch(() => null)
        : Promise.resolve(null);

      Promise.all([cachePromise, swPromise]).finally(finish);
    } catch(_e) {}
  })();
