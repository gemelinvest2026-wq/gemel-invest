...
(() => {
  ...
  function setSession_(sess){
    try{
      localStorage.setItem(SESSION_LS_KEY, JSON.stringify(sess || null));
    }catch(_){}
  }
  ...
  function showLogin_(){
    ...
    // If you later set CSS variable --login-bg-image, you can mark data-has-image="1"
    // ov.dataset.hasImage = '1';
    ...
    try{
      const lastU = localStorage.getItem('gemel_last_user') || '';
      if(u && !u.value) u.value = lastU;
    }catch(_){}

    setTimeout(() => { try{ u && u.focus(); }catch(_){} }, 30);
  }
  ...
  function authenticateLocal_(username, password){
    const users = getUsers_();
    const u = users.find(x => x.username === username);
    if(!u) return { ok:false, msg:'שם משתמש לא קיים' };
    if(u.active === false) return { ok:false, msg:'המשתמש מושבת' };
    if(String(u.password||'') !== String(password||'')) return { ok:false, msg:'סיסמה שגויה' };
  ...
  function doLogin_(){
    ...
    const username = normalize_(userEl ? userEl.value : '');
    const password = String(passEl ? passEl.value : '');

    if(errEl) errEl.textContent = '';

    if(!username || !password){
      if(errEl) errEl.textContent = 'נא למלא שם משתמש וסיסמה';
      return;
    }

    const res = authenticateLocal_(username, password);
    if(!res.ok){
      if(errEl) errEl.textContent = res.msg || 'שגיאת התחברות';
      return;
    }

    state.currentUser = res.user;
    state.agentName = res.user.displayName || res.user.username;
    applyPermissions_();

    try{ localStorage.setItem('gemel_last_user', username); }catch(_){}

    if(rememberEl && rememberEl.checked){
      setSession_({ username: res.user.username });
    }else{
      clearSession_();
    }
  ...
  function wireLoginUI_(){
    ...
    const userEl = document.getElementById('loginUser');
    const passEl = document.getElementById('loginPass');
    const hintBtn = document.getElementById('loginShowHint');
    const hint = document.getElementById('loginHint');
    const logoutBtn = document.getElementById('logoutBtn');

    if(btn) btn.addEventListener('click', doLogin_);
    const onEnter = (e) => { if(e.key === 'Enter') doLogin_(); };
  ...
  function tryRestoreSession_(){
    ...
    state.currentUser = { username:u.username, role:u.role, displayName:u.displayName };
    state.agentName = u.displayName || u.username;
    applyPermissions_();
    return true;
  }
  ...
  (function bootAuth_(){
    ...
    // Try restore remembered session
