...
calcAge(dateStr){
  ...
  const MirrorsUI = {
    ...
    updateSteps(rec){
      ...
      const activeStep = !rec ? 1 : (this.consent !== 'yes' ? 2 : (verify?.reflectionOpened ? 4 : 3));
    ...
    renderReflection(rec){
      ...
      this.els.reflectCard.innerHTML = `<div class="mirrorsCard__head">
          <div>
            ...
            <div class="mirrorsCard__hint">מסך ההקראה הבא לנציג לאחר בחירת אופן קבלת הדיוורים. אם הלקוח מתקן נתונים, אפשר לערוך ישירות בתוך הכרטיסים ולשמור.</div>
          </div>
        ...
        ${harConsent === 'yes' ? `
          ...
          <div class="mirrorsReflectActions">
            <button class="btn btn--primary" data-reflect-save type="button">שמור שיקוף ועדכונים</button>
          </div>
        ` : ''}`;
    },
    ...
    renderReflectionPolicyRow(row, idx){
      ...
      const type = safeTrim(p.type) || '—';
      const statusValue = safeTrim(row.cancellation?.status || p.status || p.policyStatus || p.state || '');
      ...
      const statusOptions = this.getCancellationStatusOptions().map(opt => `<option value="${escapeHtml(opt.value)}"${statusValue === opt.value ? ' selected' : ''}>${escapeHtml(opt.label)}</option>`).join('');
      return `<tr>
        <td><div class="mirrorsReflectInsured">${escapeHtml(insuredText || '—')}</div></td>
        <td><input class="input" data-reflect-index="${idx}" data-reflect-field="company" value="${escapeHtml(company)}" /></td>
        <td><input class="input" data-reflect-index="${idx}" data-reflect-field="type" value="${escapeHtml(type)}" /></td>
        <td><select class="input" data-reflect-index="${idx}" data-reflect-field="status">${statusOptions}</select></td>
        <td><input class="input" data-reflect-index="${idx}" data-reflect-field="monthlyPremium" value="${escapeHtml(premium)}" /></td>
        <td>${this.getPolicyAmountLabel(p) === 'כיסויים' ? `<textarea class="textarea mirrorsReflectTextarea" data-reflect-index="${idx}" data-reflect-field="covers">${escapeHtml(Array.isArray(p.covers) ? p.covers.join(', ') : amountValue)}</textarea>` : `<input class="input" data-reflect-index="${idx}" data-reflect-field="${this.getPolicyAmountLabel(p) === 'סכום פיצוי' ? 'compensation' : 'sumInsured'}" value="${escapeHtml(amountValue)}" />`}</td>
        <td><input class="input" data-reflect-index="${idx}" data-reflect-field="startDate" value="${escapeHtml(startDate)}" /></td>
      </tr>`;
    },
