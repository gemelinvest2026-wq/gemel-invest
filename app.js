...
init(){
    ...
    reset(){
      const make = (type, label) => ({
        ...
        data: {
          ...
          // step7 (case-level stored on primary insured)
          healthDecl: {
            version: "v1",
            updatedAt: null,
            // answers: { [qId]: { [insId]: { a: 'yes'|'no'|null, d: string } } }
            answers: {}
          }
        }
      });
    ...
    renderBody(){
      ...
      let ins = this.getActive();
      if(!ins){
        // Safety: if somehow insureds array got cleared, recreate base insured to prevent wizard crash
        this.reset();
        ins = this.getActive();
      }
      ...
      else if(this.step === 6) body = this.renderHealthDeclStep();
      ...
      if(this.step === 6) this.bindHealthDecl();
    },
...
// open health covers drawer (Health only)
    ...
    renderStep5(){
      ...
      const isRisk = (d.type === "ריסק" || d.type === "ריסק משכנתא");
    ...
    validateDraftPolicy(){
      ...
      if((d.type === "ריסק" || d.type === "ריסק משכנתא") && d.pledge){
...
// ---------- Step 6 ----------
    ...
    healthDeclQuestions(){
      // Universal base questions (30). Mapping to company/product questionnaires will be added next.
      return [
        { id:"q01", t:"האם אתה/את במעקב רפואי שוטף או בטיפול רפואי קבוע (כולל תרופות קבועות)?" },
        { id:"q02", t:"האם אובחנת אי פעם במחלה כרונית/מתמשכת כלשהי?" },
        { id:"q03", t:"האם במהלך 5 השנים האחרונות אושפזת או הופנית לאשפוז?" },
        { id:"q04", t:"האם עברת ניתוח או פעולה פולשנית (כולל אשפוז יום) במהלך 10 השנים האחרונות?" },
        { id:"q05", t:"האם בוצעו/ממתינות לך בדיקות, בירורים או טיפולים רפואיים שלא הושלמו?" },
        { id:"q06", t:"האם אובחנת או טופלת בבעיה בלב/כלי דם (כולל יתר לחץ דם, כאבים בחזה, הפרעות קצב)?" },
        { id:"q07", t:"האם אובחנת או טופלת בסוכרת/טרום-סוכרת/בעיות בבלוטת התריס/הפרעות הורמונליות?" },
        { id:"q08", t:"האם אובחנת או טופלת בבעיה בדרכי הנשימה/ריאות (אסתמה, COPD וכו')?" },
        { id:"q09", t:"האם אובחנת או טופלת בבעיה במערכת העיכול/כבד/לבלב (כולל צהבת, כבד שומני, קרוהן/קוליטיס)?" },
        { id:"q10", t:"האם אובחנת או טופלת בבעיה בכליות/דרכי השתן (כולל אבנים, דלקות חוזרות, אי-ספיקה)?" },
        { id:"q11", t:"האם אובחנת או טופלת בבעיה נוירולוגית (כולל אפילפסיה, טרשת, שבץ, מיגרנות חמורות)?" },
        { id:"q12", t:"האם אובחנת או טופלת בבעיה נפשית/התנהגותית (כולל חרדה/דיכאון, טיפול פסיכיאטרי, אשפוז)?" },
        { id:"q13", t:"האם אובחנת או טופלת בסרטן/גידול/ממצא טרום-סרטני?" },
        { id:"q14", t:"האם אובחנת או טופלת במחלה אוטואימונית/ראומטית (לופוס, פסוריאזיס מפרקי, RA וכו')?" },
        { id:"q15", t:"האם אובחנת או טופלת בבעיה אורטופדית משמעותית (גב/צוואר/ברכיים) או כאב כרוני/מגבלה תפקודית?" },
        { id:"q16", t:"האם אובחנת או טופלת בבעיה בעיניים/ראייה (כולל גלאוקומה, רשתית) או שמיעה משמעותית?" },
        { id:"q17", t:"האם אובחנת או טופלת בבעיה בעור משמעותית/כרונית (למשל פסוריאזיס, אקזמה קשה, נגעים)?" },
        { id:"q18", t:"האם אובחנת או טופלת בבעיה דם/קרישה (כולל אנמיה משמעותית, טרומבוזה, DVT)?" },
        { id:"q19", t:"האם אובחנת או טופלת בזיהומים כרוניים/משמעותיים (כולל HIV, הפטיטיס, שחפת)?" },
        { id:"q20", t:"האם אובחנת או טופלת בבעיה גינקולוגית/אורולוגית משמעותית?" },
        { id:"q21", t:"לנשים: האם הינך בהריון/בתהליכי בירור או טיפולי פוריות?" },
        { id:"q22", t:"האם יש לך נכות/מגבלה/אחוזי נכות או בקשה לנכות (ביטוח לאומי/חברות ביטוח)?" },
        { id:"q23", t:"האם חווית תאונה/חבלה עם השלכות רפואיות במהלך 5 השנים האחרונות?" },
        { id:"q24", t:"האם קיימת תביעה/הצעה/דחייה/החרגה/תוספת פרמיה בביטוחי בריאות/חיים בעבר?" },
        { id:"q25", t:"האם בוצעו/ממתינים לך מעקבים רפואיים מומלצים (לדוגמה מעקב מומחה/בדיקות שנתיות) בעקבות ממצא?" },
        { id:"q26", t:"האם אתה/את צורך/צרכת אלכוהול בכמות חריגה, או שימוש בסמים/חומרים (כולל קנאביס)?" },
        { id:"q27", t:"עישון: האם הנך מעשן/ת או עישנת בשנתיים האחרונות סיגריות/טבק/נרגילה/וייפ/סיגריה אלקטרונית או כל מוצר טבק אחר? האם שימוש בקנאביס בעישון?" },
        { id:"q28", t:"האם עברת בדיקות מעבדה/הדמיה עם ממצאים חריגים משמעותיים שלא נסגרו?" },
        { id:"q29", t:"האם קיימת בעיה רפואית אחרת שלא נשאלה כאן ושחשוב לדווח עליה?" },
        { id:"q30", t:"האם כל המידע שמסרת נכון, מלא ומדויק, וידוע לך שתשובות לא מלאות עשויות להשפיע על תנאי החיתום/כיסוי?" }
      ];
    },

    _healthDeclRoot(){
      const p = this.insureds?.[0];
      if(!p) return null;
      if(!p.data) p.data = {};
      if(!p.data.healthDecl) p.data.healthDecl = { version:"v1", updatedAt:null, answers:{} };
      if(!p.data.healthDecl.answers) p.data.healthDecl.answers = {};
      return p.data.healthDecl;
    },

    _hdEnsureQ(qId){
      const hd = this._healthDeclRoot();
      if(!hd) return;
      if(!hd.answers[qId]) hd.answers[qId] = {};
      this.insureds.forEach(ins => {
        if(!hd.answers[qId][ins.id]) hd.answers[qId][ins.id] = { a:null, d:"" };
      });
    },

    renderHealthDeclStep(){
      const qs = this.healthDeclQuestions();
      const hd = this._healthDeclRoot();
      if(!hd) return `<div class="lcWSection"><div class="lcWTitle">הצהרת בריאות</div><div class="muted">שגיאה: אין מבוטח ראשי</div></div>`;

      // compute companies/products selected (Step 5)
      const pickedCompanies = new Set();
      const pickedTypes = new Set();
      (this.newPolicies || []).forEach(p => {
        if(safeTrim(p.company)) pickedCompanies.add(safeTrim(p.company));
        if(safeTrim(p.type)) pickedTypes.add(safeTrim(p.type));
      });
      const compStr = Array.from(pickedCompanies).join(", ") || "—";
      const typeStr = Array.from(pickedTypes).join(", ") || "—";

      const tabs = this.insureds.map(ins => {
        const full = (safeTrim(ins.data?.firstName)+" "+safeTrim(ins.data?.lastName)).trim() || ins.label;
        return `<div class="hdIns" data-hd-ins="${escapeHtml(ins.id)}"><div class="hdIns__name">${escapeHtml(full)}</div><div class="hdIns__tag">${escapeHtml(ins.label)}</div></div>`;
      }).join("");

      const qBlocks = qs.map((q, idx) => {
        this._hdEnsureQ(q.id);
        const rowBtns = this.insureds.map(ins => {
          const cell = hd.answers?.[q.id]?.[ins.id] || { a:null, d:"" };
          const a = cell.a;
          const yesOn = a === "yes";
          const noOn = a === "no";
          return `
            <div class="hdAns" data-q="${escapeHtml(q.id)}" data-ins="${escapeHtml(ins.id)}">
              <div class="hdAns__who">${escapeHtml(ins.label)}</div>
              <div class="hdAns__btns">
                <button class="hdBtn ${yesOn?"is-on":""}" data-hd="yes" type="button">כן</button>
                <button class="hdBtn ${noOn?"is-on":""}" data-hd="no" type="button">לא</button>
                <button class="hdBtn hdBtn--na ${(a===null)?"is-on":""}" data-hd="na" type="button">לא נענָה</button>
              </div>
            </div>
          `;
        }).join("");
        ...
        const details = this.insureds.map(ins => {
          const cell = hd.answers?.[q.id]?.[ins.id] || { a:null, d:"" };
          if(cell.a !== "yes") return "";
          const placeholder = (q.id === "q27") ? "פרט/י סוג מוצר, כמות, תדירות ושנתיים אחרונות" : "פרט/י אבחנה, מתי, טיפול, מצב כיום";
          return `
            <div class="hdDet" data-det="${escapeHtml(q.id)}" data-ins="${escapeHtml(ins.id)}">
              <div class="hdDet__head">פירוט · ${escapeHtml(ins.label)}</div>
              <textarea class="input hdDet__ta" rows="2" placeholder="${escapeHtml(placeholder)}" data-hd-detail="${escapeHtml(q.id)}" data-hd-ins="${escapeHtml(ins.id)}">${escapeHtml(cell.d||"")}</textarea>
            </div>
          `;
        }).join("");

        const anyYes = this.insureds.some(ins => (hd.answers?.[q.id]?.[ins.id]?.a === "yes"));

        return `
          <div class="hdQ" data-hd-qwrap="${escapeHtml(q.id)}">
            <div class="hdQ__top">
              <div class="hdQ__num">${idx+1}</div>
              <div class="hdQ__text">${escapeHtml(q.t)}</div>
              <div class="hdQ__bulk">
                <button class="hdBulk" data-hd-bulk="no" data-q="${escapeHtml(q.id)}" type="button">לא לכולם</button>
                <button class="hdBulk" data-hd-bulk="yes" data-q="${escapeHtml(q.id)}" type="button">כן לכולם</button>
              </div>
            </div>
            <div class="hdQ__grid">${rowBtns}</div>
            <div class="hdQ__details ${anyYes?"is-open":""}">${details || ""}</div>
          </div>
        `;
      }).join("");
      ...
      return `
        <div class="lcWSection">
          ...
          <div class="muted small">בחרת בפוליסות חדשות: <b>${escapeHtml(typeStr)}</b> · חברות: <b>${escapeHtml(compStr)}</b></div>
          <div class="help">כל שאלה נשאלת פעם אחת, ומסמנים למי מהמבוטחים התשובה <b>כן/לא</b>. אם נבחר <b>כן</b> — נדרש פירוט.</div>
          <div class="divider"></div>
          <div class="hdInsRow">${tabs}</div>
        </div>

        <div class="lcWSection">
          <div class="lcWTitle">30 שאלות בסיס</div>
          <div class="muted small">מיפוי אוטומטי לשאלוני חברות לפי מוצר/חברה (כולל פתיחת השאלון הרפואי המדויק) יתווסף בשלב הבא.</div>
        </div>
    ...
    bindHealthDecl(){
      ...
      // bulk actions per question
      $$('[data-hd-bulk]', this.els.body).forEach(btn => {
        on(btn, 'click', () => {
          const qId = btn.getAttribute('data-q');
          const mode = btn.getAttribute('data-hd-bulk');
          if(!qId) return;
          this._hdEnsureQ(qId);
          this.insureds.forEach(ins => {
            hd.answers[qId][ins.id].a = (mode === 'yes') ? 'yes' : 'no';
            if(mode !== 'yes') hd.answers[qId][ins.id].d = '';
          });
          hd.updatedAt = nowISO();
          this.render();
        });
      ...
      // per insured answer buttons
      $$('[data-hd]', this.els.body).forEach(btn => {
        on(btn, 'click', () => {
          const wrap = btn.closest?.('.hdAns');
          if(!wrap) return;
          const qId = wrap.getAttribute('data-q');
          const insId = wrap.getAttribute('data-ins');
          const val = btn.getAttribute('data-hd');
          if(!qId || !insId || !val) return;
          this._hdEnsureQ(qId);
          if(val === 'na') {
            hd.answers[qId][insId].a = null;
            hd.answers[qId][insId].d = '';
          } else {
            hd.answers[qId][insId].a = (val === 'yes') ? 'yes' : 'no';
            if(val !== 'yes') hd.answers[qId][insId].d = '';
          }
          hd.updatedAt = nowISO();
          this.render();
        });
      ...
      // details typing
      $$('[data-hd-detail]', this.els.body).forEach(ta => {
        on(ta, 'input', () => {
          const qId = ta.getAttribute('data-hd-detail');
          const insId = ta.getAttribute('data-hd-ins');
          if(!qId || !insId) return;
          this._hdEnsureQ(qId);
          hd.answers[qId][insId].d = safeTrim(ta.value);
          hd.updatedAt = nowISO();
        });
      });
    },
    ...
    validateStep7(){
      const hd = this._healthDeclRoot();
      if(!hd) return { ok:false, msg:"חסר מבוטח ראשי" };
      const qs = this.healthDeclQuestions();
      for(const q of qs){
        this._hdEnsureQ(q.id);
        for(const ins of this.insureds){
          const cell = hd.answers?.[q.id]?.[ins.id];
          const a = cell?.a;
          if(a !== 'yes' && a !== 'no') {
            return { ok:false, msg:`חסר מענה (כן/לא) בשאלה ${qs.findIndex(x=>x.id===q.id)+1} עבור ${ins.label}` };
          }
          if(a === 'yes'){
            if(!safeTrim(cell?.d)){
              return { ok:false, msg:`נדרש פירוט בשאלה ${qs.findIndex(x=>x.id===q.id)+1} עבור ${ins.label}` };
            }
          }
        }
      }
    ...
    renderStep8(){
      ...
      return `
        <div class="lcWSection">
          ...
          <div class="divider"></div>
          <div class="help">בשלב הזה כבר יש לנו מבנה מלא של שלבים 1–7 (כולל הצהרת בריאות), טאבים למבוטחים, ולוגיקה של מעבר שלב לפי השלמת חובה.</div>
        </div>
    ...
    validateStep(stepId){
      ...
      // Step 6: health declaration (case-level)
