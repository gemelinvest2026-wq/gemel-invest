...
txt.textContent = "";
  ...
  const CustomersUI = {
    ...
    init(){
      ...
      on(this.els.editBtn, "click", async (ev) => {
        ev?.preventDefault?.();
        const rec = this.current();
        if(!rec) return;
        const opsStore = rec?.payload?.opsWorkbench && typeof rec.payload.opsWorkbench === "object" ? rec.payload.opsWorkbench : null;
        if(opsStore?.active && !Auth.isOps()){
          const agentName = safeTrim(Auth?.current?.name) || "נציג";
          const shouldSend = window.confirm("התיק בתהליך הקמה במחלקת התפעול. לחץ אישור כדי לשלוח תיקון לנציגת התפעול.");
          if(!shouldSend) return;
          const note = window.prompt("רשום כאן את התיקון הנדרש עבור מחלקת התפעול:", "");
          if(!safeTrim(note)) return;
          if(!Array.isArray(rec.payload.opsCorrections)) rec.payload.opsCorrections = [];
          rec.payload.opsCorrections.unshift({
            id: "corr_" + Date.now() + "_" + Math.random().toString(16).slice(2,6),
            text: safeTrim(note),
            createdAt: nowISO(),
            byName: agentName,
            byRole: safeTrim(Auth?.current?.role) || "agent",
            unread: true,
            handled: false
          });
          rec.updatedAt = nowISO();
          State.data.meta.updatedAt = nowISO();
          try { await App.persist("התיקון נשלח למחלקת התפעול"); } catch(_e) {}
          try { if(window.OpsWorkbenchUI?.isOpenFor?.(rec.id)) window.OpsWorkbenchUI.render(); } catch(_e) {}
          window.alert("התיקון נשלח למחלקת התפעול.");
          return;
        }
...
ההכנסה מעבודה שאתה צפוי להרוויח תעמוד על סך של ______ ₪ לחודש. ידוע לך כי בקרות מקרה הביטוח יוגבלו תגמולי הפיצוי החודשי להם תהיה זכאי לגובה שלא יעלה על 75% מהכנסתך מעבודה בפועל ב־12 החודשים שקדמו למועד קרות מקרה הביטוח, או בתקופה ממועד תחילת הביטוח ועד מועד קרות מקרה הביטוח, אם חלפו פחות מ־12 חודשים.`
  ...
  const ProcessesUI = {
    ...
    render(){
        ...
        return `<tr class="lcMyProcessesRow" data-tone="${escapeHtml(row.tone)}">
          ...
          <td>
            <div class="lcMyProcessesActions">
              ${Auth.isOps() ? `<button class="btn btn--primary" data-process-ops="${escapeHtml(rec.id)}" type="button">מסך תפעול</button>` : ``}
              <button class="btn ${Auth.isOps() ? '' : 'btn--primary'}" data-process-open="${escapeHtml(rec.id)}" type="button">פתח תהליך</button>
      ...
      UI.els.myProcessesTbody.querySelectorAll("[data-process-ops]").forEach(btn => {
        btn.onclick = () => OpsWorkbenchUI.open(btn.getAttribute("data-process-ops"));
      });
  ...
  const OpsWorkbenchUI = {
    customerId: null,
    els: {},
    _watchTimer: null,
    init(){
      if(document.getElementById('opsWorkbench')) return;
      document.body.insertAdjacentHTML('beforeend', `
        <div class="opsWorkbench" id="opsWorkbench" aria-hidden="true">
          <div class="opsWorkbench__backdrop" data-opswb-close></div>
          <div class="opsWorkbench__panel" role="dialog" aria-modal="true" aria-labelledby="opsWorkbenchTitle">
            <div class="opsWorkbench__head">
              <div>
                <div class="opsWorkbench__kicker">GEMEL INVEST • מחלקת תפעול</div>
                <div class="opsWorkbench__title" id="opsWorkbenchTitle">מסך תפעול</div>
                <div class="opsWorkbench__sub" id="opsWorkbenchSub">העתקה מהירה לשורנס</div>
              </div>
              <div class="opsWorkbench__headActions">
                <button class="opsWorkbench__mail" id="opsWorkbenchCorrectionsBtn" type="button">✉️ <span>תיקונים</span><em id="opsWorkbenchCorrectionsCount">0</em></button>
                <button class="opsWorkbench__close" type="button" data-opswb-close>סגור</button>
              </div>
            </div>
            <div class="opsWorkbench__body">
              <div class="opsWorkbench__content" id="opsWorkbenchContent"></div>
              <aside class="opsWorkbench__side">
                <div class="opsWorkbench__progressCard">
                  <div class="opsWorkbench__progressLabel">התקדמות העתקה</div>
                  <div class="opsWorkbench__progressBar"><span id="opsWorkbenchProgressBar"></span></div>
                  <div class="opsWorkbench__progressText" id="opsWorkbenchProgressText">0 / 0</div>
                </div>
                <div class="opsWorkbench__liveCard">
                  <div class="opsWorkbench__liveTitle">תיקונים מהנציג</div>
                  <div class="opsWorkbench__liveHint">ברגע שנציג שולח תיקון, הוא יופיע כאן ובמעטפה.</div>
                  <div class="opsWorkbench__liveList" id="opsWorkbenchLiveList"></div>
                </div>
              </aside>
            </div>
            <div class="opsWorkbench__drawer" id="opsWorkbenchDrawer" aria-hidden="true">
              <div class="opsWorkbench__drawerHead">
                <strong>תיקונים</strong>
                <button type="button" class="opsWorkbench__drawerClose" id="opsWorkbenchDrawerClose">סגור</button>
              </div>
              <div class="opsWorkbench__drawerList" id="opsWorkbenchDrawerList"></div>
            </div>
            <div class="opsWorkbench__toast" id="opsWorkbenchToast"></div>
          </div>
        </div>`);
      this.els.wrap = document.getElementById('opsWorkbench');
      this.els.content = document.getElementById('opsWorkbenchContent');
      this.els.sub = document.getElementById('opsWorkbenchSub');
      this.els.mailBtn = document.getElementById('opsWorkbenchCorrectionsBtn');
      this.els.count = document.getElementById('opsWorkbenchCorrectionsCount');
      this.els.liveList = document.getElementById('opsWorkbenchLiveList');
      this.els.drawer = document.getElementById('opsWorkbenchDrawer');
      this.els.drawerList = document.getElementById('opsWorkbenchDrawerList');
      this.els.drawerClose = document.getElementById('opsWorkbenchDrawerClose');
      this.els.toast = document.getElementById('opsWorkbenchToast');
      this.els.progressBar = document.getElementById('opsWorkbenchProgressBar');
      this.els.progressText = document.getElementById('opsWorkbenchProgressText');
      this.els.wrap.querySelectorAll('[data-opswb-close]').forEach(btn => btn.addEventListener('click', () => this.close()));
      this.els.mailBtn.addEventListener('click', () => this.toggleDrawer());
      this.els.drawerClose.addEventListener('click', () => this.closeDrawer());
      this.els.content.addEventListener('click', (ev) => {
        const btn = ev.target.closest('[data-copy-value]');
        if(!btn) return;
        this.copy(btn.getAttribute('data-copy-value') || '', btn);
      });
      this.els.drawerList.addEventListener('click', async (ev) => {
        const btn = ev.target.closest('[data-correction-handle]');
        if(!btn) return;
        const rec = this.current();
        if(!rec) return;
        const id = btn.getAttribute('data-correction-handle');
        const row = (Array.isArray(rec.payload.opsCorrections) ? rec.payload.opsCorrections : []).find(item => String(item.id) === String(id));
        if(!row) return;
        row.handled = true;
        row.unread = false;
        rec.updatedAt = nowISO();
        State.data.meta.updatedAt = nowISO();
        this.render();
        try { await App.persist('התיקון סומן כטופל'); } catch(_e) {}
      });
    },
    current(){
      return (State.data.customers || []).find(item => String(item.id) === String(this.customerId)) || null;
    },
    formatAt(v){
      if(!v) return '—';
      const d = new Date(v);
    ...
    isOpenFor(id){
      return this.customerId && String(this.customerId) === String(id) && this.els.wrap?.classList.contains('is-open');
    },
    ensureStore(rec){
      if(!rec.payload) rec.payload = {};
      if(!rec.payload.opsWorkbench || typeof rec.payload.opsWorkbench !== 'object') rec.payload.opsWorkbench = {};
      const store = rec.payload.opsWorkbench;
      if(!Array.isArray(store.copiedKeys)) store.copiedKeys = [];
      store.active = true;
      store.startedAt = store.startedAt || nowISO();
      store.startedBy = store.startedBy || safeTrim(Auth?.current?.name) || 'נציגת תפעול';
      return store;
    },
    getSections(rec){
      const payload = rec.payload || {};
      const insureds = Array.isArray(payload.insureds) && payload.insureds.length ? payload.insureds : (Array.isArray(payload?.operational?.insureds) ? payload.operational.insureds : []);
      const primary = insureds[0]?.data || payload.primary || {};
      const secondary = insureds.slice(1);
      const newPolicies = Array.isArray(payload.newPolicies) && payload.newPolicies.length ? payload.newPolicies : (Array.isArray(payload?.operational?.newPolicies) ? payload.operational.newPolicies : []);
      const payment = payload.payment || payload.payer || {};
      const sections = [];
      sections.push({ title:'פרטי מבוטח ראשי', rows:[
        ['שם מלא', rec.fullName || primary.fullName || ''],
        ['תעודת זהות', rec.idNumber || primary.idNumber || ''],
        ['טלפון', rec.phone || primary.phone || ''],
        ['אימייל', rec.email || primary.email || ''],
        ['עיר', primary.city || ''],
        ['רחוב', primary.street || ''],
        ['מספר בית', primary.houseNumber || ''],
        ['מיקוד', primary.zip || '']
      ].filter(r => safeTrim(r[1])) });
      secondary.forEach((ins, idx) => {
        const d = ins?.data || {};
        sections.push({ title:`מבוטח משני ${idx+1} - ${ins.label || d.fullName || ''}`.trim(), rows:[
          ['שם מלא', d.fullName || ''], ['תעודת זהות', d.idNumber || ''], ['טלפון', d.phone || ''], ['סוג מבוטח', ins.type || '']
        ].filter(r => safeTrim(r[1])) });
      });
      newPolicies.forEach((pol, idx) => {
        sections.push({ title:`פוליסה חדשה ${idx+1}`, rows:[
          ['חברה', pol.company || ''], ['מוצר', pol.product || ''], ['פרמיה חודשית', pol.monthlyPremium || pol.premium || ''], ['סכום ביטוח', pol.coverageAmount || pol.sumInsured || ''], ['תאריך התחלה', pol.startDate || '']
        ].filter(r => safeTrim(r[1])) });
      });
      sections.push({ title:'אמצעי תשלום', rows:[
        ['סוג תשלום', payment.method || payment.paymentMethod || ''], ['בנק', payment.bankName || ''], ['סניף', payment.branchNumber || ''], ['חשבון', payment.accountNumber || ''], ['בעל כרטיס', payment.cardHolderName || ''], ['4 ספרות אחרונות', payment.last4 || payment.cardLast4 || '']
      ].filter(r => safeTrim(r[1])) });
      return sections.filter(sec => Array.isArray(sec.rows) && sec.rows.length);
    },
    render(){
      const rec = this.current();
      if(!rec || !this.els.wrap) return;
      const store = this.ensureStore(rec);
      const sections = this.getSections(rec);
      const copied = new Set((store.copiedKeys || []).map(String));
      this.els.sub.textContent = `${safeTrim(rec.fullName || '')} • התיק בטיפול מחלקת התפעול`;
      let total = 0;
      let copiedCount = 0;
      this.els.content.innerHTML = sections.map((section, sIdx) => `
        <section class="opsWorkbenchCard">
          <div class="opsWorkbenchCard__head"><h3>${escapeHtml(section.title)}</h3></div>
          <div class="opsWorkbenchRows">
            ${section.rows.map((row, rIdx) => {
              const key = `${sIdx}:${rIdx}:${row[0]}`;
              total += 1;
              const isCopied = copied.has(key);
              if(isCopied) copiedCount += 1;
              return `<div class="opsWorkbenchRow">
                <div class="opsWorkbenchRow__meta"><span class="opsWorkbenchRow__label">${escapeHtml(row[0])}</span><strong class="opsWorkbenchRow__value">${escapeHtml(row[1])}</strong></div>
                <button class="opsWorkbenchCopyBtn ${isCopied ? 'is-copied' : ''}" type="button" data-copy-key="${escapeHtml(key)}" data-copy-value="${escapeHtml(String(row[1]))}">${isCopied ? 'הנתון הועתק' : 'העתק נתון'}</button>
              </div>`;
            }).join('')}
          </div>
        </section>`).join('') || `<div class="emptyState"><div class="emptyState__icon">📄</div><div class="emptyState__title">עדיין אין נתונים זמינים למסך תפעול</div></div>`;
      const corrections = Array.isArray(rec.payload.opsCorrections) ? rec.payload.opsCorrections : [];
      const unread = corrections.filter(item => item && item.unread && !item.handled).length;
      this.els.count.textContent = String(unread);
      this.els.mailBtn.classList.toggle('has-unread', unread > 0);
      this.els.liveList.innerHTML = corrections.length ? corrections.slice(0,4).map(item => `<div class="opsWorkbenchCorrection ${item.handled ? 'is-handled' : ''}"><strong>מהנציג: ${escapeHtml(item.byName || 'נציג')}</strong><span>${escapeHtml(item.text || '')}</span></div>`).join('') : `<div class="opsWorkbenchEmpty">עדיין לא נשלחו תיקונים מהנציג.</div>`;
      this.els.drawerList.innerHTML = corrections.length ? corrections.map(item => `<article class="opsWorkbenchCorrectionCard ${item.unread && !item.handled ? 'is-unread' : ''}"><div class="opsWorkbenchCorrectionCard__top"><strong>${escapeHtml(item.byName || 'נציג')}</strong><span>${escapeHtml(this.formatAt(item.createdAt))}</span></div><div class="opsWorkbenchCorrectionCard__text">${escapeHtml(item.text || '')}</div><div class="opsWorkbenchCorrectionCard__actions">${item.handled ? '<span class="opsWorkbenchHandled">טופל</span>' : `<button class="btn btn--primary" type="button" data-correction-handle="${escapeHtml(item.id)}">סמן כטופל</button>`}</div></article>`).join('') : `<div class="opsWorkbenchEmpty">אין כרגע תיקונים להצגה.</div>`;
      const pct = total ? Math.round((copiedCount / total) * 100) : 0;
      this.els.progressText.textContent = `${copiedCount} / ${total}`;
      this.els.progressBar.style.width = `${pct}%`;
    },
    async copy(value, btn){
      const rec = this.current();
      if(!rec) return;
      const store = this.ensureStore(rec);
      ...
      if(key && !store.copiedKeys.includes(key)) store.copiedKeys.push(key);
      rec.updatedAt = nowISO();
      State.data.meta.updatedAt = nowISO();
      btn.classList.add('is-copied');
      btn.textContent = 'הנתון הועתק';
      this.render();
    ...
    notifyNewCorrection(item){
      if(!this.els.toast) return;
      this.els.toast.textContent = `נכנס תיקון מהנציג: ${safeTrim(item?.byName) || 'נציג'}`;
      this.els.toast.classList.add('is-visible');
      window.clearTimeout(this._toastTimer);
      this._toastTimer = window.setTimeout(() => this.els.toast.classList.remove('is-visible'), 2600);
    },
    open(id){
      ...
      if(!rec) return;
      const store = this.ensureStore(rec);
      rec.status = safeTrim(rec.status) || 'בטיפול מחלקת תפעול';
      if(!safeTrim(rec.status).includes('תפעול')) rec.status = 'בטיפול מחלקת תפעול';
      rec.updatedAt = nowISO();
      State.data.meta.updatedAt = nowISO();
      this.els.wrap.classList.add('is-open');
      this.els.wrap.setAttribute('aria-hidden','false');
      document.body.style.overflow = 'hidden';
      this.closeDrawer();
      const prevUnread = this._lastUnread || 0;
      this.render();
      const currentUnread = (Array.isArray(rec.payload.opsCorrections) ? rec.payload.opsCorrections.filter(item => item && item.unread && !item.handled).length : 0);
      if(currentUnread > prevUnread){
        this.notifyNewCorrection((rec.payload.opsCorrections || [])[0]);
      }
...
const SystemRepairUI = {
  ...
  const App = {
    ...
    async reloadSessionState(){
      ...
      if (r.ok) {
        ...
        try { if (Auth.isAdmin()) UsersUI.render(); } catch(err) { console.error("USERS_RENDER_FAILED:", err); }
        if (Auth.current) {
          try { CustomersUI.render(); } catch(err) { console.error("CUSTOMERS_RENDER_FAILED:", err); }
          try { ProposalsUI.render(); } catch(err) { console.error("PROPOSALS_RENDER_FAILED:", err); }
          if (Auth.isOps()) {
            try { ProcessesUI.render(); } catch(err) { console.error("PROCESSES_RENDER_FAILED:", err); }
            try { OpsEventsUI.renderToolbarState(); OpsEventsUI.checkReminders(); } catch(_e) {}
          }
        }
      } else {
    ...
    async syncNow(){
      ...
      if (r.ok) {
        ...
        if (Auth.current) {
          try { CustomersUI.render(); } catch(err) { console.error("CUSTOMERS_RENDER_FAILED:", err); }
          try { ProposalsUI.render(); } catch(err) { console.error("PROPOSALS_RENDER_FAILED:", err); }
          if (Auth.isOps()) try { ProcessesUI.render(); } catch(err) { console.error("PROCESSES_RENDER_FAILED:", err); }
        }
      } else {
  ...
  // ---------- Start ----------
  function safeInitModule(name, initFn){
    try {
      if(typeof initFn === "function") initFn();
      return true;
    } catch(err) {
      try {
        console.error("INIT_MODULE_FAILED:", name, err);
        window.__startupErrors = Array.isArray(window.__startupErrors) ? window.__startupErrors : [];
        window.__startupErrors.push({ name, error: String(err?.message || err || "INIT_FAILED") });
      } catch(_e) {}
      return false;
    }
  }

  safeInitModule("UI", () => UI.init());
  safeInitModule("Auth", () => Auth.init());
  safeInitModule("ForgotPasswordUI", () => ForgotPasswordUI.init());
  safeInitModule("CustomersUI", () => CustomersUI.init());
  safeInitModule("CustomerEditUI", () => CustomerEditUI.init());
  safeInitModule("ArchiveCustomerUI", () => ArchiveCustomerUI.init());
  safeInitModule("MirrorsUI", () => MirrorsUI.init());
  safeInitModule("ProcessesUI", () => ProcessesUI.init());
  safeInitModule("OpsEventsUI", () => OpsEventsUI.init());
  safeInitModule("OpsWorkbenchUI", () => OpsWorkbenchUI.init());
  safeInitModule("Wizard", () => Wizard.init());
  safeInitModule("SystemRepairUI", () => SystemRepairUI.init());
  safeInitModule("SupportNoticeUI", () => SupportNoticeUI.init());
  safeInitModule("NewCustomerEntryUI", () => NewCustomerEntryUI.init());
  safeInitModule("LeadShellUI", () => LeadShellUI.init());
  safeInitModule("ChatUI", () => ChatUI.init());
  safeInitModule("InactivityGuard", () => InactivityGuard.init());
  safeInitModule("LiveRefresh", () => LiveRefresh.start());
  App._bootPromise = (async () => {
    try {
      return await App.boot();
    } catch(err) {
      try { console.error("APP_BOOT_FAILED:", err); } catch(_e) {}
      try {
        const startupErrors = Array.isArray(window.__startupErrors) ? window.__startupErrors : [];
        const summary = startupErrors.length ? (" | מודולים: " + startupErrors.map((item) => item?.name).filter(Boolean).join(", ")) : "";
        showLoginError("שגיאה זמנית בטעינת המערכת. רענן את העמוד ונסה שוב." + summary);
      } catch(_e) {}
      return null;
    }
  })();
