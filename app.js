...
// ---------- Step 6 ----------
    ...
    getPhoenixQuestionnaireCatalog(){
      const detailFields = [
        { key:'diagnosis', label:'אבחנה / מחלה / בדיקה', type:'text' },
        { key:'dates', label:'מועד התחלה / סיום', type:'text' },
      ...
      return {
        short_risk: {
          ...
          steps: [
            { key:'s2_treatment', text:'האם בשנה האחרונה טופלת או הומלץ על טיפול תרופתי יותר מ-3 שבועות?', fields:[
              { key:'medName', label:'שם התרופה', type:'text' },
              { key:'reason', label:'סיבת טיפול', type:'textarea' }
            ]},
            { key:'s3_tests', text:'האם בשנה האחרונה הומלץ לך או שהינך מועמד לביצוע בדיקה פולשנית, בדיקת הדמיה או ניתוח?', fields:[
              { key:'testType', label:'סוג בדיקה / ניתוח', type:'text' },
              { key:'date', label:'תאריך', type:'text' },
              { key:'hospitalDays', label:'משך אשפוז', type:'text' }
            ]},
            { key:'s4_smoking', text:'האם הינך מעשן או עישנת במהלך השנתיים האחרונות?', fields:[
              { key:'cigarettes', label:'כמות סיגריות', type:'text' },
              { key:'quitDate', label:'תאריך הפסקת עישון', type:'text' }
            ]},
            ...
            { key:'s12_disability', text:'האם נקבעה נכות או שהינך בהליך תביעת נכות?', questionnaireNos:['19'], fields:[
              { key:'percent', label:'דרגת נכות %', type:'text' },
              { key:'reason', label:'סיבת הנכות / ההליך', type:'textarea' }
            ]}
          ]
        },
        extended_risk: {
          ...
          steps: [
            { key:'e2_weight', text:'האם היו שינויים של למעלה מ-5 ק״ג במשקל בשנה האחרונה?', fields:[
              { key:'change', label:'כמה ק״ג ובאיזה כיוון', type:'text' },
              { key:'reason', label:'סיבה לשינוי', type:'textarea' }
            ]},
            { key:'e3_meds', text:'האם בשנה האחרונה נטלת תרופות שנרשמו על ידי רופא למשך יותר מ-3 שבועות או נוטל תרופות ללא מרשם באופן קבוע?', fields:[
              { key:'medName', label:'שם התרופה', type:'text' },
              { key:'reason', label:'סיבת טיפול', type:'textarea' }
            ]},
            { key:'e4_hospital', text:'האם אושפזת ב-5 השנים האחרונות כולל למטרת ניתוח?', questionnaireNos:['20'], fields:[
              { key:'hospitalType', label:'סוג אשפוז / ניתוח', type:'text' },
              { key:'date', label:'תאריך', type:'text' },
              { key:'days', label:'משך אשפוז', type:'text' }
            ]},
            { key:'e5_disability', text:'האם נקבעה לך נכות מכל סיבה שהיא או שהינך בתהליך קביעת נכות / תביעת נכות בשנתיים האחרונות?', questionnaireNos:['19'], fields:[
              { key:'percent', label:'דרגת נכות %', type:'text' },
              { key:'reason', label:'פירוט סיבה / הליך', type:'textarea' }
            ]},
            { key:'e6_tests', text:'האם עברת או הומלץ לך לעבור ב-5 השנים האחרונות בדיקות פולשניות או בדיקות הדמיה?', questionnaireNos:['18'], fields:[
              { key:'testType', label:'סוג בדיקה', type:'text' },
              { key:'date', label:'תאריך', type:'text' },
              { key:'result', label:'תוצאה / סטטוס', type:'textarea' }
            ]},
            { key:'e7_surgery', text:'האם עברת ניתוח או הומלץ על ניתוח בעתיד או השתלת איבר ב-10 השנים האחרונות?', questionnaireNos:['20'], fields:[
              { key:'procedure', label:'איזה ניתוח / השתלה', type:'text' },
              { key:'date', label:'תאריך', type:'text' },
              { key:'status', label:'מצב כיום / מה הומלץ', type:'textarea' }
            ]},
            { key:'e8_smoking', text:'האם הינך מעשן או עישנת במהלך השנתיים האחרונות?', fields:[
              { key:'cigarettes', label:'כמות סיגריות', type:'text' },
              { key:'quitDate', label:'תאריך הפסקת עישון', type:'text' }
            ]},
            { key:'e9_drugs', text:'האם השתמשת אי פעם או שהינך משתמש בסמים מכל סוג שהוא?', fields:[
              { key:'drugType', label:'סוג', type:'text' },
              { key:'freq', label:'תדירות', type:'text' },
              { key:'stopDate', label:'מועד הפסקה', type:'text' }
            ]},
            { key:'e10_alcohol', text:'האם הינך צורך או צרכת בעבר יותר מ-14 כוסות / פחיות משקאות חריפים בשבוע?', fields:[
              { key:'amount', label:'כמות שבועית', type:'text' },
        ...
        full_health: {
          ...
          steps: [
            { key:'fh_smoking', text:'האם הנך מעשן או עישנת בשנתיים האחרונות, לרבות סיגריה אלקטרונית ו/או נרגילה?', fields:[
              { key:'cigarettes', label:'כמות סיגריות ליום', type:'text' }
            ]},
            ...
            { key:'fh_drugs', text:'האם הינך צורך כעת או צרכת בעבר סמים מסוג כלשהו?', fields:[
              { key:'drugType', label:'סוג', type:'text' },
              { key:'freq', label:'תדירות', type:'text' },
              { key:'stopDate', label:'מועד הפסקה', type:'text' }
            ]},
            { key:'fh_alcohol', text:'האם הינך צורך או צרכת בעבר באופן קבוע יותר מ-2 כוסות משקה אלכוהולי ליום?', fields:[
              { key:'amount', label:'כמות יומית', type:'text' },
              { key:'details', label:'פירוט', type:'textarea' }
            ]},
            ...
            { key:'fh_premature', text:'לילדים עד גיל שנה – האם נולד פג?', fields:[
              { key:'week', label:'שבוע לידה', type:'text' },
              { key:'details', label:'פירוט מצב בלידה / אשפוז', type:'textarea' }
            ]},
            { key:'fh_congenital', text:'האם קיימים מומים מולדים, עיכוב התפתחותי או אבחנה בילדות?', fields: detailFields },
            { key:'fh_additional_tests', text:'האם עברת או הומלץ לך לעבור בדיקות פולשניות / הדמיה ב-5 השנים האחרונות?', questionnaireNos:['18'], fields:[
              { key:'testType', label:'סוג בדיקה', type:'text' },
              { key:'date', label:'תאריך', type:'text' },
              { key:'result', label:'תוצאה / סטטוס', type:'textarea' }
            ]},
            { key:'fh_surgery', text:'האם אושפזת, עברת ניתוח או הומלץ על ניתוח עתידי?', questionnaireNos:['20'], fields:[
              { key:'procedure', label:'איזה אשפוז / ניתוח', type:'text' },
              { key:'date', label:'תאריך', type:'text' },
              { key:'status', label:'מצב כיום', type:'textarea' }
            ]},
            { key:'fh_meds', text:'האם הינך נוטל או הומלץ לך ליטול תרופות באופן קבוע ב-3 השנים האחרונות?', fields:[
              { key:'medName', label:'שם התרופה', type:'text' },
              { key:'reason', label:'סיבת טיפול', type:'textarea' }
            ]},
            { key:'fh_disability', text:'האם נקבעה לך נכות זמנית / צמיתה או שהינך בתהליך קביעת נכות?', questionnaireNos:['19'], fields:[
              { key:'percent', label:'דרגת נכות %', type:'text' },
              { key:'reason', label:'פירוט סיבה / תביעה', type:'textarea' }
            ]}
          ]
        },
        critical_illness: {
          ...
          steps: [
            { key:'ci_smoking', text:'האם הנך מעשן או עישנת בשנתיים האחרונות?', fields:[
              { key:'cigarettes', label:'כמות סיגריות ליום', type:'text' }
            ]},
            ...
            { key:'ci_tests', text:'האם עברת או הומלץ לך לעבור בדיקות פולשניות / הדמיה או בדיקות לגילוי מוקדם של סרטן ב-5 השנים האחרונות?', questionnaireNos:['18'], fields:[
              { key:'testType', label:'איזו בדיקה', type:'text' },
            ...
            { key:'ci_heightweight', text:'האם יש ממצא חריג בגובה / משקל או BMI שדורש פירוט?', fields:[
              { key:'height', label:'גובה', type:'text' },
              { key:'weight', label:'משקל', type:'text' },
              { key:'details', label:'פירוט', type:'textarea' }
            ]},
            { key:'ci_alcohol', text:'האם הינך צורך באופן קבוע יותר מ-2 כוסות משקה אלכוהולי ליום?', fields:[
              { key:'amount', label:'כמות יומית', type:'text' },
              { key:'details', label:'פירוט', type:'textarea' }
            ]},
            { key:'ci_hospital', text:'האם ב-5 השנים האחרונות אושפזת, עברת ניתוח או הומלץ לך לעבור ניתוח עתידי?', questionnaireNos:['20'], fields:[
              { key:'procedure', label:'איזה ניתוח / אשפוז', type:'text' },
              { key:'date', label:'מתי', type:'text' },
              { key:'status', label:'מצב כיום', type:'textarea' }
            ]},
            { key:'ci_meds', text:'האם הינך נוטל או הומלץ לך ליטול תרופות באופן קבוע בשלוש השנים האחרונות?', fields:[
              { key:'medName', label:'שם התרופה', type:'text' },
              { key:'reason', label:'סיבת טיפול', type:'textarea' }
            ]},
    ...
    getPhoenixDetailedQuestionnaires(){
      return {
        '5': { title:'שאלון יתר שומנים בדם (5)', bullets:['ערך ממוצע / אחרון של כולסטרול.'] },
        '6': { title:'שאלון סוכרת (6)', bullets:['סוג סוכרת, מועד אבחנה, טיפול, פגיעה באיברי מטרה, HbA1c ותוצאות בשתן.'] },
        '8': { title:'שאלון אפילפסיה (8)', bullets:['סוג אפילפסיה, טיפול, תדירות התקפים ומועד התקף אחרון.'] },
        '9': { title:'שאלון דרכי עיכול (9)', bullets:['אבחנה, טיפול ומעורבות מחוץ למעי.'] },
        '10': { title:'שאלון כבד וצהבת / הפטיטיס (10)', bullets:['אבחנה, תפקודי כבד ותיעוד רופא / ביופסיה אם קיימים.'] },
        '12': { title:'שאלון עמוד שדרה (12)', bullets:['אבחנה ומיקום בעמוד השדרה.'] },
        '13': { title:'שאלון שלד (13)', bullets:['אבחנה, מיקום פגיעה, צד, מועד, ניתוח ומגבלה תפקודית.'] },
        '14': { title:'שאלון מפרקים ומחלות ראומטולוגיות (14)', bullets:['אבחנה, טיפול, פגיעה כלייתית / חלבון בשתן וסיבוכים.'] },
        '15': { title:'שאלון מחלות נפש (15)', bullets:['אבחנה, טיפול, נכות, אשפוז, ניסיונות אובדניים וביקור פסיכיאטר.'] },
        '16': { title:'שאלון דרכי נשימה (16)', bullets:['אבחנה, תכיפות טיפול, פרדניזון, אשפוז בשנה האחרונה ותפקודי ריאות לפי הצורך.'] },
        '17': { title:'שאלון אונקולוגי (17)', bullets:['אבחנה, מועד אבחנה, טיפול, תום טיפול, מצב עדכני, Stage / Grade ולפי סוג סרטן גם שדות נוספים.'] },
        '18': { title:'שאלון בדיקות / הדמיה (18)', bullets:['איזו בדיקה, מועד ותוצאה / המשך בירור.'] },
        '19': { title:'שאלון נכות (19)', bullets:['דרגת נכות, סיבה ונכות קיימת / תביעה.'] },
        '20': { title:'שאלון אשפוז / ניתוח (20)', bullets:['סוג אשפוז / ניתוח, תאריך, משך אשפוז והמצב כיום.'] },
        '22': { title:'שאלון היסטוריה משפחתית (22)', bullets:['איזה קרוב, איזו מחלה ובאיזה גיל אובחן.'] }
      };
    },

    getPhoenixPolicyDeclaration(policy, ins){
      const company = safeTrim(policy?.company);
      const type = safeTrim(policy?.type);
      if(company !== 'הפניקס' || !type) return null;
      const catalog = this.getPhoenixQuestionnaireCatalog();
      if(type === 'בריאות'){
        return { key:'phoenix_full_health', title:catalog.full_health.title, sourceLabel:catalog.full_health.sourceLabel, steps:catalog.full_health.steps, policyId:policy.id, policyLabel:this.getPolicyLabel(policy) };
      }
      if(type === 'מחלות קשות' || type === 'סרטן'){
        return { key:'phoenix_critical', title:catalog.critical_illness.title, sourceLabel:catalog.critical_illness.sourceLabel, steps:catalog.critical_illness.steps, policyId:policy.id, policyLabel:this.getPolicyLabel(policy) };
      }
      if(type === 'ריסק' || type === 'ריסק משכנתא'){
        const age = this.calcAge(ins?.data?.birthDate);
        const sum = this.parseMoneyNumber(policy?.sumInsured);
        const isShort = age !== null && age <= 55 && sum !== null && sum <= 2000000;
        const src = isShort ? catalog.short_risk : catalog.extended_risk;
        const ruleLabel = isShort ? 'נשלפה מקוצרת לפי גיל עד 55 וסכום עד 2,000,000' : 'נשלפה מורחבת לפי גיל מעל 55 או סכום מעל 2,000,000 או כאשר חסר נתון';
        return { key:isShort ? 'phoenix_risk_short' : 'phoenix_risk_extended', title:src.title, sourceLabel:src.sourceLabel, ruleLabel, steps:src.steps, policyId:policy.id, policyLabel:this.getPolicyLabel(policy) };
      }
      return null;
    },
    ...
    getHealthDeclarations(){
      ...
      this.insureds.forEach(ins => {
        const policies = this.getHealthPoliciesForInsured(ins);
        policies.forEach(policy => {
          const decl = this.getPhoenixPolicyDeclaration(policy, ins);
          if(!decl) return;
          const seenKey = `${decl.key}::${ins.id}`;
          if(seen.has(seenKey)) return;
          seen.add(seenKey);
          out.push({ ...decl, insuredId: ins.id, insuredLabel: ins.label, insuredType: ins.type });
        });
      });
      return out;
    },
    ...
    getHealthQuestionsFiltered(){
      const declarations = this.getHealthDeclarations();
      return declarations.map(dec => ({
        key: `${dec.key}__${dec.insuredId}`,
        title: `${dec.title} — ${dec.insuredLabel}`,
        summary: `${dec.policyLabel}${dec.ruleLabel ? ` · ${dec.ruleLabel}` : ''}`,
        declaration: dec,
        questions: (dec.steps || []).map(step => ({
          ...step,
          key: `${dec.key}__${dec.insuredId}__${step.key}`,
          applicableInsuredIds: [dec.insuredId],
          declarationKey: dec.key,
          declarationTitle: dec.title,
          policyLabel: dec.policyLabel,
          sourceLabel: dec.sourceLabel,
          ruleLabel: dec.ruleLabel || '',
          questionnaireNos: Array.isArray(step.questionnaireNos) ? step.questionnaireNos.slice() : []
        }))
      }));
    },
    ...
    getHealthQuestionList(){
      ...
      const list = [];
      cats.forEach(cat => {
        (cat.questions || []).forEach(q => list.push({ catKey: cat.key, catTitle: cat.title, catSummary: cat.summary || '', question: q, declaration: cat.declaration }));
      });
      return list;
    },
    ...
    getHealthCategoryStatus(cat){
      ...
      let yes = 0, pending = 0;
      questions.forEach(q => {
        (q.applicableInsuredIds || []).forEach(insId => {
          const r = this.getHealthResponse(q.key, insId);
          if(r.answer === 'yes'){
            yes += 1;
    ...
    getInsuredHealthStatus(ins){
      const list = this.getHealthQuestionList().filter(item => (item.question.applicableInsuredIds || []).includes(ins.id));
      let yes = 0, pending = 0;
      list.forEach(item => {
      ...
      if(!list.length) return { cls:'muted', text:'לא רלוונטי בשלב זה', icon:'•' };
    ...
    getHealthQuestionRequirements(question){
      const out = [];
      if(question.ruleLabel) out.push({ company:'מנוע בחירה', items:[question.ruleLabel] });
      if(question.sourceLabel) out.push({ company:'טופס בסיס', items:[question.sourceLabel] });
      const detailed = this.getPhoenixDetailedQuestionnaires();
      (question.questionnaireNos || []).forEach(no => {
        const q = detailed[String(no)];
        if(q) out.push({ company:`שאלון ${no}`, items:[q.title].concat(q.bullets || []) });
      });
      return out;
    },
    ...
    renderHealthSidebar(currentItem){
      ...
      this.insureds.forEach(ins => {
        ...
        this.getHealthQuestionList().forEach(item => {
          if(!(item.question.applicableInsuredIds || []).includes(ins.id)) return;
          const r = this.getHealthResponse(item.question.key, ins.id);
        ...
        if(findings.some(x => !x.saved)) alerts.push(`<div class="lcHAlertItem warn"><strong>${escapeHtml(ins.label)}</strong><span>יש פירוט פתוח שטרם נשמר</span></div>`);
        findings.slice(0, 3).forEach(f => alerts.push(`<div class="lcHAlertItem"><strong>${escapeHtml(ins.label)}</strong><span>${escapeHtml(f.q.text)}</span></div>`));
      });
    ...
    renderHealthSummary(){
      const declarations = this.getHealthDeclarations();
      const topLabels = declarations.map(d => `${d.insuredLabel} · ${d.sourceLabel}`).filter(Boolean);
      ...
      return `<div class="lcHLayout"><div class="lcHMain"><div class="lcHFinishHero">
        <div class="lcHFinishHero__kicker">שלב 8 · הפניקס</div>
        <div class="lcHFinishHero__title">סיכום הצהרות הבריאות</div>
        <div class="lcHFinishHero__text">המערכת בנתה את ההצהרה לפי החברה, המוצר, גיל המבוטח וסכום הביטוח כאשר מדובר בריסק.</div>
        <div class="lcHCompanies">${topLabels.map(c => `<span class="lcHChip lcHChip--top">${escapeHtml(c)}</span>`).join('')}</div>
      </div>
    ...
    renderStep8(){
      ...
      const list = this.getHealthQuestionList();
      const store = this.getHealthStore();
      if(!declarations.length || !list.length){
        return `<div class="lcHealthEmpty"><div class="lcHealthEmpty__icon">🩺</div><div class="lcHealthEmpty__title">הצהרת בריאות</div><div class="lcHealthEmpty__text">כדי להציג שלב זה יש לבחור בשלב 5 פוליסה חדשה של הפניקס עם מוצר נתמך. בריסק המערכת מזהה אוטומטית אם לשלוף טופס מקוצר או מורחב לפי גיל המבוטח וסכום הביטוח.</div></div>`;
      }
      ...
      const applicable = this.insureds.filter(ins => (q.applicableInsuredIds || []).includes(ins.id));
      const reqs = this.getHealthQuestionRequirements(q);
      const matrix = applicable.map(ins => {
        ...
        <div class="lcHMain">
          <div class="lcHHeroCard">
            <div class="lcHHeroCard__top">
              <div>
                <div class="lcHHeroCard__kicker">שלב 8 · הצהרת בריאות דינמית</div>
          ...
          <div class="lcHQuestionCard">
            <div class="lcHQuestionCard__head">
              <div>
                <div class="lcHQuestionCard__eyebrow">${escapeHtml(q.sourceLabel || 'הפניקס')}</div>
                <div class="lcHQuestionCard__title">${escapeHtml(q.text)}</div>
                <div class="lcHQuestionCard__sub">${escapeHtml(q.policyLabel || '')}</div>
              </div>
              <div class="lcHCompanies"><span class="lcHChip">${escapeHtml(q.declarationTitle || 'הפניקס')}</span>${q.ruleLabel ? `<span class="lcHChip">${escapeHtml(q.ruleLabel)}</span>` : ''}</div>
            </div>
            <div class="lcHQuestionCard__body">${matrix}</div>
            ${reqs.length ? `<div class="lcHRequirements"><div class="lcHRequirements__title">שאלון המשך רלוונטי</div>${reqs.map(r => `<div class="lcHReqBlock"><div class="lcHReqBlock__name">${escapeHtml(r.company)}</div><ul>${r.items.map(x => `<li>${escapeHtml(x)}</li>`).join('')}</ul></div>`).join('')}</div>` : ''}
            <div class="lcHNavRow">
    ...
    bindHealthInputs(){
      ...
      $$('[data-hans]', this.els.body).forEach(btn => {
        on(btn, 'click', () => {
          ...
          if(ans === 'no') this.setHealthResponse(qKey, insId, { answer:'no', fields:{}, saved:false });
          else {
            const prev = this.getHealthResponse(qKey, insId);
      ...
      $$('[data-hnav]', this.els.body).forEach(btn => on(btn, 'click', () => {
    ...
    getHealthBlockingIssue(){
      const list = this.getHealthQuestionList();
      if(!list.length) return { ok:false, msg:'אין שאלות הצהרת בריאות להצגה. בחר פוליסה רלוונטית של הפניקס בשלב 5.' };
      for(const item of list){
        const applicable = this.insureds.filter(ins => (item.question.applicableInsuredIds || []).includes(ins.id));
        for(const ins of applicable){
          const r = this.getHealthResponse(item.question.key, ins.id);
          if(r.answer !== 'yes' && r.answer !== 'no') return { ok:false, msg:`חסרה תשובה בהצהרת הבריאות עבור ${ins.label}` };
          if(r.answer === 'yes'){
            if(!this.validateHealthDetail(item.question, ins.id)) return { ok:false, msg:`יש להשלים את כל שדות התת־שאלון עבור ${ins.label}` };
            if(!r.saved) return { ok:false, msg:`יש לשמור את פירוט השאלה עבור ${ins.label}` };
          }
        }
      }
      return { ok:true };
