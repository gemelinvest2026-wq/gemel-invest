...
(() => {
  ...
  const BUILD = "20260405-phoenix-health-v41";
  ...
  const CustomerEditUI = {
    ...
    defaultPrimary(){
      return {
        ...
        occupation:"", clinic:"", shaban:"", communicationPreference:"email",
        payerChoice:"insured", selectedPayerId:"", paymentMethod:"cc",
        externalPayer:{ relation:"", firstName:"", lastName:"", idNumber:"", birthDate:"", phone:"", businessName:"", street:"", houseNumber:"", city:"", zip:"" },
        cc:{ holderName:"", holderId:"", cardNumber:"", exp:"" },
        ho:{ account:"", branch:"", bankName:"", bankNo:"", collectionDay:"10" },
        policyPayers:{},
    ...
    defaultNewPolicy(insuredId=""){
      return {
        ...
        healthCovers:[],
        phoenixHealth:{
          policyTrack:"basic_bundle",
          agentName:"", agentNumber:"", businessManager:"", proposalNumber:"",
          hasBasicPolicy:"", wantsBasicCompletion:"", age60Exception:false,
          existingHealthPolicy:"", existingHealthAction:"", existingCriticalIllnessPolicy:"", existingCriticalIllnessAction:"", cancellationMethod:"agent",
          ambulatoryPackage:"", serviceBasket:[],
          declarations:{ needsFit:true, fitCopyGiven:true, materialGiven:true, noDoubleInsurance:true, privacyConsent:true, medicalWaiver:true, marketingOptOut:false },
          requestedStartDate:""
        }
      };
    },
    ...
    buildDraft(rec){
      ...
      newPolicies = newPolicies.map((policy) => {
        ...
        next.phoenixHealth = Object.assign({}, this.deepClone(this.defaultNewPolicy().phoenixHealth), this.deepClone(next.phoenixHealth || {}));
        next.phoenixHealth.declarations = Object.assign({}, this.defaultNewPolicy().phoenixHealth.declarations, this.deepClone(next.phoenixHealth?.declarations || {}));
        next.phoenixHealth.serviceBasket = Array.isArray(next.phoenixHealth.serviceBasket) ? next.phoenixHealth.serviceBasket.filter(Boolean) : [];
        return next;
      });
...
// open health covers drawer (Health only)
    ...
    getStep1Questions(ins){
      ...
      return [
        ...
        },
        {
          ...
          fields: isChild ? ['birthDate','gender'] : ['birthDate','gender','maritalStatus','occupation','clinic','shaban'],
          requiredMsg: isChild ? 'נא להשלים תאריך לידה ומין לפני שממשיכים' : 'נא להשלים תאריך לידה, מין, מצב משפחתי, עיסוק, קופת חולים ושב״ן לפני שממשיכים',
          render:() => `
            <div class="lcStep1Group">
                ...
                ${!isChild ? `
                  ...
                  <div class="field">
                    ...
                    <select class="input" data-bind="clinic">
                      ...
                      ${this.clinics.map((clinic) => `<option value="${escapeHtml(clinic)}"${d.clinic===clinic?" selected":""}>${escapeHtml(clinic)}</option>`).join('')}
                    </select>
                  </div>
                  <div class="field">
                    <label class="label">שב״ן</label>
                    <select class="input" data-bind="shaban">
                      <option value="">בחר…</option>
                      ${(this.shabanMap[d.clinic] || []).map((plan) => `<option value="${escapeHtml(plan)}"${d.shaban===plan?" selected":""}>${escapeHtml(plan)}</option>`).join('')}
                    </select>
                  </div>
        ...
        },
        {
          ...
          fields: isChild ? [] : ['phone','email','city','street','houseNumber','communicationPreference'],
          requiredMsg:'נא להשלים טלפון, אימייל, עיר, כתובת, מספר בית ואופן קבלת מסמכים לפני שממשיכים',
          render:() => isChild
            ? `
              <div class="lcStep1Group">
                <div class="lcStep1Group__grid lcStep1Group__grid--3">
            ...
            : `
              <div class="lcStep1Group">
                <div class="lcStep1Group__grid lcStep1Group__grid--3">
                  ...
                  ${this.fieldText('דוא״ל','email', d.email, 'email')}
                  <div class="field">
                    <label class="label">אופן קבלת מסמכים</label>
                    <select class="input" data-bind="communicationPreference">
                      <option value="">בחר…</option>
                      <option value="email" ${d.communicationPreference==='email'?'selected':''}>דואר אלקטרוני</option>
                      <option value="post" ${d.communicationPreference==='post'?'selected':''}>דואר ישראל</option>
                    </select>
                  </div>
    ...
    getStep1SummaryValue(ins, q){
      ...
      switch(q?.key){
        ...
        case 'personalDetails': {
          ...
          if(ins.type !== 'child' && safeTrim(d.clinic)) parts.push(safeTrim(d.clinic));
          if(ins.type !== 'child' && safeTrim(d.shaban)) parts.push(safeTrim(d.shaban));
          return parts.join(' · ');
        }
        case 'contactDetails':
          ...
          return [safeTrim(d.phone), safeTrim(d.email), safeTrim(d.communicationPreference === 'post' ? 'דואר ישראל' : (d.communicationPreference === 'email' ? 'דוא״ל' : '')), safeTrim(d.city), safeTrim(d.street), safeTrim(d.houseNumber), safeTrim(d.zip)].filter(Boolean).join(' · ');
        default:
          return '';
      }
    },
    ...
    addDraftPolicy(){
      ...
      const p = {
        ...
        phoenixHealth: this.deepClone(d.phoenixHealth || this.defaultNewPolicy().phoenixHealth)
      };
      ...
      this.policyDraft.phoenixHealth = this.deepClone(this.defaultNewPolicy().phoenixHealth);
    ...
    startEditNewPolicy(pid){
      ...
      this.policyDraft = {
        ...
        phoenixHealth: this.deepClone(p.phoenixHealth || this.defaultNewPolicy().phoenixHealth)
      };
      ...
      setTimeout(() => {
        ...
        $$("[data-pdraft]", this.els.body).forEach(el => {
          on(el, "input", () => {
            ...
            if(!k) return;
            if(el.type === "checkbox") {
              if(String(k).includes('.')) this.setPath(this.policyDraft, k, !!el.checked);
              else this.policyDraft[k] = !!el.checked;
            } else {
              if(String(k).includes('.')) this.setPath(this.policyDraft, k, el.value);
        ...
        $$('[data-ph-track]', this.els.body).forEach((btn) => {
          on(btn, 'click', () => {
            this.ensurePolicyDraft();
            if(!this.policyDraft) return;
            this.policyDraft.phoenixHealth = this.policyDraft.phoenixHealth || this.defaultNewPolicy().phoenixHealth;
            this.policyDraft.phoenixHealth.policyTrack = safeTrim(btn.getAttribute('data-ph-track'));
            this.render();
          });
        });
        $$('[data-ph-service]', this.els.body).forEach((el) => {
          on(el, 'change', () => {
            this.ensurePolicyDraft();
            if(!this.policyDraft) return;
            this.togglePhoenixService(this.policyDraft, safeTrim(el.getAttribute('data-ph-service')), !!el.checked);
            this.setHint('עודכנו כתבי השירות של פניקס');
          });
        });
        $$('[data-pdraft-check]', this.els.body).forEach((el) => {
          on(el, 'change', () => {
            this.ensurePolicyDraft();
            const path = safeTrim(el.getAttribute('data-pdraft-check'));
            if(!path || !this.policyDraft) return;
            this.setPath(this.policyDraft, path, !!el.checked);
            this.setHint('עודכן שדה פניקס');
          });
        });
    ...
    isPhoenixHealthPolicy(policy={}){
      return safeTrim(policy?.company) === "הפניקס" && safeTrim(policy?.type) === "בריאות";
    },

    getPhoenixHealthTracks(){
      return [
        { value:"basic_bundle", label:"פוליסת בריאות בסיסית מלאה", note:'השתלות + תרופות + ניתוחים וטיפולים מחליפי ניתוח בחו"ל' },
        { value:"transplants_only", label:'השתלות וטיפולים מיוחדים מחו"ל', note:"9922" },
        { value:"drugs_only", label:"תרופות מחוץ לסל שירותי הבריאות", note:"9921" },
        { value:"abroad_surgery_only", label:'ניתוחים וטיפולים מחליפי ניתוח בחו"ל', note:"9919" },
        { value:"israel_meshalem_5000", label:'משלים שב"ן 5,000 ₪', note:"9920" },
        { value:"israel_meshalem", label:'משלים שב"ן ללא השתתפות עצמית', note:"9917" },
        { value:"israel_first_shekel", label:"ניתוחים בישראל מהשקל הראשון", note:"9918" },
        { value:"ambulatory_consult", label:"ייעוץ ובדיקות", note:"9916" },
        { value:"ambulatory_diagnosis", label:"אבחון רפואי מהיר", note:"9924" },
        { value:"ambulatory_combo", label:"ייעוץ ובדיקות + אבחון מהיר", note:"9981" },
        { value:"ambulatory_support", label:"ליווי רפואי וטיפולים אגב אירוע רפואי", note:"9928" }
      ];
    },

    getPhoenixServiceOptions(){
      return [
        "רפואה משלימה",
    ...
    getPhoenixTrackRequiresBasic(track){
      return ["israel_meshalem_5000","israel_meshalem","israel_first_shekel","ambulatory_consult","ambulatory_diagnosis","ambulatory_combo","ambulatory_support"].includes(safeTrim(track));
    },

    getPhoenixTrackIsAmbulatory(track){
      return ["ambulatory_consult","ambulatory_diagnosis","ambulatory_combo","ambulatory_support"].includes(safeTrim(track));
    },

    togglePhoenixService(policy, value, enabled){
      if(!policy || !this.isPhoenixHealthPolicy(policy)) return;
      const store = policy.phoenixHealth && typeof policy.phoenixHealth === 'object' ? policy.phoenixHealth : (policy.phoenixHealth = this.defaultNewPolicy().phoenixHealth);
      const current = Array.isArray(store.serviceBasket) ? store.serviceBasket.filter(Boolean) : [];
      const next = enabled ? Array.from(new Set([...current, value])) : current.filter((item) => item !== value);
      store.serviceBasket = next;
    },

    renderPhoenixHealthBuilder(policy){
      const d = policy?.phoenixHealth && typeof policy.phoenixHealth === 'object' ? policy.phoenixHealth : (policy.phoenixHealth = this.defaultNewPolicy().phoenixHealth);
      const tracks = this.getPhoenixHealthTracks();
      const serviceOptions = this.getPhoenixServiceOptions();
      const declarations = d.declarations && typeof d.declarations === 'object' ? d.declarations : (d.declarations = this.defaultNewPolicy().phoenixHealth.declarations);
      const chosenTrack = safeTrim(d.policyTrack) || 'basic_bundle';
      const ambulatoryChosen = this.getPhoenixTrackIsAmbulatory(chosenTrack);
      const requiresBasic = this.getPhoenixTrackRequiresBasic(chosenTrack);
      const basicQuestionCls = requiresBasic ? '' : ' is-muted';
      return `
        <div class="lcPhoenixBox">
          <div class="lcPhoenixBox__head">
            <div>
              <div class="lcPhoenixBox__kicker">הפניקס · מצב בריאות</div>
              <div class="lcPhoenixBox__title">שדות משלימים לפי טופס ההצעה המקורי</div>
            </div>
            <div class="lcPhoenixBox__badge">V41</div>
          </div>

          <div class="lcPhoenixGrid lcPhoenixGrid--meta">
            <div class="field"><label class="label">שם סוכן</label><input class="input" data-pdraft="phoenixHealth.agentName" value="${escapeHtml(d.agentName || '')}" /></div>
            <div class="field"><label class="label">מס׳ סוכן</label><input class="input" inputmode="numeric" data-pdraft="phoenixHealth.agentNumber" value="${escapeHtml(d.agentNumber || '')}" /></div>
            <div class="field"><label class="label">מנהל / מפקח</label><input class="input" data-pdraft="phoenixHealth.businessManager" value="${escapeHtml(d.businessManager || '')}" /></div>
            <div class="field"><label class="label">מס׳ הצעה</label><input class="input" inputmode="numeric" data-pdraft="phoenixHealth.proposalNumber" value="${escapeHtml(d.proposalNumber || '')}" /></div>
            <div class="field"><label class="label">תאריך מבוקש לתחילת הביטוח</label><input class="input" type="date" data-pdraft="phoenixHealth.requestedStartDate" value="${escapeHtml(d.requestedStartDate || '')}" /></div>
          </div>

          <div class="lcPhoenixChoiceGroup">
            <div class="lcPhoenixChoiceGroup__title">מסלול פניקס בריאות</div>
            <div class="lcPhoenixChoiceCards">
              ${tracks.map((track) => {
                const active = chosenTrack === track.value ? ' is-active' : '';
                return `<button type="button" class="lcPhoenixChoiceCard${active}" data-ph-track="${escapeHtml(track.value)}"><span class="lcPhoenixChoiceCard__title">${escapeHtml(track.label)}</span><span class="lcPhoenixChoiceCard__sub">${escapeHtml(track.note || '')}</span></button>`;
              }).join('')}
            </div>
          </div>

          <div class="lcPhoenixGrid lcPhoenixGrid--logic${basicQuestionCls}">
            <div class="field">
              <label class="label">האם קיימת פוליסה בסיסית מלאה?</label>
              <select class="input" data-pdraft="phoenixHealth.hasBasicPolicy">
                <option value="" ${!d.hasBasicPolicy ? 'selected' : ''}>בחר…</option>
                <option value="yes" ${d.hasBasicPolicy==='yes' ? 'selected' : ''}>כן</option>
                <option value="no" ${d.hasBasicPolicy==='no' ? 'selected' : ''}>לא</option>
              </select>
            </div>
            <div class="field">
              <label class="label">אם אין בסיס מלא – האם לרכוש השלמה במסגרת ההצעה?</label>
              <select class="input" data-pdraft="phoenixHealth.wantsBasicCompletion">
                <option value="" ${!d.wantsBasicCompletion ? 'selected' : ''}>בחר…</option>
                <option value="yes" ${d.wantsBasicCompletion==='yes' ? 'selected' : ''}>כן</option>
                <option value="no" ${d.wantsBasicCompletion==='no' ? 'selected' : ''}>לא</option>
              </select>
            </div>
            <label class="lcCheckCard"><input type="checkbox" data-pdraft-check="phoenixHealth.age60Exception" ${d.age60Exception ? 'checked' : ''} /><span>חריג גיל 60+ — ללא רכישת פוליסה בסיסית מלאה</span></label>
          </div>

          <div class="lcPhoenixGrid lcPhoenixGrid--logic">
            <div class="field">
              <label class="label">האם קיימת פוליסת בריאות שיפוי דומה?</label>
              <select class="input" data-pdraft="phoenixHealth.existingHealthPolicy">
                <option value="" ${!d.existingHealthPolicy ? 'selected' : ''}>בחר…</option>
                <option value="yes" ${d.existingHealthPolicy==='yes' ? 'selected' : ''}>כן</option>
                <option value="no" ${d.existingHealthPolicy==='no' ? 'selected' : ''}>לא</option>
              </select>
            </div>
            <div class="field">
              <label class="label">פעולה מול פוליסת הבריאות הקיימת</label>
              <select class="input" data-pdraft="phoenixHealth.existingHealthAction">
                <option value="" ${!d.existingHealthAction ? 'selected' : ''}>בחר…</option>
                <option value="cancel" ${d.existingHealthAction==='cancel' ? 'selected' : ''}>לבטל</option>
                <option value="keep" ${d.existingHealthAction==='keep' ? 'selected' : ''}>להשאיר ללא שינוי</option>
              </select>
            </div>
            <div class="field">
              <label class="label">האם קיימת פוליסת מחלות קשות?</label>
              <select class="input" data-pdraft="phoenixHealth.existingCriticalIllnessPolicy">
                <option value="" ${!d.existingCriticalIllnessPolicy ? 'selected' : ''}>בחר…</option>
                <option value="yes" ${d.existingCriticalIllnessPolicy==='yes' ? 'selected' : ''}>כן</option>
                <option value="no" ${d.existingCriticalIllnessPolicy==='no' ? 'selected' : ''}>לא</option>
              </select>
            </div>
            <div class="field">
              <label class="label">פעולה מול מחלות קשות</label>
              <select class="input" data-pdraft="phoenixHealth.existingCriticalIllnessAction">
                <option value="" ${!d.existingCriticalIllnessAction ? 'selected' : ''}>בחר…</option>
                <option value="cancel" ${d.existingCriticalIllnessAction==='cancel' ? 'selected' : ''}>לבטל</option>
                <option value="keep" ${d.existingCriticalIllnessAction==='keep' ? 'selected' : ''}>להשאיר</option>
              </select>
            </div>
            <div class="field">
              <label class="label">אופן הגשת הביטול</label>
              <select class="input" data-pdraft="phoenixHealth.cancellationMethod">
                <option value="agent" ${d.cancellationMethod==='agent' ? 'selected' : ''}>באמצעות סוכן הביטוח</option>
                <option value="phoenix" ${d.cancellationMethod==='phoenix' ? 'selected' : ''}>באמצעות הפניקס</option>
                <option value="self" ${d.cancellationMethod==='self' ? 'selected' : ''}>באופן עצמאי</option>
              </select>
            </div>
          </div>

          <div class="lcPhoenixChoiceGroup ${ambulatoryChosen ? 'is-highlight' : ''}">
            <div class="lcPhoenixChoiceGroup__title">כתבי שירות משלימים</div>
            <div class="lcPhoenixServiceList">
              ${serviceOptions.map((value) => {
                const checked = Array.isArray(d.serviceBasket) && d.serviceBasket.includes(value);
                return `<label class="lcPhoenixServiceItem"><input type="checkbox" data-ph-service="${escapeHtml(value)}" ${checked ? 'checked' : ''} /><span>${escapeHtml(value)}</span></label>`;
              }).join('')}
            </div>
          </div>

          <div class="lcPhoenixDeclarations">
            <label class="lcCheckCard"><input type="checkbox" data-pdraft-check="phoenixHealth.declarations.needsFit" ${declarations.needsFit ? 'checked' : ''} /><span>בוצעה התאמת צרכים</span></label>
            <label class="lcCheckCard"><input type="checkbox" data-pdraft-check="phoenixHealth.declarations.fitCopyGiven" ${declarations.fitCopyGiven ? 'checked' : ''} /><span>נמסר העתק מסמך התאמת צרכים</span></label>
            <label class="lcCheckCard"><input type="checkbox" data-pdraft-check="phoenixHealth.declarations.materialGiven" ${declarations.materialGiven ? 'checked' : ''} /><span>נמסר מידע מהותי על הפוליסה</span></label>
            <label class="lcCheckCard"><input type="checkbox" data-pdraft-check="phoenixHealth.declarations.noDoubleInsurance" ${declarations.noDoubleInsurance ? 'checked' : ''} /><span>הוסבר איסור כפל ביטוח</span></label>
            <label class="lcCheckCard"><input type="checkbox" data-pdraft-check="phoenixHealth.declarations.privacyConsent" ${declarations.privacyConsent ? 'checked' : ''} /><span>אישור פרטיות</span></label>
            <label class="lcCheckCard"><input type="checkbox" data-pdraft-check="phoenixHealth.declarations.medicalWaiver" ${declarations.medicalWaiver ? 'checked' : ''} /><span>ויתור סודיות רפואית</span></label>
            <label class="lcCheckCard"><input type="checkbox" data-pdraft-check="phoenixHealth.declarations.marketingOptOut" ${declarations.marketingOptOut ? 'checked' : ''} /><span>לא מסכים למסרים שיווקיים</span></label>
          </div>
        </div>`;
    },
    ...
    validateDraftPolicy(){
      ...
      if(this.isPhoenixHealthPolicy(d)){
        const ph = d.phoenixHealth && typeof d.phoenixHealth === 'object' ? d.phoenixHealth : {};
        const decl = ph.declarations && typeof ph.declarations === 'object' ? ph.declarations : {};
        if(!safeTrim(ph.agentName) || !safeTrim(ph.agentNumber)) return { ok:false, msg:"בפניקס בריאות חובה למלא שם סוכן ומספר סוכן" };
        if(!safeTrim(ph.requestedStartDate)) return { ok:false, msg:"בפניקס בריאות חובה למלא תאריך מבוקש לתחילת הביטוח" };
        if(this.getPhoenixTrackRequiresBasic(ph.policyTrack)){
          const okBasic = ph.hasBasicPolicy === 'yes' || ph.wantsBasicCompletion === 'yes' || !!ph.age60Exception;
          if(!okBasic) return { ok:false, msg:'במסלול פניקס שנבחר יש להשלים תשובת פוליסה בסיסית / השלמה / חריג גיל 60+' };
        }
        if(ph.existingHealthPolicy === 'yes' && !safeTrim(ph.existingHealthAction)) return { ok:false, msg:'יש לבחור פעולה מול פוליסת הבריאות הקיימת' };
        if(ph.existingCriticalIllnessPolicy === 'yes' && !safeTrim(ph.existingCriticalIllnessAction)) return { ok:false, msg:'יש לבחור פעולה מול מחלות קשות קיימת' };
        if(!decl.needsFit || !decl.materialGiven || !decl.privacyConsent || !decl.medicalWaiver) return { ok:false, msg:'יש לאשר את ההצהרות המרכזיות של פניקס בריאות' };
      }
...
// ---------- Step 6 ----------
        ...
        <div class="lcWSection">
          ...
          ` : `
            <div class="lcWGrid">
              ...
              ${this.fieldText("שם עסק / חברה","externalPayer.businessName", d.externalPayer?.businessName || "")}
              ${this.fieldText("רחוב","externalPayer.street", d.externalPayer?.street || "")}
              ${this.fieldText("מס׳ בית","externalPayer.houseNumber", d.externalPayer?.houseNumber || "", "numeric")}
              ${this.fieldText("יישוב","externalPayer.city", d.externalPayer?.city || "")}
              ${this.fieldText("מיקוד","externalPayer.zip", d.externalPayer?.zip || "", "numeric")}
            </div>
          `}
          ...
          ` : `
            <div class="lcWGrid">
              ...
              <div class="field">
                <label class="label">יום גבייה מבוקש</label>
                <select class="input" data-payer="ho.collectionDay">
                  <option value="10" ${(d.ho?.collectionDay||'10')==='10'?'selected':''}>10 לחודש</option>
                  <option value="1" ${(d.ho?.collectionDay||'')==='1'?'selected':''}>1 לחודש</option>
                </select>
              </div>
            </div>
          `}
    ...
    isStepCompleteForInsured(stepId, ins){
      ...
      if(stepId === 1){
        ...
        const adultReq = baseReq.concat(["maritalStatus","occupation","clinic","shaban","phone","email","communicationPreference","city","street","houseNumber"]);
      ...
      if(stepId === 6){
        ...
        if(d.payerChoice === "insured"){
          ...
          const req = ["relation","firstName","lastName","idNumber","birthDate","phone","street","houseNumber","city"];
        ...
        if(safeTrim(d.paymentMethod) === "cc"){
          ...
          const ho = d.ho || {};
          const req = ["account","branch","bankName","bankNo","collectionDay"];
