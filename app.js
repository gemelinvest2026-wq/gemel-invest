...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  // ---------- Server helpers ----------
  // We allow dynamic server URL from Settings (saved in localStorage), with fallback to GOOGLE_SCRIPT_URL.
  const SERVER_URL_LS_KEY = 'gemel_server_url_v1';

  function getServerUrl_(){
    try{
      const fromState = (state && state.server && state.server.url) ? String(state.server.url||'').trim() : '';
      if(fromState) return fromState;
    }catch(_){}
    try{
      const fromLS = String(localStorage.getItem(SERVER_URL_LS_KEY) || '').trim();
      if(fromLS) return fromLS;
    }catch(_){}
    return (typeof GOOGLE_SCRIPT_URL !== 'undefined') ? String(GOOGLE_SCRIPT_URL||'').trim() : '';
  }
  ...
  function setServerUrl_(url){
    ...
    try{
      if(u) localStorage.setItem(SERVER_URL_LS_KEY, u);
      else localStorage.removeItem(SERVER_URL_LS_KEY);
    }catch(_){}
    return u;
  }
...
$$('.navItem').forEach(b => b.addEventListener('click', (e) => {
  ...
  async function loginAttempt_(username, password){
    ...
    // If we have a server URL, prefer server login (works across devices)
    const base = getServerUrl_();
    if(base){
      ...
      if(ping && ping.ok){
        ...
        if(!r || !r.ok) return { ok:false, msg: (r && (r.msg||r.error)) ? String(r.msg||r.error) : 'שגיאת התחברות' };

        // Expect: { ok:true, user:{ username, role, displayName } }
        const user = r.user || {};
        setSession_({ username: String(user.username||u), at: Date.now() });
        setIdentity_({
          username: String(user.username||u),
          role: String(user.role||'agent'),
          displayName: String(user.displayName||user.username||u)
        });
        showApp_();

        // optional compact UI for PWA
        try{ window.__applyCompactUI && window.__applyCompactUI(); }catch(_){}
        return { ok:true };
      }
      // If ping fails, fall back to local
...
function renderKpisSettings(){
  ...
  function renderSettings(){
    ...
    // prefill input from saved value (if exists)
    try{
      const saved = String(localStorage.getItem(SERVER_URL_LS_KEY)||'').trim();
      if(serverUrlInput && !String(serverUrlInput.value||'').trim() && saved) serverUrlInput.value = saved;
    }catch(_){}
    ...
    $('#connectBtn')?.addEventListener('click', async () => {
      ...
      const hint = $('#connHint');
      if(hint) hint.textContent = state.connected ? '✅ מחובר' : '⚠️ לא מחובר';
      toast(state.connected ? 'מחובר' : 'לא מחובר', state.connected ? 'השרת ענה בהצלחה' : ('שגיאה: ' + (ping?.error || 'בדוק URL/הרשאות')));
    });

    $('#pullBtn')?.addEventListener('click', async () => {
      const base = getServerUrl_();
      if(!base){
        toast('חסר URL', 'הדבק URL של Web App ואז לחץ "בדוק חיבור"');
        return;
      }
      const ping = await apiPing();
      if(!ping || !ping.ok){
        toast('לא מחובר', 'בדוק URL/הרשאות ואז נסה שוב');
        return;
      }
      state.connected = true;
      try{
        ...
        toast('שגיאה', String(e && e.message ? e.message : e));
      }
    });
  }
