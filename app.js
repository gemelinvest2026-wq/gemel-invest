...
calcAge(dateStr){
  ...
  const MirrorsUI = {
    ...
    updateSteps(rec){
      ...
      const activeStep = !rec
        ? 1
        : (this.consent !== 'yes'
          ? 2
          : (!verify?.reflectionOpened
            ? 3
            : (safeTrim(verify?.harConsent) !== 'yes'
              ? 4
              : (!safeTrim(verify?.cancelSavedAt) ? 5 : 6))));
    ...
    getVerifyState(rec){
      ...
      if(typeof store.disclosureOpened !== 'boolean') store.disclosureOpened = false;
      return store;
    },
    ...
    renderReflection(rec){
      ...
      this.els.reflectCard.innerHTML = `<div class="mirrorsCard__head">
        ...
        ${harConsent === 'yes' ? `
          <div class="mirrorsReflectQuestion">לאחר ביצוע בדיקה בהר הביטוח, שתקף לחמישה ימי עבודה, להלן הביטוחים הקיימים לך כיום:</div>
          ...
          <div class="mirrorsReflectScript mirrorsReflectScript--followup">
    ...
    renderDisclosure(rec){
      ...
      const store = this.getVerifyState(rec);
      if(this.consent !== 'yes' || !store.reflectionOpened || safeTrim(store.harConsent) !== 'yes' || !safeTrim(store.cancelSavedAt)){
      ...
      store.disclosureOpened = true;
      const items = this.getDisclosureEntries(rec);
      if(!items.length){
        this.els.disclosureCard.innerHTML = `<div class="mirrorsCard__head">
            <div>
              <div class="mirrorsCard__title">גילוי נאות</div>
              <div class="mirrorsCard__hint">לא נמצאו פוליסות חדשות להצגת גילוי נאות בשלב זה.</div>
            </div>
            <span class="mirrorsSummaryBadge">שלב 6 · גילוי נאות</span>
          </div>
          <div class="mirrorsDisclosureEmpty">לא נמצאה פוליסה חדשה בשלב זה ולכן אין טקסט גילוי נאות להקראה.</div>`;
        return;
      }
      this.els.disclosureCard.innerHTML = `<div class="mirrorsCard__head">
          <div>
            <div class="mirrorsCard__title">גילוי נאות</div>
            <div class="mirrorsCard__hint">המערכת שולפת את נוסח ההקראה לפי חברה ומוצר. עבור כל פוליסה חדשה מוצג בלוק קריאה נפרד.</div>
          </div>
          <span class="mirrorsSummaryBadge">שלב 6 · גילוי נאות</span>
        </div>
        <div class="mirrorsDisclosureIntro">כעת אקריא לך את הגילוי הנאות הרלוונטי בהתאם לחברה ולמוצר שנבחרו עבורך. יש להקריא כל בלוק במלואו לפני מעבר לפוליסה הבאה.</div>
        <div class="mirrorsDisclosureStack">
          ${items.map((item, idx) => {
            const logoSrc = this.getCompanyLogo(item.company);
            const logo = logoSrc
              ? `<span class="mirrorsReflectCompany__logo"><img src="${escapeHtml(logoSrc)}" alt="${escapeHtml(item.company)}" /></span>`
              : `<span class="mirrorsReflectCompany__fallback">${escapeHtml((item.company || '•').slice(0,1))}</span>`;
            return `<div class="mirrorsDisclosureItem">
              <div class="mirrorsDisclosureItem__head">
                <div class="mirrorsDisclosureItem__title">
                  ${logo}
                  <div class="mirrorsDisclosureItem__titleText">
                    <strong>${escapeHtml(item.productLabel)}</strong>
                    <small>${escapeHtml(item.company)} · ${escapeHtml(item.insuredText || 'מבוטח')}</small>
                  </div>
                </div>
                <span class="mirrorsDisclosureItem__badge">גילוי ${idx + 1}</span>
              </div>
              <div class="mirrorsDisclosureText">${escapeHtml(item.text)}</div>
              ${item.fallback ? `<div class="mirrorsDisclosureNote">שים לב: לא נמצא במאגר נוסח מדויק אחד-לאחד עבור המוצר הזה, ולכן הוצג נוסח כללי תואם חברה ותחום. בהמשך אפשר להוסיף לו נוסח מדויק למיפוי מלא.</div>` : ``}
            </div>`;
          }).join('')}
        </div>
        <div class="mirrorsReflectActions">
          <button class="btn btn--primary" data-mirror-disclosure-save type="button">שמור שלב גילוי נאות</button>
        </div>`;
    },
    ...
    async saveDisclosure(){
      ...
      const items = this.getDisclosureEntries(rec);
      if(!items.length){
        alert('לא נמצאו פוליסות חדשות עבור שלב גילוי נאות.');
        return;
      }
      store.disclosureSavedAt = nowISO();
      store.disclosureSavedBy = safeTrim(Auth?.current?.name);
      State.data.meta.updatedAt = nowISO();
      rec.updatedAt = State.data.meta.updatedAt;
    ...
    getDisclosureEntries(rec){
      return this.getNewPolicies(rec).map((policy, idx) => {
        const company = safeTrim(policy?.company) || 'חברה לא מוגדרת';
        const productLabel = safeTrim(policy?.type) || 'מוצר לא מוגדר';
        ...
        const record = this.getDisclosureRecord(company, productLabel, policy);
        const text = this.fillDisclosureTemplate(record.text, {
          company,
          product: productLabel,
          amountLabel,
          amountValue,
          premium: safeTrim(policy?.premiumMonthly || policy?.monthlyPremium || policy?.premium || ''),
          startDate: safeTrim(policy?.startDate || policy?.policyStartDate || policy?.beginDate || ''),
          insuredText
        });
        return {
          id: safeTrim(policy?.id) || `disc_${idx}`,
          company,
          productLabel,
          insuredText,
          text,
    ...
    fillDisclosureTemplate(text, vars){
      let out = String(text || '');
      Object.keys(vars || {}).forEach((key) => {
        const value = safeTrim(vars[key]) || '—';
        out = out.replace(new RegExp(`{{${key}}}`, 'g'), value);
      });
      return out;
    },

    getDisclosureRecord(company, productLabel, policy){
      const companyKey = this.normalizeDisclosureCompany(company);
      const productKey = this.normalizeDisclosureProduct(productLabel, policy);
      const lib = this.getDisclosureLibrary();
      const companyLib = lib[companyKey] || {};
      if(companyLib[productKey]) return { text: companyLib[productKey], fallback: false };
      const genericKey = this.getGenericDisclosureProduct(productKey);
      if(genericKey && companyLib[genericKey]) return { text: companyLib[genericKey], fallback: true };
      return { text: this.buildFallbackDisclosure(company, productLabel, policy), fallback: true };
    },

    normalizeDisclosureCompany(company){
      const txt = safeTrim(company);
      if(txt.includes('הפניקס')) return 'הפניקס';
      if(txt.includes('מגדל')) return 'מגדל';
      if(txt.includes('כלל')) return 'כלל';
      if(txt.includes('איילון')) return 'איילון';
      return txt;
    },

    normalizeDisclosureProduct(productLabel, policy){
      const raw = `${safeTrim(productLabel)} ${(Array.isArray(policy?.healthCovers) ? policy.healthCovers.join(' ') : '')}`.toLowerCase();
      if(raw.includes('תרופות')) return 'תרופות מחוץ לסל';
      if(raw.includes('השתלות')) return 'השתלות וטיפולים מיוחדים בחו"ל';
      if(raw.includes('חו"ל') && raw.includes('ניתוח')) return 'ניתוחים וטיפולים מחליפי ניתוח בחו"ל';
      if(raw.includes('שקל ראשון')) return 'ניתוחים בישראל שקל ראשון';
      if((raw.includes('ללא השתתפות עצמית') || raw.includes('ללא השתתפות')) && raw.includes('שב')) return 'ניתוחים בישראל משלים שב"ן ללא השתתפות עצמית';
      if((raw.includes('עם השתתפות עצמית') || raw.includes('5000')) && raw.includes('שב')) return 'ניתוחים בישראל משלים שב"ן עם השתתפות עצמית';
      if(raw.includes('ייעוץ') || raw.includes('בדיקות') || raw.includes('אמבולטורי')) return 'ייעוץ ובדיקות';
      if(raw.includes('אבחון מהיר')) return 'אבחון מהיר';
      if(raw.includes('רופא און') || raw.includes('און ליין') || raw.includes('בקליק')) return 'רופא און ליין';
      if(raw.includes('רפואה משלימה')) return 'רפואה משלימה';
      if(raw.includes('סרטן')) return 'סרטן';
      if(raw.includes('מחלות קשות') || raw.includes('מרפא')) return 'מחלות קשות';
      if(raw.includes('מטריה')) return 'מטריה ביטוחית';
      if(raw.includes('ריסק') || raw.includes('חיים')) return 'חיים';
      if(raw.includes('נכות') && raw.includes('תאונה')) return 'נכות מתאונה';
      if(raw.includes('מוות') && raw.includes('תאונה')) return 'מוות מתאונה';
      if(raw.includes('ספורטיבי')) return 'איילון ספורטיבי';
      return 'כללי';
    },

    getGenericDisclosureProduct(productKey){
      const map = {
        'אבחון מהיר': 'ייעוץ ובדיקות',
        'רופא און ליין': 'ייעוץ ובדיקות',
        'רפואה משלימה': 'ייעוץ ובדיקות',
        'כללי': 'כללי'
      };
      return map[productKey] || productKey;
    },

    buildFallbackDisclosure(company, productLabel, policy){
      const amountLabel = this.getPolicyAmountLabel(policy);
      const amountValue = this.getPolicyAmountValue(policy) || 'לפי ההצעה שנמסרה לך';
      return `פוליסת ${productLabel} של חברת ${company} שאותה אנו ממליצים לך לרכוש כפופה לתנאי הפוליסה, להחרגות, למצב רפואי קיים ולתקופות אכשרה כמפורט במסמכי החברה. ${amountLabel !== 'כיסויים' ? `הכיסוי הרלוונטי בהצעה הוא ${amountLabel} ${amountValue}. ` : ''}הפוליסה מתחדשת בהתאם לתנאי החברה והפרמיה עשויה להשתנות לפי גיל, מסלול או תנאי החיתום.`;
    },

    getDisclosureLibrary(){
      return {
        'מגדל': {
          'תרופות מחוץ לסל': 'פוליסת הבריאות של מגדל כוללת כיסוי לתרופות שאינן בסל, לרבות תרופות להתוויה שונה, תרופות יתום ובדיקות גנטיות להתאמת טיפול. הפוליסה מתחדשת כל שנתיים ותקפה לכל החיים. קיימת תקופת אכשרה של 90 יום, ויכולות לחול החרגות, מגבלות סכום והשתתפות עצמית בחלק מן הכיסויים.',
          'השתלות וטיפולים מיוחדים בחו"ל': 'הכיסוי במגדל נותן מענה להשתלות ולטיפולים מיוחדים בחו"ל, כולל הוצאות רפואיות ונלוות, גמלה חודשית למועמד להשתלה וגמלת החלמה, בכפוף לתנאי הפוליסה. הפוליסה מתחדשת כל שנתיים ותקפה לכל החיים, עם תקופת אכשרה של 90 יום.',
          'ניתוחים וטיפולים מחליפי ניתוח בחו"ל': 'הכיסוי במגדל כולל ניתוחים פרטיים וטיפולים מחליפי ניתוח בחו"ל, התייעצויות עם רופא מומחה בישראל והוצאות נלוות כמו הטסה רפואית. הפוליסה מתחדשת כל שנתיים ותקפה לכל החיים. קיימת תקופת אכשרה של 90 יום.',
          'ניתוחים בישראל משלים שב"ן עם השתתפות עצמית': 'מדובר בכיסוי משלים שב"ן לניתוחים ולטיפולים מחליפי ניתוח בישראל, לאחר מיצוי הזכויות בקופת החולים, ובכפוף להשתתפות עצמית של 5,000 ₪ לפי תנאי התכנית. הפוליסה מתחדשת כל שנתיים ותקפה לכל החיים. קיימת תקופת אכשרה של 90 יום, ובמקרה של הריון או לידה 12 חודשים.',
          'ניתוחים בישראל משלים שב"ן ללא השתתפות עצמית': 'מדובר בכיסוי משלים שב"ן לניתוחים ולטיפולים מחליפי ניתוח בישראל ללא השתתפות עצמית, לאחר מיצוי הזכויות בקופת החולים. הפוליסה מתחדשת כל שנתיים ותקפה לכל החיים. קיימת תקופת אכשרה של 90 יום, ובמקרה של הריון או לידה 12 חודשים.',
          'ניתוחים בישראל שקל ראשון': 'מדובר בכיסוי בסיס לניתוחים ולטיפולים מחליפי ניתוח בישראל מהשקל הראשון, לרבות התייעצויות והוצאות רפואיות בבית חולים פרטי או מרפאה כירורגית פרטית. הפוליסה מתחדשת כל שנתיים ותקפה לכל החיים. קיימת תקופת אכשרה של 90 יום, ובמקרה של הריון או לידה 12 חודשים.',
          'ייעוץ ובדיקות': 'הכיסוי במגדל כולל ייעוץ ובדיקות אבחנתיות, התייעצויות עם רופאים מומחים במסלולים שונים ושירותים אמבולטוריים בהתאם למסלול שנבחר. הפוליסה מתחדשת כל שנתיים ותקפה לכל החיים. קיימת תקופת אכשרה של 90 יום וייתכנו החרגות והשתתפויות עצמיות.',
          'מחלות קשות': 'פוליסת מחלות קשות של מגדל מעניקה פיצוי חד פעמי בהתאם לתנאי המוצר שנבחר, בכפוף לתקופות אכשרה, להחרגות ולתנאי הפוליסה. חשוב לדעת שגם אם קיים כיסוי דומה, סכומי הביטוח יכולים להיות מצטברים, לפי תנאי החברה.',
          'נכות מתאונה': 'הכיסוי של מגדל לנכות מתאונה מעניק פיצוי במקרה של נכות מלאה או חלקית ותמידית עקב תאונה, כאשר התאונה היא הסיבה הישירה והמיידית לנכות. סכום הביטוח הוא {{amountValue}} והכיסוי תקף עד גיל 65 בכפוף לתנאי הפוליסה.',
          'מוות מתאונה': 'הכיסוי של מגדל למוות מתאונה מעניק למוטבים סכום חד פעמי במקרה של פטירת המבוטח עקב תאונה כהגדרתה בפוליסה. סכום הביטוח הוא {{amountValue}}, והכל בכפוף לתנאי הפוליסה והחרגותיה.',
          'כללי': 'פוליסת {{product}} של מגדל כפופה לתנאי הפוליסה, לתקופות אכשרה, להחרגות רפואיות ולתנאי החיתום של החברה. יש להסתמך על מסמכי הפוליסה הסופיים שישלחו למבוטח.'
        },
        'איילון': {
          'תרופות מחוץ לסל': 'הכיסוי באיילון כולל תרופות שאינן בסל, תרופות להתוויה שונה, תרופות יתום ותרופות מיוחדות, עם תקרות שיפוי לפי תנאי הפוליסה. הביטוח מתחדש כל שנתיים ותקף לכל החיים. קיימת תקופת אכשרה של 90 יום.',
          'השתלות וטיפולים מיוחדים בחו"ל': 'הכיסוי באיילון כולל הוצאות עבור השתלה, ניתוח או טיפול מיוחד בחו"ל, לרבות אשפוז, צוות רפואי, טיסות, שהייה, טיפולי המשך וקצבאות רלוונטיות. הביטוח מתחדש כל שנתיים ותקף לכל החיים, עם תקופת אכשרה של 90 יום.',
          'ניתוחים וטיפולים מחליפי ניתוח בחו"ל': 'הכיסוי באיילון כולל ניתוחים וטיפולים מחליפי ניתוח בחו"ל, וכן הוצאות נלוות כגון הטסה רפואית, שהייה, אחות פרטית ושיקום. הביטוח מתחדש כל שנתיים ותקף לכל החיים. קיימת תקופת אכשרה של 90 יום.',
          'ניתוחים בישראל משלים שב"ן עם השתתפות עצמית': 'מדובר בכיסוי לניתוחים ולטיפולים מחליפי ניתוח בישראל לאחר מיצוי שב"ן, עם השתתפות עצמית של 5,000 ₪. הביטוח מתחדש כל שנתיים ותקף לכל החיים. קיימת תקופת אכשרה של 90 יום, ובמקרה של הריון או לידה 12 חודשים.',
          'ניתוחים בישראל משלים שב"ן ללא השתתפות עצמית': 'מדובר בתכנית משלים שב"ן לניתוחים ולטיפולים מחליפי ניתוח פרטיים בארץ לאחר מיצוי הזכויות בקופה, ללא השתתפות עצמית. הביטוח מתחדש כל שנתיים ותקף לכל החיים. קיימת תקופת אכשרה של 90 יום, ובמקרה של הריון או לידה 12 חודשים.',
          'ניתוחים בישראל שקל ראשון': 'מדובר בתכנית לניתוחים ולטיפולים מחליפי ניתוח פרטיים בארץ מהשקל הראשון, כולל שלוש התייעצויות, שכר מנתח וכיסוי מלא עד תקרת ספקים שבהסדר. הביטוח מתחדש כל שנתיים ותקף לכל החיים. קיימת תקופת אכשרה של 90 יום, ובמקרה של הריון או לידה 12 חודשים.',
          'ייעוץ ובדיקות': 'הכיסוי האמבולטורי באיילון כולל התייעצויות, בדיקות אבחנתיות, שירותי הריון ולידה, בדיקות מניעה ושירותים מתקדמים נוספים. הביטוח מתחדש כל שנתיים ותקף לכל החיים, עם תקופות אכשרה משתנות לפי סוג השירות.',
          'איילון ספורטיבי': 'שירות איילון ספורטיבי מעניק במקרה של תאונה מזכה שירותים כגון אמבולנס, ייעוץ עם מומחים, טיפולי פיזיותרפיה והידרותרפיה, טיפול בכאב והקפאת מנוי בחדר כושר. הכיסוי כולל תקופת אכשרה של 90 יום, והפרמיה קבועה בהתאם לגיל.',
          'כללי': 'פוליסת {{product}} של איילון כפופה לתנאי החברה, לתקופות אכשרה, להחרגות מצב רפואי קיים ולתנאי החיתום. המסמכים המחייבים הם מסמכי הפוליסה שישלחו לאחר הקליטה.'
        },
        'כלל': {
          'תרופות מחוץ לסל': 'בכלל הכיסוי נותן מענה לתרופות שאינן בסל הבריאות, כולל תרופות מיוחדות וייבוא אישי, עד לתקרות הקבועות בפוליסה. קיימת תקופת אכשרה של 90 יום, ובמקרה של כיסוי חדש לא חופף מחברה אחרת תחול אכשרה לפי תנאי החברה. הביטוח מתחדש כל שנתיים ותקף לכל החיים.',
          'השתלות וטיפולים מיוחדים בחו"ל': 'הכיסוי בכלל כולל השתלות וטיפולים מיוחדים מחוץ לישראל, הוצאות נלוות וקצבה חודשית לפני ואחרי השתלה. קיימת תקופת אכשרה של 90 יום, והביטוח מתחדש כל שנתיים ותקף לכל החיים.',
          'ניתוחים וטיפולים מחליפי ניתוח בחו"ל': 'הכיסוי בכלל כולל ביצוע ניתוחים וטיפולים מחליפי ניתוח בחו"ל, יחד עם הוצאות נלוות והתייעצויות רפואיות. קיימת תקופת אכשרה של 90 יום, והביטוח מתחדש כל שנתיים ותקף לכל החיים.',
          'ניתוחים בישראל משלים שב"ן עם השתתפות עצמית': 'פוליסה זו בכלל מיועדת למי שחבר בשב"ן ומעניקה כיסוי לניתוחים פרטיים בישראל לאחר מיצוי וניכוי זכויות השב"ן, עם השתתפות עצמית של 5,000 ₪. מבין שלוש חלופות הכיסוי זו החלופה המצומצמת והזולה יותר. קיימת תקופת אכשרה של 90 יום, ובהריון או לידה 12 חודשים.',
          'ניתוחים בישראל משלים שב"ן ללא השתתפות עצמית': 'פוליסה זו בכלל מיועדת למבוטח חבר שב"ן ומעניקה כיסוי לניתוחים פרטיים בישראל לאחר מיצוי השב"ן, ללא השתתפות עצמית. זו חלופה רחבה יותר ממשלים עם השתתפות עצמית. קיימת תקופת אכשרה של 90 יום, ובהריון או לידה 12 חודשים.',
          'ניתוחים בישראל שקל ראשון': 'פוליסה זו בכלל מעניקה כיסוי לניתוחים פרטיים בישראל מהשקל הראשון, ללא תלות בזכויות בשב"ן. מדובר בחלופה הרחבה ביותר מבין שלוש חלופות הניתוחים, ולכן לרוב גם היקרה יותר. קיימת תקופת אכשרה של 90 יום, ובהריון או לידה 12 חודשים.',
          'ייעוץ ובדיקות': 'בכלל כיסוי הייעוצים והבדיקות כולל ייעוצים רפואיים, בדיקות אבחנתיות, בדיקות היריון ושירותים אמבולטוריים מגוונים. קיימת תקופת אכשרה של 90 יום והביטוח מתחדש כל שנתיים ותקף לכל החיים.',
          'אבחון מהיר': 'כתב השירות אבחון מהיר בכלל מעניק ייעוץ ואבחון רפואי ראשוני מהיר אצל רופא מאבחן ובדיקות נוספות במידת הצורך. הפרמיה קבועה וצמודה למדד, וקיימת תקופת אכשרה של 90 יום.',
          'רופא און ליין': 'כתב השירות רופא און ליין בכלל מאפשר ייעוץ רפואי בווידאו עם רופאים מומחים, רופאי ילדים ורופאי משפחה. הביטוח מתחדש כל שנתיים, הפרמיה קבועה וצמודה למדד, ולמעט רפואת משפחה וילדים קיימת תקופת אכשרה של 30 יום.',
          'רפואה משלימה': 'רפואה משלימה בכלל כוללת מגוון טיפולים אלטרנטיביים ומשלימים, עם בחירת ספק בהסדר או שיפוי לספק שאינו בהסדר, בכפוף לתנאי הפוליסה. הביטוח מתחדש כל שנתיים והפרמיה קבועה וצמודה למדד. קיימת תקופת אכשרה של 90 יום.',
          'מחלות קשות': 'פוליסת מחלות קשות של כלל מעניקה פיצוי חד פעמי בעת גילוי מחלה קשה מתוך רשימת המחלות המכוסות. סכום הביטוח המוצע הוא {{amountValue}}. קיימת תקופת אכשרה של 90 יום, ובחלק מהמוצרים יש תנאים מיוחדים למקרה ביטוח שני. חשוב לדעת שסכומי ביטוח דומים יכולים להצטבר.',
          'סרטן': 'פוליסת פיצוי לסרטן של כלל מעניקה פיצוי חד פעמי בעת גילוי מחלת הסרטן, בסכום המוצע של {{amountValue}}, בכפוף לתנאי הפוליסה. קיימת תקופת אכשרה של 90 יום, ותוקף הביטוח עד גיל 85 לפי תנאי המוצר.',
          'מטריה ביטוחית': 'פוליסת מטריה ביטוחית בכלל מעניקה, בכפוף לבחירה ולאישור החברה, כיסוי לאובדן כושר עבודה לפי עיסוק ספציפי, כיסוי לתקופת אכשרה וכיסוי לביטול קיזוז ביטוח לאומי על ידי קרן הפנסיה. כל זאת בכפוף לתנאי הפוליסה ואישור החברה.',
          'כללי': 'פוליסת {{product}} של כלל כפופה לתנאי הפוליסה, לתקופות אכשרה, להחרגות מצב רפואי קיים ולתנאי החיתום של החברה. המסמך המחייב הוא הפוליסה שתישלח לאחר הקליטה.'
        },
        'הפניקס': {
          'תרופות מחוץ לסל': 'בפניקס הכיסוי לתרופות שאינן בסל מעניק מענה לתרופות מחוץ לסל, תרופות להתוויה שונה, תרופות יתום ותרופות מיוחדות, עד התקרות הקבועות בפוליסה. הפוליסה מתחדשת כל שנתיים ותקפה לכל החיים. קיימת תקופת אכשרה של 90 יום וייתכנו החרגות והשתתפויות עצמיות.',
          'השתלות וטיפולים מיוחדים בחו"ל': 'הכיסוי בפניקס כולל הוצאות רפואיות ונלוות הקשורות בהשתלה או בטיפולים מיוחדים בחו"ל, כולל צוות רפואי, טיסה, שהייה וגמלת החלמה, ובחלק מהמקרים גם מסלול פיצוי חלופי. הפוליסה מתחדשת כל שנתיים ותקפה לכל החיים. קיימת תקופת אכשרה של 90 יום.',
          'ניתוחים וטיפולים מחליפי ניתוח בחו"ל': 'הכיסוי בפניקס כולל ניתוחים וטיפולים מחליפי ניתוח בחו"ל, וכן הוצאות רפואיות נוספות כמו התייעצויות והבאת מומחה לישראל. הפוליסה מתחדשת כל שנתיים ותקפה לכל החיים. קיימת תקופת אכשרה של 90 יום.',
          'ניתוחים בישראל משלים שב"ן עם השתתפות עצמית': 'מדובר בכיסוי מעל הזכאות בשב"ן עבור ניתוחים, התייעצויות וטיפולים מחליפי ניתוח בישראל. כאשר אין השתתפות של תכנית השב"ן לכלל העמיתים, הכיסוי הוא בניכוי השתתפות עצמית של 5,000 ₪ ובהתאם לתנאי התכנית. הפוליסה מתחדשת כל שנתיים ותקפה לכל החיים.',
          'ניתוחים בישראל משלים שב"ן ללא השתתפות עצמית': 'מדובר בכיסוי מעל הזכאות בשב"ן עבור ניתוחים, התייעצויות וטיפולים מחליפי ניתוח בישראל, ללא השתתפות עצמית ובהתאם לתקרות התכנית. הפוליסה מתחדשת כל שנתיים ותקפה לכל החיים, עם תקופת אכשרה של 90 יום.',
          'ניתוחים בישראל שקל ראשון': 'מדובר בכיסוי מהשקל הראשון להוצאות רפואיות הקשורות בניתוח, התייעצות וטיפול מחליף ניתוח בישראל, כולל שכר מנתח והוצאות בית חולים פרטי. הפוליסה מתחדשת כל שנתיים ותקפה לכל החיים. קיימת תקופת אכשרה של 90 יום ו-12 חודשים במקרה של הריון או לידה.',
          'ייעוץ ובדיקות': 'בפניקס כיסוי אמבולטורי כולל ייעוץ עם רופא מומחה, בדיקות אבחנתיות ומניעה ושירותים בגין הריון ולאחר לידה, בכפוף לתנאי הפוליסה ולהשתתפויות העצמיות הרלוונטיות.',
          'אבחון מהיר': 'אבחון רפואי מהיר בפניקס מעניק הליך אבחון ראשוני, בדיקה פיזיקלית ובדיקות אבחנתיות ראשוניות לצורך הגעה לאבחון מהיר, בכפוף לתנאי המוצר.',
          'רופא און ליין': 'כתב שירות רופא מומחה בקליק בפניקס מאפשר שיחות ייעוץ והכוונה עם רופאים מומחים, רפואת ילדים ומשפחה ושירותי איסוף מידע רפואי, בכפוף לתנאי כתב השירות ולתקופת השירות השנתית.',
          'מחלות קשות': 'פוליסת מחלות קשות של הפניקס מעניקה פיצוי חד פעמי בעת גילוי מחלה קשה מתוך רשימת המחלות שבמוצר. סכום הביטוח המוצע הוא {{amountValue}}. קיימת תקופת אכשרה של 90 יום, ובחלק מהמוצרים יש תקופות אכשרה נוספות למקרה ביטוח שני.',
          'סרטן': 'פוליסת מרפא סרטן של הפניקס מעניקה פיצוי חד פעמי בעת גילוי מחלת סרטן. סכום הביטוח המוצע הוא {{amountValue}}. קיימת תקופת אכשרה של 90 יום, ובמקרה ביטוח שני יש תקופת אכשרה נוספת לאחר סיום הטיפול במקרה הראשון.',
          'חיים': 'פוליסת ביטוח החיים של הפניקס מסוג ריסק מעניקה כיסוי למקרה מוות בסכום של {{amountValue}}. תוקף הביטוח הוא עד גיל 75, יש סייג התאבדות בשנה הראשונה, והפרמיה משתנה כל שנה או כל חמש שנים לפי מסלול הריסק שנבחר.',
          'מטריה ביטוחית': 'פוליסת מטריה ביטוחית של הפניקס מעניקה, בכפוף לבחירתך ולאישור החברה, כיסוי לאובדן כושר עבודה לפי עיסוק ספציפי, כיסוי לתקופת אכשרה וכיסוי לביטול קיזוז ביטוח לאומי על ידי קרן הפנסיה.',
          'כללי': 'פוליסת {{product}} של הפניקס כפופה לתנאי הפוליסה, לתקופות אכשרה, להחרגות מצב רפואי קיים ולתנאי החיתום. המסמך המחייב הוא הפוליסה שתישלח למבוטח.'
        }
      };
    },
