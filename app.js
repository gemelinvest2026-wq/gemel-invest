...
(() => {
  ...
  // ---------- Reset Access modal (admin recovery; resets ONLY users list) ----------
  function closeResetAccessModal(){
    const m = document.getElementById('resetAccessModal');
    if(m) m.style.display = 'none';
  }
  function openResetAccessModal(){
    const m = document.getElementById('resetAccessModal');
    if(m) m.style.display = 'flex';
  }
  // Expose to inline HTML handlers
  window.closeResetAccessModal = closeResetAccessModal;
  window.openResetAccessModal = openResetAccessModal;
...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  async function apiCall(action, payload){
    ...
    const res = await fetch(url, {
      method: 'POST',
      // IMPORTANT: text/plain avoids CORS preflight in Google Apps Script Web App
  ...
  function saveProcesses_(){
    try{
      ...
      try{ scheduleSyncUpAll_(); }catch(_){ }
}catch(_){}
  }
...
$$('.navItem').forEach(b => b.addEventListener('click', (e) => {
  ...
  // Admin recovery: reset users list to defaults (admin/1234) without touching business data.
  window.resetAccessNow = function(){
    try{ localStorage.removeItem('gemel_users_v1'); }catch(_){}
    try{ sessionStorage.removeItem(AUTH_SESSION_KEY); }catch(_){}
    try{ closeResetAccessModal && closeResetAccessModal(); }catch(_){}
...
`;
  ...
  function renderSummaryStep(){
    ...
    if(markDoneBtnEl) markDoneBtnEl.addEventListener('click', () => {
      ...
      if(state.connected){
        Promise.all([
          syncUpCustomer(customer).catch(()=>null),
          syncUpProposal(p).catch(()=>null)
        ]).then(()=>toast('סנכרון', 'ההצעה נשמרה לשיטס')).catch(()=>toast('סנכרון', 'שגיאה בסנכרון'));
      }
...
function debounce(fn, wait){
  ...
  let __syncUpTimer = null;

  async function syncUpAll_(){
    if(!state.connected) return { ok:false, error:'NOT_CONNECTED' };
    const payload = {
    ...
    return apiCall('pushAll', payload);
  }

  function scheduleSyncUpAll_(){
    if(!state.connected) return;
    try{
      clearTimeout(__syncUpTimer);
      __syncUpTimer = setTimeout(() => {
        syncUpAll_()
          .then(r => { if(r && r.ok) {/* silent */} })
          .catch(() => {/* silent */});
      }, 800);
    }catch(_){}
  }
  ...
  async function syncDown(){
    // Pull customers + proposals + processes from server (if available)
    const all = await apiCall('pullAll', {});
    if(all && all.ok){
      if(Array.isArray(all.customers)) state.customers = all.customers;
      if(Array.isArray(all.proposals)) state.proposals = all.proposals;
      if(Array.isArray(all.processes)) state.processes = all.processes;
