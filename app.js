...
(() => {
  ...
  const BUILD = "20260406-phoenix-health-pdf-v46-bundled-template";
...
init(){
      ...
      this.els.btnDownloadPhoenixHealthPdf = $("#lcDownloadPhoenixHealthPdf");
      this.els.btnDownloadPhoenixHealthPdfInline = $("#lcDownloadPhoenixHealthPdfInline");
      ...
      on(this.els.btnDownloadPhoenixHealthPdf, "click", () => this.exportPhoenixHealthPdf(null, this.els.btnDownloadPhoenixHealthPdf));
      on(this.els.btnDownloadPhoenixHealthPdfInline, "click", () => this.exportPhoenixHealthPdf(null, this.els.btnDownloadPhoenixHealthPdfInline));
    ...
    renderFooter(){
      ...
      this.syncFinishFlowActionState(this.getOperationalPayload());
    },
...
// ---------- Step 6 ----------
    ...
    hasPhoenixHealthPolicy(payloadOverride){
      const payload = payloadOverride && typeof payloadOverride === 'object'
        ? payloadOverride
        : this.getOperationalPayload();
      const list = Array.isArray(payload?.newPolicies) ? payload.newPolicies : [];
      return list.some((policy) => {
        const company = safeTrim(policy?.company);
        const type = safeTrim(policy?.type);
        const covers = Array.isArray(policy?.healthCovers) ? policy.healthCovers.map((item) => safeTrim(item)).filter(Boolean) : [];
        if(company !== 'הפניקס') return false;
        if(type === 'בריאות') return true;
        if(covers.length) return true;
        return /בריאות|השתלות|תרופות|שב"ן|שב״ן|ניתוחים|אמבולטורי|אבחון|רפואה משלימה|מומחה|אונליין/i.test(type);
      });
    },

    syncFinishFlowActionState(payloadOverride){
      const shouldShow = !!this.hasPhoenixHealthPolicy(payloadOverride);
      [this.els.btnDownloadPhoenixHealthPdf, this.els.btnDownloadPhoenixHealthPdfInline].forEach((btn) => {
        if(!btn) return;
    ...
    splitPhoenixAddress(source){
      const city = safeTrim(source?.city);
      const street = safeTrim(source?.street);
      const houseNumber = safeTrim(source?.houseNumber);
      return { city, street, houseNumber };
    },

    getPhoenixInsuredSlots(payloadOverride){
      const payload = this.normalizeOperationalReportPayload(payloadOverride || this.getOperationalPayload());
      const insureds = Array.isArray(payload.insureds) ? payload.insureds : [];
      const primary = insureds.find((ins) => safeTrim(ins?.type) === 'primary') || insureds[0] || null;
      const spouse = insureds.find((ins) => safeTrim(ins?.type) === 'spouse') || null;
      const children = insureds.filter((ins) => safeTrim(ins?.type) === 'child').slice(0, 4);
      return { payload, primary, spouse, children };
    },

    getPhoenixHealthPoliciesForInsured(payload, insured){
      if(!insured) return [];
      const insuredId = safeTrim(insured?.id) || safeTrim(insured?.data?.id);
      return (Array.isArray(payload?.newPolicies) ? payload.newPolicies : []).filter((policy) => {
        if(safeTrim(policy?.company) !== 'הפניקס') return false;
        const policyInsuredId = safeTrim(policy?.insuredId);
        const mode = safeTrim(policy?.insuredMode || 'single');
        if(mode === 'couple') return safeTrim(insured?.type) === 'primary' || safeTrim(insured?.type) === 'spouse';
        if(policyInsuredId) return policyInsuredId === insuredId;
        return safeTrim(insured?.type) === 'primary';
      });
    },

    getPhoenixCoversForInsured(payload, insured){
      const map = new Set();
      this.getPhoenixHealthPoliciesForInsured(payload, insured).forEach((policy) => {
        const type = safeTrim(policy?.type);
        if(type) map.add(type);
        (Array.isArray(policy?.healthCovers) ? policy.healthCovers : []).forEach((cover) => {
          const v = safeTrim(cover);
          if(v) map.add(v);
        });
      });
    ...
    getPhoenixStartDate(payload){
      const policies = Array.isArray(payload?.newPolicies) ? payload.newPolicies : [];
      const match = policies.find((policy) => safeTrim(policy?.company) === 'הפניקס' && safeTrim(policy?.startDate));
      return this.normalizePhoenixDate(match?.startDate || '');
    },

    getPhoenixAgentNumber(payload){
      const numbers = payload?.companyAgentNumbers && typeof payload.companyAgentNumbers === 'object'
        ? payload.companyAgentNumbers
        : {};
      return safeTrim(numbers['הפניקס'] || numbers['פניקס'] || '');
    },

    getPhoenixPaymentData(payload){
      const primary = payload?.primary && typeof payload.primary === 'object' ? payload.primary : {};
      const insureds = Array.isArray(payload?.insureds) ? payload.insureds : [];
      const selectedPayerId = safeTrim(primary?.selectedPayerId);
      const selectedPayer = insureds.find((ins) => String(ins?.id) === String(selectedPayerId)) || insureds[0] || null;
      const selectedPayerData = selectedPayer?.data && typeof selectedPayer.data === 'object' ? selectedPayer.data : {};
      const payerChoice = safeTrim(primary?.payerChoice || selectedPayerData?.payerChoice || 'insured');
      const method = safeTrim(primary?.paymentMethod || selectedPayerData?.paymentMethod || 'cc');
      const cc = Object.assign({}, selectedPayerData?.cc || {}, primary?.cc || {});
      const ho = Object.assign({}, selectedPayerData?.ho || {}, primary?.ho || {});
      if(payerChoice === 'external'){
        const external = Object.assign({}, primary?.externalPayer || selectedPayerData?.externalPayer || {});
        return { payerChoice, method, external, cc, ho, selectedPayerData };
      }
      return { payerChoice, method, external:null, cc, ho, selectedPayerData };
    },

    buildPhoenixPositiveFindings(payloadOverride){
      const payload = this.normalizeOperationalReportPayload(payloadOverride || this.getOperationalPayload());
      const primary = payload?.primary && typeof payload.primary === 'object' ? payload.primary : {};
      const healthDeclaration = primary?.healthDeclaration && typeof primary.healthDeclaration === 'object' ? primary.healthDeclaration : {};
      const responses = healthDeclaration?.responses && typeof healthDeclaration.responses === 'object' ? healthDeclaration.responses : {};
      ...
      (payload.insureds || []).forEach((ins, index) => {
        const insLabel = safeTrim(ins?.label) || `מבוטח ${index + 1}`;
        const insName = [safeTrim(ins?.data?.firstName), safeTrim(ins?.data?.lastName)].filter(Boolean).join(' ') || insLabel;
        const insId = safeTrim(ins?.id) || safeTrim(ins?.data?.id) || `payload_ins_${index}`;
        questionList.forEach((item) => {
          const q = item.question || {};
          const answer = responses?.[q.key]?.[insId] || null;
          if(safeTrim(answer?.answer) !== 'yes') return;
          const details = safeTrim(this.summarizeHealthFields(answer?.fields || {}));
          rows.push({ name: insName, question: q.text || q.key || '', details: details || 'סומן כן' });
        });
      });
      return rows.slice(0, 6);
    },

    applyPhoenixRadio(form, fieldName, value){
    ...
    applyPhoenixText(form, fieldName, value){
      const clean = safeTrim(value);
      if(!clean) return;
      try{
    ...
    applyPhoenixCheck(form, fieldName, shouldCheck){
      try{
    ...
    applyPhoenixHealthCoverFields(form, prefix, covers){
      const values = Array.isArray(covers) ? covers.map((item) => safeTrim(item)).filter(Boolean) : [];
      const has = (pattern) => values.some((value) => pattern.test(value));
      const check = (suffix, state) => this.applyPhoenixCheck(form, `${prefix}${suffix}`, state);
      this.applyPhoenixCheck(form, `${prefix}BasePack`,
        has(/השתלות/) && has(/תרופות/) && has(/מחוץ לישראל|חו"ל|חו״ל|טיפולים מיוחדים|מחליפי ניתוח/));
      check('C', has(/השתלות/));
      check('D', has(/תרופות/));
      check('E', has(/מחוץ לישראל|חו"ל|חו״ל|טיפולים מיוחדים|מחליפי ניתוח/));
      check('A', has(/שב"ן עם השתתפות עצמית|5,000|5000/));
      check('R', has(/משלים שב"ן|משלים שב״ן|ללא השתתפות עצמית/) && !has(/5,000|5000/));
      check('B', has(/מהשקל הראשון|שקל הראשון/));
      check('F', has(/אמבולטורי|ייעוצים ובדיקות|ייעוץ ובדיקות/));
      check('Q', has(/אבחון מהיר/));
      check('G', has(/9981|ייעוץ ובדיקות.*אבחון מהיר|אבחון מהיר.*ייעוץ ובדיקות/));
      check('I', has(/ליווי רפואי|אירוע רפואי/));
      check('K', has(/רפואה משלימה/));
      check('M', has(/התפתחות הילד|ילד/));
      check('J', has(/מומחה בקליק|אונליין|online/));
      this.applyPhoenixCheck(form, prefix.replace('chk','chkDisease') + 'A', has(/מרפא(?!\s*סרטן)|מחלות קשות/));
      this.applyPhoenixCheck(form, prefix.replace('chk','chkDisease') + 'B', has(/סרטן/));
    },

    fillPhoenixInsuredFields(form, insured, role, payload){
      if(!insured || !insured.data) return;
      const data = insured.data || {};
      const suffix = role === 'primary' ? '' : role === 'spouse' ? 'Spouse' : role;
      const shabanField = role === 'primary' ? 'ShabanR' : `Shaban${suffix}`;
      const text = (name, value) => this.applyPhoenixText(form, `${name}${suffix}`, value);
      ...
      text('PID', data.idNumber);
      text('BirthDate', this.normalizePhoenixDate(data.birthDate));
      text('OccupationCode', data.occupation);
      text('HMO', data.hmo || data.healthFund || data.kupatHolim);
      const address = this.splitPhoenixAddress(data);
      text('City', address.city);
      text('StreetName', address.street);
      text('HouseNumber', address.houseNumber);
      text('EmailAddress', data.email);
      text('CellPhoneNumber', data.phone);
      text('Hight', data.heightCm);
      text('Weight', data.weightKg);
      text('ClientSmokeNum', data.smokingAmount || data.smokingCigarettes || '');
      if(safeTrim(data.email)) this.applyPhoenixCheck(form, `Email${suffix}`, true);
      if(!safeTrim(data.email) && (safeTrim(data.city) || safeTrim(data.street))) this.applyPhoenixCheck(form, `IsraelMail${suffix}`, true);

      const genderRaw = safeTrim(data.gender);
      const genderMap = { 'זכר':'True', 'male':'True', 'm':'True', 'נקבה':'False', 'female':'False', 'f':'False' };
      this.applyPhoenixRadio(form, `Gender${suffix}`, genderMap[genderRaw] || '');
      const statusRaw = safeTrim(data.maritalStatus);
      const statusMap = { 'נשוי':'Married', 'נשואה':'Married', 'רווק':'Single', 'רווקה':'Single', 'גרוש':'Divorced', 'גרושה':'Divorced', 'אלמן':'Widow', 'אלמנה':'Widow' };
      this.applyPhoenixRadio(form, `FamilyStatus${suffix}`, statusMap[statusRaw] || '');
      const shabanRaw = safeTrim(data.shaban || data.shabanType);
      if(shabanRaw){
        const shabanVal = /לא|none|אין/i.test(shabanRaw) ? '2' : '1';
        this.applyPhoenixRadio(form, shabanField, shabanVal);
      }

      const covers = this.getPhoenixCoversForInsured(payload, insured);
      const prefix = role === 'primary' ? 'chkMain' : role === 'spouse' ? 'chkBzug' : `chk${role}`;
      this.applyPhoenixHealthCoverFields(form, prefix, covers);
    },

    fillPhoenixPositiveFindings(form, payloadOverride){
      const findings = this.buildPhoenixPositiveFindings(payloadOverride);
      findings.forEach((item, idx) => {
        const pos = idx + 1;
        this.applyPhoenixText(form, `DiagnosedName${pos}`, item.name);
        this.applyPhoenixText(form, `DiagnosisNum${pos}`, item.question);
        this.applyPhoenixText(form, `DiagnosisDetails${pos}`, item.details);
      });
    },

    async getPhoenixHealthPdfBytes(){
      if(this._phoenixHealthPdfBytes) return this._phoenixHealthPdfBytes.slice(0);
      const href = String(window.location.href || '');
      const pathName = String(window.location.pathname || '/');
      const basePath = pathName.endsWith('/') ? pathName : pathName.replace(/[^/]+$/, '');
      const origin = String(window.location.origin || '');
      const rawCandidates = [
        './phoenix-health-form.pdf',
        './בריאות פניקס.pdf',
        'phoenix-health-form.pdf',
        'בריאות פניקס.pdf',
        `${basePath}phoenix-health-form.pdf`,
        `${basePath}בריאות פניקס.pdf`,
        origin ? `${origin}${basePath}phoenix-health-form.pdf` : '',
        origin ? `${origin}${basePath}בריאות פניקס.pdf` : ''
      ].filter(Boolean);
      const candidates = [];
      rawCandidates.forEach((value) => {
        try{
          const resolved = /^https?:/i.test(value) ? value : new URL(value, href).href;
      ...
      for(const candidate of candidates){
        try{
          ...
          lastError = new Error(`HTTP ${res?.status || 'ERR'} for ${candidate}`);
        }catch(err){
          lastError = err;
        }
      }
      if(!response) throw (lastError || new Error('Phoenix health PDF source was not found'));
      const arr = new Uint8Array(await response.arrayBuffer());
      this._phoenixHealthPdfBytes = arr;
      return arr.slice(0);
    },

    getPhoenixHealthPdfFileName(payloadOverride){
      const payload = this.normalizeOperationalReportPayload(payloadOverride || this.getOperationalPayload());
      const name = [safeTrim(payload?.primary?.firstName), safeTrim(payload?.primary?.lastName)].filter(Boolean).join('-') || 'לקוח';
      const idNumber = safeTrim(payload?.primary?.idNumber) || 'ללא-תז';
      return `טופס-הפניקס-בריאות-${name}-${idNumber}.pdf`.replace(/[\\/:*?"<>|]/g, '-').replace(/\s+/g, '-');
    },

    async exportPhoenixHealthPdf(payloadOverride=null, triggerBtn=null){
      const payload = this.normalizeOperationalReportPayload(payloadOverride || this.lastSavedOperationalPayload || this.getOperationalPayload());
      if(!this.hasPhoenixHealthPolicy(payload)){
        window.alert('כדי להוריד טופס הפניקס יש לבחור פוליסת בריאות של הפניקס.');
        return;
      }
      ...
      try{
        if(triggerBtn){
          triggerBtn.disabled = true;
          triggerBtn.textContent = 'מכין טופס פניקס...';
        }
        const bytes = await this.getPhoenixHealthPdfBytes();
        const PDFDocument = window?.PDFLib?.PDFDocument;
        if(!PDFDocument) throw new Error('PDFLib missing');
        const pdfDoc = await PDFDocument.load(bytes, { ignoreEncryption: true });
        const form = pdfDoc.getForm();

        const { primary, spouse, children } = this.getPhoenixInsuredSlots(payload);
        this.applyPhoenixText(form, 'InsuranceBegin', this.getPhoenixStartDate(payload));
        this.applyPhoenixText(form, 'AgentName', safeTrim(Auth?.current?.name));
        this.applyPhoenixText(form, 'AgentNumber', this.getPhoenixAgentNumber(payload));

        this.fillPhoenixInsuredFields(form, primary, 'primary', payload);
        this.fillPhoenixInsuredFields(form, spouse, 'spouse', payload);
        children.forEach((child, index) => this.fillPhoenixInsuredFields(form, child, `Child${index + 1}`, payload));

        const payment = this.getPhoenixPaymentData(payload);
        if(payment.payerChoice === 'external' && payment.external){
          this.applyPhoenixText(form, 'PayerFirstName', payment.external.firstName);
          this.applyPhoenixText(form, 'PayerLastName', payment.external.lastName);
          this.applyPhoenixText(form, 'PayerPID', payment.external.idNumber);
          this.applyPhoenixText(form, 'FullNamePayer', [safeTrim(payment.external.firstName), safeTrim(payment.external.lastName)].filter(Boolean).join(' '));
          this.applyPhoenixText(form, 'PayerRelation', payment.external.relation);
          this.applyPhoenixText(form, 'PayerCity', payment.external.city || '');
          this.applyPhoenixText(form, 'PayerStreetName', payment.external.street || '');
          this.applyPhoenixText(form, 'PayerHouseNumber', payment.external.houseNumber || '');
        }
        if(payment.method === 'cc'){
          this.applyPhoenixText(form, 'CreditCardNumber', payment.cc.cardNumber);
          const holderParts = safeTrim(payment.cc.holderName).split(/\s+/).filter(Boolean);
          this.applyPhoenixText(form, 'FirstNameCreditCardHolder', holderParts.slice(0, -1).join(' ') || payment.cc.holderName);
          this.applyPhoenixText(form, 'LastNameCreditCardHolder', holderParts.slice(-1).join(' '));
          this.applyPhoenixText(form, 'PIDCreditCardHolder', payment.cc.holderId);
        }else{
          this.applyPhoenixText(form, 'BankAccountNumber', payment.ho.account);
          this.applyPhoenixText(form, 'BankBranch', payment.ho.branch);
          this.applyPhoenixText(form, 'BankName', payment.ho.bankName);
          this.applyPhoenixText(form, 'BankBranchCode', payment.ho.branch);
          this.applyPhoenixText(form, 'BankNameCode', payment.ho.bankNo);
          this.applyPhoenixText(form, 'HetPayBankAccOwner', [safeTrim(payment.selectedPayerData?.firstName), safeTrim(payment.selectedPayerData?.lastName)].filter(Boolean).join(' '));
          this.applyPhoenixText(form, 'PIDBankAccOwner', payment.selectedPayerData?.idNumber);
          this.applyPhoenixText(form, 'BankAccOwner', [safeTrim(payment.selectedPayerData?.firstName), safeTrim(payment.selectedPayerData?.lastName)].filter(Boolean).join(' '));
        }

        this.applyPhoenixCheck(form, 'NeedsHatama', true);
        this.applyPhoenixCheck(form, 'FileCopy', true);
        this.applyPhoenixCheck(form, 'MeidaMahuti', true);
        this.fillPhoenixPositiveFindings(form, payload);
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = this.getPhoenixHealthPdfFileName(payload);
        document.body.appendChild(a);
        a.click();
        a.remove();
        window.setTimeout(() => URL.revokeObjectURL(url), 2500);
      }catch(err){
        console.error('PHOENIX_HEALTH_PDF_EXPORT_FAILED', err);
        window.alert('לא הצלחתי להכין את טופס הפניקס הממולא. ודא שקובץ ה-PDF קיים ונסה שוב.');
      }finally{
        if(triggerBtn){
          triggerBtn.disabled = false;
          triggerBtn.textContent = originalText || 'הורד טופס הפניקס – בריאות';
        }
        this.syncFinishFlowActionState(payload);
      }
    },
    ...
    showFinishFlowSuccess(){
      ...
      this.syncFinishFlowActionState(this.lastSavedOperationalPayload || this.getOperationalPayload());
    },
    ...
    hideFinishFlow(){
      ...
      this.syncFinishFlowActionState(null);
    },
