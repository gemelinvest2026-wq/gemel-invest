            ...
            <tr class="lcPolImportedWrapRow">
              <td colspan="7" class="lcPolImportedWrapCell">
                <div class="lcPolImportedRow">
                  <div class="lcPolImportedRow__top">
                    <div class="lcPolImportedRow__companyBox">
                      <div class="lcPolImportedRow__companyBadge">
                        ${logo}
                        <span class="lcPolImportedRow__companyName">${escapeHtml(p.company || 'חברה לא ידועה')}</span>
                      </div>
                      <div class="lcPolImportedRow__miniBadges">
                        ${importedBadge}
                        ${safeTrim(p.type) ? `<span class="lcPolMetaBadge"><b>סוג:</b> ${escapeHtml(safeTrim(p.type))}</span>` : ''}
                        ${safeTrim(p.policyNumber) ? `<span class="lcPolMetaBadge"><b>מספר פוליסה:</b> ${escapeHtml(safeTrim(p.policyNumber))}</span>` : ''}
                      </div>
                    </div>
                    <div class="lcPolImportedRow__premium">
                  ...
                  <div class="lcPolImportedRow__details">
                    <div class="lcPolImportedRow__field"><span class="lcPolImportedRow__label">מבוטח</span><strong>${escapeHtml(insuredName || '—')}</strong></div>
                    <div class="lcPolImportedRow__field"><span class="lcPolImportedRow__label">ת"ז</span><strong>${escapeHtml(insuredIdNumber || '—')}</strong></div>
                    <div class="lcPolImportedRow__field"><span class="lcPolImportedRow__label">תקופת ביטוח</span><strong>${escapeHtml(periodText || '—')}</strong></div>
                    <div class="lcPolImportedRow__field"><span class="lcPolImportedRow__label">שיעבוד</span><strong>${pledgeLabel}</strong></div>
                    <div class="lcPolImportedRow__field lcPolImportedRow__field--wide"><span class="lcPolImportedRow__label">מה נכלל</span><strong>${escapeHtml(includedList.length ? includedList.join(' • ') : includedSummary || '—')}</strong></div>
                    <div class="lcPolImportedRow__field"><span class="lcPolImportedRow__label">פרמיה שנתית</span><strong>${escapeHtml(annualPremiumTxt ? `${annualPremiumTxt} ₪` : '—')}</strong></div>
                  </div>
                  <div class="lcPolImportedRow__foot">
                    <span class="lcPolImportedRow__readonly">פוליסה זו הובאה מהר הביטוח ומוצגת בשורת נתונים לקריאה בלבד.</span>
                    <button class="btn lcSmallBtn" data-del-exist="${p.id}" type="button">הסר</button>
                  </div>
                </div>
              </td>
            </tr>
