...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  function showApp_(){
    ...
    showUiZoom_(true);
    applyUiZoom_(uiZoom_);
  }

  // ---------- UI Zoom (Desktop/PWA) ----------
  const UI_ZOOM_KEY = 'gemel_ui_zoom_v1';
  let uiZoom_ = 1;

  function clamp_(n, min, max){ return Math.max(min, Math.min(max, n)); }

  function applyUiZoom_(z){
    uiZoom_ = clamp_(Number(z)||1, 0.80, 1.30);
    const appRoot = document.getElementById('appRoot');
    if(appRoot) appRoot.style.zoom = String(uiZoom_);
    try{ localStorage.setItem(UI_ZOOM_KEY, String(uiZoom_)); }catch(_){}
    const val = document.getElementById('uiZoomVal');
    if(val) val.textContent = Math.round(uiZoom_ * 100) + '%';
  }

  function initUiZoom_(){
    const wrap = document.getElementById('uiZoom');
    const btnIn = document.getElementById('uiZoomIn');
    const btnOut = document.getElementById('uiZoomOut');

    if(!wrap || !btnIn || !btnOut) return;

    // Load saved zoom
    let z = 1;
    try{
      const raw = localStorage.getItem(UI_ZOOM_KEY);
      if(raw) z = parseFloat(raw);
    }catch(_){}
    applyUiZoom_(z);

    btnIn.addEventListener('click', () => applyUiZoom_(uiZoom_ + 0.05));
    btnOut.addEventListener('click', () => applyUiZoom_(uiZoom_ - 0.05));

    // Ctrl/Cmd + / - support (inside the app)
    window.addEventListener('keydown', (e) => {
      if(!(e.ctrlKey || e.metaKey)) return;
      if(e.key === '+' || e.key === '=' ){ e.preventDefault(); applyUiZoom_(uiZoom_ + 0.05); }
      if(e.key === '-' ){ e.preventDefault(); applyUiZoom_(uiZoom_ - 0.05); }
      if(e.key === '0'){ e.preventDefault(); applyUiZoom_(1); }
    }, { passive:false });
  }

  initUiZoom_();

  function showUiZoom_(on){
    const wrap = document.getElementById('uiZoom');
    if(!wrap) return;
