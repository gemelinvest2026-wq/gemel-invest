...
(() => {
  ...
  // "ריסק" requires סכום ביטוח
  const isSumProduct = (prod) => (prod === 'ריסק');
  // "ריסק" + "משכנתא" require סכום ביטוח
...
// Keep wizard title in sync
  ...
  function renderOldPoliciesStep(){
        ...
        <table>
          ...
          <tbody>
            ${rows.length ? rows.map(({x,idx}) => {
              ...
              // keep "cancel_partial" as-is (new feature)
    ...
    $('#addOldBtn').addEventListener('click', () => {
      ...
      if(isSumProduct(product) && sumInsured <= 0){ toast('שגיאה', 'חובה למלא סכום ביטוח בריסק/משכנתא'); oldSumInput && oldSumInput.focus && oldSumInput.focus(); return; }

      // If partial cancel selected for the new record, require entering attachments first
      const pushRecord = (partialList) => {
        const rec = { insuredId, company, product, policyNo, premium, decision };
        if(isSumProduct(product)) rec.sumInsured = sumInsured;
        if(isCompProduct(product)) rec.compensation = compensation;
        if(decision === 'cancel_partial') rec.cancelPartialAttachments = partialList || [];
        p.oldPolicies.push(rec);
      ...
      if(decision === 'cancel_partial'){
        openPartialCancelModal([], (list) => {
          if(!list || !list.length){ toast('חסר', 'חובה למלא נספחים לביטול חלקי'); return; }
          pushRecord(list);
        });
        return;
      }
    ...
    $$('[data-old-decision]').forEach(sel => sel.addEventListener('change', () => {
      ...
      if(item){
        ...
        // Partial cancel: open modal to capture attachments
        if(v === 'cancel_partial'){
          openPartialCancelModal(item.cancelPartialAttachments || [], (list) => {
            if(!list || !list.length){
              // user canceled or left empty -> revert
              item.decision = prev;
              sel.value = prev;
              toast('בוטל', 'לא נשמר ביטול חלקי');
              return;
            }
            item.cancelPartialAttachments = list;
            applyLabelToast();
          });
          return;
        }

        // Any other status clears partial attachments
        if(item.cancelPartialAttachments) delete item.cancelPartialAttachments;
        applyLabelToast();
      }
    }));
;
      ...
      <div class="tableWrap">
        <table>
          ...
          <tbody>
            ...
            `).join('') : `<tr><td colspan="9" style="color:rgba(18,19,25,.55)">אין פוליסות חדשות למבוטח זה עדיין.</td></tr>`}
          </tbody>
        </table>
      </div>
...
`;
  ...
  function renderPoliciesByInsured(p, kind){
    ...
    const blocks = list.map(ins => {
      ...
      const head = (kind === 'new')
        ...
        : `<tr><th>חברה</th><th>מוצר</th><th>פרמיה</th><th>סטטוס</th><th>נספחים לביטול חלקי</th></tr>`;
            ...
            <table class="table">
              ...
              <tbody>
                ${rows.map(x => {
                  ...
                  const dec = String(x.decision||x.action||'').trim();
                  const label = (dec==='keep') ? 'להשאיר ללא שינוי'
                    : (dec==='appoint') ? 'ביצוע מינוי סוכן'
                    : (dec==='cancel_partial' || dec==='ביטול חלקי') ? 'ביטול חלקי'
                    : 'ביטול מלא';
                  const atts = (dec==='cancel_partial' || dec==='ביטול חלקי')
                    ? (Array.isArray(x.cancelPartialAttachments) && x.cancelPartialAttachments.length ? escapeHtml(x.cancelPartialAttachments.join(' • ')) : '—')
                    : '—';
                  return `<tr>
                    <td>${renderCompanyCell(x.company||'')}</td>
                    <td>${escapeHtml(x.product||'')}</td>
  ...
  function openPrintView(proposal){
    ...
    try {
      ...
      function oldDecisionLabel(dec){
        const d = String(dec||'').trim();
        if(d==='keep' || d==='נשאר ללא שינוי') return 'להשאיר ללא שינוי';
        if(d==='appoint') return 'ביצוע מינוי סוכן';
        if(d==='cancel_partial' || d==='ביטול חלקי') return 'ביטול חלקי';
        return 'ביטול מלא';
      }

      function partialAttachmentsCell(x){
        const d = String(x && (x.decision || x.action) || '').trim();
        const isPartial = (d === 'cancel_partial' || d === 'ביטול חלקי');
        if(!isPartial) return '—';
        const arr = Array.isArray(x.cancelPartialAttachments) ? x.cancelPartialAttachments : [];
        if(!arr.length) return '—';
        return `<ul class="cellList">${arr.slice(0,20).map(a => `<li>${esc(a)}</li>`).join('')}</ul>`;
      }
      ...
      const oldRows = oldPolicies.map(x => `
        <tr>
          ...
          <td>${(String(x.product||'')==='ריסק' || String(x.product||'')==='משכנתא') ? money(nnum(x.sumInsured)) : '—'}</td>
          <td>${(String(x.product||'')==='מחלות קשות' || String(x.product||'')==='מחלות סרטן') ? money(nnum(x.compensation)) : '—'}</td>
          <td>${esc(oldDecisionLabel(x.decision || x.action))}</td>
          <td>${partialAttachmentsCell(x)}</td>
        </tr>
      ...
      <body>
        <div class="page">
          ...
          <table class="t">
            <thead><tr><th>מבוטח</th><th>חברה</th><th>מוצר</th><th>פרמיה חודשית</th><th>סכום ביטוח</th><th>סכום פיצוי</th><th>סטטוס</th><th>נספחים לביטול חלקי</th></tr></thead>
            <tbody>
              ${oldRows}
              <tr class="sumRow">
...
// ---------- Partial Cancel (נספחים לביטול חלקי) ----------
function parseAttachmentList(text){
...
function openPartialCancelModal(currentList, onSave){
  const list = Array.isArray(currentList) ? currentList : parseAttachmentList(currentList);
  const overlay = document.createElement('div');
  overlay.className = 'modal-overlay';

  const esc = (s)=>escapeHtml(String(s||''));
  const placeholder = 'דוגמה:\n- נספח תאונות\n- נספח מחלות קשות\n(אפשר שורה-שורה או מופרד בפסיקים)';
  overlay.innerHTML = `
    <div class="modal-card" style="max-width:520px;text-align:right">
      <h3 style="margin:0 0 8px;font-weight:900">ביטול חלקי — נספחים לביטול</h3>
      <p style="margin:0 0 10px;color:rgba(18,19,25,.68)">כתוב אילו נספחים לבטל. זה יישמר בנתוני הלקוח ויופיע בסיכום PDF.</p>
      <textarea id="partialTxt" style="width:100%;min-height:140px;border-radius:14px;border:1px solid rgba(18,19,25,.14);padding:10px 12px;resize:vertical;outline:none" placeholder="${esc(placeholder)}">${esc(list.join('\n'))}</textarea>
      <div style="display:flex;gap:10px;justify-content:flex-start;margin-top:12px;flex-wrap:wrap">
        <button class="btn btnPrimary" type="button" data-save>שמירה</button>
        <button class="btn btnSoft" type="button" data-cancel>ביטול</button>
      </div>
  ...
  overlay.querySelector('[data-save]').addEventListener('click', () => {
    const txt = overlay.querySelector('#partialTxt').value || '';
    const parsed = parseAttachmentList(txt);
    close();
