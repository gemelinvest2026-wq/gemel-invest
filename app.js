...
(() => {
  ...
  const BUILD = "20260327-mirrors-verify-details-delivery-smoking-v1";
...
ההכנסה מעבודה שאתה צפוי להרוויח תעמוד על סך של ______ ₪ לחודש. ידוע לך כי בקרות מקרה הביטוח יוגבלו תגמולי הפיצוי החודשי להם תהיה זכאי לגובה שלא יעלה על 75% מהכנסתך מעבודה בפועל ב־12 החודשים שקדמו למועד קרות מקרה הביטוח, או בתקופה ממועד תחילת הביטוח ועד מועד קרות מקרה הביטוח, אם חלפו פחות מ־12 חודשים.`
  ...
  const MirrorsUI = {
    ...
    init(){
      ...
      on(this.els.verifyCard, "click", async (ev) => {
        ...
        if(smokeBtn){
          ...
          const raw = safeTrim(smokeBtn.getAttribute('data-mirror-smoking'));
          const [insuredKey, answer] = raw.includes(':') ? raw.split(':') : ['primary', raw];
          const entry = this.getSmokingEntry(store, insuredKey);
          entry.answer = safeTrim(answer);
          if(entry.answer !== 'yes'){
            entry.products = [];
            entry.quantity = '';
          }
          if(insuredKey === this.getInsuredKey(this.getInsureds(rec)[0], 0) || insuredKey === 'primary'){
            store.smokingAnswer = entry.answer;
            store.smokingProducts = Array.isArray(entry.products) ? entry.products : [];
            store.smokingQuantity = safeTrim(entry.quantity);
          }
          this.render();
          return;
        }
      ...
      on(this.els.verifyCard, "change", (ev) => {
        ...
        if(target?.matches?.('[data-mirror-smoke-product]')){
          ...
          const insuredKey = safeTrim(target.getAttribute('data-mirror-smoke-product'));
          const entry = this.getSmokingEntry(store, insuredKey);
          const set = new Set(Array.isArray(entry.products) ? entry.products : []);
          if(target.checked) set.add(value);
          else set.delete(value);
          entry.products = Array.from(set);
          return;
        }
        if(target?.matches?.('[data-mirror-smoke-qty]')){
          const insuredKey = safeTrim(target.getAttribute('data-mirror-smoke-qty'));
          const entry = this.getSmokingEntry(store, insuredKey);
          entry.quantity = safeTrim(target.value);
          return;
        }
    ...
    getFocusStep(rec){
      ...
      rec.payload.mirrorFlow.ui.focusStep = 2;
      return 2;
    },
    ...
    renderWizardNav(rec){
      ...
      this.els.wizardNav.style.display = 'none';
      this.els.wizardNav.innerHTML = '';
    },

    getStepTitle(step){
      const map = {1:'חיפוש לקוח',2:'פתיח שיחה'};
    ...
    getVerifyState(rec){
      ...
      if(!store.smokingByInsured || typeof store.smokingByInsured !== 'object') store.smokingByInsured = {};
    ...
    getInsuredInfoCards(ins, index){
      const data = ins?.data || {};
      const fullName = this.getInsuredDisplayName(ins, index);
      const addressText = this.getAddressText(data);
      return [
        ['שם מלא', fullName],
        ['תעודת זהות', safeTrim(data.idNumber)],
        ['תאריך לידה', safeTrim(data.birthDate)],
        ['מצב משפחתי', safeTrim(data.maritalStatus || data.familyStatus)],
        ['עיסוק נוכחי', safeTrim(data.occupation)],
        ['קופת חולים ושב״ן', [safeTrim(data.clinic || data.hmo || data.kupatHolim), safeTrim(data.shaban || data.shabanLevel)].filter(Boolean).join(' · ')],
        ['כתובת', addressText || '—']
      ];
    },

    renderInsuredInfoSections(rec){
      const insureds = this.getInsureds(rec);
      if(!insureds.length) return '';
      return insureds.map((ins, index) => {
        const title = this.getInsuredLabel(ins, index);
        const subtitle = this.getInsuredDisplayName(ins, index);
        const cards = index === 0 ? this.getVerifyInfoCards(rec) : this.getInsuredInfoCards(ins, index);
        return `<div class="mirrorsInsuredSection">
          <div class="mirrorsInsuredSection__head">
            <div class="mirrorsInsuredSection__title">${escapeHtml(title)}</div>
            <div class="mirrorsInsuredSection__name">${escapeHtml(subtitle)}</div>
          </div>
          <div class="mirrorsVerifyGrid mirrorsVerifyGrid--wide">${cards.map(([label, value]) => `<div class="mirrorsInfoCard"><span>${escapeHtml(label)}</span><strong>${escapeHtml(value || '—')}</strong></div>`).join('')}</div>
        </div>`;
      }).join('');
    },
    ...
    renderVerifyInfoSection(rec){
      ...
      if(!store.editMode){
        ...
        ${this.renderInsuredInfoSections(rec)}${savedNote}`;
      }
    ...
    getInsuredKey(ins, index){
      const data = ins?.data || {};
      return safeTrim(data.idNumber) || `insured_${index}`;
    },
    ...
    isMinorInsured(ins){
      ...
      const data = ins?.data || {};
      const rawAge = Number(data.age ?? data.currentAge ?? data.insuredAge);
      if(Number.isFinite(rawAge) && rawAge > 0) return rawAge < 18;
      const birthDate = safeTrim(data.birthDate);
      if(birthDate){
        const parts = birthDate.split(/[./-]/).map(s => safeTrim(s)).filter(Boolean);
        if(parts.length === 3){
          let day, month, year;
          if(parts[0].length === 4){
          ...
          if(Number.isFinite(day) && Number.isFinite(month) && Number.isFinite(year) && year > 1900 && month >= 1 && month <= 12 && day >= 1 && day <= 31){
    ...
    getSmokingEligibleInsureds(rec){
      return this.getInsureds(rec).map((ins, index) => ({ ins, index })).filter(({ ins }) => !this.isMinorInsured(ins));
    },

    getSmokingEntry(store, key){
      if(!store.smokingByInsured || typeof store.smokingByInsured !== 'object') store.smokingByInsured = {};
      const entry = store.smokingByInsured[key] && typeof store.smokingByInsured[key] === 'object' ? store.smokingByInsured[key] : (store.smokingByInsured[key] = {});
        ...
        <div class="mirrorsAnswerBox">
          ...
          <div class="mirrorsAnswerGrid">
            <button class="mirrorsAnswerCard mirrorsAnswerCard--yes${yesSelected ? ' is-selected' : ''}" data-mirror-answer="yes" type="button">
              ...
              <small>שלב המשך השיחה ייפתח מיד לאחר האישור</small>
            </button>
          ...
          ${noSelected ? `<div class="mirrorsDeclinedNote">הלקוח לא אישר המשך שיחה. התהליך נעצר בשלב זה.</div>` : ``}
          ${yesSelected ? `<div class="mirrorsDeclinedNote">שלבי ההמשך הוסרו זמנית מהמערכת וייבנו מחדש בהמשך.</div>` : ``}
        </div>`;
    },
    ...
    renderVerification(rec){
      ...
      this.els.verifyCard.innerHTML = `<div class="mirrorsCard__head">
          <div>
            ...
            <div class="mirrorsCard__hint">ברשותך אשאל אותך מספר שאלות. אמת מול הלקוח את פרטי המבוטח, בחר אופן קבלת דיוורים, השלם תשובות חסרות ושמור את שלב האימות.</div>
          </div>
        ...
        ${(() => {
          const smokingEligible = this.getSmokingEligibleInsureds(rec);
          if(!smokingEligible.length){
            return `<div class="mirrorsFormBlock">
              <div class="mirrorsFormBlock__title">שאלת עישון</div>
              <div class="mirrorsDeliveryNote">לא קיימים מבוטחים בגירים בשלב זה, לכן שאלת העישון לא נדרשת.</div>
            </div>`;
          }
          return `<div class="mirrorsFormBlock">
            <div class="mirrorsFormBlock__title">שאלת עישון</div>
            <div class="mirrorsFormBlock__hint">האם עישנת או שאתה מעשן בשנתיים האחרונות סיגריות, טבק, סיגריה אלקטרונית, נרגילה, קנאביס או מוצרי טבק אחרים. השאלה תופיע רק למבוטחים בגירים: ראשי / משני / בגיר. קטין לא נשאל על עישון.</div>
            ${smokingEligible.map(({ ins, index }) => {
              const insuredKey = this.getInsuredKey(ins, index);
              const entry = this.getSmokingEntry(store, insuredKey);
              const insuredTitle = `${this.getInsuredLabel(ins, index)} · ${this.getInsuredDisplayName(ins, index)}`;
              const answer = safeTrim(entry.answer);
              return `<div class="mirrorsSmokePerson">
                <div class="mirrorsSmokePerson__title">${escapeHtml(insuredTitle)}</div>
                <div class="mirrorsChoiceGrid mirrorsChoiceGrid--smoke">
                  <button class="mirrorsMiniChoice${answer === 'yes' ? ' is-selected' : ''}" data-mirror-smoking="${escapeHtml(insuredKey)}:yes" type="button">כן</button>
                  <button class="mirrorsMiniChoice${answer === 'no' ? ' is-selected' : ''}" data-mirror-smoking="${escapeHtml(insuredKey)}:no" type="button">לא</button>
                </div>
                <div class="mirrorsSmokeQuestion">האם עישנת או שאתה מעשן בשנתיים האחרונות סיגריות, טבק, סיגריה אלקטרונית, נרגילה, קנאביס או מוצרי טבק אחרים?</div>
                ${answer === 'yes' ? `
                  <div class="mirrorsSmokeBox">
                    <div class="mirrorsSmokeBox__title">סמן איזה מוצר המבוטח מעשן</div>
                    <div class="mirrorsSmokeProducts">${smokingOptions.map(option => {
                      const checked = (entry.products || []).includes(option);
                      return `<label class="mirrorsCheckTag${checked ? ' is-selected' : ''}"><input data-mirror-smoke-product="${escapeHtml(insuredKey)}" type="checkbox" value="${escapeHtml(option)}" ${checked ? 'checked' : ''} /><span>${escapeHtml(option)}</span></label>`;
                    }).join('')}</div>
                    <div class="field mirrorsInlineField">
                      <label class="label">כמות</label>
                      <input class="input" data-mirror-smoke-qty="${escapeHtml(insuredKey)}" type="text" value="${escapeHtml(entry.quantity || '')}" placeholder="לדוגמה: 10 סיגריות ביום" />
                    </div>
                  </div>
                ` : ''}
              </div>`;
    ...
    async saveVerification(options = {}){
      ...
      const deliveryMethod = safeTrim(store.deliveryMethod);
      const smokingEligible = this.getSmokingEligibleInsureds(rec);
      for(const { ins, index } of smokingEligible){
        const insuredKey = this.getInsuredKey(ins, index);
        const entry = this.getSmokingEntry(store, insuredKey);
        if(!safeTrim(entry.answer)){
          alert(`יש לסמן תשובת עישון עבור ${this.getInsuredDisplayName(ins, index)}.`);
          return false;
        }
        if(safeTrim(entry.answer) === 'yes'){
          if(!Array.isArray(entry.products) || !entry.products.length){
            alert(`יש לסמן לפחות מוצר עישון אחד עבור ${this.getInsuredDisplayName(ins, index)}.`);
            return false;
          }
          if(!safeTrim(entry.quantity)){
            alert(`יש למלא כמות עישון עבור ${this.getInsuredDisplayName(ins, index)}.`);
            return false;
          }
        }
      }
      const primaryInsured = this.getInsureds(rec)[0];
      if(primaryInsured){
        const primaryKey = this.getInsuredKey(primaryInsured, 0);
        const primaryEntry = this.getSmokingEntry(store, primaryKey);
        store.smokingAnswer = safeTrim(primaryEntry.answer);
        store.smokingProducts = Array.isArray(primaryEntry.products) ? primaryEntry.products : [];
        store.smokingQuantity = safeTrim(primaryEntry.quantity);
      }
      if(!deliveryMethod){
