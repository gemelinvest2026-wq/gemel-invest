...
// Listen to events from the GOFI iframe (optional)
  ...
  // TalkMe (Talk Mirror) bridge: allow embedded TalkMe repo to read customers/proposals from this CRM
  function getTalkMirrorOrigin_(){
    try{
      const u = new URL(String(TALK_MIRROR_URL||'').trim());
      return u.origin;
    }catch(_){
      return null;
    }
  ...
  function talkMeCustomersPayload_(){
    // keep minimal fields for search + display (safe)
  ...
  function talkMeProposalPayload_(proposal){
    if(!proposal) return null;
    // pick only what TalkMe needs
    return {
      id: proposal.id || '',
  ...
  window.addEventListener('message', (event) => {
    const origin = getTalkMirrorOrigin_();
    if(!origin || event.origin !== origin) return;

    const data = event.data || {};
    if(!data || typeof data !== 'object') return;

    // Phase 1: serve customers list
    if(data.type === 'TALKME_REQUEST_CUSTOMERS'){
      try{
        event.source && event.source.postMessage({
          type:'TALKME_CUSTOMERS',
          customers: talkMeCustomersPayload_(),
          ts: Date.now()
        }, origin);
      }catch(_){}
      return;
    ...
    // Phase 2: serve latest proposal for a specific customer (by id)
    if(data.type === 'TALKME_REQUEST_PROPOSAL'){
      const cid = String(data.customerId||'').trim();
      if(!cid) return;
      const p = latestProposalForCustomer(cid);
      try{
        event.source && event.source.postMessage({
          type:'TALKME_PROPOSAL',
          customerId: cid,
          proposal: talkMeProposalPayload_(p),
          ts: Date.now()
        }, origin);
      }catch(_){}
      return;
