...
(() => {
  ...
  const BUILD = "20260328-clal-risk-route-v1";
...
const SystemRepairUI = {
  ...
  const ClalRiskWizard = {
    ...
    defaultState(){
      return {
        insured: { firstName:"", lastName:"", idNumber:"", birthDate:"", gender:"", maritalStatus:"", childrenCount:"", mobile:"", email:"", occupation:"" },
        risk: { isSmoker:"", workHazards:"", hobbyHazards:"", abroad180:"", israeliCitizen:"" },
        beneficiaries: [{ fullName:"", idNumber:"", relationship:"", sharePercent:"100" }],
        basics: { startDate:"", docsDelivery:"דיגיטלי", initiatedMarketing:"לא" },
        covers: { sapphire:false, sapphire10:false, sapphire15:false, accidentalDeath:false, accidentalDisability:false, waiverAndDisability:false }
      };
    },
    reset(ctx = {}){
    ...
    saveDraft(showDone = false){
      try{ localStorage.setItem("GI_CLAL_RISK_DRAFT_V1", JSON.stringify({ context:this.context, state:this.state, updatedAt: nowISO() })); }catch(_e){}
      if(showDone) this.els.meta.textContent = "טיוטת כלל ריסק נשמרה. ה־PDF הייעודי יחובר בפעימה הבאה.";
    ...
    handleField(ev){
      ...
      ref[path[0]] = val;
      if(this.step === 3 && el.name.startsWith('beneficiaries.')) this.render();
    },
    render(){
      if(!this.els.modal) return;
      ...
      this.els.meta.textContent = `פאזה 1 · ${this.context.company} ${this.context.product} · שלב ${this.step} מתוך ${this.steps.length}`;
    ...
    renderCurrentStep(){
      ...
      if(this.step === 1){
        return `<div class="lcClalRiskGrid">
          <div class="lcClalRiskCard"><div class="lcClalRiskCard__title">פרטי מועמד לביטוח</div>${this.input('שם פרטי','insured.firstName',s.insured.firstName)}${this.input('שם משפחה','insured.lastName',s.insured.lastName)}${this.input('תעודת זהות','insured.idNumber',s.insured.idNumber,'text','inputmode="numeric"')}${this.input('תאריך לידה','insured.birthDate',s.insured.birthDate,'text','placeholder="DD/MM/YYYY" inputmode="numeric"')}${this.select('מין','insured.gender',s.insured.gender,['','זכר','נקבה'])}</div>
          <div class="lcClalRiskCard"><div class="lcClalRiskCard__title">פרטי קשר וסטטוס משפחתי</div>${this.select('מצב משפחתי','insured.maritalStatus',s.insured.maritalStatus,['','רווק','נשוי','גרוש','אלמן'])}${this.input('מספר ילדים','insured.childrenCount',s.insured.childrenCount,'text','inputmode="numeric"')}${this.input('נייד','insured.mobile',s.insured.mobile,'text','inputmode="numeric"')}${this.input('דוא״ל','insured.email',s.insured.email,'email')}${this.input('עיסוק','insured.occupation',s.insured.occupation)}</div>
        </div>`;
      }
      if(this.step === 2){
        return `<div class="lcClalRiskGrid">
          <div class="lcClalRiskCard"><div class="lcClalRiskCard__title">שאלות סיכון ראשוניות</div>${this.select('האם מעשן/ת בשנתיים האחרונות?','risk.isSmoker',s.risk.isSmoker,['','כן','לא'])}${this.select('חשיפה לסיכונים בעבודה','risk.workHazards',s.risk.workHazards,['','כן','לא'])}${this.select('חשיפה לספורט/תחביב מסוכן','risk.hobbyHazards',s.risk.hobbyHazards,['','כן','לא'])}</div>
          <div class="lcClalRiskCard"><div class="lcClalRiskCard__title">שהייה ואזרחות</div>${this.select('שהייה בחו״ל 180+ יום','risk.abroad180',s.risk.abroad180,['','כן','לא'])}${this.select('אזרחות ישראלית','risk.israeliCitizen',s.risk.israeliCitizen,['','כן','לא'])}<div class="muted small">שלב זה משקף את שאלות הסיכון הראשוניות מתוך טופס כלל ריסק.</div></div>
        </div>`;
      }
      if(this.step === 3){
        const ben = (s.beneficiaries||[])[0] || {};
        return `<div class="lcClalRiskGrid">
          <div class="lcClalRiskCard"><div class="lcClalRiskCard__title">מוטב ראשי</div>${this.input('שם מלא','beneficiaries.0.fullName',ben.fullName)}${this.input('תעודת זהות','beneficiaries.0.idNumber',ben.idNumber,'text','inputmode="numeric"')}${this.input('קרבה','beneficiaries.0.relationship',ben.relationship)}${this.input('חלק באחוזים','beneficiaries.0.sharePercent',ben.sharePercent,'text','inputmode="numeric"')}</div>
          <div class="lcClalRiskCard"><div class="lcClalRiskCard__title">הערה לשלב הבא</div><div class="muted">בפאזה הבאה נוסיף טבלת מוטבים מלאה עם כמה שורות, תאריך לידה וסוג מוטבים חלופי.</div></div>
        </div>`;
      }
      if(this.step === 4){
        return `<div class="lcClalRiskGrid">
          <div class="lcClalRiskCard"><div class="lcClalRiskCard__title">פרטי בסיס להצעה</div>${this.input('תאריך תחילת ביטוח','basics.startDate',s.basics.startDate,'text','placeholder="DD/MM/YYYY" inputmode="numeric"')}${this.select('אופן קבלת מסמכים','basics.docsDelivery',s.basics.docsDelivery,['דיגיטלי','דואר רגיל'])}${this.select('שיווק יזום','basics.initiatedMarketing',s.basics.initiatedMarketing,['לא','כן'])}</div>
          <div class="lcClalRiskCard"><div class="lcClalRiskCard__title">מיפוי PDF</div><div class="muted">בשלב זה אנחנו שומרים את שדות הבסיס שיידחפו אחר כך לטופס ה־PDF המקורי של כלל ריסק.</div></div>
        </div>`;
      }
      const checks = [
        ['covers.sapphire','ספיר'],['covers.sapphire10','ספיר 10'],['covers.sapphire15','ספיר 15'],['covers.accidentalDeath','מוות מתאונה'],['covers.accidentalDisability','נכות מתאונה'],['covers.waiverAndDisability','אכ״ע - פיצוי ושחרור']
      ];
      return `<div class="lcClalRiskSummary">
        <div class="lcClalRiskCard"><div class="lcClalRiskCard__title">בחירת כיסויי כלל ריסק</div><div class="lcClalRiskInline">${checks.map(([name,label]) => `<label class="lcClalRiskChip"><input type="checkbox" name="${name}" ${name.split('.').reduce((o,k)=>o&&o[k], s)?'checked':''}> <span>${escapeHtml(label)}</span></label>`).join('')}</div><div class="muted small" style="margin-top:12px;">בפאזה הבאה נוסיף סכומי ביטוח, גיל תום, הרחבות אכ״ע וחיבור PDF אמיתי.</div></div>
        <div class="lcClalRiskCard"><div class="lcClalRiskCard__title">סיכום פאזה 1</div><div class="lcClalRiskSummary">
          ...
          <div class="lcClalRiskSummaryRow"><b>כיסויים שנבחרו</b><span>${escapeHtml(checks.filter(([name]) => name.split('.').reduce((o,k)=>o&&o[k], s)).map(([,label])=>label).join(' • ') || 'לא נבחרו')}</span></div>
        </div></div>
      </div>`;
    }
  };
