...
// open health covers drawer (Health only)
    ...
    normalizeHarPolicyType(row){
      ...
      const extra = safeTrim(row?.extra);
      const source = [section, main, sub, safeTrim(row?.productType), extra].join(" | ");

      if(/משכנתא/i.test(source)) return "ריסק משכנתא";
      if(/אבדן כושר|אובדן כושר|אכ"ע/i.test(source)) return "אובדן כושר עבודה";
      if(/נכויות|נכות/i.test(source)) return "נכויות";
      if(/מקרה מוות|ביטוח חיים|חיים ואבדן כושר עבודה|חיים ואובדן כושר עבודה|ריסק/i.test(source)) return "ריסק";
      ...
      if(/בריאות|השתל|תרופ|ניתוח|ייעוץ|בדיקות|שב"ן|שב״ן|אמבולטורי/i.test(source)) return "בריאות";
      return "בריאות";
    },
    ...
    mapHarIncludedToHealthCovers(list){
      ...
      source.forEach((item) => {
        ...
        if(/ייעוץ|בדיקות|אמבולטורי/i.test(value)) mapped.add("אמבולטורי (ייעוצים ובדיקות)");
        ...
        if(/מהשקל הראשון|שקל ראשון/i.test(value)) mapped.add('ניתוחים ומחליפי ניתוח בישראל מהשקל הראשון');
        if(/אבחון מהיר/i.test(value)) mapped.add('אבחון מהיר');
      });
      return Array.from(mapped);
    },

    normalizeHarHeaderLabel(value){
    ...
    resolveHarHeaderIndex(header, aliases = []){
      const normalized = Array.isArray(header) ? header.map((cell) => this.normalizeHarHeaderLabel(cell)) : [];
      const normalizedAliases = (Array.isArray(aliases) ? aliases : []).map((item) => this.normalizeHarHeaderLabel(item)).filter(Boolean);
      for(const alias of normalizedAliases){
        const exactIndex = normalized.indexOf(alias);
        if(exactIndex !== -1) return exactIndex;
      }
      for(const alias of normalizedAliases){
        const partialIndex = normalized.findIndex((cell) => cell && (cell.includes(alias) || alias.includes(cell)));
        if(partialIndex !== -1) return partialIndex;
      }
      return -1;
    },

    isAllowedHarHealthSection(value){
      const normalized = this.normalizeHarHeaderLabel(value);
      return normalized.includes('תחום - בריאות ותאונות אישיות')
        || normalized.includes('תחום - חיים ואבדן כושר עבודה')
        || normalized.includes('תחום - חיים ואובדן כושר עבודה');
    },

    extractHarRowsFromSheet(rows = [], ins, sheetName = ''){
      if(!Array.isArray(rows) || !rows.length) return [];
      const headerIdx = rows.findIndex((row) => {
        if(!Array.isArray(row)) return false;
        const normalizedRow = row.map((cell) => this.normalizeHarHeaderLabel(cell));
        return normalizedRow.some((cell) => cell.includes('תעודת זהות'))
          && normalizedRow.some((cell) => cell.includes('פוליסה'))
          && (normalizedRow.some((cell) => cell.includes('חברה')) || normalizedRow.some((cell) => cell.includes('מבטח')));
      });
      if(headerIdx === -1) return [];

      const header = rows[headerIdx].map((x) => safeTrim(x));
      const idx = {
        idNumber: this.resolveHarHeaderIndex(header, ['תעודת זהות','מספר תעודת זהות','תז']),
        main: this.resolveHarHeaderIndex(header, ['ענף ראשי','תחום','ענף']),
        sub: this.resolveHarHeaderIndex(header, ['ענף (משני)','ענף משני','תת ענף']),
        productType: this.resolveHarHeaderIndex(header, ['סוג מוצר','מוצר','שם מוצר']),
        company: this.resolveHarHeaderIndex(header, ['חברה','חברה מבטחת','מבטח','שם חברה']),
        period: this.resolveHarHeaderIndex(header, ['תקופת ביטוח','תקופה','תקופת הפוליסה']),
        extra: this.resolveHarHeaderIndex(header, ['פרטים נוספים','פירוט נוסף','פרטים']),
        premium: this.resolveHarHeaderIndex(header, ['פרמיה בש"ח','פרמיה בשח','פרמיה','פרמיה לתשלום','סכום פרמיה']),
        premiumType: this.resolveHarHeaderIndex(header, ['סוג פרמיה','תדירות פרמיה','מחזור פרמיה']),
        policyNumber: this.resolveHarHeaderIndex(header, ['מספר פוליסה','מס׳ פוליסה','מס פוליסה','פוליסה'])
      };
      if(idx.policyNumber === -1 || idx.company === -1) return [];

      let currentSection = '';
      const extracted = [];
      rows.slice(headerIdx + 1).forEach((row) => {
        if(!Array.isArray(row)) return;
        const mainCell = idx.main > -1 ? safeTrim(row[idx.main]) : '';
        const subCell = idx.sub > -1 ? safeTrim(row[idx.sub]) : '';
        const productTypeCell = idx.productType > -1 ? safeTrim(row[idx.productType]) : '';
        const extraCell = idx.extra > -1 ? safeTrim(row[idx.extra]) : '';

        if(this.isAllowedHarHealthSection(mainCell)){
          currentSection = mainCell;
          return;
        }
        if(!currentSection && this.isAllowedHarHealthSection(subCell)){
          currentSection = subCell;
          return;
        }
        if(!this.isAllowedHarHealthSection(currentSection)) return;

        const policyNumber = idx.policyNumber > -1 ? safeTrim(row[idx.policyNumber]) : '';
        const company = this.normalizeHarCompanyName(idx.company > -1 ? row[idx.company] : '');
        if(!policyNumber || !company) return;

        const premiumRaw = Number(String(idx.premium > -1 ? (row[idx.premium] ?? '') : '').replace(/[^\d.-]/g, ''));
        const premium = Number.isFinite(premiumRaw) ? premiumRaw : 0;
        const premiumType = idx.premiumType > -1 ? safeTrim(row[idx.premiumType]) : '';
        ...
        extracted.push({
          ...
          idNumber: idx.idNumber > -1 ? safeTrim(row[idx.idNumber]) : '',
          main: mainCell,
          sub: subCell,
          productType: productTypeCell,
          company,
          period: idx.period > -1 ? safeTrim(row[idx.period]) : '',
          extra: extraCell,
          premiumType,
          policyNumber,
          monthlyPremium,
    ...
    parseHarBituachWorkbook(buffer, ins){
      ...
      sheetNames.forEach((sheetName) => {
        ...
        if(!sheet) return;
        const rows = window.XLSX.utils.sheet_to_json(sheet, { header: 1, defval: "" });
        const sheetRows = this.extractHarRowsFromSheet(rows, ins, sheetName);
        if(sheetRows.length) extracted.push(...sheetRows);
      });
      if(!extracted.length) throw new Error("מבנה קובץ הר הביטוח לא זוהה או שלא נמצאו שורות בריאות/חיים.");
      ...
      extracted.forEach((row) => {
        ...
        if(row.sourceSheet) bucket.sourceSheets.add(row.sourceSheet);
      });
      ...
      const policies = Array.from(grouped.values()).map((bucket) => {
        ...
        return {
          ...
          sourceSheets: Array.from(bucket.sourceSheets)
        };
    ...
    async handleHarBituachFile(ins, file){
      ...
      try {
        this.setHarImportState(ins, {
          status: "loading",
          fileName: file.name || "",
          message: "קורא את קובץ הר הביטוח וממפה כותרות בריאות/חיים…"
        });
        this.render();
        const buffer = await file.arrayBuffer();
        ...
        if(!policies.length){
          this.setHarImportState(ins, {
            ...
            count: 0,
            fileName: file.name || "",
            message: "לא נמצאו פוליסות מתאימות לייבוא מתוך תחומי בריאות / חיים. בדוק את מיפוי הכותרות והתחומים בקובץ."
          });
          this.render();
          return;
        }
