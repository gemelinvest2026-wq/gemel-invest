  ...
  const UsersUI = {
    ...
    _getAdminRow(){
      ...
      return {
        ...
        pin: safeTrim(adminAuth.pin) || "1234",
        role: "admin",
    ...
    openModal(mode, user){
      ...
      const isAdminSelf = !!(user && user.isSystemAdmin);
      ...
      if(E.title) {
        E.title.textContent = (this._modalMode === "edit")
          ? (isAdminSelf ? "עריכת מנהל מערכת" : "עריכת משתמש")
          : "הוסף נציג/סוכן";
      }
      ...
      setTimeout(() => (isAdminSelf ? E.username : E.name)?.focus?.(), 50);
    },
    ...
    async _saveFromModal(){
      ...
      const isAdminSelf = this._isAdminSelfId(id);
      ...
      if(isEdit && isAdminSelf){
        if(!Auth.isAdmin()){
          this._showErr(E.err, "רק מנהל מערכת יכול לערוך את המשתמש הזה");
          return;
        }
    ...
    _filtered(){
      ...
      const adminRow = this._getAdminRow();
      if(adminRow) arr.unshift(adminRow);
    ...
    render(){
      ...
      UI.els.usersTbody.innerHTML = rows.map(a => {
        const isAdminSelf = !!a.isSystemAdmin;
        const status = (a.active === false) ? "מושבת" : "פעיל";
        const role = isAdminSelf ? "מנהל מערכת" : (a.role === "manager") ? "מנהל" : (a.role === "ops") ? "תפעול" : "נציג";
        const actions = isAdminSelf
    ...
    async editUser(id){
      if(this._isAdminSelfId(id)){
        if(!Auth.isAdmin()) return;
        this.openModal("edit", this._getAdminRow());
        return;
      }
      const a = (State.data.agents || []).find(x => String(x.id) === String(id));
      if(!a) return;
    ...
    async toggleUser(id){
      if(this._isAdminSelfId(id)) return;
      const a = (State.data.agents || []).find(x => String(x.id) === String(id));
      if(!a) return;
