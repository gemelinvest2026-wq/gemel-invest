...
(() => {
  ...
  const BUILD = "20260406-phoenix-pdf-download-v45";
...
init(){
      ...
      this.els.btnDownloadPhoenixPdf = $("#lcDownloadPhoenixPdf");
      ...
      on(this.els.btnDownloadPhoenixPdf, "click", () => this.downloadPhoenixProposalPdf());
    ...
    renderFooter(){
      const phoenixDownloadVisible = !this.isElementaryFlow() && this.isPhoenixProposalFlow() && (this.step === 8 || this.step === 9);
      if(this.els.btnDownloadPhoenixPdf){
        this.els.btnDownloadPhoenixPdf.style.display = phoenixDownloadVisible ? "" : "none";
        this.els.btnDownloadPhoenixPdf.disabled = false;
      }
      if(this.isElementaryFlow()){
...
// ---------- Step 6 ----------
    ...
    isPhoenixProposalFlow(){
      return this.getPhoenixProposalPolicies().length > 0;
    },

    getPhoenixProposalPolicies(){
      return (this.newPolicies || []).filter((policy) => {
        const company = safeTrim(policy?.company);
        const type = safeTrim(policy?.type);
        return company === 'הפניקס' && ['בריאות','מחלות קשות','סרטן'].includes(type);
      });
    },

    getPhoenixProposalStore(){
      const primary = this.insureds[0] || { data:{} };
      primary.data = primary.data || {};
      if(!primary.data.phoenixProposal || typeof primary.data.phoenixProposal !== 'object') primary.data.phoenixProposal = {};
      const store = primary.data.phoenixProposal;
      if(!store.meta || typeof store.meta !== 'object') store.meta = {};
      if(!store.insureds || typeof store.insureds !== 'object') store.insureds = {};
      if(!store.payment || typeof store.payment !== 'object') store.payment = {};
      if(!store.declarations || typeof store.declarations !== 'object') store.declarations = {};
      if(!store.coverages || typeof store.coverages !== 'object') store.coverages = { byPolicyId:{} };
      if(!store.coverages.byPolicyId || typeof store.coverages.byPolicyId !== 'object') store.coverages.byPolicyId = {};
      const metaDefaults = {
        requestedStartDate:'', proposalNumber:'', supervisorName:'', agentName:'', agentNumber:'',
        needsAssessmentDone:'yes', needsAssessmentCopy:'yes', materialProvided:'yes', noDoubleInsuranceAck:'yes'
      };
      Object.keys(metaDefaults).forEach((key) => {
        if(store.meta[key] == null) store.meta[key] = metaDefaults[key];
      });
      const paymentDefaults = {
        payerType:'insured', irregularPayer:'no', relation:'', firstName:'', lastName:'', idNumber:'', businessName:'', payerKind:'id',
        street:'', houseNumber:'', city:'', zip:'', paymentMethod:'cc', collectionDay:'10', ccCompany:'', cardNumber:'', exp:'',
        holderFirstName:'', holderLastName:'', holderId:'', bank:'', branch:'', account:'', accountType:'', authorizationType:'general', chargeLimit:'', authExpiry:''
      };
      Object.keys(paymentDefaults).forEach((key) => {
        if(store.payment[key] == null) store.payment[key] = paymentDefaults[key];
      });
      const declDefaults = {
        privacyConsent:true, medicalWaiver:true, marketingConsent:true,
        insuredApproved:true, agentConfirmed:true,
        primarySignatureName:'', secondarySignatureName:'', adultChildSignatureName:'', agentSignatureName:''
      };
      Object.keys(declDefaults).forEach((key) => {
        if(store.declarations[key] == null) store.declarations[key] = declDefaults[key];
      });
      (this.insureds || []).forEach((ins) => {
        const src = ins?.data || {};
        if(!store.insureds[ins.id] || typeof store.insureds[ins.id] !== 'object') store.insureds[ins.id] = {};
        const row = store.insureds[ins.id];
        const defaults = {
          firstName: safeTrim(src.firstName),
          lastName: safeTrim(src.lastName),
          idNumber: safeTrim(src.idNumber),
          birthDate: safeTrim(src.birthDate),
          gender: safeTrim(src.gender),
          maritalStatus: safeTrim(src.maritalStatus),
          occupation: safeTrim(src.occupation),
          clinic: safeTrim(src.clinic),
          shaban: safeTrim(src.shaban),
          phone: safeTrim(src.phone),
          email: safeTrim(src.email),
          street: safeTrim(src.street),
          houseNumber: safeTrim(src.houseNumber),
          city: safeTrim(src.city),
          zip: safeTrim(src.zip),
          documentDelivery: safeTrim(src.documentDelivery || 'email'),
          basicPolicyOwned:'', basicPolicyIntent:'', over60Exception:'',
          existingHealthIndemnity:'', existingHealthAction:'', existingCritical:'', existingCriticalAction:'',
          smokingStatus:'', cigarettesPerDay:'', familyHistory:'', familyHistoryDetails:'',
          heightCm: safeTrim(src.heightCm), weightKg: safeTrim(src.weightKg)
        };
        Object.keys(defaults).forEach((key) => {
          if(row[key] == null) row[key] = defaults[key];
        });
      });
      this.getPhoenixProposalPolicies().forEach((policy) => {
        const pid = safeTrim(policy?.id);
        if(!pid) return;
        if(!store.coverages.byPolicyId[pid] || typeof store.coverages.byPolicyId[pid] !== 'object') store.coverages.byPolicyId[pid] = {};
        const row = store.coverages.byPolicyId[pid];
        const defaults = {
          selected: row.selected === false ? false : true,
          coverageAmount: safeTrim(policy?.compensation || policy?.sumInsured || policy?.coverageAmount || policy?.sum || ''),
          discountPercent: safeTrim(policy?.discountPercent || policy?.discountPct || ''),
          discountYears: safeTrim(policy?.discountYears || ''),
          notes: safeTrim(policy?.notes || ''),
          healthCovers: Array.isArray(policy?.healthCovers) ? policy.healthCovers.slice() : []
        };
        Object.keys(defaults).forEach((key) => {
          if(row[key] == null || (Array.isArray(defaults[key]) && !Array.isArray(row[key]))) row[key] = defaults[key];
        });
      });
      return store;
    },

    setPhoenixProposalValue(path, value){
      const store = this.getPhoenixProposalStore();
    ...
    getPhoenixProposalValidationIssue(){
      if(!this.isPhoenixProposalFlow()) return '';
      const store = this.getPhoenixProposalStore();
      if(!safeTrim(store.meta.requestedStartDate)) return 'יש להשלים תאריך מבוקש לתחילת הביטוח בטופס הפניקס';
      for(const ins of (this.insureds || [])){
        const row = store.insureds?.[ins.id] || {};
        const fullName = [row.firstName, row.lastName].map(safeTrim).filter(Boolean).join(' ');
        if(!safeTrim(row.firstName) || !safeTrim(row.lastName)) return `יש להשלים שם מלא עבור ${ins.label}`;
        if(!safeTrim(row.idNumber)) return `יש להשלים תעודת זהות עבור ${fullName || ins.label}`;
        if(!safeTrim(row.birthDate)) return `יש להשלים תאריך לידה עבור ${fullName || ins.label}`;
        if(!safeTrim(row.gender)) return `יש לבחור מין עבור ${fullName || ins.label}`;
        if(!safeTrim(row.clinic)) return `יש לבחור קופת חולים עבור ${fullName || ins.label}`;
        if(!safeTrim(row.occupation)) return `יש להשלים עיסוק עבור ${fullName || ins.label}`;
        if(!safeTrim(row.phone) && ins.type !== 'child') return `יש להשלים טלפון עבור ${fullName || ins.label}`;
        if(!safeTrim(row.city)) return `יש להשלים יישוב עבור ${fullName || ins.label}`;
        if(!safeTrim(row.street)) return `יש להשלים רחוב עבור ${fullName || ins.label}`;
        if(!safeTrim(row.documentDelivery)) return `יש לבחור אופן קבלת מסמכים עבור ${fullName || ins.label}`;
        if(!safeTrim(row.heightCm)) return `יש להשלים גובה עבור ${fullName || ins.label}`;
        if(!safeTrim(row.weightKg)) return `יש להשלים משקל עבור ${fullName || ins.label}`;
        if(!safeTrim(row.basicPolicyOwned)) return `יש להשלים תשובה לגבי פוליסה בסיסית עבור ${fullName || ins.label}`;
      }
      const policies = this.getPhoenixProposalPolicies();
      for(const policy of policies){
        const pid = safeTrim(policy?.id);
        const row = store.coverages?.byPolicyId?.[pid] || {};
        if(row.selected === false) continue;
        if(['מחלות קשות','סרטן'].includes(safeTrim(policy?.type)) && !safeTrim(row.coverageAmount)){
          return `יש להשלים סכום ביטוח עבור ${this.getPolicyLabel(policy)}`;
        }
      }
      if(safeTrim(store.payment.paymentMethod) === 'cc'){
        if(!safeTrim(store.payment.cardNumber)) return 'יש להשלים מספר כרטיס אשראי בטופס הפניקס';
        if(!safeTrim(store.payment.exp)) return 'יש להשלים תוקף כרטיס בטופס הפניקס';
      } else {
        if(!safeTrim(store.payment.bank) || !safeTrim(store.payment.branch) || !safeTrim(store.payment.account)) return 'יש להשלים פרטי הוראת קבע בטופס הפניקס';
      }
      if(!store.declarations.privacyConsent || !store.declarations.medicalWaiver || !store.declarations.insuredApproved || !store.declarations.agentConfirmed){
        return 'יש לאשר את כל ההצהרות החובה של טופס הפניקס';
      }
      return '';
    },

    renderPhoenixProposalMetaCard(){
      const store = this.getPhoenixProposalStore();
      const agentName = safeTrim(store.meta.agentName || Auth?.current?.name || '');
      const current = safeTrim(store.meta.agentNumber || this.getOperationalAgentNumbers()?.['הפניקס'] || '');
      const yn = (name, value, label) => `<label class="lcPhoenixChoice"><input type="radio" name="${escapeHtml(name)}" value="${escapeHtml(value)}" ${safeTrim(store.meta[name]) === value ? 'checked' : ''} data-phoenix-bind="meta.${escapeHtml(name)}" /> <span>${escapeHtml(label)}</span></label>`;
      return `<section class="lcPhoenixCard">
        <div class="lcPhoenixCard__head"><div><div class="lcPhoenixCard__eyebrow">הפניקס · עמוד 1–2</div><div class="lcPhoenixCard__title">פרטי סוכן, הצעה והתאמת צרכים</div></div><span class="badge">חובה</span></div>
        <div class="lcPhoenixGrid lcPhoenixGrid--4">
          <label class="field"><span class="label">שם הסוכן</span><input class="input" value="${escapeHtml(agentName)}" data-phoenix-bind="meta.agentName" /></label>
          <label class="field"><span class="label">מס׳ סוכן</span><input class="input" value="${escapeHtml(current)}" data-phoenix-bind="meta.agentNumber" /></label>
          <label class="field"><span class="label">שם מפקח / מנהל עסקי</span><input class="input" value="${escapeHtml(store.meta.supervisorName || '')}" data-phoenix-bind="meta.supervisorName" /></label>
          <label class="field"><span class="label">מס׳ הצעה</span><input class="input" value="${escapeHtml(store.meta.proposalNumber || '')}" data-phoenix-bind="meta.proposalNumber" /></label>
          <label class="field"><span class="label">תאריך מבוקש לתחילת הביטוח</span><input class="input" type="date" value="${escapeHtml(store.meta.requestedStartDate || '')}" data-phoenix-bind="meta.requestedStartDate" /></label>
        </div>
        <div class="lcPhoenixSplit">
          <div class="lcPhoenixMiniBlock"><div class="lcPhoenixMiniBlock__title">הליך התאמת צרכים בוצע</div><div class="lcPhoenixChoiceRow">${yn('needsAssessmentDone','yes','כן')}${yn('needsAssessmentDone','no','לא')}</div></div>
          <div class="lcPhoenixMiniBlock"><div class="lcPhoenixMiniBlock__title">נמסר העתק מסמך התאמת צרכים</div><div class="lcPhoenixChoiceRow">${yn('needsAssessmentCopy','yes','כן')}${yn('needsAssessmentCopy','no','לא')}</div></div>
          <div class="lcPhoenixMiniBlock"><div class="lcPhoenixMiniBlock__title">נמסר מידע מהותי לגבי הפוליסה</div><div class="lcPhoenixChoiceRow">${yn('materialProvided','yes','כן')}${yn('materialProvided','no','לא')}</div></div>
          <div class="lcPhoenixMiniBlock"><div class="lcPhoenixMiniBlock__title">איסור כפל ביטוח הוסבר</div><div class="lcPhoenixChoiceRow">${yn('noDoubleInsuranceAck','yes','כן')}${yn('noDoubleInsuranceAck','no','לא')}</div></div>
        </div>
      </section>`;
    },

    renderPhoenixInsuredCards(){
      const store = this.getPhoenixProposalStore();
      const genderChoices = [['זכר','זכר'],['נקבה','נקבה']];
      const maritalChoices = [['רווק/ה','ר'],['נשוי/אה','נ'],['גרוש/ה','ג'],['אלמן/ה','א']];
      const deliveryChoices = [['email','דוא״ל'],['post','דואר ישראל']];
      const ynRadio = (name, current, yes='yes', no='no') => `<label class="lcPhoenixChoice"><input type="radio" name="${escapeHtml(name)}" value="${escapeHtml(yes)}" ${current===yes ? 'checked' : ''} data-phoenix-bind="${escapeHtml(name)}" /> <span>כן</span></label><label class="lcPhoenixChoice"><input type="radio" name="${escapeHtml(name)}" value="${escapeHtml(no)}" ${current===no ? 'checked' : ''} data-phoenix-bind="${escapeHtml(name)}" /> <span>לא</span></label>`;
      return this.insureds.map((ins) => {
        const row = store.insureds?.[ins.id] || {};
        const bind = (field) => `insureds.${ins.id}.${field}`;
        return `<section class="lcPhoenixCard">
          <div class="lcPhoenixCard__head"><div><div class="lcPhoenixCard__eyebrow">${escapeHtml(ins.label)}</div><div class="lcPhoenixCard__title">פרטי מועמד ופרטי קשר</div></div><span class="badge">עמוד 1–2 + בריאות מלאה</span></div>
          <div class="lcPhoenixGrid lcPhoenixGrid--4">
            <label class="field"><span class="label">שם פרטי</span><input class="input" value="${escapeHtml(row.firstName || '')}" data-phoenix-bind="${escapeHtml(bind('firstName'))}" /></label>
            <label class="field"><span class="label">שם משפחה</span><input class="input" value="${escapeHtml(row.lastName || '')}" data-phoenix-bind="${escapeHtml(bind('lastName'))}" /></label>
            <label class="field"><span class="label">ת.ז</span><input class="input" value="${escapeHtml(row.idNumber || '')}" data-phoenix-bind="${escapeHtml(bind('idNumber'))}" /></label>
            <label class="field"><span class="label">תאריך לידה</span><input class="input" type="date" value="${escapeHtml(row.birthDate || '')}" data-phoenix-bind="${escapeHtml(bind('birthDate'))}" /></label>
            <div class="field"><span class="label">מין</span><div class="lcPhoenixChoiceRow">${genderChoices.map(([v,l]) => `<label class="lcPhoenixChoice"><input type="radio" name="gender_${escapeHtml(ins.id)}" value="${escapeHtml(v)}" ${safeTrim(row.gender)===v ? 'checked' : ''} data-phoenix-bind="${escapeHtml(bind('gender'))}" /> <span>${escapeHtml(l)}</span></label>`).join('')}</div></div>
            <div class="field"><span class="label">מצב משפחתי</span><div class="lcPhoenixChoiceRow">${maritalChoices.map(([l,v]) => `<label class="lcPhoenixChoice"><input type="radio" name="marital_${escapeHtml(ins.id)}" value="${escapeHtml(v)}" ${safeTrim(row.maritalStatus)===v ? 'checked' : ''} data-phoenix-bind="${escapeHtml(bind('maritalStatus'))}" /> <span>${escapeHtml(l)}</span></label>`).join('')}</div></div>
            <label class="field"><span class="label">עיסוק</span><input class="input" list="lcOccupationsList" value="${escapeHtml(row.occupation || '')}" data-phoenix-bind="${escapeHtml(bind('occupation'))}" /></label>
            <label class="field"><span class="label">קופת חולים</span><input class="input" list="lcClinicsList" value="${escapeHtml(row.clinic || '')}" data-phoenix-bind="${escapeHtml(bind('clinic'))}" /></label>
            <label class="field"><span class="label">שב״ן</span><input class="input" value="${escapeHtml(row.shaban || '')}" data-phoenix-bind="${escapeHtml(bind('shaban'))}" /></label>
            <label class="field"><span class="label">נייד</span><input class="input" value="${escapeHtml(row.phone || '')}" data-phoenix-bind="${escapeHtml(bind('phone'))}" /></label>
            <label class="field"><span class="label">דוא״ל</span><input class="input" value="${escapeHtml(row.email || '')}" data-phoenix-bind="${escapeHtml(bind('email'))}" /></label>
            <div class="field"><span class="label">אופן קבלת מסמכים</span><div class="lcPhoenixChoiceRow">${deliveryChoices.map(([v,l]) => `<label class="lcPhoenixChoice"><input type="radio" name="delivery_${escapeHtml(ins.id)}" value="${escapeHtml(v)}" ${safeTrim(row.documentDelivery)===v ? 'checked' : ''} data-phoenix-bind="${escapeHtml(bind('documentDelivery'))}" /> <span>${escapeHtml(l)}</span></label>`).join('')}</div></div>
            <label class="field"><span class="label">רחוב</span><input class="input" value="${escapeHtml(row.street || '')}" data-phoenix-bind="${escapeHtml(bind('street'))}" /></label>
            <label class="field"><span class="label">מס׳ בית</span><input class="input" value="${escapeHtml(row.houseNumber || '')}" data-phoenix-bind="${escapeHtml(bind('houseNumber'))}" /></label>
            <label class="field"><span class="label">יישוב</span><input class="input" value="${escapeHtml(row.city || '')}" data-phoenix-bind="${escapeHtml(bind('city'))}" /></label>
            <label class="field"><span class="label">מיקוד</span><input class="input" value="${escapeHtml(row.zip || '')}" data-phoenix-bind="${escapeHtml(bind('zip'))}" /></label>
            <label class="field"><span class="label">גובה (ס״מ)</span><input class="input" value="${escapeHtml(row.heightCm || '')}" data-phoenix-bind="${escapeHtml(bind('heightCm'))}" /></label>
            <label class="field"><span class="label">משקל</span><input class="input" value="${escapeHtml(row.weightKg || '')}" data-phoenix-bind="${escapeHtml(bind('weightKg'))}" /></label>
            <label class="field"><span class="label">עישון / מוצרי ניקוטין</span><input class="input" value="${escapeHtml(row.smokingStatus || '')}" data-phoenix-bind="${escapeHtml(bind('smokingStatus'))}" /></label>
            <label class="field"><span class="label">כמות ליום</span><input class="input" value="${escapeHtml(row.cigarettesPerDay || '')}" data-phoenix-bind="${escapeHtml(bind('cigarettesPerDay'))}" /></label>
          </div>
          <div class="lcPhoenixSplit">
            <div class="lcPhoenixMiniBlock"><div class="lcPhoenixMiniBlock__title">האם קיימת פוליסה בסיסית מלאה</div><div class="lcPhoenixChoiceRow">${ynRadio(bind('basicPolicyOwned'), safeTrim(row.basicPolicyOwned))}</div></div>
            <div class="lcPhoenixMiniBlock"><div class="lcPhoenixMiniBlock__title">אם אין פוליסה בסיסית מלאה – האם בכוונתך לרכוש במסגרת הצעה זו</div><div class="lcPhoenixChoiceRow">${ynRadio(bind('basicPolicyIntent'), safeTrim(row.basicPolicyIntent))}</div></div>
            <div class="lcPhoenixMiniBlock"><div class="lcPhoenixMiniBlock__title">חריג גיל 60+ ללא בסיס מלא</div><div class="lcPhoenixChoiceRow">${ynRadio(bind('over60Exception'), safeTrim(row.over60Exception))}</div></div>
            <div class="lcPhoenixMiniBlock"><div class="lcPhoenixMiniBlock__title">קיימת פוליסת בריאות שיפוי דומה</div><div class="lcPhoenixChoiceRow">${ynRadio(bind('existingHealthIndemnity'), safeTrim(row.existingHealthIndemnity))}</div></div>
            <div class="lcPhoenixMiniBlock"><div class="lcPhoenixMiniBlock__title">פעולה ביחס לפוליסת בריאות קיימת</div><input class="input" value="${escapeHtml(row.existingHealthAction || '')}" placeholder="לבטל / להשאיר / מעבר מקבוצתית" data-phoenix-bind="${escapeHtml(bind('existingHealthAction'))}" /></div>
            <div class="lcPhoenixMiniBlock"><div class="lcPhoenixMiniBlock__title">קיימת פוליסת מחלות קשות</div><div class="lcPhoenixChoiceRow">${ynRadio(bind('existingCritical'), safeTrim(row.existingCritical))}</div></div>
            <div class="lcPhoenixMiniBlock"><div class="lcPhoenixMiniBlock__title">אופן טיפול בפוליסת מחלות קשות קיימת</div><input class="input" value="${escapeHtml(row.existingCriticalAction || '')}" placeholder="לבטל / להשאיר / עצמאי / דרך סוכן" data-phoenix-bind="${escapeHtml(bind('existingCriticalAction'))}" /></div>
            <label class="field field--full"><span class="label">היסטוריה משפחתית / פירוט רלוונטי</span><textarea class="input" rows="2" data-phoenix-bind="${escapeHtml(bind('familyHistoryDetails'))}">${escapeHtml(row.familyHistoryDetails || '')}</textarea></label>
          </div>
        </section>`;
      }).join('');
    },

    renderPhoenixCoverageCard(){
      const store = this.getPhoenixProposalStore();
      const policies = this.getPhoenixProposalPolicies();
      return `<section class="lcPhoenixCard"><div class="lcPhoenixCard__head"><div><div class="lcPhoenixCard__eyebrow">עמוד 3</div><div class="lcPhoenixCard__title">כיסויים, סכומי ביטוח והנחות</div></div><span class="badge">מתוך הפוליסות שנבחרו</span></div>
        <div class="lcPhoenixPolicyList">${policies.map((policy) => {
          const pid = safeTrim(policy?.id);
          const row = store.coverages?.byPolicyId?.[pid] || {};
          const amountNeeded = ['מחלות קשות','סרטן'].includes(safeTrim(policy?.type));
          const healthCovers = Array.isArray(row.healthCovers) && row.healthCovers.length ? row.healthCovers.join(' · ') : this.summarizeHealthCovers(policy?.healthCovers || [], { max:3 });
          return `<article class="lcPhoenixPolicyCard">
            <div class="lcPhoenixPolicyCard__top"><label class="lcPhoenixChoice"><input type="checkbox" ${row.selected === false ? '' : 'checked'} data-phoenix-bind="coverages.byPolicyId.${escapeHtml(pid)}.selected" /> <span>${escapeHtml(this.getPolicyLabel(policy))}</span></label><span class="badge">${escapeHtml(safeTrim(policy?.type) || 'פוליסה')}</span></div>
            <div class="lcPhoenixGrid lcPhoenixGrid--3">
              ${amountNeeded ? `<label class="field"><span class="label">סכום ביטוח</span><input class="input" value="${escapeHtml(row.coverageAmount || '')}" data-phoenix-bind="coverages.byPolicyId.${escapeHtml(pid)}.coverageAmount" /></label>` : `<div class="field"><span class="label">כיסויים מסומנים</span><div class="lcPhoenixStatic">${escapeHtml(healthCovers || 'ללא כיסויים')}</div></div>`}
              <label class="field"><span class="label">אחוז הנחה</span><input class="input" value="${escapeHtml(row.discountPercent || '')}" data-phoenix-bind="coverages.byPolicyId.${escapeHtml(pid)}.discountPercent" /></label>
              <label class="field"><span class="label">שנות הנחה</span><input class="input" value="${escapeHtml(row.discountYears || '')}" data-phoenix-bind="coverages.byPolicyId.${escapeHtml(pid)}.discountYears" /></label>
              <label class="field field--full"><span class="label">הערות / פירוט נוסף</span><textarea class="input" rows="2" data-phoenix-bind="coverages.byPolicyId.${escapeHtml(pid)}.notes">${escapeHtml(row.notes || '')}</textarea></label>
            </div>
          </article>`;
        }).join('')}</div>
      </section>`;
    ...
    renderPhoenixPaymentCard(){
      const store = this.getPhoenixProposalStore();
      const payment = store.payment || {};
      const payMethod = safeTrim(payment.paymentMethod || 'cc');
      return `<section class="lcPhoenixCard"><div class="lcPhoenixCard__head"><div><div class="lcPhoenixCard__eyebrow">עמוד 5 + משלם חריג</div><div class="lcPhoenixCard__title">משלם ואמצעי תשלום</div></div><span class="badge">חובה</span></div>
        <div class="lcPhoenixSplit">
          <div class="lcPhoenixMiniBlock"><div class="lcPhoenixMiniBlock__title">סוג משלם</div><div class="lcPhoenixChoiceRow"><label class="lcPhoenixChoice"><input type="radio" name="phoenix_payerType" value="insured" ${safeTrim(payment.payerType)==='insured' ? 'checked' : ''} data-phoenix-bind="payment.payerType" /> <span>המבוטח</span></label><label class="lcPhoenixChoice"><input type="radio" name="phoenix_payerType" value="external" ${safeTrim(payment.payerType)==='external' ? 'checked' : ''} data-phoenix-bind="payment.payerType" /> <span>משלם חריג</span></label></div></div>
          <div class="lcPhoenixMiniBlock"><div class="lcPhoenixMiniBlock__title">אמצעי תשלום</div><div class="lcPhoenixChoiceRow"><label class="lcPhoenixChoice"><input type="radio" name="phoenix_paymentMethod" value="cc" ${payMethod==='cc' ? 'checked' : ''} data-phoenix-bind="payment.paymentMethod" /> <span>כרטיס אשראי</span></label><label class="lcPhoenixChoice"><input type="radio" name="phoenix_paymentMethod" value="ho" ${payMethod==='ho' ? 'checked' : ''} data-phoenix-bind="payment.paymentMethod" /> <span>הוראת קבע</span></label></div></div>
          <label class="field"><span class="label">יום גבייה מבוקש</span><input class="input" value="${escapeHtml(payment.collectionDay || '')}" data-phoenix-bind="payment.collectionDay" /></label>
        </div>
        ${safeTrim(payment.payerType)==='external' ? `<div class="lcPhoenixGrid lcPhoenixGrid--4"><label class="field"><span class="label">זיקה</span><input class="input" value="${escapeHtml(payment.relation || '')}" data-phoenix-bind="payment.relation" /></label><label class="field"><span class="label">שם פרטי</span><input class="input" value="${escapeHtml(payment.firstName || '')}" data-phoenix-bind="payment.firstName" /></label><label class="field"><span class="label">שם משפחה</span><input class="input" value="${escapeHtml(payment.lastName || '')}" data-phoenix-bind="payment.lastName" /></label><label class="field"><span class="label">ת.ז / ח.פ / ע.מ</span><input class="input" value="${escapeHtml(payment.idNumber || '')}" data-phoenix-bind="payment.idNumber" /></label><label class="field"><span class="label">שם עסק</span><input class="input" value="${escapeHtml(payment.businessName || '')}" data-phoenix-bind="payment.businessName" /></label><label class="field"><span class="label">רחוב</span><input class="input" value="${escapeHtml(payment.street || '')}" data-phoenix-bind="payment.street" /></label><label class="field"><span class="label">מס׳ בית</span><input class="input" value="${escapeHtml(payment.houseNumber || '')}" data-phoenix-bind="payment.houseNumber" /></label><label class="field"><span class="label">יישוב</span><input class="input" value="${escapeHtml(payment.city || '')}" data-phoenix-bind="payment.city" /></label><label class="field"><span class="label">מיקוד</span><input class="input" value="${escapeHtml(payment.zip || '')}" data-phoenix-bind="payment.zip" /></label></div>` : ''}
        ${payMethod==='cc' ? `<div class="lcPhoenixGrid lcPhoenixGrid--4"><label class="field"><span class="label">חברת אשראי</span><input class="input" value="${escapeHtml(payment.ccCompany || '')}" data-phoenix-bind="payment.ccCompany" /></label><label class="field"><span class="label">מס׳ כרטיס</span><input class="input" value="${escapeHtml(payment.cardNumber || '')}" data-phoenix-bind="payment.cardNumber" /></label><label class="field"><span class="label">תוקף</span><input class="input" value="${escapeHtml(payment.exp || '')}" data-phoenix-bind="payment.exp" /></label><label class="field"><span class="label">ת.ז בעל הכרטיס</span><input class="input" value="${escapeHtml(payment.holderId || '')}" data-phoenix-bind="payment.holderId" /></label><label class="field"><span class="label">שם פרטי בעל הכרטיס</span><input class="input" value="${escapeHtml(payment.holderFirstName || '')}" data-phoenix-bind="payment.holderFirstName" /></label><label class="field"><span class="label">שם משפחה בעל הכרטיס</span><input class="input" value="${escapeHtml(payment.holderLastName || '')}" data-phoenix-bind="payment.holderLastName" /></label></div>` : `<div class="lcPhoenixGrid lcPhoenixGrid--4"><label class="field"><span class="label">בנק</span><input class="input" value="${escapeHtml(payment.bank || '')}" data-phoenix-bind="payment.bank" /></label><label class="field"><span class="label">סניף</span><input class="input" value="${escapeHtml(payment.branch || '')}" data-phoenix-bind="payment.branch" /></label><label class="field"><span class="label">מס׳ חשבון</span><input class="input" value="${escapeHtml(payment.account || '')}" data-phoenix-bind="payment.account" /></label><label class="field"><span class="label">סוג חשבון</span><input class="input" value="${escapeHtml(payment.accountType || '')}" data-phoenix-bind="payment.accountType" /></label></div>`}
      </section>`;
    },

    renderPhoenixDeclarationsCard(){
      const d = this.getPhoenixProposalStore().declarations || {};
      const check = (path, label, required=true) => `<label class="lcPhoenixCheck ${required ? 'is-required' : ''}"><input type="checkbox" ${d[path] ? 'checked' : ''} data-phoenix-bind="declarations.${escapeHtml(path)}" /> <span>${escapeHtml(label)}</span></label>`;
      return `<section class="lcPhoenixCard"><div class="lcPhoenixCard__head"><div><div class="lcPhoenixCard__eyebrow">עמודים 3–5</div><div class="lcPhoenixCard__title">הצהרות, הסכמות וחתימות</div></div><span class="badge">אישור לפני הפקה</span></div>
        <div class="lcPhoenixChecks">${check('privacyConsent','אישור הגנת פרטיות', true)}${check('medicalWaiver','ויתור על סודיות רפואית', true)}${check('marketingConsent','הסכמה לקבלת מסרים שיווקיים', false)}${check('insuredApproved','המועמד מאשר את פרטי ההצעה', true)}${check('agentConfirmed','הסוכן מאשר כי שאל את כל השאלות', true)}</div>
        <div class="lcPhoenixGrid lcPhoenixGrid--4">
          <label class="field"><span class="label">חתימת מועמד ראשי (שם)</span><input class="input" value="${escapeHtml(d.primarySignatureName || '')}" data-phoenix-bind="declarations.primarySignatureName" /></label>
          <label class="field"><span class="label">חתימת מועמד שני (שם)</span><input class="input" value="${escapeHtml(d.secondarySignatureName || '')}" data-phoenix-bind="declarations.secondarySignatureName" /></label>
          <label class="field"><span class="label">חתימת ילד בוגר (שם)</span><input class="input" value="${escapeHtml(d.adultChildSignatureName || '')}" data-phoenix-bind="declarations.adultChildSignatureName" /></label>
          <label class="field"><span class="label">חתימת הסוכן (שם)</span><input class="input" value="${escapeHtml(d.agentSignatureName || '')}" data-phoenix-bind="declarations.agentSignatureName" /></label>
        </div>
      </section>`;
    },

    renderPhoenixProposalShell(){
      if(!this.isPhoenixProposalFlow()) return '';
      const selection = this.getHealthMasterSelection();
      return `<div class="lcPhoenixShell">
        <div class="lcPhoenixHero"><div><div class="lcPhoenixHero__kicker">טופס הצעה מקורי · הפניקס</div><div class="lcPhoenixHero__title">השלמת 100% שדות להצעת בריאות / מחלות קשות / סרטן</div><div class="lcPhoenixHero__text">המסך הזה מרכז את כל השדות שחסרים כדי להפיק בהמשך את טופס ההצעה המקורי של הפניקס. ההצהרה הרפואית הדינמית שלמטה נשמרת כחלק בלתי נפרד מהטופס.</div></div><div class="lcPhoenixHero__meta">${escapeHtml(selection?.sourceLabel || 'הפניקס')}</div></div>
        ${this.renderPhoenixProposalMetaCard()}
        ${this.renderPhoenixInsuredCards()}
        ${this.renderPhoenixCoverageCard()}
        ${this.renderPhoenixPaymentCard()}
        ${this.renderPhoenixDeclarationsCard()}
      </div>`;
    },
    ...
    renderStep8(){
      ...
      const list = this.getHealthQuestionList();
      const store = this.getHealthStore();
      const phoenixShell = this.renderPhoenixProposalShell();
      if(!list.length){
        return `${phoenixShell}<div class="lcHealthEmpty"><div class="lcHealthEmpty__icon">🩺</div><div class="lcHealthEmpty__title">טופס הצעה והצהרת בריאות</div><div class="lcHealthEmpty__text">כדי להציג את שלב 8 יש לבחור בשלב 5 פוליסה רלוונטית. בפוליסות הפניקס המערכת תטען הצהרת בריאות ייעודית לפי חברה + מוצר יחד עם שדות טופס ההצעה המלאים.</div></div>`;
      }
      ...
      return `${phoenixShell}<div class="lcHLayout">
        <div class="lcHMain">
    ...
    bindHealthInputs(){
      const store = this.getHealthStore();
      $$('[data-phoenix-bind]', this.els.body).forEach(el => {
        const path = safeTrim(el.getAttribute('data-phoenix-bind'));
        if(!path) return;
        const save = () => {
          const value = el.type === 'checkbox' ? !!el.checked : safeTrim(el.value);
          this.setPhoenixProposalValue(path, value);
        };
        on(el, 'input', save);
        on(el, 'change', () => {
          save();
          if(path.startsWith('payment.paymentMethod') || path.startsWith('payment.payerType')) this.render();
        });
      });
    ...
    async ensurePdfLibReady(){
      ...
      await new Promise((resolve, reject) => {
        ...
        if(window.PDFLib && window.PDFLib.PDFDocument) return done();
        script.addEventListener('load', done, { once:true });
        script.addEventListener('error', () => reject(new Error('PDF_LIB_LOAD_FAILED')), { once:true });
      });
      return window.PDFLib;
    },

    getPhoenixProposalSnapshot(){
      const store = this.getPhoenixProposalStore();
      const insureds = (this.insureds || []).map((ins) => {
        ...
        return {
          ...
          label: ins.label,
          type: ins.type,
          firstName: safeTrim(row.firstName),
          lastName: safeTrim(row.lastName),
          fullName: [row.firstName, row.lastName].map(safeTrim).filter(Boolean).join(' '),
          idNumber: safeTrim(row.idNumber),
          birthDate: safeTrim(row.birthDate),
          gender: safeTrim(row.gender),
          maritalStatus: safeTrim(row.maritalStatus),
          occupation: safeTrim(row.occupation),
          clinic: safeTrim(row.clinic),
          shaban: safeTrim(row.shaban),
          phone: safeTrim(row.phone),
          email: safeTrim(row.email),
          city: safeTrim(row.city),
          street: safeTrim(row.street),
          houseNumber: safeTrim(row.houseNumber),
          zip: safeTrim(row.zip),
          heightCm: safeTrim(row.heightCm),
          weightKg: safeTrim(row.weightKg),
          smokingStatus: safeTrim(row.smokingStatus),
          cigarettesPerDay: safeTrim(row.cigarettesPerDay),
          basicPolicyOwned: safeTrim(row.basicPolicyOwned),
          existingHealthIndemnity: safeTrim(row.existingHealthIndemnity),
          existingCritical: safeTrim(row.existingCritical)
        };
      });
      const policies = this.getPhoenixProposalPolicies().map((policy) => {
        const pid = safeTrim(policy?.id);
        const row = store.coverages?.byPolicyId?.[pid] || {};
        return {
          id: pid,
          label: this.getPolicyLabel(policy),
          type: safeTrim(policy?.type),
          selected: row.selected === false ? false : true,
          coverageAmount: safeTrim(row.coverageAmount),
          discountPercent: safeTrim(row.discountPercent),
          discountYears: safeTrim(row.discountYears),
      ...
      this.getHealthQuestionList().forEach((item) => {
        (this.insureds || []).forEach((ins) => {
          const response = this.getHealthResponse(item.question.key, ins.id);
          if(safeTrim(response?.answer) !== 'yes') return;
          const details = Object.entries(response?.fields || {})
            .filter(([,v]) => safeTrim(v))
            .map(([k,v]) => `${k}: ${safeTrim(v)}`)
            .join(' | ');
          healthItems.push(`${ins.label} · ${item.question.text}${details ? ' — ' + details : ''}`);
        });
      });
      return {
        meta: JSON.parse(JSON.stringify(store.meta || {})),
        payment: JSON.parse(JSON.stringify(store.payment || {})),
        declarations: JSON.parse(JSON.stringify(store.declarations || {})),
        insureds,
        policies,
        healthItems
      };
    ...
    async createPdfTextPng(text, options = {}){
      const content = String(text || '').trim();
      if(!content) return null;
      const canvas = document.createElement('canvas');
      const maxWidth = Number(options.maxWidth || 900);
      const fontSize = Number(options.fontSize || 18);
      const lineHeight = Number(options.lineHeight || Math.round(fontSize * 1.4));
      const paddingX = Number(options.paddingX || 12);
      const paddingY = Number(options.paddingY || 10);
      const direction = options.direction === 'ltr' ? 'ltr' : 'rtl';
      const fontFamily = options.fontFamily || 'Arial, "Noto Sans Hebrew", sans-serif';
      ...
      paragraphs.forEach((paragraph) => {
        ...
        words.forEach((word) => {
          ...
          if(ctx.measureText(trial).width > (maxWidth - paddingX * 2) && current){
            lines.push(current);
            current = word;
          } else {
            current = trial;
      ...
      canvas.width = Math.ceil(Math.min(maxWidth, Math.max(...measured, 40) + paddingX * 2));
      canvas.height = Math.ceil(lines.length * lineHeight + paddingY * 2);
      const ctx2 = canvas.getContext('2d');
      ctx2.clearRect(0, 0, canvas.width, canvas.height);
      ctx2.font = `${fontSize}px ${fontFamily}`;
      ctx2.fillStyle = options.color || '#132238';
      ctx2.textBaseline = 'top';
      ctx2.direction = direction;
      ctx2.textAlign = direction === 'rtl' ? 'right' : 'left';
      lines.forEach((line, idx) => {
        const x = direction === 'rtl' ? canvas.width - paddingX : paddingX;
    ...
    async stampPhoenixPdfText(page, text, options = {}){
      const dataUrl = await this.createPdfTextPng(text, options);
      if(!dataUrl) return;
      const pdfDoc = options.pdfDoc;
      const png = await pdfDoc.embedPng(dataUrl);
      const desiredWidth = Number(options.width || png.width);
      const scale = desiredWidth / png.width;
    ...
    formatPhoenixPolicySummaryLine(policy){
      const chunks = [safeTrim(policy.label || policy.type || 'פוליסה')];
      if(safeTrim(policy.coverageAmount)) chunks.push(`סכום: ${safeTrim(policy.coverageAmount)}`);
      if(safeTrim(policy.discountPercent)) chunks.push(`הנחה: ${safeTrim(policy.discountPercent)}%`);
      if(safeTrim(policy.discountYears)) chunks.push(`ל-${safeTrim(policy.discountYears)} שנים`);
      return chunks.join(' · ');
    },

    async downloadPhoenixProposalPdf(){
      try {
        const issue = this.getPhoenixProposalValidationIssue();
        if(issue){
          this.setHint(issue);
          return;
        }
        this.setHint('מכין את טופס הפניקס להורדה...');
        const PDFLib = await this.ensurePdfLibReady();
        const srcUrl = './phoenix-health-form.pdf';
        const res = await fetch(srcUrl);
        if(!res.ok) throw new Error('PHOENIX_PDF_FETCH_' + res.status);
        const bytes = await res.arrayBuffer();
        const pdfDoc = await PDFLib.PDFDocument.load(bytes);
        const pages = pdfDoc.getPages();
        const shot = this.getPhoenixProposalSnapshot();
        const first = shot.insureds[0] || {};
        const second = shot.insureds[1] || {};
        const selectedPolicies = shot.policies.filter((p) => p.selected !== false);

        const page1 = pages[0];
        if(page1){
          const w = page1.getWidth();
          const h = page1.getHeight();
          await this.stampPhoenixPdfText(page1, shot.meta.agentName || '', { pdfDoc, x:w-310, y:h-118, width:110, fontSize:14 });
          await this.stampPhoenixPdfText(page1, shot.meta.agentNumber || '', { pdfDoc, x:w-450, y:h-118, width:85, fontSize:14, direction:'ltr' });
          await this.stampPhoenixPdfText(page1, shot.meta.supervisorName || '', { pdfDoc, x:w-575, y:h-118, width:110, fontSize:14 });
          await this.stampPhoenixPdfText(page1, shot.meta.proposalNumber || '', { pdfDoc, x:w-690, y:h-118, width:80, fontSize:14, direction:'ltr' });
          await this.stampPhoenixPdfText(page1, shot.meta.requestedStartDate || '', { pdfDoc, x:w-165, y:h-92, width:95, fontSize:12, direction:'ltr' });
          const rows = [first, second].filter((row) => row && (row.fullName || row.idNumber));
          for(let idx = 0; idx < rows.length; idx += 1){
            const row = rows[idx];
            const y = h - 172 - (idx * 42);
            await this.stampPhoenixPdfText(page1, row.fullName || '', { pdfDoc, x:w-746, y, width:86, fontSize:12 });
            await this.stampPhoenixPdfText(page1, row.idNumber || '', { pdfDoc, x:w-642, y, width:82, fontSize:12, direction:'ltr' });
            await this.stampPhoenixPdfText(page1, row.occupation || '', { pdfDoc, x:w-555, y, width:72, fontSize:12 });
            await this.stampPhoenixPdfText(page1, row.clinic || '', { pdfDoc, x:w-474, y, width:64, fontSize:12 });
            await this.stampPhoenixPdfText(page1, row.birthDate || '', { pdfDoc, x:w-102, y, width:70, fontSize:12, direction:'ltr' });
            const contactY = h - 440 - (idx * 36);
            await this.stampPhoenixPdfText(page1, row.phone || '', { pdfDoc, x:w-145, y:contactY, width:80, fontSize:11, direction:'ltr' });
            await this.stampPhoenixPdfText(page1, row.email || '', { pdfDoc, x:w-285, y:contactY, width:120, fontSize:10, direction:'ltr' });
            await this.stampPhoenixPdfText(page1, `${row.street || ''} ${row.houseNumber || ''}`.trim(), { pdfDoc, x:w-425, y:contactY, width:110, fontSize:10 });
            await this.stampPhoenixPdfText(page1, row.city || '', { pdfDoc, x:w-545, y:contactY, width:84, fontSize:10 });
          }
        }

        const page2 = pages[1];
        if(page2){
          const w = page2.getWidth();
          const h = page2.getHeight();
          await this.stampPhoenixPdfText(page2, `התאמת צרכים: ${shot.meta.needsAssessmentDone === 'yes' ? 'כן' : 'לא'} | העתק נמסר: ${shot.meta.needsAssessmentCopy === 'yes' ? 'כן' : 'לא'} | מידע מהותי נמסר: ${shot.meta.materialProvided === 'yes' ? 'כן' : 'לא'}`, { pdfDoc, x:48, y:h-180, width:w-96, fontSize:12 });
          const existing = [first, second].filter(Boolean).map((row) => `${row.label}: בסיסית ${row.basicPolicyOwned || '—'} | בריאות קיימת ${row.existingHealthIndemnity || '—'} | מחלות קשות ${row.existingCritical || '—'}`).join('\n');
          await this.stampPhoenixPdfText(page2, existing, { pdfDoc, x:52, y:h-570, width:w-104, fontSize:11 });
        }

        const page3 = pages[2];
        if(page3){
          const w = page3.getWidth();
          const h = page3.getHeight();
          const policyText = selectedPolicies.length ? selectedPolicies.map((p) => `• ${this.formatPhoenixPolicySummaryLine(p)}`).join('\n') : 'לא סומנו פוליסות פעילות להפקה';
          await this.stampPhoenixPdfText(page3, policyText, { pdfDoc, x:56, y:88, width:w-112, fontSize:13 });
        }

        const page5 = pages[4];
        if(page5){
          const w = page5.getWidth();
          const h = page5.getHeight();
          const paymentChunks = [
            `סוג משלם: ${shot.payment.payerType === 'external' ? 'משלם חריג' : 'המבוטח'}`,
            `אמצעי תשלום: ${shot.payment.paymentMethod === 'ho' ? 'הוראת קבע' : 'כרטיס אשראי'}`,
            shot.payment.paymentMethod === 'ho'
              ? `בנק ${shot.payment.bank || ''} | סניף ${shot.payment.branch || ''} | חשבון ${shot.payment.account || ''}`
              : `כרטיס ${shot.payment.cardNumber || ''} | תוקף ${shot.payment.exp || ''} | בעל כרטיס ${[shot.payment.holderFirstName, shot.payment.holderLastName].map(safeTrim).filter(Boolean).join(' ')}`
          ].filter(Boolean).join('\n');
          await this.stampPhoenixPdfText(page5, paymentChunks, { pdfDoc, x:50, y:h-210, width:w-100, fontSize:12 });
          const signLine = [shot.declarations.primarySignatureName, shot.declarations.secondarySignatureName, shot.declarations.agentSignatureName].map(safeTrim).filter(Boolean).join(' | ');
          await this.stampPhoenixPdfText(page5, signLine, { pdfDoc, x:60, y:100, width:w-120, fontSize:12 });
        }

        const page6 = pages[5];
        if(page6){
          const w = page6.getWidth();
          const h = page6.getHeight();
          const healthSummary = shot.healthItems.length
            ? shot.healthItems.slice(0, 8).map((line) => `• ${line}`).join('\n')
            : 'לא נמצאו ממצאים חיוביים בהצהרת הבריאות.';
          await this.stampPhoenixPdfText(page6, healthSummary, { pdfDoc, x:44, y:54, width:w-88, fontSize:11 });
        }

        const summaryPage = pdfDoc.addPage([595.28, 841.89]);
        await this.stampPhoenixPdfText(summaryPage, 'סיכום נתוני הצעה – הפניקס', { pdfDoc, x:180, y:790, width:240, fontSize:22 });
        await this.stampPhoenixPdfText(summaryPage, `מבוטח ראשי: ${first.fullName || ''}\nת.ז: ${first.idNumber || ''}\nתאריך לידה: ${first.birthDate || ''}\nטלפון: ${first.phone || ''}`, { pdfDoc, x:60, y:680, width:475, fontSize:14 });
        await this.stampPhoenixPdfText(summaryPage, `כיסויים שנבחרו:\n${selectedPolicies.map((p) => this.formatPhoenixPolicySummaryLine(p)).join('\n') || 'לא נבחרו כיסויים'}`, { pdfDoc, x:60, y:470, width:475, fontSize:13 });
        await this.stampPhoenixPdfText(summaryPage, `משלם: ${shot.payment.payerType === 'external' ? 'משלם חריג' : 'המבוטח'}\nאמצעי תשלום: ${shot.payment.paymentMethod === 'ho' ? 'הוראת קבע' : 'כרטיס אשראי'}\nיום גבייה: ${shot.payment.collectionDay || ''}`, { pdfDoc, x:60, y:340, width:475, fontSize:13 });
        await this.stampPhoenixPdfText(summaryPage, `הצהרת בריאות – ממצאים חיוביים:\n${shot.healthItems.slice(0, 10).join('\n') || 'ללא ממצאים חיוביים'}`, { pdfDoc, x:60, y:110, width:475, fontSize:12 });

        const outBytes = await pdfDoc.save();
        const blob = new Blob([outBytes], { type: 'application/pdf' });
        const link = document.createElement('a');
        link.href = URL.createObjectURL(blob);
        const fileName = `phoenix-proposal-${safeTrim(first.idNumber || 'customer') || 'customer'}.pdf`;
        link.download = fileName;
        document.body.appendChild(link);
        link.click();
        link.remove();
        setTimeout(() => URL.revokeObjectURL(link.href), 1500);
        this.setHint('טופס הפניקס ירד בהצלחה לבדיקה');
      } catch (err) {
        console.error(err);
        this.setHint('אירעה שגיאה בהכנת טופס הפניקס להורדה');
      }
    },

    getHealthBlockingIssue(){
      const phoenixIssue = this.getPhoenixProposalValidationIssue();
      if(phoenixIssue) return { ok:false, msg:phoenixIssue };
      const list = this.getHealthQuestionList();
