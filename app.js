...
txt.textContent = "";
  ...
  const CustomersUI = {
    ...
    init(){
      ...
      on(UI.els.customersTbody, "click", (ev) => {
        ...
        if(archiveBtn){
          ...
          this.handleArchiveCustomerClick(ev, customerId);
        }
      });
    ...
    bindRowActionButtons(){
      ...
      $$('[data-archive-customer]', UI.els.customersTbody).forEach(btn => {
        btn.onclick = (ev) => this.handleArchiveCustomerClick(ev, btn.getAttribute('data-archive-customer'));
      });
    },
