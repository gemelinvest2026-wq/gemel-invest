    ...
    reset(){
      const make = (type, label) => ({
        ...
        data: {
          ...
          newPolicies: [],
          // payer (Step 6)
payerMode:"insured", // insured/other
selectedPayerId:"", // insured id when payerMode=insured
otherPayer: { relation:"", fullName:"", idNumber:"", birthDate:"" },
paymentMethod:"cc", // cc/ho
cc: { holderName:"", holderId:"", last4:"", expMonth:"", expYear:"" },
ho: { bankName:"", bankNo:"", branch:"", account:"", accountHolderName:"", accountHolderId:"" },
// NOTE: never persist full card details to Sheets
tempCard: { full:"", expiresAt:null }
}
      });
    ...
    renderBody(){
      ...
      else if(this.step === 6) this.bindPayerStep(this.insureds[0]);
    },
...
// Step 6 only: bind payer inputs (data-payer) + premium payer cards
bindPayerStep(ins){
  if(!this.els.body) return;
  const d = ins?.data || {};

  // change handlers for data-payer
  $$("[data-payer]", this.els.body).forEach(el => {
    on(el, "change", () => {
      const k = el.getAttribute("data-payer");
      let v = (el.type === "checkbox") ? !!el.checked : safeTrim(el.value);

      // last4: keep digits only, max 4
      if(k === "cc.last4"){
        v = String(v||"").replace(/\D/g,"").slice(-4);
        if(el.value !== v) el.value = v;
      }
      // exp month/year: digits only
      if(k === "cc.expMonth"){
        v = String(v||"").replace(/\D/g,"").slice(0,2);
        if(el.value !== v) el.value = v;
      }
      if(k === "cc.expYear"){
      ...
      // bank numbers: digits only (soft)
      if(k === "ho.bankNo" || k === "ho.branch" || k === "ho.account"){
        v = String(v||"").replace(/\D/g,"").slice(0,20);
        if(el.value !== v) el.value = v;
      }
      if(k === "otherPayer.idNumber"){
        v = String(v||"").replace(/\D/g,"").slice(0,20);
        if(el.value !== v) el.value = v;
      }
      ...
      // if switched payerMode, clear irrelevant fields
      if(k === "payerMode"){
        if(v === "insured"){
          d.otherPayer = { relation:"", fullName:"", idNumber:"", birthDate:"" };
        }else{
          d.selectedPayerId = "";
        }
      }
  ...
  // payer cards (premium)
  $$("[data-payerpick]", this.els.body).forEach(btn => {
    on(btn, "click", () => {
      const mode = btn.getAttribute("data-payerpick");
      if(mode === "other"){
        d.payerMode = "other";
        d.selectedPayerId = "";
      }else{
        d.payerMode = "insured";
        d.selectedPayerId = mode; // insured id
      }
      this.render();
    });
  });
  ...
  $$("[data-paymethod]", this.els.body).forEach(btn => {
    on(btn, "click", () => {
      const v = btn.getAttribute("data-paymethod");
      if(!v) return;
      d.paymentMethod = v;
      this.render();
    });
...
renderStep6(ins){
  ...
  // ---- migrate legacy keys (if user opened old step before) ----
  if(!d.payerMode){
    const legacyChoice = safeTrim(d.payerChoice || "insured");
    d.payerMode = (legacyChoice === "external") ? "other" : "insured";
    const ex = d.externalPayer || {};
    d.otherPayer = {
      relation: safeTrim(ex.relation),
      fullName: (safeTrim(ex.firstName) + " " + safeTrim(ex.lastName)).trim(),
      idNumber: safeTrim(ex.idNumber),
      birthDate: safeTrim(ex.birthDate)
    };
  }
  if(!d.cc || typeof d.cc !== "object") d.cc = { holderName:"", holderId:"", last4:"", expMonth:"", expYear:"" };
  if(!d.ho || typeof d.ho !== "object") d.ho = { bankName:"", bankNo:"", branch:"", account:"", accountHolderName:"", accountHolderId:"" };
  if(!d.otherPayer || typeof d.otherPayer !== "object") d.otherPayer = { relation:"", fullName:"", idNumber:"", birthDate:"" };
  if(!d.paymentMethod) d.paymentMethod = "cc";

  const primary = this.insureds[0] || null;
  const primaryId = primary?.id || "";
  const eligible = (this.insureds || [])
    .filter(x => x && x.type !== "child")
    .map(x => ({
      id: x.id,
      label: x.label,
  ...
  // rules: if no spouse/adult -> only primary or other (still allow any non-child, but we enforce eligibility list)
  const eligibleIds = eligible.map(x => x.id);
  if(d.payerMode === "insured"){
    if(!safeTrim(d.selectedPayerId)) d.selectedPayerId = primaryId;
    if(d.selectedPayerId && !eligibleIds.includes(d.selectedPayerId)) d.selectedPayerId = primaryId;
  }

  const payerIns = (d.payerMode === "insured") ? (this._findInsuredById(d.selectedPayerId) || primary) : null;
  const snap = (d.payerMode === "insured") ? {
    name: (safeTrim(payerIns?.data?.firstName) + " " + safeTrim(payerIns?.data?.lastName)).trim(),
    id: safeTrim(payerIns?.data?.idNumber),
    birth: safeTrim(payerIns?.data?.birthDate)
  } : {
    name: safeTrim(d.otherPayer.fullName),
    id: safeTrim(d.otherPayer.idNumber),
    birth: safeTrim(d.otherPayer.birthDate)
  };

  // always sync holder identity from chosen payer (prevents mismatches)
  d.cc.holderName = snap.name;
  d.cc.holderId = snap.id;
  d.ho.accountHolderName = snap.name;
  d.ho.accountHolderId = snap.id;

  const payerCards = eligible.map(x => {
    const active = (d.payerMode === "insured" && safeTrim(d.selectedPayerId) === x.id);
    return `
      <button type="button" class="lcPayerCard ${active ? "is-active":""}" data-payerpick="${escapeHtml(x.id)}">
        <div class="lcPayerCard__top">
          <div class="lcPayerCard__name">${escapeHtml(x.name)}</div>
          <div class="lcPayerCard__badge">${escapeHtml(x.label)}</div>
        </div>
        <div class="lcPayerCard__sub">${escapeHtml(safeTrim(x.id) ? "ת״ז תתמלא אוטומטית" : "")}</div>
      </button>
    `;
  }).join("");

  const otherActive = (d.payerMode === "other");
  const otherCard = `
    <button type="button" class="lcPayerCard ${otherActive ? "is-active":""}" data-payerpick="other">
      <div class="lcPayerCard__top">
        <div class="lcPayerCard__name">אחר</div>
        <div class="lcPayerCard__badge">קרבה</div>
      </div>
      <div class="lcPayerCard__sub">אב / אם / אח / אחות / קרוב משפחה / אחר</div>
    </button>
  `;

  const relOpts = ["","אב","אם","אח","אחות","קרוב משפחה","אחר"].map(r => 
    `<option value="${escapeHtml(r)}"${safeTrim(d.otherPayer.relation)===r?" selected":""}>${escapeHtml(r || "בחר…")}</option>`
  ).join("");

  const payMethod = safeTrim(d.paymentMethod || "cc");
  const pmPills = `
    <div class="lcPayPills" role="tablist" aria-label="אמצעי תשלום">
      <button type="button" class="lcPayPill ${payMethod==="cc"?"is-active":""}" data-paymethod="cc">כרטיס אשראי</button>
      <button type="button" class="lcPayPill ${payMethod==="ho"?"is-active":""}" data-paymethod="ho">הוראת קבע</button>
    </div>
  `;

  const payerSummary = `
    <div class="lcPayerSummary">
      <div class="lcPayerSummary__title">פרטי משלם</div>
      <div class="lcPayerSummary__row">
        <span class="lcDot"></span>
        <span><b>${escapeHtml(snap.name || "—")}</b></span>
        <span class="muted">·</span>
        <span dir="ltr">${escapeHtml(snap.id || "—")}</span>
        ${snap.birth ? `<span class="muted">·</span><span dir="ltr">${escapeHtml(snap.birth)}</span>` : ``}
      </div>
      <div class="help">שם + ת״ז מתמלאים אוטומטית מהמשלם שנבחר.</div>
    </div>
  `;

  const otherForm = otherActive ? `
    <div class="lcGlassCard" style="margin-top:12px">
      <div class="lcGlassCard__title">פרטי משלם · אחר</div>
      <div class="lcWGrid">
        <div class="field">
          <label class="label">קרבה</label>
          <select class="input" data-payer="otherPayer.relation">${relOpts}</select>
        </div>
        <div class="field">
          <label class="label">שם מלא</label>
          <input class="input" data-payer="otherPayer.fullName" value="${escapeHtml(d.otherPayer.fullName||"")}" />
        </div>
        <div class="field">
          <label class="label">ת״ז</label>
          <input class="input" inputmode="numeric" dir="ltr" data-payer="otherPayer.idNumber" value="${escapeHtml(d.otherPayer.idNumber||"")}" />
        </div>
        <div class="field">
          <label class="label">תאריך לידה</label>
          <input class="input" type="text" dir="ltr" inputmode="numeric" placeholder="DD/MM/YYYY" maxlength="10" data-datefmt="dmy" data-payer="otherPayer.birthDate" value="${escapeHtml(d.otherPayer.birthDate||"")}" />
        </div>
      </div>
    </div>
  ` : ``;
  ...
  const ccForm = `
    <div class="lcGlassCard" style="margin-top:12px">
      <div class="lcGlassCard__title">כרטיס אשראי</div>
      <div class="lcWGrid">
        <div class="field">
          <label class="label">מס׳ כרטיס (4 ספרות אחרונות)</label>
          <input class="input" inputmode="numeric" dir="ltr" maxlength="4" data-payer="cc.last4" value="${escapeHtml(d.cc.last4||"")}" placeholder="1234" />
          <div class="help">מטעמי אבטחה לא שומרים מספר מלא במערכת.</div>
        </div>
        <div class="field">
          <label class="label">תוקף · חודש</label>
          <input class="input" inputmode="numeric" dir="ltr" maxlength="2" data-payer="cc.expMonth" value="${escapeHtml(d.cc.expMonth||"")}" placeholder="MM" />
        </div>
        <div class="field">
          <label class="label">תוקף · שנה</label>
          <input class="input" inputmode="numeric" dir="ltr" maxlength="4" data-payer="cc.expYear" value="${escapeHtml(d.cc.expYear||"")}" placeholder="YYYY" />
        </div>
      </div>

      <div class="lcWGrid" style="margin-top:10px">
        <div class="field">
          <label class="label">שם בעל/ת הכרטיס</label>
          <input class="input" value="${escapeHtml(d.cc.holderName||"")}" disabled />
        </div>
        <div class="field">
          <label class="label">ת״ז בעל/ת הכרטיס</label>
          <input class="input" dir="ltr" value="${escapeHtml(d.cc.holderId||"")}" disabled />
        </div>
      </div>
    </div>
  `;
  ...
  const hoForm = `
    <div class="lcGlassCard" style="margin-top:12px">
      <div class="lcGlassCard__title">הוראת קבע</div>
      <div class="lcWGrid">
        <div class="field">
          <label class="label">שם בעל החשבון</label>
          <input class="input" value="${escapeHtml(d.ho.accountHolderName||"")}" disabled />
        </div>
        <div class="field">
          <label class="label">ת״ז בעל החשבון</label>
          <input class="input" dir="ltr" value="${escapeHtml(d.ho.accountHolderId||"")}" disabled />
        </div>
        ...
        <div class="field">
          ...
          <select class="input" data-payer="ho.bankName">
            ...
            ${this.bankNames.map(b => `<option value="${escapeHtml(b)}"${safeTrim(d.ho.bankName)===b?" selected":""}>${escapeHtml(b)}</option>`).join("")}
          </select>
        </div>
        <div class="field">
          <label class="label">מספר בנק</label>
          <input class="input" inputmode="numeric" dir="ltr" data-payer="ho.bankNo" value="${escapeHtml(d.ho.bankNo||"")}" />
        </div>
        <div class="field">
          <label class="label">מספר סניף</label>
          <input class="input" inputmode="numeric" dir="ltr" data-payer="ho.branch" value="${escapeHtml(d.ho.branch||"")}" />
        </div>
        <div class="field">
          <label class="label">מספר חשבון</label>
          <input class="input" inputmode="numeric" dir="ltr" data-payer="ho.account" value="${escapeHtml(d.ho.account||"")}" />
        </div>
      </div>
    </div>
  `;
  ...
  const content = `
    <div class="lcWSection">
      ...
      <div class="muted small">בחירה חכמה · מילוי אוטומטי · מראה פרימיום</div>

      <div class="lcPayerCards">
        ${payerCards}
        ${otherCard}
      </div>
  ...
  return content;
    ...
    validateStep(stepId){
      ...
      if(stepId === 6){
  // payer (case-level on primary)
  const mode = safeTrim(d.payerMode || "insured");

  const getPayerSnapshot = () => {
    if(mode === "insured"){
      const pid = safeTrim(d.selectedPayerId) || safeTrim(this.insureds?.[0]?.id);
      const p = this._findInsuredById(pid) || this.insureds?.[0] || null;
      return {
        name: (safeTrim(p?.data?.firstName) + " " + safeTrim(p?.data?.lastName)).trim(),
        id: safeTrim(p?.data?.idNumber)
      };
    }
    const o = d.otherPayer || {};
    return { name: safeTrim(o.fullName), id: safeTrim(o.idNumber) };
  };
  ...
  if(mode === "insured"){
    ...
    const o = d.otherPayer || {};
    const req = ["relation","fullName","idNumber","birthDate"];
    if(!req.every(k => safeTrim(o[k]))) return false;
  }

  const snap = getPayerSnapshot();
  if(!snap.name || !snap.id) return false;

  const method = safeTrim(d.paymentMethod || "cc");
  if(method === "cc"){
    const cc = d.cc || {};
    if(!safeTrim(cc.last4) || String(cc.last4).replace(/\D/g,"").length !== 4) return false;
    const mm = Number(String(cc.expMonth||"").replace(/\D/g,""));
    const yyRaw = String(cc.expYear||"").replace(/\D/g,"");
    const yy = Number(yyRaw);
    if(!(mm >= 1 && mm <= 12)) return false;
    if(!(yyRaw.length === 2 || yyRaw.length === 4)) return false;
    if(!(yy > 0)) return false;
    return true;
  }else{
    const ho = d.ho || {};
    const req = ["bankName","bankNo","branch","account"];
