/* PATCH: 20260302-1910 — Step 5 premiums/discount/savings */
...
// open health covers drawer (Health only)
    ...
    _formatILS(n){
      ...
      const str = asInt ? String(Math.round(rounded)) : String(rounded.toFixed(2));
      const parts = str.split(".");
      const withCommas = parts[0].replace(/\B(?=(\d{3})+(?!\d))/g, ",");
      return withCommas + (parts[1] ? "." + parts[1] : "");
    },
    _calcPremiumAfter(beforeStr, discType, discValStr){
      const before = this._parseILS(beforeStr);
      const discVal = this._parseILS(discValStr);
      if(!(before > 0)) return { before, after: 0, discountAmt: 0 };
      let discountAmt = 0;
      if(discType === "ils"){
        discountAmt = Math.min(before, Math.max(0, discVal));
      }else{
        const pct = Math.min(100, Math.max(0, discVal));
        discountAmt = before * (pct / 100);
      }
      const after = Math.max(0, before - discountAmt);
      return { before, after, discountAmt };
    },
    _baselineExistingPremium(policy){
      ...
      const maxByType = (ins) => {
        ...
        let max = 0;
        for(const p of list){
          if(safeTrim(p.type) !== type) continue;
          const v = this._parseILS(p.monthlyPremium);
          if(v > max) max = v;
        }
        return max;
      };
      ...
      if(policy?.insuredMode === "couple"){
        ...
        return maxByType(primary) + maxByType(spouse);
      }

      const ins = (this.insureds || []).find(x => String(x.id) === String(policy?.insuredId));
      return maxByType(ins);
    },
    _updatePremiumPreview(){
      try{
        const d = this.policyDraft;
        if(!d) return;
        const calc = this._calcPremiumAfter(d.premiumBefore, d.discountType, d.discountValue);
        const base = this._baselineExistingPremium(d);
        const savings = Math.max(0, base - calc.after);

        const afterEl = this.els.body?.querySelector?.("#lcPremiumAfterVal");
        const saveEl = this.els.body?.querySelector?.("#lcSavingsVal");
        const saveWrap = this.els.body?.querySelector?.("#lcSavingsWrap");
        const baseEl = this.els.body?.querySelector?.("#lcBaselineVal");

        if(afterEl) afterEl.textContent = "₪" + this._formatILS(calc.after);
        if(baseEl) baseEl.textContent = base > 0 ? ("₪" + this._formatILS(base)) : "—";

        if(saveEl) saveEl.textContent = "חסכת ₪" + this._formatILS(savings);
        if(saveWrap) saveWrap.style.display = (base > 0 && savings > 0) ? "" : "none";
      }catch(_e){}
    },
    },
    ...
    addDraftPolicy(){
      ...
      const calc = this._calcPremiumAfter(d.premiumBefore, d.discountType, d.discountValue);
      const basePremium = this._baselineExistingPremium(d);
      ...
      const p = {
        ...
        type: d.type || "",
        // premiums
        premiumBefore: String(calc.before || ""),
        discountType: (d.discountType === "ils") ? "ils" : "pct",
        discountValue: String(this._parseILS(d.discountValue) || ""),
        premiumAfter: String(calc.after || ""),
        baselinePremium: String(basePremium || ""),
        // product-specific
      ...
      try { this.insureds[0].data.newPolicies = this.newPolicies; } catch(_e) {}
      ...
      this.policyDraft.discountType = "pct";
      this.policyDraft.discountValue = "";
    ...
    validateStep5(){
      ...
      const bad = list.filter(p => {
        ...
        // premiums
        const pb = Number(p.premiumBefore);
        const pa = Number(p.premiumAfter);
        if(!(pb > 0)) return true;
        if(!(pa >= 0)) return true;
      ...
      const renderPolicyCard = (p, showCoupleBadge=false) => {
        ...
        if(p.type === "ריסק משכנתא" && p.pledge){
          meta += `<div class="small muted">שיעבוד: <b>כן</b></div>`;
                ...
                // premiums
        const pa = Number(p.premiumAfter);
        const base = Number(p.baselinePremium);
        if(Number.isFinite(pa) && pa > 0){
          const saved = (Number.isFinite(base) && base > 0) ? Math.max(0, base - pa) : 0;
          meta += `<div class="small muted">פרמיה אחרי הנחה: <b>₪${escapeHtml(this._formatILS(pa))}</b> ${saved>0 ? `<span class="lcSavingsChip">חסכת ₪${escapeHtml(this._formatILS(saved))}</span>` : ``}</div>`;
        }
        ...
        $$("[data-pdraft]", this.els.body).forEach(el => {
          on(el, "input", () => {
            ...
            if(el.type === "checkbox") this.policyDraft[k] = !!el.checked;
              ...
              this.policyDraft[k] = v;
            }
            // Re-render only when the change affects visible structure
            if(k === "type" || k === "pledge" || k === "insuredId" || k === "discountType") this.render();
            if(k === "premiumBefore" || k === "discountValue" || k === "discountType" || k === "insuredId") this._updatePremiumPreview();
          });
          on(el, "change", () => {
            this.ensurePolicyDraft();
            const k = el.getAttribute("data-pdraft");
    ...
    validateDraftPolicy(){
      ...
      // premiums (required)
      const calc = this._calcPremiumAfter(d.premiumBefore, d.discountType, d.discountValue);
      if(!(calc.before > 0)) return { ok:false, msg:"חובה למלא פרמיה לפני הנחה" };
      if((d.discountType !== "ils") && (this._parseILS(d.discountValue) > 100)) return { ok:false, msg:"אחוז הנחה לא יכול להיות מעל 100%" };
...
// ---------- Step 6 ----------
    ...
    isStepCompleteForInsured(stepId, ins){
      ...
      if(stepId === 5){
        // new policies: if exists, must have company, type, premiumBefore > 0, and valid discount
        const list = (this.newPolicies && this.newPolicies.length) ? this.newPolicies : (d.newPolicies || []);
        for(const p of list){
          if(!safeTrim(p.company) || !safeTrim(p.type)) return false;
          if(!(Number(p.premiumBefore) > 0)) return false;
          if((p.discountType !== "ils") && (Number(p.discountValue) < 0 || Number(p.discountValue) > 100)) return false;
          if((p.discountType === "ils") && (Number(p.discountValue) < 0)) return false;
          if(!(Number(p.premiumAfter) >= 0)) return false;
          const isRisk = (p.type === "ריסק" || p.type === "ריסק משכנתא");
          const isCI = (p.type === "מחלות קשות" || p.type === "סרטן");
