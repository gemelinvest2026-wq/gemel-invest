...
(() => {
  ...
  const Storage = {
    ...
    async ping(){
      ...
      try {
        const json = await fetchJsonWithTimeout(url.toString(), { method:"GET" }, 9000);
  ...
  const Auth = {
    ...
    async _submit(){
      ...
      const prevBtnText = submitBtn?.textContent || "התחבר";
      ...
      // ensure boot done, but never let login hang forever
      try {
        await Promise.race([
          App._bootPromise,
      ...
      if(!matched){ if(submitBtn){ submitBtn.disabled = false; submitBtn.textContent = prevBtnText; } return this._setError("שם משתמש לא נמצא"); }
      if(matched.active === false){ if(submitBtn){ submitBtn.disabled = false; submitBtn.textContent = prevBtnText; } return this._setError("המשתמש מושבת"); }
      const expected = safeTrim(matched.pin) || "0000";
      if(pin !== expected){ if(submitBtn){ submitBtn.disabled = false; submitBtn.textContent = prevBtnText; } return this._setError("קוד כניסה שגוי"); }
...
// ---------- Step 6 ----------
    ...
    if(!st.selectedFormKey || st.selectedFormKey !== formDef.key){
      st.selectedFormKey = formDef.key;
      st.reason = formDef.reason || '';
      st.prefillSnapshot = this.buildPhoenixPrefillSnapshot(formDef);
      st.viewerUrl = this.resolvePhoenixSourcePath(formDef.path);
      st.lastGeneratedAt = nowISO();
      st.autoLoaded = true;
    }
    const frameSrc = (st.viewerUrl || this.resolvePhoenixSourcePath(formDef.path)) + '#toolbar=0&navpanes=0&statusbar=0&messages=0&view=FitH';
    return `<div class="lcPhoenixClean"><div class="lcPhoenixClean__doc"><iframe class="lcPhoenixClean__frame" id="lcPhoenixPdfFrame" src="${escapeHtml(frameSrc)}"></iframe></div></div>`;
  },

  bindHealthInputs(){
    const st = this.getPhoenixHealthState();
    const formDef = this.choosePhoenixPrimaryForm();
    if(formDef && (!st.viewerUrl || st.selectedFormKey !== formDef.key)){
      st.selectedFormKey = formDef.key;
      st.reason = formDef.reason || '';
      st.prefillSnapshot = this.buildPhoenixPrefillSnapshot(formDef);
      st.viewerUrl = this.resolvePhoenixSourcePath(formDef.path);
  ...
  getHealthBlockingIssue(){
    ...
    const st = this.getPhoenixHealthState();
    if(!st.viewerUrl) return { ok:false, msg:'הטופס של הפניקס עדיין לא נטען.' };
    return { ok:true };
  }
});
...
const App = {
  ...
  Wizard.init();
  App._bootPromise = App.boot().catch(err => { console.error("BOOT_ERROR:", err); return null; });
