...
(() => {
  ...
  function getMirrorQueueStore(rec){
    ...
    store.queueStatus = safeTrim(store.queueStatus) || "";
    store.submittedAt = safeTrim(store.submittedAt) || "";
    store.submittedBy = safeTrim(store.submittedBy) || "";
    store.finishedAt = safeTrim(store.finishedAt) || "";
    store.finishedBy = safeTrim(store.finishedBy) || "";
    store.startedAt = safeTrim(store.startedAt) || "";
    store.startedBy = safeTrim(store.startedBy) || "";
    return store;
  }
  ...
  function markCustomerMirrorQueue(rec, status, extra={}){
    ...
    if(status === 'waiting'){
      ...
      store.submittedBy = safeTrim(extra.by) || safeTrim(Auth?.current?.name);
      store.startedAt = '';
      store.startedBy = '';
      store.finishedAt = '';
      store.finishedBy = '';
      ...
      store.savedAt = stamp;
      store.savedBy = safeTrim(extra.by) || safeTrim(Auth?.current?.name);
      store.startedAt = '';
      store.startedBy = '';
      store.finishedAt = '';
      store.finishedBy = '';
      ...
      store.startedBy = safeTrim(extra.by) || safeTrim(Auth?.current?.name);
      rec.status = 'בטיפול שיקוף';
    }else if(status === 'done'){
      store.finishedAt = stamp;
      store.finishedBy = safeTrim(extra.by) || safeTrim(Auth?.current?.name);
...
// nav
      ...
      on(this.els.btnOpenMirrorQueue, "click", () => MirrorQueueUI.open());
this.applyRoleUI();
      this.renderAuthPill();
    },
...
txt.textContent = "";
  ...
  const MirrorQueueUI = {
    ...
    list(){
      const rows = (Array.isArray(State.data?.customers) ? State.data.customers.slice() : []).filter(rec => {
        const status = getMirrorQueueStatus(rec);
    ...
    open(){
      if(!Auth.isOps() || !this.els.wrap) return;
      this.els.wrap.classList.add('is-open');
      this.els.wrap.setAttribute('aria-hidden', 'false');
      this.render();
