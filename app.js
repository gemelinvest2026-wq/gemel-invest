...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  function getSession_(){
    try{
      ...
      if(s && s.username && s.role && s.displayName){
        return { username:String(s.username), role:String(s.role), displayName:String(s.displayName) };
      }
    }catch(_){}
    return null;
  }

  function setSession_(safeUser){
    try{
      localStorage.setItem(SESSION_LS_KEY, JSON.stringify(safeUser||null));
    }catch(_){}
  }
  ...
  function showLogin_(){
    ...
    // lock scroll + ensure app does not "peek" behind the login
  ...
  function hideLogin_(){
    ...
    // allow scrolling again after login
  ...
  function verifyCredentials_(username, password){
    ...
    if(!u || !p) return { ok:false, msg:'נא למלא שם משתמש וסיסמה' };

    const users = loadUsers_(); // seeds if empty
    const found = users.find(x => (x.active !== false) && String(x.username||'') === u && String(x.password||'') === p);
    if(!found) return { ok:false, msg:'שם משתמש או סיסמה לא נכונים' };

    return { ok:true, user: { username: found.username, role: found.role || 'agent', displayName: found.displayName || found.username } };
  }

  function wireLoginUI_(){
    const uEl = document.getElementById('loginUsername');
    const pEl = document.getElementById('loginPassword');
    const bEl = document.getElementById('loginBtn');
    const eEl = document.getElementById('loginErr');
    const hintBtn = document.getElementById('loginHintBtn');

    const setErr = (msg) => { if(eEl) eEl.textContent = msg || ''; };

    const doLogin = () => {
      setErr('');
      const res = verifyCredentials_(uEl ? uEl.value : '', pEl ? pEl.value : '');
      if(!res.ok){
        setErr(res.msg || 'שגיאה');
        return;
      }
      const safe = { username: res.user.username, role: res.user.role, displayName: res.user.displayName };
      setSession_(safe);
      startApp_(safe);
    };

    if(bEl) bEl.addEventListener('click', doLogin);
    const onEnter = (ev) => {
      if(ev.key === 'Enter'){
        ev.preventDefault();
        doLogin();
    ...
    if(hintBtn){
      hintBtn.addEventListener('click', () => {
        toast('משתמשי דמו', 'admin / 3316 • agent1 / 1111');
      });
    }
  }

  function startApp_(safeUser){
    state.currentUser = safeUser;
    state.agentName = safeUser.displayName;
    hideLogin_();
    applyPermissions_();
  ...
  (function bootLogin_(){
    ...
    if(s){
      ...
      showLogin_();
      // Keep app hidden and do NOT render yet.
    }
  })();
// ---------- Render ----------
  function render(){
