...
(() => {
  ...
  function getMergedMirrorQueueCustomers(){
    ...
    const backup = Array.isArray(Storage.loadBackup?.()?.customers) ? Storage.loadBackup().customers.slice() : [];
    const byId = new Map();
    const apply = (rec, sourceLabel) => {
      ...
      const currentStatus = getMirrorQueueStatus(current);
      const incomingStatus = getMirrorQueueStatus(normalized);
      const currentUpdated = new Date(current?.updatedAt || current?.createdAt || 0).getTime() || 0;
      const incomingUpdated = new Date(normalized?.updatedAt || normalized?.createdAt || 0).getTime() || 0;
      if((!currentStatus && incomingStatus) || incomingUpdated > currentUpdated || sourceLabel === 'live'){
        byId.set(id, Object.assign({}, current, normalized, {
          payload: Object.assign({}, current.payload || {}, normalized.payload || {})
        }));
      }
    };
    live.forEach(rec => apply(rec, 'live'));
    backup.forEach(rec => apply(rec, 'backup'));
...
txt.textContent = "";
  ...
  const MirrorQueueUI = {
    ...
    async open(){
      ...
      try{ await App.reloadSessionState({ silent:true, preserveView:true }); }catch(_e){}
      this.render();
      this.els.wrap.classList.add('is-open');
      this.els.wrap.setAttribute('aria-hidden', 'false');
