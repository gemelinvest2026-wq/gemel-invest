...
txt.textContent = "";
  ...
  const CustomersUI = {
    ...
    getMedicalGroups(rec){
      ...
      const byId = {};
      insureds.forEach((ins, idx) => { byId[String(ins?.id || idx)] = { label: this.getNewPolicyInsuredLabel({insureds}, {insuredId: ins?.id}, insureds), id: String(ins?.id || idx) }; });
      const groups = {};
      Object.keys(source.responses || {}).forEach(qKey => {
        const perIns = source.responses[qKey] || {};
        Object.keys(perIns).forEach(insId => {
          if(!groups[insId]) groups[insId] = { insured: byId[insId] || {label:'מבוטח', id:insId}, items: [] };
          groups[insId].items.push({ qKey, insId, meta: { text:qKey, title:'הצהרת בריאות', fields:[] }, response: perIns[insId] || {} });
        });
      });
    ...
    renderMedicalInfo(rec){
      ...
      const summaryCards = [
        ...
        { icon:'⚠️', label:'סומנו כן', value:String(summary.positive || 0), sub:'דורש תשומת לב רפואית / פירוט' },
