...
(() => {
  ...
  const BUILD = "20260331-remove-mirrors-v18";
...
this.applyRoleUI();
    ...
    goView(view){
      ...
      document.body.classList.remove("view-users-active","view-dashboard-active","view-settings-active","view-discountSpec-active","view-customers-active","view-proposals-active","view-myProcesses-active");
  ...
  const CustomersUI = {
    ...
    getMedicalGroups(rec){
      try{
        return getMirrorHealthEntries(rec) || [];
      }catch(_e){}
      return [];
    },
...
ההכנסה מעבודה שאתה צפוי להרוויח תעמוד על סך של ______ ₪ לחודש. ידוע לך כי בקרות מקרה הביטוח יוגבלו תגמולי הפיצוי החודשי להם תהיה זכאי לגובה שלא יעלה על 75% מהכנסתך מעבודה בפועל ב־12 החודשים שקדמו למועד קרות מקרה הביטוח, או בתקופה ממועד תחילת הביטוח ועד מועד קרות מקרה הביטוח, אם חלפו פחות מ־12 חודשים.`
  ...
  const ProcessesUI = {
    ...
    openProcess(id){
      ...
      if(!rec) return;
      CustomersUI.openByIdWithLoader(String(id), 900);
    }
  };
  ...
  function getMirrorCallStore(rec){
    ...
    const payload = rec.payload && typeof rec.payload === "object" ? rec.payload : {};
    const mirrorFlow = payload.mirrorFlow && typeof payload.mirrorFlow === "object" ? payload.mirrorFlow : {};
    if(mirrorFlow.callSession && typeof mirrorFlow.callSession === "object") return mirrorFlow.callSession;
    if(mirrorFlow.call && typeof mirrorFlow.call === "object") return mirrorFlow.call;
    return {};
  }

  function getMirrorHealthEntries(rec){
    const payload = rec?.payload && typeof rec.payload === "object" ? rec.payload : {};
    const step = payload?.mirrorFlow?.healthStep && typeof payload.mirrorFlow.healthStep === "object"
      ? payload.mirrorFlow.healthStep
      : {};
    const groups = Array.isArray(step.groups) ? step.groups : [];
    return groups.map((group) => ({
      title: safeTrim(group?.title || group?.label || "הצהרת בריאות"),
  ...
  const MirrorsUI = {
    ...
    selectCustomer(id){
      if(!id) return;
      try { CustomersUI.openByIdWithLoader(String(id), 900); } catch(_e) {}
    },
    getCallState(rec){
      const call = getMirrorCallStore(rec);
      return {
        active: !!call?.active,
        startedAt: safeTrim(call?.startedAt),
        endedAt: safeTrim(call?.endedAt),
        startTime: safeTrim(call?.startTime),
        endTime: safeTrim(call?.endTime),
        durationText: safeTrim(call?.durationText)
      };
    },
    stopTimerLoop(){},
    renderCallBar(){},
    ...
    getStepTitle(step){
      ...
      return steps[Number(step)] || "תהליך";
    },
    getProgressStep(rec){
      const payload = rec?.payload && typeof rec.payload === "object" ? rec.payload : {};
      const mirrorFlow = payload?.mirrorFlow && typeof payload.mirrorFlow === "object" ? payload.mirrorFlow : {};
      const uiStep = Number(mirrorFlow?.ui?.focusStep || 0);
      if(uiStep > 0) return Math.min(9, Math.max(1, uiStep));
      if(safeTrim(mirrorFlow?.issuanceStep?.savedAt)) return 9;
      if(safeTrim(mirrorFlow?.paymentStep?.savedAt)) return 8;
      if(safeTrim(mirrorFlow?.healthStep?.savedAt)) return 7;
      if(safeTrim(mirrorFlow?.disclosure?.savedAt)) return 6;
      if(safeTrim(mirrorFlow?.verify?.reflectionSavedAt)) return 4;
      if(safeTrim(mirrorFlow?.verify?.savedAt)) return 3;
      return 1;
    },
    getMirrorHealthEntries
  };
