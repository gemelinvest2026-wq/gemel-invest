...
(() => {
  ...
  function normalizeState(s){
    ...
    out.customers = (out.customers || []).map((c, idx) => ({
      id: safeTrim(c?.id) || ("c_" + idx),
      ownerName: safeTrim(c?.ownerName),
      createdAt: safeTrim(c?.createdAt) || nowISO(),
      updatedAt: safeTrim(c?.updatedAt) || safeTrim(c?.createdAt) || nowISO(),
      fullName: safeTrim(c?.fullName),
      idNumber: safeTrim(c?.idNumber),
      phone: safeTrim(c?.phone),
      email: safeTrim(c?.email),
      city: safeTrim(c?.city),
      insureds: Array.isArray(c?.insureds) ? c.insureds : [],
      newPolicies: Array.isArray(c?.newPolicies) ? c.newPolicies : [],
      health: c?.health || {},
...
this.els.syncDot = $("#syncDot");
      ...
      this.els.customersSearchBtn = $("#customersSearchBtn");
...
// nav
      ...
      on(this.els.customersSearchBtn, "click", () => CustomersUI.render());
this.applyRoleUI();
      this.renderAuthPill();
    },
...
txt.textContent = "";
  ...
  const CustomersUI = {
    ...
    list(){
      ...
      let rows = all.filter(c => Auth.canViewAllCustomers() || safeTrim(c.ownerName) === safeTrim(Auth.current?.name));
      if(q){
        rows = rows.filter(c =>
          safeTrim(c.fullName).toLowerCase().includes(q) ||
          safeTrim(c.idNumber).toLowerCase().includes(q) ||
          safeTrim(c.phone).toLowerCase().includes(q)
        );
      }
    ...
    render(){
      this.ensure();
      const tb = UI.els.customersTbody;
      if(!tb) return;
      const rows = this.list();
      if(!rows.length){
        tb.innerHTML = `<tr><td colspan="5"><div class="emptyState"><div class="emptyState__icon">🗂️</div><div class="emptyState__title">עדיין אין לקוחות להצגה</div><div class="emptyState__text">ברגע שתשמור הקמת לקוח, הוא יופיע כאן אוטומטית עם גישה ישירה לתיק הלקוח.</div></div></td></tr>`;
        return;
      }
      tb.innerHTML = rows.map(c => `
        <tr>
          <td><div class="lcCustomerMain"><strong>${escapeHtml(c.fullName || 'ללא שם')}</strong><div class="muted small">ת״ז: ${escapeHtml(c.idNumber || '—')}</div></div></td>
          <td>${escapeHtml(c.phone || '—')}</td>
          <td>${escapeHtml(c.ownerName || '—')}</td>
          <td>${escapeHtml(this.fmtDate(c.updatedAt || c.createdAt))}</td>
          <td><button class="btn btn--primary" data-open-customer="${escapeHtml(c.id)}">פתח תיק לקוח</button></td>
        </tr>`).join('');
      $$('[data-open-customer]', tb).forEach(btn => on(btn, 'click', () => this.open(btn.getAttribute('data-open-customer'))));
    },

    fmtDate(v){
      const d = new Date(v || '');
      if(isNaN(d)) return safeTrim(v || '—');
      return d.toLocaleString('he-IL');
    },

    getById(id){
      return (State.data?.customers || []).find(c => String(c.id) === String(id)) || null;
    },

    open(id){
      this.ensure();
      const c = this.getById(id);
      if(!c || !this.els.wrap) return;
      this.currentId = c.id;
      if(this.els.name) this.els.name.textContent = c.fullName || 'תיק לקוח';
      if(this.els.meta) this.els.meta.innerHTML = `
        <span>ת״ז: ${escapeHtml(c.idNumber || '—')}</span>
        <span>טלפון: ${escapeHtml(c.phone || '—')}</span>
        <span>נציג: ${escapeHtml(c.ownerName || '—')}</span>
        <span>נפתח: ${escapeHtml(this.fmtDate(c.createdAt))}</span>`;
      const existingPolicies = (c.insureds || []).reduce((n, ins) => n + ((ins?.existingPolicies || []).length), 0);
      const newPolicies = Array.isArray(c.newPolicies) ? c.newPolicies.length : 0;
      const premium = this.sumPremium(c);
      const docs = Array.isArray(c.documents) ? c.documents.length : 0;
      if(this.els.dash) this.els.dash.innerHTML = `
        <div class="dashCard"><div class="dashCard__k">פוליסות קיימות</div><div class="dashCard__v">${existingPolicies}</div><div class="dashCard__sub">מתוך ההקמה האחרונה</div></div>
        <div class="dashCard"><div class="dashCard__k">פוליסות חדשות</div><div class="dashCard__v">${newPolicies}</div><div class="dashCard__sub">מוכנות להמשך טיפול</div></div>
        <div class="dashCard"><div class="dashCard__k">פרמיה חודשית כוללת</div><div class="dashCard__v">${escapeHtml(premium)}</div><div class="dashCard__sub">חישוב מכל הפוליסות שנשמרו</div></div>`;
      if(this.els.body) this.els.body.innerHTML = this.renderBody(c);
    ...
    sumPremium(c){
      let sum = 0;
      (c.insureds || []).forEach(ins => (ins?.existingPolicies || []).forEach(p => {
        const n = Number(String(p?.monthlyPremium ?? '').replace(/[^0-9.\-]/g,''));
        if(isFinite(n)) sum += n;
      }));
      (c.newPolicies || []).forEach(p => {
        const n = Number(String(p?.monthlyPremium ?? p?.premium ?? '').replace(/[^0-9.\-]/g,''));
        if(isFinite(n)) sum += n;
      });
      return sum ? (new Intl.NumberFormat('he-IL').format(sum) + ' ₪') : '—';
    },

    renderBody(c){
      const primary = (c.insureds || [])[0] || {};
      const existingRows = [];
      (c.insureds || []).forEach(ins => (ins?.existingPolicies || []).forEach(p => existingRows.push({ who: ins.label || 'מבוטח', company:p.company, type:p.type, premium:p.monthlyPremium })));
      const newRows = (c.newPolicies || []).map(p => ({ who:p.insuredLabel || p.insuredName || 'מבוטח', company:p.company, type:p.type || p.product, premium:p.monthlyPremium || p.premium, start:p.startDate }));
      const healthRows = [];
      (c.insureds || []).forEach(ins => {
        const cats = ins?.healthDeclaration?.categories || {};
        Object.keys(cats).forEach(k => {
          const item = cats[k];
          if(item && item.answer === 'yes') healthRows.push({ who: ins.label || 'מבוטח', q: k, text: this.healthSummary(item) });
        });
      });
      const opRows = Array.isArray(c.operations) ? c.operations : [];
      const docs = Array.isArray(c.documents) && c.documents.length ? c.documents : [
        { title:'הצעת ביטוח', status:'בקרוב' },
        { title:'טופסי הצטרפות', status:'בקרוב' },
        { title:'מסמכי חיתום', status:'בקרוב' },
        { title:'אישורים והצהרות', status:'בקרוב' },
        { title:'סיכום פגישה', status:'בקרוב' },
        { title:'מסמכים נוספים', status:'בקרוב' }
      ];
      return `
        <div class="lcFileGrid">
          <section class="card"><div class="card__head"><div class="card__title">תקציר לקוח</div></div><div class="lcFileInfoGrid">
            ${this.infoLine('שם מלא', c.fullName)}
            ${this.infoLine('תעודת זהות', c.idNumber)}
            ${this.infoLine('טלפון', c.phone)}
            ${this.infoLine('אימייל', c.email)}
            ${this.infoLine('עיר', c.city || primary.city)}
            ${this.infoLine('קופת חולים', primary.clinic)}
            ${this.infoLine('שב״ן', primary.shaban)}
            ${this.infoLine('עיסוק', primary.occupation)}
          </div></section>
          <section class="card"><div class="card__head"><div class="card__title">פוליסות קיימות</div><div class="card__hint">${existingRows.length} רשומות</div></div>${this.renderMiniTable(existingRows, ['מבוטח','חברה','מוצר','פרמיה'], r => [r.who,r.company,r.type,this.money(r.premium)])}</section>
          <section class="card"><div class="card__head"><div class="card__title">פוליסות חדשות</div><div class="card__hint">${newRows.length} רשומות</div></div>${this.renderMiniTable(newRows, ['מבוטח','חברה','מוצר','תחילה','פרמיה'], r => [r.who,r.company,r.type,r.start || '—',this.money(r.premium)])}</section>
          <section class="card"><div class="card__head"><div class="card__title">הצהרת בריאות</div><div class="card__hint">ממצאים שנשמרו בשלב 8</div></div>${healthRows.length ? this.renderMiniTable(healthRows, ['מבוטח','שאלה','פירוט'], r => [r.who,r.q,r.text]) : '<div class="emptyState"><div class="emptyState__icon">🩺</div><div class="emptyState__title">אין ממצאים שמורים</div><div class="emptyState__text">כל התשובות נשמרו בתיק הלקוח ויוצגו כאן כאשר יישמר פירוט חיובי.</div></div>'}</section>
          <section class="card"><div class="card__head"><div class="card__title">מסמכי לקוח</div><div class="card__hint">אזור עתידי למסמכים במערכת</div></div><div class="lcDocsGrid">${docs.map(d => `<div class="lcDocCard"><div class="lcDocCard__title">${escapeHtml(d.title || 'מסמך')}</div><div class="lcDocCard__status">${escapeHtml(d.status || 'בקרוב')}</div></div>`).join('')}</div></section>
          <section class="card"><div class="card__head"><div class="card__title">תפעול</div><div class="card__hint">מעקב פעולות מהתיק</div></div>${opRows.length ? this.renderMiniTable(opRows, ['פעולה','סטטוס','תאריך'], r => [r.title || '—', r.status || '—', this.fmtDate(r.at)]) : '<div class="emptyState"><div class="emptyState__icon">📌</div><div class="emptyState__title">אין פעולות תפעול עדיין</div><div class="emptyState__text">כשתשלח הצעה לתפעול או תפיק קובץ, הפעולות ירשמו כאן.</div></div>'}</section>
        </div>`;
    },

    healthSummary(item){
      const f = item?.fields || {};
      const vals = Object.values(f).map(v => safeTrim(v)).filter(Boolean);
      return vals.length ? vals.join(' · ') : 'סומן ממצא ללא פירוט נוסף';
    },

    infoLine(label, value){
      return `<div class="lcFileInfoItem"><div class="lcFileInfoItem__k">${escapeHtml(label)}</div><div class="lcFileInfoItem__v">${escapeHtml(value || '—')}</div></div>`;
    },

    renderMiniTable(rows, headers, mapFn){
      if(!rows.length) return `<div class="emptyState"><div class="emptyState__icon">📄</div><div class="emptyState__title">אין נתונים להצגה</div><div class="emptyState__text">המידע יופיע כאן מיד לאחר שיוזנו נתונים רלוונטיים בהקמה.</div></div>`;
      return `<div class="tableWrap"><table class="table"><thead><tr>${headers.map(h => `<th>${escapeHtml(h)}</th>`).join('')}</tr></thead><tbody>${rows.map(r => `<tr>${mapFn(r).map(v => `<td>${escapeHtml(v || '—')}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`;
    }
  };
...
init(){
      ...
      on(this.els.btnOpenCustomerFile, "click", () => {
        ...
        this.close();
        UI.goView("customers");
        if(this.lastCreatedCustomerId) CustomersUI.open(this.lastCreatedCustomerId);
      });
    ...
    reset(){
      ...
      this.lastCreatedCustomerId = null;
      this._flowAutoTimer = null;
...
// ---------- Step 6 ----------
    ...
    showFinishFlow(){
      ...
      clearTimeout(this._flowAutoTimer);
      ...
      setTimeout(() => {
        ...
        this._flowAutoTimer = setTimeout(() => {
          this.hideFinishFlow();
          this.close();
          UI.goView('customers');
          CustomersUI.render();
    ...
    buildCustomerRecord(){
      const primary = this.insureds[0]?.data || {};
      const primaryLabel = [safeTrim(primary.firstName), safeTrim(primary.lastName)].filter(Boolean).join(' ');
      const insureds = JSON.parse(JSON.stringify(this.insureds || []));
      const newPolicies = JSON.parse(JSON.stringify(this.newPolicies || []));
      return {
        id: 'c_' + Date.now().toString(16) + '_' + Math.random().toString(16).slice(2,8),
        ownerName: safeTrim(Auth.current?.name) || '',
        createdAt: nowISO(),
        updatedAt: nowISO(),
        fullName: primaryLabel || 'לקוח חדש',
...
const App = {
  ...
  // ---------- Start ----------
  UI.init();
      try{ CustomersUI.ensure(); }catch(e){}
  Auth.init();
  Wizard.init();
