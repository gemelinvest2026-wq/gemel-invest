...
(() => {
  ...
  const BUILD = "20260328-elementary-vehicle-progress-v2";
...
init(){
    ...
    renderBody(){
      ...
      const ins = this.getActive();
      if(this.isElementaryFlow()){
        const stepTitle = this.getCurrentStepDef()?.title || "";
        const head = this.step <= 0 ? "" : `<div class="lcWSection"><div class="row row--between"><div><div class="lcWTitle">${escapeHtml(stepTitle)}</div><div class="muted small">וויזארד אלמנטרי נפרד מתחום בריאות וסיכונים</div></div></div></div>`;
        let body = "";
...
// open health covers drawer (Health only)
    ...
    renderElementaryCategory(){
      ...
      const makeCard = (key, title, sub, badge, disabled=false) => {
        const cls = ["lcElementaryCard", selected === key ? "is-active" : "", disabled ? "is-disabled" : ""].join(" ").trim();
        return `<button type="button" class="${cls}" data-elementary-product="${escapeHtml(key)}" ${disabled ? 'disabled' : ''}>
          <span class="lcElementaryCard__badge">${escapeHtml(badge)}</span>
          <span class="lcElementaryCard__title">${escapeHtml(title)}</span>
          <span class="lcElementaryCard__sub">${escapeHtml(sub)}</span>
        </button>`;
      };
      return `<div class="lcElementaryChooser">
        <div class="lcElementaryChooser__hero">
          <div class="lcElementaryChooser__kicker">GEMEL INVEST · אלמנטרי</div>
          <div class="lcElementaryChooser__title">בחר תחום לפתיחת וויזארד אלמנטרי</div>
          <div class="lcElementaryChooser__text">בריאות וסיכונים נשארים נפרדים. כאן אנחנו בונים מסלול אלמנטרי חדש, ובשלב זה רכב פעיל לפיתוח.</div>
        </div>
        <div class="lcElementaryChooser__grid">
          ${makeCard('vehicle','רכב','וויזארד פעיל: פרטי מבוטח + ייבוא מהר הביטוח','זמין עכשיו')}
          ${makeCard('home','דירה','השלב הבא במסלול האלמנטרי','בפיתוח', true)}
        </div>
      </div>`;
    },
    ...
    renderElementaryProgress(){
      ...
      return `<div class="lcElementaryProgress" style="--elementary-progress:${progressPct}%;--elementary-car-progress:${progressPct};">
        <div class="lcElementaryProgress__top">
          <div class="lcElementaryProgress__title">מסלול רכב</div>
          <div class="lcElementaryProgress__sub">המערכת מתקדמת איתך שלב אחר שלב עד סיום קליטת הנתונים</div>
        </div>
        <div class="lcElementaryRoad">
          ...
          <div class="lcElementaryRoad__car" aria-hidden="true">
            <svg viewBox="0 0 64 28" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M55 16.5H8.6c-1.6 0-2.8-1.3-2.7-2.9l.4-4.1c.2-2 1.7-3.6 3.7-3.9l8.3-1.2c1-.2 1.8-.7 2.4-1.5l2-2.7c1-1.3 2.5-2 4.1-2H41c1.4 0 2.8.5 3.9 1.4l7.1 5.9c1 .8 1.5 2 1.7 3.3l.8 5.7c.2 1.1-.7 2.1-1.8 2.1Z" fill="currentColor" fill-opacity="0.18" stroke="currentColor" stroke-width="1.8" stroke-linejoin="round"/>
              <path d="M45.2 7.2H25.7c-.8 0-1.2-.9-.7-1.5l1.4-1.9c.6-.8 1.6-1.3 2.6-1.3h10.4c.8 0 1.6.3 2.2.8l4.4 3.7c.6.5.2 1.5-.8 1.5Z" fill="currentColor" fill-opacity="0.14"/>
              <circle cx="18" cy="18.5" r="4.8" fill="white" fill-opacity="0.94" stroke="currentColor" stroke-width="1.8"/>
              <circle cx="18" cy="18.5" r="2" fill="currentColor"/>
              <circle cx="45" cy="18.5" r="4.8" fill="white" fill-opacity="0.94" stroke="currentColor" stroke-width="1.8"/>
              <circle cx="45" cy="18.5" r="2" fill="currentColor"/>
              <path d="M10 16.5H54.7" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/>
              <path d="M13.5 11.4h7.2" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/>
            </svg>
          </div>
    ...
    renderElementaryVehiclePolicyRows(rows){
      ...
      if(!list.length){
        return `<div class="emptyState lcElementaryEmpty"><div class="emptyState__icon">🚗</div><div class="emptyState__title">עדיין לא יובאו פוליסות רכב</div><div class="emptyState__text">לחץ על “ייבוא מהר הביטוח”, העלה קובץ Excel מהר הביטוח, והמערכת תשלוף רק תחום כללי &gt; ביטוח רכב.</div></div>`;
      }
    ...
    renderElementaryStep2(ins){
      ...
      return `<div class="lcElementaryWrap">
        ${this.renderElementaryProgress()}
        <div class="lcElementaryImportHero">
          <div>
            <div class="lcElementaryIntro__title">ייבוא פוליסות רכב מהר הביטוח</div>
            <div class="lcElementaryIntro__text">המנוע מסנן רק תחום כללי ורק קטגוריית רכב. פוליסות שמגיעות מהר הביטוח מוצגות לקריאה בלבד ואי אפשר לערוך או להסיר אותן.</div>
          </div>
          <div class="lcElementaryImportHero__actions">
            <button class="btn btn--primary" id="lcElementaryImportHarBtn" type="button">ייבוא מהר הביטוח</button>
            <input id="lcElementaryHarFile" type="file" accept=".xlsx,.xls" hidden />
          </div>
        </div>
        <div class="lcElementaryFieldLegend">
          <span>שדות שליפה: ת״ז · ענף משני · סוג מוצר · חברה · תקופת ביטוח · פרטים נוספים · פרמיה בש״ח · סוג פרמיה · מספר פוליסה</span>
        </div>
