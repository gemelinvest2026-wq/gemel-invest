...
txt.textContent = "";
      ...
      UI.els.customersTbody.innerHTML = rows.length ? rows.map(rec => {
        ...
        return `<tr>
          ...
          <td><div class="lcCustomers__rowActions">
            ...
            ${Auth.isAdmin() ? `<button class="btn btn--danger lcCustomers__archiveBtn" data-archive-customer="${escapeHtml(rec.id)}" type="button">גנוז לקוח</button>` : ""}
          </div></td>
        </tr>`;
  ...
  const ArchiveCustomerUI = {
    ...
    open(id){
      ...
      if(!Auth.isAdmin()){
        alert("רק מנהל מערכת יכול למחוק לקוח.");
        return;
      }
      if(!rec || !this.els.wrap) return;
    ...
    async confirm(){
      ...
      if(!Auth.isAdmin()){
        this.showError("רק מנהל מערכת יכול לבצע מחיקה.");
        return;
      }
      const rec = CustomersUI.byId(id);
      if(!id || !rec){
