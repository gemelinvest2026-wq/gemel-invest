...
(() => {
  ...
  const BUILD = "20260330-har-bituach-parser-v11";
...
// open health covers drawer (Health only)
    ...
    parseHarBituachWorkbook(buffer, ins){
      ...
      const normalizeContent = (value) => normalizeHeader(value)
        .replace(/[–—-]/g, " ")
        .replace(/[()[\]{}]/g, " ")
        .replace(/\s+/g, " ")
        .trim();
      ...
      const matchHeader = (value, patterns=[]) => {
        const normalized = normalizeHeader(value);
        return !!normalized && patterns.some((pattern) => pattern.test(normalized));
      };

      const headerFieldScore = (row = []) => {
        const cells = Array.isArray(row) ? row.map((cell) => normalizeHeader(cell)).filter(Boolean) : [];
        const matched = new Set();
        Object.entries(headerMatchers).forEach(([key, patterns]) => {
          if(cells.some((value) => patterns.some((pattern) => pattern.test(value)))) matched.add(key);
        });
        let score = matched.size;
        if(matched.has("idNumber")) score += 2;
        if(matched.has("main")) score += 2;
        if(matched.has("company")) score += 2;
        if(matched.has("policyNumber")) score += 1;
        if(matched.has("premium")) score += 1;
        if(matched.has("productType")) score += 1;
        return { score, matched };
      };

      const findBestHeaderRow = (rows = []) => {
        let best = { index: -1, score: 0, matched: new Set() };
        rows.slice(0, Math.min(rows.length, 35)).forEach((row, index) => {
          if(!Array.isArray(row)) return;
          const result = headerFieldScore(row);
          if(result.score > best.score) best = { index, score: result.score, matched: result.matched };
        });
        if(best.score >= 6) return best;
        if(best.score >= 4 && (best.matched.has("company") || best.matched.has("policyNumber")) && (best.matched.has("main") || best.matched.has("productType"))) return best;
        return { index: -1, score: 0, matched: new Set() };
      };

      const looksLikeSectionCell = (value) => /^תחום\s*-/.test(normalizeContent(value));
      const isBlankishRow = (row = []) => !Array.isArray(row) || row.every((cell) => !normalizeContent(cell));

      const extractNumericPremium = (value) => {
        if(typeof value === "number" && Number.isFinite(value)) return value;
        const normalized = String(value ?? "").replace(/[^\d.,-]/g, "").replace(/,(?=\d{3}\b)/g, "");
        const withDot = normalized.replace(",", ".");
        const parsed = Number(withDot);
        return Number.isFinite(parsed) ? parsed : 0;
      };

      const summarizeText = (row = {}) => normalizeContent([
        row.main,
        row.sub,
        row.productType,
        row.section,
        row.extra,
        row.planClassification
      ].filter(Boolean).join(" | "));

      const isLikelyDataRow = (row = {}) => {
        const source = summarizeText(row);
        if(!source && !safeTrim(row.company) && !safeTrim(row.policyNumber) && !Number(row.premiumValue || 0)) return false;
        if(/^תחום\s*-/.test(source)) return false;
        if(/סה["״]?כ|סך הכל|סיכום|מספר הרשומות|הופק מאתר הר הביטוח/i.test(source)) return false;
        if(/ענף ראשי|ענף \(משני\)|סוג מוצר|מספר פוליסה|פרמיה/i.test(source)) return false;
        const signalCount = [
          ...
          safeTrim(row.sub),
          safeTrim(row.productType),
          Number(row.premiumValue || 0) ? "premium" : ""
      ...
      const isHealthLifeRow = (row = {}) => {
        ...
        const positivePatterns = [
          ...
          /ריסק/,
          /מקרה מוות/,
          /אבדן כושר|אובדן כושר/,
          /נכויות/,
          /משכנתא/
        ];

        const negativePatterns = [
          /ביטוח רכב|רכב חובה|ביטוח מקיף|שירותי רכב|גרירה|שמשות|צד שלישי/,
          /ביטוח דירה|תכולה/,
          /נסיעות לחו["״]?ל/,
          /עסק|סחורה|ציוד הנדסי/
        ];

        const positiveHits = positivePatterns.filter((pattern) => pattern.test(source)).length;
        const negativeHits = negativePatterns.filter((pattern) => pattern.test(source)).length;

        if(positiveHits === 0) return false;
        if(negativeHits > 0 && positiveHits <= negativeHits) return false;
        return true;
      };

      const synthesizePolicyNumber = (payload = {}) => {
        const seed = [
          safeTrim(payload.idNumber),
          safeTrim(payload.company),
          safeTrim(payload.main),
          safeTrim(payload.sub),
          safeTrim(payload.productType),
          safeTrim(payload.period),
          String(payload.monthlyPremium ?? ""),
          safeTrim(payload.planClassification)
        ].filter(Boolean).join("|");
        let hash = 0;
      ...
      sheetNames.forEach((sheetName) => {
        ...
        if(!sheet) return;
        const rows = window.XLSX.utils.sheet_to_json(sheet, { header: 1, defval: "" });
        const headerMeta = findBestHeaderRow(rows);
        const sheetDebug = {
          sheetName,
          headerRow: headerMeta.index,
          headerScore: headerMeta.score,
          matchedFields: Array.from(headerMeta.matched || []),
          scannedRows: rows.length,
          extractedRows: 0
        };
        debug.sheets.push(sheetDebug);
        if(headerMeta.index === -1) return;

        const header = rows[headerMeta.index].map((x) => normalizeHeader(x));
        ...
        let currentSection = "";
        let acceptedInSheet = 0;

        rows.slice(headerMeta.index + 1).forEach((row) => {
          if(!Array.isArray(row) || isBlankishRow(row)) return;

          const sectionCell = row.find((cell) => looksLikeSectionCell(cell));
          if(sectionCell){
            currentSection = normalizeContent(sectionCell);
            return;
          }

          const rawCompany = idx.company >= 0 ? row[idx.company] : "";
          const rawPolicyNumber = safeTrim(idx.policyNumber >= 0 ? row[idx.policyNumber] : "");
          const premiumValue = idx.premium >= 0 ? extractNumericPremium(row[idx.premium]) : 0;

          const record = {
            section: currentSection,
            idNumber: safeTrim(idx.idNumber >= 0 ? row[idx.idNumber] : ""),
            main: safeTrim(idx.main >= 0 ? row[idx.main] : ""),
            ...
            planClassification: safeTrim(idx.planClassification >= 0 ? row[idx.planClassification] : ""),
            policyNumber: rawPolicyNumber,
            premiumValue
          };

          if(!isLikelyDataRow(record)) return;
          if(!record.company && !record.policyNumber) return;
          if(!isHealthLifeRow(record)) return;

          const premium = Number(record.premiumValue || 0);
          ...
          record.policyNumber = record.policyNumber || synthesizePolicyNumber(record);
        ...
        if(acceptedInSheet){
          debug.matchedSheet = debug.matchedSheet || sheetName;
          debug.headerRow = debug.headerRow === -1 ? headerMeta.index : debug.headerRow;
        }
        debug.totalExtractedRows += acceptedInSheet;
        sheetDebug.extractedRows = acceptedInSheet;
      });

      if(!extracted.length){
        console.log("HAR BITUACH DEBUG", debug);
        return { policies: [], fileName: "", debug };
      }
      ...
      debug.acceptedRows = policies.length;
      console.log("HAR BITUACH DEBUG", debug);
      return { policies, fileName: "", debug };
    },
