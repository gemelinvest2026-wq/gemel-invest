...
(() => {
  ...
  const BUILD = "20260329-mfa-auth-email-fix-v5";
...
const SystemRepairUI = {
  ...
  Auth._showMfaStep = function(agent, factorId){
    this._pendingMfa = { agent, factorId };
  ...
  Auth._verifyPendingMfa = async function(){
    ...
    const code = safeTrim($('#lcLoginMfaCode')?.value);
  ...
  const SecurityUI = {
    ...
    async render(){
      ...
      if(hasAuthSession) factor = await SupabaseMFA.getVerifiedTotpFactor();
      ...
      this.currentFactorId = active ? safeTrim(factor.factor?.id) : '';
      if(this.els.status) this.els.status.textContent = active ? 'האימות הדו־שלבי פעיל ומאומת.' : 'האימות הדו־שלבי עדיין לא הופעל.';
    ...
    async startEnrollment(){
      ...
      const existing = await SupabaseMFA.getVerifiedTotpFactor();
      if(existing.ok && existing.factor){ this.currentFactorId = safeTrim(existing.factor?.id); setAgentSecurity(agent.id,{ mfaEnabled:true, factorId:this.currentFactorId }); await App.persist('MFA already enabled'); return this.render(); }
    ...
    async verifyEnrollment(){
      const agent=this.getCurrentAgent(); if(!agent) return;
      const code=safeTrim(this.els.code?.value); if(!code) return this.setError('נא להזין קוד אימות');
