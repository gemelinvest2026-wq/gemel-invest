...
function renderKpisSettings(){
  ...
  function openPartialCancelEndorsementsModal_(opt){
    ...
    return new Promise(resolve => {
      const overlay = document.createElement('div');
      overlay.className = 'modalOverlay pcOverlay';
...
// Keep wizard title in sync
  ...
  function renderOldPoliciesStep(){
    ...
    // partial cancel endorsements picker (modal)
    if($('#old_decision')){
      $('#old_decision').addEventListener('change', async () => {
        const sel = $('#old_decision');
        const addBtn = $('#addOldBtn');
        if(!sel) return;

        if(sel.value === 'cancel_partial'){
          const prevRaw = (addBtn && addBtn.dataset.partialCancelNotes) ? String(addBtn.dataset.partialCancelNotes||'') : '';
          const prev = prevRaw ? prevRaw.split(',').map(s => s.trim()).filter(Boolean) : [];
          const res = await openPartialCancelEndorsementsModal_({ selected: prev });

          if(!res || !res.ok){
            // user cancelled -> revert to full cancel
            sel.value = 'cancel_full';
            if(addBtn) delete addBtn.dataset.partialCancelNotes;
            return;
          }

          if(addBtn) addBtn.dataset.partialCancelNotes = (res.selected || []).join(', ');
        }else{
          if(addBtn) delete addBtn.dataset.partialCancelNotes;
        }
      });
