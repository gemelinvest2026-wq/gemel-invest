...
const MIRROR_DISCLOSURE_LIBRARY = {
  ...
  const MirrorsUI = {
    ...
    getDisclosureKeysForPolicy(policy){
      ...
      if(type === 'מחלות קשות') return ['critical_illness'];
      if(type === 'סרטן') return ['cancer'];
      if(type === 'ריסק') return ['risk'];
      if(type === 'ריסק משכנתא') return ['mortgage'];
      if(type === 'תאונות אישיות') return ['accident_death','accident_disability'];
      return [];
    },
    ...
    getDisclosureEntries(rec){
      ...
      policies.forEach((policy, idx) => {
        const company = safeTrim(policy?.company);
        const companyLib = MIRROR_DISCLOSURE_LIBRARY[company];
