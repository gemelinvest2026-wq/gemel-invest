...
// ---------- AUTH (Clean, hardcoded users; no password storage) ----------
// NOTE: Per request, removed all prior login/password persistence logic.
// Users: admin + 2/3 agents. Password is the same for all: 1234.
const AUTH_PASSWORD = '1234';
const AUTH_USERS = [
  { username:'admin', role:'admin', displayName:'מנהל מערכת' },
  { username:'agent1', role:'agent', displayName:'נציג 1' },
  { username:'agent2', role:'agent', displayName:'נציג 2' },
  { username:'agent3', role:'agent', displayName:'נציג 3' },
];
...
function applyPermissions_(){
  ...
  if(badgeNameEl){
    const dn = (state.currentUser && state.currentUser.displayName) ? state.currentUser.displayName : (state.agentName || 'אורח');
    badgeNameEl.textContent = 'נציג: ' + dn;
  }
  // admin-only items
...
function visibleCustomers_(){
  ...
  const me = (state.currentUser && state.currentUser.displayName) ? state.currentUser.displayName : state.agentName;
...
function getSession_(){
  try{
    ...
    const u = AUTH_USERS.find(x => x.username === s.username);
...
function enterApp_(user){
  state.currentUser = { username:user.username, role:user.role, displayName:user.displayName };
  state.agentName = user.displayName; // keep compatibility with existing UI
  applyPermissions_();
  ...
  // Ensure processes are loaded (local demo data)
  loadProcesses_();
...
function initLogin_(){
  ...
  if(form){
    form.addEventListener('submit', (e) => {
      ...
      const u = AUTH_USERS.find(x => x.username === username);
      if(!u){
        setErr('שם משתמש לא קיים');
        return;
      }
...
function renderKpisSettings(){
  ...
  // Per request: removed password/user management screens to avoid login/password bugs.
