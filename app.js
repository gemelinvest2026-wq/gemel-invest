...
(function(){
  ...
  // =========================
  (function initTalkMeBridge_(){
    const ALLOWED_TALKME_ORIGINS = new Set([
      'https://oriasomech-jpg.github.io',
      'https://gemel.invest.github.io'
    ]);

    function safeCopy_(obj){
      try{ return JSON.parse(JSON.stringify(obj)); }catch(_){ return obj; }
    }

    function findLatestProposalForCustomer_(customerId){
      const cid = String(customerId || '').trim();
      if(!cid) return null;
      // state.proposals is newest-first (unshift). Return first match.
      const arr = Array.isArray(state.proposals) ? state.proposals : [];
      return arr.find(p => String(p && p.customerId || '') === cid) || null;
    }

    window.addEventListener('message', (event) => {
      try{
        const origin = event && event.origin ? String(event.origin) : '';
        if(!ALLOWED_TALKME_ORIGINS.has(origin)) return;
        ...
        if(data.type === 'TALKME_REQUEST_CUSTOMERS'){
          const payload = {
            ...
            customers: safeCopy_(Array.isArray(state.customers) ? state.customers : [])
          };
        ...
        // 2) Proposal for specific customer
        if(data.type === 'TALKME_REQUEST_PROPOSAL'){
          const customerId = String(data.customerId || '').trim();
          const proposal = findLatestProposalForCustomer_(customerId);
          const payload = {
            type: 'TALKME_PROPOSAL',
            customerId,
            proposal: proposal ? safeCopy_(proposal) : null
          };
          if(event.source && event.source.postMessage){
            event.source.postMessage(payload, origin);
          }
