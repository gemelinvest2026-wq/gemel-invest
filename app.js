...
(() => {
  ...
  const BUILD = "20260328-clal-risk-pdf-phase2-v1";
...
const ClalRiskWizard = {
    ...
    buildPdfPage1(data){
      ...
      return `<div class="lcClalRiskPdfPage">
        ...
        ${this.pdfText(d.insured.lastName, 63.8, 31.2)}
        ${this.pdfText(d.insured.firstName, 51.8, 31.2)}
        ${this.pdfText(d.insured.idNumber, 38.7, 31.2,'lcClalRiskPdfText--ltr')}
        ${this.pdfText(d.insured.birthDate, 27.2, 31.2,'lcClalRiskPdfText--ltr')}
        ${this.pdfCheck(genderMale, 16.4, 31.0)}
        ${this.pdfCheck(genderFemale, 11.7, 31.0)}
        ${this.pdfCheck(marital === 'רווק', 90.3, 35.3)}
        ${this.pdfCheck(marital === 'נשוי', 86.0, 35.3)}
        ${this.pdfCheck(marital === 'גרוש', 81.9, 35.3)}
        ${this.pdfCheck(marital === 'אלמן', 77.5, 35.3)}
        ${this.pdfText(d.insured.childrenCount, 60.0, 35.8,'lcClalRiskPdfText--ltr')}
        ${this.pdfText(d.insured.mobile, 46.8, 35.8,'lcClalRiskPdfText--ltr')}
        ${this.pdfText(d.insured.phone, 33.2, 35.8,'lcClalRiskPdfText--ltr')}
        ${this.pdfText(d.insured.email, 17.5, 35.8,'lcClalRiskPdfText--small lcClalRiskPdfText--ltr')}
        ${this.pdfText(d.insured.street, 62.8, 40.5)}
        ${this.pdfText(d.insured.houseNumber, 47.0, 40.5,'lcClalRiskPdfText--ltr')}
        ${this.pdfText(d.insured.city, 37.4, 40.5)}
        ${this.pdfText(d.insured.zipCode, 28.7, 40.5,'lcClalRiskPdfText--ltr')}
        ${this.pdfText(d.insured.occupation, 15.8, 40.5)}
        ${this.pdfCheck(d.basics.docsDelivery === 'דיגיטלי', 39.1, 46.8)}
        ${this.pdfCheck(d.basics.docsDelivery === 'דואר רגיל', 29.7, 46.8)}
        ${this.pdfCheck(d.basics.initiatedMarketing === 'כן', 10.8, 17.8)}
        ${this.pdfCheck(risk.abroad180 === 'כן', 36.9, 66.6)}
        ${this.pdfCheck(risk.abroad180 === 'לא', 43.0, 66.6)}
        ${this.pdfText(risk.abroad180Details, 18.2, 66.4,'lcClalRiskPdfText--tiny')}
        ${this.pdfCheck(risk.israeliCitizen === 'כן', 36.9, 70.7)}
        ${this.pdfCheck(risk.israeliCitizen === 'לא', 43.0, 70.7)}
        ${this.pdfCheck(risk.isSmoker === 'כן', 36.9, 75.0)}
        ${this.pdfCheck(risk.isSmoker === 'לא', 43.0, 75.0)}
        ${this.pdfText('מעשן/ת בשנתיים האחרונות', 15.8, 75.1,'lcClalRiskPdfText--tiny')}
        ${this.pdfCheck(risk.workHazards === 'כן', 36.9, 79.0)}
        ${this.pdfCheck(risk.workHazards === 'לא', 43.0, 79.0)}
        ${this.pdfText(risk.workHazardsDetails, 15.2, 79.0,'lcClalRiskPdfText--tiny')}
        ${this.pdfCheck(risk.hobbyHazards === 'כן', 36.9, 83.0)}
        ${this.pdfCheck(risk.hobbyHazards === 'לא', 43.0, 83.0)}
        ${this.pdfText(risk.hobbyHazardsDetails, 15.2, 83.0,'lcClalRiskPdfText--tiny')}
        ${this.pdfText(ben.fullName, 63.2, 92.8)}
        ${this.pdfText(ben.idNumber, 48.1, 92.8,'lcClalRiskPdfText--ltr')}
        ${this.pdfText(ben.relationship, 28.8, 92.8)}
        ${this.pdfText(ben.sharePercent, 9.8, 92.8,'lcClalRiskPdfText--ltr')}
        ${this.pdfText(d.basics.startDate, 62.8, 98.1,'lcClalRiskPdfText--ltr')}
      </div>`;
    },
    buildPdfPage2(data){
      const c = data.covers || {};
      return `<div class="lcClalRiskPdfPage">
        ...
        ${this.pdfCheck(c.sapphire?.enabled, 92.2, 23.0)}
        ${this.pdfText(c.sapphire?.amount, 55.5, 23.3,'lcClalRiskPdfText--ltr')}
        ${this.pdfCheck(c.sapphire10?.enabled, 92.2, 28.5)}
        ${this.pdfText(c.sapphire10?.amount, 55.5, 28.9,'lcClalRiskPdfText--ltr')}
        ${this.pdfCheck(c.sapphire15?.enabled, 92.2, 34.3)}
        ${this.pdfText(c.sapphire15?.amount, 55.5, 34.6,'lcClalRiskPdfText--ltr')}
        ${this.pdfCheck(c.accidentalDeath?.enabled, 92.2, 40.0)}
        ${this.pdfText(c.accidentalDeath?.amount, 55.5, 40.2,'lcClalRiskPdfText--ltr')}
        ${this.pdfCheck(c.accidentalDisability?.enabled, 92.2, 45.7)}
        ${this.pdfText(c.accidentalDisability?.amount, 55.5, 45.9,'lcClalRiskPdfText--ltr')}
        ${this.pdfCheck(c.waiverAndDisability?.enabled, 92.2, 51.8)}
        ${this.pdfText(c.waiverAndDisability?.monthlyBenefit, 55.5, 59.1,'lcClalRiskPdfText--ltr')}
        ${this.pdfCheck(c.waiverAndDisability?.termAge === '60', 62.0, 53.5)}
        ${this.pdfCheck(c.waiverAndDisability?.termAge === '62', 62.0, 57.3)}
        ${this.pdfCheck(c.waiverAndDisability?.termAge === '64', 62.0, 61.2)}
        ${this.pdfCheck(c.waiverAndDisability?.termAge === '65', 62.0, 65.2)}
        ${this.pdfCheck(c.waiverAndDisability?.termAge === '67', 62.0, 69.1)}
        ${this.pdfCheck(c.waiverAndDisability?.specificOccupation, 92.2, 59.8)}
        ${this.pdfCheck(c.waiverAndDisability?.franchise, 92.2, 63.5)}
        ${this.pdfCheck(c.waiverAndDisability?.noBituachLeumiOffset, 92.2, 67.0)}
        ${this.pdfCheck(c.waiverAndDisability?.shortWaiting, 92.2, 70.8)}
      </div>`;
    },
    buildPdfPage3(data){
      ...
      return `<div class="lcClalRiskPdfPage">
        ...
        ${this.pdfText(data.selectedCoverLabels.join(' • '), 54.0, 24.5,'lcClalRiskPdfText--tiny')}
        ${this.pdfText(data.covers?.waiverAndDisability?.monthlyBenefit, 67.7, 27.0,'lcClalRiskPdfText--ltr')}
        ${this.pdfText(ben.fullName, 41.0, 27.0)}
        ${this.pdfText(ben.relationship, 19.8, 27.0)}
        ${this.pdfCheck(data.covers?.waiverAndDisability?.enabled, 92.2, 49.1)}
        ${this.pdfCheck(data.covers?.waiverAndDisability?.termAge === '60', 61.8, 50.8)}
        ${this.pdfCheck(data.covers?.waiverAndDisability?.termAge === '62', 61.8, 55.0)}
        ${this.pdfCheck(data.covers?.waiverAndDisability?.termAge === '64', 61.8, 59.1)}
        ${this.pdfCheck(data.covers?.waiverAndDisability?.termAge === '65', 61.8, 63.3)}
        ${this.pdfCheck(data.covers?.waiverAndDisability?.termAge === '67', 61.8, 67.4)}
        ${this.pdfCheck(data.covers?.waiverAndDisability?.specificOccupation, 92.2, 57.1)}
        ${this.pdfCheck(data.covers?.waiverAndDisability?.franchise, 92.2, 61.0)}
        ${this.pdfCheck(data.covers?.waiverAndDisability?.shortWaiting, 92.2, 65.2)}
        ${this.pdfText(`נוצר מתוך וויזארד כלל ריסק של GEMEL INVEST · ${this.formatDisplayDate(data.generatedAt)}`, 34.0, 83.5,'lcClalRiskPdfText--tiny')}
      </div>`;
    },
    ...
    async exportPdf(){
      try{
        ...
        try{
          host = document.createElement('div');
          host.className = 'lcClalRiskPdfHost';
          host.setAttribute('dir','rtl');
          host.style.position = 'fixed';
          host.style.top = '0';
          host.style.left = '-20000px';
          ...
          host.style.pointerEvents = 'none';
          host.style.zIndex = '-1';
          host.innerHTML = this.buildPdfMarkup(data);
          document.body.appendChild(host);
          ...
          const pages = Array.from(host.querySelectorAll('.lcClalRiskPdfPage'));
          if(!pages.length) throw new Error('No Clal Risk PDF pages found');
          ...
          for(let i = 0; i < pages.length; i += 1){
            ...
            if(i > 0) pdf.addPage();
            pdf.addImage(imgData, 'JPEG', marginX, marginY, usableWidth, usableHeight, undefined, 'FAST');
          }
          ...
          pdf.save(fileName);
          this.els.meta.textContent = 'PDF ראשוני של כלל ריסק נוצר בהצלחה.';
