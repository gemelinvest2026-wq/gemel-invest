...
(() => {
  ...
  const BUILD = "20260328-clal-risk-pdf-direct-overlay-v5";
...
const ClalRiskWizard = {
    ...
    getDirectPdfFieldMap(data){
      ...
      const marital = safeTrim(insured.maritalStatus);
      const draw = [];
      const pushText = (page, text, x, y, options = {}) => {
        if(safeTrim(text) === '') return;
        draw.push({ page, kind:'text', text: String(text), x, y, options });
      };
      const pushCheck = (page, active, x, y, options = {}) => {
        if(!active) return;
        draw.push({ page, kind:'check', text: options.mark || '✓', x, y, options });
      };

      pushText(0, insured.lastName, 64.4, 30.1, { fontSize:12, align:'right' });
      pushText(0, insured.firstName, 52.2, 30.1, { fontSize:12, align:'right' });
      pushText(0, insured.idNumber, 39.05, 30.1, { fontSize:12, align:'left', direction:'ltr' });
      pushText(0, insured.birthDate, 27.3, 30.1, { fontSize:12, align:'left', direction:'ltr' });
      pushCheck(0, insured.gender === 'זכר', 16.55, 29.95, { fontSize:12 });
      pushCheck(0, insured.gender === 'נקבה', 12.0, 29.95, { fontSize:12 });
      pushCheck(0, marital === 'רווק', 89.55, 34.3, { fontSize:12 });
      pushCheck(0, marital === 'נשוי', 85.35, 34.3, { fontSize:12 });
      pushCheck(0, marital === 'גרוש', 81.1, 34.3, { fontSize:12 });
      pushCheck(0, marital === 'אלמן', 76.8, 34.3, { fontSize:12 });
      pushText(0, insured.childrenCount, 60.1, 34.7, { fontSize:12, align:'left', direction:'ltr' });
      pushText(0, insured.mobile, 46.85, 34.7, { fontSize:12, align:'left', direction:'ltr' });
      pushText(0, insured.phone, 33.2, 34.7, { fontSize:12, align:'left', direction:'ltr' });
      pushText(0, insured.email, 18.2, 34.7, { fontSize:10, align:'left', direction:'ltr' });
      pushText(0, insured.street, 63.1, 39.2, { fontSize:12, align:'right' });
      pushText(0, insured.houseNumber, 47.6, 39.2, { fontSize:12, align:'left', direction:'ltr' });
      pushText(0, insured.city, 37.55, 39.2, { fontSize:12, align:'right' });
      pushText(0, insured.zipCode, 28.9, 39.2, { fontSize:12, align:'left', direction:'ltr' });
      pushText(0, insured.occupation, 16.25, 39.2, { fontSize:11, align:'right' });
      pushCheck(0, basics.docsDelivery === 'דיגיטלי', 39.15, 45.6, { fontSize:12 });
      pushCheck(0, basics.docsDelivery === 'דואר רגיל', 29.35, 45.6, { fontSize:12 });
      pushCheck(0, basics.initiatedMarketing === 'כן', 10.55, 17.0, { fontSize:12 });
      pushCheck(0, risk.abroad180 === 'כן', 36.65, 65.45, { fontSize:12 });
      pushCheck(0, risk.abroad180 === 'לא', 42.75, 65.45, { fontSize:12 });
      pushText(0, risk.abroad180Details, 18.6, 65.45, { fontSize:9, align:'right' });
      pushCheck(0, risk.israeliCitizen === 'כן', 36.65, 69.5, { fontSize:12 });
      pushCheck(0, risk.israeliCitizen === 'לא', 42.75, 69.5, { fontSize:12 });
      pushCheck(0, risk.isSmoker === 'כן', 36.65, 73.6, { fontSize:12 });
      pushCheck(0, risk.isSmoker === 'לא', 42.75, 73.6, { fontSize:12 });
      pushText(0, 'מעשן/ת בשנתיים האחרונות', 15.9, 73.6, { fontSize:9, align:'right' });
      pushCheck(0, risk.workHazards === 'כן', 36.65, 77.6, { fontSize:12 });
      pushCheck(0, risk.workHazards === 'לא', 42.75, 77.6, { fontSize:12 });
      pushText(0, risk.workHazardsDetails, 15.5, 77.6, { fontSize:9, align:'right' });
      pushCheck(0, risk.hobbyHazards === 'כן', 36.65, 81.55, { fontSize:12 });
      pushCheck(0, risk.hobbyHazards === 'לא', 42.75, 81.55, { fontSize:12 });
      pushText(0, risk.hobbyHazardsDetails, 15.5, 81.55, { fontSize:9, align:'right' });
      pushText(0, ben.fullName, 63.55, 91.5, { fontSize:12, align:'right' });
      pushText(0, ben.idNumber, 48.45, 91.5, { fontSize:12, align:'left', direction:'ltr' });
      pushText(0, ben.relationship, 29.1, 91.5, { fontSize:11, align:'right' });
      pushText(0, ben.sharePercent, 10.35, 91.5, { fontSize:12, align:'left', direction:'ltr' });
      pushText(0, basics.startDate, 63.0, 96.95, { fontSize:12, align:'left', direction:'ltr' });

      pushCheck(1, covers.sapphire?.enabled, 91.6, 22.4, { fontSize:12 });
      pushText(1, covers.sapphire?.amount, 55.7, 22.75, { fontSize:12, align:'left', direction:'ltr' });
      pushCheck(1, covers.sapphire10?.enabled, 91.6, 28.0, { fontSize:12 });
      pushText(1, covers.sapphire10?.amount, 55.7, 28.25, { fontSize:12, align:'left', direction:'ltr' });
      pushCheck(1, covers.sapphire15?.enabled, 91.6, 33.6, { fontSize:12 });
      pushText(1, covers.sapphire15?.amount, 55.7, 33.95, { fontSize:12, align:'left', direction:'ltr' });
      pushCheck(1, covers.accidentalDeath?.enabled, 91.6, 39.3, { fontSize:12 });
      pushText(1, covers.accidentalDeath?.amount, 55.7, 39.6, { fontSize:12, align:'left', direction:'ltr' });
      pushCheck(1, covers.accidentalDisability?.enabled, 91.6, 45.0, { fontSize:12 });
      pushText(1, covers.accidentalDisability?.amount, 55.7, 45.25, { fontSize:12, align:'left', direction:'ltr' });
      pushCheck(1, covers.waiverAndDisability?.enabled, 91.6, 51.0, { fontSize:12 });
      pushText(1, covers.waiverAndDisability?.monthlyBenefit, 55.7, 51.25, { fontSize:12, align:'left', direction:'ltr' });
      pushCheck(1, covers.waiverAndDisability?.termAge === '60', 61.5, 52.8, { fontSize:12 });
      pushCheck(1, covers.waiverAndDisability?.termAge === '62', 61.5, 56.7, { fontSize:12 });
      pushCheck(1, covers.waiverAndDisability?.termAge === '64', 61.5, 60.6, { fontSize:12 });
      pushCheck(1, covers.waiverAndDisability?.termAge === '65', 61.5, 64.55, { fontSize:12 });
      pushCheck(1, covers.waiverAndDisability?.termAge === '67', 61.5, 68.45, { fontSize:12 });
      pushCheck(1, !!covers.waiverAndDisability?.specificOccupation, 91.6, 58.7, { fontSize:12 });
      pushCheck(1, !!covers.waiverAndDisability?.franchise, 91.6, 62.35, { fontSize:12 });
      pushCheck(1, !!covers.waiverAndDisability?.noBituachLeumiOffset, 91.6, 65.95, { fontSize:12 });
      pushCheck(1, !!covers.waiverAndDisability?.shortWaiting, 91.6, 69.7, { fontSize:12 });

      return draw;
    },
    async fetchArrayBuffer(urlCandidates = []){
      let lastError = null;
      ...
      throw lastError || new Error('ASSET_NOT_FOUND');
    },
    async buildTextSpriteBytes(textValue, options = {}){
      ...
      const color = safeTrim(options.color) || '#17386d';
      const fontFamily = options.fontFamily || 'Arial, "Noto Sans Hebrew", "Segoe UI", sans-serif';
      const padX = Math.max(6, Number(options.padX) || 10);
      const padY = Math.max(4, Number(options.padY) || 6);
      const scale = Math.max(2, Math.ceil(window.devicePixelRatio || 2));
      const canvas = document.createElement('canvas');
      const ctx = canvas.getContext('2d');
      const fontDecl = `${fontSize}px ${fontFamily}`;
      ctx.font = fontDecl;
      const metrics = ctx.measureText(text || ' ');
      const width = Math.max(24, Math.ceil(metrics.width + (padX * 2)));
      const height = Math.max(fontSize + (padY * 2), Math.ceil(fontSize * 1.9));
      ...
      const drawX = align === 'left' ? padX : (width - padX);
    ...
    async drawSpriteOnPage(pdfDoc, page, item, pageSize){
      if(!item || !page) return;
      const opts = item.options || {};
      const sprite = await this.buildTextSpriteBytes(item.text, opts);
      const image = await pdfDoc.embedPng(sprite.bytes);
      const pageWidth = pageSize.width;
      const pageHeight = pageSize.height;
      const targetHeight = Math.max(8, Number(opts.targetHeight) || (Number(opts.fontSize) || 12) * 1.18);
      const ratio = sprite.width / Math.max(1, sprite.height);
      const targetWidth = targetHeight * ratio;
      let x = (pageWidth * Number(item.x || 0)) / 100;
      const yCenter = pageHeight - ((pageHeight * Number(item.y || 0)) / 100);
      let y = yCenter - (targetHeight / 2);
      if(opts.align === 'left'){
        // x stays as the left edge
    ...
    async exportPdf(){
      try{
        ...
        if(!PdfLibApi?.PDFDocument){
          window.alert('מנוע ה-PDF החדש עדיין לא נטען. רענן את המערכת ונסה שוב.');
          return;
        }
        ...
        try{
          ...
          const { PDFDocument } = PdfLibApi;
          const pdfDoc = await PDFDocument.create();
          const pageBuffers = await Promise.all([
            this.fetchArrayBuffer(['./clal-risk-page-1.png']),
            this.fetchArrayBuffer(['./clal-risk-page-2.png']),
            this.fetchArrayBuffer(['./clal-risk-page-3.png'])
          ]);

          const embeddedPages = await Promise.all(pageBuffers.map((buf) => pdfDoc.embedPng(buf)));
          const pageSizes = embeddedPages.map((img) => ({ width: img.width, height: img.height }));
          const pdfPages = embeddedPages.map((img, idx) => {
          ...
          const items = this.getDirectPdfFieldMap(data);
          for(const item of items){
            await this.drawSpriteOnPage(pdfDoc, pdfPages[item.page], item, pageSizes[item.page]);
          }
          ...
          this.els.meta.textContent = 'PDF של כלל ריסק נוצר במנוע החדש של pdf-lib.';
        ...
        window.alert('לא הצלחתי להפיק את ה-PDF החדש של כלל ריסק. ודא שקבצי הרקע clal-risk-page-1/2/3.png קיימים בריפו ונסה שוב.');
      }
    }
  };
