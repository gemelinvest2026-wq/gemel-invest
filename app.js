...
(() => {
  ...
  const BUILD = "20260329-mfa-clean-enroll-v7";
...
const SystemRepairUI = {
  ...
  const SupabaseMFA = {
    ...
    extractQrMarkup(data){
      ...
      if(qr) return `<img alt="QR" src="${qr.startsWith('data:') ? qr : 'data:image/svg+xml;utf8,' + encodeURIComponent(qr)}"/>`;
      return '<div class="muted">לא התקבל QR מהרישום.</div>';
    }
  };
