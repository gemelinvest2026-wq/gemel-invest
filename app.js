...
// open health covers drawer (Health only)
    ...
    renderStep1(ins){
      ...
      const pct = Math.round(((idx + 1) / Math.max(questions.length, 1)) * 100);
      const prevLabel = idx > 0 ? (questions[idx - 1]?.title || '') : '';
      const nextLabel = idx < questions.length - 1 ? (questions[idx + 1]?.title || '') : 'מעבר לשלב BMI';

      return `
        <div class="lcStep1Premium">
          <div class="lcStep1Premium__main">
            <div class="lcStep1Hero">
              <div class="lcStep1Hero__eyebrow">שאלון מדורג פרימיום · שלב 1</div>
              <div class="lcStep1Hero__title">פרטי ${escapeHtml(ins.label)}</div>
              <div class="lcStep1Hero__sub">ממלאים שאלה אחרי שאלה, בצורה נקייה, ממוקדת ויוקרתית — בלי לאבד אף שדה מהטופס המקורי.</div>
              <div class="lcStep1FlowBar"><span style="width:${pct}%"></span></div>
              <div class="lcStep1FlowMeta"><span>שאלה ${idx + 1} מתוך ${questions.length}</span><span>${pct}% הושלם</span></div>
            </div>

            <section class="lcStep1QuestionCard">
              <div class="lcStep1QuestionCard__top">
                <div class="lcStep1QuestionCard__tag">${escapeHtml(ins.label)}</div>
                <div class="lcStep1QuestionCard__mini">${escapeHtml(this.steps.find(s => s.id === 1)?.title || 'פרטי לקוח')}</div>
              </div>
              ...
              <div class="lcStep1QuestionCard__peek">
                <div class="lcStep1QuestionCard__peekItem"><span>הקודם</span><strong>${escapeHtml(prevLabel || 'תחילת השלב')}</strong></div>
                <div class="lcStep1QuestionCard__peekItem"><span>הבא</span><strong>${escapeHtml(nextLabel || '')}</strong></div>
              </div>
            </section>
          </div>
...
// ---------- Step 6 ----------
    ...
    async saveDraft(){
      ...
      const recordId = this.editingDraftId || ("prop_" + Date.now().toString(16) + "_" + Math.random().toString(16).slice(2,8));
      const record = normalizeProposalRecord({
        ...
        createdAt: (() => {
          const existing = (State.data?.proposals || []).find(x => String(x.id) === String(recordId));
          return existing?.createdAt || nowISO();
        })(),
        updatedAt: nowISO(),
        currentStep: this.step || 1,
      ...
      ProposalsUI.render();
      if(persistRes?.ok){
        this.setHint("ההצעה נשמרה בהצלחה ותופיע בתיקיית ההצעות");
      }else{
        this.setHint("ההצעה נשמרה במערכת, אבל הייתה בעיית חיבור ל-Google Sheets. בדוק סנכרון.");
      }
    },
