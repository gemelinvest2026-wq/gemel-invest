...
// open health covers drawer (Health only)
    ...
    normalizeHarPolicyType(row){
      ...
      const source = [main, sub, productType].join(" | ");
      const fallback = [section, source].join(" | ");
      ...
      if(/בריאות|השתלות|תרופות|ניתוחים|ייעוץ|בדיקות|אמבולטורי|שב"ן|שב״ן/i.test(source)) return "בריאות";
      if(/מקרה מוות|ביטוח חיים/i.test(source)) return "ריסק";

      if(/תחום\s*-\s*בריאות|תחום\s*-\s*חיים|תחום\s*-\s*תאונות/i.test(fallback)){
        if(/חיים/i.test(section)) return "ריסק";
        return "בריאות";
      }
      return "";
    },

    isRelevantHarHealthRow(row){
      const section = safeTrim(row?.section);
      const main = safeTrim(row?.main);
      const sub = safeTrim(row?.sub);
      const productType = safeTrim(row?.productType);
      const source = [section, main, sub, productType].join(" | ");
      if(!source) return false;
      if(/ביטוח רכב|רכב חובה|ביטוח מקיף|דירה|מבנה|תכולה/i.test(source)) return false;
      if(/תחום\s*-\s*בריאות|תחום\s*-\s*חיים|תחום\s*-\s*תאונות/i.test(source)) return true;
      return /סיעוד|סיעודי|בריאות|השתלות|תרופות|ניתוחים|ייעוץ|בדיקות|אמבולטורי|מחלות קשות|סרטן|תאונות אישיות|אבדן כושר|אובדן כושר|ביטוח חיים|מקרה מוות|נכויות|משכנתא/i.test(source);
    },
    ...
    parseHarBituachWorkbook(buffer, ins){
      ...
      sheetNames.forEach((sheetName) => {
        ...
        rows.slice(headerIdx + 1).forEach((row) => {
          ...
          if(!this.isRelevantHarHealthRow(draft)) return;
          const policyNumber = safeTrim(row[idx.policyNumber]);
          const company = this.normalizeHarCompanyName(row[idx.company]);
          if(!policyNumber || !company) return;
          const normalizedType = this.normalizeHarPolicyType(draft);
          ...
          extracted.push({
            ...
            annualPremium,
            normalizedType,
            includedLabel: this.normalizeHarIncludedLabel(draft)
          });
        });
      });

      if(!matchedSheets) throw new Error("מבנה קובץ הר הביטוח לא זוהה.");
