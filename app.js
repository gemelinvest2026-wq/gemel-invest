...
if(route === 'settings'){
  ...
  function renderDiscountSpecPage(){
    // Full-page embedded reference to external discounts spec site
    viewEl.innerHTML = `
      <div class="panel specPanel">
        <div class="specHead">
          <div class="specTitle">מפרט הנחות</div>
          <div class="specActions">
            <button class="btn btnSoft" id="specOpenNew">פתח בחלון חדש</button>
            <button class="btn" id="specBack">חזור</button>
          </div>
        </div>
        ...
        <div class="specFrameWrap">
          <iframe
            id="specFrame"
            class="specFrame"
            src="${escapeAttr(POLICY_SPEC_URL)}"
            loading="lazy"
            referrerpolicy="no-referrer-when-downgrade"
          ></iframe>
        </div>

        <div class="sideHint" style="margin-top:10px">אם הדף לא נטען בתוך המערכת (חסימת iframe), לחץ “פתח בחלון חדש”.</div>
      </div>
    `;

    const openBtn = document.getElementById('specOpenNew');
    if(openBtn) openBtn.addEventListener('click', () => {
      window.open(POLICY_SPEC_URL, '_blank', 'noopener');
    });

    const backBtn = document.getElementById('specBack');
    if(backBtn) backBtn.addEventListener('click', () => setRoute('customers'));
  }
