...
(() => {
  ...
  const BUILD = "20260329-mfa-google-auth-v1";
  ...
  const Auth = {
    ...
    async _submit(){
      ...
      if(authEmail){
        const authResult = await SupabaseMFA.signInWithPassword(authEmail, pin);
        if(!authResult.ok) return this._setError("האימייל/סיסמה של משתמש ה-Auth לא תואמים. בדוק את auth email או את הסיסמה.");
...
this.applyRoleUI();
  ...
  const SecurityUI = {
    ...
    async render(){
      ...
      if(this.els?.start) this.els.start.disabled = !security.authEmail;
    ...
    async startEnrollment(){
      ...
      const enrolled = await SupabaseMFA.enrollTotp();
      if(!enrolled.ok) return this.setError("לא הצלחתי להתחיל הרשמה ל-Google Authenticator.");
