...
(() => {
  ...
  function saveSession_(sess){
    try{
      localStorage.setItem(SESSION_LS_KEY, JSON.stringify(sess || {}));
    }catch(_){}
  }
  ...
  function authenticate_(username, password){
    ...
    if(!u || !p) return { ok:false, msg:'חסר שם משתמש או סיסמה' };
    const user = findUser_(u);
    if(!user || user.active === false) return { ok:false, msg:'שם משתמש או סיסמה שגויים' };
    if(String(user.password||'') !== p) return { ok:false, msg:'שם משתמש או סיסמה שגויים' };
    return { ok:true, user };
  }

  function loginAs_(user){
    state.currentUser = { username:user.username, role:user.role, displayName:user.displayName };
    state.agentName = user.displayName;
    applyPermissions_();
    saveSession_({ username:user.username });
    setAuthLocked_(false);
    render();
  }
  ...
  function initAuth_(){
    ...
    if(form){
      form.addEventListener('submit', (e) => {
        e.preventDefault();
        showLoginError_('');
        const r = authenticate_(userEl ? userEl.value : '', passEl ? passEl.value : '');
        if(!r.ok){
          showLoginError_(r.msg || 'שגיאה');
    ...
    const u = sess && sess.username ? findUser_(sess.username) : null;
    if(u && u.active !== false){
      loginAs_(u);
    }else{
      setAuthLocked_(true);
...
`;
  ...
  function openPrintView(proposal){
    ...
    try {
      ...
      // Mode 2 (as requested): show EVERYTHING, including empty/0/"לא" values.
      const PRINT_SHOW_ALL = true;

      function isPlainObject(x){ return !!x && typeof x==='object' && !Array.isArray(x); }
      function kvLines(obj, prefix='', depth=0, out=[]){
        if(depth>3) return out;
        if(Array.isArray(obj)){
          if(!obj.length){ out.push([prefix || 'list', '—']); return out; }
          obj.forEach((v,i)=>{
            const key = prefix ? `${prefix}[${i}]` : `[${i}]`;
            if(isPlainObject(v) || Array.isArray(v)) kvLines(v, key, depth+1, out);
            else out.push([key, (v===0 || v===false) ? String(v) : (String(v ?? '').trim() || '—')]);
          });
          return out;
        }
        ...
        out.push([prefix || 'value', (obj===0 || obj===false) ? String(obj) : (String(obj ?? '').trim() || '—')]);
        return out;
      }

      function renderKVBox(title, obj){
        const pairs = kvLines(obj);
        const rows = pairs.map(([k,v]) => `<tr><td class="k">${esc(k)}</td><td class="v">${esc(v)}</td></tr>`).join('');
        return `
          <div class="box">
            <div class="boxTitle">${esc(title)}</div>
            <table class="t kv">
              <thead><tr><th>שדה</th><th>ערך</th></tr></thead>
              <tbody>${rows || `<tr><td colspan="2" class="mutedCell">—</td></tr>`}</tbody>
            </table>
          </div>
        `;
      }
      ...
      // Medical (Mode 2: show ALL questions for each insured)
      function medicalSection(){
        const qMap = getQuestionLabelMap();
        const out = [];

        // Keep question order as defined in the questionnaire steps
        const qOrder = Object.keys(qMap);

        for(const ins of insuredList){
          const bucketSet = p.medical && p.medical[ins.id];
          const bucket = bucketSet ? (bucketSet['DEFAULT|MEDICAL'] || bucketSet[Object.keys(bucketSet)[0]]) : null;
          const safeBucket = bucket || {};

          const rows = [];
          const ids = qOrder.length ? qOrder : Object.keys(safeBucket).filter(k=>!String(k).endsWith('_details'));
          for(const qid of ids){
            const meta = qMap[qid] || null;
            const label = meta ? meta.label : qid;
            const v = safeBucket[qid];
            const valRaw = (v===undefined || v===null || String(v).trim()==='') ? '—' : String(v);
            const det = safeBucket[qid + '_details'];
            const detailsRaw = (det===undefined || det===null || String(det).trim()==='') ? '—' : String(det);
            rows.push(`<tr><td>${esc(label)}</td><td>${esc(valRaw)}</td><td>${esc(detailsRaw)}</td></tr>`);
          }
          ...
          // Extra raw dump (still within the medical section) for absolute completeness
          if(PRINT_SHOW_ALL){
            out.push(renderKVBox(`שאלון רפואי — פירוט מלא (Raw) — ${labelInsured(ins.id)}`, safeBucket));
          }
        }
      ...
      <body>
        <div class="page">
          ...
          ${PRINT_SHOW_ALL ? renderKVBox('מבוטחים — פירוט מלא (Raw)', p.insuredList || []) : ''}
          ...
          ${PRINT_SHOW_ALL ? renderKVBox('פוליסות קיימות — פירוט מלא (Raw)', oldPolicies) : ''}
          ...
          ${PRINT_SHOW_ALL ? renderKVBox('פוליסות חדשות — פירוט מלא (Raw)', newPolicies) : ''}

          <h2>אמצעי תשלום</h2>
          <div class="muted">${esc(paymentSummary())}</div>

          ${PRINT_SHOW_ALL ? renderKVBox('אמצעי תשלום — פירוט מלא (Raw)', p.payer || {}) : ''}

          <h2>פרטי לקוח — פירוט מלא</h2>
          ${renderKVBox('לקוח (Raw)', p.customer || {})}
