...
(() => {
  ...
  const Auth = {
    ...
    async _submit(){
      ...
      const adminAuth = State.data?.meta?.adminAuth || null;
      const adminOk = !!(adminAuth && adminAuth.active !== false && username === safeTrim(adminAuth.username) && pin === safeTrim(adminAuth.pin));

      if (adminOk) {
        this.current = { name: safeTrim(adminAuth.username) || "מנהל מערכת", role:"admin" };
        if(remember) this._saveSession(this.current);
        else {
          try { localStorage.removeItem(LS_SESSION_KEY); } catch(_) {}
        }
        this.unlock();
...
this.els.syncDot = $("#syncDot");
      ...
      // admin auth (settings)
      this.els.adminAuthBox = $("#adminAuthBox");
      this.els.adminAuthUser = $("#adminAuthUser");
      this.els.adminAuthPin = $("#adminAuthPin");
      this.els.btnAdminAuthSave = $("#btnAdminAuthSave");
      this.els.adminAuthErr = $("#adminAuthErr");
...
// nav
      ...
      // admin auth save
      on(this.els.btnAdminAuthSave, "click", async () => {
        if(!Auth.isAdmin()) return;
        const u = safeTrim(this.els.adminAuthUser?.value);
        const p = safeTrim(this.els.adminAuthPin?.value);
        const errEl = this.els.adminAuthErr;
        const setErr = (msg) => {
          if(!errEl) return;
        ...
        setErr("");
        if(!u) return setErr("נא להזין שם משתמש חדש");
        if(!p) return setErr("נא להזין PIN חדש");

        State.data.meta = State.data.meta || {};
        State.data.meta.adminAuth = { username: u, pin: p, active: true };
        State.data.meta.updatedAt = nowISO();

        try {
          await App.persist("עודכנו פרטי מנהל מערכת");
        } catch(e){
          setErr("שגיאה בשמירה: " + String(e?.message || e));
          return;
        }

        // update current session/pill immediately
        if(Auth.current && Auth.isAdmin()){
          Auth.current.name = u;
          try { Auth._saveSession(Auth.current); } catch(_) {}
        }
        this.renderAuthPill();
        this.renderSyncStatus("פרטי מנהל נשמרו", "ok", nowISO());
        setErr("");
      });
...
txt.textContent = "";
    ...
    renderAdminAuthForm(){
      const box = this.els.adminAuthBox;
      if(!box) return;
      const isAdmin = Auth.isAdmin();
      ...
      if(this.els.adminAuthUser) this.els.adminAuthUser.value = safeTrim(adminAuth.username);
      if(this.els.adminAuthPin) this.els.adminAuthPin.value = safeTrim(adminAuth.pin);

      if(this.els.adminAuthErr){
        this.els.adminAuthErr.textContent = "";
        this.els.adminAuthErr.style.display = "none";
      }
    },
...
// ---------- Step 6 ----------
    renderStep6(ins){
      ...
      const chooser = `
        <div class="lcWSection lcStep6Payer">
          ...
          <div class="divider"></div>
          <div class="lcWGrid lcStep6Payer__payGrid">
            <div class="field lcSpanAll lcStep6Payer__method">
