...
function openUserModal_(opt){
  ...
  function validateMedicalStep(){
    ...
    p.agentInfo = p.agentInfo || { number:'', company:'' };
...
`;
  ...
  function renderSummaryStep(){
    ...
    wizardMain.innerHTML = `
      ...
      <div class="hr"></div>
      <div class="sectionTitle">פרטי סוכן (חובה)</div>
      <div class="grid2" style="margin-top:8px">
        <div class="field">
          <label for="agent_number">מספר סוכן (חובה)</label>
          <input id="agent_number" inputmode="numeric" placeholder="הקלד מספר סוכן" value="${escapeHtml((p.agentInfo && p.agentInfo.number) ? p.agentInfo.number : '')}" />
        </div>
        ${companySelectField('חברה (חובה)', 'agent_company', ['|בחר חברה','הראל','כלל','מנורה','הפניקס','מגדל','הכשרה'])}
      </div>
    ...
    // חובה: פרטי סוכן בסיכום (שדות חובה + שמירה להצעה)
    p.agentInfo = p.agentInfo || { number:'', company:'' };

    const requireAgentInfo_ = () => {
      const num = String((document.getElementById('agent_number')||{}).value || '').trim();
      const comp = String((document.getElementById('agent_company')||{}).value || '').trim();

      if(!num){
        toast('חסר', 'מלא מספר סוכן');
        document.getElementById('agent_number')?.focus?.();
        return false;
      }
      if(!comp || comp === '|בחר חברה'){
        toast('חסר', 'בחר חברה');
        document.getElementById('agent_company')?.focus?.();
        return false;
      }

      p.agentInfo.number = num;
      p.agentInfo.company = comp;
      markDraft();
      return true;
    };

    // Bind agent fields (so progress + save work even without clicking PDF)
    bindInput('agent_number', v => { p.agentInfo.number = String(v||'').trim(); markDraft(); });
    bindSelect('agent_company', v => { p.agentInfo.company = String(v||'').trim(); markDraft(); });
    setSelectValue('agent_company', p.agentInfo.company || '');
    wireCompanySelectLogo('agent_company');


    const pdfBtnEl = $('#pdfBtn');
    if(pdfBtnEl) pdfBtnEl.addEventListener('click', () => {
      if(!requireAgentInfo_()) return;
      p.pdfGenerated = true;
      openPrintView(p);
      markDraft();
      renderWizard();
        ...
        <div class="page">
          <div class="topbar">
            ...
            <div class="meta">
              ...
              <div><b>מספר סוכן:</b> ${esc((p.agentInfo && p.agentInfo.number) ? p.agentInfo.number : '')}</div>
              <div><b>חברה:</b> ${esc((p.agentInfo && p.agentInfo.company) ? p.agentInfo.company : '')}</div>
  ...
  function computeCompletion(p){
    ...
    const ai = p.agentInfo || {};
    const okAgent = !!String(ai.number || '').trim() && !!String(ai.company || '').trim();
    ...
    const items = [
      ...
      { key:'agent', label:'פרטי סוכן', ok: okAgent },
