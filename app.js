...
calcAge(dateStr){
  ...
  const MirrorsUI = {
    ...
    getPolicyStatusValue(policy){
      return safeTrim(policy?.status || policy?.policyStatus || policy?.state || 'פעיל');
    },
    ...
    renderReflection(rec){
      ...
      this.els.reflectCard.innerHTML = `<div class="mirrorsCard__head">
        ...
        ${harConsent === 'yes' ? `
          ...
          <div class="mirrorsReflectionList">${policies.length ? policies.map((row, idx) => this.renderReflectionPolicyCard(row, idx)).join('') : `<div class="mirrorsReflectNote">לא נמצאו פוליסות קיימות שמורות בתיק הלקוח.</div>`}</div>
