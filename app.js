...
Object.assign(Wizard, {
  phoenixForms: {
    ...
    mortgage_short: { key:'mortgage_short', path:'./assets/forms/phoenix/phoenix-mortgage-up-to-2m.pdf', title:'הפניקס · משכנתא עד 2 מיליון', description:'הטופס המקורי של הפניקס למשכנתא עד 2 מיליון. קובץ זה אינו כולל שדות AcroForm, לכן בשלב זה הוא מוצג כמקור המקורי ללא ירושה דיגיטלית מלאה.', questionnairePath:'./assets/forms/phoenix/phoenix-questionnaires.pdf' },
    mortgage_long: { key:'mortgage_long', path:'./assets/forms/phoenix/phoenix-mortgage-over-2m.pdf', title:'הפניקס · משכנתא מעל 2 מיליון', description:'הטופס המקורי של הפניקס למשכנתא מעל 2 מיליון. קובץ זה אינו כולל שדות AcroForm, לכן בשלב זה הוא מוצג כמקור המקורי ללא ירושה דיגיטלית מלאה.', questionnairePath:'./assets/forms/phoenix/phoenix-questionnaires.pdf' }
  },
  ...
  async createFilledPhoenixPdf(formDef, questionnaireMode=false){
    ...
    const resolvedSourcePath = (() => {
      try{ return new URL(sourcePath, window.location.href).href; }catch(_e){ return sourcePath; }
    })();
    const st = this.getPhoenixHealthState();
    st.prefillSnapshot = this.buildPhoenixPrefillSnapshot(formDef);

    // כאשר המערכת פתוחה ישירות מהדיסק (file://) או כשהדפדפן חוסם fetch ל-PDF,
    // נופלים חזרה למסמך המקורי עצמו כדי לא לעצור את שלב 8.
    let bytes = null;
    let fetchFailed = false;
    try{
      const res = await fetch(resolvedSourcePath);
    ...
    if(fetchFailed || !(window.PDFLib) || /mortgage/.test(formDef.key)){
      return { url: resolvedSourcePath, usedOriginal:true, fetchFailed };
    }
  ...
  async openPhoenixPdf(questionnaireMode=false){
    ...
    try{
      ...
      if(res.fetchFailed){
        this.setHint(questionnaireMode
          ? 'קובץ השאלונים נפתח ישירות מהתיקייה כי הדפדפן לא אפשר fetch מקומי.'
          : 'הטופס נפתח ישירות מהתיקייה. אם פתחת את המערכת מהדיסק המקומי, זה תקין.');
      }else{
  ...
  renderStep8(){
    ...
    const mortgageWarn = /mortgage/.test(formDef.key) ? `<div class="lcPhoenix__alert">שים לב: שני קבצי המשכנתא המקוריים ששלחת אינם מכילים כרגע שדות AcroForm מובנים, לכן בשלב זה המערכת מציגה אותם כמסמך המקורי עצמו, אך בלי שפיכה דיגיטלית מלאה של השדות לתוכו.</div>` : '';
