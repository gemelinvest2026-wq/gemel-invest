...
init(){
      ...
      on(document, "keydown", (ev) => {
        if(ev.key === "Escape") this.closeHealthFindingsModal();
      });
    ...
    renderBody(){
      ...
      else if(this.step === 9) this.bindAgentNumberInputs();
    },
...
// ---------- Step 6 ----------
    ...
    getAgentNumberStore(){
      ...
      if(!primary.data.agentNumbersByCompany || typeof primary.data.agentNumbersByCompany !== "object") primary.data.agentNumbersByCompany = {};
      return primary.data.agentNumbersByCompany;
    },

    getNewPolicyCompanies(){
      const seen = new Set();
      return (this.newPolicies || []).reduce((acc, p) => {
        const company = safeTrim(p?.company);
        if(!company || seen.has(company)) return acc;
        seen.add(company);
        acc.push(company);
        return acc;
      }, []);
    ...
    getExistingPolicyStatusLabel(status){
      ...
      return map[key] || 'טרם הוגדר';
    },

    getExistingPolicyStatusTone(status){
      const key = safeTrim(status);
      if(key === 'full' || key === 'partial_health') return 'danger';
      if(key === 'agent_appoint') return 'success';
      if(key === 'nochange_client' || key === 'nochange_collective') return 'neutral';
      return 'pending';
    },

    bindAgentNumberInputs(){
      const store = this.getAgentNumberStore();
      $$('[data-agent-company]', this.els.body).forEach(el => {
        const company = safeTrim(el.getAttribute('data-agent-company'));
        if(!company) return;
        const setVal = () => {
    ...
    renderStep9(){
      ...
      const renderExistingCards = (ins) => {
        const list = ins.data?.existingPolicies || [];
        if(!list.length) return `<div class="muted small">אין פוליסות קיימות עבור ${escapeHtml(ins.label)}.</div>`;
        return `<div class="lcSummaryCards">${list.map((p, idx) => {
          const logoSrc = this.getCompanyLogoSrc(p.company);
          const c = (ins.data?.cancellations || {})[p.id] || {};
          const tone = this.getExistingPolicyStatusTone(c.status);
          const statusLabel = this.getExistingPolicyStatusLabel(c.status);
          const monthly = safeTrim(p.monthlyPremium) ? this.formatMoneyValue(this.asMoneyNumber(p.monthlyPremium)) : '—';
          return `<article class="lcSummaryPolicyCard lcSummaryPolicyCard--open lcDelay-${(idx % 6) + 1}">
            <div class="lcSummaryPolicyCard__top">
              <div class="lcSummaryPolicyCard__company">
                ${logoSrc ? `<img class="lcSummaryPolicyCard__logo" src="${escapeHtml(logoSrc)}" alt="${escapeHtml(p.company||'')}" />` : `<span class="lcSummaryPolicyCard__logo lcSummaryPolicyCard__logo--empty" aria-hidden="true"></span>`}
                <div>
                  <div class="lcSummaryPolicyCard__name">${escapeHtml(p.company || 'חברה')}</div>
                  <div class="lcSummaryPolicyCard__type">${escapeHtml(p.type || 'פוליסה קיימת')}</div>
                </div>
              </div>
              <span class="lcSummaryStatus lcSummaryStatus--${escapeHtml(tone)}">${escapeHtml(statusLabel)}</span>
            </div>
            <div class="lcSummaryPolicyCard__grid">
              <div><small>מספר פוליסה</small><strong>${escapeHtml(p.policyNumber || '—')}</strong></div>
              <div><small>פרמיה חודשית</small><strong>${escapeHtml(monthly)}</strong></div>
              <div><small>מבוטח</small><strong>${escapeHtml(ins.label || 'מבוטח')}</strong></div>
            </div>
          </article>`;
        }).join('')}</div>`;
      };
      ...
      const renderNewCards = () => {
        const list = this.newPolicies || [];
        if(!list.length) return `<div class="muted small">לא נוספו פוליסות חדשות.</div>`;
        return `<div class="lcSummaryCards">${list.map((p, idx) => {
          const logoSrc = this.getCompanyLogoSrc(p.company);
          const after = this.formatMoneyValue(this.getPolicyPremiumAfterDiscount(p));
          const before = safeTrim(p.premiumMonthly) ? this.formatMoneyValue(this.asMoneyNumber(p.premiumMonthly)) : '—';
          const discountText = `${escapeHtml(String(this.getPolicyDiscountPct(p)))}%${safeTrim(p.discountYears) ? ` · ${escapeHtml(p.discountYears)} שנים` : ''}`;
          return `<article class="lcSummaryPolicyCard lcSummaryPolicyCard--new lcSummaryPolicyCard--open lcDelay-${(idx % 6) + 1}">
            <div class="lcSummaryPolicyCard__top">
              <div class="lcSummaryPolicyCard__company">
                ${logoSrc ? `<img class="lcSummaryPolicyCard__logo" src="${escapeHtml(logoSrc)}" alt="${escapeHtml(p.company||'')}" />` : `<span class="lcSummaryPolicyCard__logo lcSummaryPolicyCard__logo--empty" aria-hidden="true"></span>`}
                <div>
                  <div class="lcSummaryPolicyCard__name">${escapeHtml(p.company || 'חברה')}</div>
                  <div class="lcSummaryPolicyCard__type">${escapeHtml(this.isMedicareCompany(p.company) ? 'מדיקר' : (p.type || 'פוליסה חדשה'))}</div>
                </div>
              </div>
              <span class="lcSummaryStatus lcSummaryStatus--success">נמכרה</span>
            </div>
            <div class="lcSummaryPolicyCard__grid">
              <div><small>מבוטח</small><strong>${escapeHtml(formatPolicyInsured(p))}</strong></div>
              <div><small>פרמיה</small><strong>${escapeHtml(before)}</strong></div>
              <div><small>אחרי הנחה</small><strong>${escapeHtml(after)}</strong></div>
              <div><small>הנחה</small><strong>${discountText}</strong></div>
            </div>
          </article>`;
        }).join('')}</div>`;
      };

      const companies = this.getNewPolicyCompanies();
      const store = this.getAgentNumberStore();
      ...
      const totalAfter = (this.newPolicies || []).reduce((acc, p) => acc + this.getPolicyPremiumAfterDiscount(p), 0);

      const companyFields = companies.length ? `<div class="lcAgentCompanyGrid">${companies.map((company, idx) => {
        const src = this.getCompanyLogoSrc(company);
        return `<label class="lcAgentCompanyRow lcDelay-${(idx % 6) + 1}">
          <div class="lcAgentCompanyRow__brand">
            ${src ? `<img class="lcAgentCompanyRow__logo" src="${escapeHtml(src)}" alt="${escapeHtml(company)}" />` : `<span class="lcAgentCompanyRow__logo lcAgentCompanyRow__logo--empty" aria-hidden="true"></span>`}
            <div>
              <div class="lcAgentCompanyRow__name">${escapeHtml(company)}</div>
              <div class="lcAgentCompanyRow__sub">מלא מספר סוכן עבור החברה שנמכרה</div>
            </div>
          </div>
          <input class="input lcAgentCompanyRow__input" data-agent-company="${escapeHtml(company)}" inputmode="numeric" placeholder="מספר סוכן" type="text" value="${escapeHtml(store[company] || '')}" />
        </label>`;
      }).join('')}</div>` : `<div class="muted small">לא נמכרו פוליסות חדשות, לכן אין צורך במספרי סוכן.</div>`;

      return `
        <section class="lcSummaryStage">
          <div class="lcSummaryHero lcSummaryHero--final">
            <div class="lcSummaryHero__kicker">GEMEL INVEST</div>
            <div class="lcSummaryHero__title">סיכום תפעולי לפני סיום הקמת לקוח</div>
            <div class="lcSummaryHero__text">המערכת מרכזת את כל הפוליסות, פותחת אותן בהדרגה ומכינה את פרטי התפעול לשמירה ולדוח.</div>
            <div class="lcSummaryMeta">
              ...
              <div class="lcSummaryMetaCard"><span class="lcSummaryMetaCard__k">פרמיה אחרי הנחה</span><strong class="lcSummaryMetaCard__v">${escapeHtml(this.formatMoneyValue(totalAfter))}</strong></div>
            </div>
          </div>

          <div class="lcWSection lcSummarySection lcSummarySection--existing">
            <div class="lcWTitle">פוליסות קיימות וסטטוס טיפול</div>
            <div class="muted small">כל פוליסה קיימת מוצגת עם הסטטוס שנבחר בשלב ביטול בחברה נגדית.</div>
            ${this.insureds.map(ins => `<div class="lcSummaryInsuredBlock"><div class="lcSummaryInsuredBlock__title">${escapeHtml(ins.label)}</div>${renderExistingCards(ins)}</div>`).join('')}
          </div>

          <div class="lcWSection lcSummarySection lcSummarySection--new">
            <div class="lcWTitle">פוליסות חדשות</div>
            <div class="muted small">כאן מוצגות כל הפוליסות החדשות שנמכרו, כולל פרמיה לפני ואחרי הנחה.</div>
            ${renderNewCards()}
          </div>

          <div class="lcWSection lcSummarySection lcSummarySection--agents">
            <div class="lcWTitle">מספרי סוכן לפי חברות שנמכרו</div>
            <div class="muted small">מופיעות רק חברות מתוך הפוליסות החדשות, וכל שורה כוללת גם לוגו כדי להקל על הנציג.</div>
            ${companyFields}
          </div>
        </section>`;
    },
    ...
    getDraftPayload(){
      ...
      return {
        ...
        agentNumbersByCompany: JSON.parse(JSON.stringify((primary.data?.agentNumbersByCompany) || {})),
        operational: {
          createdAt: nowISO(),
    ...
    loadDraftData(rec){
      ...
      const agentNumbersByCompany = JSON.parse(JSON.stringify(payload.agentNumbersByCompany || payload?.operational?.agentNumbersByCompany || {}));
      this.insureds[0].data = this.insureds[0].data || {};
      this.insureds[0].data.agentNumbersByCompany = agentNumbersByCompany;
    ...
    renderOperationalReport(){
      ...
      const agentNumberRows = Object.entries(payload.agentNumbersByCompany || payload.primary?.agentNumbersByCompany || {}).map(([company, number]) => [this.renderReportValue(company), this.renderReportValue(number)]);
      const healthItems = [];
      this.getHealthQuestionList().forEach(item => {
        payload.insureds.forEach(ins => {
    ...
    async finishWizard(){
      ...
      const v = this.validateStep(this.steps.length);
      if(!v.ok){
    ...
    validateStep(stepId){
      ...
      if(stepId === 9){
        ...
        for(const company of companies){
          if(!safeTrim(store[company])) return { ok:false, msg:`חסר מספר סוכן עבור ${company}` };
        }
        return { ok:true };
      }
