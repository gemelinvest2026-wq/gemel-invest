...
// open health covers drawer (Health only)
      ...
      $$("[data-payer]", this.els.body).forEach(el => {
        on(el, "change", () => {
          ...
          if(k === "payerChoice"){
            if(v === "insured"){
              const payerId = safeTrim(ins.data.selectedPayerId) || safeTrim(this.insureds.find(x => x.type === "primary")?.id);
              ins.data.selectedPayerId = payerId;
              this.syncStep6HolderFromSelectedPayer(ins.data);
            }else{
              ins.data.selectedPayerId = "";
              if(ins.data.externalPayer){
                ins.data.cc = ins.data.cc || {};
                ins.data.cc.holderName = `${safeTrim(ins.data.externalPayer.firstName)} ${safeTrim(ins.data.externalPayer.lastName)}`.trim();
                ins.data.cc.holderId = safeTrim(ins.data.externalPayer.idNumber);
              }
            }
          }

          if(k === "selectedPayerId"){
            this.syncStep6HolderFromSelectedPayer(ins.data);
          }

          if(k.startsWith("externalPayer.")){
            if(ins.data.payerChoice === "external"){
              ins.data.cc = ins.data.cc || {};
              ins.data.cc.holderName = `${safeTrim(ins.data.externalPayer?.firstName)} ${safeTrim(ins.data.externalPayer?.lastName)}`.trim();
              ins.data.cc.holderId = safeTrim(ins.data.externalPayer?.idNumber);
            }
    ...
    syncStep6HolderFromSelectedPayer(targetData){
      if(!targetData) return;
      const selectedId = safeTrim(targetData.selectedPayerId);
      const payer = this.insureds.find(x => x.id === selectedId);
      const holderName = payer ? ((safeTrim(payer.data.firstName) + " " + safeTrim(payer.data.lastName)).trim()) : "";
      const holderId = payer ? safeTrim(payer.data.idNumber) : "";
      targetData.cc = targetData.cc || {};
      if(holderName) targetData.cc.holderName = holderName;
      if(holderId) targetData.cc.holderId = holderId;
    },
    ...
    renderStep6(ins){
      ...
      const insuredPayers = this.insureds
        ...
        .map(x => ({
          id:x.id,
          label:x.label,
          firstName: safeTrim(x.data.firstName),
          lastName: safeTrim(x.data.lastName),
          idNumber: safeTrim(x.data.idNumber),
      ...
      const selectedPayer = insuredPayers.find(x => x.id === safeTrim(d.selectedPayerId)) || null;
      const inheritedHolderName = d.payerChoice === "insured" ? safeTrim(selectedPayer?.name) : `${safeTrim(d.externalPayer?.firstName)} ${safeTrim(d.externalPayer?.lastName)}`.trim();
      const inheritedHolderId = d.payerChoice === "insured" ? safeTrim(selectedPayer?.idNumber) : safeTrim(d.externalPayer?.idNumber);
      const method = safeTrim(d.paymentMethod || "cc");
      const effectiveHolderName = safeTrim(d.cc?.holderName) || inheritedHolderName;
      const effectiveHolderId = safeTrim(d.cc?.holderId) || inheritedHolderId;
      const chooser = `
        <div class="lcWSection lcPayerStep">
          <div class="lcWTitle">פרטי משלם</div>
          <div class="lcPayerStep__sub">בחירת משלם, אמצעי תשלום ופרטי חיוב מסודרים לפי שיטת התשלום.</div>

          <div class="lcWGrid lcPayerStep__topGrid">
            <div class="field">
          ...
          ${d.payerChoice === "insured" ? `
            <div class="lcPayerStep__stack">
              ...
              <div class="lcPayerSummary${selectedPayer ? " is-ready" : ""}">
                <div class="lcPayerSummary__head">פרטי מחזיק בירושה מהלקוח הנבחר</div>
                <div class="lcPayerSummary__grid">
                  <div class="field">
                    <label class="label">שם המחזיק</label>
                    <input class="input" value="${escapeHtml(inheritedHolderName)}" disabled />
                  </div>
                  <div class="field">
                    <label class="label">ת״ז המחזיק</label>
                    <input class="input" value="${escapeHtml(inheritedHolderId)}" disabled />
                  </div>
                </div>
              </div>
            </div>
          ` : `
            <div class="lcWGrid lcPayerStep__externalGrid">
          ...
          ${method==="cc" ? `
            <div class="lcPayerMethod">
              <div class="lcPayerMethod__title">פרטי כרטיס אשראי</div>
              <div class="lcWGrid lcPayerMethod__grid">
                <div class="field">
                  <label class="label">שם מחזיק/ה</label>
                  <input class="input" data-payer="cc.holderName" value="${escapeHtml(effectiveHolderName)}" ${d.payerChoice === "insured" ? "readonly" : ""} />
                </div>
                <div class="field">
                  <label class="label">ת״ז מחזיק/ה</label>
                  <input class="input" data-payer="cc.holderId" value="${escapeHtml(effectiveHolderId)}" inputmode="numeric" ${d.payerChoice === "insured" ? "readonly" : ""} />
                </div>
          ...
          ` : `
            <div class="lcPayerMethod">
              <div class="lcPayerMethod__title">פרטי הוראת קבע</div>
              <div class="lcWGrid lcPayerMethod__grid">
                <div class="field">
    ...
    isStepCompleteForInsured(stepId, ins){
      ...
      if(stepId === 6){
        ...
        if(d.payerChoice === "insured"){
          ...
          const payer = this.insureds.find(x => x.id === safeTrim(d.selectedPayerId));
          holderName = payer ? ((safeTrim(payer.data.firstName) + " " + safeTrim(payer.data.lastName)).trim()) : "";
          holderId = payer ? safeTrim(payer.data.idNumber) : "";
          if(!holderName || !holderId) return false;
        }else{
          ...
          holderName = `${safeTrim(ex.firstName)} ${safeTrim(ex.lastName)}`.trim();
          holderId = safeTrim(ex.idNumber);
        }

        if(safeTrim(d.paymentMethod) === "cc"){
          const cc = d.cc || {};
          if(!safeTrim(cc.holderName) && holderName) cc.holderName = holderName;
          if(!safeTrim(cc.holderId) && holderId) cc.holderId = holderId;
