...
// Set your GOFI module URL (GitHub Pages) here when ready
  ...
  const mailFlashHost = document.getElementById('mailFlashHost');
  ...
  function mailMe_(){
    ...
    return { username:String(u.username), displayName:String(u.displayName||u.username) };
  }

  function mailKey_(box, username){
    return 'gemel_mail_' + box + '_' + String(username||'').trim();
  }

  function mailLoadBox_(box, username){
    const key = mailKey_(box, username);
    try{
  ...
  function mailSaveBox_(box, username, arr){
    const key = mailKey_(box, username);
    try{ localStorage.setItem(key, JSON.stringify(arr || [])); }catch(_){}
  }

  function mailNowText_(ts){
  ...
  function mailTrimPreview_(s){
    const t = String(s||'').replace(/\s+/g,' ').trim();
    return t.length > 46 ? (t.slice(0,46) + '…') : t;
  }

  function mailUnreadCount_(){
    const me = mailMe_();
    if(!me) return 0;
    const inbox = mailLoadBox_('inbox', me.username);
    return inbox.filter(m => m && m.read !== true).length;
  }

  function mailUpdateBadge_(){
    if(!mailBadge) return;
    const n = mailUnreadCount_();
    if(n > 0){
      mailBadge.textContent = String(Math.min(99, n));
  ...
  function mailPlayBeep_(){
    try{
      ...
      o.stop(t0 + 0.24);
      setTimeout(() => { try{ ctx.close(); }catch(_){ } }, 380);
    }catch(_){}
  }

  function mailFlash_(fromName, subject){
    if(!mailFlashHost) return;
    const el = document.createElement('div');
    el.className = 'mailFlash';
    el.innerHTML = `<b>מייל חדש מ: ${escapeHtml(String(fromName||''))}</b><span>${escapeHtml(String(subject||'ללא נושא'))}</span>`;
    mailFlashHost.appendChild(el);
    setTimeout(() => { try{ el.remove(); }catch(_){ } }, 4000);
  }

  
  // Drag + position persistence for mail window (standalone card)
  const MAIL_POS_KEY = 'gemel_mail_window_pos_v1';
  function mailLoadPos_(){
    try{
      const raw = localStorage.getItem(MAIL_POS_KEY);
      const o = raw ? JSON.parse(raw) : null;
      if(o && typeof o.left === 'number' && typeof o.top === 'number') return o;
    }catch(_){}
    return null;
  }
  function mailSavePos_(left, top){
    try{ localStorage.setItem(MAIL_POS_KEY, JSON.stringify({ left, top })); }catch(_){}
  }

  function mailApplySavedPos_(){
    if(!mailWindow) return;
    const p = mailLoadPos_();
    if(!p) return;
    mailWindow.classList.add('dragged');
    mailWindow.style.left = p.left + 'px';
    mailWindow.style.top = p.top + 'px';
    mailWindow.style.right = 'auto';
    mailWindow.style.bottom = 'auto';
  }

  function mailInitDrag_(){
    if(!mailWindow || mailWindow.__dragInit) return;
    const header = mailWindow.querySelector('.mailHeader');
    if(!header) return;
    mailWindow.__dragInit = true;
    ...
    const onDown = (e) => {
      // Don't start drag when clicking buttons in the header
      const t = e.target;
    ...
    const onMove = (e) => {
      ...
      const w = mailWindow.offsetWidth || 520;
      const h = mailWindow.offsetHeight || 620;
      ...
      mailWindow.style.left = left + 'px';
      mailWindow.style.top  = top + 'px';
    };
    ...
    const onUp = () => {
      ...
      const rect = mailWindow.getBoundingClientRect();
      mailSavePos_(Math.round(rect.left), Math.round(rect.top));
    };
...
function mailOpen_(){
  ...
  function mailToggle_(){
    if(!mailWindow) return;
    if(mailWindow.classList.contains('hidden')) mailOpen_();
    else mailClose_();
  }

  function mailSetTab_(t){
    mailTab = (t === 'sent') ? 'sent' : 'inbox';
    $$('.mailTab').forEach(b => b.classList.toggle('active', (b.dataset && b.dataset.mailTab) === mailTab));
    mailSelectedId = null;
    mailRender_();
  }

  function mailGetList_(){
    const me = mailMe_();
    if(!me) return [];
    const arr = mailLoadBox_(mailTab, me.username);
    // newest first
    return arr.slice().sort((a,b) => (b.ts||0) - (a.ts||0));
  }

  function mailMarkRead_(id){
    const me = mailMe_();
    if(!me || !id) return;
    const inbox = mailLoadBox_('inbox', me.username);
    const idx = inbox.findIndex(m => m && m.id === id);
    if(idx >= 0){
      inbox[idx].read = true;
      mailSaveBox_('inbox', me.username, inbox);
      mailUpdateBadge_();
    }
  }

  function mailRender_(){
    const me = mailMe_();
    if(!me || !mailListEl || !mailViewEl) return;

    const list = mailGetList_();
    mailListEl.innerHTML = list.map(m => {
      const from = mailTab === 'sent' ? (m.toNames || '—') : (m.fromName || '—');
      const subj = m.subject || 'ללא נושא';
      const prev = mailTrimPreview_(m.body || '');
      const unread = (mailTab === 'inbox' && m.read !== true) ? ' unread' : '';
      const sel = (m && m.id === mailSelectedId) ? ' style="box-shadow:0 0 0 3px var(--goldSoft);border-color:rgba(241,230,204,.32)"' : '';
      return `
        <div class="mailItem${unread}" data-mail-id="${escapeAttr(m.id||'')}"${sel}>
          <div class="mailItemTop">
            <div class="mailFrom">${escapeHtml(from)}</div>
            <div class="mailTime">${escapeHtml(mailNowText_(m.ts))}</div>
          </div>
          <div class="mailSubj">${escapeHtml(subj)}</div>
          <div class="mailPrev">${escapeHtml(prev)}</div>
        </div>
      `;
    }).join('') || '<div class="mailEmpty" style="margin:6px 0">אין מיילים</div>';

    // View
    const cur = list.find(x => x && x.id === mailSelectedId) || null;
    if(!cur){
      mailViewEl.innerHTML = '<div class="mailEmpty">בחר מייל כדי לראות תוכן</div>';
    }else{
      const metaLeft = mailTab === 'sent'
        ? `<div><b>אל:</b> ${escapeHtml(cur.toNames || '—')}</div>`
        : `<div><b>מאת:</b> ${escapeHtml(cur.fromName || '—')}</div>`;
      const meta = `
        <div class="mailViewHead">
          <div class="mailViewMeta">
            ${metaLeft}
            <div><b>נושא:</b> ${escapeHtml(cur.subject || 'ללא נושא')}</div>
            <div><b>שעה:</b> ${escapeHtml(mailNowText_(cur.ts))}</div>
          </div>
          <div>
            ${mailTab === 'inbox' ? '<button class="btn btnTiny btnSoft" id="mailReplyBtn" type="button">השב</button>' : ''}
          </div>
        </div>
        <div class="mailBodyText">${escapeHtml(cur.body || '')}</div>
      `;
      mailViewEl.innerHTML = meta;

      if(mailTab === 'inbox'){
        mailMarkRead_(cur.id);
        const replyBtn = document.getElementById('mailReplyBtn');
        if(replyBtn){
          replyBtn.addEventListener('click', () => mailCompose_({ toUsernames: [cur.fromUsername], subject: 'RE: ' + (cur.subject||'') }));
        }
      }
    }

    // Wire click on list
    mailListEl.querySelectorAll('[data-mail-id]').forEach(el => {
  ...
  function mailUsersForRecipients_(){
    const users = getUsers_() || [];
    return users.filter(u => u && u.active !== false && u.username);
  }

  function mailCompose_(prefill){
    if(!mailWindow) return;
    const me = mailMe_();
    if(!me) return;

    const users = mailUsersForRecipients_().filter(u => u.username !== me.username);
    const pre = prefill || {};
    const preTo = Array.isArray(pre.toUsernames) ? pre.toUsernames : [];
    const preSubj = String(pre.subject || '').trim();

    const overlay = document.createElement('div');
    overlay.className = 'mailComposeOverlay';
    overlay.innerHTML = `
      <div class="mailComposeCard" role="dialog" aria-modal="true" aria-label="מייל חדש">
        <div class="mailComposeTop">
          <span>מייל חדש</span>
          <button class="btn btnTiny btnSoft" id="mailComposeX" type="button" aria-label="סגור">✕</button>
        </div>
        <div class="mailComposeBody">
          <div>
            <div class="mailFieldLabel">נמענים (אפשר לבחור כמה)</div>
            <div class="mailRecipients" id="mailRecipients">
              ${users.map(u => {
                const checked = preTo.includes(u.username) ? 'checked' : '';
                return `<label class="mailRecipPill"><input type="checkbox" value="${escapeAttr(u.username)}" ${checked}/> ${escapeHtml(u.displayName||u.username)}</label>`;
              }).join('')}
            </div>
          </div>

          <div>
            <div class="mailFieldLabel">נושא</div>
            <input class="mailField" id="mailSubject" type="text" placeholder="נושא קצר" value="${escapeAttr(preSubj)}"/>
          </div>

          <div>
            <div class="mailFieldLabel">תוכן</div>
            <textarea class="mailField" id="mailBody" rows="6" placeholder="כתוב הודעה..."></textarea>
          </div>
        </div>
        <div class="mailComposeActions">
          <button class="btn btnSoft" id="mailComposeCancel" type="button">ביטול</button>
          <button class="btn btnPrimary" id="mailComposeSend" type="button">שלח</button>
        </div>
      </div>
    `;
    mailWindow.appendChild(overlay);

    const close = () => { try{ overlay.remove(); }catch(_){ } };
    const x = overlay.querySelector('#mailComposeX');
    const cancel = overlay.querySelector('#mailComposeCancel');
    if(x) x.addEventListener('click', close);
    if(cancel) cancel.addEventListener('click', close);

    const sendBtn = overlay.querySelector('#mailComposeSend');
    if(sendBtn){
      sendBtn.addEventListener('click', () => {
        const subj = (overlay.querySelector('#mailSubject') || {}).value || '';
        const body = (overlay.querySelector('#mailBody') || {}).value || '';
        const recips = Array.from(overlay.querySelectorAll('#mailRecipients input[type="checkbox"]:checked')).map(x => x.value).filter(Boolean);

        if(!recips.length){
          toast('שגיאה', 'בחר לפחות נמען אחד');
          return;
        }
        if(!String(body||'').trim()){
          toast('שגיאה', 'תוכן ההודעה ריק');
          return;
        }

        const ok = mailSend_(recips, subj, body);
        if(ok){
          close();
          mailSetTab_('sent');
          toast('נשלח', 'המייל נשלח בהצלחה');
        }else{
          toast('שגיאה', 'לא הצלחתי לשלוח (בדוק אחסון דפדפן)');
        }
      });
    }
  }

  function mailUserMap_(){
    const users = getUsers_() || [];
    const map = {};
    users.forEach(u => { if(u && u.username) map[u.username] = u; });
    return map;
  }

  function mailSend_(toUsernames, subject, body){
    const me = mailMe_();
    if(!me) return false;
    const to = (Array.isArray(toUsernames) ? toUsernames : []).map(x => String(x||'').trim()).filter(Boolean);
    if(!to.length) return false;

    const umap = mailUserMap_();
    const toNames = to.map(u => (umap[u] && umap[u].displayName) ? umap[u].displayName : u).join(', ');

    const id = 'm' + Date.now() + '_' + Math.random().toString(16).slice(2,8);
    const ts = Date.now();
    ...
    // Sent box (sender)
    const sent = mailLoadBox_('sent', me.username);
    sent.unshift(senderMsg);
    mailSaveBox_('sent', me.username, sent);

    // Inbox for each recipient
    to.forEach(u => {
      const inbox = mailLoadBox_('inbox', u);
      const rmsg = {
        id, ts,
        fromUsername: me.username,
        fromName: me.displayName,
      ...
      inbox.unshift(rmsg);
      mailSaveBox_('inbox', u, inbox);
    });

    // Broadcast event
    try{
      const payload = { type:'MAIL_NEW', to: to.slice(), from: me.username, fromName: me.displayName, subject: String(subject||'').trim(), id, ts };
      if(mailChannel && typeof mailChannel.postMessage === 'function'){
        mailChannel.postMessage(payload);
      }
      // Also trigger storage event fallback by touching a shared key
      localStorage.setItem('gemel_mail_ping', JSON.stringify({ at: Date.now(), id }));
    }catch(_){}
  ...
  function mailHandleIncoming_(payload){
    const me = mailMe_();
    if(!me) return;

    try{
      // If payload targets me, refresh badge and maybe show flash
      const to = (payload && Array.isArray(payload.to)) ? payload.to : [];
      if(to.includes(me.username)){
        mailUpdateBadge_();
        mailFlash_(payload.fromName || payload.from || 'נציג', payload.subject || 'ללא נושא');
        mailPlayBeep_();
        // If window is open and on inbox, refresh list
        if(mailWindow && !mailWindow.classList.contains('hidden') && mailTab === 'inbox'){
          mailRender_();
        }
      }
    }catch(_){}
  ...
  function initMail_(){
    ...
    // UI wiring
    if(mailBtn) mailBtn.addEventListener('click', () => mailToggle_());
    if(mailCloseBtn) mailCloseBtn.addEventListener('click', () => mailClose_());
    if(mailComposeBtn) mailComposeBtn.addEventListener('click', () => mailCompose_());
    $$('.mailTab').forEach(b => b.addEventListener('click', () => mailSetTab_(b.dataset.mailTab)));

    // Listen for incoming
    try{
      mailChannel = ('BroadcastChannel' in window) ? new BroadcastChannel('gemel_internal_mail_v1') : null;
      if(mailChannel){
        mailChannel.onmessage = (ev) => {
          const p = ev && ev.data ? ev.data : null;
          if(p && p.type === 'MAIL_NEW') mailHandleIncoming_(p);
        };
      }
    }catch(_){}
    ...
    window.addEventListener('storage', (e) => {
      if(!e) return;
      if(e.key === 'gemel_mail_ping'){
        // lightweight refresh; actual messages are stored per-user keys
        mailUpdateBadge_();
      }
    });
...
$$('.navItem').forEach(b => b.addEventListener('click', (e) => {
  ...
  function setIdentity_(u){
    ...
    applyPermissions_();
    try{ initMail_(); }catch(_){}
    try{ mailUpdateBadge_ && mailUpdateBadge_(); }catch(_){}
  }
