...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  // ---------- Existing policies: extra company logos ----------
  // Only used in "פוליסות קיימות" list (table + selector), so new policies remain unchanged.
  const OLD_COMPANY_LOGOS = {
    ...COMPANY_LOGOS,
    'איילון': 'assets/companies/ayalon.png',
    'AIG': 'assets/companies/aig.png',
    'ביטוח ישיר': 'assets/companies/beytuyashir.png'
  };
  function oldCompanyLogoSrc(name){
    const n = String(name||'').trim();
    return OLD_COMPANY_LOGOS[n] || '';
  }
  function renderOldCompanyCell(name){
    const src = oldCompanyLogoSrc(name);
          ...
          <tbody>
            ${rows.length ? rows.map(({x,idx}) => {
              ...
              if(dec==='לביטול') dec='cancel_full';
                            return `
              <tr>
                <td>${renderOldCompanyCell(x.company||'')}</td>
