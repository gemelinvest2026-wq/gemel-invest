...
(() => {
  ...
  function normalizeCustomerRecord(c, idx=0){
    ...
    const rawOps = (payload?.opsMirror && typeof payload.opsMirror === 'object') ? payload.opsMirror : {};
    const opsLocked = (typeof c?.opsLocked === 'boolean') ? c.opsLocked : !!rawOps.locked;
    const opsClientSigned = (typeof c?.opsClientSigned === 'boolean') ? c.opsClientSigned : !!rawOps.clientSigned;
    const opsPhase = safeTrim(c?.opsPhase) || safeTrim(rawOps.phase);
    const opsLockedBy = safeTrim(c?.opsLockedBy) || safeTrim(rawOps.lockedBy);
    const opsLockedAt = safeTrim(c?.opsLockedAt) || safeTrim(rawOps.lockedAt);
    const opsSessionStartedAt = safeTrim(c?.opsSessionStartedAt) || safeTrim(rawOps.sessionStartedAt);
    const opsSessionEndedAt = safeTrim(c?.opsSessionEndedAt) || safeTrim(rawOps.sessionEndedAt);
    const opsDurationSec = Number(c?.opsDurationSec || rawOps.durationSec || 0) || 0;
    const opsDurationText = safeTrim(c?.opsDurationText) || safeTrim(rawOps.durationText);
    const opsLastSessionDate = safeTrim(c?.opsLastSessionDate) || safeTrim(rawOps.lastSessionDate);
    const opsLastSessionStartTime = safeTrim(c?.opsLastSessionStartTime) || safeTrim(rawOps.lastSessionStartTime);
    const opsLastSessionDurationText = safeTrim(c?.opsLastSessionDurationText) || safeTrim(rawOps.lastSessionDurationText);
    const opsSignedAt = safeTrim(c?.opsSignedAt) || safeTrim(rawOps.signedAt);
    const opsSignedBy = safeTrim(c?.opsSignedBy) || safeTrim(rawOps.signedBy);
    return {
      ...
      opsDurationText,
      opsLastSessionDate,
      opsLastSessionStartTime,
      opsLastSessionDurationText,
      opsSignedAt,
      opsSignedBy,
      payload
    };
...
txt.textContent = "";
  ...
  const CustomersUI = {
    ...
    syncOpsMirrorStateToRecord(rec, state){
      if(!rec) return state || {};
      ...
      s.locked = !!s.locked;
      s.clientSigned = !!s.clientSigned;
      s.phase = safeTrim(s.phase);
      s.lockedBy = safeTrim(s.lockedBy);
      s.lockedAt = safeTrim(s.lockedAt);
      s.sessionStartedAt = safeTrim(s.sessionStartedAt);
      s.sessionEndedAt = safeTrim(s.sessionEndedAt);
      s.durationSec = Number(s.durationSec || 0) || 0;
      s.durationText = safeTrim(s.durationText);
      s.lastSessionDate = safeTrim(s.lastSessionDate);
      s.lastSessionStartTime = safeTrim(s.lastSessionStartTime);
      s.lastSessionDurationText = safeTrim(s.lastSessionDurationText);
      s.signedAt = safeTrim(s.signedAt);
      s.signedBy = safeTrim(s.signedBy);
      rec.opsLocked = s.locked;
      rec.opsClientSigned = s.clientSigned;
      rec.opsPhase = s.phase;
      rec.opsLockedBy = s.lockedBy;
      rec.opsLockedAt = s.lockedAt;
      rec.opsSessionStartedAt = s.sessionStartedAt;
      rec.opsSessionEndedAt = s.sessionEndedAt;
      rec.opsDurationSec = s.durationSec;
      rec.opsDurationText = s.durationText;
      rec.opsLastSessionDate = s.lastSessionDate;
      rec.opsLastSessionStartTime = s.lastSessionStartTime;
      rec.opsLastSessionDurationText = s.lastSessionDurationText;
      rec.opsSignedAt = s.signedAt;
      rec.opsSignedBy = s.signedBy;
      return s;
    },
    ...
    getOpsMirrorState(rec){
      if(!rec) return {};
      const rootState = {
        locked: !!rec.opsLocked,
        clientSigned: !!rec.opsClientSigned,
        phase: safeTrim(rec.opsPhase),
        lockedBy: safeTrim(rec.opsLockedBy),
        lockedAt: safeTrim(rec.opsLockedAt),
        sessionStartedAt: safeTrim(rec.opsSessionStartedAt),
        sessionEndedAt: safeTrim(rec.opsSessionEndedAt),
        durationSec: Number(rec.opsDurationSec || 0) || 0,
        durationText: safeTrim(rec.opsDurationText),
        lastSessionDate: safeTrim(rec.opsLastSessionDate),
        lastSessionStartTime: safeTrim(rec.opsLastSessionStartTime),
        lastSessionDurationText: safeTrim(rec.opsLastSessionDurationText),
        signedAt: safeTrim(rec.opsSignedAt),
        signedBy: safeTrim(rec.opsSignedBy)
      };
      const payloadState = (rec.payload && typeof rec.payload === 'object' && rec.payload.opsMirror && typeof rec.payload.opsMirror === 'object') ? rec.payload.opsMirror : {};
      const merged = {
        locked: (typeof payloadState.locked === 'boolean') ? payloadState.locked : rootState.locked,
        clientSigned: (typeof payloadState.clientSigned === 'boolean') ? payloadState.clientSigned : rootState.clientSigned,
        phase: safeTrim(payloadState.phase) || rootState.phase,
        lockedBy: safeTrim(payloadState.lockedBy) || rootState.lockedBy,
        lockedAt: safeTrim(payloadState.lockedAt) || rootState.lockedAt,
        sessionStartedAt: safeTrim(payloadState.sessionStartedAt) || rootState.sessionStartedAt,
        sessionEndedAt: safeTrim(payloadState.sessionEndedAt) || rootState.sessionEndedAt,
        durationSec: Number(payloadState.durationSec || rootState.durationSec || 0) || 0,
        durationText: safeTrim(payloadState.durationText) || rootState.durationText,
        lastSessionDate: safeTrim(payloadState.lastSessionDate) || rootState.lastSessionDate,
        lastSessionStartTime: safeTrim(payloadState.lastSessionStartTime) || rootState.lastSessionStartTime,
        lastSessionDurationText: safeTrim(payloadState.lastSessionDurationText) || rootState.lastSessionDurationText,
        signedAt: safeTrim(payloadState.signedAt) || rootState.signedAt,
        signedBy: safeTrim(payloadState.signedBy) || rootState.signedBy,
      };
      return this.syncOpsMirrorStateToRecord(rec, merged);
    },
    ...
    async markClientSigned(customerId){
      ...
      this.syncOpsMirrorStateToRecord(rec, s);
    ...
    render(){
      ...
      UI.els.customersTbody.innerHTML = rows.length ? rows.map(rec => {
        ...
        const locked = !!lockState.locked || safeTrim(rec.status) === 'בטיפול מחלקת תפעול';
        if(locked && !lockState.locked){
          lockState.locked = true;
          lockState.phase = safeTrim(lockState.phase) || 'in_progress';
          this.syncOpsMirrorStateToRecord(rec, lockState);
        }
        ...
        const opsInline = locked ? `<span class="lcCustomers__opsInline">בטיפול מחלקת התפעול</span>` : '';
        ...
        return `<tr class="${locked ? 'lcCustomersRow is-ops-locked' : 'lcCustomersRow'}">
          ...
          <td><div class="lcCustomers__rowActions">
            <button class="btn btn--primary" data-open-customer="${escapeHtml(rec.id)}" type="button" ${openDisabled}>פתח תיק</button>
            ${opsInline}
            ${signedBtn}
  ...
  const MirrorsUI = {
    ...
    ensureSessionChrome(){
      if(!this.els.sessionBar){
        ...
        const flow = this.els.flow || document.body;
        if(flow && flow.parentNode) flow.parentNode.insertBefore(bar, flow);
      ...
      if(!this.els.startModal){
        ...
        on(this.els.startModalStart, 'click', () => this.startSession());
        on(modal, 'click', (ev) => {
          if(ev.target === modal || ev.target.classList.contains('mirrorsStartModal__backdrop')) this.cancelStartSession();
        });
      }
    },
            ...
            safeTrim(verify?.harConsent) !== 'yes' ? 4 : (
              !safeTrim(verify?.cancelSavedAt) ? 5 : (
                !safeTrim(disclosure?.savedAt) ? 6 : (
                  !safeTrim(this.getMirrorHealthState(rec)?.savedAt) ? 7 : 8
                )
              )
            )
          )
    ...
    async liveRefreshTick(){
      ...
      try {
        ...
        if(r.ok){
          ...
          CustomersUI.render();
          const customerPanelOpen = !!(CustomersUI.els?.wrap && CustomersUI.els.wrap.classList.contains('is-open'));
          if(customerPanelOpen && CustomersUI.currentId){
