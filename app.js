...
txt.textContent = "";
  ...
  const CustomersUI = {
    ...
    getOpsMirrorState(rec){
      ...
      if(typeof s.locked !== 'boolean') s.locked = !!rec.opsLocked;
      if(typeof s.clientSigned !== 'boolean') s.clientSigned = !!rec.opsClientSigned;
      if(!safeTrim(s.phase) && safeTrim(rec.opsPhase)) s.phase = safeTrim(rec.opsPhase);
      if(!safeTrim(s.lockedBy) && safeTrim(rec.opsLockedBy)) s.lockedBy = safeTrim(rec.opsLockedBy);
      if(!safeTrim(s.lockedAt) && safeTrim(rec.opsLockedAt)) s.lockedAt = safeTrim(rec.opsLockedAt);
      rec.opsLocked = !!s.locked;
      rec.opsClientSigned = !!s.clientSigned;
      rec.opsPhase = safeTrim(s.phase);
      rec.opsLockedBy = safeTrim(s.lockedBy);
      rec.opsLockedAt = safeTrim(s.lockedAt);
      return s;
    },
    ...
    render(){
      ...
      UI.els.customersTbody.innerHTML = rows.length ? rows.map(rec => {
        ...
        const locked = this.isOpsLocked(rec);
        const statusClass = locked ? 'lcOpsStatusBadge is-locked' : '';
        const openDisabled = locked ? 'disabled aria-disabled="true"' : '';
        const opsInline = locked ? `<span class="lcCustomers__opsInline">בטיפול מחלקת תפעול</span>` : '';
...
ההכנסה מעבודה שאתה צפוי להרוויח תעמוד על סך של ______ ₪ לחודש. ידוע לך כי בקרות מקרה הביטוח יוגבלו תגמולי הפיצוי החודשי להם תהיה זכאי לגובה שלא יעלה על 75% מהכנסתך מעבודה בפועל ב־12 החודשים שקדמו למועד קרות מקרה הביטוח, או בתקופה ממועד תחילת הביטוח ועד מועד קרות מקרה הביטוח, אם חלפו פחות מ־12 חודשים.`
  ...
  const MirrorsUI = {
    ...
    init(){
      ...
      this.els.startModal = $("#mirrorsStartSessionModal");
      this.els.startModalText = $("#mirrorsStartSessionText");
      this.els.startModalStart = $("#btnMirrorsStartSession");
      this.els.startModalCancel = $("#btnMirrorsCancelStartSession");

      on(this.els.openBtn, "click", () => this.openSearch());
      on(this.els.closeBtn, "click", () => this.closeSearch());
      on(this.els.startModalCancel, "click", () => this.closeStartModal());
      on(this.els.startModalStart, "click", () => this.confirmStartSession());
    ...
    getOpsMirrorState(rec){
      return CustomersUI.getOpsMirrorState(rec);
    },
    ...
    async confirmStartSession(){
      ...
      s.lockedAt = stamp;
      rec.opsLocked = true;
      rec.opsClientSigned = false;
      rec.opsPhase = 'in_progress';
      rec.opsLockedBy = s.lockedBy;
      rec.opsLockedAt = stamp;
      rec.status = 'בטיפול מחלקת תפעול';
      rec.updatedAt = stamp;
      State.data.meta.updatedAt = stamp;
      this.selectedId = String(rec.id);
      this.consent = '';
      this.closeSearch();
      ...
      this.els.results.innerHTML = this.lastResults.map(rec => {
        ...
        return `<button class="mirrorsResultCard${selected ? ' is-selected' : ''}" data-mirror-customer="${escapeHtml(rec.id)}" type="button">
          ...
          <div class="mirrorsResultCard__side">
            ...
            ${CustomersUI.isOpsLocked && CustomersUI.isOpsLocked(rec) ? '<span class="mirrorsResultCard__lock">בטיפול תפעול</span>' : ''}
          </div>
        </button>`;
      }).join("");
    },
    ...
    render(){
      ...
      if(this.els.heroMeta){
        this.els.heroMeta.textContent = rec ? `שיקוף פעיל · ${rec.fullName || 'לקוח'} · ${CustomersUI.getCustomerStatusText ? CustomersUI.getCustomerStatusText(rec) : (rec.phone || rec.idNumber || '—')}` : 'טרם נבחר לקוח לשיקוף';
      }
      if(this.els.openBtn){
