...
(() => {
  ...
  const BUILD = "20260329-mfa-google-auth-v2";
  ...
  function normalizeAgentSecurityEntry(raw){
  ...
  function normalizeAgentSecurityMap(raw){
    const input = raw && typeof raw === "object" ? raw : {};
    const out = {};
    Object.entries(input).forEach(([key, value]) => {
      if(!key) return;
      out[String(key)] = normalizeAgentSecurityEntry(value);
    });
    return out;
  }

  function getAgentSecurityStore(){
  ...
  function getAgentSecurity(agentId){
    const key = safeTrim(agentId);
    if(!key) return normalizeAgentSecurityEntry({});
    const store = getAgentSecurityStore();
    return normalizeAgentSecurityEntry(store[key] || {});
  }

  function setAgentSecurity(agentId, patch = {}){
    const key = safeTrim(agentId);
    if(!key) return normalizeAgentSecurityEntry({});
    const store = getAgentSecurityStore();
  ...
  const Storage = {
    ...
    buildMetaRow(state){
      return {
        ...
        payload: {
          ...
          agentSecurity: normalizeAgentSecurityMap(state?.meta?.agentSecurity),
          updatedAt: nowISO()
        },
        updated_at: nowISO()
      };
    ...
    mapMeta(metaRow){
      ...
      return {
        ...
        agentSecurity: normalizeAgentSecurityMap(payload?.agentSecurity)
      };
    },
  ...
  const SupabaseMFA = {
    ...
    async signInWithPassword(email, password){
      ...
      if(error) return { ok:false, error: String(error.message || error), code: String(error.code || "") };
      return { ok:true, data: data || {} };
    },
    async listFactors(){
    ...
    extractTotpFactors(payload){
      const list = [];
      const addMany = (arr) => (Array.isArray(arr) ? arr : []).forEach((item) => { if(item && typeof item === "object") list.push(item); });
      ...
      return list.filter((factor) => {
        const fid = safeTrim(factor?.id);
        const type = safeTrim(factor?.factor_type || factor?.factorType || factor?.type).toLowerCase();
    ...
    async getVerifiedTotpFactor(){
      const listed = await this.listFactors();
      if(!listed.ok) return { ok:false, error: listed.error };
      return { ok:true, factor: this.getVerifiedTotpFactorFromData(listed.data), data: listed.data };
    },
    async verifyCode(factorId, code){
      const api = this.getClient()?.auth?.mfa;
      if(!api) return { ok:false, error:"MFA_VERIFY_NOT_AVAILABLE" };
      if(typeof api.challengeAndVerify === "function"){
        const { data, error } = await api.challengeAndVerify({ factorId, code });
        if(error) return { ok:false, error: String(error.message || error) };
        return { ok:true, data: data || {} };
      }
      ...
      const challengeId = safeTrim(challenge?.data?.id || challenge?.data?.challengeId);
      const { data, error } = await api.verify({ factorId, challengeId, code });
      if(error) return { ok:false, error: String(error.message || error) };
      return { ok:true, data: data || {} };
    },
    async enrollTotp(friendlyName = ""){ 
      ...
      const { data, error } = await api.enroll({ factorType: "totp", issuer: "GEMEL INVEST", friendlyName: safeTrim(friendlyName) || "GEMEL INVEST" });
      if(error) return { ok:false, error: String(error.message || error), code: String(error.code || "") };
      return { ok:true, data: data || {} };
    },
    extractQrMarkup(data){ const totp = data?.totp || data; return safeTrim(totp?.qr_code || totp?.qrCode || totp?.svg || totp?.svgCode); },
    extractUri(data){ const totp = data?.totp || data; return safeTrim(totp?.uri || totp?.otpauth_url || totp?.otpauthUrl); },
    async unenrollFactor(factorId){
      const api = this.getClient()?.auth?.mfa;
      if(!api || typeof api.unenroll !== "function") return { ok:false, error:"MFA_UNENROLL_NOT_AVAILABLE" };
      const { data, error } = await api.unenroll({ factorId });
      if(error) return { ok:false, error: String(error.message || error) };
      return { ok:true, data: data || {} };
    },
    async clearTotpFactors(options = {}){
      const listed = await this.listFactors();
      if(!listed.ok) return { ok:false, error: listed.error || "LIST_FACTORS_FAILED" };
      const factors = this.extractTotpFactors(listed.data);
      const skipVerified = options.skipVerified === true;
      for(const factor of factors){
        const status = safeTrim(factor?.status).toLowerCase();
        if(skipVerified && (status === "verified" || status === "enabled")) continue;
        const factorId = safeTrim(factor?.id);
        if(!factorId) continue;
        const removed = await this.unenrollFactor(factorId);
        if(!removed.ok) return { ok:false, error: removed.error || "UNENROLL_FAILED" };
      }
  ...
  const Auth = {
    ...
    init(){
      this.els = {
        ...
        pin: $("#lcLoginPin"),
        err: $("#lcLoginError"),
        credentialsStep: $("#lcLoginCredentialsStep"),
        mfaStep: $("#lcLoginMfaStep"),
        mfaName: $("#lcLoginMfaUserName"),
        mfaCode: $("#lcLoginMfaCode"),
        mfaVerify: $("#btnVerifyLoginMfa"),
        mfaBack: $("#btnBackLoginMfa"),
        mfaHint: $("#lcLoginMfaHint")
      };
      ...
      on(this.els.mfaVerify, "click", async () => { await this._verifyMfaStep(); });
      on(this.els.mfaBack, "click", async () => { await this._cancelMfaStep(); });
    },
    ...
    logout(reason = "manual"){
      this.current = null;
      try { SupabaseMFA.signOutSilently(); } catch(_e) {}
      BirthdaysUI.stop();
      try { localStorage.removeItem(LS_SESSION_KEY); } catch(_) {}
    ...
    _showCredentialsStep(){
      ...
      try { this.els.credentialsStep?.removeAttribute("hidden"); } catch(_e) {}
      try { this.els.mfaStep?.setAttribute("hidden", "hidden"); } catch(_e) {}
      if(this.els.mfaCode) this.els.mfaCode.value = "";
      if(this.els.mfaHint) this.els.mfaHint.textContent = "פתח את Google Authenticator והזן את הקוד העדכני.";
    },

    _showMfaStep(agent){
      try { this.els.credentialsStep?.setAttribute("hidden", "hidden"); } catch(_e) {}
      try { this.els.mfaStep?.removeAttribute("hidden"); } catch(_e) {}
      if(this.els.mfaName) this.els.mfaName.textContent = safeTrim(agent?.name) || "נציג";
      if(this.els.mfaCode){ this.els.mfaCode.value = ""; setTimeout(() => this.els.mfaCode?.focus?.(), 40); }
      if(this.els.mfaHint) this.els.mfaHint.textContent = "פתח את Google Authenticator והזן את הקוד העדכני.";
    },

    async _cancelMfaStep(){
      this.pendingMfa = null;
      await SupabaseMFA.signOutSilently();
      this._showCredentialsStep();
      this._setError("אימות דו־שלבי בוטל.");
    },

    async _verifyMfaStep(){
      const pending = this.pendingMfa;
      if(!pending?.factorId || !pending?.agent) return this._setError("לא נמצא אתגר אימות פעיל.");
      const code = safeTrim(this.els.mfaCode?.value);
      if(!/^\d{6}$/.test(code)) return this._setError("נא להזין קוד בן 6 ספרות.");
      this._setError("");
      const verifyBtn = this.els.mfaVerify;
      const prev = verifyBtn?.textContent || "אמת והיכנס";
      if(verifyBtn){ verifyBtn.disabled = true; verifyBtn.textContent = "מאמת…"; }
      const verified = await SupabaseMFA.verifyCode(pending.factorId, code);
      if(verifyBtn){ verifyBtn.disabled = false; verifyBtn.textContent = prev; }
      if(!verified.ok) return this._setError("קוד אימות שגוי או שפג תוקפו.");
      setAgentSecurity(pending.agent.id, { mfaEnabled:true, factorId: pending.factorId, lastVerifiedAt: nowISO() });
      await this._completeAgentLogin(pending.agent);
      this.pendingMfa = null;
      this._showCredentialsStep();
    },

    async _completeAgentLogin(agent){
      this.current = { name: agent.name, role: (agent.role === "manager" ? "manager" : agent.role === "ops" ? "ops" : "agent") };
      try { localStorage.removeItem(LS_SESSION_KEY); } catch(_) {}
      ...
      if(security.authEmail && security.mfaRequired && !security.mfaEnabled){
        setTimeout(() => { try { SecurityUI.open({ forceEnroll:true }); } catch(_e) {} }, 180);
      }
    },
    ...
    async _submit(){
      ...
      const localPinOk = (pin === expected);
      const security = getAgentSecurity(matched.id);
      const authEmail = safeTrim(security.authEmail);

      if(authEmail){
        let authResult = await SupabaseMFA.signInWithPassword(authEmail, pin);
        if(!authResult.ok){
          if(!localPinOk) return this._setError("קוד כניסה שגוי");
          const authPwd = window.prompt("למשתמש הזה מוגדר חיבור ל-Supabase Auth.\nכדי להמשיך ולהפעיל/להשתמש באימות דו־שלבי, הזן את סיסמת ה-Auth של המשתמש.", "");
          if(authPwd == null){
            if(security.mfaRequired || security.mfaEnabled) return this._setError("כדי להמשיך יש להזין את סיסמת ה-Auth של המשתמש.");
            await this._completeAgentLogin(matched);
            return;
          }
          authResult = await SupabaseMFA.signInWithPassword(authEmail, safeTrim(authPwd));
          if(!authResult.ok) return this._setError("סיסמת ה-Auth של המשתמש שגויה או שהמייל ב-Supabase אינו תקין.");
        }
        const factorResult = await SupabaseMFA.getVerifiedTotpFactor();
        if(factorResult.ok && factorResult.factor){
          this.pendingMfa = { agent: matched, factorId: safeTrim(factorResult.factor.id) };
          this._showMfaStep(matched);
          return;
        }
        setAgentSecurity(matched.id, { mfaEnabled:false, factorId:"" });
        await this._completeAgentLogin(matched);
        return;
      }
  ...
  const UI = {
    ...
    init(){
      ...
      this.els.btnOpenSecurity = $("#btnOpenSecurityCenter");
...
this.applyRoleUI();
    ...
    renderAuthPill(){
      ...
      if(Auth.current) {
        pill.style.display = "";
        const currentAgent = (State.data?.agents || []).find(a => safeTrim(a?.name) === safeTrim(Auth.current?.name) || safeTrim(a?.username) === safeTrim(Auth.current?.name)) || null;
        const security = currentAgent ? getAgentSecurity(currentAgent.id) : normalizeAgentSecurityEntry({});
        txt.textContent = Auth.current.name + (Auth.isAdmin() ? " (מנהל מערכת)" : Auth.isManager() ? " (מנהל)" : Auth.isOps() ? " (תפעול)" : "") + (security.mfaEnabled ? " · 2FA" : "");
        if(this.els.btnOpenSecurity) this.els.btnOpenSecurity.style.display = Auth.isAdmin() ? "none" : "";
        BirthdaysUI.decorateUserPill(BirthdaysUI.getCurrentAgentRecord());
      } else {
        pill.style.display = "none";
        txt.textContent = "";
        if(this.els.btnOpenSecurity) this.els.btnOpenSecurity.style.display = "none";
        BirthdaysUI.stop();
      }
    },
  ...
  const UsersUI = {
    ...
    openModal(mode, user){
      ...
      const security = user ? getAgentSecurity(user.id) : normalizeAgentSecurityEntry({});
      if(E.pin) E.pin.value = user ? (user.pin || "") : "0000";
      if(E.authEmail) E.authEmail.value = security.authEmail || "";
      if(E.mfaRequired) E.mfaRequired.checked = security.mfaRequired === true;
    ...
    async _saveFromModal(){
      ...
      const authEmail = safeTrim(E.authEmail?.value).toLowerCase();
      const mfaRequired = !!E.mfaRequired?.checked;
      ...
      if(isEdit){
        ...
        setAgentSecurity(a.id, { authEmail, mfaRequired, mfaEnabled: authEmail ? getAgentSecurity(a.id).mfaEnabled : false, factorId: authEmail ? getAgentSecurity(a.id).factorId : "" });
        ...
        setAgentSecurity(newId, { authEmail, mfaRequired, mfaEnabled:false, factorId:"" });
        State.data.meta.updatedAt = nowISO();
    ...
    render(){
      ...
      UI.els.usersTbody.innerHTML = rows.map(a => {
        ...
        const security = getAgentSecurity(a.id);
        const securityLabel = security.authEmail ? (security.mfaEnabled ? "Auth + 2FA" : (security.mfaRequired ? "Auth · ממתין ל-2FA" : "Auth בלבד")) : "PIN מקומי";
        return `
          <tr>
            <td>${escapeHtml(a.name)}<div class="lcUsers__sub">${escapeHtml(safeTrim(security.authEmail) || a.username)}</div></td>
            <td>${role}</td>
            <td>${escapeHtml(formatBirthDateDisplay(a.birthDate))}</td>
            <td><span class="badge">${status}</span></td>
            <td><span class="badge">${escapeHtml(securityLabel)}</span></td>
            <td>
  ...
  const SecurityUI = {
    ...
    init(){
      this.els = {
        wrap: $("#lcSecurityModal"), close: $("#lcSecurityClose"), cancel: $("#lcSecurityCancel"), title: $("#lcSecurityAgentName"), status: $("#lcSecurityStatus"), authEmail: $("#lcSecurityAuthEmail"), authNote: $("#lcSecurityAuthNote"), qrWrap: $("#lcSecurityQrWrap"), qr: $("#lcSecurityQr"), secret: $("#lcSecuritySecret"), code: $("#lcSecurityCode"), start: $("#lcSecurityStartSetup"), verify: $("#lcSecurityVerify"), refresh: $("#lcSecurityRefresh"), disable: $("#lcSecurityDisable"), error: $("#lcSecurityError")
      };
      on(this.els.close, "click", () => this.close());
      on(this.els.cancel, "click", () => this.close());
      on(this.els.wrap, "click", (ev) => { if(ev.target?.getAttribute?.("data-close") === "1") this.close(); });
      on(this.els.start, "click", () => this.startEnrollment());
      on(this.els.verify, "click", () => this.verifyEnrollment());
      on(this.els.refresh, "click", () => this.render());
      on(this.els.disable, "click", () => this.disableCurrentFactor());
    },
    getCurrentAgent(){ if(!Auth.current || Auth.isAdmin()) return null; return (State.data?.agents || []).find((a) => safeTrim(a?.name) === safeTrim(Auth.current?.name) || safeTrim(a?.username) === safeTrim(Auth.current?.name)) || null; },
    open(options = {}){ this.forceEnroll = options.forceEnroll === true; this.currentFactorId = ""; this.render(); this.els.wrap?.classList.add("is-open"); this.els.wrap?.setAttribute("aria-hidden","false"); },
    close(){ if(this.forceEnroll) return; this.els.wrap?.classList.remove("is-open"); this.els.wrap?.setAttribute("aria-hidden","true"); this.setError(""); },
    ...
    async render(){
      ...
      if(this.els?.title) this.els.title.textContent = safeTrim(agent?.name) || "—";
      ...
      if(this.els?.authNote) this.els.authNote.textContent = !security.authEmail
        ? "לפני הפעלת 2FA יש להגדיר למשתמש auth email במסך ניהול משתמשים."
        : (hasAuthSession
            ? "המשתמש מחובר דרך Supabase Auth עם המייל הזה."
            : "כדי להפעיל 2FA יש להתנתק ולהיכנס מחדש עם סיסמת ה-Auth, או להזין אותה כאשר תלחץ על הפעל Google Authenticator.");
      let factor = { ok:false, factor:null };
      if(hasAuthSession){
        factor = await SupabaseMFA.getVerifiedTotpFactor();
      }
      const active = !!factor.factor;
      if(agent && active){ setAgentSecurity(agent.id, { mfaEnabled:true, factorId: safeTrim(factor.factor?.id), mfaEnrolledAt: getAgentSecurity(agent.id).mfaEnrolledAt || nowISO() }); }
      if(this.els?.status) this.els.status.textContent = active ? "האימות הדו־שלבי פעיל ומאומת." : (security.mfaRequired ? "נדרש להפעיל Google Authenticator כדי להמשיך." : "האימות הדו־שלבי עדיין לא הופעל.");
      if(this.els?.start) this.els.start.disabled = !security.authEmail || active;
      if(this.els?.disable) this.els.disable.disabled = !active;
      if(this.els?.verify) this.els.verify.disabled = !this.currentFactorId;
    },
    async ensureAuthSessionForCurrentAgent(agent, security){
      const email = safeTrim(security?.authEmail);
      if(!agent || !email) return { ok:false, error:"NO_AUTH_EMAIL" };
      const current = await SupabaseMFA.getUser();
      const currentEmail = safeTrim(current?.user?.email).toLowerCase();
      if(current.ok && currentEmail && currentEmail === email.toLowerCase()) return { ok:true, reused:true };
      const pwd = window.prompt("כדי להפעיל Google Authenticator יש להזין את סיסמת ה-Auth של המשתמש.\nאם לא הגדרת עדיין סיסמה ב-Supabase Auth, יש ליצור אותה קודם.", "");
      if(pwd == null) return { ok:false, error:"CANCELLED" };
      const authResult = await SupabaseMFA.signInWithPassword(email, safeTrim(pwd));
      if(!authResult.ok) return { ok:false, error: authResult.error || "AUTH_SIGNIN_FAILED" };
      return { ok:true, reused:false };
    },
    async startEnrollment(){
      const agent = this.getCurrentAgent(); if(!agent) return;
      const security = getAgentSecurity(agent.id);
      if(!security.authEmail) return this.setError("חסר auth email למשתמש הזה. יש לעדכן במסך ניהול משתמשים.");
      this.setError("");
      const authReady = await this.ensureAuthSessionForCurrentAgent(agent, security);
      if(!authReady.ok){
        if(authReady.error === "CANCELLED") return this.setError("הפעלת 2FA בוטלה.");
        return this.setError("לא הצלחתי להתחבר למשתמש ה-Auth. בדוק שהמייל והסיסמה שהוגדרו ב-Supabase תקינים.");
      }
      const currentFactor = await SupabaseMFA.getVerifiedTotpFactor();
      if(currentFactor.ok && currentFactor.factor){
        this.currentFactorId = safeTrim(currentFactor.factor?.id);
        setAgentSecurity(agent.id, { mfaEnabled:true, factorId: this.currentFactorId, mfaEnrolledAt: getAgentSecurity(agent.id).mfaEnrolledAt || nowISO() });
        await this.render();
        return this.setError("למשתמש הזה כבר מוגדר Google Authenticator פעיל.");
      }
      const cleared = await SupabaseMFA.clearTotpFactors();
      if(!cleared.ok){
        return this.setError("לא הצלחתי לנקות ניסיון הרשמה קודם. " + String(cleared.error || ""));
      }
      const enrolled = await SupabaseMFA.enrollTotp(agent.name || "GEMEL INVEST");
      if(!enrolled.ok) return this.setError("לא הצלחתי להתחיל הרשמה ל-Google Authenticator. " + String(enrolled.error || ""));
      const data = enrolled.data || {};
      this.currentFactorId = safeTrim(data?.id || data?.factorId || data?.totp?.id);
      if(this.els?.qr) this.els.qr.innerHTML = SupabaseMFA.extractQrMarkup(data) || "";
      if(this.els?.secret) this.els.secret.textContent = SupabaseMFA.extractUri(data) || "סרוק את ה-QR מתוך Google Authenticator";
      if(this.els?.qrWrap) this.els.qrWrap.hidden = false;
      if(this.els?.verify) this.els.verify.disabled = !this.currentFactorId;
      if(this.els?.code) { this.els.code.value = ""; this.els.code.focus(); }
    },
    async verifyEnrollment(){
      const agent = this.getCurrentAgent();
      if(!agent || !this.currentFactorId) return this.setError("אין הרשמה פעילה לאימות דו־שלבי.");
      const code = safeTrim(this.els?.code?.value);
      if(!/^\d{6}$/.test(code)) return this.setError("נא להזין קוד בן 6 ספרות.");
      const verified = await SupabaseMFA.verifyCode(this.currentFactorId, code);
      if(!verified.ok) return this.setError("הקוד שגוי או שפג תוקפו.");
      setAgentSecurity(agent.id, { mfaEnabled:true, mfaEnrolledAt: nowISO(), factorId: this.currentFactorId, lastVerifiedAt: nowISO() });
      await App.persist("האימות הדו־שלבי הופעל");
      this.currentFactorId = "";
      if(this.els?.qrWrap) this.els.qrWrap.hidden = true;
      await this.render();
      this.forceEnroll = false;
    },
    async disableCurrentFactor(){
      const agent = this.getCurrentAgent(); if(!agent) return;
      const factor = await SupabaseMFA.getVerifiedTotpFactor();
      if(!factor.ok || !factor.factor) return this.setError("לא נמצא פקטור פעיל למחיקה.");
      const removed = await SupabaseMFA.unenrollFactor(safeTrim(factor.factor.id));
      if(!removed.ok) return this.setError("לא הצלחתי להסיר את הפקטור הפעיל.");
      setAgentSecurity(agent.id, { mfaEnabled:false, factorId:"", mfaEnrolledAt:"" });
      await App.persist("האימות הדו־שלבי הוסר");
      await this.render();
    }
  };
