...
// open health covers drawer (Health only)
    ...
    parseHarBituachWorkbook(buffer, ins){
      ...
      const headerMatches = (value, patterns = []) => {
      ...
      const isHealthLifeRow = (row = {}) => {
        ...
        if(/ביטוח רכב|רכב חובה|ביטוח מקיף|גרירה|שמשות|צד שלישי|ביטוח דירה|מבנה|תכולה/i.test(source)) return false;
        return /בריאות|תאונות אישיות|סיעוד|סיעודי|מחלות קשות|סרטן|השתלות|תרופות|ניתוחים|ייעוץ|בדיקות|שב"ן|שב״ן|חיים|מקרה מוות|אבדן כושר|אובדן כושר|נכויות|משכנתא/i.test(source);
      };
      ...
      sheetNames.forEach((sheetName) => {
        ...
        const headerIdx = rows.findIndex((row) => {
          ...
          return row.some((cell) => headerMatches(cell, [/^תעודת זהות$/, /^מספר תעודת זהות$/]))
            && row.some((cell) => headerMatches(cell, [/^ענף ראשי$/, /^ענף\s*\(?.*ראשי.*\)?$/]))
            && row.some((cell) => headerMatches(cell, [/^חברה$/, /חברה מבטחת/, /מבטח/]));
        });
        ...
        const idx = {
          idNumber: findIndex(/^תעודת זהות$/, /^מספר תעודת זהות$/),
          main: findIndex(/^ענף ראשי$/, /^ענף\s*\(?.*ראשי.*\)?$/),
          sub: findIndex(/^ענף \(משני\)$/, /^ענף משני$/, /^ענף\s*\(?.*משני.*\)?$/),
          productType: findIndex(/^סוג מוצר$/, /^מוצר$/, /^שם מוצר$/),
          company: findIndex(/^חברה$/, /חברה מבטחת/, /מבטח/, /שם חברה/),
          period: findIndex(/^תקופת ביטוח$/, /תקופה/),
          extra: findIndex(/^פרטים נוספים$/, /פרטים נוספים/, /מידע נוסף/),
          premium: findIndex(/^פרמיה בש"ח$/, /^פרמיה בשח$/, /^פרמיה$/, /פרמיה לתשלום/, /סכום פרמיה/),
          premiumType: findIndex(/^סוג פרמיה$/, /תדירות פרמיה/),
          policyNumber: findIndex(/^מספר פוליסה$/, /^מס' פוליסה$/, /^מס פוליסה$/, /פוליסה/)
        };
        ...
        rows.slice(headerIdx + 1).forEach((row) => {
          ...
          const company = this.normalizeHarCompanyName(idx.company >= 0 ? row[idx.company] : "");
          if(!company) return;
