...
// ---------- Step 6 ----------
    ...
    getHealthStore(){
      ...
      if(!primary.data.healthDeclaration || typeof primary.data.healthDeclaration !== 'object'){
        primary.data.healthDeclaration = { version:'matrix_v1', activeCategory:'general', answers:{} };
      }
      const store = primary.data.healthDeclaration;
      if(!store.answers || typeof store.answers !== 'object') store.answers = {};
      if(!safeTrim(store.version)) store.version = 'matrix_v1';
      if(!safeTrim(store.activeCategory)) store.activeCategory = 'general';
      return store;
    },
    ...
    getHealthQuestionMeta(qKey){
      const map = {
        chronic_condition: { category:'general', short:'מצב כללי', question:'האם קיימת מחלה כרונית / מצב רפואי מתמשך?' },
        medications: { category:'general', short:'תרופות קבועות', question:'האם יש תרופות קבועות / טיפול פעיל?' },
        surgery_hospitalization: { category:'general', short:'אשפוזים / ניתוחים', question:'האם היו אשפוזים / ניתוחים / בירור משמעותי?' },
        heart_bp: { category:'heart', short:'לב / לחץ דם', question:'האם קיימת בעיה בלב / לחץ דם / כלי דם?' },
        respiratory: { category:'resp', short:'נשימה / ריאות', question:'האם קיימת בעיה בריאות / נשימה / דום נשימה?' },
        neuro: { category:'neuro', short:'נוירולוגיה', question:'האם קיימת בעיה נוירולוגית / אפילפסיה / התעלפויות?' },
        musculoskeletal: { category:'ortho', short:'שלד / גב / מפרקים', question:'האם קיימת בעיה בגב / שלד / מפרקים / נכות?' },
        endocrine: { category:'chronic', short:'סוכרת / בלוטות', question:'האם קיימת סוכרת / בלוטות / בעיה הורמונלית / מטבולית?' },
        cancer_tumor: { category:'severe', short:'סרטן / גידול', question:'האם הייתה מחלה ממארת / גידול / ביופסיה / ממצא חריג?' },
        digestion_liver_kidney: { category:'chronic', short:'עיכול / כבד / כליות', question:'האם קיימת בעיה בעיכול / כבד / כליות / שתן?' },
        mental: { category:'severe', short:'נפש', question:'האם הייתה בעיה נפשית / טיפול פסיכולוגי / פסיכיאטרי?' },
        habits: { category:'habits', short:'עישון / אלכוהול / סמים', question:'האם יש עישון / אלכוהול / סמים / קנאביס?' },
        family_history: { category:'family', short:'היסטוריה משפחתית', question:'האם קיימת היסטוריה משפחתית מהותית?' },
        pregnancy: { category:'women', short:'היריון / נשים', question:'האם קיים היריון / בירור / ממצא נשי רלוונטי?' }
      };
      return map[qKey] || { category:'general', short:'כללי', question:'' };
    },

    getHealthCategories(){
      return [
        { key:'general', label:'מצב רפואי כללי' },
        { key:'heart', label:'לב וכלי דם' },
        { key:'resp', label:'ריאות ונשימה' },
        { key:'neuro', label:'נוירולוגיה' },
        { key:'chronic', label:'מחלות כרוניות' },
        { key:'severe', label:'מחלות קשות' },
        { key:'habits', label:'אורח חיים' },
        { key:'family', label:'משפחה' },
        { key:'women', label:'נשים / היריון' },
        { key:'ortho', label:'שלד ומפרקים' }
      ];
    },
      ...
      return [
        {
          ...
          fields:[
            { key:'diagnosis', label:'אבחנה', type:'text', required:true },
            { key:'diagnosedAt', label:'מועד אבחנה', type:'text', required:true },
            { key:'lastEpisode', label:'אירוע / החמרה אחרונה', type:'text' },
            { key:'currentStatus', label:'מצב כיום', type:'textarea', required:true },
            { key:'followup', label:'מעקב / בדיקות / רופא מומחה', type:'textarea' }
          ]
        },
        {
          key:'medications',
          ...
          fields:[
            ...
            { key:'reason', label:'סיבה לטיפול', type:'textarea', required:true },
        ...
        },
        {
          ...
          fields:[
            ...
            { key:'result', label:'תוצאה / מצב כיום', type:'textarea', required:true },
            { key:'documents', label:'מסמכים קיימים', type:'textarea' }
          ]
        },
        {
          key:'heart_bp',
          title:'לב / לחץ דם / כלי דם',
          summary:'לב, לחץ דם, הפרעות קצב, צנתור, אוטם, מסתמים וכלי דם.',
          companies: allCompanies,
          fields:[
            { key:'diagnosis', label:'אבחנה', type:'text', required:true },
            { key:'diagnosedAt', label:'מועד אבחנה', type:'text', required:true },
            { key:'lastEvent', label:'אירוע / צנתור / אשפוז אחרון', type:'text' },
            { key:'treatment', label:'טיפול / תרופות / ניתוח', type:'textarea', required:true },
            { key:'status', label:'מצב כיום / ערכי לחץ דם אם ידועים', type:'textarea', required:true }
          ]
        },
        {
          key:'respiratory',
          title:'ריאות / נשימה / דום נשימה',
          summary:'אסטמה, COPD, ברונכיטיס כרונית, סרקואידוזיס, דום נשימה ובעיות נשימה נוספות.',
          companies: allCompanies,
          fields:[
            { key:'diagnosis', label:'אבחנה', type:'text', required:true },
            { key:'diagnosedAt', label:'מועד אבחנה', type:'text', required:true },
            { key:'frequency', label:'חומרה / תדירות / התקפים', type:'text', required:true },
            { key:'treatment', label:'טיפול', type:'textarea', required:true },
            { key:'tests', label:'בדיקות / אשפוזים / תפקודי ריאות', type:'textarea' }
          ]
        },
        {
          key:'neuro',
          title:'נוירולוגיה / מוח / אפילפסיה / התעלפויות',
          summary:'אפילפסיה, אירוע מוחי, חבלת ראש, טרשת, רעד, סחרחורות והתעלפויות.',
          companies: allCompanies,
          fields:[
            { key:'diagnosis', label:'אבחנה', type:'text', required:true },
            { key:'diagnosedAt', label:'מועד אבחנה / אירוע ראשון', type:'text', required:true },
            { key:'lastEvent', label:'מועד אירוע / התקף אחרון', type:'text' },
            { key:'frequency', label:'תדירות / מספר התקפים', type:'text' },
        ...
        },
        {
          ...
          summary:'בעיות גב, פריצות דיסק, שברים, מניסקוס, רצועות, מחלות ראומטולוגיות, תאונות ומגבלה תפקודית.',
          companies: allCompanies,
          fields:[
            { key:'diagnosis', label:'אבחנה / סוג פגיעה', type:'text', required:true },
            { key:'location', label:'מיקום בגוף / צד הפגיעה', type:'text', required:true },
            { key:'when', label:'מתי קרה / אובחן', type:'text', required:true },
            { key:'treatment', label:'טיפול / פיזיותרפיה / ניתוח / זריקות', type:'textarea' },
            { key:'limitation', label:'מגבלה / נכות / מצב כיום', type:'textarea', required:true }
          ]
        },
        {
          key:'endocrine',
          title:'סוכרת / בלוטות / הורמונים / מטבולי',
          summary:'סוכרת, בלוטת התריס, בלוטות נוספות, הורמונים, ועוד.',
          companies: allCompanies,
          fields:[
            { key:'diagnosis', label:'אבחנה', type:'text', required:true },
            { key:'diagnosedAt', label:'מועד אבחנה', type:'text', required:true },
        ...
        },
        {
          ...
          companies: allCompanies,
          fields:[
            { key:'diagnosis', label:'אבחנה / סוג גידול / ממצא', type:'textarea', required:true },
            { key:'diagnosedAt', label:'מועד גילוי', type:'text', required:true },
            { key:'treatment', label:'טיפול / ניתוח / הקרנות / כימו', type:'textarea', required:true },
            { key:'currentStatus', label:'מצב כיום / מעקב', type:'textarea', required:true }
          ]
        },
        {
          key:'digestion_liver_kidney',
          title:'עיכול / כבד / כליות / שתן',
          summary:'שאלה מאוחדת למחלות כבד, לבלב, מעי, כליות, חלבון/דם בשתן, אבנים, וכדומה.',
          companies: allCompanies,
          fields:[
            { key:'diagnosis', label:'אבחנה', type:'text', required:true },
            { key:'diagnosedAt', label:'מועד אבחנה', type:'text', required:true },
            { key:'treatment', label:'טיפול / בירור / בדיקות', type:'textarea', required:true },
            { key:'status', label:'מצב כיום', type:'textarea' }
          ]
        },
        {
          key:'mental',
          title:'נפש / פסיכולוגי / פסיכיאטרי',
          summary:'דיכאון, חרדה, טיפול פסיכולוגי, טיפול פסיכיאטרי, תרופות, אשפוזים וניסיונות אובדניים.',
          companies: allCompanies,
          fields:[
            { key:'diagnosis', label:'אבחנה', type:'text', required:true },
            { key:'treatment', label:'טיפול / תרופות', type:'textarea', required:true },
            { key:'hospitalization', label:'אשפוזים / ניסיונות אובדניים / נכות', type:'textarea' },
            { key:'status', label:'מצב כיום', type:'textarea', required:true }
          ]
        },
        {
          key:'habits',
          title:'עישון / אלכוהול / סמים / קנאביס',
          summary:'כולל טבק, סיגריות, קנאביס רפואי / לא רפואי, סמים, גמילה וצריכת אלכוהול חריגה.',
          companies: allCompanies,
          fields:[
            { key:'substance', label:'מה רלוונטי? עישון / אלכוהול / קנאביס / סם', type:'text', required:true },
            { key:'frequency', label:'תדירות / כמות', type:'text', required:true },
            { key:'sinceWhen', label:'ממתי / מתי הופסק', type:'text', required:true },
            { key:'medicalReason', label:'אם שימוש רפואי - מה הסיבה?', type:'textarea' },
            { key:'rehab', label:'גמילה / טיפול / מעקב', type:'textarea' }
          ]
        },
        {
          key:'family_history',
          title:'רקע משפחתי מהותי',
          summary:'מחלות משפחתיות מדרגה ראשונה כמו סרטן, לב, סוכרת, כליות, נוירולוגיה.',
          ...
          fields:[
            ...
            { key:'count', label:'כמה קרובי משפחה רלוונטיים?', type:'text' }
          ]
        },
        {
          key:'pregnancy',
          title:'היריון / בירור נשי רלוונטי',
          summary:'מוצג רק כאשר יש מבוטחת בתיק, עבור היריון, סיבוכים ובירורים רלוונטיים.',
          companies: allCompanies,
          onlyForFemale:true,
          fields:[
            { key:'details', label:'פירוט', type:'textarea', required:true },
            { key:'weekOrDate', label:'שבוע היריון / מועד', type:'text' },
    ...
    getRelevantHealthQuestions(context){
      ...
      return this.getHealthSchema().filter(q => {
        if(q.onlyForFemale){
          const femaleInsured = (this.insureds || []).some(ins => safeTrim(ins?.data?.gender) === 'נקבה');
          if(!femaleInsured) return false;
        }
        return (q.companies || []).some(c => ctx.companies.includes(c));
      });
    },
    ...
    getInsuredDisplayName(ins){
      const full = (safeTrim(ins?.data?.firstName) + ' ' + safeTrim(ins?.data?.lastName)).trim();
      return full || ins?.label || 'מבוטח';
    },

    getHealthQuestionState(qKey){
      const store = this.getHealthStore();
      if(!store.answers[qKey] || typeof store.answers[qKey] !== 'object'){
        store.answers[qKey] = { insureds:{} };
      }
      if(!store.answers[qKey].insureds || typeof store.answers[qKey].insureds !== 'object'){
        store.answers[qKey].insureds = {};
      }
      return store.answers[qKey];
    },

    getHealthAnswer(qKey, insId){
      const qState = this.getHealthQuestionState(qKey);
      const s = qState.insureds[insId] || { answer:'', fields:{} };
      if(!s.fields || typeof s.fields !== 'object') s.fields = {};
      qState.insureds[insId] = s;
      return s;
    },

    setHealthAnswer(qKey, insId, answer){
      const s = this.getHealthAnswer(qKey, insId);
      s.answer = answer;
      if(answer === 'no') s.fields = {};
    },

    renderHealthField(qKey, insId, field, values){
      const val = safeTrim(values?.[field.key] || '');
      const req = field.required ? ' <span style="color:#b42318">*</span>' : '';
      const path = `${qKey}|${insId}|${field.key}`;
      if(field.type === 'textarea'){
        return `<div class="lcHField" style="grid-column:1 / -1"><label class="lcHLabel">${escapeHtml(field.label)}${req}</label><textarea class="lcHTextarea" rows="3" data-healthf="${escapeHtml(path)}">${escapeHtml(val)}</textarea></div>`;
      }
      if(field.type === 'select'){
        ...
        return `<div class="lcHField"><label class="lcHLabel">${escapeHtml(field.label)}${req}</label><select class="lcHSelect" data-healthf="${escapeHtml(path)}">${opts}</select></div>`;
      }
      return `<div class="lcHField"><label class="lcHLabel">${escapeHtml(field.label)}${req}</label><input class="lcHInput" type="text" value="${escapeHtml(val)}" data-healthf="${escapeHtml(path)}" /></div>`;
    },

    renderHealthInsuredSummary(ins){
      const context = this.getHealthContext();
      const questions = this.getRelevantHealthQuestions(context);
      let yesCount = 0;
      let missingCount = 0;
      questions.forEach(q => {
        const s = this.getHealthAnswer(q.key, ins.id);
        if(s.answer === 'yes') yesCount += 1;
        if(s.answer === 'yes'){
          (q.fields || []).forEach(f => {
            if(f.required && !safeTrim(s.fields?.[f.key])) missingCount += 1;
          });
        }
      });
      const cls = missingCount ? 'warn' : (yesCount ? 'ok' : 'idle');
      const status = missingCount ? 'חסר פירוט' : (yesCount ? 'יש ממצאים' : 'ללא ממצאים');
      return `<div class="lcHInsuredChip ${cls}"><span class="lcHInsuredChip__name">${escapeHtml(this.getInsuredDisplayName(ins))}</span><span class="lcHInsuredChip__meta">${escapeHtml(status)}</span></div>`;
    },
    ...
    renderStep8(){
      ...
      const store = this.getHealthStore();
      const allQuestions = this.getRelevantHealthQuestions(context);
      const categories = this.getHealthCategories().filter(cat => allQuestions.some(q => this.getHealthQuestionMeta(q.key).category === cat.key));
      if(!categories.some(c => c.key === store.activeCategory)) store.activeCategory = categories[0]?.key || 'general';
      const activeCategory = store.activeCategory;
      const questions = allQuestions.filter(q => this.getHealthQuestionMeta(q.key).category === activeCategory);
      ...
      const insuredChips = this.insureds.map(ins => this.renderHealthInsuredSummary(ins)).join('');
      const categoryTabs = categories.map(cat => `<button type="button" class="lcHCatTab ${cat.key===activeCategory?'is-active':''}" data-healthcat="${escapeHtml(cat.key)}">${escapeHtml(cat.label)}</button>`).join('');

      const cards = questions.map(q => {
        const meta = this.getHealthQuestionMeta(q.key);
        ...
        const rows = this.insureds.map(ins => {
          if(q.onlyForFemale && safeTrim(ins?.data?.gender) !== 'נקבה') return '';
          const state = this.getHealthAnswer(q.key, ins.id);
          return `<div class="lcHMatrixRow">
            <div class="lcHMatrixRow__name">${escapeHtml(this.getInsuredDisplayName(ins))}</div>
            <div class="lcHDecision lcHDecision--seg">
              <button type="button" class="lcHToggle ${state.answer === 'yes' ? 'is-active' : ''}" data-healthq="${escapeHtml(q.key)}|${escapeHtml(ins.id)}|yes">כן</button>
              <button type="button" class="lcHToggle ${state.answer === 'no' ? 'is-active' : ''}" data-healthq="${escapeHtml(q.key)}|${escapeHtml(ins.id)}|no">לא</button>
            </div>
            <div class="lcHMatrixRow__status">${state.answer === 'yes' ? 'כן' : state.answer === 'no' ? 'לא' : 'טרם נענה'}</div>
          </div>`;
        }).join('');

        const details = this.insureds.map(ins => {
          if(q.onlyForFemale && safeTrim(ins?.data?.gender) !== 'נקבה') return '';
          const state = this.getHealthAnswer(q.key, ins.id);
          if(state.answer !== 'yes') return '';
          return `<div class="lcHInsuredPanel">
            <div class="lcHInsuredPanel__head">
              <div class="lcHInsuredPanel__title">פירוט עבור ${escapeHtml(this.getInsuredDisplayName(ins))}</div>
              <button type="button" class="btn btn--primary lcHSaveBtn" data-healthsave="${escapeHtml(q.key)}|${escapeHtml(ins.id)}">שמור</button>
            </div>
            <div class="lcHFields">${(q.fields || []).map(f => this.renderHealthField(q.key, ins.id, f, state.fields || {})).join('')}</div>
          </div>`;
        }).join('');

        return `<div class="lcHQuestionCard">
          <div class="lcHQuestionCard__head">
            <div>
              <div class="lcHQuestionCard__kicker">${escapeHtml(meta.short)}</div>
              <div class="lcHQuestionCard__title">${escapeHtml(meta.question || q.title)}</div>
              <div class="lcHQuestionCard__summary">${escapeHtml(q.summary)}</div>
              <div class="lcHCat__tags">${companyTags}</div>
            </div>
          </div>
          <div class="lcHMatrix">${rows}</div>
          ${details ? `<div class="lcHQuestionCard__body">${details}</div>` : ''}
        </div>`;
      }).join('');

      return `
        <div class="lcHealthWrap lcHealthWrap--matrix">
          <div class="lcWSection">
            <div class="lcWTitle">הצהרת בריאות – שאלון מאוחד לכל המבוטחים</div>
            <div class="muted" style="margin-top:6px">הנציג שואל כל שאלה פעם אחת בלבד, מסמן כן / לא עבור כל מבוטח, והמערכת פותחת פירוט רק למבוטח שסומן כן. המידע נשמר לכל מבוטח בנפרד.</div>
            ...
            <div class="lcHInsuredBar">${insuredChips}</div>
          </div>
          <div class="lcHCategoryTabs">${categoryTabs}</div>
          ${cards || `<div class="lcHealthEmpty"><div class="lcHealthEmpty__title">אין שאלות להצגה בקטגוריה זו</div></div>`}
        </div>`;
    },

    bindHealthInputs(){
      $$('[data-healthcat]', this.els.body).forEach(btn => {
        on(btn, 'click', () => {
          const key = safeTrim(btn.getAttribute('data-healthcat'));
          if(!key) return;
          this.getHealthStore().activeCategory = key;
          this.render();
        });
      });
      $$('[data-healthq]', this.els.body).forEach(btn => {
        on(btn, 'click', () => {
          ...
          const [qKey, insId, ans] = raw.split('|');
          if(!qKey || !insId || !ans) return;
          this.setHealthAnswer(qKey, insId, ans);
          this.render();
        });
      });
      $$('[data-healthf]', this.els.body).forEach(el => {
        const save = () => {
          ...
          const [qKey, insId, fieldKey] = raw.split('|');
          if(!qKey || !insId || !fieldKey) return;
          const prev = this.getHealthAnswer(qKey, insId);
          prev.answer = 'yes';
      ...
      $$('[data-healthsave]', this.els.body).forEach(btn => {
        on(btn, 'click', () => {
          const raw = btn.getAttribute('data-healthsave') || '';
          const [qKey, insId] = raw.split('|');
          const ins = this._findInsuredById(insId);
          const q = (this.getRelevantHealthQuestions(this.getHealthContext()) || []).find(x => x.key === qKey);
          this.setHint(`נשמר פירוט עבור ${this.getInsuredDisplayName(ins)} · ${q?.title || ''}`);
        });
      });
    },
    ...
    validateStep8(){
      ...
      for(const q of questions){
        for(const ins of (this.insureds || [])){
          if(q.onlyForFemale && safeTrim(ins?.data?.gender) !== 'נקבה') continue;
          const state = this.getHealthAnswer(q.key, ins.id);
          if(state.answer !== 'yes' && state.answer !== 'no'){
            return { ok:false, msg:`יש להשיב כן / לא עבור ${this.getInsuredDisplayName(ins)} בשאלה: ${this.getHealthQuestionMeta(q.key).question || q.title}` };
          }
          if(state.answer === 'yes'){
            for(const field of (q.fields || [])){
              if(field.required && !safeTrim(state.fields?.[field.key])){
                return { ok:false, msg:`חסר פירוט חובה עבור ${this.getInsuredDisplayName(ins)} ב-${q.title}: ${field.label}` };
              }
            }
          }
        }
