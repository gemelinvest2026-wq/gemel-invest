...
(() => {
  ...
  const BUILD = "20260405-phoenix-health-smart-pdf-v42-inline-btn";
...
init(){
    ...
    renderFooter(){
      if(this.isElementaryFlow()){
        ...
        this.syncPhoenixInlineButtonState();
        return;
      }
...
// ---------- Step 6 ----------
    ...
    isPhoenixHealthPolicy(policy){
      const company = safeTrim(policy?.company);
      const type = safeTrim(policy?.type);
      return company === "הפניקס" && type === "בריאות";
    },

    hasPhoenixHealthSelection(payloadOverride){
      const payload = payloadOverride || this.getOperationalPayload?.() || {};
      const direct = Array.isArray(payload?.newPolicies) ? payload.newPolicies : [];
      if(direct.some((policy) => this.isPhoenixHealthPolicy(policy))) return true;
      const insuredPolicies = Array.isArray(payload?.insureds)
        ? payload.insureds.flatMap((insured) => Array.isArray(insured?.data?.newPolicies) ? insured.data.newPolicies : [])
        : [];
      return insuredPolicies.some((policy) => this.isPhoenixHealthPolicy(policy));
    },

    getPhoenixHealthPdfFileName(payloadOverride){
      const payload = payloadOverride || this.getOperationalPayload?.() || {};
      const primary = payload?.primary || {};
      const name = [safeTrim(primary.firstName), safeTrim(primary.lastName)].filter(Boolean).join('_') || 'לקוח';
      const idNumber = safeTrim(primary.idNumber) || 'ללא-תז';
      return `טופס-הפניקס-בריאות-${name}-${idNumber}.pdf`.replace(/\s+/g, '-');
    },

    syncFinishFlowActionState(payloadOverride){
      const visible = this.hasPhoenixHealthSelection(payloadOverride);
      [this.els.btnDownloadPhoenixHealthPdf, this.els.btnDownloadPhoenixHealthPdfInline].forEach((btn) => {
        if(!btn) return;
        btn.style.display = visible ? '' : 'none';
        btn.disabled = !visible;
      });
      this.syncPhoenixInlineButtonState(payloadOverride);
    },

    syncPhoenixInlineButtonState(payloadOverride){
      const btn = this.els.btnDownloadPhoenixHealthPdfInline;
      if(!btn) return;
      const atFinishStep = Number(this.step) >= Number(this.steps?.length || 0);
      const visible = atFinishStep && this.hasPhoenixHealthSelection(payloadOverride);
      btn.style.display = visible ? '' : 'none';
      btn.disabled = !visible;
    },

    async exportPhoenixHealthPdf(payloadOverride, sourceBtn){
      const payload = payloadOverride || this.getOperationalPayload?.() || {};
      if(!this.hasPhoenixHealthSelection(payload)){
        window.alert('כפתור זה זמין רק כאשר נבחרה פוליסת הפניקס מסוג בריאות.');
        return;
      }

      const triggerBtn = sourceBtn || this.els.btnDownloadPhoenixHealthPdfInline || this.els.btnDownloadPhoenixHealthPdf || null;
      const originalText = triggerBtn ? triggerBtn.textContent : '';
      if(triggerBtn){
        triggerBtn.disabled = true;
        triggerBtn.textContent = 'מוריד טופס...';
      }
      ...
      try{
        ...
        console.error('exportPhoenixHealthPdf failed', err);
        window.alert('לא הצלחתי להוריד את טופס הפניקס כרגע. בדוק שקובץ ה-PDF קיים לצד המערכת ונסה שוב.');
      }finally{
        if(triggerBtn){
          triggerBtn.disabled = false;
    ...
    async finishWizard(){
      ...
      try{
        ...
        this.lastSavedOperationalPayload = saved?.payload || payload;
