...
// ---------- Step 6 ----------
    ...
    getHealthStoreForInsured(ins){
      ins = ins || this.insureds[0] || { data:{} };
      ins.data = ins.data || {};
      if(!ins.data.healthDeclaration || typeof ins.data.healthDeclaration !== 'object'){
        ins.data.healthDeclaration = { version:'matrix_v1', categories:{} };
      }
      const store = ins.data.healthDeclaration;
      if(!store.categories || typeof store.categories !== 'object') store.categories = {};
      if(!safeTrim(store.version)) store.version = 'matrix_v1';
      return store;
    },

    getHealthState(ins, catKey, qKey){
      const store = this.getHealthStoreForInsured(ins);
      store.categories[catKey] = store.categories[catKey] || {};
      if(!store.categories[catKey][qKey] || typeof store.categories[catKey][qKey] !== 'object'){
        store.categories[catKey][qKey] = { answer:'', fields:{}, saved:false, open:false };
      }
      const st = store.categories[catKey][qKey];
      if(!st.fields || typeof st.fields !== 'object') st.fields = {};
      if(typeof st.saved !== 'boolean') st.saved = false;
      if(typeof st.open !== 'boolean') st.open = false;
      return st;
    },

    setHealthAnswer(ins, catKey, qKey, ans){
      const st = this.getHealthState(ins, catKey, qKey);
      if(ans === 'no'){
        st.answer = 'no';
        st.fields = {};
        st.saved = false;
    ...
    clearHealthAnswer(ins, catKey, qKey){
      const st = this.getHealthState(ins, catKey, qKey);
      st.answer = '';
      st.fields = {};
      st.saved = false;
    ...
    saveHealthDetail(ins, catKey, qKey){
      const st = this.getHealthState(ins, catKey, qKey);
      st.answer = 'yes';
      st.saved = true;
      st.open = false;
    ...
    toggleHealthEdit(ins, catKey, qKey, open){
      const st = this.getHealthState(ins, catKey, qKey);
      st.open = !!open;
    },

    healthInsuredStatus(ins){
      const store = this.getHealthStoreForInsured(ins);
      let yesCount = 0, openCount = 0, savedCount = 0;
      Object.values(store.categories || {}).forEach(cat => {
        Object.values(cat || {}).forEach(st => {
          if(st && st.answer === 'yes'){
            yesCount += 1;
            if(st.saved) savedCount += 1;
            else openCount += 1;
          }
        });
      });
      if(openCount > 0) return { cls:'warn', text:'חסר פירוט', summary:`${openCount} פירוטים פתוחים` };
      if(yesCount > 0) return { cls:'ok', text:'יש ממצאים', summary:`${savedCount} ממצאים שמורים` };
      return { cls:'neutral', text:'ללא ממצאים', summary:'לא סומן כן בשאלות שנענו' };
    },
      ...
      return [
        {
          ...
          questions:[
            { key:'chronic_condition', text:'האם קיימות מחלות כרוניות / מצבים רפואיים מתמשכים?', companies: allCompanies, fields:[
              { key:'diagnosis', label:'מה האבחנה / המצב הרפואי?', type:'textarea' },
              { key:'diagnosedAt', label:'מועד אבחנה / תחילת הבעיה', type:'text' },
              { key:'currentStatus', label:'מה המצב כיום?', type:'textarea' },
              { key:'followup', label:'האם יש מעקב / מומחה / בדיקות קבועות?', type:'textarea' }
            ] },
            { key:'medications', text:'האם יש תרופות קבועות / טיפול פעיל?', companies: allCompanies, fields:[
              { key:'medicationNames', label:'שם התרופות / סוג הטיפול', type:'textarea' },
              { key:'reason', label:'בגלל איזו בעיה רפואית ניטל הטיפול?', type:'textarea' },
              { key:'sinceWhen', label:'ממתי הטיפול קיים?', type:'text' },
              { key:'stable', label:'האם המצב יציב כיום?', type:'text' }
            ] },
            { key:'surgery_hospitalization', text:'האם היו ניתוחים / אשפוזים / בירור משמעותי?', companies: allCompanies, fields:[
              { key:'procedureType', label:'איזה ניתוח / אשפוז / בירור בוצע?', type:'textarea' },
              { key:'reason', label:'מה הייתה הסיבה?', type:'textarea' },
              { key:'when', label:'מתי זה קרה?', type:'text' },
              { key:'result', label:'מה התוצאה / מצב כיום?', type:'textarea' }
            ] }
          ]
        },
        {
          ...
          companies: allCompanies,
          questions:[
            { key:'heart_bp', text:'האם קיימת בעיה בלב / לחץ דם / כלי דם?', companies: allCompanies, fields:[
              { key:'diagnosis', label:'איזו בעיה בלב / לחץ דם קיימת?', type:'textarea' },
              { key:'diagnosedAt', label:'מועד אבחנה', type:'text' },
              { key:'lastEvent', label:'מועד אירוע אחרון / צנתור / אשפוז', type:'text' },
        ...
        },
        {
          ...
          companies: allCompanies,
          questions:[
            { key:'respiratory', text:'האם קיימות בעיות ריאות / נשימה / דום נשימה?', companies: allCompanies, fields:[
              { key:'diagnosis', label:'איזו בעיית נשימה / ריאות קיימת?', type:'textarea' },
              { key:'diagnosedAt', label:'מועד אבחנה', type:'text' },
              { key:'frequency', label:'חומרה / תדירות התקפים / קוצר נשימה', type:'text' },
              { key:'treatment', label:'טיפול (משאף / CPAP / כדורים / אחר)', type:'textarea' },
              { key:'tests', label:'בדיקות / אשפוזים / מעקב', type:'textarea' }
            ] }
          ]
        },
        {
          ...
          companies: allCompanies,
          questions:[
            { key:'neuro_main', text:'האם קיימות בעיות נוירולוגיות / אפילפסיה / התעלפויות?', companies: allCompanies, fields:[
              { key:'diagnosis', label:'איזו בעיה נוירולוגית / אירועים היו?', type:'textarea' },
              { key:'diagnosedAt', label:'מועד אבחנה / אירוע ראשון', type:'text' },
              { key:'lastEvent', label:'מועד התקף / אירוע אחרון', type:'text' },
              { key:'frequency', label:'תדירות / מספר התקפים / אירועים', type:'text' },
              { key:'treatment', label:'טיפול / תרופות / מצב נוכחי', type:'textarea' }
            ] }
          ]
        },
        {
          ...
          companies: allCompanies,
          questions:[
            { key:'endocrine', text:'האם קיימת סוכרת / בלוטות / הורמונים / מצב מטבולי?', companies: allCompanies, fields:[
              { key:'diagnosis', label:'איזו אבחנה קיימת?', type:'textarea' },
              { key:'diagnosedAt', label:'מועד אבחנה', type:'text' },
              { key:'treatment', label:'טיפול / אינסולין / תרופות / מעקב', type:'textarea' },
              { key:'complications', label:'סיבוכים / בדיקות / מצב איזון', type:'textarea' }
            ] },
            { key:'cancer_tumor', text:'האם הייתה מחלה ממארת / גידול / ביופסיה / ממצא חריג?', companies: allCompanies, fields:[
              { key:'finding', label:'איזו מחלה / ממצא / גידול היה או ישנו?', type:'textarea' },
              { key:'diagnosedAt', label:'מועד אבחנה / גילוי', type:'text' },
              { key:'treatment', label:'טיפולים / ניתוח / הקרנות / כימו / מעקב', type:'textarea' },
              { key:'status', label:'מה המצב כיום? האם יש הישנות / מעקב?', type:'textarea' }
            ] },
            { key:'mental', text:'האם היו בעיות נפש / קשב / טיפול רגשי?', companies: allCompanies, fields:[
              { key:'diagnosis', label:'איזו אבחנה / סיבה לטיפול קיימת?', type:'textarea' },
              { key:'diagnosedAt', label:'מועד אבחנה / תחילת טיפול', type:'text' },
              { key:'treatment', label:'טיפול / תרופות / פסיכולוג / פסיכיאטר', type:'textarea' },
              { key:'status', label:'מה המצב כיום?', type:'textarea' }
            ] }
          ]
        },
        {
          ...
          companies: allCompanies,
          questions:[
            { key:'substances', text:'האם יש עישון / אלכוהול / סמים / קנאביס?', companies: allCompanies, fields:[
              { key:'type', label:'מה סוג העישון / חומר / אלכוהול?', type:'textarea' },
              { key:'frequency', label:'תדירות / כמות', type:'text' },
              { key:'sinceWhen', label:'ממתי / מתי הופסק?', type:'text' },
        ...
        },
        {
          ...
          companies: allCompanies,
          questions:[
            { key:'family_history', text:'האם קיימת היסטוריה משפחתית רפואית משמעותית?', companies: allCompanies, fields:[
              { key:'relative', label:'מי בן המשפחה?', type:'text' },
              { key:'diagnosis', label:'איזו מחלה אובחנה?', type:'textarea' },
              { key:'ageAtDx', label:'גיל אבחנה משוער', type:'text' },
        ...
        },
        {
          ...
          companies: allCompanies,
          questions:[
            { key:'musculoskeletal', text:'האם קיימות בעיות גב / שלד / מפרקים / נכות / תאונה?', companies: allCompanies, fields:[
              { key:'diagnosis', label:'איזו פגיעה / מחלה / בעיה קיימת?', type:'textarea' },
              { key:'location', label:'מיקום בגוף / צד הפגיעה', type:'text' },
              { key:'when', label:'מתי אובחן / קרה?', type:'text' },
              { key:'treatment', label:'טיפול / ניתוח / פיזיותרפיה / זריקות', type:'textarea' },
              { key:'limitation', label:'האם נשארה מגבלה / נכות / כאב כיום?', type:'textarea' }
            ] }
          ]
        },
        {
          ...
          femaleOnly:true,
          questions:[
            { key:'pregnancy', text:'האם יש היריון / לידה / סיבוך נשי רלוונטי?', companies: allCompanies, fields:[
              { key:'details', label:'פירוט', type:'textarea' },
              { key:'when', label:'מתי?', type:'text' },
              { key:'status', label:'מה המצב כיום?', type:'textarea' }
            ] }
          ]
        }
    ...
    renderHealthField(insId, catKey, qKey, field, values){
      const val = safeTrim(values?.[field.key] || '');
      if(field.type === 'textarea'){
        return `<div class="lcHField lcHField--full"><label class="lcHLabel">${escapeHtml(field.label)}</label><textarea class="lcHTextarea" rows="3" data-healthf="${escapeHtml(insId)}|${escapeHtml(catKey)}|${escapeHtml(qKey)}|${escapeHtml(field.key)}">${escapeHtml(val)}</textarea></div>`;
      }
      return `<div class="lcHField"><label class="lcHLabel">${escapeHtml(field.label)}</label><input class="lcHInput" type="text" value="${escapeHtml(val)}" data-healthf="${escapeHtml(insId)}|${escapeHtml(catKey)}|${escapeHtml(qKey)}|${escapeHtml(field.key)}" /></div>`;
    },

    renderHealthStatusBar(){
      const items = this.insureds.map(ins => {
        const st = this.healthInsuredStatus(ins);
        return `<div class="lcHPersonStatus ${st.cls}">
          <div class="lcHPersonStatus__title">${escapeHtml(ins.label)}</div>
          <div class="lcHPersonStatus__pill">${escapeHtml(st.text)}</div>
          <div class="lcHPersonStatus__sub">${escapeHtml(st.summary)}</div>
        </div>`;
      }).join('');
      return `<div class="lcHStatusWrap">${items}</div>`;
    },

    renderStep8(){
      const companies = this.getHealthCompanies();
      if(!companies.length){
        return `<div class="lcWSection"><div class="lcWTitle">הצהרת בריאות</div><div class="muted">כדי להציג את שלב 8 יש לבחור בשלב 5 לפחות פוליסה רלוונטית לחיתום.</div></div>`;
      }
      const chips = companies.map(c => `<span class="lcHChip lcHChip--top">${escapeHtml(c)}</span>`).join('');
      const categories = this.getHealthSchema().filter(cat => {
        if(cat.femaleOnly){
          return this.insureds.some(ins => safeTrim(ins?.data?.gender) === 'נקבה');
        }
        return cat.companies.some(c => companies.includes(c));
      });
      const catsHtml = categories.map(cat => {
        const qHtml = (cat.questions || []).map(q => {
          const matrixRows = this.insureds.map(ins => {
            if(cat.femaleOnly && safeTrim(ins?.data?.gender) !== 'נקבה') return '';
            const st = this.getHealthState(ins, cat.key, q.key);
            const yesClass = st.answer === 'yes' ? 'is-active' : '';
            const noClass = st.answer === 'no' ? 'is-active' : '';
            const detailOpen = st.answer === 'yes' && (st.open || !st.saved);
            const savedLine = st.answer === 'yes' && st.saved && !detailOpen ? `<div class="lcHInlineSaved"><span class="lcHInlineSaved__check">✓</span> נשמר עבור ${escapeHtml(ins.label)} <button type="button" class="lcHMiniBtn" data-healthedit="${escapeHtml(ins.id)}|${escapeHtml(cat.key)}|${escapeHtml(q.key)}">ערוך</button> <button type="button" class="lcHMiniBtn lcHMiniBtn--danger" data-healthclear="${escapeHtml(ins.id)}|${escapeHtml(cat.key)}|${escapeHtml(q.key)}">נקה</button></div>` : '';
            const detailHtml = detailOpen ? `<div class="lcHDetailCard">
              <div class="lcHDetailCard__title">פירוט עבור: ${escapeHtml(ins.label)}</div>
              <div class="lcHFields">${(q.fields || []).map(f => this.renderHealthField(ins.id, cat.key, q.key, f, st.fields || {})).join('')}</div>
              <div class="lcHDetailActions">
                <button type="button" class="btn btn--primary" data-healthsave="${escapeHtml(ins.id)}|${escapeHtml(cat.key)}|${escapeHtml(q.key)}">שמור</button>
                <button type="button" class="btn" data-healtheditclose="${escapeHtml(ins.id)}|${escapeHtml(cat.key)}|${escapeHtml(q.key)}">סגור</button>
              </div>
            </div>` : '';
            return `<div class="lcHMatrixRow">
              <div class="lcHMatrixRow__label">${escapeHtml(ins.label)}</div>
              <div class="lcHMatrixRow__controls">
                <button type="button" class="lcHToggle ${yesClass}" data-healthq="${escapeHtml(ins.id)}|${escapeHtml(cat.key)}|${escapeHtml(q.key)}|yes">כן</button>
                <button type="button" class="lcHToggle ${noClass}" data-healthq="${escapeHtml(ins.id)}|${escapeHtml(cat.key)}|${escapeHtml(q.key)}|no">לא</button>
              </div>
              <div class="lcHMatrixRow__content">${savedLine}${detailHtml}</div>
            </div>`;
          }).join('');
          return `<div class="lcHQuestionCard">
            <div class="lcHQuestionCard__head">
              <div>
                ...
                <div class="lcHCat__tags">${(q.companies || []).filter(c => companies.includes(c)).map(c => `<span class="lcHChip">${escapeHtml(c)}</span>`).join('')}</div>
              </div>
            </div>
            <div class="lcHMatrix">${matrixRows}</div>
          </div>`;
        }).join('');
        return `<div class="lcHCat">
          <div class="lcHCat__head">
            <div>
              <div class="lcHCat__title">${escapeHtml(cat.title)}</div>
              <div class="lcHCat__summary">הנציג שואל פעם אחת, מסמן כן / לא לכל מבוטח, וממלא פירוט רק למי שסומן כן.</div>
            </div>
          </div>
          <div class="lcHCat__body">${qHtml}</div>
        </div>`;
      }).join('');
      return `<div class="lcHealthWrap">
        <div class="lcWSection"><div class="lcWTitle">הצהרת בריאות</div><div class="muted small">חברות שנמצאו בשלב 5: ${chips}</div></div>
    ...
    bindHealthInputs(){
      $$('[data-healthq]', this.els.body).forEach(btn => {
        on(btn, 'click', () => {
          ...
          const [insId, catKey, qKey, ans] = raw.split('|');
          const ins = this._findInsuredById(insId);
          if(!ins || !catKey || !qKey || !ans) return;
          this.setHealthAnswer(ins, catKey, qKey, ans);
          this.render();
        });
      });
      ...
      $$('[data-healthf]', this.els.body).forEach(el => {
        const save = () => {
          ...
          const [insId, catKey, qKey, fieldKey] = raw.split('|');
          const ins = this._findInsuredById(insId);
          if(!ins || !catKey || !qKey || !fieldKey) return;
          const st = this.getHealthState(ins, catKey, qKey);
          st.fields[fieldKey] = safeTrim(el.value);
        };
        on(el, 'input', save);
        on(el, 'change', save);
      });
      ...
      $$('[data-healthsave]', this.els.body).forEach(btn => {
        on(btn, 'click', () => {
          ...
          const [insId, catKey, qKey] = raw.split('|');
          const ins = this._findInsuredById(insId);
          if(!ins || !catKey || !qKey) return;
          this.saveHealthDetail(ins, catKey, qKey);
          this.setHint('נשמר עבור ' + (ins.label || 'מבוטח'));
          this.render();
        });
      });

      $$('[data-healthedit]', this.els.body).forEach(btn => {
        on(btn, 'click', () => {
          const raw = btn.getAttribute('data-healthedit') || '';
          const [insId, catKey, qKey] = raw.split('|');
          const ins = this._findInsuredById(insId);
          if(!ins || !catKey || !qKey) return;
          this.toggleHealthEdit(ins, catKey, qKey, true);
          this.render();
        });
      });

      $$('[data-healtheditclose]', this.els.body).forEach(btn => {
        on(btn, 'click', () => {
          const raw = btn.getAttribute('data-healtheditclose') || '';
          const [insId, catKey, qKey] = raw.split('|');
          const ins = this._findInsuredById(insId);
          if(!ins || !catKey || !qKey) return;
          this.toggleHealthEdit(ins, catKey, qKey, false);
          this.render();
        });
      });

      $$('[data-healthclear]', this.els.body).forEach(btn => {
        on(btn, 'click', () => {
          const raw = btn.getAttribute('data-healthclear') || '';
          const [insId, catKey, qKey] = raw.split('|');
          const ins = this._findInsuredById(insId);
          if(!ins || !catKey || !qKey) return;
          this.clearHealthAnswer(ins, catKey, qKey);
          this.setHint('נוקה עבור ' + (ins.label || 'מבוטח'));
          this.render();
        });
      });
    },
