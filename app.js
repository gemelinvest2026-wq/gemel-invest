...
(() => {
  ...
  const BUILD = "20260329-elementary-proposal-edit-fix-v7";
  ...
  const OPS_RESULT_OPTIONS = {
    ...
    waitingAgentInfo: "ממתין להשלמת מידע מה"
  };
  ...
  function normalizeOpsEvent(ev, idx=0){
    ...
    return {
      ...
      createdByName: safeTrim(ev.createdByName) || "",
  ...
  function currentAgentIdentity(){
    ...
    return {
      ...
      name: safeTrim(found?.name) || currentName || '',
      ...
      root.innerHTML = `
        ...
        <div class="lcBirthdayOverlay__panel" role="dialog" aria-modal="true" aria-label="ברכת יום הולדת">
          ...
          <div class="lcBirthdayOverlay__content">
            ...
            <div class="lcBirthdayOverlay__name">${escapeHtml(agent.name || 'ל שלנו')}</div>
...
this.applyRoleUI();
  ...
  const UsersUI = {
    ...
    openModal(mode, user){
      ...
      if(E.title) E.title.textContent = (this._modalMode === "edit") ? "עריכת משתמש" : "הוסף /סוכן";
    ...
    render(){
      ...
      UI.els.usersTbody.innerHTML = rows.map(a => {
        ...
        const role = (a.role === "manager") ? "מנהל" : (a.role === "ops") ? "תפעול" : "";
        return `
          <tr>
  ...
  const CustomersUI = {
    ...
    render(){
      ...
      UI.els.customersTbody.innerHTML = rows.length ? rows.map(rec => {
        return `<tr class="lcCustomerRow">
          ...
          <td><div class="lcCustomers__agentCell"><span class="lcCustomers__agentLabel"></span><span class="lcCustomers__agentName">${escapeHtml(rec.agentName || "—")}</span></div></td>
          <td>
    ...
    renderHeroMeta(rec){
      ...
      const bottomRow = agentName ? `<div class="customerHero__metaInfo customerHero__metaInfo--agent">${agentIcon}<span class="customerHero__metaLabel"> מטפל</span><strong class="customerHero__metaValue">${escapeHtml(agentName)}</strong></div>` : '';
      return `
    ...
    open(id){
      ...
      if(this.els.meta){
        this.els.meta.innerHTML = [
          ...
          rec.agentName ? `: <strong>${escapeHtml(rec.agentName)}</strong>` : ""
      ...
      return `
        <section class="customerEditSection">
          ...
          <div class="customerEditGrid">
            ...
            ${!isExternal ? this.renderSelect("בחירת משלם","primary.selectedPayerId", this.field("primary.selectedPayerId"), payerOptions) : ``}
...
init(){
    ...
    renderCallBar(){
      ...
      if(this.els.callMeta) this.els.callMeta.textContent = `התחיל ב־${store.startTime || this.formatClock(store.startedAt)} · ${store.dateFull || this.formatFullDate(store.startedAt)} · ${safeTrim(store.startedBy) || ''}`;
    },
    ...
    getStep1Questions(ins){
      ...
      const questions = [
        ...
        },
        {
          ...
          sub:'כך  את הלקוח במערכת, בחיפוש ובתיק הלקוח.',
...
// ---------- Step 6 ----------
    ...
    renderHealthSummary(){
      ...
      return `<div class="lcHLayout"><div class="lcHMain"><div class="lcHFinishHero">
        ...
        <div class="lcHFinishHero__text">זהו מסך סיכום פנימי ל. הנתונים נשמרים על כל מבוטח בנפרד, יחד עם הממצאים שסומנו בשלב 8.</div>
    ...
    buildOperationalPdfMarkup(payloadOverride, options = {}){
      ...
      const currentAgentName = safeTrim(Auth?.current?.name) || safeTrim(payload?.agentName) || ' מטפל';
...
ההכנסה מעבודה שאתה צפוי להרוויח תעמוד על סך של ______ ₪ לחודש. ידוע לך כי בקרות מקרה הביטוח יוגבלו תגמולי הפיצוי החודשי להם תהיה זכאי לגובה שלא יעלה על 75% מהכנסתך מעבודה בפועל ב־12 החודשים שקדמו למועד קרות מקרה הביטוח, או בתקופה ממועד תחילת הביטוח ועד מועד קרות מקרה הביטוח, אם חלפו פחות מ־12 חודשים.`
  ...
  const ProcessesUI = {
    ...
    render(){
      ...
      UI.els.myProcessesTbody.innerHTML = rows.length ? rows.map(row => {
        ...
        const ownerRoleText = row.isMine ? "התהליך משויך אליך" : (rec.agentRole === "ops" ? "משתמש תפעול" : (rec.agentRole === "manager" ? "מנהל מערכת" : " מערכת"));
  ...
  const OpsEventsUI = {
          ...
          <div class="opsEventListModal__dialog" role="dialog" aria-modal="true" aria-labelledby="opsEventsListTitle">
            <div class="opsEventListModal__head">
              <div>
                ...
                <div class="opsEventModal__hint">כל י התפעול יכולים לצפות. ההתראה נשלחת רק ליוצר האירוע.</div>
              </div>
    ...
    renderList(){
          ...
          <article class="opsEventCard" data-event-id="${escapeHtml(ev.id)}">
            <div class="opsEventCard__head">
              <div>
                ...
                <div class="opsEventCard__sub">שיחת שיקוף · יוצר: ${escapeHtml(ev.createdByName || '')}</div>
              </div>
      ...
      }).join('') : `<div class="emptyState"><div class="emptyState__icon">🗓️</div><div class="emptyState__title">אין אירועים מתוזמנים</div><div class="emptyState__text">אחרי יצירת אירוע שיחת שיקוף הוא יופיע כאן לכל י התפעול.</div></div>`;
  ...
  const MirrorsUI = {
      ...
      this.els.results.innerHTML = this.lastResults.map(rec => {
        ...
        return `<button class="mirrorsResultCard${selected ? ' is-selected' : ''}" data-mirror-customer="${escapeHtml(rec.id)}" type="button">
          ...
          <div class="mirrorsResultCard__side">
            <span class="mirrorsResultCard__agent">${escapeHtml(rec.agentName || 'ללא ')}</span>
    ...
    renderCustomerHero(rec){
      ...
      this.els.customerHero.innerHTML = `<div class="mirrorsCustomerHero__main">
        <div>
          ...
          <div class="mirrorsCustomerHero__meta">ת״ז ${escapeHtml(rec.idNumber || primary.idNumber || '—')} · טלפון ${escapeHtml(rec.phone || primary.phone || '—')} ·  מטפל ${escapeHtml(rec.agentName || '—')}</div>
        </div>
    ...
    renderScript(rec){
      ...
      const opsName = safeTrim(Auth?.current?.name) || ' תפעול';
      const companies = this.getCompanyNames(rec);
      const companyText = companies.length ? companies.join(', ') : 'החברות שסומנו בהקמת הלקוח';
      const sellingAgent = safeTrim(rec.agentName) || 'ה המטפל';
      ...
      this.els.scriptCard.innerHTML = `<div class="mirrorsCard__head">
        ...
        <div class="mirrorsScriptBody">
          ...
          <p>ואני  מכירות מטעם סוכן גרגורי משווק הביטוחים של חברת <strong>${escapeHtml(companyText)}</strong>.</p>
          <p>אני יוצר איתך קשר בהמשך לפנייתך ולשיחתך עם ה <strong>${escapeHtml(sellingAgent)}</strong>.</p>
    ...
    renderReflection(rec){
      ...
      this.els.reflectCard.innerHTML = `<div class="mirrorsCard__head">
          <div>
            ...
            <div class="mirrorsCard__hint">מסך ההקראה הבא ל לאחר בחירת אופן קבלת הדיוורים. הפוליסות הקיימות מוצגות כאן כשורות מידע לקריאה בלבד, אחד־אחד, ולאחר מכן מוצגות הפוליסות החדשות שהמערכת מציעה למכירה.</div>
          </div>
    ...
    renderCancellation(rec){
      ...
      const agentName = safeTrim(rec?.agentName) || safeTrim(Auth?.current?.name) || 'ה המטפל';
      this.els.cancelCard.innerHTML = `<div class="mirrorsCard__head">
        ...
        ${cancelMode === 'agent' ? `<div class="mirrorsCancelAgentBox">
          <div class="mirrorsCancelAgentBox__label">ה המטפל בביטול</div>
    ...
    renderIssuanceStep(rec){
      ...
      this.els.issuanceCard.innerHTML = `<div class="mirrorsCard__head">
        ...
        <div class="mirrorsIssuanceOutro">
          <div class="mirrorsIssuanceOutro__title">המשך הקריאה של ה</div>
          ...
          <button class="mirrorsReadToggle${step.agentRead ? ' is-read' : ''}" data-mirror-issuance-read type="button">${step.agentRead ? '✓ ה סימן שהקריא את ההמשך' : 'סמן שה הקריא את ההמשך'}</button>
        </div>
        ${savedNote}
...
const SystemRepairUI = {
  ...
  const ChatUI = {
    ...
    refreshCurrentUser(){
      if(!Auth.current) return;
      const roleMap = { admin:"מנהל מערכת", manager:"מנהל", ops:" תפעול", agent:"" };
      ...
      this.currentUser = {
        ...
        role: roleMap[Auth.current?.role] || "",
    ...
    avatarMarkup(user, className, extraClass=""){
      ...
      const label = this.escapeHtml(this.initials(user?.name || ""));
    ...
    renderAvatarNode(el, user, className){
      ...
      if(url){
        ...
        el.textContent = this.initials(user?.name || "");
      }
    },
    ...
    roleLabel(raw){
      return ({ admin:'מנהל מערכת', manager:'מנהל', ops:' תפעול', agent:'' })[raw] || '';
    },
    ...
    renderUsers(){
      ...
      if(!users.length){
        wrap.innerHTML = '<div class="giChatSidebar__setupHint" style="display:block;margin:0 6px 8px;">אין כרגע ים זמינים להצגה.</div>';
        return;
      }
    ...
    renderMessages(){
      ...
      host.innerHTML = fresh.length ? fresh.map((msg) => {
        ...
        const user = mine ? this.currentUser : (this.usersMap.get(msg.fromId) || { id: msg.fromId, name: msg.fromName || '' });
        return `
            ...
            <div class="giChatMsg__row">
              ...
              <div class="giChatMsg__body">
                ...
                <div class="giChatMsg__meta">
                  <span>${this.escapeHtml(mine ? 'אתה' : (msg.fromName || ''))}</span>
    ...
    renderSettingsPreview(){
      ...
      const initials = this.escapeHtml(this.initials(this.currentUser?.name || ''));
    ...
    notifyIncoming(message){
      ...
      const from = this.usersMap.get(message.fromId)?.name || message.fromName || '';
      if(!isChatWindowOpen){
