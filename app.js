...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  function setSession_(user){
    try{ sessionStorage.setItem(AUTH_SESSION_KEY, JSON.stringify(user)); }catch(_){ }
  }
  ...
  function loginAttempt_(username, password){
    ...
    const found = AUTH_USERS.find(x => x.username === u);
    if(!found) return { ok:false, msg:'שם משתמש לא קיים' };
    if(p !== AUTH_PASSWORD) return { ok:false, msg:'סיסמה שגויה' };
    setSession_(found);
    setIdentity_(found);
    showApp_();
    if(location.hash){
  ...
  // Expose a simple logout (optional)
  ...
  (function bootAuth_(){
    ...
    const found = sess && AUTH_USERS.find(x => x.username === sess.username);
    if(found){
      setIdentity_(found);
      showApp_();
      if(!location.hash) setRoute('customers');
      else setRoute(location.hash.replace('#','').trim() || 'customers');
      return;
    }
