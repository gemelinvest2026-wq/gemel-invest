...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  function deleteUser_(username){
    ...
    if(!uName) return { ok:false, msg:'שם משתמש חסר' };

    // prevent deleting current logged-in user (avoid inconsistent session)
    if(state.currentUser && state.currentUser.username === uName){
      return { ok:false, msg:'לא ניתן למחוק משתמש מחובר. התנתק ונסה שוב.' };
    }
...
if(route === 'settings'){
      ...
      if(tab !== 'general' && tab !== 'users') tab = 'general';
...
function renderKpisSettings(){
  ...
  function renderUsers(){
              ...
              return `
                <tr>
                  ...
                  <td style="text-align:left; white-space:nowrap">
                    ...
                    <button class="btn btnTiny btnDanger" data-user-delete="${escapeAttr(u.username)}">מחק</button>
                  </td>
                </tr>
              `;
            }).join('')}
    ...
    $$('[data-user-delete]').forEach(b => b.addEventListener('click', () => openDeleteUserModal_(b.dataset.userDelete)));
  }
...
function openUserModal_(opt){
    ...
    overlay.innerHTML = `
      <div class="usersDialog">
        ...
        <div class="usersDialogFoot">
          <button class="btn btnSoft" data-cancel>ביטול</button>
          <button class="btn btnPrimary btnDanger" data-del style="${mode==='edit'?'':'display:none'}">מחק משתמש</button>
    ...
    overlay.querySelector('[data-del]')?.addEventListener('click', () => {
      ...
      const res0 = deleteUser_(existing?.username || '');
      if(!res0.ok){
        showErr(res0.msg || 'לא ניתן למחוק');
        return;
      }
  ...
  function openDeleteUserModal_(username){
    const u = getUsers_().find(x => x.username === username);
    if(!u) return;
    ...
    overlay.innerHTML = `
      <div class="usersDialog">
        <div class="usersDialogHead">
          <div>
            <div class="usersDialogTitle">מחיקת משתמש</div>
            <div class="usersDialogSub">${escapeHtml(u.displayName||'')} • ${escapeHtml(u.username||'')}</div>
          </div>
          <button class="btn btnSoft" data-x>סגור</button>
        </div>
        ...
        <div class="usersDialogBody">
          <div class="sideHint" style="margin:0">
            פעולה זו תמחק את המשתמש מהמכשיר (LocalStorage). ניתן ליצור אותו מחדש בכל רגע.
            <b>שימו לב:</b> אי אפשר למחוק את המנהל האחרון.
          </div>
          <div class="usersError hidden" id="udErr" role="alert" style="margin-top:10px"></div>
        </div>

        <div class="usersDialogFoot">
          <button class="btn btnSoft" data-cancel>ביטול</button>
          <button class="btn btnPrimary btnDanger" data-del>מחק משתמש</button>
        </div>
      </div>
    `;
    document.body.appendChild(overlay);
    ...
    const err = overlay.querySelector('#udErr');
    const showErr = (msg) => {
      if(!err) return;
      err.textContent = msg || 'שגיאה';
      err.classList.remove('hidden');
    ...
    overlay.querySelector('[data-del]')?.addEventListener('click', () => {
      if(err) err.classList.add('hidden');
      const res = deleteUser_(u.username);
      if(!res.ok){
        showErr(res.msg || 'לא ניתן למחוק');
        return;
      }
      toast('נמחק', 'המשתמש נמחק');
      close();
