...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  function loadCustomers_(){
    ...
    // seed current demo customers as initial data
    try{ localStorage.setItem(CUSTOMERS_LS_KEY, JSON.stringify(state.customers || [])); }catch(_){}
  }
  ...
  function openProcessModal_(customerId){
    ...
    const usersList = (loadUsers_ ? loadUsers_() : getUsers_ ? getUsers_() : [])
    ...
    overlay.querySelector('[data-save]')?.addEventListener('click', () => {
      ...
      // IMPORTANT: Forwarding a שיקוף should NOT switch the logged-in agent context.
      // If the user chose to forward the customer to another agent, we only reassign THIS customer.
      if(type === 'שיקוף'){
        c.assignedAgent = assignedTo;
        saveCustomers_();
      }
  ...
  function latestProposalForCustomer_(customerId){
    ...
    if(!cid) return null;
    const list = (state.proposals||[]).filter(p => p && String(p.customerId||'') === cid);
...
// Set your GOFI module URL (GitHub Pages) here when ready
  ...
  let route = 'customers';
  let tab = 'table';
  let focusCustomerId = null;

  function requestFocusCustomer_(cid){
    focusCustomerId = cid ? String(cid) : null;
  }
...
function renderKpisSettings(){
  ...
  function renderCustomersTable(rows){
    viewEl.innerHTML = `
      <div class="tableWrap">
        <table>
          ...
          <tbody>
            ${rows.map(c => `
              <tr id="custRow_${escapeAttr(c.id)}">
                ...
                <td style="text-align:left;white-space:nowrap">
                      ${(() => { const lp = latestProposalForCustomer_(c.id); const label = lp ? 'ערוך הצעה' : 'צור הצעה'; const pid = lp ? lp.id : ''; return `<button class="btn btnSoft" data-open-proposal="${c.id}" data-proposal-id="${escapeAttr(pid)}">${label}</button>`; })()}
    ...
    $$('[data-open-proposal]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        const pid = (btn.getAttribute('data-proposal-id') || '').trim();
        if(pid){
    ...
    // After saving a proposal, we may want to jump back and focus the customer row
    if(focusCustomerId){
      const rid = 'custRow_' + focusCustomerId;
      const rowEl = document.getElementById(rid);
      if(rowEl){
        try{ rowEl.scrollIntoView({ block:'center', behavior:'smooth' }); }catch(_){ rowEl.scrollIntoView(); }
        rowEl.classList.add('rowFocus');
        setTimeout(() => rowEl && rowEl.classList && rowEl.classList.remove('rowFocus'), 1600);
      }
  ...
  function openWizard({ customerId=null, proposalId=null }){
    ...
    if(!proposal){
      ...
      if(customer && !customer.assignedAgent){ customer.assignedAgent = state.agentName; saveCustomers_(); }
      proposal = {
...
`;
  ...
  function renderSummaryStep(){
    ...
    if(markDoneBtnEl) markDoneBtnEl.addEventListener('click', () => {
      ...
      // UX: after saving, return to "לקוחות" and focus the customer row
      requestFocusCustomer_(p.customerId);
      closeWizard();
      setRoute('customers');
    });
  }
