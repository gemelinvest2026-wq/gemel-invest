...
this.applyRoleUI();
  ...
  const CustomersUI = {
    ...
    init(){
      ...
      this.assignModal = {
        wrap: $("#customerAssignModal"),
        backdrop: $("#customerAssignModalBackdrop"),
        close: $("#customerAssignModalClose"),
        cancel: $("#customerAssignCancel"),
        primary: $("#customerAssignPrimary"),
        codeStep: $("#customerAssignCodeStep"),
        selectStep: $("#customerAssignSelectStep"),
        codeInput: $("#customerAssignCodeInput"),
        target: $("#customerAssignTarget"),
        error: $("#customerAssignError")
      };
      ...
      on(this.els.assignBtn, "click", (ev) => {
        ev?.preventDefault?.();
        this.openAssignModal();
      });
      on(this.assignModal?.close, "click", () => this.closeAssignModal());
      on(this.assignModal?.cancel, "click", () => this.closeAssignModal());
      on(this.assignModal?.backdrop, "click", () => this.closeAssignModal());
      on(this.assignModal?.primary, "click", async () => {
        if(this._assignStep === 'select') await this.confirmAssignCustomer();
        else this.submitAssignCode();
      });
      on(this.assignModal?.codeInput, "input", () => {
        if(this.assignModal?.error) this.assignModal.error.textContent = "";
      });
      on(this.assignModal?.target, "change", () => {
        if(this.assignModal?.error) this.assignModal.error.textContent = "";
      });
    ...
    openById(id, opts={}){
      ...
      try{
        ...
        this.refreshAssignButtonVisibility();
        if(this.els.dash){
    ...
    refreshAssignButtonVisibility(){
      if(!this.els.assignBtn) return;
      const shouldShow = !!(this.currentId && this.canAssignCustomer());
      this.els.assignBtn.hidden = !shouldShow;
    },

    resetAssignModal(){
      this._assignStep = 'code';
      if(this.assignModal?.codeStep) this.assignModal.codeStep.hidden = false;
      if(this.assignModal?.selectStep) this.assignModal.selectStep.hidden = true;
      if(this.assignModal?.codeInput) this.assignModal.codeInput.value = '';
      if(this.assignModal?.target) this.assignModal.target.value = '';
      if(this.assignModal?.error) this.assignModal.error.textContent = '';
      if(this.assignModal?.primary) this.assignModal.primary.textContent = 'המשך';
    },

    getAssignableUsers(){
      const users = Array.isArray(State.data?.agents) ? State.data.agents : [];
      return users
        .filter((user) => user && user.active !== false)
        .map((user) => ({
          id: safeTrim(user.id),
          name: safeTrim(user.name) || safeTrim(user.username) || 'משתמש',
          role: safeTrim(user.role) || 'agent'
        }))
        .filter((user) => user.id && user.name);
    },

    fillAssignTargets(){
      if(!this.assignModal?.target) return;
      const roleMap = {admin:'מנהל מערכת', manager:'מנהל', ops:'נציג תפעול', agent:'נציג'};
      const options = this.getAssignableUsers();
      this.assignModal.target.innerHTML = [
        '<option value="">בחר משתמש לשיוך</option>',
        ...options.map((user) => `<option value="${escapeHtml(user.id)}">${escapeHtml(user.name)} · ${escapeHtml(roleMap[user.role] || 'נציג')}</option>`)
      ].join('');
    },

    openAssignModal(){
      if(!this.canAssignCustomer() || !this.current()) return;
      this.fillAssignTargets();
      this.resetAssignModal();
      if(!this.assignModal?.wrap) return;
      this.assignModal.wrap.classList.add('is-open');
      this.assignModal.wrap.setAttribute('aria-hidden', 'false');
      document.body.style.overflow = 'hidden';
      window.setTimeout(() => this.assignModal?.codeInput?.focus?.(), 40);
    },

    closeAssignModal(silent=false){
      if(!this.assignModal?.wrap) return;
      this.assignModal.wrap.classList.remove('is-open');
      this.assignModal.wrap.setAttribute('aria-hidden', 'true');
      this.resetAssignModal();
      if(!silent && this.els.wrap?.classList.contains('is-open')) document.body.style.overflow = 'hidden';
    },

    submitAssignCode(){
      const code = safeTrim(this.assignModal?.codeInput?.value);
      if(code !== '2002'){
        if(this.assignModal?.error) this.assignModal.error.textContent = 'קוד מנהל שגוי';
        this.assignModal?.codeInput?.focus?.();
        return;
      }
      this._assignStep = 'select';
      if(this.assignModal?.codeStep) this.assignModal.codeStep.hidden = true;
      if(this.assignModal?.selectStep) this.assignModal.selectStep.hidden = false;
      if(this.assignModal?.primary) this.assignModal.primary.textContent = 'שייך לקוח';
      if(this.assignModal?.error) this.assignModal.error.textContent = '';
      window.setTimeout(() => this.assignModal?.target?.focus?.(), 40);
    },

    async confirmAssignCustomer(){
      const rec = this.current();
      const targetId = safeTrim(this.assignModal?.target?.value);
      if(!rec) return this.closeAssignModal();
      if(!targetId){
        if(this.assignModal?.error) this.assignModal.error.textContent = 'יש לבחור משתמש';
        this.assignModal?.target?.focus?.();
        return;
      }
      const target = this.getAssignableUsers().find((user) => String(user.id) === String(targetId));
      if(!target){
        if(this.assignModal?.error) this.assignModal.error.textContent = 'המשתמש שנבחר לא נמצא';
        return;
      }
      rec.agentName = target.name;
      rec.agentRole = target.role;
      rec.updatedAt = nowISO();
      if(rec.payload && typeof rec.payload === 'object'){
        rec.payload.agentName = target.name;
        rec.payload.agentRole = target.role;
      }
      const result = await App.persist('לקוח שויך בהצלחה');
      if(!result?.ok){
        if(this.assignModal?.error) this.assignModal.error.textContent = 'אירעה שגיאה בשמירה. נסה שוב.';
        return;
      }
      this.closeAssignModal();
      this.render();
      this.openById(rec.id, { section: this.currentSection || 'wallet', skipLoader: true });
    },
