...
// ---------- Step 6 ----------
    ...
    getHealthCompanies(){
      ...
      (this.newPolicies || []).forEach(p => {
        ...
        if(!c || this.isMedicareCompany(c)) return;
        found.add(c);
      });
    ...
    getHealthStore(){
      ...
      if(!primary.data.healthDeclaration || typeof primary.data.healthDeclaration !== 'object'){
        primary.data.healthDeclaration = { version:'smart_v2', answers:{} };
      }
      const store = primary.data.healthDeclaration;
      if(!store.answers || typeof store.answers !== 'object') store.answers = {};
      if(!safeTrim(store.version)) store.version = 'smart_v2';
      return store;
    },
    ...
    getHealthContext(){
      const primary = this.insureds[0] || { data:{} };
      const gender = safeTrim(primary?.data?.gender);
      return {
        companies: this.getHealthCompanies(),
        products: this.getHealthProducts(),
        gender,
        hasFemalePrimary: (gender === 'נקבה'),
        hasHealthLikeProduct: (this.getHealthProducts().some(t => ['בריאות','מחלות קשות','סרטן','תאונות אישיות','ריסק','ריסק משכנתא'].includes(t)))
      };
    },

    getHealthPrimaryData(){
      const primary = this.insureds[0] || { data:{} };
      const d = primary?.data || {};
      const h = Number(d.heightCm || 0);
      const w = Number(d.weightKg || 0);
      const bmi = Number(d.bmi || 0) || 0;
      return {
        age: this.getAgeFromBirthDate ? this.getAgeFromBirthDate(d.birthDate) : null,
        heightCm: h || null,
        weightKg: w || null,
        bmi: bmi || null,
    ...
    getUnderwritingConditionMap(){
      const allCompanies = ['איילון','הראל','כלל','מגדל','מנורה','הפניקס','הכשרה'];
      const mk = (category, names, score, questionnaire) => names.map((name, idx) => ({
        id: `${category}_${idx+1}`,
        category,
        name,
        score,
      ...
      return [
        ...mk('heart', ['יתר לחץ דם','לחץ דם גבולי','כולסטרול גבוה','הפרעת קצב','פלפיטציות','איוושה','מחלת לב איסכמית','תעוקת חזה','התקף לב','צנתור','ניתוח מעקפים','אי ספיקת לב','קרדיומיופתיה','מחלת מסתמים','מום לב מולד ASD','מום לב מולד VSD','PFO','מפרצת','מחלת כלי דם פריפריאלית','קרישיות יתר'], 3, 'שאלון לב וכלי דם'),
        ...mk('resp', ['אסתמה','COPD','אמפיזמה','ברונכיטיס כרונית','סרקואידוזיס','סיסטיק פיברוזיס','פנאומוטורקס','דום נשימה בשינה','סינוסיטיס כרונית','קוצר נשימה במאמץ','תסחיף ריאתי','שחפת','דלקת ריאות חוזרת','מעקב רופא ריאות','שימוש ב-CPAP'], 3, 'שאלון ריאות ונשימה'),
        ...mk('neuro', ['אפילפסיה','פרכוסים','התעלפויות','סחרחורות','כאב ראש חדש','מיגרנה','TIA','שבץ מוחי','דימום מוחי','טרשת נפוצה','פרקינסון','נוירופתיה','מיוזטניה גרביס','פוליו','חבלת ראש','גידול במוח','מנינגיטיס','אנצפליטיס'], 4, 'שאלון נוירולוגי'),
        ...mk('mental', ['חרדה','דיכאון','PTSD','OCD','הפרעת הסתגלות','הפרעת אכילה','ביפולר','סכיזופרניה','טיפול פסיכולוגי ממושך','טיפול פסיכיאטרי','אשפוז פסיכיאטרי','ניסיון אובדני'], 4, 'שאלון בריאות נפש'),
        ...mk('endo', ['סוכרת סוג 1','סוכרת סוג 2','טרום סוכרת','תת פעילות בלוטת התריס','יתר פעילות בלוטת התריס','השימוטו','גויטר','קשרית בבלוטת התריס','אדנומה','תסמונת מטבולית','השמנת יתר','שומנים גבוהים','שיגדון','הפרעת הורמונים'], 3, 'שאלון סוכרת / הורמונלי'),
        ...mk('cancer', ['גידול שפיר','ביופסיה חריגה','פוליפ במעי','סרטן שד','סרטן מעי גס','סרטן ערמונית','סרטן עור','סרטן בלוטת התריס','לימפומה','לוקמיה','מלנומה','טיפול כימותרפי','טיפול קרינתי','מעקב אונקולוגי'], 6, 'שאלון אונקולוגי'),
        ...mk('gastro', ['רפלוקס כרוני','כיב קיבה','דימום במערכת העיכול','קרוהן','קוליטיס','צליאק','כבד שומני','הפטיטיס','שחמת','אבני מרה','דלקת לבלב','מחלת מעי כרונית','פוליפים במעי','בקע סרעפתי','ניתוח בריאטרי'], 3, 'שאלון מערכת עיכול'),
        ...mk('renal', ['אבנים בכליות','אי ספיקת כליות','מחלת כליות כרונית','חלבון בשתן','דם בשתן','דלקות שתן חוזרות','כליה פוליציסטית','הגדלת ערמונית','PSA חריג','דיאליזה'], 4, 'שאלון כליות ודרכי שתן'),
        ...mk('ortho', ['כאב גב כרוני','בלט דיסק','פריצת דיסק','ספינל סטנוזיס','ספונדילוליסטזיס','עקמת','שבר חוליה','שבר כתף','שבר ברך','שבר ירך','קרע ברצועה','קרע מניסקוס','קרע בגיד','כתף קפואה','אוסטיאוארטריטיס','אוסטיאופורוזיס','RA','אנקילוזינג ספונדיליטיס','FMF','פיברומיאלגיה'], 2, 'שאלון שלד / גב / מפרקים'),
        ...mk('immune', ['לופוס','פסוריאזיס','דלקת מפרקים שגרונית','קולגן','וסקוליטיס','טרומבוציטופניה','אנמיה משמעותית','הפרעת קרישה','חסר חיסוני','מחלה אוטואימונית אחרת'], 4, 'שאלון דם / חיסון / אוטואימוני'),
        ...mk('substances', ['עישון נוכחי','עישון לשעבר','סיגריה אלקטרונית','אלכוהול מופרז','גמילה מאלכוהול','קנאביס רפואי','קנאביס לא רפואי','קוקאין','אופיאטים','הרואין','אקסטזי','גמילה מסמים'], 3, 'שאלון עישון / אלכוהול / סמים')
      ];
    },

    getMedicalTriggersTable(){
      return [
        { id:'bmi30', title:'BMI גבוה', when:'BMI ≥ 30', score:2, questionnaire:'שאלון משקל / מטבולי', severity:'medium' },
        { id:'bmi35', title:'BMI קיצוני', when:'BMI ≥ 35', score:3, questionnaire:'שאלון משקל מורחב', severity:'high' },
        { id:'heart', title:'טריגר לב', when:'תשובת כן בלב / לחץ דם / כלי דם', score:4, questionnaire:'שאלון לב', severity:'high' },
        { id:'resp', title:'טריגר ריאות', when:'תשובת כן בריאות / נשימה', score:3, questionnaire:'שאלון ריאות', severity:'medium' },
        { id:'neuro', title:'טריגר נוירולוגי', when:'תשובת כן נוירולוגיה / מוח', score:4, questionnaire:'שאלון נוירולוגי', severity:'high' },
        { id:'mental', title:'טריגר נפשי', when:'תשובת כן בריאות נפש', score:4, questionnaire:'שאלון בריאות נפש', severity:'high' },
        { id:'endo', title:'טריגר סוכרת / הורמונים', when:'תשובת כן סוכרת / בלוטות / מטבולי', score:3, questionnaire:'שאלון סוכרת / בלוטת תריס', severity:'medium' },
        { id:'cancer', title:'טריגר אונקולוגי', when:'תשובת כן סרטן / גידול / ביופסיה', score:6, questionnaire:'שאלון אונקולוגי', severity:'high' },
        { id:'ortho', title:'טריגר אורטופדי', when:'תשובת כן שלד / גב / מפרקים', score:2, questionnaire:'שאלון שלד / גב / מפרקים', severity:'medium' },
        { id:'substances', title:'טריגר התמכרויות', when:'תשובת כן עישון / אלכוהול / סמים', score:3, questionnaire:'שאלון עישון / אלכוהול / סמים', severity:'medium' },
        { id:'family', title:'טריגר משפחתי', when:'תשובת כן רקע משפחתי', score:2, questionnaire:'שאלון היסטוריה משפחתית', severity:'medium' },
        { id:'surgery', title:'טריגר ניתוחים / אשפוזים', when:'תשובת כן ניתוח / אשפוז / בירור', score:2, questionnaire:'שאלון ניתוחים / אשפוזים', severity:'medium' }
      ];
    },

    getHealthQuestionnaireMap(){
      return {
        heart_bp: 'שאלון לב וכלי דם',
        respiratory: 'שאלון ריאות ונשימה',
        neuro: 'שאלון נוירולוגי',
        mental: 'שאלון בריאות נפש',
        endocrine: 'שאלון סוכרת / בלוטות / מטבולי',
        cancer_tumor: 'שאלון אונקולוגי',
        musculoskeletal: 'שאלון שלד / גב / מפרקים',
        substances: 'שאלון עישון / אלכוהול / סמים',
        family_history: 'שאלון היסטוריה משפחתית',
        surgery_hospitalization: 'שאלון ניתוחים / אשפוזים',
        chronic_condition: 'שאלון מצב רפואי כללי',
        medications: 'שאלון תרופות קבועות',
        pregnancy: 'שאלון היריון / נשים',
        endocrine: 'שאלון סוכרת / בלוטות / מטבולי',
        cancer_tumor: 'שאלון אונקולוגי'
      };
    },

    getUnderwritingEngine(context){
      const ctx = context || this.getHealthContext();
      const store = this.getHealthStore();
      const pd = this.getHealthPrimaryData();
      const questions = this.getRelevantHealthQuestions(ctx);
      const triggers = [];
      const conditionMap = this.getUnderwritingConditionMap();
      const questionnaireMap = this.getHealthQuestionnaireMap();
      const conditionHits = [];
      let score = 0;

      const addTrigger = (id, title, reason, points, questionnaire, severity='medium') => {
        const key = `${id}|${questionnaire}`;
        if(triggers.some(t => t.key === key)) return;
        score += Number(points || 0);
        triggers.push({ key, id, title, reason, points:Number(points||0), questionnaire, severity });
      };

      if(pd.bmi && pd.bmi >= 30) addTrigger('bmi30','BMI גבוה', `BMI ${pd.bmi}`, pd.bmi >= 35 ? 3 : 2, pd.bmi >= 35 ? 'שאלון משקל מורחב' : 'שאלון משקל / מטבולי', pd.bmi >= 35 ? 'high' : 'medium');

      questions.forEach(q => {
        const ans = store.answers?.[q.key] || { answer:'', fields:{} };
        if(ans.answer !== 'yes') return;
        const qText = Object.values(ans.fields || {}).join(' | ').toLowerCase();
        const questionnaire = questionnaireMap[q.key] || 'שאלון רפואי משלים';
        let pts = 2;
        let sev = 'medium';
        if(['heart_bp','neuro','mental'].includes(q.key)) { pts = 4; sev = 'high'; }
        if(['cancer_tumor'].includes(q.key)) { pts = 6; sev = 'high'; }
        if(['respiratory','endocrine','substances'].includes(q.key)) { pts = 3; sev = 'medium'; }
        addTrigger(q.key, q.title, `תשובת כן בשאלה הראשית`, pts, questionnaire, sev);

        conditionMap.forEach(c => {
          const n = String(c.name || '').toLowerCase();
          if(n && qText.includes(n) && !conditionHits.some(x => x.name === c.name)) {
            conditionHits.push(c);
          }
        });
      });
      ...
      const hasLife = products.some(p => ['ריסק','ריסק משכנתא','חיים + משכנתא','חיים'].includes(p));
      const hasCritical = products.some(p => ['מחלות קשות','סרטן'].includes(p));
      const hasHealth = products.some(p => ['בריאות','תאונות אישיות'].includes(p));

      Object.keys(companyBase).forEach(c => {
        let adj = 0;
        if(hasLife && ['מגדל','כלל'].includes(c)) adj += 1;
        if(hasHealth && ['הפניקס','מנורה'].includes(c)) adj -= 1;
        if(hasCritical && ['הראל','מגדל'].includes(c)) adj += 1;
        if(triggers.some(t => t.id === 'cancer_tumor')) adj += 2;
        if(triggers.some(t => t.id === 'heart_bp')) adj += (c === 'מנורה' ? 0 : 1);
        if(triggers.some(t => t.id === 'substances')) adj += (c === 'כלל' ? 1 : 0);
        if(pd.bmi && pd.bmi >= 35) adj += (['הראל','מגדל'].includes(c) ? 2 : 1);
        companyBase[c] = score + adj;
      });

      let status = 'green';
      let statusText = '🟢 סיכוי אישור גבוה';
      if(score >= 10){ status = 'red'; statusText = '🔴 סיכון גבוה להחרגה / דחייה'; }
      else if(score >= 5){ status = 'yellow'; statusText = '🟡 צפוי בירור / החרגה / תוספת פרמיה'; }

      const recommendations = Object.entries(companyBase)
        .filter(([name]) => ctx.companies.includes(name))
        .map(([name, s]) => ({ name, score:s, state: s >= 12 ? 'red' : s >= 7 ? 'yellow' : 'green' }))
      ...
      return {
        ...
        triggers,
        conditionHits,
        conditionCount: conditionMap.length,
        recommendations,
        bmi: pd.bmi,
        age: pd.age,
    ...
    renderUnderwritingSummary(context){
      const engine = this.getUnderwritingEngine(context);
      const bmiHtml = engine.bmi ? `<div class="lcUSStat"><div class="lcUSStat__k">BMI משלב 2</div><div class="lcUSStat__v">${escapeHtml(String(engine.bmi))}</div></div>` : '';
      const ageHtml = engine.age ? `<div class="lcUSStat"><div class="lcUSStat__k">גיל</div><div class="lcUSStat__v">${escapeHtml(String(engine.age))}</div></div>` : '';
      const scoreHtml = `<div class="lcUSStat"><div class="lcUSStat__k">ציון חיתום פנימי</div><div class="lcUSStat__v">${escapeHtml(String(engine.score))}</div></div>`;
      const triggers = engine.triggers.length
        ? engine.triggers.map(t => `<div class="lcTrig"><div><div class="lcTrig__t">${escapeHtml(t.title)}</div><div class="lcTrig__s">${escapeHtml(t.reason)} · ${escapeHtml(t.questionnaire)}</div></div><div class="lcTrig__p">+${escapeHtml(String(t.points))}</div></div>`).join('')
        : '<div class="muted">עדיין לא הופעלו טריגרים רפואיים. לאחר תשובות כן, המערכת תציג שאלונים, ציון חיתום והמלצת שליחה.</div>';
      const recs = engine.recommendations.length
        ? engine.recommendations.map(r => `<div class="lcRec lcRec--${escapeHtml(r.state)}"><span>${escapeHtml(r.name)}</span><strong>${r.state==='green'?'עדיף לשליחה':r.state==='yellow'?'בירור נוסף':'רגיש לחיתום'}</strong></div>`).join('')
        : '<div class="muted">אין עדיין חברות פעילות בתיק.</div>';
      return `
        <div class="lcUSWrap">
          <div class="lcUSHead">
            <div>
              <div class="lcUSTitle">מנוע חיתום אוטומטי לסוכן</div>
              <div class="lcUSSub">מבוסס על הצהרת הבריאות הראשית + שלב 2 (גובה/משקל) + טבלת טריגרים רפואיים + מפת חיתום פנימית של ${escapeHtml(String(engine.conditionCount))} מצבים רפואיים.</div>
            </div>
            <div class="lcUSBadge lcUSBadge--${escapeHtml(engine.status)}">${escapeHtml(engine.statusText)}</div>
          </div>
          <div class="lcUSStats">${scoreHtml}${bmiHtml}${ageHtml}</div>
          <div class="lcUSGrid">
            <div class="lcUSCard">
              <div class="lcUSCard__title">טריגרים רפואיים פעילים</div>
              ${triggers}
            </div>
            <div class="lcUSCard">
              <div class="lcUSCard__title">חברות מומלצות כרגע לשליחה</div>
              <div class="lcRecList">${recs}</div>
            </div>
          </div>
        </div>`;
    },
      ...
      return [
        {
          ...
          title:'מחלות כרוניות / מצבים רפואיים מתמשכים',
          summary:'שאלה חכמה מאוחדת שמכסה מצבים רפואיים פעילים או קודמים שיכולים להשפיע על חיתום בכל החברות.',
          companies: allCompanies,
          docHints: {
            'איילון':'בחלק מהמצבים איילון דורשת לצרף סיכום רופא מטפל או מסמכים עדכניים.',
            'הפניקס':'במצבים מהותיים הפניקס מבקשת תיעוד רפואי מעודכן לפי התחום.',
            'מנורה':'שאלוני ההרחבה של מנורה דורשים פירוט מלא וחתימה בכל עמוד רלוונטי.',
            'כלל':'במענה חיובי בכלל נדרש להשלים את כל שאלות המשנה של אותו נושא.'
          },
          fields:[
            { key:'diagnosis', label:'מה האבחנה / המצב הרפואי?', type:'textarea', required:true },
            { key:'diagnosedAt', label:'מועד אבחנה / תחילת הבעיה', type:'text', required:true },
            { key:'lastEpisode', label:'מועד אירוע / החמרה אחרונה', type:'text' },
            { key:'currentStatus', label:'מה המצב כיום?', type:'textarea', required:true },
            { key:'followup', label:'האם יש מעקב / מומחה / בדיקות קבועות?', type:'textarea' }
          ]
        },
        {
          key:'medications',
          title:'תרופות קבועות / טיפול פעיל',
          summary:'שאלה מרוכזת על תרופות, משאפים, זריקות, טיפול ביולוגי או טיפול קבוע אחר.',
          companies: allCompanies,
          fields:[
            { key:'medicationNames', label:'שם התרופות / סוג הטיפול', type:'textarea', required:true },
            { key:'reason', label:'בגלל איזו בעיה רפואית ניטל הטיפול?', type:'textarea', required:true },
            { key:'dosage', label:'מינון / תדירות', type:'text' },
            { key:'sinceWhen', label:'ממתי הטיפול קיים?', type:'text', required:true },
            { key:'stable', label:'האם המצב יציב כיום?', type:'select', options:['','כן','לא','לא ידוע'], required:true }
          ]
        },
        {
          key:'surgery_hospitalization',
          title:'ניתוחים / אשפוזים / בירור משמעותי',
          summary:'מכסה ניתוחים, אשפוזים, מיון, צנתורים, בדיקות חריגות, ביופסיות והמשך בירור.',
          companies: allCompanies,
          docHints: {
            'איילון':'איילון מדגישה צירוף מסמכים נלווים כאשר יש ניתוחים או בדיקות רלוונטיות.',
            'הפניקס':'במקרים רפואיים משמעותיים הפניקס מבקשת תיעוד רפואי עדכני.',
            'מגדל':'בשאלונים של מגדל יש דרישה לפירוט מצב רפואי, תאריכים וסיכומי מומחה.'
          },
          fields:[
            { key:'procedureType', label:'איזה ניתוח / אשפוז / בירור בוצע?', type:'textarea', required:true },
            { key:'reason', label:'מה הייתה הסיבה?', type:'textarea', required:true },
            { key:'when', label:'מתי זה קרה?', type:'text', required:true },
            { key:'result', label:'מה התוצאה / מצב כיום?', type:'textarea', required:true },
            { key:'documents', label:'אילו מסמכים קיימים? (סיכום אשפוז / ניתוח / בדיקה)', type:'textarea' }
          ]
        },
        {
          key:'heart_bp',
          title:'לב / לחץ דם / כלי דם',
          summary:'שאלה אחת מאוחדת ללב, לחץ דם, הפרעות קצב, צנתור, אוטם, מסתמים וכלי דם.',
          companies: allCompanies,
          docHints: {
            'מנורה':'בשאלון לב מנורה מפורטים סוגי מחלות לב, מסתמים ומומי לב מולדים.',
            'כלל':'בכלל יש שאלון מפורט למערכת הלב וכלי הדם עם שאלות משנה מלאות.',
            'הפניקס':'הפניקס דורשת פירוט לפי אבחנה ומצב רפואי עדכני.'
          },
          fields:[
            { key:'diagnosis', label:'איזו בעיה בלב / לחץ דם קיימת?', type:'textarea', required:true },
            { key:'diagnosedAt', label:'מועד אבחנה', type:'text', required:true },
            { key:'lastEvent', label:'מועד אירוע אחרון / החמרה / צנתור / אשפוז', type:'text' },
            { key:'treatment', label:'טיפול / תרופות / צנתור / ניתוח', type:'textarea', required:true },
            { key:'status', label:'מה המצב כיום? כולל ערכי לחץ דם אם ידועים', type:'textarea', required:true }
          ]
        },
        {
          key:'respiratory',
          title:'ריאות / נשימה / דום נשימה',
          summary:'שאלה חכמה לאסטמה, COPD, ברונכיטיס כרונית, סרקואידוזיס, דום נשימה ובעיות נשימה נוספות.',
          companies: allCompanies,
          docHints: {
            'איילון':'באיילון נדרש לעיתים סיכום רופא ריאות ותפקודי ריאות עדכניים.',
            'הפניקס':'הפניקס דורשת תיעוד רפואי ותפקודי ריאות ב-COPD ובאשפוזים.',
            'מגדל':'מגדל כוללת שאלון מפורט למערכת הנשימה עם פירוט טיפולים ואירועים.'
          },
          fields:[
            { key:'diagnosis', label:'איזו בעיית נשימה / ריאות קיימת?', type:'textarea', required:true },
            { key:'diagnosedAt', label:'מועד אבחנה', type:'text', required:true },
            { key:'frequency', label:'חומרה / תדירות התקפים / קוצר נשימה', type:'text', required:true },
            { key:'treatment', label:'טיפול (משאף / CPAP / כדורים / אחר)', type:'textarea', required:true },
            { key:'tests', label:'בדיקות / אשפוזים / מעקב רופא ריאות', type:'textarea' }
          ]
        },
        {
          key:'neuro',
          title:'נוירולוגיה / מוח / אפילפסיה / התעלפויות',
          summary:'מאחד אפילפסיה, אירוע מוחי, חבלת ראש, טרשת, רעד, סחרחורות והתעלפויות.',
          companies: allCompanies,
          docHints: {
            'איילון':'לאפילפסיה באיילון נדרש פירוט על מספר התקפים וטיפול תרופתי.',
            'מנורה':'במנורה קיים שאלון ייעודי לאפילפסיה עם תדירות התקפים ושימוש בתרופות.',
            'כלל':'בכלל קיים שאלון מורחב למערכת העצבים עם הרבה שאלות משנה.'
          },
          fields:[
            { key:'diagnosis', label:'איזו בעיה נוירולוגית / אירועים היו?', type:'textarea', required:true },
            { key:'diagnosedAt', label:'מועד אבחנה / אירוע ראשון', type:'text', required:true },
            { key:'lastEvent', label:'מועד התקף / אירוע אחרון', type:'text' },
            { key:'frequency', label:'תדירות / מספר התקפים / אירועים', type:'text' },
            { key:'treatment', label:'טיפול / תרופות / מצב נוכחי', type:'textarea', required:true }
          ]
        },
        {
          key:'musculoskeletal',
          title:'גב / שלד / מפרקים / תאונה / נכות',
          summary:'מאחד בעיות גב, פריצות דיסק, שברים, מניסקוס, רצועות, מחלות ראומטולוגיות, תאונות ומגבלה תפקודית.',
          companies: allCompanies,
          docHints: {
            'איילון':'בשאלון מערכת השלד של איילון יש פירוט לפי אזור, טיפול ומגבלה תפקודית.',
            'הפניקס':'הפניקס שואלת בנפרד על עמוד שדרה, שלד ומחלות ראומטולוגיות.',
            'הכשרה':'בהכשרה יש דגש על תאונה, נכות, בדיקות וניתוחים.'
          },
          fields:[
            { key:'diagnosis', label:'איזו פגיעה / מחלה / בעיה קיימת?', type:'textarea', required:true },
            { key:'location', label:'מיקום בגוף / צד הפגיעה', type:'text', required:true },
            { key:'when', label:'מתי אובחן / קרה?', type:'text', required:true },
            { key:'treatment', label:'טיפול / ניתוח / פיזיותרפיה / זריקות', type:'textarea' },
            { key:'limitation', label:'האם נשארה מגבלה / נכות / כאב כיום?', type:'textarea', required:true }
          ]
        },
        {
          key:'endocrine',
          title:'סוכרת / בלוטות / הורמונים / מטבולי',
          summary:'שאלה מאוחדת לסוכרת, בלוטת התריס, בלוטות נוספות, הורמונים, עודף משקל ומצבים מטבוליים.',
          companies: allCompanies,
          docHints: {
            'איילון':'לאבחנות בבלוטת התריס איילון מבקשת פירוט סוג הבעיה, טיפול ומועד אבחנה.',
            'כלל':'בכלל יש שאלונים ייעודיים למערכות פנימיות ומחלות כרוניות.',
            'הראל':'בשאלוני הראל יש פירוט לפי מערכות רפואיות והמשך שאלות משנה.'
          },
          fields:[
            { key:'diagnosis', label:'איזו אבחנה קיימת?', type:'textarea', required:true },
            { key:'diagnosedAt', label:'מועד אבחנה', type:'text', required:true },
            { key:'treatment', label:'טיפול / אינסולין / תרופות / מעקב', type:'textarea', required:true },
            { key:'complications', label:'סיבוכים / בדיקות / מצב איזון', type:'textarea' }
          ]
        },
        {
          key:'cancer_tumor',
          title:'סרטן / גידול / ממצא חריג / ביופסיה',
          summary:'כולל ממאירות, גידול שפיר, ממצאים בבירור, ביופסיה, פוליפים ומעקב אונקולוגי.',
          companies: allCompanies,
          docHints: {
            'איילון':'בממאירויות איילון דורשת מכתב עדכני מאונקולוג / רופא מטפל.',
            'כלל':'בכלל קיימות שאלות משנה מפורטות על גידולים והיסטוריה משפחתית.',
            'מגדל':'מגדל דורשת פירוט מלא לגבי אבחנה, מועד סיום טיפול ומצב כיום.'
          },
          fields:[
            { key:'finding', label:'איזו מחלה / ממצא / גידול היה או ישנו?', type:'textarea', required:true },
            { key:'diagnosedAt', label:'מועד אבחנה / גילוי', type:'text', required:true },
            { key:'treatment', label:'טיפולים שבוצעו / ניתוח / הקרנות / כימו / מעקב', type:'textarea', required:true },
            { key:'status', label:'מה המצב כיום? האם יש הישנות / מעקב?', type:'textarea', required:true }
          ]
        },
        {
          key:'mental',
          title:'בריאות נפש / קשב / טיפול רגשי',
          summary:'מכסה חרדה, דיכאון, PTSD, OCD, אשפוז פסיכיאטרי, טיפול רגשי, טיפול תרופתי והפרעות קשב.',
          companies: allCompanies,
          fields:[
            { key:'diagnosis', label:'איזו אבחנה / סיבה לטיפול קיימת?', type:'textarea', required:true },
            { key:'diagnosedAt', label:'מועד אבחנה / תחילת טיפול', type:'text', required:true },
            { key:'treatment', label:'טיפול / תרופות / פסיכולוג / פסיכיאטר', type:'textarea', required:true },
            { key:'hospitalization', label:'אשפוז / נכות / ניסיון אובדני / החמרות', type:'textarea' },
            { key:'status', label:'מה המצב כיום?', type:'textarea', required:true }
          ]
        },
        {
          key:'substances',
          title:'עישון / אלכוהול / סמים / קנאביס',
          summary:'שאלה מאוחדת שמכסה שימוש קיים או בעבר, כולל קנאביס רפואי, גמילה ותדירות שימוש.',
          companies: allCompanies,
          docHints: {
            'כלל':'בכלל קיים שאלון ייעודי לסמים וקנאביס עם שאלות משנה.',
            'מנורה':'במנורה יש שאלוני הרחבה נפרדים לאלכוהול ולסמים.',
            'הכשרה':'בהכשרה מופיע שאלון ייעודי בנושא זה.'
          },
          fields:[
            { key:'type', label:'מה סוג העישון / חומר / אלכוהול?', type:'textarea', required:true },
            { key:'frequency', label:'תדירות / כמות', type:'text', required:true },
            { key:'sinceWhen', label:'ממתי / מתי הופסק?', type:'text', required:true },
            { key:'medicalReason', label:'אם מדובר בשימוש רפואי - עבור איזו בעיה?', type:'textarea' },
            { key:'rehab', label:'האם הייתה גמילה / טיפול / מעקב? פרט', type:'textarea' }
          ]
        },
        {
          key:'family_history',
          title:'רקע משפחתי מהותי',
          summary:'שאלה חכמה למחלות משפחתיות מדרגה ראשונה כמו סרטן, לב, סוכרת, כליות, נוירולוגיה.',
          companies:['כלל','הראל','מגדל','מנורה'],
          fields:[
            { key:'conditions', label:'אילו מחלות קיימות במשפחה?', type:'textarea', required:true },
            { key:'who', label:'אצל מי במשפחה ובאיזה גיל בערך?', type:'textarea', required:true },
            { key:'count', label:'כמה קרובי משפחה רלוונטיים קיימים?', type:'text' }
          ]
        },
        {
          key:'pregnancy',
          title:'היריון / בדיקות נשיות רלוונטיות',
          summary:'מוצג רק כאשר המבוטחת הראשית מסומנת כנקבה. נועד לכסות היריון קיים, סיבוכים ובירורים רלוונטיים.',
          companies:['הראל','מגדל','מנורה','הפניקס','כלל','הכשרה','איילון'],
          onlyForFemale:true,
          fields:[
            { key:'details', label:'האם יש היריון / בירור / ממצא רלוונטי? פרט', type:'textarea', required:true },
            { key:'weekOrDate', label:'שבוע היריון / מועד הבדיקה / האירוע', type:'text' },
            { key:'complications', label:'סיבוכים / מעקב / מצב כיום', type:'textarea', required:true }
          ]
        }
      ];
    },

    getRelevantHealthQuestions(context){
      const ctx = context || this.getHealthContext();
      return this.getHealthSchema().filter(q => {
        if(q.onlyForFemale && !ctx.hasFemalePrimary) return false;
        const companyHit = (q.companies || []).some(c => ctx.companies.includes(c));
        return companyHit;
      });
    },

    getHealthDocHints(question, companies){
      const hints = [];
      const map = question?.docHints || {};
      (companies || []).forEach(c => {
        const msg = safeTrim(map[c]);
        if(msg && !hints.includes(msg)) hints.push(msg);
      });
      return hints;
    },
    ...
    renderHealthField(qKey, field, values){
      ...
      const req = field.required ? ' <span style="color:#b42318">*</span>' : '';
      const reqTag = field.required ? '<span class="lcHChip">חובה</span>' : '';
      if(field.type === 'textarea'){
        return `<div class="lcHField" style="grid-column:1 / -1"><label class="lcHLabel">${escapeHtml(field.label)}${req}</label><div class="lcHFieldTags">${reqTag}</div><textarea class="lcHTextarea" rows="3" data-healthf="${escapeHtml(qKey)}.${escapeHtml(field.key)}">${escapeHtml(val)}</textarea></div>`;
      }
      if(field.type === 'select'){
        const opts = (field.options || []).map(opt => `<option value="${escapeHtml(opt)}"${String(val)===String(opt)?' selected':''}>${escapeHtml(opt || 'בחר…')}</option>`).join('');
        return `<div class="lcHField"><label class="lcHLabel">${escapeHtml(field.label)}${req}</label><div class="lcHFieldTags">${reqTag}</div><select class="lcHSelect" data-healthf="${escapeHtml(qKey)}.${escapeHtml(field.key)}">${opts}</select></div>`;
      }
      return `<div class="lcHField"><label class="lcHLabel">${escapeHtml(field.label)}${req}</label><div class="lcHFieldTags">${reqTag}</div><input class="lcHInput" type="text" value="${escapeHtml(val)}" data-healthf="${escapeHtml(qKey)}.${escapeHtml(field.key)}" /></div>`;
    },
    ...
    renderStep8(){
      ...
      const store = this.getHealthStore();
      const questions = this.getRelevantHealthQuestions(context);

      if(!companies.length){
        return `<div class="lcHealthEmpty"><div class="lcHealthEmpty__icon">🩺</div><div class="lcHealthEmpty__title">הצהרת בריאות חכמה</div><div class="lcHealthEmpty__text">כדי להציג את שלב 8 יש לבחור בשלב 5 לפחות פוליסה של אחת מחברות הביטוח. המנוע יזהה אוטומטית את החברות בתיק ויציג שאלון אחד מאוחד.</div></div>`;
      }

      const companyChips = companies.map(c => `<span class="lcHChip lcHChip--top">${escapeHtml(c)}</span>`).join('');
      const productChips = (products.length ? products : ['ללא סוג מוצר']).map(p => `<span class="lcHChip">${escapeHtml(p)}</span>`).join('');

      const cards = questions.map(q => {
        const qCompanies = (q.companies || []).filter(c => companies.includes(c));
        const qState = store.answers[q.key] || { answer:'', fields:{} };
        const companyTags = qCompanies.map(c => `<span class="lcHChip">${escapeHtml(c)}</span>`).join('');
        const hints = this.getHealthDocHints(q, qCompanies);
        const hintsHtml = hints.length ? `<div class="help" style="margin-top:10px">ייתכן שיידרש בהמשך: ${escapeHtml(hints.join(' · '))}</div>` : '';
        const followup = qState.answer === 'yes'
          ? `<div class="lcHCat__body"><div class="lcHFields">${(q.fields || []).map(f => this.renderHealthField(q.key, f, qState.fields || {})).join('')}</div>${hintsHtml}</div>`
          : '';
        return `<div class="lcHCat">
          <div class="lcHCat__head">
            <div>
              <div class="lcHCat__title">${escapeHtml(q.title)}</div>
              <div class="lcHCat__summary">${escapeHtml(q.summary)}</div>
              <div class="lcHCat__tags">${companyTags}</div>
            </div>
            <div class="lcHDecision">
              <button type="button" class="lcHToggle ${qState.answer === 'yes' ? 'is-active' : ''}" data-healthq="${escapeHtml(q.key)}|yes">כן</button>
              <button type="button" class="lcHToggle ${qState.answer === 'no' ? 'is-active' : ''}" data-healthq="${escapeHtml(q.key)}|no">לא</button>
            </div>
          </div>
          ${followup}
        </div>`;
      ...
      return `
        <div class="lcHealthWrap">
          <div class="lcWSection">
            <div class="lcWTitle">הצהרת בריאות אוניברסלית חכמה</div>
            <div class="muted" style="margin-top:6px">זהו שאלון אחד מאוחד לכל החברות שנבחרו בתיק. בכל שאלה לחץ כן / לא. תשובת כן תפתח פירוט מחמיר יותר כדי לכסות את דרישות החיתום של החברות שנבחרו.</div>
            <div class="lcHCompanies">${companyChips}</div>
            <div class="muted small" style="margin-top:10px">מוצרים בתיק:</div>
            <div class="lcHCompanies">${productChips}</div>
          </div>
          ${this.renderUnderwritingSummary(context)}
          ${cards}
        </div>`;
    },
    ...
    bindHealthInputs(){
      ...
      $$('[data-healthq]', this.els.body).forEach(btn => {
        on(btn, 'click', () => {
          ...
          const [qKey, ans] = raw.split('|');
          if(!qKey || !ans) return;
          const prev = store.answers[qKey] || { answer:'', fields:{} };
          store.answers[qKey] = { answer: ans, fields: prev.fields || {} };
          this.render();
        });
      });
      $$('[data-healthf]', this.els.body).forEach(el => {
        const save = () => {
          ...
          const prev = store.answers[qKey] || { answer:'yes', fields:{} };
          prev.answer = 'yes';
          prev.fields = prev.fields || {};
          prev.fields[fieldKey] = safeTrim(el.value);
          store.answers[qKey] = prev;
        };
        on(el, 'input', save);
        on(el, 'change', save);
      });
    ...
    validateStep8(){
      ...
      const companies = context.companies;
      const questions = this.getRelevantHealthQuestions(context);
      if(!companies.length || !questions.length) return { ok:true };

      const store = this.getHealthStore();
      for(const q of questions){
        const qState = store.answers[q.key] || { answer:'', fields:{} };
        if(qState.answer !== 'yes' && qState.answer !== 'no'){
          return { ok:false, msg:`יש להשיב כן / לא על השאלה: ${q.title}` };
        }
        if(qState.answer === 'yes'){
          for(const field of (q.fields || [])){
            if(field.required && !safeTrim(qState.fields?.[field.key])){
              return { ok:false, msg:`חסר פירוט חובה ב-${q.title}: ${field.label}` };
            }
          }
        }
      }
