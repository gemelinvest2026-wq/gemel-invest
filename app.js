...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  function writeSession_(username, remember){
    try{
      if(remember){
        localStorage.setItem(SESSION_LS_KEY, JSON.stringify({ username, ts: Date.now() }));
      }else{
        // session-only: keep in memory via state; clear persisted
  ...
  function setLoggedInUser_(user, remember){
    ...
    hideAuth_();
    render();
    writeSession_(safe.username, remember);
    if(!location.hash) setRoute('customers');
  }

  function tryRestoreSession_(){
    const s = readSession_();
    if(!s || !s.username) return false;
    const users = getUsers_();
    const u = users.find(x => (x.username||'') === s.username && x.active !== false);
    if(!u) return false;
    setLoggedInUser_(u, true);
    return true;
  }
  ...
  (function bootAuth_(){
    ...
    if(form){
      form.addEventListener('submit', (e) => {
        ...
        if(!u || u.active === false){
          fail('שם משתמש לא קיים או מושבת');
          return;
        }
        ...
        // success
        if(err){ err.style.display='none'; err.textContent=''; }
        setLoggedInUser_(u, remember);
      });
    }

    // restore session or show login
    if(!tryRestoreSession_()){
      showAuth_();
    }
  })();
