...
init(){
      ...
      on(this.els.btnOpenCustomerFile, "click", () => {
        this.hideFinishFlow();
        this.close();
        CustomerFile.openFromWizard(this);
      });
...
// ---------- Step 6 ----------
    ...
    finishWizard(){
      ...
      this.lastCompletedCase = this.createCaseSnapshot();
      this.showFinishFlow();
    },
    ...
    createCaseSnapshot(){
      const insureds = (this.insureds || []).map(ins => {
        ...
        return {
          ...
          fullName: safeTrim((d.firstName || "") + " " + (d.lastName || "")),
          idNumber: safeTrim(d.idNumber),
          birthDate: safeTrim(d.birthDate),
          age: this.calcAgeFromDate(d.birthDate),
          ...
          existingPolicies: Array.isArray(d.existingPolicies) ? d.existingPolicies.map(p => ({...p})) : [],
          healthDeclaration: d.healthDeclaration ? JSON.parse(JSON.stringify(d.healthDeclaration)) : { categories:{} }
        };
      });
      ...
      return {
        ...
        payment: {
          payerChoice: safeTrim(this.insureds?.[0]?.data?.payerChoice),
          paymentMethod: safeTrim(this.insureds?.[0]?.data?.paymentMethod),
          externalPayer: JSON.parse(JSON.stringify(this.insureds?.[0]?.data?.externalPayer || {})),
          cc: JSON.parse(JSON.stringify(this.insureds?.[0]?.data?.cc || {})),
          ho: JSON.parse(JSON.stringify(this.insureds?.[0]?.data?.ho || {}))
        }
      };
    },
...
calcAge(dateStr){
  ...
  const CustomerFile = {
    ...
    tabs: [
      ...
      { id:"existing", label:"פוליסות קיימות" },
      { id:"new", label:"פוליסות חדשות" },
    ...
    init(){
      ...
      this.els.edit = $("#customerFileEditBtn");
      this.els.addPolicy = $("#customerFileAddPolicyBtn");
      this.els.sendOps = $("#customerFileSendOpsBtn");
      this.els.download = $("#customerFileDownloadBtn");
      ...
      on(this.els.edit, "click", () => this.toast("עריכת פרטים תחובר בשלב הבא."));
      on(this.els.addPolicy, "click", () => this.toast("הוספת פוליסה מתוך תיק לקוח תחובר בשלב הבא."));
      on(this.els.sendOps, "click", () => {
        if(typeof Wizard !== "undefined" && Wizard.openOperationalReport){
          Wizard.openOperationalReport();
        }else{
          this.toast("דוח תפעולי עדיין לא זמין.");
        }
      });
      on(this.els.download, "click", () => {
        if(typeof Wizard !== "undefined" && Wizard.exportOperationalPdf){
          Wizard.exportOperationalPdf();
        }else{
          this.toast("הורדת סיכום תחובר בשלב הבא.");
        }
      });
    },
    ...
    openFromWizard(wizard){
      const payload = wizard?.lastCompletedCase || wizard?.createCaseSnapshot?.();
      if(!payload) return this.toast("לא נמצאו נתוני לקוח להצגה.");
      this.open(payload);
    },

    open(payload){
      this.current = payload || null;
      this.activeTab = "overview";
      if(!this.current || !this.els.wrap) return;
      this.render();
      this.els.wrap.classList.add("is-open");
      this.els.wrap.setAttribute("aria-hidden","false");
      document.body.style.overflow = "hidden";
    ...
    render(){
      ...
      if(this.els.name) this.els.name.textContent = primary.fullName || "תיק לקוח";
      if(this.els.meta){
        const parts = [
          primary.idNumber ? "ת״ז: " + primary.idNumber : "",
          primary.phone ? "טלפון: " + primary.phone : "",
          primary.email ? "אימייל: " + primary.email : "",
          this.current.ownerName ? "נציג מטפל: " + this.current.ownerName : "",
          this.current.createdAt ? "נפתח: " + this.formatDate(this.current.createdAt) : ""
        ].filter(Boolean);
        this.els.meta.innerHTML = parts.map(x => `<span>${escapeHtml(x)}</span>`).join("");
      }
      if(this.els.dash){
        this.els.dash.innerHTML = `
          <div class="dashCard">
            <div class="dashCard__k">מבוטחים בתיק</div>
            <div class="dashCard__v">${summary.insuredCount}</div>
            <div class="dashCard__sub">ראשי + בני משפחה משויכים</div>
          </div>
          <div class="dashCard">
            <div class="dashCard__k">פוליסות בתיק</div>
            <div class="dashCard__v">${summary.policyCount}</div>
            <div class="dashCard__sub">${summary.existingCount} קיימות · ${summary.newCount} חדשות</div>
          </div>
          <div class="dashCard">
            <div class="dashCard__k">פרמיה חודשית</div>
            <div class="dashCard__v">${this.money(summary.monthlyPremium)}</div>
            <div class="dashCard__sub">סה״כ פרמיות מכלל הפוליסות</div>
          </div>`;
      }
      this.renderTabs();
      this.renderBody();
    ...
    renderTabs(){
      if(!this.els.tabs) return;
      this.els.tabs.innerHTML = this.tabs.map(tab => `
        <button class="customerTab ${this.activeTab === tab.id ? "is-active" : ""}" data-cftab="${tab.id}" type="button">${escapeHtml(tab.label)}</button>
      `).join("");
      $$("[data-cftab]", this.els.tabs).forEach(btn => {
        on(btn, "click", () => {
          this.activeTab = btn.getAttribute("data-cftab") || "overview";
          this.renderTabs();
          this.renderBody();
        });
      });
    ...
    renderBody(){
      if(!this.els.body || !this.current) return;
      if(this.activeTab === "overview") this.els.body.innerHTML = this.renderOverview();
      if(this.activeTab === "insureds") this.els.body.innerHTML = this.renderInsureds();
      if(this.activeTab === "existing") this.els.body.innerHTML = this.renderPolicies("existing");
      if(this.activeTab === "new") this.els.body.innerHTML = this.renderPolicies("new");
      if(this.activeTab === "docs") this.els.body.innerHTML = this.renderDocs();
      if(this.activeTab === "history") this.els.body.innerHTML = this.renderHistory();
    },

    getSummary(){
      const insureds = Array.isArray(this.current?.insureds) ? this.current.insureds : [];
      const existingPolicies = insureds.flatMap(ins => Array.isArray(ins.existingPolicies) ? ins.existingPolicies.map(p => ({...p, insuredName: ins.fullName || ins.label || "מבוטח"})) : []);
      const newPolicies = Array.isArray(this.current?.newPolicies) ? this.current.newPolicies.map(p => ({...p})) : [];
      const monthlyPremium = existingPolicies.reduce((s,p) => s + this.num(p.monthlyPremium), 0) + newPolicies.reduce((s,p) => s + this.num(p.premiumMonthly), 0);
      return {
        insuredCount: insureds.length,
        existingCount: existingPolicies.length,
        newCount: newPolicies.length,
        policyCount: existingPolicies.length + newPolicies.length,
        monthlyPremium,
        existingPolicies,
        newPolicies
      };
    ...
    getInsuredNameById(id){
      const found = (this.current?.insureds || []).find(ins => ins.id === id);
      return found?.fullName || found?.label || "מבוטח";
    },

    coverageMap(){
      const summary = this.getSummary();
      const types = [
        { key:"health", label:"בריאות", match:["בריאות","health","כיסויים"] },
        { key:"critical", label:"מחלות קשות", match:["מחלות קשות","סרטן"] },
        { key:"risk", label:"ריסק", match:["ריסק"] },
        { key:"mortgage", label:"ריסק משכנתא", match:["משכנתא"] },
        { key:"disability", label:"אובדן כושר עבודה", match:["אכ\"ע","אובדן כושר"] },
        { key:"nursing", label:"סיעודי", match:["סיעוד","סיעודי"] }
      ];
      const all = [
        ...summary.existingPolicies.map(p => String(p.type || "")),
        ...summary.newPolicies.map(p => String(p.type || ""))
      ].map(x => x.toLowerCase());

      return types.map(item => {
        const hit = item.match.some(term => all.some(val => val.includes(String(term).toLowerCase())));
        return {
          ...item,
          status: hit ? "קיים בתיק" : "טרם הוזן בתיק",
          cls: hit ? "is-ok" : "is-missing"
        };
      });
    },
    ...
    renderOverview(){
      const primary = this.current.primary || {};
      const summary = this.getSummary();
      const insuredPremiumRows = (this.current.insureds || []).map(ins => {
        const existing = (ins.existingPolicies || []).reduce((s,p) => s + this.num(p.monthlyPremium), 0);
        const nw = summary.newPolicies
          .filter(p => p.insuredMode === "couple" ? (ins.type === "primary" || ins.type === "spouse") : p.insuredId === ins.id)
          .reduce((s,p) => s + this.num(p.premiumMonthly), 0);
        return `
          <div class="cfSummaryPill">
            <div>
              <div class="cfSummaryPill__k">${escapeHtml(ins.fullName || this.roleLabel(ins.type, ins.label))}</div>
              <div class="cfSummaryPill__v">${this.money(existing + nw)}</div>
            </div>
            <span class="cfStatBadge">${escapeHtml(this.roleLabel(ins.type, ins.label))}</span>
          </div>`;
      }).join("");

      return `
        <div class="cfCard cfHeaderPremium">
          <div class="cfHeaderPremium__head">
            <div>
              <div class="cfHeaderPremium__title">סקירת תיק 360°</div>
              <div class="cfHeaderPremium__sub">תמונת מצב מרוכזת של הלקוח, המבוטחים והפוליסות שנבנו בהקמה.</div>
            </div>
            <div class="cfHeaderPremium__chips">
              <div class="cfChip"><span class="cfChip__icon">👤</span><span class="cfChip__val">${escapeHtml(primary.fullName || "—")}</span></div>
              <div class="cfChip cfChip--phone"><span class="cfChip__icon">📞</span><span class="cfChip__val">${escapeHtml(primary.phone || "—")}</span></div>
              <div class="cfChip cfChip--email"><span class="cfChip__icon">✉️</span><span class="cfChip__val">${escapeHtml(primary.email || "—")}</span></div>
              <div class="cfChip"><span class="cfChip__icon">🗂️</span><span class="cfChip__val">${this.money(summary.monthlyPremium)}</span></div>
            </div>
          </div>
        </div>

        <div class="cfSummaryBar">
          <div class="cfSummaryPill"><div><div class="cfSummaryPill__k">סטטוס תיק</div><div class="cfSummaryPill__v">פעיל</div></div><span class="cfStatBadge">Premium CRM</span></div>
          <div class="cfSummaryPill"><div><div class="cfSummaryPill__k">מבוטחים</div><div class="cfSummaryPill__v">${summary.insuredCount}</div></div><span class="cfStatBadge">משויכים</span></div>
          <div class="cfSummaryPill"><div><div class="cfSummaryPill__k">פוליסות</div><div class="cfSummaryPill__v">${summary.policyCount}</div></div><span class="cfStatBadge">${summary.existingCount} קיימות · ${summary.newCount} חדשות</span></div>
          <div class="cfSummaryPill"><div><div class="cfSummaryPill__k">נפתח בתאריך</div><div class="cfSummaryPill__v">${escapeHtml(this.formatDate(this.current.createdAt))}</div></div><span class="cfStatBadge">${escapeHtml(this.current.ownerName || "ללא נציג")}</span></div>
        </div>

        <div class="cfSectionGrid">
          <div class="cfCards2">
            <div class="cfCard">
              <div class="cfCard__head"><div class="cfCard__title">פרטי לקוח</div><div class="cfCard__hint">נתוני הליבה של המבוטח הראשי</div></div>
              <div class="cfCard__body cfGroupRows">
                ${this.infoRow("שם מלא", primary.fullName || "—")}
                ${this.infoRow("ת״ז", primary.idNumber || "—")}
                ${this.infoRow("תאריך לידה", primary.birthDate || "—")}
                ${this.infoRow("גיל", primary.age || "—")}
                ${this.infoRow("טלפון", primary.phone || "—")}
                ${this.infoRow("אימייל", primary.email || "—")}
                ${this.infoRow("עיר", primary.city || "—")}
                ${this.infoRow("כתובת", [primary.street, primary.houseNumber].filter(Boolean).join(" ") || "—")}
                ${this.infoRow("קופת חולים", primary.clinic || "—")}
                ${this.infoRow("שב״ן", primary.shaban || "—")}
                ${this.infoRow("עיסוק", primary.occupation || "—")}
              </div>
            </div>

            <div class="cfCard">
              <div class="cfCard__head"><div class="cfCard__title">מפת ביטוחים</div><div class="cfCard__hint">צילום מצב מהיר של הכיסויים בתיק</div></div>
              <div class="cfCard__body">
                <div class="cfCoverageList">
                  ${this.coverageMap().map(item => `
                    <div class="cfCoverageRow ${item.cls}">
                      <div>
                        <div class="cfCoverageName">${escapeHtml(item.label)}</div>
                        <div class="cfCoverageStatus">${escapeHtml(item.status)}</div>
                      </div>
                      <span class="cfStatBadge">${item.cls === "is-ok" ? "קיים" : item.cls === "is-partial" ? "חלקי" : "חסר"}</span>
                      <span class="cfCoverageDot" aria-hidden="true"></span>
                    </div>`).join("")}
                </div>
              </div>
            </div>
            ...
            <div class="cfCard">
              <div class="cfCard__head"><div class="cfCard__title">סיכום פרמיות לפי מבוטח</div><div class="cfCard__hint">מציג חיבור של קיימות וחדשות</div></div>
              <div class="cfCard__body">
                <div class="cfSummaryBar">${insuredPremiumRows || `<div class="cfEmpty">עדיין אין פרמיות להצגה.</div>`}</div>
              </div>
            </div>

            <div class="cfCard">
              <div class="cfCard__head"><div class="cfCard__title">פעולות מהירות</div><div class="cfCard__hint">קפיצה מהירה לפעולות המרכזיות</div></div>
              <div class="cfCard__body">
                <button class="dashCard dashCard--action" type="button" onclick="document.getElementById('customerFileAddPolicyBtn')?.click()">➕ הוסף פוליסה</button>
                <button class="dashCard dashCard--action" type="button" onclick="document.getElementById('customerFileSendOpsBtn')?.click()">📤 שלח לתפעול</button>
                <button class="dashCard dashCard--action" type="button" onclick="document.getElementById('customerFileDownloadBtn')?.click()">📄 הורד סיכום</button>
              </div>
            </div>
          </div>

          <div class="cfCard">
            <div class="cfCard__head"><div class="cfCard__title">ציר פעילות</div><div class="cfCard__hint">מה קרה בתיק עד כה</div></div>
            <div class="cfCard__body">
              <div class="cfTimeline">
                ${this.historyItems().slice(0,5).map(item => `
                  <div class="cfTimelineItem">
                    <span class="cfTimelineDot" aria-hidden="true"></span>
                    <div>
                      <div class="cfTimelineTitle">${escapeHtml(item.title)}</div>
                      <div class="cfTimelineSub">${escapeHtml(item.sub)}</div>
                    </div>
                  </div>`).join("")}
              </div>
            </div>
    ...
    infoRow(k, v){
      return `<div class="cfRow"><div class="cfRow__k">${escapeHtml(k)}</div><div class="cfRow__v">${escapeHtml(String(v || "—"))}</div></div>`;
    },

    renderInsureds(){
      const insureds = Array.isArray(this.current?.insureds) ? this.current.insureds : [];
      if(!insureds.length) return `<div class="cfEmpty">לא נמצאו מבוטחים בתיק.</div>`;
      return `<div class="cfInsGrid">
        ${insureds.map(ins => `
          <div class="cfInsCard">
            <div class="cfInsCard__top">
              <div>
                <div class="cfInsCard__name">${escapeHtml(ins.fullName || "—")}</div>
                <div class="cfInsCard__role">${escapeHtml(this.roleLabel(ins.type, ins.label))}</div>
              </div>
              <span class="cfStatBadge">${ins.age ? "גיל " + escapeHtml(String(ins.age)) : "ללא גיל"}</span>
            </div>
            <div class="cfInsCard__meta">
              <div class="cfMetaLine"><b>ת״ז</b><span>${escapeHtml(ins.idNumber || "—")}</span></div>
              <div class="cfMetaLine"><b>טלפון</b><span>${escapeHtml(ins.phone || "—")}</span></div>
              <div class="cfMetaLine"><b>אימייל</b><span>${escapeHtml(ins.email || "—")}</span></div>
              <div class="cfMetaLine"><b>קופת חולים</b><span>${escapeHtml(ins.clinic || "—")}</span></div>
              <div class="cfMetaLine"><b>שב״ן</b><span>${escapeHtml(ins.shaban || "—")}</span></div>
              <div class="cfMetaLine"><b>עיסוק</b><span>${escapeHtml(ins.occupation || "—")}</span></div>
              <div class="cfMetaLine"><b>פוליסות קיימות</b><span>${(ins.existingPolicies || []).length}</span></div>
            </div>
          </div>`).join("")}
      </div>`;
    },
    ...
    renderPolicies(kind){
      const summary = this.getSummary();
      const rows = kind === "existing"
        ? summary.existingPolicies.map(p => ({
            company: p.company || "",
            type: p.type || "",
        ...
        : summary.newPolicies.map(p => ({
            company: p.company || "",
            type: p.type || "",
            insuredName: p.insuredMode === "couple" ? "ראשי + בן/בת זוג" : this.getInsuredNameById(p.insuredId),
            premium: p.premiumMonthly || "",
            startDate: p.startDate || "",
            status: "חדש"
          }));

      if(!rows.length){
        return `<div class="cfEmpty">אין עדיין ${kind === "existing" ? "פוליסות קיימות" : "פוליסות חדשות"} בתיק.</div>`;
      }

      return `
        <div class="cfCard">
          <div class="cfCard__head">
            <div class="cfCard__title">${kind === "existing" ? "פוליסות קיימות" : "פוליסות חדשות"}</div>
            <div class="cfCard__hint">${rows.length} רשומות בתיק</div>
          </div>
          <div class="cfCard__body cfPolicyTableWrap">
            <table class="cfPolicyTable">
              <thead>
                <tr>
                  <th>חברה</th>
                  <th>מוצר</th>
                  <th>מבוטח</th>
                  <th>פרמיה</th>
                  <th>${kind === "existing" ? "סיווג" : "תאריך התחלה"}</th>
                  <th>סטטוס</th>
                </tr>
              </thead>
              <tbody>
                ${rows.map(row => `
                  <tr>
                    ...
                    <td>${escapeHtml(row.insuredName || "—")}</td>
                    <td>${row.premium ? escapeHtml(this.money(row.premium)) : "—"}</td>
                    <td>${escapeHtml(row.startDate || "—")}</td>
                    <td><span class="cfPolicyStatus">${escapeHtml(row.status || "—")}</span></td>
                  </tr>`).join("")}
              </tbody>
            </table>
          </div>
    ...
    renderDocs(){
      const docs = [
        { title:"דוח תפעולי", sub:"מוכן להורדה מתוך מסך הסיום או מהתיק עצמו", action:"הורד" },
        { title:"טופס הצטרפות", sub:"Placeholder לשלב הבא של העלאת מסמכים", action:"בקרוב" },
        { title:"הצהרת בריאות", sub:"נשמרה בהקמת הלקוח ותשולב למסמכים בהמשך", action:"קיים" }
      ];
      return `<div class="cfCard">
        <div class="cfCard__head"><div class="cfCard__title">מסמכים</div><div class="cfCard__hint">שלד פרימיום מוכן לחיבור העלאות בהמשך</div></div>
        <div class="cfCard__body">
          <div class="cfDocList">
            ${docs.map(doc => `
              <div class="cfDocRow">
                <div class="cfDocMeta">
                  <div class="cfDocTitle">${escapeHtml(doc.title)}</div>
                  <div class="cfDocSub">${escapeHtml(doc.sub)}</div>
                </div>
                <span class="cfStatBadge">${escapeHtml(doc.action)}</span>
              </div>`).join("")}
          </div>
        </div>
      </div>`;
    ...
    historyItems(){
      const summary = this.getSummary();
      const items = [
        { title:"הלקוח הוקם בהצלחה", sub:this.formatDate(this.current?.createdAt) + " · מתוך שלב 8 הצהרת בריאות" },
        { title:"נבנה תיק לקוח Premium", sub:"נפתח דרך כפתור 'פתח תיק לקוח' במסך הסיום" }
      ];
      if(summary.existingCount) items.push({ title:"נקלטו פוליסות קיימות", sub: summary.existingCount + " פוליסות קיימות שויכו למבוטחים בתיק" });
      if(summary.newCount) items.push({ title:"נוספו פוליסות חדשות", sub: summary.newCount + " פוליסות חדשות נשמרו בהקמה" });
      if(this.current?.ownerName) items.push({ title:"נציג מטפל שויך לתיק", sub: this.current.ownerName });
      return items;
    },
    ...
    renderHistory(){
      ...
      return `<div class="cfCard">
        <div class="cfCard__head"><div class="cfCard__title">היסטוריית תיק</div><div class="cfCard__hint">ציר פעולות ראשוני של התיק</div></div>
        <div class="cfCard__body">
          <div class="cfTimeline">
            ${items.map(item => `
              <div class="cfTimelineItem">
