...
// ---------- Step 6 ----------
        ...
        <div class="lcHMain">
          ...
          <div class="lcHQuestionCard">
            <div class="lcHQuestionCard__head">
              <div>
                <div class="lcHQuestionCard__eyebrow">הצהרה אחת משותפת לכל המבוטחים</div>
                <div class="lcHQuestionCard__title">${escapeHtml(q.text)}</div>
                <div class="lcHQuestionCard__sub">אפשר לסמן כן רק למבוטח הרלוונטי. שאר המבוטחים יסומנו אוטומטית כ"לא" אם טרם נענו.</div>
              </div>
            ...
            <div class="lcHQuickRow">
              <button type="button" class="btn" data-hbulk="${escapeHtml(q.key)}|all-no">לא לכל המבוטחים</button>
              <button type="button" class="btn" data-hbulk="${escapeHtml(q.key)}|clear">נקה תשובות</button>
            </div>
    ...
    bindHealthInputs(){
      ...
      $$('[data-hans]', this.els.body).forEach(btn => {
        on(btn, 'click', () => {
          ...
          if(ans === 'no'){
            ...
            this.insureds.forEach(other => {
              if(safeTrim(other?.id) === safeTrim(insId)) return;
              const otherResp = this.getHealthResponse(qKey, other.id);
              if(otherResp.answer !== 'yes' && otherResp.answer !== 'no'){
                this.setHealthResponse(qKey, other.id, { answer:'no', fields:{}, saved:false });
              }
            });
          }
          this.render();
      ...
      $$('[data-hbulk]', this.els.body).forEach(btn => {
        on(btn, 'click', () => {
          const [qKey, action] = String(btn.getAttribute('data-hbulk') || '').split('|');
          if(!qKey || !action) return;
          if(action === 'all-no'){
            this.insureds.forEach(ins => {
              this.setHealthResponse(qKey, ins.id, { answer:'no', fields:{}, saved:false });
            });
            this.setHint('סומן "לא" לכל המבוטחים בשאלה זו');
          } else if(action === 'clear'){
            this.insureds.forEach(ins => {
              this.setHealthResponse(qKey, ins.id, { answer:'', fields:{}, saved:false });
            });
            this.setHint('נוקו התשובות בשאלה זו');
          }
          this.render();
        });
      });
