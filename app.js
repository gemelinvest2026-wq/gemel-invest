...
(() => {
  ...
  const BUILD = "20260329-mfa-first-login-tight-v11";
...
const SystemRepairUI = {
  ...
  const SupabaseMFA = {
    ...
    extractQrMarkup(data){
      ...
      if(!qr) return '<div class="muted">לא התקבל QR מהרישום.</div>';
      if(qr.startsWith('data:')) return `<img alt="QR" src="${qr}"/>`;
      const svgText = String(qr);
      const svgBase64 = (typeof btoa === 'function')
        ? btoa(unescape(encodeURIComponent(svgText)))
        : '';
      const src = svgBase64 ? `data:image/svg+xml;base64,${svgBase64}` : ('data:image/svg+xml;utf8,' + encodeURIComponent(svgText));
      return `<img alt="QR" src="${src}"/>`;
    }
  };
  ...
  UsersUI._saveFromModal = async function(){
    ...
    if(this._modalMode === 'edit' && safeTrim(E.id?.value)) setAgentSecurity(safeTrim(E.id.value), { authEmail });
    await _saveFromModal();
    if(this._modalMode !== 'edit'){
      const arr = Array.isArray(State.data.agents) ? State.data.agents : [];
      const last = arr[arr.length-1];
      if(last?.id) { setAgentSecurity(last.id, { authEmail }); await App.persist('עודכן auth email'); }
  ...
  Auth._showMfaStep = function(agent, factorId, options = {}){
    ...
    this._pendingMfa = { agent, factorId, mode:isSetup ? 'setup' : 'verify' };
    ...
    const titleEl = $('#lcLoginMfaTitle'); if(titleEl) titleEl.textContent = 'אימות דו־שלבי';
    const hintEl = $('#lcLoginMfaHint');
    if(hintEl) hintEl.textContent = isSetup
    ...
    if(setupBox) setupBox.hidden = !isSetup;
    if(qrBox) qrBox.innerHTML = isSetup ? SupabaseMFA.extractQrMarkup(options.enrollData || {}) : '';
  ...
  Auth._hideMfaStep = function(){
    ...
    const setupBox = $('#lcLoginMfaSetupBox'); if(setupBox) setupBox.hidden = true;
    const qrBox = $('#lcLoginMfaQrBox'); if(qrBox) qrBox.innerHTML = '';
    const codeEl = $('#lcLoginMfaCode'); if(codeEl) codeEl.value = '';
  };
  Auth._verifyPendingMfa = async function(){
    ...
    setAgentSecurity(pending.agent.id, { mfaEnabled:true, factorId:pending.factorId, lastVerifiedAt:nowISO(), mfaEnrolledAt:getAgentSecurity(pending.agent.id).mfaEnrolledAt || nowISO() });
    await App.persist('אומת MFA');
    this._hideMfaStep();
  ...
  Auth._submit = async function(){
    ...
    if(sec.authEmail){
      ...
      if(sr.ok) authSigned = true;
      else if(pin !== expected) return this._setError('סיסמת Auth שגויה או PIN שגוי');
    } else if(pin !== expected) {
      return this._setError('קוד כניסה שגוי');
    }
    if(authSigned){
      const factor = await SupabaseMFA.getVerifiedTotpFactor();
      if(factor.ok && factor.factor){
        setAgentSecurity(matched.id, { authEmail:sec.authEmail, mfaEnabled:true, factorId:safeTrim(factor.factor?.id), mfaEnrolledAt:getAgentSecurity(matched.id).mfaEnrolledAt || nowISO() });
        await App.persist('MFA login pending');
        return this._showMfaStep(matched, safeTrim(factor.factor?.id));
      }
      const enrolled = await SupabaseMFA.enroll();
      if(!enrolled.ok) return this._setError(enrolled.error || 'לא הצלחתי להתחיל רישום ל-Google Authenticator');
      const factorId = safeTrim(enrolled.data?.id || enrolled.data?.factorId || enrolled.data?.totp?.id);
      if(!factorId) return this._setError('לא התקבל מזהה רישום עבור Google Authenticator');
      setAgentSecurity(matched.id, {
        authEmail:sec.authEmail,
        mfaEnabled:false,
        factorId,
        mfaEnrolledAt:getAgentSecurity(matched.id).mfaEnrolledAt || nowISO(),
        lastVerifiedAt:''
      });
      await App.persist('MFA enrollment started');
      return this._showMfaStep(matched, factorId, { mode:'setup', enrollData:enrolled.data });
    }
  ...
  const SecurityUI = {
    ...
    async verifyEnrollment(){
      ...
      setAgentSecurity(agent.id,{ mfaEnabled:true, factorId:this.currentFactorId, mfaEnrolledAt:getAgentSecurity(agent.id).mfaEnrolledAt || nowISO(), lastVerifiedAt:nowISO() });
    ...
    async disableCurrentFactor(){
      ...
      setAgentSecurity(agent.id,{ mfaEnabled:false, factorId:'', mfaEnrolledAt:'', lastVerifiedAt:'' }); await App.persist('MFA removed');
