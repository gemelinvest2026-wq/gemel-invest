...
calcAge(dateStr){
  ...
  const MirrorsUI = {
    ...
    renderVerification(rec){
      ...
      const primary = this.getPrimary(rec);
      const fields = [
        ['שם מלא', rec.fullName || `${safeTrim(primary.firstName)} ${safeTrim(primary.lastName)}`.trim()],
        ['תעודת זהות', rec.idNumber || primary.idNumber],
        ['טלפון', rec.phone || primary.phone],
        ['אימייל', rec.email || primary.email],
        ['עיר', rec.city || primary.city],
        ['כתובת', primary.address],
        ['תאריך לידה', primary.birthDate],
        ['עיסוק', primary.occupation],
        ['סטטוס משפחתי', primary.familyStatus],
        ['קופת חולים', primary.hmo || primary.kupatHolim],
        ['שם הנציג שמכר', rec.agentName],
        ['חברות שנבחרו בפוליסות חדשות', this.getCompanyNames(rec).join(', ')]
      ...
      this.els.verifyCard.innerHTML = `<div class="mirrorsCard__head">
          <div>
            ...
            <div class="mirrorsCard__hint">המערכת שלפה את הנתונים שנשמרו בהקמת הלקוח, כדי שנציג התפעול יאמת אותם מול הלקוח.</div>
          </div>
          <span class="mirrorsVerifyBadge">אימות פעיל</span>
        </div>
        <div class="mirrorsVerifyGrid">${fields.map(([label, value]) => `<div class="mirrorsInfoCard"><span>${escapeHtml(label)}</span><strong>${escapeHtml(value || '—')}</strong></div>`).join('')}</div>`;
    },
