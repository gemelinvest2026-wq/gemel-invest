...
`;
  ...
  function renderSummaryStep(){
    ...
    if(markDoneBtnEl) markDoneBtnEl.addEventListener('click', () => {
      p.status = 'נסגר';
      // Save proposal as history on the customer (so TalkMe can pull policies reliably)
      const updatedCustomer = appendProposalToCustomerHistory_(p);
      if(updatedCustomer && GOOGLE_SCRIPT_URL){
        syncUpCustomer(updatedCustomer).then(()=>toast('שמירה', 'ההצעה נשמרה ללקוח (היסטוריה)')).catch(()=>toast('שמירה', 'שגיאה בשמירת היסטוריה'));
      }
...
function debounce(fn, wait){
  ...
  // ---------- Proposal History (per-customer) ----------
  const CUSTOMER_CACHE_KEY = 'gemel_customers_cache_v1';

  function cacheCustomersLocally_(){
    try{ localStorage.setItem(CUSTOMER_CACHE_KEY, JSON.stringify(state.customers||[])); }catch(_){}
  }

  function appendProposalToCustomerHistory_(proposal){
    if(!proposal) return null;
    const cid = proposal.customerId || (proposal.customer && proposal.customer.id) || null;
    if(!cid) return null;

    let customer = state.customers.find(c => c.id === cid) || null;
    if(!customer){
      // create a customer shell if missing
      const snap = proposal.customer || {};
      customer = { id: cid, fullName: snap.fullName || proposal.customerName || '', tz: snap.tz || '', email: snap.email || '', phone: snap.phone || '', status: snap.status || 'פעיל' };
      state.customers.unshift(customer);
    }

    if(!Array.isArray(customer.proposalsHistory)) customer.proposalsHistory = [];

    // create a compact snapshot (keep what TalkMe needs)
    const snap = {
      proposalId: proposal.id,
      savedAt: new Date().toISOString(),
      status: proposal.status || '',
      internalStatus: proposal.internalStatus || '',
      assignedAgent: proposal.assignedAgent || customer.assignedAgent || '',
      oldPolicies: Array.isArray(proposal.oldPolicies) ? proposal.oldPolicies : [],
      newPolicies: Array.isArray(proposal.newPolicies) ? proposal.newPolicies : [],
      insuredList: Array.isArray(proposal.insuredList) ? proposal.insuredList : [],
      medical: proposal.medical || {},
      payment: proposal.payment || proposal.payer || null
    };

    customer.proposalsHistory.push(snap);
    customer.lastProposalId = snap.proposalId;
    customer.lastProposalSavedAt = snap.savedAt;

    // Also reflect latest snapshot at top-level for convenience (non-breaking)
    customer.oldPolicies = snap.oldPolicies;
    customer.newPolicies = snap.newPolicies;

    cacheCustomersLocally_();
    return customer;
  }
